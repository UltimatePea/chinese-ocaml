...
let create_test_database () =
  ...
  let hua_group = create_rhyme_group_data HuaRhyme 
    [List.nth test_items 0; List.nth test_items 1; List.nth test_items 2] 
    [("description", "花韵组测试数据")] in
  
  let an_group = create_rhyme_group_data AnRhyme
    [List.nth test_items 3; List.nth test_items 4; List.nth test_items 5]
    [("description", "安韵组测试数据")] in
    
  let feng_group = create_rhyme_group_data FengRhyme
    [List.nth test_items 6; List.nth test_items 7; List.nth test_items 8]
    [("description", "风韵组测试数据")] in
    
  let yue_group = create_rhyme_group_data YueRhyme
    [List.nth test_items 9; List.nth test_items 10; List.nth test_items 11]
    [("description", "月韵组测试数据")] in
...
let run_tests () =
  ...
  try
    ...
    let (complete_analysis, updated_engine) = analyze_poetry_complete test_verses_qiyan_jueju engine_with_data in
    ...
    let poem_batches = [test_verses_qiyan_jueju; [test_single_verse]; test_verses_wuyan_lushi] in
