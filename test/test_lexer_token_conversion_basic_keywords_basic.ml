...
let coverage_tests = [ test_case "可用关键字覆盖测试" `Quick test_available_keywords ]

(** 主测试运行器 *)
let () =
  run "Lexer_token_conversion_basic_keywords 基础测试"
    [ ("核心转换功能", core_conversion_tests); ("稳定性测试", stability_tests); ("覆盖率测试", coverage_tests) ]
