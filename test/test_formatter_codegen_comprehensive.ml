(** 代码生成格式化器全面测试套件
    测试覆盖formatter_codegen.ml模块的所有核心功能 *)
...
module TestHelpers = struct
  ...
  (** 验证生成的代码不为空 *)
  let assert_not_empty code =
    String.length (String.trim code) > 0
...
let test_ccodegen_function_calls () =
  ...
  check bool "函数调用应包含函数名" true
    (TestHelpers.contains_substring result "printf");
  check bool "函数调用应包含参数" true
    (TestHelpers.contains_substring result "Hello");
  check bool "函数调用应包含括号" true
    (TestHelpers.contains_substring result "(");
  ...
  check bool "一元函数调用应包含一个参数" true
    (TestHelpers.contains_substring unary_result "value")
...
let test_ccodegen_luoyan_calls () =
  ...
  check bool "骆言调用应包含参数数量" true
    (TestHelpers.contains_substring luoyan_call_result "2");

  (* 测试骆言变量绑定 *)
  let bind_result = CCodegen.luoyan_bind_var "x" "42" in
  check bool "骆言变量绑定应包含变量名" true
    (TestHelpers.contains_substring bind_result "x");
  check bool "骆言变量绑定应包含值" true
    (TestHelpers.contains_substring bind_result "42")
...
let test_ccodegen_luoyan_literals () =
  ...
  check bool "骆言字符串应包含内容" true
    (TestHelpers.contains_substring str_result "测试");

  (* 测试骆言整数 *)
  let int_result = CCodegen.luoyan_int 42 in
  check bool "骆言整数应包含luoyan_int" true
    (TestHelpers.contains_substring int_result "luoyan_int");
  check bool "骆言整数应包含数值" true
    (TestHelpers.contains_substring int_result "42");
  ...
  check bool "骆言布尔值true应包含true" true
    (TestHelpers.contains_substring bool_true_result "true");
  check bool "骆言布尔值false应包含false" true
    (TestHelpers.contains_substring bool_false_result "false");

  (* 测试骆言单元值 *)
  let unit_result = CCodegen.luoyan_unit () in
  check bool "骆言单元值应包含luoyan_unit" true
    (TestHelpers.contains_substring unit_result "luoyan_unit")

let test_ccodegen_luoyan_advanced () =
  (* 测试骆言let表达式 *)
  let let_result = CCodegen.luoyan_let "x" "42" "body_code" in
  check bool "骆言let应包含变量名" true
    (TestHelpers.contains_substring let_result "x");
  check bool "骆言let应包含值代码" true
    (TestHelpers.contains_substring let_result "42");

  (* 测试骆言函数创建 *)
  let func_create_result = CCodegen.luoyan_function_create "test_func" "param" in
  check bool "骆言函数创建应包含函数名" true
    (TestHelpers.contains_substring func_create_result "test_func");

  (* 测试骆言模式匹配 *)
  let pattern_match_result = CCodegen.luoyan_pattern_match "expr_var" in
  check bool "骆言模式匹配应包含表达式变量" true
    (TestHelpers.contains_substring pattern_match_result "expr_var")
...
let test_ccodegen_exception_handling () =
  ...
  check bool "骆言try-catch应包含try代码" true
    (TestHelpers.contains_substring try_catch_result "try_code");
...
let test_ccodegen_c_structures () =
  ...
  check bool "C语句应以分号结尾" true
    (TestHelpers.ends_with stmt_result ";");
  ...
  check bool "C语句块应包含所有语句" true
    (TestHelpers.contains_substring block_result "int x = 42;");

  (* 测试C变量声明 *)
  let var_decl_result = CCodegen.c_variable_declaration "int" "x" "42" in
  check bool "C变量声明应包含类型" true
    (TestHelpers.contains_substring var_decl_result "int");
  check bool "C变量声明应包含变量名" true
    (TestHelpers.contains_substring var_decl_result "x")

let test_ccodegen_control_flow () =
  (* 测试C if语句 *)
  let if_result = CCodegen.c_if_statement "condition" "then_block" in
  check bool "C if语句应包含条件" true
    (TestHelpers.contains_substring if_result "condition");
  check bool "C if语句应包含then块" true
    (TestHelpers.contains_substring if_result "then_block");

  (* 测试C if-else语句 *)
  let if_else_result = CCodegen.c_if_else_statement "condition" "then_block" "else_block" in
  check bool "C if-else语句应包含else" true
    (TestHelpers.contains_substring if_else_result "else");

  (* 测试C while循环 *)
  let while_result = CCodegen.c_while_loop "condition" "body" in
  check bool "C while循环应包含while关键字" true
    (TestHelpers.contains_substring while_result "while");

  (* 测试C for循环 *)
  let for_result = CCodegen.c_for_loop "init" "condition" "increment" "body" in
  check bool "C for循环应包含for关键字" true
    (TestHelpers.contains_substring for_result "for");
  check bool "C for循环应包含初始化" true
    (TestHelpers.contains_substring for_result "init")

let test_ccodegen_function_definitions () =
  (* 测试C函数声明 *)
  let func_decl_result = CCodegen.c_function_declaration "int" "add" ["int a"; "int b"] in
  check bool "C函数声明应包含返回类型" true
    (TestHelpers.contains_substring func_decl_result "int");
  check bool "C函数声明应包含函数名" true
    (TestHelpers.contains_substring func_decl_result "add");

  (* 测试C函数定义 *)
  let func_def_result = CCodegen.c_function_definition "int" "add" ["int a"; "int b"] "return a + b;" in
  check bool "C函数定义应包含函数体" true
    (TestHelpers.contains_substring func_def_result "return a + b;")
...
let test_ccodegen_struct_definitions () =
  ...
  check bool "C结构体应包含typedef" true
    (TestHelpers.contains_substring struct_result "typedef");
  check bool "C结构体应包含结构体名" true
    (TestHelpers.contains_substring struct_result "Point");

  (* 测试C枚举定义 *)
  let enum_result = CCodegen.c_enum_definition "Color" ["RED"; "GREEN"; "BLUE"] in
  check bool "C枚举应包含enum关键字" true
    (TestHelpers.contains_substring enum_result "enum");
  check bool "C枚举应包含枚举值" true
    (TestHelpers.contains_substring enum_result "RED")
...
let test_enhanced_ccodegen_type_operations () =
  ...
  check bool "类型转换应包含目标类型" true
    (TestHelpers.contains_substring cast_result "int");
  check bool "类型转换应包含表达式" true
    (TestHelpers.contains_substring cast_result "value");

  (* 测试构造器匹配 *)
  let constructor_result = EnhancedCCodegen.constructor_match "expr" "Some" in
  check bool "构造器匹配应包含构造器名" true
    (TestHelpers.contains_substring constructor_result "Some");
...
let test_enhanced_ccodegen_advanced_calls () =
  ...
  check bool "带转换的调用应包含类型转换" true
    (TestHelpers.contains_substring cast_call_result "int");

  (* 测试条件绑定 *)
  let cond_bind_result = EnhancedCCodegen.luoyan_conditional_binding "var" "condition" "true_expr" "false_expr" in
  check bool "条件绑定应包含变量名" true
    (TestHelpers.contains_substring cond_bind_result "var");
  check bool "条件绑定应包含三元操作符" true
    (TestHelpers.contains_substring cond_bind_result "?");
...
let test_enhanced_ccodegen_memory_management () =
  ...
  check bool "内存分配应包含luoyan_alloc" true
    (TestHelpers.contains_substring alloc_result "luoyan_alloc");
  check bool "内存分配应包含大小" true
    (TestHelpers.contains_substring alloc_result "1024");

  (* 测试内存释放 *)
  let free_result = EnhancedCCodegen.luoyan_free "ptr" in
  check bool "内存释放应包含luoyan_free" true
    (TestHelpers.contains_substring free_result "luoyan_free");
...
let test_enhanced_ccodegen_data_structures () =
  ...
  check bool "记录获取应包含字段名" true
    (TestHelpers.contains_substring record_get_result "field");
...
let test_enhanced_ccodegen_error_handling () =
  ...
  check bool "错误抛出应包含错误代码" true
    (TestHelpers.contains_substring error_throw_result "404");
  check bool "错误抛出应包含错误消息" true
    (TestHelpers.contains_substring error_throw_result "Not found");
...
let test_enhanced_ccodegen_debugging () =
  ...
  check bool "调试跟踪应包含函数名" true
    (TestHelpers.contains_substring debug_trace_result "func_name");
...
let test_codegen_utilities_comments () =
  ...
  check bool "行注释应以//开头" true
    (TestHelpers.starts_with line_comment_result "//");
  check bool "行注释应包含注释内容" true
    (TestHelpers.contains_substring line_comment_result "这是注释");

  (* 测试块注释 *)
  let block_comment_result = CodeGenUtilities.c_block_comment "块注释" in
  check bool "块注释应包含/*" true
    (TestHelpers.contains_substring block_comment_result "/*");
  check bool "块注释应包含*/" true
    (TestHelpers.contains_substring block_comment_result "*/");

  (* 测试文档注释 *)
  let doc_comment_result = CodeGenUtilities.c_doc_comment "文档注释" in
  check bool "文档注释应包含/**" true
    (TestHelpers.contains_substring doc_comment_result "/**")

let test_codegen_utilities_formatting () =
  (* 测试代码缩进 *)
  let indent_result = CodeGenUtilities.c_indent_block "line1\nline2" 1 in
  check bool "缩进结果应不为空" true
    (TestHelpers.assert_not_empty indent_result);
  check bool "缩进结果应包含空格" true
    (TestHelpers.contains_substring indent_result "    ");

  (* 测试参数列表格式化 *)
  let param_list_result = CodeGenUtilities.c_format_parameter_list ["int x"; "char* y"] in
  check bool "参数列表应包含参数" true
    (TestHelpers.contains_substring param_list_result "int x");
...
let test_codegen_utilities_preprocessor () =
  ...
  check bool "系统头文件包含应使用尖括号" true
    (TestHelpers.contains_substring system_include_result "<stdio.h>");
  ...
  check bool "宏定义应包含#define" true
    (TestHelpers.contains_substring define_result "#define");
  check bool "宏定义应包含宏名" true
    (TestHelpers.contains_substring define_result "MAX_SIZE");

  (* 测试条件编译 *)
  let ifdef_result = CodeGenUtilities.c_ifdef "DEBUG" in
  check bool "ifdef应包含#ifdef" true
    (TestHelpers.contains_substring ifdef_result "#ifdef");

  let ifndef_result = CodeGenUtilities.c_ifndef "RELEASE" in
  check bool "ifndef应包含#ifndef" true
    (TestHelpers.contains_substring ifndef_result "#ifndef");
...
let test_codegen_utilities_blocks () =
  ...
  check bool "命名空间应包含名称" true
    (TestHelpers.contains_substring namespace_result "MyNamespace")
...
let test_codegen_performance_large_generation () =
  ...
  check bool "大量代码生成应在合理时间内完成" true
...
let performance_tests = [
  test_case "大量代码生成性能测试" `Slow test_codegen_performance_large_generation;
]
