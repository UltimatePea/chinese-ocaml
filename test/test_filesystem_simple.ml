...
let cleanup_test_file filename = if Sys.file_exists filename then Sys.remove filename

let cleanup_test_directory dirname =
  if Sys.file_exists dirname && Sys.is_directory dirname then try Unix.rmdir dirname with _ -> ()
...
let () =
  run "文件系统模块基础测试"
    [
      ...
      ("目录操作", [ test_case "基础目录操作" `Quick test_basic_directory_operations ]);
      ( "路径处理",
        [
