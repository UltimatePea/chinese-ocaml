...
module Test_ErrorHandlingFormatter = struct
  ...
  let test_structured_error_output () =
    ...
    let json_error = "{\"type\":\"error\",\"message\":\"类型不匹配\",\"file\":\"main.ly\",\"line\":15}" in
...
let () =
  run "骆言错误格式化模块测试"
    [
      ...
      ( "错误处理格式化器",
        [
          test_case "错误消息格式化规范" `Quick Test_ErrorHandlingFormatter.test_error_message_formatting_standards;
      ...
      ( "边界情况和特殊场景",
        [
          test_case "空值和特殊字符处理" `Quick Test_EdgeCasesAndSpecialScenarios.test_null_and_special_character_handling;
          test_case "超长错误消息处理" `Quick Test_EdgeCasesAndSpecialScenarios.test_long_error_message_handling;
          test_case "递归和嵌套错误处理" `Quick Test_EdgeCasesAndSpecialScenarios.test_recursive_and_nested_error_handling;
          test_case "并发和异步错误处理" `Quick Test_EdgeCasesAndSpecialScenarios.test_concurrent_and_async_error_handling;
        ] );
    ]