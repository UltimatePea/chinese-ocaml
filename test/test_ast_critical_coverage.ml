...
let test_patterns () =
  ...
  let () = check bool "构造器模式" true (constructor_pat = ConstructorPattern ("Some", [VarPattern "x"])) in
  ...
  let () = check bool "Cons模式" true (cons_pat = ConsPattern (VarPattern "head", VarPattern "tail")) in
  ...
  let () = check bool "异常模式" true (exception_pat = ExceptionPattern ("MyException", Some (VarPattern "e"))) in
  let () = check bool "多态变体模式" true (poly_variant_pat = PolymorphicVariantPattern ("Tag", Some (VarPattern "x"))) in
  ()
...
let test_type_exprs () =
  ...
  let poly_variant_type = PolymorphicVariantType [("Tag1", Some (BaseTypeExpr IntType)); ("Tag2", None)] in
  
  let () = check bool "基础类型表达式" true (base_type = BaseTypeExpr IntType) in
  let () = check bool "类型变量" true (type_var = TypeVar "a") in
  let () = check bool "函数类型" true (fun_type = FunType (BaseTypeExpr IntType, BaseTypeExpr StringType)) in
  let () = check bool "元组类型" true (tuple_type = TupleType [BaseTypeExpr IntType; BaseTypeExpr StringType]) in
  let () = check bool "列表类型" true (list_type = ListType (BaseTypeExpr IntType)) in
  let () = check bool "构造类型" true (construct_type = ConstructType ("MyType", [BaseTypeExpr IntType])) in
  let () = check bool "引用类型" true (ref_type = RefType (BaseTypeExpr IntType)) in
  let () = check bool "多态变体类型" true (poly_variant_type = PolymorphicVariantType [("Tag1", Some (BaseTypeExpr IntType)); ("Tag2", None)]) in
  ()
...
let test_type_defs () =
  ...
  let record_type = RecordType [("field1", BaseTypeExpr IntType); ("field2", BaseTypeExpr StringType)] in
  let private_type = PrivateType (BaseTypeExpr IntType) in
  let poly_variant_typedef = PolymorphicVariantTypeDef [("Tag1", Some (BaseTypeExpr IntType)); ("Tag2", None)] in
  
  let () = check bool "别名类型" true (alias_type = AliasType (BaseTypeExpr IntType)) in
  let () = check bool "代数类型" true (algebraic_type = AlgebraicType [("Cons", Some (BaseTypeExpr IntType)); ("Nil", None)]) in
  let () = check bool "记录类型" true (record_type = RecordType [("field1", BaseTypeExpr IntType); ("field2", BaseTypeExpr StringType)]) in
  let () = check bool "私有类型" true (private_type = PrivateType (BaseTypeExpr IntType)) in
  let () = check bool "多态变体类型定义" true (poly_variant_typedef = PolymorphicVariantTypeDef [("Tag1", Some (BaseTypeExpr IntType)); ("Tag2", None)]) in
  ()
...
let test_expressions () =
  ...
  let () = check bool "二元运算表达式" true (binary_op_expr = BinaryOpExpr (VarExpr "x", Add, VarExpr "y")) in
  let () = check bool "一元运算表达式" true (unary_op_expr = UnaryOpExpr (Neg, VarExpr "x")) in
  let () = check bool "函数调用表达式" true (fun_call_expr = FunCallExpr (VarExpr "f", [VarExpr "x"; VarExpr "y"])) in
  let () = check bool "条件表达式" true (cond_expr = CondExpr (VarExpr "condition", VarExpr "then_expr", VarExpr "else_expr")) in
...
let test_tone_pattern () =
  ...
  let () = check bool "平仄序列匹配" true (tone_pattern.tone_sequence = [LevelTone; FallingTone; LevelTone; FallingTone]) in
  let () = check bool "平仄约束匹配" true (tone_pattern.tone_constraints = [AlternatingTones; ParallelTones]) in
  ()
...
let test_statements () =
  ...
  let () = check bool "带类型Let语句" true (let_stmt_with_type = LetStmtWithType ("x", BaseTypeExpr IntType, VarExpr "value")) in
  let () = check bool "递归Let语句" true (rec_let_stmt = RecLetStmt ("f", VarExpr "recursive_func")) in
  let () = check bool "语义Let语句" true (semantic_let_stmt = SemanticLetStmt ("x", "数值变量", VarExpr "value")) in
  let () = check bool "类型定义语句" true (type_def_stmt = TypeDefStmt ("MyType", AliasType (BaseTypeExpr IntType))) in
  let () = check bool "异常定义语句" true (exception_def_stmt = ExceptionDefStmt ("MyException", Some (BaseTypeExpr StringType))) in
  ()
...
let test_macro_system () =
  ...
  let macro_call = {
    macro_call_name = "my_macro";
    args = [VarExpr "arg1"; VarExpr "arg2"];
  } in
...
let test_module_system () =
  ...
  let () = check bool "类型声明签名" true (sig_type_decl = SigTypeDecl ("type_name", Some (AliasType (BaseTypeExpr IntType)))) in
  let () = check bool "异常签名" true (sig_exception = SigException ("exception_name", Some (BaseTypeExpr StringType))) in
...
let test_helper_functions () =
  ...
  let () = check bool "make_binary_op辅助函数" true (binary_expr = BinaryOpExpr (VarExpr "x", Add, VarExpr "y")) in
  let () = check bool "make_call辅助函数" true (call_expr = FunCallExpr (VarExpr "f", [VarExpr "x"; VarExpr "y"])) in
  ()
...
let test_poetry_annotated_expressions () =
  ...
  let tone_pattern = {
    tone_sequence = [LevelTone; FallingTone];
  ...
  let () = check bool "诗词注解表达式" true (poetry_expr = PoetryAnnotatedExpr (VarExpr "诗句", FiveCharPoetry)) in
  let () = check bool "对偶结构表达式" true (parallel_expr = ParallelStructureExpr (VarExpr "上联", VarExpr "下联")) in
  let () = check bool "押韵注解表达式" true (rhyme_expr = RhymeAnnotatedExpr (VarExpr "押韵句", rhyme_info)) in
  let () = check bool "平仄注解表达式" true (tone_expr = ToneAnnotatedExpr (VarExpr "平仄句", tone_pattern)) in
  let () = check bool "韵律验证表达式" true (meter_expr = MeterValidatedExpr (VarExpr "韵律句", meter_constraint)) in
  ()
...
let test_advanced_expressions () =
  ...
  let () = check bool "记录表达式" true (record_expr = RecordExpr [("field1", VarExpr "value1"); ("field2", VarExpr "value2")]) in
  let () = check bool "字段访问" true (field_access = FieldAccessExpr (VarExpr "record", "field")) in
  let () = check bool "记录更新" true (record_update = RecordUpdateExpr (VarExpr "record", [("field1", VarExpr "new_value")])) in
  let () = check bool "数组表达式" true (array_expr = ArrayExpr [VarExpr "elem1"; VarExpr "elem2"]) in
  let () = check bool "数组访问" true (array_access = ArrayAccessExpr (VarExpr "array", VarExpr "index")) in
  let () = check bool "数组更新" true (array_update = ArrayUpdateExpr (VarExpr "array", VarExpr "index", VarExpr "value")) in
  ...
  let () = check bool "赋值表达式" true (assign_expr = AssignExpr (VarExpr "ref", VarExpr "new_value")) in
  let () = check bool "构造器表达式" true (constructor_expr = ConstructorExpr ("Some", [VarExpr "value"])) in
  let () = check bool "多态变体表达式" true (poly_variant_expr = PolymorphicVariantExpr ("Tag", Some (VarExpr "value"))) in
  let () = check bool "类型注解表达式" true (type_annotation = TypeAnnotationExpr (VarExpr "expr", BaseTypeExpr IntType)) in
  ()
...
let test_labeled_functions () =
  ...
  let () = check bool "标签函数表达式" true (labeled_fun_expr = LabeledFunExpr ([label_param], VarExpr "body")) in
  let () = check bool "标签函数调用" true (labeled_call_expr = LabeledFunCallExpr (VarExpr "func", [label_arg])) in
  ()
...
let () =
  ...
  run "AST核心模块测试套件" [
    "基础类型", [
      test_case "基础类型创建和比较" `Quick test_base_types;
    ];
    "字面量", [
      test_case "各种字面量类型" `Quick test_literals;
    ];
    "运算符", [
      test_case "二元运算符" `Quick test_binary_ops;
      test_case "一元运算符" `Quick test_unary_ops;
    ];
    "模式匹配", [
      test_case "所有模式类型" `Quick test_patterns;
    ];
    "类型系统", [
      test_case "类型表达式" `Quick test_type_exprs;
      test_case "类型定义" `Quick test_type_defs;
    ];
    "表达式", [
    ...
    "语句", [
      test_case "各种语句类型" `Quick test_statements;
    ];
    "宏系统", [
      test_case "宏参数和定义" `Quick test_macro_system;
    ];
    "模块系统", [
      test_case "模块和签名" `Quick test_module_system;
    ];
    "异步", [
      test_case "异步表达式" `Quick test_async_expressions;
    ];
    "匹配", [
      test_case "匹配分支" `Quick test_match_branches;
    ];
    "标签函数", [
      test_case "标签参数和调用" `Quick test_labeled_functions;
    ];
    "辅助函数", [
      test_case "AST构造辅助函数" `Quick test_helper_functions;
    ];
    "程序", [
      test_case "程序结构" `Quick test_program;
    ];
  ]