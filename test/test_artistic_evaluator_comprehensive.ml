    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          (* 验证评价结果格式 *)
          check (float 0.001)
            (Printf.sprintf "%s - 韵律评分在有效范围" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0);
          check bool (Printf.sprintf "%s - 韵律评价维度正确" desc) true (result.dimension = Rhyme);

          Printf.printf "%s 韵律评分: %.3f\n" desc result.score
        with exn -> fail (Printf.sprintf "%s 韵律评价失败: %s" desc (Printexc.to_string exn)))
      classical_poems
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          check (float 0.001)
            (Printf.sprintf "%s - 声调评分在有效范围" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0);
          check bool (Printf.sprintf "%s - 声调评价维度正确" desc) true (result.dimension = Tone);

          Printf.printf "%s 声调评分: %.3f\n" desc result.score
        with exn -> fail (Printf.sprintf "%s 声调评价失败: %s" desc (Printexc.to_string exn)))
      classical_poems
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          check (float 0.001)
            (Printf.sprintf "%s - 对仗评分在有效范围" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0);

          Printf.printf "%s 对仗评分: %.3f\n" desc result.score
        with exn -> fail (Printf.sprintf "%s 对仗评价失败: %s" desc (Printexc.to_string exn)))
      parallel_pairs
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          check (float 0.001)
            (Printf.sprintf "%s - 意象评分在有效范围" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0);

          Printf.printf "%s 意象评分: %.3f\n" desc result.score
        with exn -> fail (Printf.sprintf "%s 意象评价失败: %s" desc (Printexc.to_string exn)))
      classical_poems
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          check (float 0.001)
            (Printf.sprintf "%s - 节奏评分在有效范围" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0);

          Printf.printf "%s 节奏评分: %.3f\n" desc result.score
        with exn -> fail (Printf.sprintf "%s 节奏评价失败: %s" desc (Printexc.to_string exn)))
      classical_poems
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          check (float 0.001)
            (Printf.sprintf "%s - 雅致评分在有效范围" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0);

          Printf.printf "%s 雅致评分: %.3f\n" desc result.score
        with exn -> fail (Printf.sprintf "%s 雅致评价失败: %s" desc (Printexc.to_string exn)))
      classical_poems
end
  ...
  let test_context_creation () =
    List.iter
      (fun (poem, desc) ->
        ...
        with exn -> fail (Printf.sprintf "%s 上下文创建失败: %s" desc (Printexc.to_string exn)))
      classical_poems
  ...
  let test_empty_poem_handling () =
    ...
    List.iter
      (fun empty_poem ->
        ...
        with exn -> fail (Printf.sprintf "空诗句处理失败: %s" (Printexc.to_string exn)))
      empty_cases
  ...
  let test_complete_poem_evaluation () =
    List.iter
      (fun (poem, desc) ->
        ...
        with exn -> fail (Printf.sprintf "%s 完整评价失败: %s" desc (Printexc.to_string exn)))
      classical_poems
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          check (float 0.001)
            (Printf.sprintf "%s - 繁体字处理" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0);

          Printf.printf "%s 韵律评分: %.3f\n" desc result.score
        with exn -> fail (Printf.sprintf "%s 繁体字处理失败: %s" desc (Printexc.to_string exn)))
      traditional_poems
    ...
    List.iter
      (fun (text, desc) ->
        try
          ...
          check (float 0.001)
            (Printf.sprintf "%s - Unicode处理" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0)
        with exn -> fail (Printf.sprintf "%s Unicode处理失败: %s" desc (Printexc.to_string exn)))
      unicode_texts
end
...
module PerformanceTests = struct
  ...
  let test_long_poem_processing () =
    ...
    with exn -> fail (Printf.sprintf "长诗词处理失败: %s" (Printexc.to_string exn))

  (** 测试批量评价性能 *)
  let test_batch_evaluation_performance () =
    let poems = List.init 50 (fun i -> Printf.sprintf "诗句%d：春眠不觉晓，处处闻啼鸟。" i) in
    ...
    List.iter
      (fun (text, desc) ->
        try
          ...
          let result = evaluate_dimension ctx Rhyme in
          check (float 0.001)
            (Printf.sprintf "%s - 异常字符处理" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0)
        with exn -> fail (Printf.sprintf "%s 异常字符处理失败: %s" desc (Printexc.to_string exn)))
      abnormal_texts
    ...
    List.iter
      (fun (text, desc) ->
        try
          ...
          let result = evaluate_dimension ctx Tone in
          check (float 0.001)
            (Printf.sprintf "%s - 极限情况处理" desc)
            true
            (result.score >= 0.0 && result.score <= 1.0)
        with exn -> fail (Printf.sprintf "%s 极限情况处理失败: %s" desc (Printexc.to_string exn)))
      extreme_cases
end
