(** 诗词艺术性评价器全面测试模块 - Phase 25 测试覆盖率提升
    ...
    本测试模块专门针对 poetry/artistic_evaluator.ml 进行深度测试， 验证诗词艺术性评价的各个维度和算法准确性。
...
(** 测试用的现代诗句 *)
let modern_poems =
  [ ("今天天气很好", "现代简单描述"); ("123456789", "数字序列"); ("Hello World", "英文句子"); ("混合中English文字", "中英混合") ]
...
module EvaluationDimensionTests = struct
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          check bool
            (Internal_formatter.Test_message_formatter.dimension_correct_message desc "韵律")
            true (result.dimension = Rhyme);

          Printf.printf "%s 韵律评分: %.3f\n" desc result.score
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.evaluation_failure_message desc "韵律"
      ...
      (fun (poem, desc) ->
        try
          ...
          check (float 0.001)
            ...
            (result.score >= 0.0 && result.score <= 1.0);
          check bool
            (Internal_formatter.Test_message_formatter.dimension_correct_message desc "声调")
            true (result.dimension = Tone);

          Printf.printf "%s 声调评分: %.3f\n" desc result.score
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.evaluation_failure_message desc "声调"
               (Printexc.to_string exn)))
      classical_poems
  ...
  let test_parallelism_evaluation () =
    (* 测试典型的对仗句 *)
    let parallel_pairs =
      [ ("白日依山尽，黄河入海流。", "经典对仗"); ("两个黄鹂鸣翠柳，一行白鹭上青天。", "杜甫名句对仗"); ("山重水复疑无路，柳暗花明又一村。", "陆游名句对仗") ]
    in
    ...
    List.iter
      (fun (poem, desc) ->
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.evaluation_failure_message desc "对仗"
               (Printexc.to_string exn)))
      parallel_pairs
  ...
  let test_imagery_evaluation () =
    List.iter
      (fun (poem, desc) ->
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.evaluation_failure_message desc "意象"
               (Printexc.to_string exn)))
      classical_poems
  ...
  let test_rhythm_evaluation () =
    List.iter
      (fun (poem, desc) ->
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.evaluation_failure_message desc "节奏"
               (Printexc.to_string exn)))
      classical_poems
  ...
  let test_elegance_evaluation () =
    List.iter
      (fun (poem, desc) ->
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.evaluation_failure_message desc "雅致"
               (Printexc.to_string exn)))
      classical_poems
end
    ...
    List.iter
      (fun (poem, desc) ->
        try
          let ctx = create_evaluation_context poem in
          check bool
            (Internal_formatter.Test_message_formatter.context_creation_message desc)
            true true
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.context_creation_failure desc
               (Printexc.to_string exn)))
      classical_poems
  ...
  let test_empty_poem_handling () =
    ...
    List.iter
      (fun empty_poem ->
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.empty_poem_failure (Printexc.to_string exn)))
      empty_cases
...
module ComprehensiveEvaluationTests = struct
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          (* 验证所有维度都有评分 *)
          check int
            (Internal_formatter.Test_message_formatter.dimension_count_message desc)
            6 (List.length results);
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.complete_evaluation_failure desc
               (Printexc.to_string exn)))
      classical_poems
  ...
  let test_evaluation_consistency () =
    ...
    List.iter (fun result -> check (float 0.0001) "评价结果一致性" first_score result.score) results
...
module UnicodeTests = struct
  ...
  let test_traditional_chinese_poems () =
    let traditional_poems =
      [ ("床前明月光，疑是地上霜。", "简体版"); ("牀前明月光，疑是地上霜。", "繁体版"); ("舉頭望明月，低頭思故鄉。", "全繁体版") ]
    in
    ...
    List.iter
      (fun (poem, desc) ->
        try
          ...
          check (float 0.001)
            (Internal_formatter.Test_message_formatter.unicode_processing_message_with_feature desc
               "繁体字")
            true
            (result.score >= 0.0 && result.score <= 1.0);
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.unicode_processing_failure desc "繁体字"
               (Printexc.to_string exn)))
      traditional_poems

  (** 测试Unicode字符 *)
  let test_unicode_characters () =
    let unicode_texts = [ ("春風吹🌸花開", "带表情符号"); ("αβγδε诗词", "带希腊字母"); ("مرحبا世界", "带阿拉伯文") ] in
    ...
    List.iter
      (fun (text, desc) ->
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.unicode_processing_failure desc "Unicode"
               (Printexc.to_string exn)))
      unicode_texts
end
...
module PerformanceTests = struct
  ...
  let test_long_poem_processing () =
    ...
    with exn ->
      fail (Internal_formatter.Test_message_formatter.long_poem_failure (Printexc.to_string exn))

  (** 测试批量评价性能 *)
  let test_batch_evaluation_performance () =
    let poems = List.init 50 (fun i -> Internal_formatter.poem_generation i) in
...
module ErrorHandlingTests = struct
  ...
  let test_abnormal_character_handling () =
    let abnormal_texts =
      [ ("!@#$%^&*()", "特殊符号"); ("12345", "纯数字"); ("ABCDEF", "纯英文"); ("\x00\x01\x02", "控制字符") ]
    in
    ...
    List.iter
      (fun (text, desc) ->
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.abnormal_char_failure desc
               (Printexc.to_string exn)))
      abnormal_texts
  ...
  let test_extreme_cases () =
    ...
    List.iter
      (fun (text, desc) ->
        ...
        with exn ->
          fail
            (Internal_formatter.Test_message_formatter.extreme_case_failure desc
               (Printexc.to_string exn)))
      extreme_cases
end
  ...
  [
    ...
    ( "综合评价",
      [
        ...
        test_case "经典vs现代区分" `Quick
          ComprehensiveEvaluationTests.test_classical_vs_modern_distinction;
      ] );
    ( "Unicode支持",
      [
