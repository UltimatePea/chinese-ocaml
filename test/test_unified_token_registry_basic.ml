...
let test_multiple_mapping_registration () =
  ...
  List.iter (fun entry ->
    ...
    (match lookup_result with
    | Some found_entry ->
        check string ("查找" ^ entry.source) entry.source found_entry.source
    | None ->
        fail ("无法找到映射: " ^ entry.source))
  ) entries;
  ()
...
let test_priority_based_lookup () =
  ...
  let low_priority_entry = create_test_entry "测试" (Unified_token_core.IdentifierToken "identifier") 3 "标识符" true in
...
let test_enable_disable_mappings () =
  ...
  | Some found_entry ->
      check bool "启用的映射应该被找到" true found_entry.enabled
  | None ->
      fail "启用的映射应该能被找到");
  ()
...
let test_empty_string_handling () =
  ...
  (* 测试空字符串作为源 *)
  let empty_source_entry = create_test_entry "" (Unified_token_core.IdentifierToken "identifier") 1 "空字符串" true in
...
let test_long_string_handling () =
  ...
  let long_entry = create_test_entry long_string (Unified_token_core.IdentifierToken "identifier") 1 "长字符串" true in
...
let test_large_scale_stability () =
  ...
  (* 注册多个映射 *)
  for i = 1 to 50 do
    let entry = create_test_entry ("测试" ^ string_of_int i) (Unified_token_core.IdentifierToken "identifier") 1 "性能测试" true in
    TokenRegistry.register_mapping entry;
  done;
...
let batch_operation_tests = [
  test_case "批量注册测试" `Quick test_batch_registration;
  test_case "批量清理测试" `Quick test_batch_cleanup;
]
...
let performance_tests = [
  test_case "大规模稳定性测试" `Quick test_large_scale_stability;
]

(** 主测试运行器 *)
let () = run "Unified_token_registry 基础测试" [
