...
let test_safe_lookup_token_success () =
  ...
  match result with
  | Ok _token -> check bool "safe_lookup_token handles known tokens without error" true true
  ...
  | Error _other_error -> check bool "safe_lookup_token returns appropriate error type" true false
...
let test_safe_lookup_token_unknown_token () =
  ...
  match result with
  | Ok _token -> check bool "safe_lookup_token should fail for unknown token" true false
  | Error (Token_errors.UnknownToken (text, _pos)) ->
  ...
  | Error _other_error -> check bool "safe_lookup_token returns correct error type" true false
...
let test_safe_lookup_token_empty_string () =
  ...
  (* 验证：应该返回UnknownToken错误 *)
  match result with
  | Ok _token -> check bool "safe_lookup_token should fail for empty string" true false
  | Error (Token_errors.UnknownToken (text, _pos)) ->
...
let test_safe_lookup_token_error_severity () =
  ...
  | Ok _token -> check bool "should return error for unknown token" true false
...
let test_safe_create_position_valid () =
  ...
  | Error _error -> check bool "safe_create_position should succeed for valid input" true false
...
let test_safe_create_position_invalid () =
  ...
  match result with
  | Ok _position -> check bool "safe_create_position should fail for invalid line" true false
  | Error (Token_errors.InvalidPosition _pos) ->
      check bool "safe_create_position returns InvalidPosition error" true true
  | Error _other_error -> check bool "safe_create_position returns correct error type" true false
...
let test_safe_process_token_stream_basic () =
  (* 简化：直接测试空token流的处理 *)
  let result =
    Token_errors.SafeOps.safe_process_token_stream [] (fun tokens -> List.length tokens)
  in

  (* 验证：空流应该返回EmptyTokenStream错误 *)
  match result with
  | Ok _result -> check bool "safe_process_token_stream should fail for empty stream" true false
