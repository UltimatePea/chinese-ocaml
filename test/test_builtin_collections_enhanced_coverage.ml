(** 骆言Builtin_Collections内置集合函数模块增强测试覆盖 - Fix #1310
    ...
    本测试模块为 builtin_collections.ml 提供全面的增强测试覆盖，作为提升项目测试覆盖率
    从26.21%到50%目标的第三阶段实施。专注于内置集合操作函数的核心功能测试。
...
let create_add_one_function () =
  BuiltinFunctionValue (fun args ->
    match args with
    | [IntValue i] -> IntValue (i + 1)
    | _ -> runtime_error "加一函数期望一个整数参数")
...
let test_concat_function_basic () =
  ...
  (match first_func with
   | BuiltinFunctionValue func ->
     ...
     check bool "列表连接功能" true (result = ListValue [IntValue 1; IntValue 2; IntValue 3; IntValue 4])
...
let test_concat_function_error_handling () =
  ...
  (match func with
   | BuiltinFunctionValue f ->
     assert_runtime_error (fun () -> f [StringValue "错误"])
...
let test_filter_function_error_handling () =
  ...
  (match filter_func with
   | BuiltinFunctionValue func ->
     assert_runtime_error (fun () -> func [ListValue [IntValue 1]])
   | _ -> check bool "应该返回函数" false true)
...
let test_map_function_error_handling () =
  ...
  (match map_func with
   | BuiltinFunctionValue func ->
     assert_runtime_error (fun () -> func [StringValue "错误"])
   | _ -> check bool "应该返回函数" false true)
...
let test_contains_function_error_handling () =
  ...
  (match contains_func with
   | BuiltinFunctionValue func ->
     assert_runtime_error (fun () -> func [IntValue 42])
   | _ -> check bool "应该返回函数" false true)
...
let () =
  run "骆言Builtin_Collections内置集合函数模块增强测试覆盖 - Fix #1310" [
    ...
    "集合函数表测试", [
      test_case "函数表完整性验证" `Quick test_collection_functions_table;
    ];
  ]