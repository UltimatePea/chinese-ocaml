...
let () =
  Printf.printf "🧪 骆言内置I/O模块全面测试开始\n\n";

  (* 测试 print_function *)
  Printf.printf "🖨️ 测试 print_function\n";
  (try
    (* 测试字符串打印 *)
  ...
  with
  | e -> Printf.printf "❌ write_file_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    | StringValue content ->
        ...
        if String.equal content expected_content then
          ...
          Printf.printf "❌ 文件内容不匹配\n期望: \"%s\"\n实际: \"%s\"\n" expected_content content
    | _ -> Printf.printf "❌ 文件读取返回类型错误\n");
  ...
  with
  | e -> Printf.printf "❌ read_file_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    | ListValue contents ->
        ...
        if missing_files = [] then
          ...
          Printf.printf "❌ 缺少文件: %s\n" (String.concat ", " missing_files)
    | _ -> Printf.printf "❌ 目录列举返回类型错误\n");
  ...
  with
  | e -> Printf.printf "❌ list_directory_function 测试失败: %s\n" (Printexc.to_string e));
