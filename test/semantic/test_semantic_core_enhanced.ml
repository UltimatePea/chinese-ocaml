...
let test_simple_expression_analysis () =
  Alcotest.check
    ...
    "Simple expression analysis" (Ok "analyzed")
    (try
       let context = Semantic_module.create_initial_context () in
...
let test_variable_semantic_analysis () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Variable semantic analysis" (Ok "analyzed")
    (try
       let context = Semantic_module.create_initial_context () in
...
let test_type_expression_resolution () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Type expression resolution" (Ok "resolved")
    (try
       let context = Semantic_module.create_initial_context () in
...
let test_statement_semantic_analysis () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Statement semantic analysis" (Ok "analyzed")
    (try
       let context = Semantic_module.create_initial_context () in
...
let test_expression_semantics_check () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Expression semantics check" (Ok "checked")
    (try
...
let test_pattern_semantics_check () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Pattern semantics check" (Ok "checked")
    (try
       let _context = Semantic_module.create_initial_context () in
...
let test_successful_program_analysis () =
  let _simple_program = [ LetStmt ("x", LitExpr (IntLit 5)); LetStmt ("y", VarExpr "x") ] in
...
let test_semantic_error_handling () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Semantic error handling" (Error "semantic_error")
    (try
       let _ = Semantic_module.SemanticError "测试错误" in
       Error "semantic_error"
     with _ -> Error "other_error")

(* 符号表到环境转换测试 *)
let test_symbol_table_to_env_conversion () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Symbol table to environment conversion" (Ok "converted")
    (try
...
let test_complex_expression_analysis () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Complex expression analysis" (Ok "analyzed")
    (try
       let context = Semantic_module.create_initial_context () in
       let complex_expr =
         BinaryOpExpr
...
let test_chinese_identifier_semantics () =
  Alcotest.check
    (Alcotest.result Alcotest.string Alcotest.string)
    "Chinese identifier semantics" (Ok "analyzed")
    (try
       let context = Semantic_module.create_initial_context () in
