(** 骆言语义分析器增强综合测试 - Fix #985 提升语义分析模块测试覆盖率从37.5%到65%+的专项测试模块 *)
...
let check_expression_type context expr expected_type =
  ...
  match type_opt with Some typ -> typ = expected_type | None -> false
...
let test_variable_expressions () =
  ...
  check bool "未定义变量检测" true
    (check_expression_analysis_fails context_with_bool_var undefined_var_expr)
...
let test_conditional_expressions () =
  ...
  (* 简单条件表达式 *)
  let simple_cond = CondExpr (LitExpr (BoolLit true), LitExpr (IntLit 1), LitExpr (IntLit 2)) in
...
let test_function_expressions () =
  ...
  (* 多参数函数 *)
  let add_fun = FunExpr ([ "x"; "y" ], BinaryOpExpr (VarExpr "x", Add, VarExpr "y")) in
  check bool "多参数函数创建成功" true (check_expression_analysis_succeeds context add_fun);

  (* 高阶函数 *)
  let apply_fun = FunExpr ([ "f"; "x" ], FunCallExpr (VarExpr "f", [ VarExpr "x" ])) in
...
let test_function_calls () =
  ...
  (* 定义函数 *)
  let context_with_add_fun =
    add_symbol context_with_scope "加法函数"
      (FunType_T (IntType_T, FunType_T (IntType_T, IntType_T)))
      false
  in
  let context_with_identity =
    add_symbol context_with_add_fun "恒等函数" (FunType_T (IntType_T, IntType_T)) false
  in

  (* 单参数函数调用 *)
  let identity_call = FunCallExpr (VarExpr "恒等函数", [ LitExpr (IntLit 42) ]) in
  check bool "单参数函数调用分析" true
    (check_expression_analysis_succeeds context_with_identity identity_call);
  ...
  check bool "部分应用函数调用分析" true
    (check_expression_analysis_succeeds context_with_identity partial_call)
...
let test_tuple_expressions () =
  ...
  (* 嵌套元组 *)
  let nested_tuple =
    TupleExpr [ TupleExpr [ LitExpr (IntLit 1); LitExpr (IntLit 2) ]; LitExpr (StringLit "nested") ]
  in
...
let test_reference_expressions () =
  ...
  (* 引用解除 *)
  let context_with_scope = enter_scope context in
  let context_with_ref = add_symbol context_with_scope "int_ref_var" (RefType_T IntType_T) false in
...
let test_array_expressions () =
  ...
  (* 数组访问 *)
  let context_with_scope = enter_scope context in
  let context_with_array =
    add_symbol context_with_scope "test_array" (ArrayType_T IntType_T) false
  in
  ...
  check bool "数组访问表达式2分析成功" true
    (check_expression_analysis_succeeds context_with_array access_expr2)
        ...
        [
          {
            ...
            expr = BinaryOpExpr (VarExpr "n", Add, LitExpr (IntLit 1));
            guard = None;
          };
        ] )
  in
        ...
        [
          {
            ...
            expr = TupleExpr [ VarExpr "y"; VarExpr "x" ];
            guard = None;
          };
        ] )
  in
...
let test_try_catch_expressions () =
  ...
  in
  check bool "带finally的Try-catch表达式分析成功" true
    (check_expression_analysis_succeeds context try_with_finally);
...
let test_or_else_expressions () =
  ...
  (* 基本OrElse表达式 *)
  let basic_or_else = OrElseExpr (LitExpr (IntLit 1), LitExpr (IntLit 2)) in
  check bool "基本OrElse表达式分析成功" true (check_expression_analysis_succeeds context basic_or_else);

  (* 复杂OrElse表达式 *)
  let complex_or_else =
    OrElseExpr (BinaryOpExpr (LitExpr (IntLit 5), Div, LitExpr (IntLit 0)), LitExpr (IntLit 0))
  in
  check bool "复杂OrElse表达式分析成功" true (check_expression_analysis_succeeds context complex_or_else);

  (* 嵌套OrElse表达式 *)
  let nested_or_else =
    OrElseExpr (OrElseExpr (LitExpr (IntLit 1), LitExpr (IntLit 2)), LitExpr (IntLit 3))
  in
...
let test_let_statements () =
  ...
  (* 复杂let语句 *)
  let complex_let =
    LetStmt ("result", BinaryOpExpr (LitExpr (IntLit 10), Mul, LitExpr (IntLit 5)))
  in
  ...
  (* 函数定义let语句 *)
  let fun_let =
    LetStmt ("square", FunExpr ([ "n" ], BinaryOpExpr (VarExpr "n", Mul, VarExpr "n")))
  in
          ...
          ( [ "n" ],
            CondExpr
              ( BinaryOpExpr (VarExpr "n", Le, LitExpr (IntLit 1)),
                ...
                BinaryOpExpr
                  ( FunCallExpr
                      (VarExpr "fibonacci", [ BinaryOpExpr (VarExpr "n", Sub, LitExpr (IntLit 1)) ]),
                    Add,
                    FunCallExpr
                      (VarExpr "fibonacci", [ BinaryOpExpr (VarExpr "n", Sub, LitExpr (IntLit 2)) ])
                  ) ) ) )
  in
...
let test_expression_statements () =
  ...
  (* 复杂表达式语句 *)
  let complex_expr_stmt = ExprStmt (BinaryOpExpr (LitExpr (IntLit 3), Mul, LitExpr (IntLit 14))) in
  let _, result2 = analyze_statement context complex_expr_stmt in
  check bool "复杂表达式语句分析成功" true (result2 <> None);

  (* 函数调用表达式语句 *)
  let context_with_scope = enter_scope context in
  let context_with_fun =
    add_symbol context_with_scope "test_fun" (FunType_T (IntType_T, IntType_T)) false
  in
  let call_expr_stmt = ExprStmt (FunCallExpr (VarExpr "test_fun", [ LitExpr (IntLit 5) ])) in
...
let test_type_definitions () =
  ...
  (* 函数类型别名 *)
  let fun_alias =
    TypeDefStmt ("IntToInt", AliasType (FunType (BaseTypeExpr IntType, BaseTypeExpr IntType)))
  in
...
let test_variable_shadowing () =
  ...
  check bool "变量遮蔽测试" true
    (match inner_lookup with Some symbol -> symbol.symbol_type = StringType_T | None -> false);
  ...
  check bool "退出遮蔽作用域后原变量恢复" true
    (match outer_lookup with Some symbol -> symbol.symbol_type = IntType_T | None -> false)
...
let test_program_with_errors () =
  let program =
    [
      LetStmt ("x", LitExpr (IntLit 10));
      LetStmt ("y", VarExpr "undefined_var");
      (* 使用未定义变量 *)
      ExprStmt (BinaryOpExpr (VarExpr "x", Add, VarExpr "y"));
    ]
  in
