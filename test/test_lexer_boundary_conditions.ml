(** 词法分析器边界条件测试模块 - Phase 1测试覆盖率提升
    ...
    专注于边界条件、错误处理和复杂场景的词法分析测试 目标：提升Lexer模块测试覆盖率，增强系统稳定性
...
let check_token_list_equal msg expected actual =
  ...
  check
    (list (of_pp (fun fmt tok -> Format.pp_print_string fmt (show_token tok))))
...
let test_complex_chinese_characters () =
  let complex_inputs =
    [ "𠜎𠜱𠝹𠱓"; (* 复杂中文字符 *) "龘龖龗龘"; (* 生僻字 *) "𪚥𪚦𪚧𪚨"; (* 扩展中文字符 *) "👨‍💻🚀" (* emoji 和符号 *) ]
  in
  ...
  List.iter
    (fun input ->
      ...
      with
      | LexError (msg, _) -> check bool ("复杂UTF-8字符错误处理: " ^ input) true (String.length msg > 0)
      | _ -> fail ("意外异常: " ^ input))
    complex_inputs
...
let test_invalid_character_sequences () =
  let invalid_inputs =
    [ ("@#$%", "ASCII特殊字符"); ("abc123", "英文字母数字混合"); ("@@@@", "重复ASCII符号"); ("####", "重复井号") ]
  in
...
let test_deeply_nested_structures () =
  ...
  let nested_parentheses =
    String.concat "" (List.init 50 (fun _ -> left_paren))
    ^ String.concat "" (List.init 50 (fun _ -> right_paren))
  in
  ...
  let check_line_positions () =
    ...
    let rec check_positions tokens line_starts =
      ...
      | (token, pos) :: rest_tokens, (expected_token, expected_line) :: rest_starts ->
          ...
          else check_positions rest_tokens ((expected_token, expected_line) :: rest_starts)
      | _, [] -> ()
      | [], _ -> ()
    in
...
let test_performance_many_tokens () =
  ...
  check bool "大量token性能测试" true (duration < 1.0);
  (* 应该在1秒内完成 *)
...
let test_mixed_encoding_scenarios () =
  ...
  List.iter
    (fun (input, description) ->
      ...
      with
      | LexError (msg, _) -> check bool (description ^ " 错误处理") true (String.length msg > 0)
  ...
  List.iter
    (fun (input, expected_token) ->
      try
        ...
        | [] -> fail ("数字处理失败: " ^ input)
      with _ -> fail ("数字处理异常: " ^ input))
    number_scenarios

(** 关键字冲突边界测试 *)
let test_keyword_conflict_boundaries () =
  let conflict_scenarios =
    [ ("让让让", "重复关键字"); ("函数函数", "重复函数关键字"); ("如果否则", "相邻条件关键字"); ("数据结构算法", "复合关键字") ]
  in
...
let test_large_string_handling () =
  ...
  try
    ...
    match tokens with
    | (StringToken content, _) :: _ -> check int "大字符串长度" 10000 (String.length content)
    | _ -> fail "大字符串处理失败"
  with _ -> fail "大字符串处理异常"
