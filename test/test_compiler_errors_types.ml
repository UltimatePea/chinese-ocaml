...
let test_error_severity () =
  ...
  check bool "警告级别构造正确" true (match test_warning_severity with
    | Compiler_errors_types.Warning -> true
    | _ -> false);
  
  check bool "致命级别构造正确" true (match test_fatal_severity with
    | Compiler_errors_types.Fatal -> true
    | _ -> false);
...
let test_error_info () =
  ...
  let error_info = {
    Compiler_errors_types.error = error;
    severity = Compiler_errors_types.Warning;  (* 使用Warning避免与Error构造子冲突 *)
...
let test_error_result () =
  (* 成功结果 *)
  let success_result : string Compiler_errors_types.error_result = Compiler_errors_types.Ok "编译成功" in
  ...
  let error_result : string Compiler_errors_types.error_result = Compiler_errors_types.Error error_info in
...
let test_compiler_exception () =
  ...
  let exception_test () =
    raise (Compiler_errors_types.CompilerError error_info)
  in
...
let () =
  run "编译器错误类型模块综合测试"
    [
      ( "位置类型测试",
        [
          test_case "位置类型创建和比较" `Quick test_position_type;
        ] );
      ( "编译器错误类型测试",
        [
          test_case "基本编译器错误类型" `Quick test_compiler_error_types;
        ] );
      ( "错误严重级别和信息",
        [
      ...
      ( "异常处理",
        [
          test_case "编译器异常" `Quick test_compiler_exception;
        ] );
    ]