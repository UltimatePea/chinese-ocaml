  ...
  make_function_info "过滤大于十" ["列表"]
    (make_match (make_variable "列表") [
      ...
      ("头::尾", make_condition 
        ...
        (make_function_call "cons" [make_variable "头"; make_function_call "过滤大于十" [make_variable "尾"]])
...
let test_formatting () =
  ...
  let simple_func = make_function_info "相加" ["x"; "y"] 
    (make_binary (make_variable "x") "+" (make_variable "y")) false in
...
let test_different_configs () =
  ...
  let simple_func = make_function_info "简单相加" ["a"; "b"] 
    (make_binary (make_variable "a") "+" (make_variable "b")) false in
  
  (* 简要模式 *)
  let brief_config = { default_config with 
    detail_level = `Brief; include_examples = false } in
  let brief_doc = generate_function_doc simple_func brief_config in
  
  (* 详细模式 *)
  let detailed_config = { default_config with 
    detail_level = `Comprehensive; include_examples = true } in
...
let test_complex_functions () =
  ...
  (* 测试递归函数 *)
  let recursive_func = make_function_info "阶乘" ["n"]
    (make_condition 
  ...
  (* 测试模式匹配函数 *)
  let pattern_match_func = make_function_info "长度" ["列表"]
    (make_match (make_variable "列表") [
      ("[]", make_literal "0");
      ("_::尾", make_binary (make_literal "1") "+" 
        (make_function_call "长度" [make_variable "尾"]))
    ]) false in
...
let test_edge_cases () =
  ...
  (* 测试复杂返回类型 *)
  let tuple_func = make_function_info "创建三元组" ["x"; "y"; "z"] 
    (make_tuple [make_variable "x"; make_variable "y"; make_variable "z"]) false in
...
let test_integration () =
  ...
  assert (List.length module_doc.functions = 3); (* 应该有3个函数 *)
...
let test_feature_detection () =
  ...
  (* 测试递归特征检测 *)
  let recursive_func = List.hd sample_functions in (* 斐波那契函数 *)
  assert (recursive_func.is_recursive);
  
  (* 测试列表处理特征检测 *)
  let list_func = List.nth sample_functions 2 in (* 过滤函数 *)
  let list_doc = generate_function_doc list_func default_config in
  let has_list_feature = List.exists (fun note -> 
    string_contains note "列表" || string_contains note "匹配"
  ) list_doc.notes in
...
let test_chinese_optimization () =
  ...
  (* 测试中文函数名的动词识别 *)
  let calculation_func = make_function_info "计算总和" ["数组"] 
    (make_function_call "求和" [make_variable "数组"]) false in
  let calc_doc = generate_function_doc calculation_func default_config in
  
  assert (string_contains calc_doc.summary "计算");
  
  (* 测试处理类函数 *)
  let process_func = make_function_info "处理数据" ["输入"] 
    (make_function_call "处理" [make_variable "输入"]) false in
