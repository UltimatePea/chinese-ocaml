...
let test_supported_chinese_symbols () =
  let test_cases = [
    ...
    (* 组合测试 *)
    ("「函数名」（参数一，参数二）：返回三。", 
     [QuotedIdentifierToken "函数名"; ChineseLeftParen; 
      IdentifierToken "参数一"; ChineseComma; IdentifierToken "参数二"; 
      ChineseRightParen; ChineseColon; IdentifierToken "返回三"; Dot; EOF]);
  ] in
...
let test_forbidden_symbols () =
  ...
  List.iter (fun (input, _error_type) ->
    ...
    with 
    | LexError (msg, _pos) ->
      if Str.string_match (Str.regexp ".*禁.*") msg 0 then
        Printf.printf "✓ 正确抛出错误: %s\n" msg
      else
        Printf.printf "✗ 错误消息不正确: %s\n" msg
    | e ->
