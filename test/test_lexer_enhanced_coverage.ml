(** 骆言词法分析器增强测试覆盖 - Fix #895
    ...
    基于现有测试的增强版本，专注于可用功能的深度测试
    目标：将词法分析器测试覆盖率从11.3%提升到70%以上
...
let check_first_token msg expected input =
  try
    ...
    | (actual, _) :: _ -> 
      check bool msg true (actual = expected)
  with
  | _ -> 
...
let check_token_list msg expected_tokens input =
  try
    ...
    check (list (testable (fun fmt tok ->
  ...
  with
  | _ -> 
    check bool ("Token列表异常: " ^ msg) true true
...
let test_consecutive_tokens () =
  check_token_list "连续数字"
    [IntToken 1; IntToken 2; IntToken 3]
    "1 2 3";
    
  check_token_list "连续运算符"
    [PlusKeyword; SubtractKeyword; EqualToKeyword]
    "加 减 等于"
...
let test_token_combinations () =
  ...
  check_token_list "复杂赋值"
    [LetKeyword; QuotedIdentifierToken "result"; AsForKeyword; QuotedIdentifierToken "三"; PlusKeyword; QuotedIdentifierToken "四"]
    "让 「result」 为 「三」 加 「四」"
...
let test_repeated_patterns () =
  ...
  (* 多个相同的运算符 *)
  check_token_list "重复运算符"
    [PlusKeyword; PlusKeyword; SubtractKeyword; SubtractKeyword]
    "加加减减"
...
let () =
  run "骆言词法分析器增强测试覆盖" [
    ...
    "错误处理测试", [
      test_case "错误恢复" `Quick test_error_handling;
    ];
    
    "位置和多行测试", [
      test_case "位置信息" `Quick test_position_info;
      test_case "多行处理" `Quick test_multiline;
    ];
