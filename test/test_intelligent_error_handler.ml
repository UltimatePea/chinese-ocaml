...
let test_undefined_variable_analysis () =
  ...
  let report =
    generate_ai_error_report "undefined_variable" [ "用户姓名"; "用户名;姓名;年龄;分数" ] (Some "函数内部")
  in

  Printf.printf "%s\n" report;

  let report2 =
    generate_ai_error_report "undefined_variable" [ "计数器"; "计数;数量;总数;索引" ] (Some "循环体内")
  in
...
let test_type_mismatch_analysis () =
  ...
  let report = generate_ai_error_report "type_mismatch" [ "整数类型"; "字符串类型" ] (Some "算术表达式") in

  Printf.printf "%s\n" report;

  let report2 = generate_ai_error_report "type_mismatch" [ "列表类型"; "整数类型" ] (Some "函数参数") in
...
let test_function_arity_analysis () =
  ...
  let report = generate_ai_error_report "function_arity" [ "3"; "2"; "计算平均值" ] (Some "函数调用") in

  Printf.printf "%s\n" report;

  let report2 = generate_ai_error_report "function_arity" [ "1"; "3"; "打印消息" ] (Some "主函数") in
...
let test_pattern_match_analysis () =
  ...
  let report =
    generate_ai_error_report "pattern_match" [ "Some分支"; "None分支" ] (Some "Option类型处理")
  in

  Printf.printf "%s\n" report;

  let report2 = generate_ai_error_report "pattern_match" [ "[]"; "head::tail" ] (Some "列表递归处理") in
...
let test_comprehensive_error_scenarios () =
  ...
  let report1 =
    generate_ai_error_report "undefined_variable"
      [ "用户详细信息"; "用户信息;用户数据;详细信息;个人信息;用户资料" ]
      (Some "嵌套函数内部，当前作用域包含多个相关变量")
  in
  Printf.printf "%s\n" report1;
  ...
  let report2 =
    generate_ai_error_report "type_mismatch" [ "函数类型"; "整数类型" ] (Some "高阶函数调用中，期望函数但传入了数值")
  in
  Printf.printf "%s\n" report2;
