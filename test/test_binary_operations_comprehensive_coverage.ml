...
module IntegerArithmeticTests = struct
  let test_integer_addition () =
    ...
    (* 零加法 *)
    TestUtils.check_value_equal "零加法" (IntValue 5)
      (execute_binary_op Add (IntValue 0) (IntValue 5))
  ...
  let test_integer_subtraction () =
    ...
    TestUtils.check_value_equal "减法零" (IntValue 0)
      (execute_binary_op Sub (IntValue 5) (IntValue 5))
  ...
  let test_integer_division () =
    ...
    check TestUtils.bool_testable "除零应抛出错误" true
      (TestUtils.expect_runtime_error (fun () -> 
        execute_binary_op Div (IntValue 10) (IntValue 0)))
...
module FloatArithmeticTests = struct
  ...
  let test_float_division () =
    ...
    | FloatValue f when Float.is_infinite f -> 
        check TestUtils.bool_testable "浮点数除零应返回无穷大" true true
    | _ -> fail "浮点数除零应返回无穷大")
end
...
module StringOperationTests = struct
  let test_string_concatenation () =
    ...
    check TestUtils.bool_testable "非字符串Concat应失败" true
      (TestUtils.expect_runtime_error (fun () ->
        execute_binary_op Concat (IntValue 1) (IntValue 2)))
end

(** 一元运算测试 *)
module UnaryOperationTests = struct
  let test_negation () =
    TestUtils.check_value_equal "整数取负" (IntValue (-5))
      (execute_unary_op Neg (IntValue 5));
    
    TestUtils.check_value_equal "负数取负" (IntValue 5)
      (execute_unary_op Neg (IntValue (-5)));
    
    TestUtils.check_value_equal "零取负" (IntValue 0)
      (execute_unary_op Neg (IntValue 0));
    ...
    check TestUtils.bool_testable "字符串取负应失败" true
      (TestUtils.expect_runtime_error (fun () ->
        execute_unary_op Neg (StringValue "hello")))
        
  let test_logical_not () =
    TestUtils.check_value_equal "真的逻辑非" (BoolValue false)
      (execute_unary_op Not (BoolValue true));
    
    TestUtils.check_value_equal "假的逻辑非" (BoolValue true)
      (execute_unary_op Not (BoolValue false));
    
    (* 测试非布尔值的逻辑非 *)
    TestUtils.check_value_equal "整数逻辑非" (BoolValue true)
      (execute_unary_op Not (IntValue 0));
    
    TestUtils.check_value_equal "非零整数逻辑非" (BoolValue false)
      (execute_unary_op Not (IntValue 5));
    
    TestUtils.check_value_equal "空字符串逻辑非" (BoolValue true)
      (execute_unary_op Not (StringValue ""));
...
module ErrorHandlingTests = struct
  ...
  let test_division_by_zero () =
    ...
    check TestUtils.bool_testable "整数模零错误" true
      (TestUtils.expect_runtime_error (fun () ->
        execute_binary_op Mod (IntValue 15) (IntValue 0)))
  ...
  let test_error_messages () =
    ...
    check TestUtils.bool_testable "除零错误消息" true
      (TestUtils.expect_error_with_message (fun () ->
        execute_binary_op Div (IntValue 10) (IntValue 0)) "除零");
...
module EdgeCaseTests = struct
  let test_extreme_integer_values () =
    (* 最大整数运算 *)
    TestUtils.check_value_equal "最大整数加一" (IntValue (max_int + 1))
      (execute_binary_op Add (IntValue max_int) (IntValue 1));
    
    (* 最小整数运算 *)
    TestUtils.check_value_equal "最小整数减一" (IntValue (min_int - 1))
  ...
  let test_extreme_float_values () =
    (* 极小浮点数 *)
    TestUtils.check_value_equal "极小浮点数加法" (FloatValue (1e-10 +. 1e-10))
  ...
  let test_special_float_values () =
    ...
    | FloatValue f when Float.is_nan f ->
        check TestUtils.bool_testable "NaN运算应返回NaN" true true
    | _ -> fail "NaN运算应返回NaN");
    ...
    | FloatValue f when Float.is_infinite f ->
        check TestUtils.bool_testable "无穷大运算应返回无穷大" true true
    | _ -> fail "无穷大运算应返回无穷大")
  ...
  let test_empty_and_special_strings () =
    ...
    TestUtils.check_value_equal "长字符串连接" (StringValue (long_str ^ long_str))
...
module PerformanceTests = struct
  ...
  let test_deep_nesting_performance () =
    ...
    let rec create_nested_expr depth acc =
      if depth = 0 then acc
      else create_nested_expr (depth - 1) 
        (execute_binary_op Add acc (IntValue 1))
    in
...
let test_suite = [
  ...
  ("混合类型运算", [
    test_case "整数浮点数混合运算" `Quick MixedArithmeticTests.test_int_float_operations;
  ]);
  ...
  ("字符串运算", [
    test_case "字符串连接" `Quick StringOperationTests.test_string_concatenation;
  ]);
