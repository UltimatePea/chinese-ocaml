...
let test_characters =
  [
    { character = "春"; category = "平声"; group = "安韵"; metadata = [ ("test", "spring") ] };
    { character = "花"; category = "仄声"; group = "思韵"; metadata = [ ("test", "flower") ] };
    { character = "雪"; category = "入声"; group = "月韵"; metadata = [ ("test", "snow") ] };
  ]
...
let setup_test_environment () =
  ...
  register_data_source source_id test_data_loader
    (fun _ -> true)
    [ ("description", "Issue 1795 integrity test") ];
  (* 重建索引以确保数据可查询 *)
