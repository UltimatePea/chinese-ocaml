...
let create_luoyan_list n =
  let rec aux acc i = if i >= n then acc else aux (IntValue i :: acc) (i + 1) in
  ListValue (aux [] 0)

(* 从骆言函数表获取函数 *)
let get_luoyan_function name =
  try List.assoc name collection_functions
  with Not_found -> failwith ("Function not found: " ^ name)

(* 执行骆言函数 *)
let call_luoyan_function func args =
  match func with BuiltinFunctionValue f -> f args | _ -> failwith "Not a builtin function"
  ...
  List.iter2
    (fun size name ->
      let test_list =
        List.hd
          (List.filter
             (fun l -> match l with ListValue lst -> List.length lst = size | _ -> false)
             lists)
      in
  ...
  let double_func =
    BuiltinFunctionValue
      (fun args ->
        ...
        | _ -> failwith "double function expects one integer argument")
  in
...
let benchmark_filter lists =
  ...
  let is_even_func =
    BuiltinFunctionValue
      (fun args ->
        ...
        | _ -> failwith "is_even function expects one integer argument")
  in
  ...
  List.iter2
    (fun size name ->
      ...
      let memory_after = get_memory_usage () in
      let memory_used = memory_after - memory_before in
      let filtered_count = match result with ListValue lst -> List.length lst | _ -> 0 in
  ...
  let add_func =
    BuiltinFunctionValue
      (fun args ->
        ...
        | _ -> failwith "add function expects two integer arguments")
  in
    ...
    (fun size name ->
      ...
      let result, time_taken =
        time_function
          ...
          ()
      in
      let sum_result = match result with IntValue sum -> sum | _ -> 0 in
    ...
    (fun size name ->
      ...
      let result, time_taken =
        time_function
          ...
          ()
      in
      let final_length = match result with ListValue lst -> List.length lst | _ -> 0 in
...
let test_stack_overflow () =
  ...
  try
    ...
    let inc_func =
      BuiltinFunctionValue
        (fun args ->
          match args with [ IntValue x ] -> IntValue (x + 1) | _ -> failwith "inc error")
    in
    ...
    let is_even_func =
      BuiltinFunctionValue
        (fun args ->
          match args with [ IntValue x ] -> BoolValue (x mod 2 = 0) | _ -> failwith "even error")
    in
    let partial_filter = call_luoyan_function filter_func [ is_even_func ] in
...
let performance_regression_test () =
  ...
  let double_func =
    BuiltinFunctionValue
      (fun args ->
        match args with [ IntValue x ] -> IntValue (x * 2) | _ -> failwith "double error")
  in
  let _, map_time =
    time_function
      (fun _ ->
  ...
  let add_func =
    BuiltinFunctionValue
      (fun args ->
        match args with [ IntValue a; IntValue b ] -> IntValue (a + b) | _ -> failwith "add error")
  in
  let _, fold_time =
    time_function
      (fun _ ->
