...
let make_assignment var value = LetStmt (var, value)
let make_if condition then_stmt else_stmt = ExprStmt (CondExpr (condition, then_stmt, else_stmt))
let make_while condition body = ExprStmt (CondExpr (condition, body, LitExpr UnitLit))
let make_function_def name params body = LetStmt (name, FunExpr (params, body))
...
let test_comprehensive_analysis _ =
  ...
  (* 综合分析应该返回完整报告 *)
  check bool "应该返回建议列表" (List.length suggestions >= 0);
  check bool "命名报告不应为空" (String.length naming_report > 0);
...
let test_suggestion_statistics _ =
  let test_suggestions =
    [
      { hint_type = "命名"; message = "建议1"; confidence = 0.9; location = "位置1"; fix = "修复1" };
      { hint_type = "性能"; message = "建议2"; confidence = 0.7; location = "位置2"; fix = "修复2" };
      { hint_type = "复杂度"; message = "建议3"; confidence = 0.8; location = "位置3"; fix = "修复3" };
    ]
  in
...
let test_quality_assessment _ =
  let program_with_issues =
    [
      ...
      (* 可能的重复代码 *)
      make_while (make_var "true") (make_assignment "z" (make_int 1));
      (* 复杂度问题 *)
    ]
  in
...
let test_naming_analysis _ =
  let context = create_test_context () in
  let expr_with_poor_naming = make_var "x" in
  (* 通用变量名 *)
    ...
    make_while (make_var "条件1")
      (make_while (make_var "条件2")
         (make_while (make_var "条件3")
            (make_while (make_var "条件4") (make_assignment "深层变量" (make_int 1)))))
  in
...
let test_code_pattern_detection _ =
  ...
  (* 创建相似的代码模式 *)
  let pattern1 =
    make_if (make_var "x") (make_assignment "结果" (make_int 1)) (make_assignment "结果" (make_int 0))
  in
  let pattern2 =
    make_if (make_var "y") (make_assignment "结果" (make_int 1)) (make_assignment "结果" (make_int 0))
  in
...
let test_integrated_refactoring_analysis _ =
  let complex_program =
    [
      make_function_def "calculateValue" [ "x"; "y" ]
        ((* 英文命名问题 *)
         make_if
           (make_var "x") (* 可能的复杂逻辑 *)
  ...
  (* 集成分析应该产生多种类型的建议 *)
  check bool "集成分析应该完成" (List.length suggestions >= 0);
