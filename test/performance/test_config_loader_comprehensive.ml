...
let () =
  ...
  (try
     ...
     if buffer_size > 0 && timeout > 0.0 then Printf.printf "✅ 默认配置值合理性检查通过\n"
     else Printf.printf "❌ 默认配置值不合理\n"
   with e -> Printf.printf "❌ 默认配置获取测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "❌ 配置设置和获取测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     List.iter
       (fun (input, expected) ->
         ...
         if result = expected then Printf.printf "✅ 布尔值解析: '%s' -> %b\n" input result
         else Printf.printf "❌ 布尔值解析失败: '%s' 期望 %b，实际 %b\n" input expected result)
       bool_tests;
   ...
   with e -> Printf.printf "❌ 环境变量解析测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     else Printf.printf "⚠️  环境变量配置加载部分成功（某些值可能有限制）\n";
   ...
   with e -> Printf.printf "❌ 环境变量配置加载测试失败: %s\n" (Printexc.to_string e));
  ...
  "c_compiler": "clang"
     ...
     if load_success then (
       ...
       if after_buffer = 4096 && after_timeout = 60.0 && after_output_dir = "/tmp/luoyan_output"
       then Printf.printf "✅ 配置文件内容正确应用\n"
       else Printf.printf "⚠️  配置文件内容部分应用（某些值可能有限制）\n")
     else Printf.printf "❌ 配置文件加载失败\n";
  ...
  "invalid_json": syntax error
     ...
     if not invalid_load_success then Printf.printf "✅ 无效配置文件正确被拒绝\n"
     else Printf.printf "❌ 无效配置文件应该被拒绝\n";

     (* 测试不存在的配置文件 *)
     let non_existent_load_success = load_from_file "/tmp/non_existent_config.json" in
     if not non_existent_load_success then Printf.printf "✅ 不存在的配置文件正确被拒绝\n"
     else Printf.printf "❌ 不存在的配置文件应该被拒绝\n";
   ...
   with e -> Printf.printf "❌ JSON配置文件测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if validation_errors = [] then Printf.printf "✅ 当前配置验证通过，无错误\n"
     else (
   ...
   with e -> Printf.printf "❌ 配置验证测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "❌ 配置打印测试失败: %s\n" (Printexc.to_string e));
     ...
     List.iter
       (fun (name, getter) ->
         try
           let value = getter () in
           Printf.printf "✅ %s: %s\n" name value
         with e -> Printf.printf "❌ %s 访问失败: %s\n" name (Printexc.to_string e))
       access_tests
   with e -> Printf.printf "❌ 便捷访问函数测试失败: %s\n" (Printexc.to_string e));
