...
let () =
  Printf.printf "TEST: 骆言运行时配置模块全面测试开始\n\n";
  ...
  (try
     ...
     if max_error_count > 0 then Printf.printf "√ 默认运行时配置值合理性检查通过\n"
     else Printf.printf "X 默认运行时配置值不合理\n"
   with e -> Printf.printf "X 默认运行时配置测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "X 运行时配置获取设置测试失败: %s\n" (Printexc.to_string e));

  (* 测试调试模式配置 *)
  Printf.printf "\nDEBUG: 测试调试模式配置\n";
  (try
     let original_debug = Get.debug_mode () in
     Printf.printf "STAT: 原始调试模式: %b\n" original_debug;
     ...
     Printf.printf "STAT: 关闭调试模式: %b\n" debug_disabled;

     if debug_enabled = true && debug_disabled = false then Printf.printf "√ 调试模式配置切换测试通过\n"
     else Printf.printf "WARN:  调试模式配置切换部分成功\n";

     Unix.putenv "CHINESE_OCAML_DEBUG" ""
   with e -> Printf.printf "X 调试模式配置测试失败: %s\n" (Printexc.to_string e));

  (* 测试详细日志配置 *)
  Printf.printf "\nLOG: 测试详细日志配置\n";
  (try
     let original_verbose = Get.verbose_logging () in
     Printf.printf "STAT: 原始详细日志: %b\n" original_verbose;

     (* 测试详细日志模式切换 *)
     Unix.putenv "CHINESE_OCAML_VERBOSE" "true";
     load_from_env ();
     let verbose_enabled = Get.verbose_logging () in
     Printf.printf "STAT: 开启详细日志: %b\n" verbose_enabled;

     Unix.putenv "CHINESE_OCAML_VERBOSE" "false";
     load_from_env ();
     let verbose_disabled = Get.verbose_logging () in
     Printf.printf "STAT: 关闭详细日志: %b\n" verbose_disabled;

     if verbose_enabled = true && verbose_disabled = false then Printf.printf "√ 详细日志配置切换测试通过\n"
     else Printf.printf "WARN:  详细日志配置切换部分成功\n";

     Unix.putenv "CHINESE_OCAML_VERBOSE" ""
   with e -> Printf.printf "X 详细日志配置测试失败: %s\n" (Printexc.to_string e));
     ...
     List.iter
       (fun level ->
         ...
         (try
          ...
          with _ -> Printf.printf "√ 无效日志级别 '%s' 正确被拒绝\n" level);
         Unix.putenv "CHINESE_OCAML_LOG_LEVEL" "")
       invalid_levels;

     Printf.printf "√ 日志级别配置测试完成\n"
   with e -> Printf.printf "X 日志级别配置测试失败: %s\n" (Printexc.to_string e));

  (* 测试综合运行时配置场景 *)
  Printf.printf "\n🔄 测试综合运行时配置场景\n";
  (try
     Printf.printf "TEST: 场景1: 开发模式配置\n";
     Unix.putenv "CHINESE_OCAML_DEBUG" "true";
     Unix.putenv "CHINESE_OCAML_VERBOSE" "true";
     ...
     else Printf.printf "WARN:  开发模式配置部分成功\n";
     ...
     else Printf.printf "WARN:  生产模式配置部分成功\n";
     ...
     else Printf.printf "WARN:  测试模式配置部分成功\n";

     (* 清理环境变量 *)
     List.iter
       (fun var -> Unix.putenv var "")
       [
         "CHINESE_OCAML_DEBUG";
   ...
   with e -> Printf.printf "X 综合运行时配置场景测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     Printf.printf "STAT: 初始配置: 调试=%b, 最大错误=%d\n" initial_debug initial_max_errors;
     ...
     Unix.putenv "CHINESE_OCAML_DEBUG" "";
     Unix.putenv "CHINESE_OCAML_MAX_ERRORS" ""
   with e -> Printf.printf "X 配置一致性测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if duration < 1.0 then Printf.printf "√ 运行时配置访问性能优秀\n"
     else Printf.printf "WARN:  运行时配置访问性能可能需要优化\n"
   with e -> Printf.printf "X 性能测试失败: %s\n" (Printexc.to_string e));

  Printf.printf "\n🎉 骆言运行时配置模块全面测试完成！\n";
  Printf.printf "STAT: 测试涵盖: 调试模式、错误处理、智能功能、输出格式、日志级别\n";
  Printf.printf "CONFIG: 包含综合场景测试、一致性验证、性能测试\n";
