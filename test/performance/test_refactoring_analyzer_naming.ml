(** 命名质量分析器模块测试
    ...
    测试覆盖refactoring_analyzer_naming.ml模块的所有核心功能
    技术债务修复：提升关键编译器模块测试覆盖率 Fix #1617
...
let test_analyze_naming_quality () =
  ...
  check bool "应包含建议使用中文命名信息" true 
    (contains_substring suggestion.message "中");
  ...
  check bool "应包含避免混用信息" true 
    (contains_substring mixed_suggestion.message "混");
...
let test_get_naming_statistics () =
  ...
  let suggestions = [english_suggestion; mixed_suggestion; short_suggestion; 
                    meaningless_suggestion; non_naming_suggestion] in
...
let test_suggestion_type_classification () =
  ...
  | NamingImprovement improvement_type -> 
      check string "英文命名建议类型应正确" "建议使用中文命名" improvement_type
  ...
  | NamingImprovement improvement_type -> 
      check string "混用命名建议类型应正确" "避免中英文混用" improvement_type
...
let () =
  run "命名质量分析器测试" [
    "英文命名检测", [
      test_case "is_english_naming基础功能" `Quick test_is_english_naming;
    ];
    "中英文混用检测", [
      test_case "is_mixed_naming基础功能" `Quick test_is_mixed_naming;
    ];
    "过短命名检测", [
      test_case "is_too_short基础功能" `Quick test_is_too_short;
    ];
    "无意义命名检测", [
      test_case "is_meaningless_naming基础功能" `Quick test_is_meaningless_naming;
    ];
    "命名质量分析", [
      test_case "analyze_naming_quality基础功能" `Quick test_analyze_naming_quality;
    ];
    "批量命名分析", [
      test_case "analyze_multiple_names基础功能" `Quick test_analyze_multiple_names;
    ];
    "命名统计功能", [
      test_case "get_naming_statistics基础功能" `Quick test_get_naming_statistics;
    ];
    "报告生成功能", [
      test_case "generate_naming_report基础功能" `Quick test_generate_naming_report;
    ];
    "中文字符处理", [
      test_case "中文字符边界条件" `Quick test_chinese_character_handling;
    ];
    "建议类型分类", [
      test_case "建议类型正确性" `Quick test_suggestion_type_classification;
    ];
  ]