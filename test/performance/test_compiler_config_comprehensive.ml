...
let () =
  ...
  (try
     ...
     if
       buffer_size > 0 && large_buffer_size >= buffer_size && compilation_timeout > 0.0
       && optimization_level >= 0 && hashtable_size > 0
       && large_hashtable_size >= hashtable_size
     then Printf.printf "√ 默认编译器配置值合理性检查通过\n"
     else Printf.printf "X 默认编译器配置值不合理\n"
   with e -> Printf.printf "X 默认编译器配置测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "X 编译器配置获取设置测试失败: %s\n" (Printexc.to_string e));
     ...
     List.iter
       (fun size ->
         ...
         (try
          ...
          with _ -> Printf.printf "√ 无效缓冲区大小 '%s' 正确被拒绝\n" size);
         Unix.putenv "CHINESE_OCAML_BUFFER_SIZE" "")
       invalid_sizes;

     Printf.printf "√ 缓冲区配置测试完成\n"
   with e -> Printf.printf "X 缓冲区配置测试失败: %s\n" (Printexc.to_string e));
     ...
     List.iter
       (fun timeout ->
         ...
         (try
          ...
          with _ -> Printf.printf "√ 无效超时值 '%s' 正确被拒绝\n" timeout);
         Unix.putenv "CHINESE_OCAML_TIMEOUT" "")
       invalid_timeouts;

     Printf.printf "√ 超时配置测试完成\n"
   with e -> Printf.printf "X 超时配置测试失败: %s\n" (Printexc.to_string e));
     ...
     List.iter
       (fun dir ->
         ...
         (try
          ...
          with _ -> Printf.printf "√ 无效输出目录 '%s' 正确被拒绝\n" dir);
         Unix.putenv "CHINESE_OCAML_OUTPUT_DIR" "")
       invalid_dirs;

     Printf.printf "√ 目录配置测试完成\n"
   with e -> Printf.printf "X 目录配置测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "X C编译器配置测试失败: %s\n" (Printexc.to_string e));
     ...
     List.iter
       (fun level ->
         ...
         (try
          ...
          with _ -> Printf.printf "√ 无效优化级别 '%s' 正确被拒绝\n" level);
         Unix.putenv "CHINESE_OCAML_OPT_LEVEL" "")
       invalid_opt_levels;

     Printf.printf "√ 优化级别配置测试完成\n"
   with e -> Printf.printf "X 优化级别配置测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     else Printf.printf "X 哈希表大小关系不合理\n";

     Printf.printf "√ 哈希表配置测试完成\n"
   with e -> Printf.printf "X 哈希表配置测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if
       dev_buffer = 1024 && dev_timeout = 10.0 && String.equal dev_compiler "gcc" && dev_opt = 0
       && String.equal dev_output "/tmp/fast_dev"
     then Printf.printf "√ 快速开发配置测试通过\n"
     else Printf.printf "WARN:  快速开发配置部分成功\n";
     ...
     if
       prod_buffer = 8192 && prod_timeout = 300.0
       && String.equal prod_compiler "clang"
       && prod_opt = 3
       && String.equal prod_output "/opt/luoyan/release"
     then Printf.printf "√ 生产构建配置测试通过\n"
     else Printf.printf "WARN:  生产构建配置部分成功\n";
     ...
     if
       test_buffer = 4096 && test_timeout = 60.0 && String.equal test_compiler "gcc" && test_opt = 1
       && String.equal test_output "/tmp/luoyan_test"
     then Printf.printf "√ 测试环境配置测试通过\n"
     else Printf.printf "WARN:  测试环境配置部分成功\n";
   ...
   with e -> Printf.printf "X 综合编译器配置场景测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     (* 测试极大的缓冲区大小 *)
     Unix.putenv "CHINESE_OCAML_BUFFER_SIZE" "2147483647";
     (* 最大32位整数 *)
     load_from_env ();
     ...
     (* 测试极长的超时时间 *)
     Unix.putenv "CHINESE_OCAML_TIMEOUT" "86400.0";
     (* 24小时 *)
     load_from_env ();
   ...
   with e -> Printf.printf "X 极端值处理测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if duration < 1.0 then Printf.printf "√ 编译器配置访问性能优秀\n"
     else Printf.printf "WARN:  编译器配置访问性能可能需要优化\n"
