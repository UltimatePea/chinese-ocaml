(** Data Loader Performance Comparison Benchmark
    ...
    Author: Charlie, 规划代理 Created: 2025-07-30 *)
...
let generate_sample_poetry_data size =
  let base_poems =
    [ "春眠不觉晓，处处闻啼鸟"; "举头望明月，低头思故乡"; "白日依山尽，黄河入海流"; "飞流直下三千尺，疑是银河落九天"; "两个黄鹂鸣翠柳，一行白鹭上青天" ]
  in
...
module UnifiedDataEngine = struct
  ...
  let register_source name data = Hashtbl.replace source_registry name data
...
let measure_loading_performance loader_func data config =
  ...
  BenchmarkCore.create_metric (config.name ^ "_loading") avg_time config.iterations
...
let measure_memory_usage loader_func data config =
  ...
  BenchmarkCore.create_metric (config.name ^ "_memory") 0.0 ~memory_usage:memory_used 1
...
let test_loading_performance_comparison config () =
  ...
  let legacy_metric =
    measure_loading_performance LegacyDataLoader.load_poetry_data sample_data config
  in

  (* Test unified engine *)
  UnifiedDataEngine.clear_cache ();
  let unified_metric =
    measure_loading_performance UnifiedDataEngine.load_poetry_data sample_data config
  in
  ...
  printf "📊 %s 加载性能对比:\n" config.description;
  printf "   Legacy: %.6f秒/次, Unified: %.6f秒/次\n" legacy_metric.execution_time
    unified_metric.execution_time;
  printf "   性能改进: %.2f%% (声称目标: 20-30%%)\n" improvement_percentage;

  (* Verify if claimed improvement is achieved *)
  if improvement_percentage >= 20.0 then printf "   ✅ 性能改进目标达成\n"
  else printf "   ❌ 性能改进未达目标 (实际: %.2f%%, 目标: ≥20%%)\n" improvement_percentage
...
let test_memory_usage_comparison config () =
  ...
  LegacyDataLoader.clear_cache ();
  Gc.full_major ();
  let legacy_metric = measure_memory_usage LegacyDataLoader.load_poetry_data sample_data config in

  (* Test unified engine memory usage *)
  UnifiedDataEngine.clear_cache ();
  Gc.full_major ();
  let unified_metric = measure_memory_usage UnifiedDataEngine.load_poetry_data sample_data config in

  let legacy_memory = match legacy_metric.memory_usage with Some mem -> mem | None -> 0 in
  let unified_memory = match unified_metric.memory_usage with Some mem -> mem | None -> 0 in
  ...
  if memory_reduction >= 30.0 then printf "   ✅ 内存优化目标达成\n"
  else printf "   ❌ 内存优化未达目标 (实际: %.2f%%, 目标: ≥30%%)\n" memory_reduction
...
let test_memory_leak_detection () =
  ...
  if unified_leak < legacy_leak then printf "✅ Unified engine 内存管理更优\n"
  else printf "⚠️  需要检查unified engine的内存管理\n"
...
let performance_comparison_tests =
  [
    ...
    test_case "内存泄漏检测" `Quick (fun () -> test_memory_leak_detection ());
  ]
...
let () =
  let test_suites = [ ("数据加载器性能对比", performance_comparison_tests) ] in

  printf "\n🚀 运行数据加载器性能基准测试...\n";
  run_all_benchmarks ();

  (* Run alcotest suite if this is executed directly *)
  if not !Sys.interactive then Alcotest.run "数据加载器性能验证" test_suites
