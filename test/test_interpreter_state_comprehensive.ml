...
module TestUtils = struct
  ...
  let create_test_macro name body =
    {
      macro_def_name = name;
      params = [ExprParam "x"; ExprParam "y"];
      body = body;
    }
  ...
  (** 创建测试用的表达式 *)
  let create_test_expr () =
    VarExpr "test_var"

  (** 创建测试用的模块类型 *)
  let create_test_module_type () =
    Signature []
end
...
module MacroTableTests = struct
  ...
  let test_macro_replacement () =
    ...
    | (Some _, Some found) ->
        check string "宏名应保持不变" "replace_macro" found.macro_def_name
    | _ ->
        check bool "宏替换测试失败" false true
...
module ModuleTableTests = struct
  ...
  let test_module_replacement () =
    ...
    | _ ->
        check bool "模块替换测试失败" false true
  ...
  let test_empty_module_bindings () =
    ...
    match found_module with
    | Some bindings ->
        check int "空模块绑定数量应为0" 0 (List.length bindings)
    | None ->
        check bool "应该能找到空模块" false true
end
...
module RecursiveFunctionTests = struct
  ...
  let test_recursive_function_add_and_find () =
    ...
    | Some (IntValue 42) ->
        check bool "递归函数值应正确" true true
    | Some _ ->
        check bool "递归函数值类型错误" false true
    | None ->
        check bool "应该找到递归函数" false true
  ...
  let test_recursive_function_replacement () =
    ...
    (* 验证函数被替换 *)
    match (first_find, second_find) with
    | (Some (IntValue 1), Some (IntValue 2)) ->
        check bool "递归函数替换成功" true true
    | _ ->
        check bool "递归函数替换失败" false true
...
module FunctorTableTests = struct
  ...
  let test_functor_add_and_find () =
    ...
    | Some (param, _, _) ->
        check string "函子参数名应正确" "param" param
    | None ->
        check bool "应该找到函子" false true
  ...
  let test_functor_replacement () =
    ...
    | _ ->
        check bool "函子替换测试失败" false true
...
module StateResetTests = struct
  ...
  let test_reset_and_readd () =
    ...
    match found_macro with
    | Some found ->
        check string "重置后宏添加正常" "new_macro" found.macro_def_name
    | None ->
        check bool "重置后应能添加新宏" false true
end
...
module ComplexScenarioTests = struct
  ...
  let test_complex_state_operations () =
    ...
    | _ ->
        check bool "复杂场景测试失败" false true
  ...
  let test_edge_cases () =
    ...
    | Some found ->
        check string "空名称宏应能正常处理" "" found.macro_def_name
    | None ->
    ...
    | Some bindings ->
        check string "特殊字符模块名应能处理" "中文变量" (fst (List.hd bindings))
    | None ->
        check bool "特殊字符模块名查找失败" false true
end
