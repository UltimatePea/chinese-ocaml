...
let () =
  ...
  (try
     ...
     if buffer_size > 0 && timeout > 0.0 then Printf.printf "✅ 初始化后配置合理性检查通过\n"
     else Printf.printf "❌ 初始化后配置不合理\n"
   with e -> Printf.printf "❌ 配置初始化测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     let expected_updates =
       after_debug = true && after_verbose = false && after_buffer = 4096
       && abs_float (after_timeout -. 45.0) < 0.1
       && String.equal after_output "/tmp/unified_test"
       && String.equal after_compiler "clang"
       && after_opt_level = 2 && after_max_errors = 30 && after_colored = true
     in

     if expected_updates then Printf.printf "✅ 环境变量统一配置加载测试完全通过\n"
     else Printf.printf "⚠️  环境变量统一配置加载部分成功\n";

     (* 清理环境变量 *)
     List.iter (fun (var, _) -> Unix.putenv var "") env_vars;
     Printf.printf "🧹 环境变量清理完成\n"
   with e -> Printf.printf "❌ 环境变量统一配置加载测试失败: %s\n" (Printexc.to_string e));
  ...
  "spell_correction": true
     ...
     let json_updates_correct =
       after_debug = true && after_buffer = 8192
       && abs_float (after_timeout -. 120.0) < 0.1
       && String.equal after_output "/tmp/json_config_test"
       && String.equal after_compiler "gcc" && after_opt_level = 3 && after_colored = false
       && after_error_recovery = true
     in

     if json_updates_correct then Printf.printf "✅ JSON配置文件统一加载测试完全通过\n"
     else Printf.printf "⚠️  JSON配置文件统一加载部分成功\n";

     (* 清理配置文件 *)
     if Sys.file_exists config_file then Sys.remove config_file;
     Printf.printf "🧹 JSON配置文件清理完成\n"
   with e -> Printf.printf "❌ JSON配置文件统一加载测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if validation_errors = [] then Printf.printf "✅ 当前统一配置验证通过，无错误\n"
     else (
       Printf.printf "⚠️  统一配置验证发现 %d 个问题:\n" (List.length validation_errors);
       List.iteri (fun i error -> Printf.printf "  %d. %s\n" (i + 1) error) validation_errors);
     ...
     let invalid_env_tests =
       [
         ("CHINESE_OCAML_BUFFER_SIZE", "0");
         (* 无效：零或负数 *)
         ("CHINESE_OCAML_TIMEOUT", "-10.0");
         (* 无效：负数 *)
         ("CHINESE_OCAML_OPT_LEVEL", "10");
         (* 无效：超出范围 *)
         ("CHINESE_OCAML_MAX_ERRORS", "-1");
         (* 无效：负数 *)
       ]
     in
     ...
     List.iter
       (fun (var, invalid_value) ->
         ...
         (try
            ...
            if current_errors = [] then Printf.printf "⚠️  无效配置 %s='%s' 未被检测到\n" var invalid_value
            else Printf.printf "✅ 无效配置 %s='%s' 被正确检测\n" var invalid_value
          with e ->
            Printf.printf "✅ 无效配置 %s='%s' 导致异常: %s\n" var invalid_value (Printexc.to_string e));
         Unix.putenv var "")
       invalid_env_tests
   with e -> Printf.printf "❌ 统一配置验证测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "❌ 配置重置测试失败: %s\n" (Printexc.to_string e));
  ...
  "timeout": 30.0
     ...
     else Printf.printf "⚠️  配置优先级测试结果待验证\n";
     ...
     Unix.putenv "CHINESE_OCAML_BUFFER_SIZE" "";
     Unix.putenv "CHINESE_OCAML_TIMEOUT" ""
   with e -> Printf.printf "❌ 配置优先级测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if !inconsistency_count = 0 then Printf.printf "✅ 配置持久性和一致性测试通过\n"
     else Printf.printf "❌ 发现配置不一致问题\n";

     Unix.putenv "CHINESE_OCAML_DEBUG" "";
     Unix.putenv "CHINESE_OCAML_BUFFER_SIZE" ""
   with e -> Printf.printf "❌ 配置持久性测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if duration < 2.0 then Printf.printf "✅ 统一配置操作性能优秀\n" else Printf.printf "⚠️  统一配置操作性能可能需要优化\n";

     (* 清理 *)
     Unix.putenv "CHINESE_OCAML_DEBUG" "";
     Unix.putenv "CHINESE_OCAML_BUFFER_SIZE" ""
   with e -> Printf.printf "❌ 统一配置性能测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "⚠️  配置状态恢复过程中出现问题: %s\n" (Printexc.to_string e));
