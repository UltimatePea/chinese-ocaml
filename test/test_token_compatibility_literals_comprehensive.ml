(** Token兼容性字面量映射测试套件 - 全面覆盖所有映射功能
    ...
    测试目标: token_compatibility_literals.ml
    覆盖范围: 
    - 数字字面量映射（整数、浮点数）
    - 布尔字面量映射
    - 单位字面量映射  
...
let test_performance () =
  (* 测试大量映射操作的性能 *)
  let large_numbers = Array.init 1000 (fun i -> string_of_int (i + 1)) in (* 避免空字符串 *)
  Array.iter (fun num ->
    ignore (map_legacy_literal_to_unified num)
  ) large_numbers;
