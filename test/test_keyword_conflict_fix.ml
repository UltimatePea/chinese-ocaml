...
let test_zhe_token_is_wenyan_then () =
  ...
  | AncientDefineKeyword -> 
    fail "关键字冲突未修复：'者'仍然映射到AncientDefineKeyword"
  | _ ->
    fail "关键字冲突修复：'者'映射到未知token"
...
let test_ancient_function_uses_wenyan_then () =
  ...
  | Some AncientDefineKeyword ->
    fail "古雅体中的'者'错误映射到AncientDefineKeyword"
  | _ ->
    fail "古雅体测试中没有找到'者'对应的ThenWenyanKeyword"

let test_wenyan_conditional_uses_then () =
  let input = "若 条件 者 结果" in
  let token_list = Yyocamlc_lib.Lexer.tokenize input "test" in
  let tokens = List.map (fun (token, _) -> token) token_list in
  let third_token = List.nth tokens 2 in (* 者 - ThenWenyanKeyword *)
...
let () =
  run "关键字冲突修复测试" [
    ("词法分析器关键字冲突修复", keyword_conflict_tests);
  ]