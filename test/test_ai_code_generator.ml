...
let test_generation_request_creation () =
  ...
  test_assertion "请求创建"
    (request.description <> "" && request.target_type = Function)
    "请求对象创建失败";
...
let test_intent_analysis () =
  ...
  List.iter (fun (description, expected_type) ->
    let (actual_type, _) = analyze_generation_intent description in
    let matches = match (actual_type, expected_type) with
...
let test_function_generation () =
  ...
  List.iter (fun description ->
    ...
    test_assertion ("语法检查: " ^ description)
      ((try let _ = Str.search_forward (Str.regexp_string "让") result.generated_code 0 in true with Not_found -> false) ||
       (try let _ = Str.search_forward (Str.regexp_string "递归") result.generated_code 0 in true with Not_found -> false))
      "生成的代码不包含基本的骆言语法";
...
let test_algorithm_generation () =
  ...
  let algorithm_tests = [
    ...
    (Searching, "二分查找算法");
    (Recursive, "递归计算斐波那契");
    (Mathematical, "计算数学平均值");
  ] in

  List.iter (fun (alg_type, description) ->
    ...
    test_assertion ("算法复杂度: " ^ description)
      (result.quality_metrics.efficiency > 0.5)
      "算法效率评估过低"
  ) algorithm_tests
...
let test_data_processing_generation () =
  ...
  List.iter (fun (operations, description) ->
    let result = generate_data_processing_code operations description in

    test_assertion ("数据处理: " ^ description)
...
let test_template_matching () =
  ...
  let keywords_tests = [
    (["斐波那契"; "数列"], 1); (* 应该匹配到斐波那契模板 *)
    (["排序"; "快速"], 1);     (* 应该匹配到快速排序模板 *)
    (["求和"; "列表"], 1);     (* 应该匹配到求和模板 *)
    (["未知关键词"], 0);       (* 不应该匹配任何模板 *)
  ] in

  List.iter (fun (keywords, expected_matches) ->
    let matches = match_templates keywords function_templates in
    let actual_matches = List.length matches in

    test_assertion ("模板匹配: " ^ String.concat "," keywords)
      (if expected_matches = 0 then actual_matches = 0 else actual_matches > 0)
      ("期望匹配 " ^ string_of_int expected_matches ^ " 个模板，实际匹配 " ^ string_of_int actual_matches ^ " 个")
  ) keywords_tests
...
let test_intelligent_generation () =
  ...
  List.iter (fun description ->
    ...
    test_assertion ("置信度检查: " ^ description)
...
let test_multiple_candidates () =
  ...
  test_assertion "候选方案数量"
    (List.length candidates <= 3 && List.length candidates > 0)
    "候选方案数量不符合预期";
...
let test_optimization_suggestions () =
  ...
  List.iter (fun (code, expected_type) ->
    let suggestions = suggest_optimizations code in

    test_assertion ("优化建议: " ^ expected_type)
...
let test_error_cases () =
  ...
  let error_cases = [
    "";                                    (* 空描述 *)
    "完全无法理解的描述abc123xyz";          (* 无意义描述 *)
    "使用不存在的编程概念xyzabc";           (* 未知概念 *)
  ] in

  List.iter (fun description ->
    let result = intelligent_code_generation description () in

    test_assertion ("错误处理: " ^ (if description = "" then "空描述" else "无效描述"))
      (result.generated_code <> "")
      "错误情况未能生成兜底代码";

    test_assertion ("错误置信度: " ^ (if description = "" then "空描述" else "无效描述"))
...
let test_performance () =
  ...
  let descriptions = [
    "创建斐波那契函数"; "实现快速排序"; "计算平均值"; "过滤偶数"; "查找最大值";
    "字符串反转"; "二分查找"; "阶乘计算"; "列表求和"; "数据映射";
  ] in
...
let test_integration () =
  ...
  test_assertion "意图解析器集成"
    (List.length intent_result > 0)
    "与意图解析器集成失败";

  (* 使用代码生成器 *)
  let generation_result = intelligent_code_generation description () in
  test_assertion "代码生成器集成"
    (generation_result.generated_code <> "")
    "代码生成器集成失败";

  (* 比较两个模块的结果一致性 *)
  let intent_has_function = List.exists (fun s ->
    try let _ = Str.search_forward (Str.regexp_string "让") s.Ai.Intent_parser.code 0 in true with Not_found -> false
  ) intent_result in
  let generation_has_function =
    try let _ = Str.search_forward (Str.regexp_string "让") generation_result.generated_code 0 in true with Not_found -> false in

  test_assertion "模块结果一致性"
    (intent_has_function = generation_has_function)
    "两个AI模块的结果不一致";
