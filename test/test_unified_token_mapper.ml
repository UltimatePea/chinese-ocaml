...
let test_backward_compatibility () =
  ...
  List.iter (fun (name, expected_token) ->
    ...
    | Success actual_token ->
        Printf.printf "❌ %s: 不兼容 (期望: %s, 实际: %s)\n" 
          name (Lexer_tokens.show_token expected_token) (Lexer_tokens.show_token actual_token)
    | NotFound _ ->
        Printf.printf "❌ %s: 未找到\n" name
    | ConversionError (_, error) ->
        Printf.printf "❌ %s: 转换错误 - %s\n" name error
  ) legacy_tokens
...
let test_completeness () =
  ...
  (* 验证所有基础token类型都有映射 *)
  let required_tokens = [
    "IntToken"; "FloatToken"; "StringToken"; "BoolToken"; "ChineseNumberToken";
    ...
    "TypeKeyword"; "IntTypeKeyword"; "StringTypeKeyword";
    "Plus"; "Minus"; "Equal"; "Arrow";
  ] in
  
  let missing_tokens = List.filter (fun token_name ->
    match find_token_mapping token_name with
    | None -> true
    | Some _ -> false
  ) required_tokens in
  
  if missing_tokens = [] then
    Printf.printf "✅ 所有必需的token映射都已注册\n"
  else
    Printf.printf "❌ 缺少以下token映射: %s\n" (String.concat ", " missing_tokens)
