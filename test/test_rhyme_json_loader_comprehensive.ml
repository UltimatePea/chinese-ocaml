...
module JsonParsingTests = struct
  ...
  let test_basic_json_parsing () =
    ...
    with
    | Json_parse_error msg -> fail (Printf.sprintf "JSON解析失败: %s" msg)
    | exn -> fail (Printf.sprintf "意外异常: %s" (Printexc.to_string exn))
  ...
  let test_empty_json_handling () =
    ...
    with exn -> fail (Printf.sprintf "空JSON处理失败: %s" (Printexc.to_string exn))
  ...
  let test_invalid_json_error_handling () =
    ...
    | exn -> fail (Printf.sprintf "错误类型不匹配: %s" (Printexc.to_string exn))
...
module RhymeDataValidationTests = struct
  ...
  let test_rhyme_group_structure () =
    try
      ...
      List.iter
        ...
        data
    with exn -> fail (Printf.sprintf "韵组结构验证失败: %s" (Printexc.to_string exn))
  ...
  let test_rhyme_category_classification () =
    ...
    with exn -> fail (Printf.sprintf "韵类分类测试失败: %s" (Printexc.to_string exn))
  ...
  let test_character_uniqueness () =
    ...
    with exn -> fail (Printf.sprintf "字符唯一性测试失败: %s" (Printexc.to_string exn))
end
    ...
    | Some data ->
        ...
        List.iter
          (fun char ->
            ...
            with Rhyme_data_not_found _ -> fail (Printf.sprintf "字符 %s 应该存在" char))
          test_chars;
  ...
  let test_unicode_character_support () =
    ...
    try
      ...
      List.iter
        ...
        unicode_chars
    with exn -> fail (Printf.sprintf "Unicode测试失败: %s" (Printexc.to_string exn))
  ...
  let test_traditional_simplified_chinese () =
    ...
    try
      ...
      List.iter
        ...
        char_pairs
    with exn -> fail (Printf.sprintf "繁简字符测试失败: %s" (Printexc.to_string exn))
end
...
module PerformanceTests = struct
  ...
  let test_large_data_processing () =
    ...
    with exn -> fail (Printf.sprintf "大规模数据测试失败: %s" (Printexc.to_string exn))
  ...
  let test_memory_usage () =
    ...
    with exn -> fail (Printf.sprintf "内存使用测试失败: %s" (Printexc.to_string exn))
end
...
module EdgeCaseTests = struct
  ...
  let test_extreme_string_lengths () =
    ...
    with exn -> fail (Printf.sprintf "长字符名测试失败: %s" (Printexc.to_string exn))
  ...
  let test_special_characters () =
    ...
    with exn -> fail (Printf.sprintf "特殊字符测试失败: %s" (Printexc.to_string exn))
  ...
  let test_error_recovery () =
    ...
    with Json_parse_error _ -> (
      ...
      with exn -> fail (Printf.sprintf "错误恢复失败: %s" (Printexc.to_string exn)))
end
