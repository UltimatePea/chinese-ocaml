(** 韵律JSON加载器全面测试模块 - Phase 25 测试覆盖率提升
    ...
    本测试模块专门针对 poetry/rhyme_json_loader.ml 进行深度测试，
    覆盖所有关键功能包括数据加载、错误处理、性能验证。
...
let large_rhyme_data = {|
{
  "rhyme_groups": {
|}
^ (String.concat "," (List.init 100 (fun i -> 
    Printf.sprintf {|"group_%d": {"category": "平声", "characters": %s}|} i
      (String.concat "," (List.map (Printf.sprintf "\"%s\"") 
        (List.init 10 (fun j -> Printf.sprintf "字%d_%d" i j))))
  ))) ^ {|
  }
}
|}
...
module JsonParsingTests = struct
  ...
  let test_basic_json_parsing () =
    ...
    | Json_parse_error msg -> 
        fail (Printf.sprintf "JSON解析失败: %s" msg)
    | exn ->
        fail (Printf.sprintf "意外异常: %s" (Printexc.to_string exn))
  ...
  let test_empty_json_handling () =
    ...
    with
    | exn ->
        fail (Printf.sprintf "空JSON处理失败: %s" (Printexc.to_string exn))
  ...
  let test_invalid_json_error_handling () =
    ...
    with
    | Json_parse_error _ -> 
        check bool "正确检测到JSON错误" true true
    | exn ->
        fail (Printf.sprintf "错误类型不匹配: %s" (Printexc.to_string exn))

  (** 测试格式错误的JSON *)
  let test_malformed_json () =
    let malformed_cases = [
      ("{", "不完整的JSON");
      ("invalid json", "无效的JSON语法");
      ("{\"key\"}", "缺少值的JSON");
      ("", "空字符串");
    ] in
    List.iter (fun (json, desc) ->
      try
        ...
        fail (Printf.sprintf "%s 应该产生错误" desc)
      with
      | Json_parse_error _ -> ()
      | exn -> fail (Printf.sprintf "%s 错误类型不正确: %s" desc (Printexc.to_string exn))
    ) malformed_cases
...
module RhymeDataValidationTests = struct
  ...
  let test_rhyme_group_structure () =
    ...
    with
    | exn -> fail (Printf.sprintf "韵组结构验证失败: %s" (Printexc.to_string exn))
  ...
  let test_rhyme_category_classification () =
    ...
    with
    | exn -> fail (Printf.sprintf "韵类分类测试失败: %s" (Printexc.to_string exn))
  ...
  let test_character_uniqueness () =
    ...
    with
    | exn -> fail (Printf.sprintf "字符唯一性测试失败: %s" (Printexc.to_string exn))
...
module QueryFunctionTests = struct
  ...
  let setup_test_data () =
    try
      Some (parse_rhyme_json sample_rhyme_data)
    with
    | _ -> None
    ...
    | Some data ->
        ...
        List.iter (fun char ->
          try
            ...
            check bool (Printf.sprintf "找到字符 %s" char) true (rhyme_info <> None)
          with
          | Rhyme_data_not_found _ -> fail (Printf.sprintf "字符 %s 应该存在" char)
        ) test_chars;
        ...
        List.iter (fun char ->
          try
            let rhyme_info = lookup_character lookup_table char in
            check bool (Printf.sprintf "字符 %s 不应该存在" char) true (rhyme_info = None)
          with
          | Rhyme_data_not_found _ -> () (* 预期的异常 *)
        ) non_existent_chars
  ...
  let test_rhyme_group_lookup () =
    ...
    | Some data ->
        ...
        List.iter (fun char ->
          ...
          check bool (Printf.sprintf "字符 %s 应属于鱼韵" char) true (found_group <> None)
        ) fish_rhyme_chars
  ...
  let test_rhyme_matching () =
    ...
    | Some data ->
        ...
        List.iter (fun (char1, char2, should_match) ->
          let matches = characters_rhyme lookup_table char1 char2 in
          let desc = Printf.sprintf "%s 和 %s %s" char1 char2 
            (if should_match then "应该押韵" else "不应该押韵") in
...
module UnicodeTests = struct
  ...
  let test_unicode_character_support () =
    ...
    try
      ...
      List.iter (fun char ->
        check bool (Printf.sprintf "Unicode字符 %s 被正确处理" char) true (List.mem char all_chars)
      ) unicode_chars
    with
    | exn -> fail (Printf.sprintf "Unicode测试失败: %s" (Printexc.to_string exn))
  ...
  let test_traditional_simplified_chinese () =
    ...
    try
      ...
      List.iter (fun (simp, trad) ->
        let simp_info = lookup_character lookup_table simp in
        let trad_info = lookup_character lookup_table trad in
        check bool (Printf.sprintf "简体字 %s 被识别" simp) true (simp_info <> None);
        check bool (Printf.sprintf "繁体字 %s 被识别" trad) true (trad_info <> None)
      ) char_pairs
    with
    | exn -> fail (Printf.sprintf "繁简字符测试失败: %s" (Printexc.to_string exn))
...
module PerformanceTests = struct
  ...
  let test_large_data_processing () =
    ...
    with
    | exn -> fail (Printf.sprintf "大规模数据测试失败: %s" (Printexc.to_string exn))

  (** 测试查询性能 *)
  let test_query_performance () =
    match JsonParsingTests.test_basic_json_parsing (); parse_rhyme_json sample_rhyme_data with
    | data ->
        let lookup_table = create_lookup_table data in
        ...
        for _i = 1 to 1000 do
          List.iter (fun char ->
            ignore (lookup_character lookup_table char)
          ) test_chars
        done;
    ...
    | exception exn -> fail (Printf.sprintf "查询性能测试失败: %s" (Printexc.to_string exn))
  ...
  let test_memory_usage () =
    ...
    with
    | exn -> fail (Printf.sprintf "内存使用测试失败: %s" (Printexc.to_string exn))
...
module EdgeCaseTests = struct
  ...
  let test_extreme_string_lengths () =
    ...
    let long_char_data = Printf.sprintf {|
    {
      "rhyme_groups": {
        "test": {
          "category": "平声",
    ...
    with
    | exn -> fail (Printf.sprintf "长字符名测试失败: %s" (Printexc.to_string exn))
  ...
  let test_special_characters () =
    ...
    with
    | exn -> fail (Printf.sprintf "特殊字符测试失败: %s" (Printexc.to_string exn))
  ...
  let test_error_recovery () =
    ...
    | Json_parse_error _ ->
        ...
        with
        | exn -> fail (Printf.sprintf "错误恢复失败: %s" (Printexc.to_string exn))
