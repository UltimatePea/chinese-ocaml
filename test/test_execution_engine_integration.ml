...
let execute_test_program program =
  try
    ...
    result
  with e -> Error ("执行错误: " ^ Printexc.to_string e)
...
let test_simple_arithmetic_program () =
  ...
  match result with Ok (IntValue 80) -> () (* (15 + 25) * 2 = 80 *) | _ -> failwith "简单算术程序集成执行失败"
...
let test_string_processing_program () =
  ...
  let program =
    [
      ...
      LetStmt ("分隔符", str_lit " - ");
      LetStmt ("完整名称", BinaryOpExpr (BinaryOpExpr (var "前缀", Add, var "分隔符"), Add, var "后缀"));
      ExprStmt (var "完整名称");
    ]
  in
  let result = execute_test_program program in
  match result with Ok (StringValue "骆言 - 编程语言") -> () | _ -> failwith "字符串处理程序集成执行失败"
...
let test_conditional_execution_program () =
  ...
  let program =
    [
      ...
      LetStmt
        ("分类", CondExpr (BinaryOpExpr (var "年龄", Ge, int_lit 18), str_lit "成年人", str_lit "未成年人"));
      ExprStmt (var "分类");
    ]
  in
  let result = execute_test_program program in
  match result with Ok (StringValue "成年人") -> () | _ -> failwith "条件执行程序集成失败"
...
let test_function_definition_and_call () =
  ...
  let program =
    [
      LetStmt ("加法函数", FunExpr ([ "a"; "b" ], BinaryOpExpr (var "a", Add, var "b")));
      LetStmt ("结果", FunCallExpr (var "加法函数", [ int_lit 10; int_lit 20 ]));
      ExprStmt (var "结果");
    ]
  in
  let result = execute_test_program program in
  match result with Ok (IntValue 30) -> () | _ -> failwith "函数定义和调用集成失败"
...
let test_recursive_function_integration () =
      ...
      RecLetStmt
        ( "阶乘",
          FunExpr
            ( [ "n" ],
              CondExpr
                ( BinaryOpExpr (var "n", Le, int_lit 1),
                  int_lit 1,
                  BinaryOpExpr
                    ( var "n",
                      Mul,
  ...
  in
  let result = execute_test_program program in
  match result with Ok (IntValue 120) -> () (* 5! = 120 *) | _ -> failwith "递归函数集成失败"

let test_higher_order_function () =
  (* 测试高阶函数集成 *)
  let program =
    [
      LetStmt ("应用函数", FunExpr ([ "函数"; "值" ], FunCallExpr (var "函数", [ var "值" ])));
      LetStmt ("平方函数", FunExpr ([ "x" ], BinaryOpExpr (var "x", Mul, var "x")));
      LetStmt ("结果", FunCallExpr (var "应用函数", [ var "平方函数"; int_lit 7 ]));
      ExprStmt (var "结果");
    ]
  in
  let result = execute_test_program program in
  match result with Ok (IntValue 49) -> () (* 7^2 = 49 *) | _ -> failwith "高阶函数集成失败"
...
let test_simple_pattern_matching () =
  ...
  in
  let result = execute_test_program program in
  match result with Ok (StringValue "答案") -> () | _ -> failwith "简单模式匹配集成失败"
...
let test_list_pattern_matching () =
          ...
          MatchExpr
            ( var "测试列表",
              [
                ...
                {
                  pattern = ConsPattern (VarPattern "头", VarPattern "尾");
                  guard = None;
                  expr = BinaryOpExpr (str_lit "头部: ", Add, FunCallExpr (var "整数转字符串", [ var "头" ]));
                };
              ] ) );
      ExprStmt (var "处理结果");
    ]
  in
  let result = execute_test_program program in
  match result with Ok (StringValue "头部: 1") -> () | _ -> failwith "列表模式匹配集成失败"
...
let test_guard_pattern_matching () =
  ...
  in
  let result = execute_test_program program in
  match result with Ok (StringValue "大于10") -> () | _ -> failwith "带guard的模式匹配集成失败"
...
let test_algebraic_type_definition () =
  ...
  let program =
    [
      TypeDefStmt
        ( "结果",
          AlgebraicType
            [ ("成功", Some (BaseTypeExpr IntType)); ("失败", Some (BaseTypeExpr StringType)) ] );
        ...
        ( "处理成功",
          MatchExpr
            ( var "成功值",
              [
                {
                  pattern = ConstructorPattern ("成功", [ VarPattern "值" ]);
                  guard = None;
                  expr = BinaryOpExpr (str_lit "成功: ", Add, FunCallExpr (var "整数转字符串", [ var "值" ]));
                };
                {
                  pattern = ConstructorPattern ("失败", [ VarPattern "错误" ]);
                  guard = None;
  ...
  in
  let result = execute_test_program program in
  match result with Ok (StringValue "成功: 42") -> () | _ -> failwith "代数类型定义集成失败"
...
let test_exception_definition_and_handling () =
  ...
  let program =
    [
      ...
      LetStmt ("抛出异常", FunExpr ([ "信息" ], RaiseExpr (FunCallExpr (var "自定义错误", [ var "信息" ]))));
      LetStmt
        ( "处理结果",
          TryExpr
  ...
  in
  let result = execute_test_program program in
  match result with Ok (StringValue "捕获到: 测试异常") -> () | _ -> failwith "异常定义和处理集成失败"
        ...
        ( "描述",
          BinaryOpExpr
            ( BinaryOpExpr (var "获取姓名", Add, str_lit "今年"),
              Add,
              BinaryOpExpr (FunCallExpr (var "整数转字符串", [ var "获取年龄" ]), Add, str_lit "岁") ) );
      ExprStmt (var "描述");
    ]
  in
  let result = execute_test_program program in
  match result with Ok (StringValue "张三今年30岁") -> () | _ -> failwith "记录操作集成失败"
...
let test_error_recovery_integration () =
  ...
  let program =
    [
      LetStmt ("容错函数", FunExpr ([ "x" ], BinaryOpExpr (var "x", Add, int_lit 10)));
      (* 调用时参数过多，但启用了错误恢复 *)
  ...
  in
  let result = execute_test_program program in
  match result with Ok (IntValue 15) -> () (* 只使用第一个参数: 5 + 10 = 15 *) | _ -> failwith "错误恢复集成失败"
...
let test_nested_function_calls () =
    ...
    [
      ...
      LetStmt
        ( "复合计算",
          FunExpr
            ( [ "n" ],
              FunCallExpr (var "乘法", [ FunCallExpr (var "加法", [ var "n"; int_lit 5 ]); int_lit 3 ])
            ) );
  ...
  in
  let result = execute_test_program program in
  match result with Ok (IntValue 36) -> () (* (7 + 5) * 3 = 36 *) | _ -> failwith "嵌套函数调用集成失败"
          ...
          FunExpr
            ( [ "参数" ],
              LetExpr
                ( "局部变量",
                  int_lit 50,
                  BinaryOpExpr (BinaryOpExpr (var "全局变量", Add, var "局部变量"), Add, var "参数") ) ) );
...
let test_closure_capture_integration () =
      ...
      LetStmt
        ( "闭包生成器",
          FunExpr
            ( [ "增量" ],
              FunExpr
                ([ "输入" ], BinaryOpExpr (BinaryOpExpr (var "外部值", Add, var "增量"), Add, var "输入")) )
        );
  ...
  in
  let result = execute_test_program program in
  match result with Ok (IntValue 35) -> () (* 10 + 20 + 5 = 35 *) | _ -> failwith "闭包捕获集成失败"
...
let test_tail_recursion_integration () =
      ...
      RecLetStmt
        ( "尾递归累加",
          FunExpr
            ( [ "n"; "累计" ],
              CondExpr
                ( BinaryOpExpr (var "n", Le, int_lit 0),
                  var "累计",
                  FunCallExpr
                    ( var "尾递归累加",
                      [
                        BinaryOpExpr (var "n", Sub, int_lit 1); BinaryOpExpr (var "累计", Add, var "n");
                      ] ) ) ) );
  ...
  in
  let result = execute_test_program program in
  match result with Ok (IntValue 55) -> () (* 1+2+...+10 = 55 *) | _ -> failwith "尾递归集成失败"
...
let test_complete_mini_application () =
  ...
  let program =
    [
      (* 定义数据类型 *)
      TypeDefStmt
        ("用户", RecordType [ ("姓名", BaseTypeExpr StringType); ("余额", BaseTypeExpr IntType) ]);
      ExceptionDefStmt ("余额不足", Some (BaseTypeExpr StringType));
      (* 定义操作函数 *)
      LetStmt
        ("创建用户", FunExpr ([ "姓名"; "初始余额" ], RecordExpr [ ("姓名", var "姓名"); ("余额", var "初始余额") ]));
      LetStmt
        ( "提取资金",
          FunExpr
            ( [ "用户"; "金额" ],
              LetExpr
                ( "当前余额",
                  FieldAccessExpr (var "用户", "余额"),
                  CondExpr
                    ( BinaryOpExpr (var "当前余额", Ge, var "金额"),
                      RecordUpdateExpr
                        (var "用户", [ ("余额", BinaryOpExpr (var "当前余额", Sub, var "金额")) ]),
  ...
  in
  let result = execute_test_program program in
  match result with Ok (IntValue 700) -> () (* 1000 - 300 = 700 *) | _ -> failwith "完整迷你应用程序集成失败"
