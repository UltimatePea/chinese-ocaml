(** Integration test for Issue #75 - Unicode字符处理优化
    ...
    This test verifies that the enhanced Unicode processing fixes Issue #75
    where fullwidth Unicode digits caused infinite loops in the lexer.
    
    Author: Whisky, PR Worker
    Issue: #1847 - Unicode字符处理优化 (fixes #75)
...
let test_fullwidth_digits_no_infinite_loop () =
  ...
  List.iter (fun (input, description) ->
    ...
    | exn ->
        ...
        failwith (Printf.sprintf "❌ %s: Unexpected exception: %s" description (Printexc.to_string exn))
  ) test_cases
...
let test_enhanced_unicode_processing_active () =
  ...
  List.iter (fun input ->
    try
      ...
      Printf.printf "✅ Enhanced processing working for: %s (got %d tokens)\n" input (List.length tokens)
    with
    | exn ->
        failwith (Printf.sprintf "❌ Enhanced processing failed for supported input '%s': %s" input (Printexc.to_string exn))
  ) supported_inputs
...
let test_issue_75_specific_case () =
  ...
  try
    ...
    if duration > 1.0 then
      failwith (Printf.sprintf "❌ Issue #75 not fully fixed: took %.2f seconds (possible infinite loop)" duration)
    else
      Printf.printf "✅ Issue #75 FIXED: Processed in %.4f seconds (no infinite loop)\n" duration
  ...
  | Lexer_tokens.LexError (msg, _pos) ->
      ...
      if duration > 1.0 then
        failwith (Printf.sprintf "❌ Issue #75 not fully fixed: took %.2f seconds even with error" duration)
      else
...
let test_performance_no_regression () =
  ...
  Printf.printf "✅ Performance test: %d iterations in %.4f seconds (avg: %.6f seconds per iteration)\n" 
