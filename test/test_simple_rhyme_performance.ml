(** 简单韵律性能基准测试 - 修复 Issue #1463
    ...
    Author: Alpha, 主工作代理 Fix #1463 - 添加性能基准测试验证 CI rebuild trigger: 2025-07-27 *)
...
let test_analyze_performance () =
  benchmark "韵律数据分析" 1000 (fun () -> ignore (analyze_rhyme_data test_entries))

(** 主测试函数 *)
let run_performance_tests () =
  Printf.printf "=== 简化韵律模块性能基准测试 ===\n";
  Printf.printf "测试数据: %d 个字符, %d 个韵律条目\n" (List.length sample_characters)
    (List.length test_entries);
  Printf.printf "\n";
