...
let test_labeled_function_call () =
  ...
  let expected =
    [
      ExprStmt
        (LabeledFunCallExpr (VarExpr "加法", [ { arg_label = "x"; arg_value = LitExpr (IntLit 3) } ]));
    ]
  in
...
let test_labeled_parameter_with_type () =
  ...
  try
    ...
    let _ = ast in
    print_endline "通过"
  with e -> Printf.printf "失败: %s\n" (Printexc.to_string e)

let run_tests () =
  print_endline "=== 标签参数系统测试 ===";
  (try test_labeled_function_definition ()
   with e -> print_endline ("标签函数定义测试失败: " ^ Printexc.to_string e));

  (try test_labeled_function_call ()
   with e -> print_endline ("标签函数调用测试失败: " ^ Printexc.to_string e));

  (try test_optional_labeled_parameter ()
   with e -> print_endline ("可选标签参数测试失败: " ^ Printexc.to_string e));

  (try test_labeled_parameter_with_default ()
   with e -> print_endline ("带默认值的标签参数测试失败: " ^ Printexc.to_string e));

  (try test_labeled_parameter_with_type ()
   with e -> print_endline ("带类型注解的标签参数测试失败: " ^ Printexc.to_string e));
