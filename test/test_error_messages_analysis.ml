(** 错误消息分析模块测试套件
    ...
    验证error_messages_analysis.ml模块的智能错误分析功能
    包括字符串相似度计算、错误分析、建议生成和修复提示
...
let test_pattern_match_analysis () =
  ...
  check (bool) "应有通配符模式建议" true (List.exists (fun hint -> String.contains hint '_') empty_patterns_analysis.fix_hints);
...
let test_intelligent_error_analysis () =
  ...
  (* 测试未定义变量错误 *)
  let undef_analysis = intelligent_error_analysis "undefined_variable" ["test_var"; "var1;var2;var3"] (Some "测试上下文") in
  check (string) "未定义变量错误类型应正确" "undefined_variable" undef_analysis.error_type;
  
  (* 测试类型不匹配错误 *)
  let type_analysis = intelligent_error_analysis "type_mismatch" ["整数类型"; "字符串类型"] (Some "类型检查") in
  check (string) "类型不匹配错误类型应正确" "type_mismatch" type_analysis.error_type;
  
  (* 测试函数参数错误 *)
  let arity_analysis = intelligent_error_analysis "function_arity" ["3"; "1"; "test_func"] (Some "函数调用") in
  check (string) "函数参数错误类型应正确" "function_arity" arity_analysis.error_type;
  
  (* 测试模式匹配错误 *)
  let pattern_analysis = intelligent_error_analysis "pattern_match" ["Some _"; "None"] (Some "匹配表达式") in
  check (string) "模式匹配错误类型应正确" "pattern_match" pattern_analysis.error_type;
  
  (* 测试未知错误类型 *)
  let unknown_analysis = intelligent_error_analysis "unknown_error" ["detail1"; "detail2"] (Some "未知上下文") in
...
let test_error_details_parsing () =
  ...
  (* 测试未定义变量详情解析 *)
  let var_name, available_vars = parse_undefined_variable_details ["test_var"; "var1;var2;var3"] in
