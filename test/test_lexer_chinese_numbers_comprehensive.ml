...
let test_convert_integer_sequences () =
  ...
  check token_testable "convert 三万五千二百一十" (IntToken 35210) (convert_chinese_number_sequence "三万五千二百一十")
...
let test_large_numbers () =
  check token_testable "convert 九万九千九百九十九" (IntToken 99999) (convert_chinese_number_sequence "九万九千九百九十九");
...
let test_lexer_state_updates () =
  ...
  check int "position updated correctly" 15 new_state.position; (* 5 UTF-8 chars × 3 bytes each *)
  check int "column updated correctly" 16 new_state.current_column; (* original column + byte difference *)
...
let utf8_parsing_tests = [
  test_case "utf8_to_char_list parsing" `Quick test_utf8_to_char_list;
]
...
let state_management_tests = [
  test_case "lexer state updates" `Quick test_lexer_state_updates;
]
