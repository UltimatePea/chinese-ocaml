(** AST全面测试模块 - Phase 1测试覆盖率提升
    ...
    针对AST模块的全面测试，覆盖所有主要类型和函数 目标：提升AST模块测试覆盖率到60%+
...
let test_type_expressions_comprehensive () =
  let simple_types =
    [ BaseTypeExpr IntType; BaseTypeExpr StringType; TypeVar "'a"; TypeVar "'b" ]
  in
...
let test_poetry_forms_comprehensive () =
  ...
  List.iteri (fun i form -> check bool ("诗词形式" ^ string_of_int i ^ "相等性") true (form = form)) forms
...
let test_tone_constraints_comprehensive () =
  ...
  List.iter
    (fun constraint_val ->
      ...
      | SpecificPattern pattern -> check bool "特定模式约束" true (List.length pattern >= 0))
    constraints
...
let test_tone_patterns_comprehensive () =
  let patterns =
    [
      ...
      {
        tone_sequence = [ LevelTone; LevelTone; FallingTone; FallingTone ];
...
let test_meter_constraints_comprehensive () =
  let constraints =
    [
      ...
      {
        character_count = 4;
        syllable_pattern = Some "平仄平仄";
        caesura_position = Some 2;
        rhyme_scheme = Some "AABB";
      };
    ]
  in
...
let test_expression_nesting () =
  ...
  let tuple_expr =
    TupleExpr [ LitExpr (IntLit 1); LitExpr (StringLit "测试"); LitExpr (BoolLit true) ]
  in
