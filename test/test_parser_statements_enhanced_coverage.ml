(** 骆言语法分析器语句解析模块增强测试覆盖 - Fix #1304
    ...
    本测试文件专门提升 parser_statements.ml 的测试覆盖率， 重点测试边界情况、错误处理和复杂语句解析场景。
...
let test_macro_params_complex () =
  ...
  let tokens =
    TH.simple_tokens
      [
        QuotedIdentifierToken "参数1";
        Colon;
        QuotedIdentifierToken "表达式";
        Comma;
        QuotedIdentifierToken "参数2";
        Colon;
        QuotedIdentifierToken "语句";
        Comma;
        QuotedIdentifierToken "参数3";
        Colon;
  ...
  let param_names =
    List.map
      (function ExprParam n -> "表达式:" ^ n | StmtParam n -> "语句:" ^ n | TypeParam n -> "类型:" ^ n)
      params
  in
...
let test_macro_params_error_cases () =
  ...
  (* 测试无效参数类型 *)
  let tokens =
    TH.simple_tokens [ QuotedIdentifierToken "参数"; Colon; QuotedIdentifierToken "无效类型"; EOF ]
  in
  let state = TH.create_state tokens in
...
let test_statement_parsing_complex () =
  ...
  (* 测试简单Let语句解析 *)
  let tokens =
    TH.simple_tokens [ LetKeyword; QuotedIdentifierToken "变量"; AsForKeyword; IntToken 42; EOF ]
  in
  let state = TH.create_state tokens in
...
(** 运行测试 *)
let () = run "骆言Parser语句模块增强测试覆盖 - Fix #1304" [ ("增强测试覆盖", enhanced_coverage_tests) ]
