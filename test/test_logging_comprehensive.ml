...
let test_performance_monitoring () =
  ...
  (* 测试性能测量 *)
  let result = Luoyan_logging.Log_core.time_operation "test_module" "test_operation" (fun () ->
    (* 模拟一些工作 *)
    Unix.sleepf 0.001;
    42
  ) in
...
let test_log_messages () =
  ...
  let arity_mismatch_msg = Luoyan_logging.Log_messages.Error.function_arity_mismatch "test_func" 2 3 in
  ...
  let module_member_msg = Luoyan_logging.Log_messages.Error.module_member_not_found "Test" "member" in
  ...
  let function_call_msg = Luoyan_logging.Log_messages.Debug.function_call "test_func" ["arg1"; "arg2"] in
