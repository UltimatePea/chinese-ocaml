...
let () =
  ...
  let original_env_values = List.map (fun var ->
    (var, try Some (Sys.getenv var) with Not_found -> None)
  ) test_env_vars in
  ...
  (try
    ...
    List.iter (fun (var, value, expected) ->
  ...
  (try
    let int_test_cases = [
      ...
      ("CHINESE_OCAML_BUFFER_SIZE", "0", None);      (* 非正数应被拒绝 *)
      ("CHINESE_OCAML_BUFFER_SIZE", "-100", None);   (* 负数应被拒绝 *)
      ("CHINESE_OCAML_BUFFER_SIZE", "abc", None);    (* 非数字应被拒绝 *)
      ("CHINESE_OCAML_BUFFER_SIZE", "2147483648", None); (* 溢出应被处理 *)
    ...
    List.iter (fun (var, value, expected) ->
      ...
      | (Some r, Some e) when r = e ->
          Printf.printf "✅ %s='%s' -> %d\n" var value r
      | (None, None) ->
          Printf.printf "✅ %s='%s' 正确被拒绝\n" var value
      | (Some r, None) ->
          Printf.printf "❌ %s='%s' 应被拒绝，但得到 %d\n" var value r
      | (None, Some e) ->
  ...
  (try
    let float_test_cases = [
      ...
      ("CHINESE_OCAML_TIMEOUT", "0.0", None);     (* 非正数应被拒绝 *)
      ("CHINESE_OCAML_TIMEOUT", "-10.5", None);   (* 负数应被拒绝 *)
      ("CHINESE_OCAML_TIMEOUT", "abc", None);     (* 非数字应被拒绝 *)
  ...
  (try
    let string_test_cases = [
      ...
      ("CHINESE_OCAML_OUTPUT_DIR", "", None);           (* 空字符串应被拒绝 *)
      ("CHINESE_OCAML_OUTPUT_DIR", "   ", None);        (* 只有空格应被拒绝 *)
  ...
  (try
    let range_test_cases = [
      ...
      ("CHINESE_OCAML_OPT_LEVEL", "4", 0, 3, None);    (* 超出范围 *)
      ("CHINESE_OCAML_OPT_LEVEL", "-1", 0, 3, None);   (* 低于范围 *)
      ...
      ("CHINESE_OCAML_MAX_ERRORS", "101", 1, 100, None); (* 超出范围 *)
    ] in
    
    List.iter (fun (var, value, min_val, max_val, expected) ->
      Unix.putenv var value;
  ...
  (try
    let enum_test_cases = [
      ...
      ("CHINESE_OCAML_LOG_LEVEL", "DEBUG", ["debug"; "info"; "warn"; "error"], None); (* 大小写敏感 *)
    ] in
    
    List.iter (fun (var, value, valid_values, expected) ->
      Unix.putenv var value;
  ...
  (try
    ...
    if key_updates_correct then
      Printf.printf "✅ 综合环境变量配置加载测试完全通过\n"
    else
      Printf.printf "⚠️  综合环境变量配置加载部分成功（某些值可能有系统限制）\n";
  with
  | e -> Printf.printf "❌ 综合环境变量配置加载测试失败: %s\n" (Printexc.to_string e));

  (* 测试无效环境变量的处理 *)
  Printf.printf "\n❌ 测试无效环境变量处理\n";
  (try
    let invalid_env_tests = [
      ("CHINESE_OCAML_DEBUG", "maybe");        (* 无效布尔值 *)
      ("CHINESE_OCAML_BUFFER_SIZE", "huge");   (* 无效整数 *)
      ("CHINESE_OCAML_TIMEOUT", "forever");    (* 无效浮点数 *)
      ("CHINESE_OCAML_OUTPUT_DIR", "");        (* 空字符串 *)
      ("CHINESE_OCAML_OPT_LEVEL", "10");       (* 超出范围 *)
      ("CHINESE_OCAML_LOG_LEVEL", "trace");    (* 无效枚举值 *)
    ] in
  ...
  (try
    ...
    if duration < 5.0 then
      Printf.printf "✅ 环境变量处理性能良好\n"
    else
      Printf.printf "⚠️  环境变量处理性能可能需要优化\n";
  with
  ...
  with
  | e -> Printf.printf "⚠️  环境变量状态恢复过程中出现问题: %s\n" (Printexc.to_string e));
