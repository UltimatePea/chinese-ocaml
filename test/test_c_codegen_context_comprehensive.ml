...
let test_escape_identifier () =
  ...
  check string "特殊字符转义"
    "\231\137\185\230\174\138_dash_\229\173\151\231\172\166_dot_\230\181\139\232\175\149" special_id;
...
let test_c_type_of_luoyan_type () =
  ...
  (* 测试函数类型转换 *)
  let func_type =
    c_type_of_luoyan_type (FunType_T (TupleType_T [ IntType_T; StringType_T ], BoolType_T))
  in
  check string "函数类型转换" "luoyan_function_t*" func_type;

  (* 测试记录类型转换 *)
  let record_type =
    c_type_of_luoyan_type (RecordType_T [ ("字段1", IntType_T); ("字段2", StringType_T) ])
  in
  check string "记录类型转换" "luoyan_record_t*" record_type;

  (* 测试自定义类型转换 *)
  let custom_type = c_type_of_luoyan_type (ConstructType_T ("我的类型", [])) in
  check string "自定义类型转换" "luoyan_user_\230\136\145\231\154\132\231\177\187\229\158\139_t*"
    custom_type
...
let test_nested_type_conversion () =
  ...
  (* 测试复杂函数类型 *)
  let complex_func =
    c_type_of_luoyan_type (FunType_T (ListType_T IntType_T, ListType_T StringType_T))
  in
  check string "复杂函数类型转换" "luoyan_function_t*" complex_func;

  (* 测试复杂记录类型 *)
  let complex_record =
    c_type_of_luoyan_type
      (RecordType_T
         [
           ("整数", IntType_T);
...
let test_edge_cases () =
  ...
  (* 测试空配置字段 *)
  let empty_config =
    { c_output_file = ""; include_debug = false; optimize = false; runtime_path = "" }
  in
...
let test_type_conversion_edge_cases () =
  ...
  check string "递归自定义类型转换" "luoyan_user_\233\128\146\229\189\146\231\177\187\229\158\139_t*"
    recursive_custom;
...
let () =
  run "C_codegen_context 综合测试"
    [
      ...
      ( "名称生成",
        [ test_case "变量名生成" `Quick test_gen_var_name; test_case "标签名生成" `Quick test_gen_label_name ]
      );
      ( "标识符处理",
        [
      ...
      ("状态管理", [ test_case "上下文状态修改" `Quick test_context_state_modification ]);
      ( "性能测试",
        [
          test_case "大量变量名生成" `Slow test_large_var_generation;
          test_case "大量标签名生成" `Slow test_large_label_generation;
        ] );
      ("边界条件和错误处理", [ test_case "边界条件测试" `Quick test_edge_cases ]);
    ]
