...
let () =
  run "统一错误处理系统主入口模块测试"
    [
      ( "错误类型重新导出测试",
        [
          test_case "错误类型重新导出" `Quick test_error_types_export;
        ] );
      ( "转换函数重新导出测试",
        [
          test_case "转换函数重新导出" `Quick test_conversion_functions_export;
        ] );
      ( "基本函数导出测试",
        [
          test_case "基本函数导出" `Quick test_basic_functions_export;
        ] );
    ]