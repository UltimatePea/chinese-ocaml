  ...
  let wenyan_keyword_tokens =
    List.filter
      (function
        ...
        | Lexer.AsForKeyword | Lexer.NumberKeyword ->
            true
        | _ -> false)
      actual_keywords
  in
...
let test_wenyan_extended_lexer () =
  ...
  in
  let actual_keywords = List.map (fun (token, _) -> token) token_list in
  let wenyan_extended_tokens =
    List.filter
      (function
        | Lexer.WantExecuteKeyword | Lexer.MustFirstGetKeyword | Lexer.ForThisKeyword
        | Lexer.TimesKeyword | Lexer.EndCloudKeyword | Lexer.IfWenyanKeyword
        | Lexer.ThenWenyanKeyword | Lexer.GreaterThanWenyan | Lexer.LessThanWenyan
        | Lexer.OfParticle ->
            true
        | _ -> false)
      actual_keywords
...
let test_wenyan_full_parsing () =
  (* 测试吾有语句解析 *)
  let code = "吾有一数名曰「数值」其值「四」也在「数值」" in
  let tokens = Lexer.tokenize code "test_wenyan.ly" in
