...
module TestUtils = struct
  ...
  let check_token_not_equal desc left right =
    check bool desc false (equal_token left right)
...
module OperatorTokenTests = struct
  ...
  let test_arithmetic_operators () =
    ...
    (* 测试操作符令牌自身相等 *)
    List.iter2 (fun op token -> 
      TestUtils.check_token_equal (show_operator_token op ^ "令牌自身相等") token token
    ) ops op_tokens;
  ...
  let test_bitwise_operators () =
    let bitwise_ops = [BitwiseAnd; BitwiseOr; BitwiseXor; BitwiseNot; ShiftLeft; ShiftRight; ArithShiftRight] in
  ...
  let test_special_operators () =
    let special_ops = [Assign; Dereference; Reference; Compose; PipeForward; PipeBackward; Arrow; DoubleArrow] in
...
module KeywordTokenTests = struct
  ...
  let test_basic_keywords () =
    let basic_keywords = [LetKeyword; RecKeyword; InKeyword; FunKeyword; IfKeyword; ThenKeyword; ElseKeyword] in
    let keyword_tokens = List.map (fun kw -> KeywordToken kw) basic_keywords in
    
    (* 测试关键字令牌自身相等 *)
    List.iter2 (fun kw token ->
  ...
  let test_module_keywords () =
    let module_keywords = [ModuleKeyword; ModuleTypeKeyword; OpenKeyword; IncludeKeyword; SigKeyword; StructKeyword; EndKeyword; FunctorKeyword; ValKeyword; ExternalKeyword] in
  ...
  let test_classical_keywords () =
    let classical_keywords = [BeginKeyword; FinishKeyword; WenyanNow; WenyanHave; WenyanIs; ClassicalLet; ClassicalIn; ClassicalBe] in
  ...
  let test_poetry_keywords () =
    let poetry_keywords = [PoetryStart; PoetryEnd; VerseStart; VerseEnd; RhymePattern; TonePattern; ParallelStart; ParallelEnd] in
...
module DelimiterTokenTests = struct
  ...
  let test_ascii_delimiters () =
    let ascii_delims = [LeftParen; RightParen; LeftBracket; RightBracket; LeftBrace; RightBrace; Comma; Semicolon; Colon] in
    let ascii_tokens = List.map (fun del -> DelimiterToken del) ascii_delims in
    
    List.iter2 (fun del token ->
  ...
  let test_chinese_delimiters () =
    let chinese_delims = [ChineseLeftParen; ChineseRightParen; ChineseLeftBracket; ChineseRightBracket; ChineseComma; ChineseSemicolon; ChineseColon] in
  ...
  let test_special_delimiters () =
    let special_delims = [LeftArray; RightArray; AssignArrow; LeftQuote; RightQuote; ChineseDoubleColon; ChineseArrow; ChineseDoubleArrow] in
...
module TokenUtilityTests = struct
  ...
  let test_token_classification_comprehensive () =
    let test_cases = [
      ...
      (IdentifierToken (Identifiers.QuotedIdentifierToken "「x」"), "标识符", [TokenUtils.is_identifier]);
      (DelimiterToken Delimiters.LeftParen, "分隔符", [TokenUtils.is_delimiter]);
      (SpecialToken Special.EOF, "特殊令牌", [TokenUtils.is_special]);
    ] in
    
    List.iter (fun (token, desc, predicates) ->
      (* 测试正确的分类返回true *)
      List.iter (fun predicate ->
        check bool (desc ^ "分类正确") true (predicate token)
      ) predicates;
...
let () =
  run "骆言令牌类型系统综合测试" [
    ...
    "特殊令牌测试", [
      test_case "特殊令牌" `Quick SpecialTokenTests.test_special_tokens;
    ];
    "位置信息测试", [
