(** 骆言语法分析器运算符表达式解析模块测试 - 整合版 增强版
    ...
    本测试模块验证 parser_expressions_operators_consolidated.ml 的完整功能。 从基础接口测试扩展为全面的功能测试。
...
let test_unary_negation () =
  ...
  try
    let result, final_state =
      parse_unary_expression TestUtils.mock_parse_primary TestUtils.mock_parse_primary state
    in
...
let test_precedence_chain () =
  let tokens = [ TestUtils.make_token (IntToken 2) 1 1; TestUtils.make_token EOF 1 2 ] in
  let state = TestUtils.create_test_state tokens in
  try
    let ( parse_expr,
          _parse_or_else,
          ...
          _parse_mult,
          _parse_unary ) =
      create_operator_precedence_chain TestUtils.mock_parse_primary
    in
...
let multiplicative_tests =
  [
    test_case "简单乘法" `Quick test_simple_multiplication; test_case "简单除法" `Quick test_simple_division;
  ]
...
let logical_tests =
  [ test_case "逻辑与" `Quick test_logical_and; test_case "逻辑或" `Quick test_logical_or ]

let unary_tests =
  [ test_case "一元负号" `Quick test_unary_negation; test_case "一元非" `Quick test_unary_not ]
...
let () =
  run "Parser_expressions_operators_consolidated Enhanced Tests"
    [
      ("基础模块测试", basic_tests);
