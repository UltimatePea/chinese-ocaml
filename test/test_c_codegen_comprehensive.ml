(** 骆言编译器C代码生成器综合测试 为issue #749提升测试覆盖率至50%+ - C代码生成模块测试增强 *)
...
let check_c_output test_name expected_substrings actual_output =
  List.iter
    (fun substr ->
      check bool
        (Printf.sprintf "%s_contains_%s" test_name substr)
        true
        (string_contains actual_output substr))
    expected_substrings
...
let test_complex_expressions_codegen () =
  ...
  (* 测试函数调用链 *)
  let chained_call =
    FunCallExpr (VarExpr "f", [ FunCallExpr (VarExpr "g", [ LitExpr (IntLit 42) ]) ])
  in
  ...
  check_c_output "nested_conditional"
    [ "x"; "0"; "10"; "small positive"; "large positive" ]
    cond_code;
...
let test_function_definition_codegen () =
  ...
  (* 测试简单函数定义 *)
  let simple_func = FunExpr ([ "a"; "b" ], BinaryOpExpr (VarExpr "a", Add, VarExpr "b")) in
...
let test_performance_codegen () =
    ...
    List.init 100 (fun i ->
        LetStmt
          ( Printf.sprintf "func_%d" i,
            FunExpr ([ "x" ], BinaryOpExpr (VarExpr "x", Add, LitExpr (IntLit i))) ))
  in
  ...
  let complex_expr =
    List.fold_left
      (fun acc i ->
        BinaryOpExpr
          (acc, Mul, BinaryOpExpr (VarExpr (Printf.sprintf "x_%d" i), Add, LitExpr (IntLit i))))
      (LitExpr (IntLit 1))
  ...
  let nested_expr =
    CondExpr
      ( LitExpr (BoolLit true),
        LetExpr ("x", LitExpr (IntLit 1), LetExpr ("y", LitExpr (IntLit 2), VarExpr "y")),
        LitExpr (IntLit 0) )
  in
...
let test_end_to_end_codegen () =
  ...
  let complete_program =
    [
      LetStmt ("main", FunExpr ([], FunCallExpr (VarExpr "print", [ LitExpr (IntLit 42) ])));
      LetStmt
        ( "fibonacci",
          FunExpr
            ( [ "n" ],
