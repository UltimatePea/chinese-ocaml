...
let test_poem_structure_analysis () =
  ...
  test_assert "简单诗句字符数正确" (String.length simple_verse = 15); (* UTF-8编码下的字节数 *)
  
  (* 测试多句诗词结构 *)
  let verses = ["春眠不觉晓"; "处处闻啼鸟"; "夜来风雨声"; "花落知多少"] in
  test_assert "多句诗词结构完整" (List.length verses = 4);
  test_assert "每句长度一致" (List.for_all (fun v -> String.length v = String.length (List.hd verses)) verses)
