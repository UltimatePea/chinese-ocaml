...
let test_suggestion_statistics () =
  ...
  try
    ...
    (* 验证基本统计信息结构 *)
    check bool "statistics should be tuple" true 
      (match by_type with (_, _, _, _) -> true);
    check bool "priority stats should be tuple" true
      (match by_priority with (_, _, _) -> true)
  with
  | _ -> (* 如果统计失败，至少确保函数可以调用 *)
