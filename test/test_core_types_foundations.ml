...
module TestUtils = struct
  ...
  let check_type_equal desc expected actual =
    check bool desc true (equal_typ expected actual)
  
  (** 检查类型不相等 *)
  let check_type_not_equal desc left right =
    check bool desc false (equal_typ left right)
  
  (** 检查字符串包含某个子字符串 *)
  let check_string_contains desc haystack needle =
    check bool desc true (String.length needle = 0 || 
...
module BasicTypeTests = struct
  ...
  let test_basic_type_creation () =
    ...
    List.iter (fun (typ, desc) -> 
      TestUtils.check_type_equal desc typ typ
    ) test_cases
...
module CompoundTypeTests = struct
  ...
  let test_record_types () =
    ...
    let complex_record = RecordType_T [
      ("用户", simple_record);
      ("活跃", BoolType_T);
      ("分数", FloatType_T)
    ] in
...
module AdvancedTypeTests = struct
  ...
  let test_private_and_variant_types () =
    ...
    let variant_type = PolymorphicVariantType_T [
      ("成功", Some StringType_T);
      ("失败", None);
      ("警告", Some IntType_T)
    ] in
