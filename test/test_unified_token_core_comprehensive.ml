...
module TestHelpers = struct
  (** 创建测试位置信息 *)
  let make_test_position filename line column offset = { filename; line; column; offset }
...
module TokenEqualityTests = struct
  ...
  let test_same_tokens_equality () =
    List.iter (fun token -> check_token_equal "相同token应该相等" token token) sample_tokens
  ...
  let test_identifier_tokens_equality () =
    ...
    check bool "不同构造器token不等" false
      (equal_token (ConstructorToken "Some") (ConstructorToken "None"))
end
