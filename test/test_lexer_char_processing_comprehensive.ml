...
let test_integration_with_utf8_utils () =
  ...
  List.iter
    (fun (char, is_chinese, is_letter_chinese, is_digit_expected, is_whitespace_expected) ->
...
let integration_tests = [ test_case "UTF8工具集成测试" `Quick test_integration_with_utf8_utils ]

(** 主测试运行器 *)
let () =
  run "Lexer_char_processing 综合测试"
    [
