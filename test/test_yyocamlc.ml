...
let test_parser_function () =
  ...
  let program = Parser.parse_program token_list in
  match program with
  | [Ast.LetStmt ("f", Ast.FunExpr (["x"], Ast.BinaryOpExpr (Ast.VarExpr "x", Ast.Add, Ast.LitExpr (Ast.IntLit 1))))] -> ()
  | _ -> failwith "函数定义解析失败"
...
let test_codegen_conditional () =
  let expr = Ast.CondExpr (
    Ast.LitExpr (Ast.BoolLit true),
    Ast.LitExpr (Ast.IntLit 1),
    Ast.LitExpr (Ast.IntLit 0)
  ) in
  let result = Codegen.eval_expr [] expr in
  match result with
  | Codegen.IntValue 1 -> ()
  | _ -> failwith "条件表达式求值失败"

(** 测试代码生成 - 函数调用 *)
let test_codegen_function_call () =
  let func_expr = Ast.FunExpr (["x"], Ast.BinaryOpExpr (Ast.VarExpr "x", Ast.Add, Ast.LitExpr (Ast.IntLit 1))) in
...
let test_codegen_builtin_functions () =
  let env = [("打印", Codegen.BuiltinFunctionValue (function
    | [Codegen.StringValue s] -> print_endline s; Codegen.UnitValue
    | _ -> failwith "打印函数参数错误"))] in
