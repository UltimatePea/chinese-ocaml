(** 错误处理核心模块测试套件
    ...
    验证error_handler_core.ml模块的核心错误处理功能 包括错误处理流程、错误创建、统计记录和格式化输出
...
let create_test_type_error msg = make_error_info (TypeError (msg, Some (create_test_position ())))

let create_test_runtime_error msg =
  make_error_info (RuntimeError (msg, Some (create_test_position ())))

let create_test_internal_error msg = make_error_info ~severity:Fatal (InternalError msg)
...
let test_handle_error () =
  ...
  check bool "语法错误处理结果应包含适当的错误信息" true (String.length formatted_syntax > 0);
  ...
  check bool "类型错误处理结果应包含格式化消息" true (String.length formatted_type > 0);
  ...
  check bool "严重错误处理结果应包含适当信息" true (String.length formatted_internal > 0);
...
let test_batch_error_handling () =
  ...
  check bool "批量错误处理应返回继续标志" true (should_continue || not should_continue);
