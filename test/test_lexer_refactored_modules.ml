...
let check_token_types expected_types actual_tokens =
  ...
  let actual_types = List.map (fun token ->
    match token with
    ...
    | Newline -> "Newline"
    | _ -> "OtherToken"
  ) (extract_tokens actual_tokens) in
...
let test_wenyan_keyword_recognition () =
  let test_cases = [
    ...
    ("一", ["OneKeyword"; "EOF"]);
    ("名曰", ["NameKeyword"; "EOF"]);
    ("「呼」", ["QuotedIdentifierToken(\"呼\")"; "EOF"]);  (* 单字符 "呼" 需要引用 *)
    ("为", ["AsForKeyword"; "EOF"]);
    ("「倍」", ["QuotedIdentifierToken(\"倍\")"; "EOF"]);  (* 单字符 "倍" 需要引用 *)
...
let test_complex_expressions () =
  let test_cases = [
    ("让 「变量」 为 『字符串』", [
      "LetKeyword"; "QuotedIdentifierToken(\"变量\")"; "AsForKeyword"; "StringToken(\"字符串\")"; "EOF"
    ]);
    ("如果 「条件」 那么 真 否则 假", [
      "IfKeyword"; "QuotedIdentifierToken(\"条件\")"; "ThenKeyword"; "BoolToken(true)"; "ElseKeyword"; "BoolToken(false)"; "EOF"
    ]);
    ("函数 「参数」 匹配 与 其他", [
      "FunKeyword"; "QuotedIdentifierToken(\"参数\")"; "MatchKeyword"; "WithKeyword"; "OtherKeyword"; "EOF"
    ]);
    ("「变量」 乘以 一", [
      "QuotedIdentifierToken(\"变量\")"; "MultiplyKeyword"; "OneKeyword"; "EOF"
    ]);
    ("算法 据开始 据结束", [
...
let test_performance () =
  let large_input = String.concat " " (List.init 1000 (fun i -> 
    match i mod 5 with
    | 0 -> "让"
