(** 骆言内置数组模块测试 - Phase 26 内置模块测试覆盖率提升
    ...
    本测试模块专门针对 builtin_array.ml 模块进行全面功能测试，
    重点测试数组操作的正确性、边界条件和错误处理。
  ...
  let values_equal v1 v2 = match (v1, v2) with
    ...
    | (ArrayValue a, ArrayValue b) -> 
        ...
        (let rec compare_arrays i = 
           ...
           else a.(i) = b.(i) && compare_arrays (i + 1)
         in compare_arrays 0)
...
module ArrayAccessTests = struct
  ...
  (** 测试数组元素获取 *)
  let test_array_get () =
    let test_array = TestUtils.make_test_array [StringValue "第一"; StringValue "第二"; StringValue "第三"] in
...
module ArrayConversionTests = struct
  ...
  let test_array_to_list () =
    let test_array = TestUtils.make_test_array [StringValue "一"; StringValue "二"; StringValue "三"] in
    let result = array_to_list_function [test_array] in
    
    let expected_list = TestUtils.make_test_list [StringValue "一"; StringValue "二"; StringValue "三"] in
  ...
  let test_conversion_round_trip () =
    (* 数组 -> 列表 -> 数组 *)
    let original_array = TestUtils.make_test_array [BoolValue true; BoolValue false; BoolValue true] in
