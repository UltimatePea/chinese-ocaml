(** 骆言Token系统整合重构 - 统一Token模块综合测试
    验证新的Token架构的基础功能和整合效果 - Fix #1325 *)
  ...
  let test_token_self_equality () =
    ...
    List.iteri (fun i token ->
      assert_token_equal ~expected:token ~actual:token 
        (Printf.sprintf "Token[%d]应该等于自身" i)
    ) tokens
end
...
module TokenStringConversionTests = struct
  ...
  let test_comprehensive_string_conversion () =
    ...
    List.iter (fun token ->
      ...
      check bool "Token字符串转换不应为空" 
        true (String.length token_str > 0);
      check bool "Token字符串应为有效UTF-8" 
        true (String.length token_str > 0)
    ) all_tokens
...
module TokenListTests = struct
  ...
  let test_token_list_equality () =
    ...
    check bool "不同Token列表应该不等" false 
      (List.for_all2 equal_token tokens1 tokens3)
  ...
  let test_empty_token_list () =
    ...
    check bool "空列表和非空列表应该不等" false 
      (List.length empty1 = List.length non_empty)
  ...
  let test_large_token_list () =
    ...
    assert_token_list_equal ~expected:large_list ~actual:list_copy 
      "相同生成的大型Token列表应该相等";
    
    check int "大型Token列表长度应正确" 
      (List.length large_list) (List.length list_copy)
end
...
module IntegrationTests = struct
  ...
  let test_token_system_integration () =
    ...
    List.iteri (fun i token ->
      let token_str = token_to_string token in
      check bool (Printf.sprintf "Token[%d]字符串转换成功" i) 
        true (String.length token_str > 0);
      
      let type_name = get_token_type_name token in
      check bool (Printf.sprintf "Token[%d]类型识别成功" i) 
        true (String.length type_name > 0)
    ) tokens
