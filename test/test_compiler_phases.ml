...
module Test_helpers = struct
  ...
  (** 安静模式选项，用于测试 *)
  let test_options = { default_options with quiet_mode = true; log_level = "quiet" }
...
let test_perform_lexical_analysis () =
  ...
  check bool "显示词元模式返回相同数量词元" (List.length token_list = List.length verbose_tokens) true
...
let test_perform_c_code_generation () =
  ...
  else check bool "语义分析为C代码生成准备" true true
...
let test_perform_interpretation () =
  ...
  else check bool "语义分析为解释执行准备" true true
...
let test_error_handling () =
  ...
  with _ -> check bool "错误处理-异常捕获正确" true true
