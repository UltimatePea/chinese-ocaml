...
let test_converter_config_completeness () =
  ...
  (* 验证所有预期的转换器类型都被注册 *)
  let expected_types = [
    UC.LiteralConverter; UC.KeywordConverter; UC.OperatorConverter;
    UC.IdentifierConverter; UC.DelimiterConverter
  ] in
  
  List.iter (fun converter_type ->
    let converters = UC.ConverterRegistry.get_converters converter_type in
    let type_name = match converter_type with
      ...
      | _ -> "OtherConverter"
    in
    check bool (Printf.sprintf "%s类型应有转换器注册" type_name) 
      (List.length converters > 0) true
  ) expected_types
...
let test_registration_statistics () =
  ...
  (* 验证统计数据变化 *)
  check bool "注册前后统计数据变化" 
    (fst after_stats > fst before_stats) true;
...
let test_converter_functionality_preserved () =
  ...
  check bool "整数转换功能保持" 
    (match int_result with UC.Success _ -> true | UC.Failure _ -> false) true;

  (* 测试字符串字面量转换 *)
  let string_result = UC.convert "\"hello\"" test_pos in
  check bool "字符串转换功能保持"
    (match string_result with UC.Success _ -> true | UC.Failure _ -> false) true
...
let test_batch_conversion_functionality () =
  ...
  (* 验证所有转换都有结果 *)
  let all_processed = List.for_all (fun (_, result) ->
    match result with 
    | UC.Success _ | UC.Failure _ -> true
  ) results in
...
let statistics_tests = [
  test_case "注册统计功能" `Quick test_registration_statistics;
]
