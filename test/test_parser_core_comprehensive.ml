...
module TestUtils = struct
  ...
  (** 测试表达式相等性 *)
  let check_expr desc expected actual = check (testable pp_expr equal_expr) desc expected actual

  (** 测试语句相等性 *)
  let check_stmt desc expected actual = check (testable pp_stmt equal_stmt) desc expected actual
end
...
module LiteralParsingTests = struct
  ...
  let test_integer_literal_parsing () =
    let test_cases =
      [
        (* 基础整数测试 *)
        ([ make_token (IntToken 0) 1 1; eof_token 1 2 ], LitExpr (IntLit 0), "零值整数解析");
        ([ make_token (IntToken 42) 1 1; eof_token 1 3 ], LitExpr (IntLit 42), "正整数解析");
        ([ make_token (IntToken (-17)) 1 1; eof_token 1 4 ], LitExpr (IntLit (-17)), "负整数解析");
        ( [ make_token (IntToken 2147483647) 1 1; eof_token 1 11 ],
          LitExpr (IntLit 2147483647),
          "最大32位整数解析" );
        ( [ make_token (IntToken (-2147483648)) 1 1; eof_token 1 12 ],
          LitExpr (IntLit (-2147483648)),
          "最小32位整数解析" );
      ]
    in
    List.iter
  ...
  let test_boolean_literal_parsing () =
    let test_cases =
      [
        ([ make_token TrueKeyword 1 1; eof_token 1 5 ], LitExpr (BoolLit true), "布尔真值解析");
        ([ make_token FalseKeyword 1 1; eof_token 1 6 ], LitExpr (BoolLit false), "布尔假值解析");
      ]
    in
    List.iter
      (fun (tokens, expected, desc) ->
    ...
    let test_cases =
      [
        ...
        ( [
            ...
            make_token Plus 1 3;
            make_token (IntToken 2) 1 5;
            eof_token 1 6;
          ],
          BinaryOpExpr (LitExpr (IntLit 1), Add, LitExpr (IntLit 2)),
          "加法运算解析" );
        ( [
            make_token (IntToken 5) 1 1;
            make_token Minus 1 3;
            make_token (IntToken 3) 1 5;
            eof_token 1 6;
          ],
          BinaryOpExpr (LitExpr (IntLit 5), Sub, LitExpr (IntLit 3)),
          "减法运算解析" );
        ( [
            make_token (IntToken 4) 1 1;
            make_token Star 1 3;
            make_token (IntToken 6) 1 5;
            eof_token 1 6;
          ],
          BinaryOpExpr (LitExpr (IntLit 4), Mul, LitExpr (IntLit 6)),
          "乘法运算解析" );
        ( [
            make_token (IntToken 8) 1 1;
            make_token Slash 1 3;
            make_token (IntToken 2) 1 5;
            eof_token 1 6;
          ],
  ...
  let test_comparison_operators () =
    let test_cases =
      [
        ...
        ( [
            make_token (IntToken 1) 1 1;
            make_token Equal 1 3;
            make_token (IntToken 1) 1 6;
            eof_token 1 7;
          ],
          BinaryOpExpr (LitExpr (IntLit 1), Eq, LitExpr (IntLit 1)),
          "等于运算解析" );
        ( [
            make_token (IntToken 2) 1 1;
            make_token NotEqual 1 3;
            make_token (IntToken 3) 1 7;
            eof_token 1 8;
          ],
          BinaryOpExpr (LitExpr (IntLit 2), Neq, LitExpr (IntLit 3)),
          "不等于运算解析" );
        ( [
            make_token (IntToken 1) 1 1;
            make_token Less 1 3;
          ...
          "小于运算解析" );
        ( [
            make_token (IntToken 3) 1 1;
            make_token Greater 1 3;
            make_token (IntToken 1) 1 5;
            eof_token 1 6;
          ],
          BinaryOpExpr (LitExpr (IntLit 3), Gt, LitExpr (IntLit 1)),
          "大于运算解析" );
      ]
    in
    List.iter
  ...
  let test_logical_operators () =
    let test_cases =
      [
        (* 逻辑运算符测试 *)
        ( [
            make_token TrueKeyword 1 1;
            make_token AndKeyword 1 6;
            make_token FalseKeyword 1 10;
            eof_token 1 15;
          ],
          BinaryOpExpr (LitExpr (BoolLit true), And, LitExpr (BoolLit false)),
          "逻辑与运算解析" );
        ( [
            make_token FalseKeyword 1 1;
            make_token OrKeyword 1 7;
            make_token TrueKeyword 1 10;
            eof_token 1 14;
          ],
    ...
    let test_cases =
      [
        ...
        ( [ make_token NotKeyword 1 1; make_token TrueKeyword 1 2; eof_token 1 6 ],
          UnaryOpExpr (Not, LitExpr (BoolLit true)),
          "逻辑非运算解析" );
      ]
    in
    List.iter
...
module OperatorPrecedenceTests = struct
  ...
  let test_comparison_precedence () =
    ...
    let tokens =
      [
        ...
        make_token Less 1 7;
        make_token (IntToken 3) 1 9;
        make_token Plus 1 11;
        make_token (IntToken 1) 1 13;
        eof_token 1 14;
      ]
    in
    let expected =
      BinaryOpExpr
        ( BinaryOpExpr (LitExpr (IntLit 1), Add, LitExpr (IntLit 2)),
          Lt,
          BinaryOpExpr (LitExpr (IntLit 3), Add, LitExpr (IntLit 1)) )
    in
    let state = create_parser_with_tokens tokens in
    let expr, _ = parse_expression state in
  ...
  let test_logical_precedence () =
    ...
    let tokens =
      [
        make_token (QuotedIdentifierToken "a") 1 1;
        make_token AndKeyword 1 3;
        make_token (QuotedIdentifierToken "b") 1 7;
        make_token OrKeyword 1 9;
        make_token (QuotedIdentifierToken "c") 1 12;
        eof_token 1 13;
      ]
    in
    let expected = BinaryOpExpr (BinaryOpExpr (VarExpr "a", And, VarExpr "b"), Or, VarExpr "c") in
    let state = create_parser_with_tokens tokens in
    let expr, _ = parse_expression state in
...
module ParenthesesTests = struct
  ...
  let test_simple_parentheses () =
    let tokens =
      [
        ...
        make_token (IntToken 5) 1 2;
        make_token RightParen 1 3;
        eof_token 1 4;
      ]
    in
  ...
  let test_parentheses_override_precedence () =
    ...
    let tokens =
      [
        make_token LeftParen 1 1;
        make_token (IntToken 1) 1 2;
        make_token Plus 1 4;
        make_token (IntToken 2) 1 6;
        make_token RightParen 1 7;
        make_token Star 1 9;
        make_token (IntToken 3) 1 11;
        eof_token 1 12;
      ]
    in
    let expected =
  ...
  let test_nested_parentheses () =
    ...
    let tokens =
      [
        ...
        make_token (IntToken 1) 1 3;
        make_token Plus 1 5;
        make_token (IntToken 2) 1 7;
        make_token RightParen 1 8;
        make_token Star 1 10;
        make_token (IntToken 3) 1 12;
        make_token RightParen 1 13;
        eof_token 1 14;
      ]
    in
    let expected =
...
module IdentifierTests = struct
  ...
  let test_simple_identifiers () =
    let test_cases =
      [
        ([ make_token (QuotedIdentifierToken "x") 1 1; eof_token 1 2 ], VarExpr "x", "单字符变量名");
        ( [ make_token (QuotedIdentifierToken "variable") 1 1; eof_token 1 9 ],
          VarExpr "variable",
          "英文变量名" );
        ([ make_token (QuotedIdentifierToken "变量") 1 1; eof_token 1 3 ], VarExpr "变量", "中文变量名");
  ...
  let test_qualified_identifiers () =
    ...
    let tokens =
      [
        make_token (QuotedIdentifierToken "Module") 1 1;
        make_token Dot 1 7;
        make_token (QuotedIdentifierToken "value") 1 8;
        eof_token 1 13;
      ]
    in
...
module FunctionCallTests = struct
  ...
  let test_simple_function_calls () =
    let test_cases =
      [
        ...
        (* 单参数整数函数调用 *)
        ( [
            make_token (QuotedIdentifierToken "f") 1 1;
            make_token LeftParen 1 2;
            make_token (IntToken 42) 1 3;
            make_token RightParen 1 5;
            eof_token 1 6;
          ],
          FunCallExpr (VarExpr "f", [ LitExpr (IntLit 42) ]),
          "单参数整数函数调用" );
      ]
    in
    List.iter
  ...
  let test_nested_function_calls () =
    ...
    let tokens =
      [
        make_token (QuotedIdentifierToken "outer") 1 1;
        make_token LeftParen 1 6;
        make_token (QuotedIdentifierToken "inner") 1 7;
        make_token LeftParen 1 12;
        make_token (IntToken 5) 1 13;
        make_token RightParen 1 14;
        make_token RightParen 1 15;
        eof_token 1 16;
      ]
    in
    let expected =
      FunCallExpr (VarExpr "outer", [ FunCallExpr (VarExpr "inner", [ LitExpr (IntLit 5) ]) ])
    in
    let state = create_parser_with_tokens tokens in
    let expr, _ = parse_expression state in
...
module StatementParsingTests = struct
  ...
  let test_variable_declarations () =
    let test_cases =
      [
        ...
        ( [
            make_token LetKeyword 1 1;
            make_token (QuotedIdentifierToken "x") 1 5;
            make_token AsForKeyword 1 7;
            make_token (IntToken 42) 1 9;
            eof_token 1 11;
          ],
        ...
        ( [
            make_token LetKeyword 1 1;
            make_token (QuotedIdentifierToken "sum") 1 5;
            make_token AsForKeyword 1 9;
            make_token (IntToken 1) 1 11;
            make_token Plus 1 13;
            make_token (IntToken 2) 1 15;
            eof_token 1 16;
          ],
  ...
  let test_if_statements () =
    ...
    let tokens =
      [
        make_token IfKeyword 1 1;
        make_token TrueKeyword 1 4;
        make_token ThenKeyword 1 9;
        make_token (IntToken 1) 1 14;
        make_token ElseKeyword 1 16;
        make_token (IntToken 0) 1 21;
        eof_token 1 22;
      ]
    in
    let expected =
      ExprStmt (CondExpr (LitExpr (BoolLit true), LitExpr (IntLit 1), LitExpr (IntLit 0)))
    in
    let state = create_parser_with_tokens tokens in
    let stmt, _ = parse_statement state in
  ...
  let test_expression_statements () =
    let test_cases =
      [
        (* 表达式语句 *)
        ([ make_token (IntToken 42) 1 1; eof_token 1 3 ], ExprStmt (LitExpr (IntLit 42)), "整数表达式语句");
        ( [
            make_token (QuotedIdentifierToken "func") 1 1;
            make_token LeftParen 1 5;
            make_token RightParen 1 6;
            eof_token 1 7;
          ],
          ExprStmt (FunCallExpr (VarExpr "func", [])),
          "函数调用表达式语句" );
      ]
    in
    List.iter
...
module ErrorHandlingTests = struct
  ...
  let test_syntax_error_recovery () =
    (* 测试语法错误恢复机制 *)
    let invalid_tokens =
      [ make_token (IntToken 1) 1 1; make_token Plus 1 3; (* 缺少右操作数 *) eof_token 1 4 ]
    in
    (* 这里应该抛出解析错误 *)
  ...
  let test_unmatched_parentheses () =
    (* 测试不匹配的括号 *)
    let invalid_tokens =
      [ make_token LeftParen 1 1; make_token (IntToken 5) 1 2; (* 缺少右括号 *) eof_token 1 3 ]
    in
    let state = create_parser_with_tokens invalid_tokens in
    try
      let _ = parse_expression state in
...
module IntegrationTests = struct
  ...
  let test_complex_expression_parsing () =
    ...
    let tokens =
      [
        ...
        make_token Star 1 9;
        make_token (IntToken 3) 1 11;
        make_token RightParen 1 12;
        make_token Greater 1 14;
        make_token (IntToken 10) 1 16;
        eof_token 1 18;
      ]
    in
    let expected =
  ...
  let test_mixed_statement_parsing () =
    ...
    let tokens =
      [
        make_token LetKeyword 1 1;
        make_token (QuotedIdentifierToken "result") 1 5;
        make_token AsForKeyword 1 12;
        make_token IfKeyword 1 14;
        make_token (QuotedIdentifierToken "x") 1 17;
        make_token Greater 1 19;
        make_token (IntToken 0) 1 21;
        make_token ThenKeyword 1 23;
        make_token (QuotedIdentifierToken "x") 1 28;
        make_token Star 1 30;
        make_token (IntToken 2) 1 32;
        make_token ElseKeyword 1 34;
        make_token (IntToken 0) 1 39;
        eof_token 1 40;
      ]
    in
    let expected =
...
(** 运行测试 *)
let () = Alcotest.run "骆言Parser核心模块全面测试覆盖 - Fix #933" [ ("parser_core_comprehensive", test_suite) ]
