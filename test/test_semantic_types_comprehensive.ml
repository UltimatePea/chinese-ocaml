...
let create_test_context () =
  ...
  (* 添加一些基础类型定义 *)
  ctx
  |> fun c -> add_type_definition c "整数" IntType_T
  |> fun c -> add_type_definition c "字符串" StringType_T
  |> fun c -> add_type_definition c "布尔" BoolType_T
...
let compare_types expected actual =
  let typ_to_string = function
    ...
    | ListType_T t -> "ListType_T(" ^ (match t with IntType_T -> "IntType_T" | StringType_T -> "StringType_T" | _ -> "Other") ^ ")"
    | ConstructType_T (s, []) -> "ConstructType_T(" ^ s ^ ")"
    | FunType_T (arg, ret) -> 
      let arg_str = match arg with IntType_T -> "IntType_T" | StringType_T -> "StringType_T" | TupleType_T _ -> "TupleType_T" | _ -> "Other" in
      let ret_str = match ret with IntType_T -> "IntType_T" | StringType_T -> "StringType_T" | _ -> "Other" in
      "FunType_T(" ^ arg_str ^ ", " ^ ret_str ^ ")"
    | RecordType_T fields -> 
      let field_strs = List.map (fun (name, t) -> 
        name ^ ":" ^ (match t with IntType_T -> "IntType_T" | StringType_T -> "StringType_T" | _ -> "Other")) fields in
...
let test_resolve_function_type_expr () =
  ...
  (* 测试多参数函数类型 *)
  let multi_param_expr = FunType (TupleType [BaseTypeExpr IntType; BaseTypeExpr BoolType], BaseTypeExpr StringType) in
...
let test_resolve_record_type_expr () =
  ...
  (* 测试记录类型解析 - 使用自定义类型表示 *)
  let record_expr = ConstructType ("RecordType", [BaseTypeExpr StringType; BaseTypeExpr IntType]) in
...
let test_symbol_table_to_env () =
  ...
  let ctx_with_symbols = 
    ctx
    |> fun c -> add_symbol c "变量A" IntType_T false
    |> fun c -> add_symbol c "变量B" StringType_T false
    |> fun c -> add_symbol c "变量C" (ListType_T BoolType_T) true
  in
  ...
  (* 检查环境中的变量类型 *)
  let find_in_env name = 
    try Some (TypeEnv.find name env)
    with Not_found -> None
  in
...
let test_resolve_nested_type_expr () =
  ...
  (* 测试复杂函数类型 *)
  let complex_func_expr = FunType (
    ListType (BaseTypeExpr IntType),
    ListType (BaseTypeExpr StringType)
  ) in
...
let test_complex_record_types () =
  ...
  (* 创建包含嵌套结构的记录类型 *)
  let complex_record_expr = ConstructType ("ComplexRecord", [BaseTypeExpr StringType; BaseTypeExpr IntType; BaseTypeExpr BoolType]) in
...
let () =
  run "Semantic_types 综合测试" [
    ...
    "性能测试", [
      test_case "大量类型环境" `Slow test_large_type_environment;
    ];
  ]