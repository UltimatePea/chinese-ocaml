...
let test_literal_token_conversions () =
  ...
  check bool "布尔false Token转换正确性" true
    (match bool_false_token with
    | Token_system_core.Token_types.BoolToken false -> true
    | _ -> false);

  (* 测试中文数字Token转换 *)
  let chinese_num_token =
    Token_system_compatibility.Legacy_type_bridge.convert_chinese_number_token "四十二"
  in
  check bool "中文数字Token转换正确性" true
    (match chinese_num_token with
    | Token_system_core.Token_types.ChineseNumberToken "四十二" -> true
    | _ -> false)
...
let test_identifier_token_conversions () =
  ...
  check bool "简单标识符转换正确性" true
    (match simple_id with
    | Token_system_core.Token_types.SimpleIdentifier "变量名" -> true
    | _ -> false);

  (* 测试引用标识符转换 *)
  let quoted_id = Token_system_compatibility.Legacy_type_bridge.convert_quoted_identifier "引用变量" in
  check bool "引用标识符转换正确性" true
    (match quoted_id with
    | Token_system_core.Token_types.QuotedIdentifierToken "引用变量" -> true
    | _ -> false);

  (* 测试特殊标识符转换 *)
  let special_id =
    Token_system_compatibility.Legacy_type_bridge.convert_special_identifier "__内部变量__"
  in
  check bool "特殊标识符转换正确性" true
    (match special_id with
    | Token_system_core.Token_types.IdentifierTokenSpecial "__内部变量__" -> true
    | _ -> false)
...
let test_token_construction_utilities () =
  ...
  let literal_token =
    Token_system_compatibility.Legacy_type_bridge.make_literal_token int_literal
  in
  ...
  let simple_id =
    Token_system_compatibility.Legacy_type_bridge.convert_simple_identifier "test_var"
  in
  let identifier_token =
    Token_system_compatibility.Legacy_type_bridge.make_identifier_token simple_id
  in
  ...
  let keyword_token =
    Token_system_compatibility.Legacy_type_bridge.make_core_language_token let_keyword
  in
...
let test_token_classification_functions () =
  ...
  in
  let delimiter_token =
    Token_system_compatibility.Legacy_type_bridge.make_delimiter_token
      (Token_system_compatibility.Legacy_type_bridge.convert_comma ())
  in
...
let test_batch_processing_utilities () =
  ...
  let get_count type_name = try List.assoc type_name type_counts with Not_found -> 0 in
  ...
  let validation_result =
    Token_system_compatibility.Legacy_type_bridge.validate_token_stream mixed_tokens
  in
...
let test_debug_and_diagnostics () =
  let test_token =
    Token_system_compatibility.Legacy_type_bridge.make_literal_token
      (Token_system_compatibility.Legacy_type_bridge.convert_string_token "调试测试")
  in
  ...
  check bool "调试信息包含Token类型" true (String.contains debug_info 'L');
...
let test_experimental_features () =
  ...
  let inferred_int =
    Token_system_compatibility.Legacy_type_bridge.infer_token_type_from_string "123"
  in
  check string "整数字符串类型推断" "Literal" inferred_int;

  let inferred_identifier =
    Token_system_compatibility.Legacy_type_bridge.infer_token_type_from_string "variable_name"
  in
  check string "标识符字符串类型推断" "Identifier" inferred_identifier;

  let inferred_keyword =
    Token_system_compatibility.Legacy_type_bridge.infer_token_type_from_string "let"
  in
...
let test_edge_cases_and_error_handling () =
  ...
  check bool "空字符串Token转换" true
    (match empty_string_token with
    | Token_system_core.Token_types.StringToken "" -> true
    | _ -> false);

  (* 测试极大整数处理 *)
  let max_int_token = Token_system_compatibility.Legacy_type_bridge.convert_int_token max_int in
  check bool "最大整数Token转换" true
    (match max_int_token with
    | Token_system_core.Token_types.IntToken i when i = max_int -> true
    | _ -> false);

  (* 测试特殊浮点数处理 *)
  let nan_token = Token_system_compatibility.Legacy_type_bridge.convert_float_token nan in
  check bool "NaN浮点数Token转换" true
    (match nan_token with
    | Token_system_core.Token_types.FloatToken f when Float.is_nan f -> true
    | _ -> false);

  (* 测试空Token列表处理 *)
  let empty_list_validation =
...
let test_complete_token_workflow () =
  ...
  (* 验证Token序列的语法正确性 *)
  let validation_result =
    Token_system_compatibility.Legacy_type_bridge.validate_token_stream workflow_tokens
  in
  check bool "工作流Token序列验证通过" true validation_result;

  (* 统计Token类型分布 *)
  let type_stats =
    Token_system_compatibility.Legacy_type_bridge.count_token_types workflow_tokens
  in
...
let literal_conversion_tests = [ test_case "字面量Token转换测试" `Quick test_literal_token_conversions ]

let identifier_conversion_tests =
  [ test_case "标识符Token转换测试" `Quick test_identifier_token_conversions ]

let keyword_conversion_tests = [ test_case "关键字Token转换测试" `Quick test_core_keyword_conversions ]

let operator_conversion_tests = [ test_case "操作符Token转换测试" `Quick test_operator_conversions ]

let delimiter_conversion_tests = [ test_case "分隔符Token转换测试" `Quick test_delimiter_conversions ]
...
let experimental_feature_tests = [ test_case "实验性功能测试" `Quick test_experimental_features ]

let edge_case_tests = [ test_case "边界条件和错误处理测试" `Quick test_edge_cases_and_error_handling ]

let integration_tests = [ test_case "完整Token工作流测试" `Quick test_complete_token_workflow ]
