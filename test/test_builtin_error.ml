  ...
  let contains_chinese_chars msg =
    let rec check_chars i =
      if i >= String.length msg then false
        ...
        if Char.code c > 127 then true
        else check_chars (i + 1)
    in
    check_chars 0
end
...
module ArgumentCountTests = struct
  ...
  let test_correct_arg_count () =
    ...
    with
    | RuntimeError _ -> fail "正确参数数量不应抛出错误"
  ...
  let test_zero_args () =
    ...
    with
    | RuntimeError _ -> fail "零参数数量检查不应抛出错误"
end
...
module DoubleArgumentTests = struct
  ...
  let test_valid_double_args () =
    ...
    check bool "双参数检查应返回正确参数" true 
      (arg1 = IntValue 1 && arg2 = StringValue "test")
...
module NoArgumentTests = struct
  ...
  let test_valid_no_args () =
    ...
    with
    | RuntimeError _ -> fail "无参数检查不应抛出错误"
...
module ErrorMessageTests = struct
  ...
  let test_error_message_contains_function_name () =
    ...
    let error_msg = TestUtils.get_runtime_error_message (fun () ->
      check_single_arg [] function_name
    ) in
    
    match error_msg with
    | Some msg -> 
      check bool "错误消息应包含函数名" true (String.contains msg (String.get function_name 0))
    | None -> fail "应该产生错误消息"
  ...
  let test_chinese_error_messages () =
    ...
    List.iter (fun error_case ->
      let error_msg = TestUtils.get_runtime_error_message error_case in
      match error_msg with
      | Some msg ->
        check bool "错误消息应包含中文字符" true (TestUtils.contains_chinese_chars msg)
      | None -> fail "应该产生错误消息"
    ) test_cases
  ...
  let test_error_message_consistency () =
    (* 同类型错误应该有一致的消息格式 *)
    let msg1 = TestUtils.get_runtime_error_message (fun () ->
      check_single_arg [] "函数A"
    ) in
    let msg2 = TestUtils.get_runtime_error_message (fun () ->
      check_single_arg [] "函数B"
    ) in
...
module AdvancedTypeCheckTests = struct
  ...
  let test_expect_number () =
    ...
    check bool "浮点数应被识别为数字" true (match result2 with FloatValue f -> abs_float (f -. 3.14) < 1e-10 | _ -> false);
  ...
  let test_expect_string_or_list () =
    ...
    check bool "字符串应通过字符串或列表检查" true 
      (match result1 with StringValue _ -> true | _ -> false);

    (* 测试列表 *)
    let result2 = expect_string_or_list (ListValue [IntValue 1]) "测试函数" in
    check bool "列表应通过字符串或列表检查" true 
      (match result2 with ListValue _ -> true | _ -> false);
