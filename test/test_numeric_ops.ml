  ...
  let test_add_operation () =
    ...
    List.iter (fun (v1, v2, expected) ->
      ...
      Printf.printf "    %s + %s = %s (期望: %s) %s\n" 
        (value_to_string v1) (value_to_string v2) (value_to_string result) (value_to_string expected)
...
module TestNumericAggregation = struct
  let test_fold_numeric_list () =
    ...
    List.iter (fun (lst, initial, expected) ->
  ...
  let test_process_nonempty_numeric_list () =
    ...
    let non_empty_tests = [
      ...
      ([IntValue 42], add_op, IntValue 42);  (* 单元素列表 *)
    ] in
    
    List.iter (fun (lst, op, expected) ->
  ...
  let test_create_nonempty_numeric_aggregator () =
    ...
    with
    | _ ->
        Printf.printf "    max_aggregator(空列表): 正确抛出异常 ✓\n");
...
module TestCustomNumericOperations = struct
  ...
  let test_custom_aggregators () =
    ...
    let difference_tests = [
      ([IntValue 10; IntValue 3; IntValue 2], IntValue 5);  (* 10 - 3 - 2 = 5 *)
      ([FloatValue 15.0; FloatValue 5.0; FloatValue 2.5], FloatValue 7.5);  (* 15.0 - 5.0 - 2.5 = 7.5 *)
      ([IntValue 100], IntValue 100);  (* 单元素 *)
    ] in
...
module TestErrorHandlingAndEdgeCases = struct
  let test_error_handling () =
    ...
    List.iter (fun lst ->
      ...
      with
      | _ ->
          Printf.printf "    invalid list: 正确抛出异常 ✓\n"
    ) invalid_lists;
  ...
  let test_edge_cases () =
    ...
    List.iter (fun (v1, v2, op, expected) ->
  ...
  let test_precision_issues () =
    ...
    let precision_tests = [
      ...
      (FloatValue 1.0, FloatValue 3.0, divide_op, 0.33333333);  (* 需要自定义除法操作 *)
    ] in
...
module TestPerformance = struct
  ...
  let test_type_checking_performance () =
    ...
    let test_type_checking () =
      Array.iter (fun v -> ignore (is_numeric v)) mixed_values
    in
  ...
  let test_validation_performance () =
    ...
    let invalid_mixed_list = List.init 5000 (fun i ->
      if i mod 10 = 0 then StringValue ("item" ^ string_of_int i)
      else IntValue i
    ) in
    
    let test_validation () =
      try let _ = validate_numeric_list valid_numeric_list "性能测试" in () with _ -> ();
      try let _ = validate_numeric_list invalid_mixed_list "性能测试" in () with _ -> ();
    in
