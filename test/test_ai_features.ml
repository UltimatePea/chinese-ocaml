...
let test_intent_parser () =
  ...
  List.iter (fun (input, _expected_category) ->
    Printf.printf "🔍 测试: %s\n" input;
    try
      ...
      if List.length suggestions > 0 then (
        ...
        if best_suggestion.confidence > 0.5 then
          incr success_count
      ) else (
        Printf.printf "❌ 没有生成建议\n"
      );
    with e ->
  ...
  if !success_count = total_count then
    Printf.printf "🎉 所有意图解析测试通过！\n"
  else
    Printf.printf "⚠️  部分测试未通过\n";
  Printf.printf "\n"
  ...
  List.iter (fun (input, cursor_pos, description) ->
    ...
    try
      ...
      if List.length completions > 0 then (
        ...
        if best.score > 0.3 then
          incr success_count
      ) else (
        Printf.printf "❌ 没有补全建议\n"
      );
    with e ->
      Printf.printf "❌ 测试失败: %s\n" (Printexc.to_string e);
    Printf.printf "\n"
  ...
  List.iter (fun (input, _expected_pattern) ->
    ...
    try
      ...
      if List.length matches > 0 then (
        ...
        if best_match.confidence > 0.2 then
          incr success_count
      ) else (
        Printf.printf "❌ 没有匹配的模式\n"
      );
    with e ->
      Printf.printf "❌ 测试失败: %s\n" (Printexc.to_string e);
    Printf.printf "\n"
...
let test_natural_language () =
  ...
  List.iter (fun (input, description) ->
    ...
    try
      ...
      if List.length semantic_units > 0 && List.length suggestions > 0 then
        incr success_count;
...
let test_integration () =
  ...
  (try
    ...
    if List.length suggestions > 0 then (
      ...
      Printf.printf "❌ 意图解析失败\n"
    );
  with e ->
    Printf.printf "❌ 意图解析异常: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    if List.length matches > 0 then (
      ...
      Printf.printf "❌ 模式匹配失败\n"
    );
  with e ->
    Printf.printf "❌ 模式匹配异常: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    if List.length code_suggestions > 0 then (
      ...
      Printf.printf "❌ 自然语言处理失败\n"
    );
  with e ->
    Printf.printf "❌ 自然语言处理异常: %s\n" (Printexc.to_string e));

  Printf.printf "\n📊 集成测试结果: %d/3 步骤成功\n" !success_steps;
  if !success_steps = 3 then
    Printf.printf "🎉 AI功能集成测试完全通过！\n"
  else
    Printf.printf "⚠️  部分集成功能需要改进\n";
  Printf.printf "\n"
