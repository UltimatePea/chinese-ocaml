...
let test_current_token () =
  let tokens = [ make_token (IntToken 42) 1 1; make_token Plus 1 4; make_token EOF 1 6 ] in
  let state = create_parser_state tokens in
...
let test_peek_token () =
  let tokens =
    [ make_token (IntToken 42) 1 1; make_token Plus 1 4; make_token (IntToken 24) 1 6 ]
  in
  let state = create_parser_state tokens in
...
let test_advance_parser () =
  let tokens = [ make_token (IntToken 42) 1 1; make_token Plus 1 4 ] in
  let state = create_parser_state tokens in
...
let test_expect_token_success () =
  let tokens = [ make_token LeftParen 1 1; make_token (IntToken 42) 1 2 ] in
  let state = create_parser_state tokens in
  let new_state = expect_token state LeftParen in
  check int "期望token成功后位置" 1 new_state.current_pos

let test_expect_token_failure () =
  let tokens = [ make_token RightParen 1 1; make_token (IntToken 42) 1 2 ] in
  let state = create_parser_state tokens in
...
let test_state_operations_consistency () =
  let tokens =
    [ make_token (IntToken 1) 1 1; make_token (IntToken 2) 1 2; make_token (IntToken 3) 1 3 ]
  in
  let state = create_parser_state tokens in
...
let () = run "Parser_utils模块全面测试" [ ("Parser Utils", parser_utils_tests) ]
