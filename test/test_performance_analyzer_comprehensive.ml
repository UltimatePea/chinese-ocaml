...
let test_recursive_analysis _ =
  ...
  let nested_expr = make_binary_op (make_int 1) "+" (make_binary_op (make_int 2) "*" (make_int 3)) in
...
let test_complex_pattern_matching _ =
  (* 测试复杂模式匹配（多层嵌套） *)
  let complex_cases = List.init 20 (fun i -> 
    (IntLiteral i, make_string ("数字" ^ string_of_int i))
  ) in
...
let test_recursion_analysis _ =
  (* 创建一个递归函数定义 *)
  let factorial_body = make_app (make_var "factorial") [make_binary_op (make_var "n") "-" (make_int 1)] in
...
let test_performance_suggestions_quality _ =
  (* 测试建议质量 *)
  let test_suggestion = make_performance_suggestion
    ~hint_type:"性能"
    ~message:"这是一个测试建议"
    ~confidence:0.9
    ~location:"测试函数"
  ...
  (* 置信度应该在合理范围内 *)
  assert_bool "置信度应该在0-1之间" 
    (test_suggestion.confidence >= 0.0 && test_suggestion.confidence <= 1.0)
...
(** 测试套件组织 *)
let base_analyzer_tests = "性能分析器基础测试" >::: [
  "分析器创建测试" >:: test_base_analyzer_creation;
  "递归分析测试" >:: test_recursive_analysis;
  "建议构建器测试" >:: test_suggestion_builder;
]

let list_analyzer_tests = "列表性能分析测试" >::: [
  "列表性能分析" >:: test_list_performance_analysis;
  "列表连接分析" >:: test_list_concatenation_analysis;
]

let pattern_matching_tests = "模式匹配性能分析测试" >::: [
  "模式匹配分析" >:: test_pattern_matching_analysis;
  "复杂模式匹配" >:: test_complex_pattern_matching;
]

let recursion_tests = "递归性能分析测试" >::: [
  "递归分析" >:: test_recursion_analysis;
  "尾递归检测" >:: test_tail_recursion_detection;
]

let complexity_tests = "复杂度分析测试" >::: [
  "复杂度分析" >:: test_complexity_analysis;
  "嵌套层级检测" >:: test_nesting_level_detection;
]

let data_structure_tests = "数据结构性能分析测试" >::: [
  "数据结构分析" >:: test_data_structure_analysis;
  "记录字段访问" >:: test_record_field_access;
]
...
let boundary_tests = "边界条件测试" >::: [
  "空表达式测试" >:: test_empty_expressions;
  "单元素结构测试" >:: test_single_element_structures;
]

let suite = "性能分析器综合测试套件" >::: [
  base_analyzer_tests;
  list_analyzer_tests;
