...
let test_basic_keywords () =
  ...
  List.iter
    (fun (token, desc) -> check bool desc true (TestHelpers.tokens_equal token token))
    keywords;
...
let test_unicode_handling () =
  (* 测试Unicode字符串 *)
  let unicode_strings = [ "你好世界"; "🌟🎨🔥"; "αβγδε"; "中文编程语言"; "古雅体诗词编程" ] in

  List.iter
    (fun unicode_str ->
      let unicode_token = StringToken unicode_str in
      check bool
        ("Unicode字符串: " ^ unicode_str)
        true
        (TestHelpers.tokens_equal unicode_token (StringToken unicode_str)))
    unicode_strings
...
let test_performance () =
  ...
  check bool "token比较性能测试" true (duration < 1.0)
(* 应该在1秒内完成 *)
...
let type_tests = [ test_case "type_keywords" `Quick test_type_keywords ]
...
let special_tests = [ test_case "special_tokens" `Quick test_special_tokens ]
...
let error_tests = [ test_case "lex_error" `Quick test_lex_error ]
