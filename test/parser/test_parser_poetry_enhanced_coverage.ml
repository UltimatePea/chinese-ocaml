(** 骆言Parser古典诗词模块增强测试覆盖 - Fix #1307
    ...
    本测试模块为 parser_poetry.ml 提供全面的增强测试覆盖，作为提升项目测试覆盖率 从22%到50%目标的第二阶段实施。专注于古典诗词解析器的工具函数测试。
...
let test_validate_char_count () =
  ...
  (* 测试字数不匹配的错误情况 *)
  assert_poetry_parse_error (fun () -> validate_char_count 4 "春风得意马蹄疾");

  assert_poetry_parse_error (fun () -> validate_char_count 7 "雄关漫道");

  assert_poetry_parse_error (fun () -> validate_char_count 4 "床前明月光");

  assert_poetry_parse_error (fun () -> validate_char_count 3 "春风得意马蹄疾一路向前");
...
let test_poetry_parse_error_exception () =
  ...
  with
  | PoetryParseError msg -> check string "异常消息正确" "测试异常消息" msg
  | _ -> (
      ...
      try
        ...
        check bool "应该抛出异常" false true
      with
      | PoetryParseError msg -> check string "空异常消息" "" msg
      | _ -> check bool "应该抛出PoetryParseError" false true)
...
let test_validate_char_count_extreme_cases () =
  ...
  (* 测试大数值不匹配 *)
  assert_poetry_parse_error (fun () -> validate_char_count 50 long_poem);
  ...
  (* 测试一字验证失败 *)
  assert_poetry_parse_error (fun () -> validate_char_count 2 "诗")
...
(** 主测试运行器 *)
let () = run "骆言Parser古典诗词模块增强测试覆盖 - Fix #1307" [ ("增强测试覆盖", enhanced_coverage_tests) ]
