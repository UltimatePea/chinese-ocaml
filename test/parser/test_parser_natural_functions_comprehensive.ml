(** 骆言自然语言函数解析器综合测试 - 技术债务改进：提升测试覆盖率 Fix #962
    ...
    本测试模块专门针对 parser_natural_functions.ml 模块进行全面功能测试， 重点测试自然语言函数定义解析功能的正确性和健壮性。
...
module TestUtils = struct
  ...
  let mock_expect_token state expected_token =
    ...
    if current_tok = expected_token then advance_parser state
    else raise (SyntaxError ("Token mismatch in test", create_test_pos 1 1))
...
module TestParseNaturalFunctionDefinition = struct
  let test_basic_function_definition () =
    let tokens =
      [
        TestUtils.make_token DefineKeyword 1 1;
        (* 定义 *)
        TestUtils.make_token (QuotedIdentifierToken "求平方") 1 2;
        (* function name *)
        TestUtils.make_token AcceptKeyword 1 3;
        (* 接受 *)
        TestUtils.make_token (QuotedIdentifierToken "x") 1 4;
        (* parameter name *)
        TestUtils.make_token Colon 1 5;
        (* : *)
        TestUtils.make_token ReturnWhenKeyword 1 6;
        (* 时返回 *)
        TestUtils.make_token (QuotedIdentifierToken "x") 1 7;
        TestUtils.make_token MultiplyKeyword 1 8;
        (* 乘以 *)
    ...
    try
      let result_expr, _final_state =
        parse_natural_function_definition ~expect_token:TestUtils.mock_expect_token
          ~parse_identifier:TestUtils.mock_parse_identifier
          ~skip_newlines:TestUtils.mock_skip_newlines ~parse_expr:TestUtils.mock_parse_expr state
      in
      (* 验证解析成功并返回有效表达式 *)
  ...
  let test_function_definition_with_parameters () =
    let tokens =
      [
        TestUtils.make_token DefineKeyword 1 1;
        (* 定义 *)
        TestUtils.make_token (QuotedIdentifierToken "计算和") 1 2;
        (* function name *)
        TestUtils.make_token AcceptKeyword 1 3;
        (* 接受 *)
        TestUtils.make_token (QuotedIdentifierToken "x") 1 4;
        ...
        (* 时返回 *)
        TestUtils.make_token (QuotedIdentifierToken "x") 1 7;
        TestUtils.make_token AddToKeyword 1 8;
        (* 加上 *)
  ...
  let test_invalid_function_definition () =
    ...
    let state = TestUtils.create_test_state tokens in
    let should_fail =
      TestUtils.expect_syntax_error (fun () ->
          parse_natural_function_definition ~expect_token:TestUtils.mock_expect_token
            ~parse_identifier:TestUtils.mock_parse_identifier
            ~skip_newlines:TestUtils.mock_skip_newlines ~parse_expr:TestUtils.mock_parse_expr state)
    in
  ...
  let tests =
    [
      ...
      test_case "Function definition with parameters" `Quick
...
module TestParseNaturalFunctionBody = struct
  let test_simple_function_body () =
    let tokens =
      [
        TestUtils.make_token ElseReturnKeyword 1 1;
        (* 否则返回 *)
    ...
    try
      let result_expr, _final_state =
        parse_natural_function_body ~expect_token:TestUtils.mock_expect_token
          ~parse_identifier:TestUtils.mock_parse_identifier
          ~skip_newlines:TestUtils.mock_skip_newlines ~parse_expr:TestUtils.mock_parse_expr
          "test_function" state
      in
      match result_expr with
...
module TestParseNaturalExpr = struct
  let test_simple_natural_expression () =
    ...
    try
      let _result_expr, _final_state =
        parse_natural_expr ~parse_expr:TestUtils.mock_parse_expr "取值操作" state
      in
  ...
  let test_arithmetic_natural_expression () =
    ...
    try
      let _result_expr, _final_state =
        parse_natural_expr ~parse_expr:TestUtils.mock_parse_expr "算术操作" state
      in
...
module TestParseNaturalConditional = struct
  let test_basic_conditional () =
    ...
    let state = TestUtils.create_test_state tokens in
    try
      let var_name, _op, _condition, _then_expr, _final_state =
        parse_natural_conditional ~expect_token:TestUtils.mock_expect_token
          ~parse_identifier:TestUtils.mock_parse_identifier
          ~skip_newlines:TestUtils.mock_skip_newlines ~parse_expr:TestUtils.mock_parse_expr "条件判断"
          state
      in
  ...
  let test_complex_conditional () =
    ...
    try
      let var_name, _op, _condition, _then_expr, _final_state =
        parse_natural_conditional ~expect_token:TestUtils.mock_expect_token
          ~parse_identifier:TestUtils.mock_parse_identifier
          ~skip_newlines:TestUtils.mock_skip_newlines ~parse_expr:TestUtils.mock_parse_expr "复杂条件"
          state
      in
...
module TestParseConditionalRelationWord = struct
  let test_comparison_operators () =
    let test_cases =
      [ ("大于", Gt); ("小于", Lt); ("等于", Eq); ("大于等于", Ge); ("小于等于", Le); ("不等于", Neq) ]
    in
    List.iter
      (fun (chinese_op, expected_op) ->
        let tokens =
          [
  ...
  let test_invalid_relation_word () =
    let tokens =
      [ TestUtils.make_token (QuotedIdentifierToken "无效关系词") 1 1; TestUtils.make_token EOF 1 2 ]
    in
    let state = TestUtils.create_test_state tokens in
    let should_fail =
      TestUtils.expect_syntax_error (fun () -> parse_conditional_relation_word state)
    in
...
module TestParseNaturalArithmeticContinuation = struct
  let test_addition_continuation () =
    ...
    try
      let _result_expr, _final_state =
        parse_natural_arithmetic_continuation ~parse_expr:TestUtils.mock_parse_expr base_expr "加法运算"
          state
      in
  ...
  let test_multiplication_continuation () =
    ...
    try
      let _result_expr, _final_state =
        parse_natural_arithmetic_continuation ~parse_expr:TestUtils.mock_parse_expr base_expr "乘法运算"
          state
      in
...
module TestParseNaturalComparisonPatterns = struct
  let test_basic_comparison_pattern () =
    ...
    try
      let _result_expr, _final_state =
        parse_natural_comparison_patterns ~parse_expr:TestUtils.mock_parse_expr "比较模式" state
      in
  ...
  let test_complex_comparison_pattern () =
    ...
    try
      let _result_expr, _final_state =
        parse_natural_comparison_patterns ~parse_expr:TestUtils.mock_parse_expr "复杂比较模式" state
      in
...
module TestParseNaturalFunctionHeader = struct
  let test_basic_function_header () =
    ...
    let state = TestUtils.create_test_state tokens in
    try
      let _func_type, func_name, _final_state =
        parse_natural_function_header ~expect_token:TestUtils.mock_expect_token
          ~parse_identifier:TestUtils.mock_parse_identifier
          ~skip_newlines:TestUtils.mock_skip_newlines state
      in
...
module TestPerformSemanticAnalysis = struct
  let test_basic_semantic_analysis () =
    ...
    with _ -> check bool "基本语义分析应该成功" false true
  ...
  let test_complex_semantic_analysis () =
    ...
    with _ -> check bool "复杂语义分析应该成功" false true
  ...
  let test_complete_natural_function_parsing () =
    let tokens =
      [
        TestUtils.make_token DefineKeyword 1 1;
        (* 定义 *)
        TestUtils.make_token (QuotedIdentifierToken "阶乘") 1 2;
        (* 函数名 *)
        TestUtils.make_token AcceptKeyword 1 3;
        (* 接受 *)
        TestUtils.make_token (QuotedIdentifierToken "n") 1 4;
        (* 参数名 *)
        TestUtils.make_token Colon 1 5;
        (* : *)
        TestUtils.make_token WhenKeyword 1 6;
        (* 当 *)
