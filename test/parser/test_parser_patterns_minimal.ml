...
module BasicPatternTests = struct
  ...
  let test_wildcard_pattern () =
    let tokens = [ (Underscore, create_test_pos 1 1); (EOF, create_test_pos 1 2) ] in
    let state = create_test_state tokens in
    try
      let pattern, _final_state = parse_pattern state in
      match pattern with WildcardPattern -> check bool "通配符模式解析成功" true true | _ -> fail "期望通配符模式"
    with exn -> fail ("解析异常: " ^ Printexc.to_string exn)

  (** 测试变量模式解析 *)
  let test_variable_pattern () =
    let tokens =
      [ (QuotedIdentifierToken "变量名", create_test_pos 1 1); (EOF, create_test_pos 1 2) ]
    in
    let state = create_test_state tokens in
    try
      let pattern, _final_state = parse_pattern state in
      match pattern with VarPattern name -> check string "变量模式名称" "变量名" name | _ -> fail "期望变量模式"
    with exn -> fail ("解析异常: " ^ Printexc.to_string exn)

  (** 测试整数字面量模式解析 *)
  let test_int_literal_pattern () =
    let tokens = [ (IntToken 42, create_test_pos 1 1); (EOF, create_test_pos 1 2) ] in
    let state = create_test_state tokens in
    try
      let pattern, _final_state = parse_pattern state in
      match pattern with
  ...
  let test_string_literal_pattern () =
    let tokens = [ (StringToken "hello", create_test_pos 1 1); (EOF, create_test_pos 1 2) ] in
    let state = create_test_state tokens in
    try
      let pattern, _final_state = parse_pattern state in
      match pattern with
  ...
  let test_bool_literal_pattern () =
    let tokens = [ (BoolToken true, create_test_pos 1 1); (EOF, create_test_pos 1 2) ] in
    let state = create_test_state tokens in
    try
      let pattern, _final_state = parse_pattern state in
      match pattern with
...
module ErrorHandlingTests = struct
  ...
  let test_unsupported_token_error () =
    let tokens =
      [ (LeftParen, create_test_pos 1 1); (* 不是字面量token *) (EOF, create_test_pos 1 2) ]
    in
    let state = create_test_state tokens in
    try
...
let tests =
  [
    ...
    ("错误处理", [ test_case "未支持token错误" `Quick ErrorHandlingTests.test_unsupported_token_error ]);
  ]
