(** 函数调用表达式解析器测试套件 测试覆盖 parser_expressions_calls.ml 模块的所有主要功能 Author: Alpha, 主要工作代理 - Fix #1615 *)
...
module TestHelpers = struct
  ...
  let create_parser_state tokens =
    let positioned_tokens =
      List.map (fun t -> (t, { filename = "test"; line = 1; column = 1 })) tokens
    in
  ...
  let check_parse_error description tokens expected_error_pattern =
    ...
    with
    | SyntaxError (msg, _)
      when String.length msg >= String.length expected_error_pattern
...
let test_chinese_number_argument () =
  ...
  let chinese_one_result =
    parse_single_argument simple_parse_expr (ChineseNumberToken "一") chinese_one_state
  in
...
let test_parenthesized_argument () =
  ...
  (* 测试中文括号 *)
  let chinese_paren_state =
    create_parser_state [ ChineseLeftParen; StringToken "test"; ChineseRightParen; EOF ]
  in
  let chinese_paren_result =
    parse_single_argument simple_parse_expr ChineseLeftParen chinese_paren_state
  in
...
let test_parse_function_arguments () =
  ...
  let expected_multi_args =
    [ LitExpr (IntLit 1); LitExpr (StringLit "hello"); LitExpr (BoolLit true) ]
  in
...
let test_function_call_or_variable () =
  ...
  let func_call_result =
    parse_function_call_or_variable simple_parse_expr "test_func" func_call_state
  in
  let expected_func_call =
    FunCallExpr (VarExpr "test_func", [ LitExpr (IntLit 42); LitExpr (StringLit "world") ])
  in
...
let test_is_function_argument_token () =
  ...
  check bool "QuotedIdentifierToken是参数token" true
    (is_function_argument_token (QuotedIdentifierToken "x"));
...
let test_parse_function_call_context () =
  ...
  let expected_context =
    FunCallExpr (VarExpr "context_func", [ LitExpr (FloatLit 2.5); LitExpr (BoolLit true) ])
  in
...
let test_complex_function_calls () =
  ...
  let simple_func_result =
    parse_function_call_or_variable simple_parse_expr "simple_func" simple_func_state
  in
  let expected_simple_func =
    FunCallExpr (VarExpr "simple_func", [ LitExpr (IntLit 10); LitExpr (StringLit "nested") ])
  in
  ...
  let chinese_result =
    parse_function_call_or_variable simple_parse_expr "chinese_func" chinese_state
  in
  let expected_chinese =
    FunCallExpr (VarExpr "chinese_func", [ LitExpr (IntLit 3); LitExpr (IntLit 1) ])
  in
...
let test_edge_cases () =
  ...
  let one_only_result =
    parse_function_call_or_variable simple_parse_expr "one_func" one_only_state
  in
  ...
  let many_args_result =
    parse_function_call_or_variable simple_parse_expr "many_args_func" many_args_state
  in
  let expected_many_args =
    FunCallExpr
      ( VarExpr "many_args_func",
        [
          LitExpr (IntLit 1);
          LitExpr (IntLit 2);
          LitExpr (IntLit 3);
...
let () =
  run "Parser_expressions_calls Tests"
    [
      ...
      ("函数参数列表解析", [ test_case "参数列表解析功能" `Quick test_parse_function_arguments ]);
      ( "函数调用识别",
        [
      ...
      ("错误处理", [ test_case "错误处理机制" `Quick test_error_handling ]);
      ( "复杂场景",
        [
