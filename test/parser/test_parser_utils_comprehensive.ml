...
let test_parse_identifier () =
  ...
  try
    ...
    fail "应该抛出异常"
  with SyntaxError (msg, _) -> check bool "错误消息包含引用" true (String.contains msg (String.get "引" 0))
...
let test_expect_token_punctuation () =
  ...
  try
    ...
    fail "应该抛出异常"
  with SyntaxError (msg, _) -> check bool "错误消息包含期望" true (String.contains msg (String.get "期" 0))
...
let test_parse_literal () =
  let tokens =
    [ IntToken 42; ChineseNumberToken "七"; FloatToken 3.14; StringToken "hello"; BoolToken true ]
  in
  let state = TestHelpers.create_test_state tokens in
...
let test_try_parse_basic_type () =
  ...
  (match result1 with Some (BaseTypeExpr IntType, _) -> () | _ -> fail "整数类型解析错误");
...
let test_error_handling () =
  ...
  check bool "意外token错误" true
    (match error1 with
    | SyntaxError (msg, _) -> String.contains msg (String.get "意" 0)
    | _ -> false);

  let error2 = make_expected_but_found_error "期望" "实际" pos in
  check bool "期望错误消息" true
    (match error2 with
    | SyntaxError (msg, _) -> String.contains msg (String.get "期" 0)
    | _ -> false)
...
let test_performance () =
  ...
  let rec advance_n_times n state =
    if n <= 0 then state else advance_n_times (n - 1) (advance_parser state)
  in
  ...
  (* 大型复合标识符测试 *)
  let compound_tokens =
    Array.to_list (Array.init 50 (fun i -> QuotedIdentifierToken ("部分" ^ string_of_int i)))
    @ [ ValueKeyword ]
  in
...
let token_expectation_tests =
  [ test_case "expect_token" `Quick test_expect_token; test_case "is_token" `Quick test_is_token ]
...
let type_parsing_tests = [ test_case "try_parse_basic_type" `Quick test_try_parse_basic_type ]
...
let performance_tests = [ test_case "performance" `Quick test_performance ]
