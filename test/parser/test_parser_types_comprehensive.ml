(** 解析器类型模块全面测试套件 测试覆盖parser_types.ml模块的所有核心功能 *)
...
module TestHelpers = struct
  ...
  (** 创建基本类型表达式token序列 *)
  let create_basic_type_tokens () = [ IntTypeKeyword; EOF ]

  (** 创建函数类型token序列 *)
  let create_function_type_tokens () = [ IntTypeKeyword; Arrow; StringTypeKeyword; EOF ]
  ...
  let create_variant_type_tokens () =
    [
      VariantKeyword;
      QuotedIdentifierToken "Some";
      Colon;
      IntTypeKeyword;
      Pipe;
      QuotedIdentifierToken "None";
      EOF;
    ]
  ...
  let create_variant_labels_tokens () =
    [
      QuotedIdentifierToken "Red";
      Pipe;
      QuotedIdentifierToken "Green";
      Pipe;
      QuotedIdentifierToken "Blue";
      Colon;
      StringTypeKeyword;
      EOF;
    ]
  ...
  (** 创建类型变量token序列 *)
  let create_type_var_tokens () = [ QuotedIdentifierToken "T"; EOF ]

  (** 创建类型定义token序列 *)
  let create_type_definition_tokens () = [ IntTypeKeyword; EOF ]

  (** 创建变体构造器token序列 *)
  let create_variant_constructors_tokens () =
    [
      Pipe;
      IdentifierTokenSpecial "Some";
      OfKeyword;
      IntTypeKeyword;
      Pipe;
      IdentifierTokenSpecial "None";
      EOF;
    ]

  (** 创建私有类型token序列 *)
  let create_private_type_tokens () = [ PrivateKeyword; StringTypeKeyword; EOF ]

  (** 创建别名类型token序列 *)
  let create_alias_type_tokens () = [ IntTypeKeyword; EOF ]
  ...
  (** 创建模块类型名称token序列 *)
  let create_module_type_name_tokens () = [ QuotedIdentifierToken "MyModuleType"; EOF ]

  (** 创建值签名token序列 *)
  let create_value_signature_tokens () =
    [
      LetKeyword;
      IdentifierTokenSpecial "func";
      Colon;
      IntTypeKeyword;
      Arrow;
      StringTypeKeyword;
  ...
  (** 创建简单类型签名token序列 *)
  let create_simple_type_signature_tokens () = [ TypeKeyword; IdentifierTokenSpecial "t"; EOF ]
  ...
  (** 验证类型表达式解析结果 *)
  let verify_type_expression_parse tokens = assert_parse_success parse_type_expression tokens
  ...
  (** 验证类型定义解析结果 *)
  let verify_type_definition_parse tokens = assert_parse_success parse_type_definition tokens

  (** 验证模块类型解析结果 *)
  let verify_module_type_parse tokens = assert_parse_success parse_module_type tokens
end
...
let test_parse_advance_name_pattern () =
  ...
  check bool "advance->name模式解析应该成功" true
    (try
       let _ = parse_advance_name_pattern state in
       true
     with _ -> false)
...
let test_parse_advance_name_colon_pattern () =
  ...
  check bool "advance->name->colon模式解析应该成功" true
    (try
       let _ = parse_advance_name_colon_pattern state in
       true
     with _ -> false)
...
let test_parse_variant_labels_basic () =
  ...
  check bool "基础变体标签解析应该成功" true
    (try
       let _ = parse_variant_labels state [] in
       true
     with _ -> false)
...
let test_parse_variant_labels_edge_cases () =
  ...
  check bool "单个变体标签解析应该成功" true
    (TestHelpers.assert_parse_success
       (fun state ->
         let _, state' = parse_variant_labels state [] in
         ((), state'))
       single_label_tokens);
...
let test_parse_type_expression_basic_types () =
  ...
  check bool "基本类型表达式解析应该成功" true (TestHelpers.verify_type_expression_parse basic_type_tokens);
  ...
  List.iter
    (fun tokens -> check bool "各种基本类型解析应该成功" true (TestHelpers.verify_type_expression_parse tokens))
    type_variants

let test_parse_type_expression_function_types () =
  (* 测试函数类型解析 *)
  let function_type_tokens = TestHelpers.create_function_type_tokens () in
  check bool "函数类型表达式解析应该成功" true (TestHelpers.verify_type_expression_parse function_type_tokens);
...
let test_parse_type_expression_variant_types () =
  ...
  check bool "变体类型表达式解析应该成功" true (TestHelpers.verify_type_expression_parse variant_type_tokens)

let test_parse_type_expression_type_variables () =
  (* 测试类型变量解析 *)
  let type_var_tokens = TestHelpers.create_type_var_tokens () in
  check bool "类型变量表达式解析应该成功" true (TestHelpers.verify_type_expression_parse type_var_tokens)

let test_parse_type_expression_parenthesized () =
  (* 测试括号类型表达式 *)
  let paren_tokens = TestHelpers.create_parenthesized_type_tokens () in
  check bool "括号类型表达式解析应该成功" true (TestHelpers.verify_type_expression_parse paren_tokens);

  (* 测试中文括号类型表达式 *)
  let chinese_paren_tokens = TestHelpers.create_chinese_parenthesized_type_tokens () in
  check bool "中文括号类型表达式解析应该成功" true (TestHelpers.verify_type_expression_parse chinese_paren_tokens)
...
let test_parse_basic_type_expression () =
  ...
  check bool "基本类型表达式解析应该成功" true (TestHelpers.verify_basic_type_expression_parse basic_tokens);
...
let test_parse_type_definition_alias () =
  ...
  check bool "别名类型定义解析应该成功" true (TestHelpers.verify_type_definition_parse alias_tokens)

let test_parse_type_definition_variant_constructors () =
  (* 测试变体构造器类型定义 *)
  let variant_tokens = TestHelpers.create_variant_constructors_tokens () in
  check bool "变体构造器类型定义解析应该成功" true (TestHelpers.verify_type_definition_parse variant_tokens)

let test_parse_type_definition_private () =
  (* 测试私有类型定义 *)
  let private_tokens = TestHelpers.create_private_type_tokens () in
  check bool "私有类型定义解析应该成功" true (TestHelpers.verify_type_definition_parse private_tokens)
...
let test_parse_type_definition_polymorphic_variant () =
  ...
  let polymorphic_variant_tokens =
    [
      VariantKeyword;
      QuotedIdentifierToken "A";
      Pipe;
      QuotedIdentifierToken "B";
      Colon;
      IntTypeKeyword;
      EOF;
    ]
...
let test_parse_variant_constructors () =
  ...
  check bool "变体构造器解析应该成功" true (TestHelpers.verify_type_definition_parse constructor_tokens);
  ...
  in
  check bool "中文管道符变体构造器解析应该成功" true (TestHelpers.verify_type_definition_parse chinese_pipe_tokens)
...
let test_parse_module_type_signature () =
  ...
  check bool "模块类型签名解析应该成功" true (TestHelpers.verify_module_type_parse signature_tokens)

let test_parse_module_type_name () =
  (* 测试模块类型名称解析 *)
  let name_tokens = TestHelpers.create_module_type_name_tokens () in
  check bool "模块类型名称解析应该成功" true (TestHelpers.verify_module_type_parse name_tokens)
...
let test_parse_signature_item_value () =
  ...
  check bool "值签名项解析应该成功" true
    (try
       let _ = parse_signature_item state in
       true
     with _ -> false)
...
let test_parse_signature_item_type () =
  ...
  check bool "简单类型签名项解析应该成功" true
    (try
       let _ = parse_signature_item simple_state in
       true
     with _ -> false)
...
let test_parse_signature_items () =
  ...
  let signature_items_tokens =
    [
      LetKeyword;
      IdentifierTokenSpecial "x";
      Colon;
      IntTypeKeyword;
      TypeKeyword;
      IdentifierTokenSpecial "t";
      Assign;
      StringTypeKeyword;
      EndKeyword;
      EOF;
  ...
  check bool "签名项列表解析应该成功" true
    (try
       let _ = parse_signature_items [] state in
       true
     with _ -> false);
  ...
  check bool "空签名项列表解析应该成功" true
    (try
       let _ = parse_signature_items [] empty_state in
       true
     with _ -> false)
...
let test_error_handling_invalid_type_expressions () =
  (* 测试无效类型表达式 *)
  let invalid_tokens = [ Plus; EOF ] in
  (* 运算符不是有效的类型表达式 *)
...
let test_error_handling_invalid_variant_labels () =
  (* 测试无效变体标签 *)
  let invalid_variant_tokens = [ IntTypeKeyword; EOF ] in
  (* 不是引用标识符 *)
...
let test_error_handling_invalid_signature_items () =
  (* 测试无效签名项 *)
  let invalid_signature_tokens = [ Plus; EOF ] in
  (* 不是有效的签名项开始 *)
...
let test_complex_type_expressions () =
  (* 测试复杂的嵌套类型表达式 *)
  let complex_tokens =
    [ LeftParen; IntTypeKeyword; Arrow; StringTypeKeyword; RightParen; Arrow; BoolTypeKeyword; EOF ]
  in
  check bool "复杂嵌套类型表达式解析应该成功" true (TestHelpers.verify_type_expression_parse complex_tokens);
  ...
  in
  check bool "多层嵌套类型表达式解析应该成功" true (TestHelpers.verify_type_expression_parse nested_tokens)
...
let test_complex_variant_definitions () =
  ...
  in
  check bool "复杂变体定义解析应该成功" true (TestHelpers.verify_type_definition_parse complex_variant_tokens)
...
let test_complex_module_signatures () =
  ...
  let complex_sig_tokens =
    [
      ...
      IntTypeKeyword;
      ModuleKeyword;
      IdentifierTokenSpecial "Inner";
      Colon;
      QuotedIdentifierToken "InnerType";
      ExceptionKeyword;
      IdentifierTokenSpecial "MyError";
      OfKeyword;
      StringTypeKeyword;
      EndKeyword;
      EOF;
  ...
  in
  check bool "复杂模块签名解析应该成功" true (TestHelpers.verify_module_type_parse complex_sig_tokens)
...
let test_type_parsing_performance () =
  ...
  let rec create_large_function_type depth =
    ...
    else create_large_function_type (depth - 1) @ [ Arrow ] @ create_large_function_type (depth - 1)
  in
  ...
  check bool "大型类型表达式解析应该在合理时间内完成" true (end_time -. start_time < 1.0)
...
let test_variant_parsing_performance () =
  ...
  let rec create_many_constructors n acc =
    if n <= 0 then acc
      ...
      let constructor =
        [
          Pipe; IdentifierTokenSpecial ("Constructor" ^ string_of_int n); OfKeyword; IntTypeKeyword;
        ]
      in
  ...
  check bool "大量变体构造器解析应该在合理时间内完成" true (end_time -. start_time < 0.5)
...
let test_type_parsing_integration () =
  ...
  (* 2. 解析函数类型 *)
  let function_result =
    TestHelpers.verify_type_expression_parse [ IntTypeKeyword; Arrow; StringTypeKeyword; EOF ]
  in
  check bool "集成测试：函数类型解析应该成功" true function_result;

  (* 3. 解析变体类型 *)
  let variant_result =
    TestHelpers.verify_type_expression_parse
      [ VariantKeyword; QuotedIdentifierToken "A"; Pipe; QuotedIdentifierToken "B"; EOF ]
  in
  check bool "集成测试：变体类型解析应该成功" true variant_result;

  (* 4. 解析类型定义 *)
  let type_def_result =
    TestHelpers.verify_type_definition_parse
      [ Pipe; IdentifierTokenSpecial "Some"; OfKeyword; IntTypeKeyword; EOF ]
  in
  check bool "集成测试：类型定义解析应该成功" true type_def_result;

  (* 5. 解析模块类型 *)
  let module_type_result =
    TestHelpers.verify_module_type_parse
      [ SigKeyword; LetKeyword; IdentifierTokenSpecial "x"; Colon; IntTypeKeyword; EndKeyword; EOF ]
  in
...
let variant_constructor_tests = [ test_case "变体构造器解析测试" `Quick test_parse_variant_constructors ]
...
let signature_items_tests = [ test_case "签名项列表解析测试" `Quick test_parse_signature_items ]
...
let integration_tests = [ test_case "类型解析集成测试" `Quick test_type_parsing_integration ]
