...
module Test_PoetryFormatting = struct
  ...
  let test_detailed_evaluation_report () =
    let detailed_report = PoetryFormatting.format_evaluation_detailed_report "五言绝句" "静夜思" 9.2 "韵律工整，意境深远" in
...
module Test_ClassicalFormatting = struct
  ...
  let test_classical_terminology_formatting () =
    ...
    List.iter (fun term ->
      check bool (term ^ "术语有效") true (String.length term > 0)
    ) poetic_terms

  (** 测试古典诗词评价维度 *)
  let test_classical_evaluation_dimensions () =
    let dimensions = [
      ("音韵美", 8.5); ("意境美", 9.2); ("结构美", 7.8); 
      ("语言美", 8.9); ("情感美", 9.5); ("技巧美", 8.1)
    ] in
    
    List.iter (fun (dim, score) ->
...
module Test_AncientStyleFormatting = struct
  ...
  let test_ancient_style_specific_formatting () =
    ...
    List.iter (fun expr ->
      ...
      check bool (expr ^ "包含古典词汇") true (
        contains_substring expr "乃" || contains_substring expr "故" || 
        contains_substring expr "凡" || contains_substring expr "若" ||
        contains_substring expr "然" || contains_substring expr "盖" ||
        contains_substring expr "诚" || contains_substring expr "善"
      )
    ) ancient_expressions
  ...
  let test_ancient_grammar_structures () =
    ...
    List.iter (fun pattern ->
      check bool (pattern ^ "语法模式有效") true (String.length pattern > 0)
    ) grammar_patterns

  (** 测试古雅体错误提示 *)
  let test_ancient_style_error_messages () =
    let ancient_errors = [
      "此非古雅之道也"; "语法有误，当如是云"; "词序不当，宜改之";
      "用词不雅，可择他词"; "句式繁复，当简之"
    ] in
    
    List.iter (fun error ->
      check bool (error ^ "古雅体错误消息有效") true (String.length error > 0);
      check bool (error ^ "体现古典语言风格") true (
...
module Test_PoetryAnalysisTools = struct
  ...
  let test_structure_analysis_tools () =
    let structure_tools = [
      "对偶分析器"; "起承转合检测器"; "章法布局分析器";
      "意象层次分析器"; "情感递进分析器"; "主题统一性检查器"
    ] in
    
    List.iter (fun tool ->
      check bool (tool ^ "工具有效") true (String.length tool > 0);
      check bool (tool ^ "命名规范") true (contains_substring tool "分析" || contains_substring tool "检测" || contains_substring tool "检查")
    ) structure_tools
  ...
  let test_aesthetic_evaluation_tools () =
    ...
    List.iter (fun (criterion, description) ->
  ...
  let test_poetry_statistics_tools () =
    let statistics = [
      ("字数统计", 28); ("句数统计", 4); ("韵脚统计", 2);
      ("平声字数", 14); ("仄声字数", 14); ("重复字数", 0)
    ] in
    
    List.iter (fun (stat_name, count) ->
  ...
  let test_poetry_suggestion_tools () =
    ...
    List.iter (fun suggestion ->
      ...
      check bool (suggestion ^ "包含建议词汇") true (
        contains_substring suggestion "建议" || contains_substring suggestion "可" ||
        contains_substring suggestion "应" || contains_substring suggestion "宜"
      )
    ) suggestions
end
...
module Test_EdgeCasesAndSpecialPoetryScenarios = struct
  ...
  let test_special_characters_and_punctuation () =
    let special_cases = [
      "含标点诗句：春风，又绿江南岸。";
      "含数字诗句：一去二三里";
      "含英文诗句：Spring is here";
      "含表情符号：春天🌸来了"
    ] in
    
    List.iter (fun case ->
      check bool (case ^ "特殊情况处理有效") true (String.length case > 0)
    ) special_cases
...
let () =
  run "骆言诗词格式化模块测试"
    [
      ...
      ( "古典诗词格式化",
        [
          test_case "古典诗词特有格式化" `Quick Test_ClassicalFormatting.test_classical_poetry_specific_formatting;
          test_case "古典诗词术语格式化" `Quick Test_ClassicalFormatting.test_classical_terminology_formatting;
          test_case "古典诗词评价维度" `Quick Test_ClassicalFormatting.test_classical_evaluation_dimensions;
        ] );
      ( "古雅体格式化",
        [
          test_case "古雅体特殊格式化" `Quick Test_AncientStyleFormatting.test_ancient_style_specific_formatting;
      ...
      ( "边界情况和特殊场景",
        [
          test_case "极端长度诗词处理" `Quick Test_EdgeCasesAndSpecialPoetryScenarios.test_extreme_length_poetry;
          test_case "非标准诗体处理" `Quick Test_EdgeCasesAndSpecialPoetryScenarios.test_non_standard_poetry_forms;
          test_case "特殊字符和标点处理" `Quick Test_EdgeCasesAndSpecialPoetryScenarios.test_special_characters_and_punctuation;
          test_case "多语言诗词处理" `Quick Test_EdgeCasesAndSpecialPoetryScenarios.test_multilingual_poetry_handling;
        ] );
    ]