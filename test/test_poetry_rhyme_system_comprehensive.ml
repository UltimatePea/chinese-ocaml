...
let test_rhyme_data_initialization () =
  ...
  (* 测试数据初始化 *)
  test_assert "数据初始化成功" (initialize_data (); is_data_loaded ());
...
let test_boundary_conditions () =
  ...
  (* 测试未知字符查找 *)
  let unknown_char_val = String.get "𝓍" 0 in (* Unicode 特殊字符 *)
  ...
  (* 测试数据重新加载 *)
  test_assert "数据重新加载成功" (reload_data (); is_data_loaded ());
...
let test_rhyme_analysis_integration () =
  ...
  | Some (_, group1), Some (_, group2) ->
      test_assert "不同韵组字符韵律不匹配" (group1 <> group2)
  | _ ->
      test_assert "跨韵组差异测试基础信息可用" false)
