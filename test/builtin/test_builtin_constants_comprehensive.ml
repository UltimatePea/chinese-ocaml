...
let () =
  ...
  (try
     ...
     let test_digit name expected_value =
       ...
       | Some _ -> Printf.printf "❌ 中文数字常量 '%s' 值不匹配，期望 %d\n" name expected_value
       | None -> Printf.printf "❌ 中文数字常量 '%s' 未找到\n" name
     in
   ...
   with e -> Printf.printf "❌ 中文数字常量表测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     let missing_constants =
       List.filter (fun const -> not (List.mem const actual_constants)) expected_constants
     in
     let extra_constants =
       List.filter (fun const -> not (List.mem const expected_constants)) actual_constants
     in

     if missing_constants = [] && extra_constants = [] then Printf.printf "✅ 常量表完整性检查通过，包含所有预期常量\n"
     else (
   ...
   with e -> Printf.printf "❌ 常量表完整性测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     | _ -> Printf.printf "❌ make_chinese_number_constant() 未返回预期的错误消息\n");

     Printf.printf "✅ make_chinese_number_constant 函数测试通过\n"
   with e -> Printf.printf "❌ make_chinese_number_constant 函数测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     let check_value_type name value =
       match value with
       | IntValue n when n >= 0 && n <= 9 -> Printf.printf "✅ 常量 '%s' 类型正确: IntValue(%d)\n" name n
       | IntValue n -> Printf.printf "❌ 常量 '%s' 值超出范围: IntValue(%d)\n" name n
       | _ -> Printf.printf "❌ 常量 '%s' 类型错误，应为 IntValue\n" name
     in

     List.iter (fun (name, value) -> check_value_type name value) chinese_number_constants;
     Printf.printf "✅ 所有常量类型检查完成\n"
   with e -> Printf.printf "❌ 常量类型测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if duration < 1.0 then Printf.printf "✅ 常量访问性能良好\n" else Printf.printf "⚠️  常量访问性能需要优化\n"
  ...
  (try
     ...
     if original_length = current_length then Printf.printf "✅ 常量表保持不可变性，长度一致: %d\n" current_length
     else Printf.printf "❌ 常量表长度发生变化: %d -> %d\n" original_length current_length;

     Printf.printf "✅ 常量不可变性测试完成\n"
   with e -> Printf.printf "❌ 不可变性测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     let found_non_existent =
       List.filter (fun name -> List.assoc_opt name chinese_number_constants <> None) non_existent
     in

     if found_non_existent = [] then Printf.printf "✅ 不存在的中文数字正确未找到\n"
     else Printf.printf "⚠️  意外找到的数字: %s\n" (String.concat ", " found_non_existent);

     (* 测试Unicode字符处理 *)
     let unicode_numbers = [ "０"; "１"; "２"; "３"; "４"; "５"; "６"; "７"; "８"; "９" ] in
     let found_unicode =
       List.filter
         (fun name -> List.assoc_opt name chinese_number_constants <> None)
         unicode_numbers
     in

     if found_unicode = [] then Printf.printf "✅ 全角数字字符正确未包含在常量表中\n"
     else Printf.printf "⚠️  意外包含的全角数字: %s\n" (String.concat ", " found_unicode);

     Printf.printf "✅ 边界条件测试全部完成\n"
   with e -> Printf.printf "❌ 边界条件测试失败: %s\n" (Printexc.to_string e));
