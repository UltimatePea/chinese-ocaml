...
let () =
  ...
  (try
     ...
     | ListValue filtered_files ->
         ...
         let ly_file_names =
           List.map (function StringValue name -> name | _ -> "") filtered_files
         in
         Printf.printf "LIST: .ly文件列表: %s\n" (String.concat ", " ly_file_names);

         (* 验证结果 *)
         if List.length filtered_files = 3 then Printf.printf "√ 过滤数量正确\n"
         else Printf.printf "X 过滤数量不正确，期望3个，实际%d个\n" (List.length filtered_files)
   ...
   with e -> Printf.printf "X filter_ly_files_function 测试失败: %s\n" (Printexc.to_string e));
     ...
     List.iteri
       (fun i (input, expected) ->
         ...
         | StringValue output ->
             ...
             else Printf.printf "X 井号注释移除测试 %d 失败: 期望 \"%s\"，实际 \"%s\"\n" (i + 1) expected output
         | _ -> Printf.printf "X 井号注释移除测试 %d 返回类型错误\n" (i + 1))
       test_cases
   with e -> Printf.printf "X remove_hash_comment_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     List.iteri
       ...
       test_cases
   with e ->
     Printf.printf "X remove_double_slash_comment_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     List.iteri
       ...
       test_cases
   with e -> Printf.printf "X remove_block_comments_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     List.iteri
       ...
       test_cases
   with e -> Printf.printf "X remove_luoyan_strings_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     List.iteri
       ...
       test_cases
   with e -> Printf.printf "X remove_english_strings_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "X 综合处理测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     if duration < 10.0 then Printf.printf "√ 性能表现良好\n" else Printf.printf "⚠️  性能可能需要优化\n"
  ...
  (try
     ...
     (match result1 with
     | StringValue s when String.length s = 10000 -> Printf.printf "√ 极长字符串处理测试通过\n"
