...
module TestDataGenerator = struct
  ...
  let setup_test_dir () =
    ...
    if not (Sys.file_exists test_data_dir) then Unix.mkdir test_data_dir 0o755;
    test_data_dir
...
module TestFileIO = struct
  ...
  let test_successful_word_class_loading _test_dir =
    ...
    | Success data ->
        ...
        List.iter (fun (word, class_name) -> printf "   %s -> %s\n" word class_name) data;
  ...
  let test_parse_error_handling _test_dir =
    ...
    | Error (ParseError (_file, msg)) -> printf "✅ 正确捕获解析错误: %s\n" msg
    | Error err ->
  ...
  let test_cache_functionality _test_dir =
    ...
    | Success data1 -> (
        ...
        | Success data2 ->
            ...
            if second_load_time <= first_load_time then printf "✅ 缓存性能提升验证通过\n"
            else printf "⚠️  缓存性能提升不明显（测试数据较小）\n"
        | Error _ ->
            printf "❌ 第二次加载失败\n";
            failwith "第二次缓存加载失败")
    | Error _ ->
...
module TestValidation = struct
  let test_string_list_validation () =
    ...
    | Success _ -> printf "⚠️  包含空字符串的数据通过了验证（验证器可能较为宽松）\n"
    | Error (ValidationError (dtype, msg)) -> printf "✅ 正确捕获无效数据: %s - %s\n" dtype msg
    | Error _ ->
        printf "❌ 错误类型不正确\n";
...
module TestPerformance = struct
  let test_loading_performance _test_dir =
    ...
    if with_cache_time < no_cache_time then printf "✅ 缓存性能提升验证通过\n"
    else printf "⚠️  缓存性能提升不明显（可能受测试环境影响）\n";
    printf "\n"
end
