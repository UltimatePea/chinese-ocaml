...
let () =
  ...
  (try
   ...
   with e -> Printf.printf "X print_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "⚠️  测试环境清理: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "X file_exists_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     let result1 =
       match write_file_function [ StringValue test_file_path ] with
     ...
     let result2 =
       match write_file_function [ StringValue empty_file_path ] with
   ...
   with e -> Printf.printf "X write_file_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     with _ -> Printf.printf "√ 读取不存在文件正确抛出异常\n"
   with e -> Printf.printf "X read_file_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     (match result1 with
     | ListValue contents ->
         let content_strings = List.map (function StringValue s -> s | _ -> "") contents in
         Printf.printf "√ 目录列举测试通过: 找到 %d 个项目\n" (List.length content_strings);
         Printf.printf "📋 目录内容: %s\n" (String.concat ", " content_strings);

         (* 验证所有创建的文件都被列出 *)
         let missing_files =
           List.filter (fun file -> not (List.mem file content_strings)) test_files
         in
         if missing_files = [] then Printf.printf "√ 所有创建的文件都被正确列出\n"
         else Printf.printf "X 缺少文件: %s\n" (String.concat ", " missing_files)
     ...
     with _ -> Printf.printf "√ 列举不存在目录正确抛出异常\n"
   with e -> Printf.printf "X list_directory_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     (* 写入原始内容 *)
     let _ =
       match write_file_function [ StringValue consistency_file ] with
   ...
   with e -> Printf.printf "X 一致性测试失败: %s\n" (Printexc.to_string e));

  (* 测试边界条件和性能 *)
  Printf.printf "\n⚡ 性能和边界条件测试\n";
  (try
     (* 测试大文件写入和读取 *)
     let large_content = String.make 100000 'A' in
     (* 100K ASCII 字符 *)
     let large_file = "/tmp/luoyan_large_test.txt" in

     let start_time = Sys.time () in
     let _ =
       match write_file_function [ StringValue large_file ] with
     ...
     for i = 1 to 100 do
       ...
       let _ =
         match write_file_function [ StringValue temp_file ] with
  ...
  (try
     ...
     (* 清理测试目录 *)
     if Sys.file_exists test_dir_path then (
     ...
     Printf.printf "√ 测试环境清理完成\n"
   with e -> Printf.printf "⚠️  清理过程中出现错误: %s\n" (Printexc.to_string e));
