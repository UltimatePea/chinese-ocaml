...
module NumericConversionTests = struct
  ...
  let test_float_to_string () =
    ...
    check bool "负浮点数转字符串应正确" true
      (match result2 with
      | StringValue s -> String.contains s '-' && String.contains s '.'
      | _ -> false);
...
module FunctionRegistrationTests = struct
  ...
  let test_type_conversion_functions_table () =
    let expected_functions =
      [ "整数转字符串"; "浮点数转字符串"; "字符串转整数"; "字符串转浮点数"; "整数转浮点数"; "浮点数转整数"; "布尔值转字符串" ]
    in
  ...
  let test_all_functions_callable () =
    List.iter
      (fun (name, func_value) ->
        ...
        | _ -> fail (Printf.sprintf "函数'%s'应为内置函数类型" name))
...
module ChineseProgrammingTests = struct
  ...
  let test_chinese_error_messages () =
    ...
    with
    | RuntimeError msg ->
        check bool "错误消息应包含中文字符" true (Str.string_match (Str.regexp ".*[无法转换].*") msg 0)
...
module RoundTripTests = struct
  ...
  let test_numeric_round_trip () =
    ...
    check bool "整数浮点往返转换应一致" true
      (TestUtils.values_equal float_to_int_result (IntValue original_int))
...
let test_suite =
  [
    ...
    ("布尔值转换", [ test_case "布尔值转字符串" `Quick BooleanConversionTests.test_bool_to_string ]);
    ( "错误处理",
      [
