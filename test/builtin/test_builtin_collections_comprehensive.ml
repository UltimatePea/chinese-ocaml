...
let () =
  ...
  (try
   ...
   with e -> Printf.printf "X length_function 测试失败: %s\n" (Printexc.to_string e));

  (* 测试 concat_function *)
  Printf.printf "\nCONCAT: 测试 concat_function\n";
  (try
     let result1 =
       match concat_function [ ListValue [ IntValue 1; IntValue 2 ] ] with
       | BuiltinFunctionValue f -> (
           match f [ ListValue [ IntValue 3; IntValue 4 ] ] with ListValue lst -> lst | _ -> [])
       | _ -> []
     in
     ...
     let result2 =
       match concat_function [ ListValue [] ] with
       | BuiltinFunctionValue f -> (
           match f [ ListValue [ IntValue 1 ] ] with ListValue lst -> lst | _ -> [])
       | _ -> []
     in
     ...
     let result3 =
       match concat_function [ ListValue [ IntValue 1 ] ] with
   ...
   with e -> Printf.printf "X concat_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     let is_positive =
       BuiltinFunctionValue
         (fun args ->
           ...
           match x with IntValue n when n > 0 -> BoolValue true | _ -> BoolValue false)
     in
     let result1 =
       match filter_function [ is_positive ] with
       | BuiltinFunctionValue f -> (
           match f [ ListValue [ IntValue (-1); IntValue 0; IntValue 1; IntValue 2 ] ] with
           | ListValue lst -> lst
           | _ -> [])
       | _ -> []
     in
   ...
   with e -> Printf.printf "X filter_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     let double_fun =
       BuiltinFunctionValue
         (fun args ->
           ...
           match x with IntValue n -> IntValue (n * 2) | _ -> x)
     in
     let result1 =
       match map_function [ double_fun ] with
       | BuiltinFunctionValue f -> (
           match f [ ListValue [ IntValue 1; IntValue 2; IntValue 3 ] ] with
           | ListValue lst -> lst
           | _ -> [])
       | _ -> []
     in
   ...
   with e -> Printf.printf "X map_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     let sum_fun =
       BuiltinFunctionValue
         (fun args ->
           match args with
           | [ acc; x ] -> (
               match (acc, x) with IntValue a, IntValue b -> IntValue (a + b) | _ -> acc)
           | _ -> IntValue 0)
     in
     let result1 =
       match fold_function [ sum_fun ] with
       | BuiltinFunctionValue f1 -> (
           match f1 [ IntValue 0 ] with
   ...
   with e -> Printf.printf "X fold_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
     ...
     let _ =
       sort_function
         [ ListValue [ StringValue "骆"; StringValue "言"; StringValue "编"; StringValue "程" ] ]
     in
   ...
   with e -> Printf.printf "X sort_function 测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
   ...
   with e -> Printf.printf "X reverse_function 测试失败: %s\n" (Printexc.to_string e));

  (* 测试 contains_function *)
  Printf.printf "\nCONTAINS: 测试 contains_function\n";
  (try
     let result1 =
       match contains_function [ IntValue 2 ] with
     ...
     let result2 =
       match contains_function [ IntValue 4 ] with
       | BuiltinFunctionValue f -> f [ ListValue [ IntValue 1; IntValue 2; IntValue 3 ] ]
       | _ -> BoolValue false
     in
  ...
  (try
   ...
   with e -> Printf.printf "X 边界条件测试失败: %s\n" (Printexc.to_string e));
