(** 骆言内置数学模块测试 - Phase 26 内置模块测试覆盖率提升
    ...
    本测试模块专门针对 builtin_math.ml 模块进行全面功能测试， 重点测试数学计算的正确性、边界条件和性能表现。
...
module TestUtils = struct
  ...
  (** 获取列表长度 *)
  let get_list_length list_val = match list_val with ListValue items -> List.length items | _ -> 0
end
...
module InverseTrigonometricTests = struct
  ...
  let test_asin_function () =
    ...
    check bool "asin(1) 应为 π/2" true
      (match result2 with
      | FloatValue f -> abs_float (f -. 1.5707963267948966) < 1e-10
      | _ -> false);
...
module ChineseMathTests = struct
  ...
  let test_chinese_number_processing () =
    ...
    List.iter
      (fun expected ->
        check bool
          (Printf.sprintf "中文范围应包含%d" expected)
          true
