...
(** 测试清理函数 *)
let cleanup_test_file () = if Sys.file_exists test_file_path then Sys.remove test_file_path
...
let test_print_function () =
  ...
  check
    ...
    "打印字符串返回Unit" (create_test_unit ()) result;
  ...
  check
    (module Yyocamlc_lib.Value_operations.ValueModule)
    "打印布尔值返回Unit" (create_test_unit ()) result
...
let test_list_directory_function () =
  ...
  | ListValue files ->
      ...
      List.iter
        (fun file ->
          match file with StringValue _ -> check bool "文件名是字符串" true true | _ -> fail "文件名应该是字符串")
        files
  | _ -> fail "应该返回列表类型"
...
let test_list_directory_error_handling () =
  ...
  try
    ...
    fail "应该抛出运行时错误"
  with
  | Yyocamlc_lib.Value_operations.RuntimeError _ -> check bool "正确处理目录不存在错误" true true
  | _ -> fail "应该抛出运行时错误"
...
let test_file_operations_integration () =
  ...
  check
    (module Yyocamlc_lib.Value_operations.ValueModule)
    "初始文件不存在" (create_test_bool false) exists_result;
  ...
  check
    (module Yyocamlc_lib.Value_operations.ValueModule)
    "写入后文件存在" (create_test_bool true) exists_result;

  (* Step 4: 读取文件并验证内容 *)
  let read_result = read_file_function [ create_test_string test_file_path ] in
  check
    (module Yyocamlc_lib.Value_operations.ValueModule)
    "读取内容正确" (create_test_string test_content) read_result;
...
let test_edge_cases () =
  ...
  check
    (module Yyocamlc_lib.Value_operations.ValueModule)
    "空字符串写入读取" (create_test_string "") read_result;
  ...
  check
    (module Yyocamlc_lib.Value_operations.ValueModule)
    "中文文件名存在检查" (create_test_bool true) exists_result;

  (* 清理中文文件名测试文件 *)
  if Sys.file_exists chinese_file_path then Sys.remove chinese_file_path;
