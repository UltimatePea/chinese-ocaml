...
let test_validation_functions () =
  (* 测试验证函数存在性 *)
  check bool "验证函数应该存在" true
    (try
       ignore validate_string_list;
       true
     with _ -> false);
  check bool "词类对验证函数应该存在" true
    (try
       ignore validate_word_class_pairs;
       true
     with _ -> false)

let test_module_structure () =
  (* 测试模块结构完整性 - 通过函数存在性检查 *)
  check bool "加载函数应该存在" true
    (try
       ignore load_string_list;
       true
     with _ -> false);
  check bool "词类对加载函数应该存在" true
    (try
       ignore load_word_class_pairs;
       true
     with _ -> false);
  check bool "缓存清理函数应该存在" true
    (try
       ignore clear_cache;
       true
     with _ -> false)
