...
module TestUtils = struct
  ...
  (** 检查是否为内置函数值 *)
  let is_builtin_function_value = function 
...
module HashTableOptimizationTests = struct
  ...
  let test_hash_vs_linear_performance () =
    ...
    let test_name = match function_names with
      | [] -> "整数转字符串"
      | name :: _ -> name
    in
...
module IOFunctionTests = struct
  ...
  let test_file_exists_function () =
    (* 测试检查当前目录 *)
    let result1 = call_builtin_function "文件存在" [StringValue "."] in
  ...
  let test_list_directory_function () =
    try
      ...
      match result with
      | ListValue files -> 
          check bool "列出目录应返回文件列表" true (List.length files >= 0)
      | _ -> 
          check bool "列出目录应返回列表类型" false true
    with
    | _ -> Printf.printf "跳过列出目录测试（可能权限不足）\n"
end
...
module ArrayFunctionTests = struct
  ...
  let test_create_array_function () =
    ...
    | ArrayValue arr ->
        ...
        check bool "数组元素应正确" true (Array.for_all (fun v -> TestUtils.values_equal v (StringValue "test")) arr)
  ...
  let test_array_list_conversion () =
    ...
    | _ -> check bool "列表转数组应返回数组" false true;
...
module CollectionFunctionTests = struct
  ...
  let test_length_function () =
    ...
    check bool "字符串长度应正确" true (TestUtils.values_equal string_result (IntValue expected_string_length));
...
module UtilityFunctionTests = struct
  ...
  let test_filter_ly_files () =
    ...
    match result with
    | ListValue filtered ->
        check int "应过滤出2个ly文件" 2 (List.length filtered)
...
module ChineseNumberConstantTests = struct
  ...
  let test_all_chinese_numbers () =
    ...
    List.iter (fun (chinese_name, expected_value) ->
      let result = call_builtin_function chinese_name [] in
      check bool (Printf.sprintf "中文数字'%s'应正确" chinese_name) true 
...
module ErrorHandlingTests = struct
  ...
  let test_nonexistent_function_errors () =
    let nonexistent_functions = [
      "不存在的函数"; "invalid_function"; ""; "   "; "测试测试测试"
    ] in
    
    List.iter (fun func_name ->
      let error_case () = call_builtin_function func_name [] in
      check bool (Printf.sprintf "调用不存在函数'%s'应抛出错误" func_name) true 
...
module CompletenessValidationTests = struct
  ...
  let test_all_declared_functions_available () =
    ...
    List.iter (fun (category, functions) ->
      List.iter (fun func_name ->
        check bool (Printf.sprintf "%s类别的'%s'函数应可用" category func_name) true 
  ...
  let test_function_table_structural_integrity () =
    ...
    List.iter (fun (name, _) ->
      check bool (Printf.sprintf "函数'%s'在哈希表中应可查找" name) true 
  ...
  let test_all_modules_integrated () =
    let module_indicators = [
      ("IO模块", fun name -> List.exists (fun pattern -> String.contains name (String.get pattern 0)) ["打"; "读"; "写"; "文"]);
      ("数学模块", fun name -> List.exists (fun pattern -> String.contains name (String.get pattern 0)) ["范"; "求"; "最"]);
      ("字符串模块", fun name -> String.contains name (String.get "字" 0));
      ("数组模块", fun name -> String.contains name (String.get "数" 0));
      ("集合模块", fun name -> List.exists (fun pattern -> String.contains name (String.get pattern 0)) ["长"; "连"; "排"; "反"]);
      ("类型转换模块", fun name -> String.contains name (String.get "转" 0));
      ("工具模块", fun name -> List.exists (fun pattern -> String.contains name (String.get pattern 0)) ["过"; "移"]);
      ("常量模块", fun name -> List.exists (fun pattern -> name = pattern) ["一"; "二"; "三"; "四"; "五"]);
    ] in
    
    let all_function_names = get_builtin_function_names () in
    
    List.iter (fun (module_name, indicator_func) ->
...
let test_suite =
  [
    ("哈希表性能优化测试", [
      ...
      test_case "不存在函数哈希查找" `Quick HashTableOptimizationTests.test_nonexistent_function_hash_lookup;
    ]);
    ...
    ("并发安全测试", [
      test_case "哈希表线程安全" `Quick ConcurrencySafetyTests.test_hash_table_thread_safety;
    ]);
    
    ("中文编程深度测试", [
      test_case "中文函数名Unicode" `Quick ChineseProgrammingDeepTests.test_chinese_function_name_unicode;
      test_case "中文错误消息完整性" `Quick ChineseProgrammingDeepTests.test_chinese_error_messages_completeness;
      test_case "中文语义一致性" `Quick ChineseProgrammingDeepTests.test_chinese_semantic_consistency;
    ]);
    
    ("完整性验证测试", [
      test_case "所有声明函数可用" `Quick CompletenessValidationTests.test_all_declared_functions_available;
      test_case "函数表结构完整性" `Quick CompletenessValidationTests.test_function_table_structural_integrity;
