...
module BuiltinFunctionTableTests = struct
  ...
  let test_includes_submodule_functions () =
    ...
    (* 检查是否包含IO函数 *)
    let has_io_functions =
      List.exists (fun name -> Str.string_match (Str.regexp ".*[读写输].*") name 0) function_names
    in
    check bool "应包含IO相关函数" true has_io_functions;

    (* 检查是否包含数学函数 *)
    let has_math_functions =
      List.exists (fun name -> Str.string_match (Str.regexp ".*[加减乘除].*") name 0) function_names
    in
    check bool "应包含数学相关函数" true has_math_functions;

    (* 检查是否包含字符串函数 *)
    let has_string_functions =
      List.exists
        (fun name -> Str.string_match (Str.regexp ".*字符串.*\\|.*文本.*") name 0)
        function_names
    in
    check bool "应包含字符串相关函数" true has_string_functions;

    (* 检查是否包含类型转换函数 *)
    let has_type_functions =
      List.exists
        (fun name ->
...
module ModuleIntegrationTests = struct
      ...
      (fun (module_name, keywords) ->
        let has_functions =
          List.exists
            (fun keyword ->
              List.exists
                (fun func_name -> String.contains func_name (String.get keyword 0))
                function_names)
            keywords
        in
  ...
  let test_cross_module_function_coordination () =
    ...
    try
      ...
      | StringValue s -> check bool "模块间函数协调应正常" true (String.length s > 0)
      | _ -> fail "类型转换结果错误"
    with RuntimeError _ ->
      (* 某些函数可能不存在，跳过此测试 *)
...
module ChineseProgrammingTests = struct
    ...
    let has_chinese_names =
      List.exists
        (fun name ->
          ...
          List.exists
            (fun pattern -> Str.string_match (Str.regexp (".*" ^ pattern ^ ".*")) name 0)
            chinese_patterns)
        function_names
    in
  ...
  let test_chinese_error_messages () =
    ...
    with
    | RuntimeError msg ->
        check bool "错误消息应包含中文字符" true (Str.string_match (Str.regexp ".*[未知函数].*") msg 0)
