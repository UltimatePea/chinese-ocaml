(** 错误消息生成模块测试套件
    ...
    验证error_messages_generation.ml模块的错误消息生成功能
    包括类型不匹配、未定义变量、函数参数和模式匹配错误消息生成
...
let test_undefined_variable_error () =
  ...
  check (bool) "消息应包含无可用变量提示" true 
    (Str.search_forward (Str.regexp "没有可用变量") msg_no_vars 0 >= 0);
...
let test_function_arity_error () =
  ...
  check (bool) "消息应包含数字信息" true 
    (String.contains msg_too_few '3' || String.contains msg_too_few '1');
...
let test_message_quality () =
  ...
  (* 测试消息长度合理性 *)
  check (bool) "错误消息长度应合理（不会太短或太长）" true 
    (String.length type_msg >= 5 && String.length type_msg <= 200);
  ...
  check (bool) "函数参数消息应包含数字" true 
    (String.contains func_msg '3' && String.contains func_msg '1');
