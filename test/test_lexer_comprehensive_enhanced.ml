...
let test_comparison_operators () =
  ...
  let tokens = tokenize input "test.ly" in
  let expected = [EqualToKeyword; LessThanEqualToKeyword; GreaterThanWenyan; LessThanWenyan; EOF] in
...
let test_punctuation_symbols () =
  ...
  let input = "（） 【】" in
  let tokens = tokenize input "test.ly" in
  let expected = [ChineseLeftParen; ChineseRightParen; ChineseSquareLeftBracket; ChineseSquareRightBracket; EOF] in
...
let test_mixed_punctuation () =
  ...
  let tokens = tokenize input "test.ly" in
  let expected = [
    ChineseLeftParen; QuotedIdentifierToken "变量"; ChineseComma; ChineseRightParen; 
    ChineseSquareLeftBracket; IntToken 42; ChineseSquareRightBracket; EOF
  ] in
