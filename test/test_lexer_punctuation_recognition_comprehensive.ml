...
let test_colon_edge_cases () =
  ...
  let state_at_colon = { state_at_end with position = 6 } in (* UTF-8中文字符占3字节，"测试"占6字节 *)
  ...
  check (option (pair (module Lexer_tokens) (pair int (module Lexer_state)))) "空字符串冒号处理" None empty_result;
  ()
...
let test_punctuation_performance () =
  ...
  for i = 0 to min 50 (state.length - 3) do
    if i mod 9 = 0 then ( (* 每隔9个字节测试一次（UTF-8中文字符通常占3字节） *)
...
(** 主测试运行器 *)
let () = run "Lexer_punctuation_recognition 综合测试" [
