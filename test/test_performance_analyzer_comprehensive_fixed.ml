...
let test_base_analyzer_creation _ =
  ...
  let simple_analyzer expr =
    [Performance_analyzer_base.make_performance_suggestion
       ~hint_type:"测试"
       ~message:"测试消息"
       ~confidence:0.8
       ~location:"测试位置"
...
let test_suggestion_builder _ =
  (* 测试建议构建器 *)
  let list_suggestion = Performance_analyzer_base.SuggestionBuilder.list_optimization_suggestion "append" "使用更高效的操作" in
  assert_bool "列表优化建议应该生成" (String.length list_suggestion.message > 0);
  
  let pattern_suggestion = Performance_analyzer_base.SuggestionBuilder.pattern_matching_suggestion 10 "high" in
...
let test_integrated_performance_analysis _ =
  (* 创建一个包含多种性能问题的复杂表达式 *)
  let complex_expr = 
    make_binary_op (make_int 1) Ast.Add (make_int 2)
  in
