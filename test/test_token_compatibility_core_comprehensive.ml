...
module TestUtils = struct
  (** 创建测试用的位置信息 *)
  let _create_test_position filename line column =
  ...
  let check_token_option desc expected_opt actual_opt =
    ...
    | (Some expected, Some actual) ->
        check bool desc true (expected = actual)
    | (None, Some _) ->
        check bool (desc ^ " - 期望None但得到Some") false true
    | (Some _, None) ->
        check bool (desc ^ " - 期望Some但得到None") false true
...
module CoreConversionTests = struct
  ...
  let test_special_token_conversion () =
    let test_cases = [
      ("EOF", Some EOF);
      ("\\n", Some Newline);
      ("\\t", Some Whitespace);  (* 假设tab被映射为whitespace *)
  ...
  let test_token_conversion_with_value () =
    ...
    List.iter (fun (token_str, value_opt, expected) ->
...
module CompatiblePositionedTokenTests = struct
  ...
  let test_basic_positioned_token_creation () =
    ...
    List.iter (fun (token_str, filename, line, column, expected_token) ->
      let result = make_compatible_positioned_token token_str None filename line column in
      match result with
      | Some positioned ->
          check bool ("位置Token创建: " ^ token_str ^ " - token匹配") true (positioned.token = expected_token);
          check string ("位置Token创建: " ^ token_str ^ " - filename匹配") filename positioned.position.filename;
          ...
          check (option (module struct type t = token_metadata let pp _ _ = () let equal _ _ = true end)) 
            ("位置Token创建: " ^ token_str ^ " - metadata应为None") None positioned.metadata
      | None ->
          check bool ("位置Token创建失败: " ^ token_str) false true
    ) test_cases
  ...
  let test_positioned_token_with_value () =
    ...
    List.iter (fun (token_str, value_opt, filename, line, column, expected_token) ->
      let result = make_compatible_positioned_token token_str value_opt filename line column in
      match result with
      | Some positioned ->
          check bool ("带值位置Token: " ^ token_str ^ " - token匹配") true (positioned.token = expected_token);
          check string ("带值位置Token: " ^ token_str ^ " - filename匹配") filename positioned.position.filename;
      ...
      | None ->
          check bool ("带值位置Token创建失败: " ^ token_str) false true
    ) test_cases
  ...
  let test_edge_case_positioned_token_creation () =
    let edge_cases = [
      ("", "empty.ly", 1, 1);  (* 空字符串 *)
      ("let", "", 1, 1);       (* 空文件名 *)
      ("let", "test.ly", 0, 1); (* 行号为0 *)
      ("let", "test.ly", 1, 0); (* 列号为0 *)
      ("let", "test.ly", -1, 1); (* 负行号 *)
      ("let", "test.ly", 1, -1); (* 负列号 *)
    ] in
    
    List.iter (fun (token_str, filename, line, column) ->
      let result = make_compatible_positioned_token token_str None filename line column in
      match result with
      | Some positioned ->
          (* 即使边界条件，如果token有效，也应该能创建位置Token *)
          check string ("边界条件位置Token: " ^ token_str ^ " - filename") filename positioned.position.filename;
...
module CompatibilityCheckTests = struct
  ...
  let test_valid_token_compatibility () =
    let valid_tokens = TestUtils.common_keywords @ TestUtils.common_operators @ 
                      TestUtils.common_delimiters @ ["42"; "variable"; "MyClass"] in
    
    List.iter (fun token_str ->
  ...
  let test_edge_case_compatibility () =
    let edge_cases = [
      ("", false);      (* 空字符串应该不兼容 *)
      (" ", false);     (* 单个空格应该不兼容 *)
      ("\\n", true);    (* 换行符可能兼容（特殊Token） *)
      ("0", true);      (* 数字0应该兼容 *)
      ("_", true);      (* 下划线应该兼容（标识符） *)
    ] in
    
    List.iter (fun (token_str, expected) ->
      let is_compatible = is_compatible_with_legacy token_str in
...
module IntegratedTests = struct
  ...
  let test_complete_token_processing_flow () =
    ...
    List.iter (fun (token_str, filename, line, column, expected_token) ->
      ...
      TestUtils.check_token_option ("流程测试转换: " ^ token_str) (Some expected_token) conversion_result;
      ...
      (* 测试位置Token创建 *)
      let positioned_result = make_compatible_positioned_token token_str None filename line column in
      match positioned_result with
      | Some positioned ->
          check bool ("流程测试位置Token: " ^ token_str) true (positioned.token = expected_token)
      | None ->
          check bool ("流程测试位置Token创建失败: " ^ token_str) false true
    ) test_program_tokens
  ...
  let test_error_handling_and_recovery () =
    ...
    List.iter (fun (token_str, should_succeed) ->
      let conversion_result = convert_legacy_token_string token_str None in
      let positioned_result = make_compatible_positioned_token token_str None "error_test.ly" 1 1 in
  ...
  let test_concurrent_access_simulation () =
    ...
    (* 模拟并发处理多个token *)
    let results = List.map (fun token_str ->
    ...
    (* 验证所有结果都是一致的 *)
    List.iter (fun (token_str, conversion, compatibility, positioned) ->
  ...
  let test_memory_usage_and_cleanup () =
    ...
    (* 处理大量token以测试内存使用 *)
    for i = 1 to 10000 do
      let token_str = if i mod 4 = 0 then "let"
                     else if i mod 4 = 1 then "+"
                     else if i mod 4 = 2 then "42"
                     else "variable" ^ string_of_int i in
...
let () =
  run "Token兼容性核心转换模块综合测试"
    [
      ...
      ( "兼容位置Token创建测试",
        [
          test_case "基本位置Token创建" `Quick CompatiblePositionedTokenTests.test_basic_positioned_token_creation;
          test_case "带值位置Token创建" `Quick CompatiblePositionedTokenTests.test_positioned_token_with_value;
          test_case "无效位置Token创建" `Quick CompatiblePositionedTokenTests.test_invalid_positioned_token_creation;
          test_case "边界条件位置Token创建" `Quick CompatiblePositionedTokenTests.test_edge_case_positioned_token_creation;
          test_case "位置Token创建性能" `Slow CompatiblePositionedTokenTests.test_positioned_token_creation_performance;
        ] );
      ( "兼容性检查测试",
        [
