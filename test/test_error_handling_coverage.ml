...
let test_special_error_types () =
  ...
  (match poetry_parse_error "诗词解析错误测试" (Some pos) with
   | Error error_info ->
       Printf.printf "  诗词解析错误创建成功: %s\n" (format_error_info error_info)
   | Ok _ -> assert false);
  
  (* 测试内部错误 *)
  (match internal_error "内部错误测试" with
   | Error error_info ->
       Printf.printf "  内部错误创建成功: %s\n" (format_error_info error_info)
   | Ok _ -> assert false);
