...
let test_duplicate_registration_handling () =
  ...
  | Some data ->
    Alcotest.(check string) "应保留最后注册的数据" "第二个思韵组" data.group_description
  | None -> Alcotest.fail "应能检索到韵组数据"
...
let test_registry_order_preservation () =
  ...
  Alcotest.(check (list (module struct 
    ...
    let equal a b = 
      ...
      | _ -> false
  end))) "注册顺序应被保持" expected_groups registered_groups
...
let test_rhyme_group_data_integrity () =
  ...
  | Some data ->
    ...
    (* 验证条目包含正确的韵组信息 *)
    let has_ping_entries = List.exists (fun entry ->
      List.mem entry.character ["王"; "光"; "黄"; "方"]
    ) data.entries in
    Alcotest.(check bool) "应包含平声字符" true has_ping_entries;
    
    let has_ze_entries = List.exists (fun entry ->
      List.mem entry.character ["望"; "亮"; "况"; "放"]  
    ) data.entries in
...
let test_rhyme_statistics () =
  ...
  List.iter (fun (group_type, ping_chars, ze_chars) ->
    let config = {
      group_type;
      description = "统计测试";
  ...
  let expected_total = 2 + 1 + 2 + 3 in (* 各组字符数之和 *)
...
let test_safe_retrieval () =
  ...
  Alcotest.(check (module struct 
    ...
    let equal a b = 
      match (a, b) with
      | (UnknownRhyme, UnknownRhyme) -> true
      | _ -> false
  end)) "安全检索应返回UnknownRhyme类型" UnknownRhyme safe_result.group_name;
...
let test_registry_validation () =
  ...
  (* 验证问题报告包含相关信息 *)
  let has_empty_group_issue = List.exists (fun issue ->
    String.length issue > 0 && (
      (try ignore (Str.search_forward (Str.regexp "空") issue 0); true with Not_found -> false) ||
      (try ignore (Str.search_forward (Str.regexp "没") issue 0); true with Not_found -> false)
    )
  ) validation_issues in
...
let test_all_rhyme_groups_integration () =
  ...
  List.iter (fun (group_type, desc, ping_chars, ze_chars) ->
    let config = {
      group_type;
      description = desc;
      ping_sheng_chars = ping_chars;
...
let statistics_management_tests = [
  ("韵组统计", `Quick, test_rhyme_statistics);
  ("注册表管理功能", `Quick, test_registry_management_functions);
]

let safety_boundary_tests = [
  ("安全检索", `Quick, test_safe_retrieval);
  ("注册表验证", `Quick, test_registry_validation);
]

let integration_tests = [
  ("所有韵组集成", `Quick, test_all_rhyme_groups_integration);
]

let performance_tests = [
  ("大规模注册性能", `Slow, test_large_scale_registration);
]
