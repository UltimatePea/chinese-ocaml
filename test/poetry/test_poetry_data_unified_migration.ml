...
let test_rhyme_database_loading () =
  try
    ...
    (* 验证包含常见韵组 *)
    let has_common_groups = List.exists (fun (group_name, _group_data) ->
...
let test_rhyme_group_query () =
  try
    ...
    (* 测试每个韵组的字符查询 *)
    List.iter (fun group ->
      let characters = Poetry_json_unified.get_group_characters (match group with (name, _) -> name) in
      check bool ("group_has_characters_" ^ (match group with (name, _) -> name)) 
  ...
  try
    ...
    List.iter (fun line ->
      ...
      check int ("analysis_length_matches_" ^ (String.sub line 0 2))
        (String.length line) (String.length line) (* TODO: 使用真实分析结果 *)
...
let test_data_integrity () =
  try
    ...
    (* 验证数据一致性 *)
    let consistency_check = true in (* TODO: 实现数据一致性检查 *)
...
let test_memory_usage () =
  ...
  check bool "reasonable_memory_usage" true (memory_increase < 100); (* 100MB限制 *)
  
  (* 测试垃圾回收后内存 *)
  Gc.full_major ();
  let after_gc_memory = 0 in (* TODO: 实现内存监控 *)
  check bool "memory_stable_after_gc" true 
    (abs (after_gc_memory - after_load_memory) < 10)
