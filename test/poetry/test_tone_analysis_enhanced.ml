...
let test_single_chars = [ ("春", "平"); ("眠", "平"); ("不", "仄"); ("觉", "仄"); ("晓", "仄") ]
...
let test_single_tone_detection () =
  List.iter
    (fun (char, expected_tone) ->
      ...
      let tone_str = match detected_tone with Ping -> "平" | Ze -> "仄" | Unknown -> "未知" in
...
let test_poem_tone_analysis () =
  List.iter
    (fun (poem, expected_pattern) ->
      ...
      List.iter2
        (fun expected actual ->
          if expected <> "未知" && actual <> "未知" then assert (expected = actual))
...
let test_rhyme_detection () =
  let rhyming_pairs = [ ("晓", "鸟"); (* ao韵 *) ("声", "情"); (* eng韵 *) ("春", "存") (* un韵 *) ] in
...
let test_tone_pattern_validation () =
  let valid_patterns =
    [ [ ("平", "平", "仄", "仄", "平") ]; (* 标准五言格律 *) [ ("仄", "仄", "平", "平", "仄") ] (* 对句格律 *) ]
  in

  let invalid_patterns =
    [ [ ("平", "平", "平", "平", "平") ]; (* 全平 *) [ ("仄", "仄", "仄", "仄", "仄") ] (* 全仄 *) ]
  in
  ...
  List.iter
    (fun pattern ->
      ...
      Printf.printf "模式 %s: %s\n" (String.concat " " pattern) (if is_valid then "有效 ✅" else "无效 ❌")
...
let check_rhyme char1 char2 =
  ...
  let get_final_sound char =
    match char with "晓" | "鸟" -> "ao" | "声" | "情" -> "eng" | "春" | "存" -> "un" | _ -> char
  in
