...
let test_multiple_rhyme_groups () =
  ...
  (* 注册多个韵组 *)
  let configs = [
    { group_type = AnRhyme; description = "安韵"; ping_sheng_chars = ["山"]; ze_sheng_chars = ["产"] };
    { group_type = SiRhyme; description = "思韵"; ping_sheng_chars = ["思"]; ze_sheng_chars = ["四"] };
    { group_type = TianRhyme; description = "天韵"; ping_sheng_chars = ["天"]; ze_sheng_chars = ["面"] };
  ] in
  ...
  (* 验证各个韵组可检索 *)
  let all_registered = List.for_all (fun config ->
    is_registered config.group_type
  ) configs in
...
let test_duplicate_registration () =
  ...
  | Some data ->
    Alcotest.(check string) "应保留最后注册的数据" "第二个曲韵" data.group_description
  | None -> Alcotest.fail "应能检索到韵组数据"
...
(** 主测试运行器 *)
let () =
  Alcotest.run "韵组注册表基础测试 - Issue #1773" [
    ("基础功能", basic_tests);
  ]