...
let test_poetry_forms () =
  let forms = [ SiYanPianTi; WuYanLuShi; QiYanJueJu; CiPai "水调歌头"; ModernPoetry ] in
...
let test_evaluate_wuyan_lushi () =
  let verses = [| "春风花草香"; "夜雨竹林深"; "明月照高楼"; "清风拂碧波"; "山鸟啼声远"; "江鱼跃水欢"; "此情应长久"; "千里共婵娟" |] in
...
let test_evaluate_qiyan_jueju () =
  let verses = [| "春江潮水连海平"; "海上明月共潮生"; "滟滟随波千万里"; "何处春江无月明" |] in
...
let test_evaluate_poetry_by_form () =
  let wuyan_verses = [| "春风花草香"; "夜雨竹林深"; "明月照高楼"; "清风拂碧波"; "山鸟啼声远"; "江鱼跃水欢"; "此情应长久"; "千里共婵娟" |] in
  let qiyan_verses = [| "春江潮水连海平"; "海上明月共潮生"; "滟滟随波千万里"; "何处春江无月明" |] in
  let modern_verses = [| "代码如诗"; "逻辑似画"; "程序员的浪漫"; "在键盘上绽放" |] in
...
let test_wrong_line_count () =
  (* 测试错误的句数 *)
  let wrong_verses = [| "春风花草香"; "夜雨竹林深"; "明月照高楼" |] in
