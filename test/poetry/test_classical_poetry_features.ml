...
let test_four_character_symmetry () =
  let test_cases =
    [ ("令甲为零", "令乙为壹"); (* 变量声明对仗 *) ("函名求和", "函名求积"); (* 函数命名对仗 *) ("若甲大乙", "若丙等丁") (* 条件判断对仗 *) ]
  in
  List.iter
    (fun (first, second) ->
      Alcotest.(check int) "string length should match" (String.length first) (String.length second);
      Alcotest.(check int)
        "UTF-8 length should be 4" 4
        (Yyocamlc_lib.Utf8_utils.StringUtils.utf8_length first);
      Alcotest.(check int)
        "UTF-8 length should be 4" 4
        (Yyocamlc_lib.Utf8_utils.StringUtils.utf8_length second))
    test_cases

(* 测试四言骈体的音韵特征 *)
let test_four_character_phonetics () =
  let parallel_pairs =
    [ ("初令计数", "为零开始"); (* 平仄对应 *) ("每次循环", "输出计数"); (* 声调和谐 *) ("创建数组", "名为甲组") (* 韵脚相应 *) ]
  in
  List.iter
    (fun (first, second) ->
      Alcotest.(check int)
        "UTF-8 length should match"
        (Yyocamlc_lib.Utf8_utils.StringUtils.utf8_length first)
        (Yyocamlc_lib.Utf8_utils.StringUtils.utf8_length second))
    parallel_pairs
...
let test_seven_character_conciseness () =
  ...
  List.iter
    (fun line ->
      Alcotest.(check int)
        "UTF-8 length should be 7" 7
        (Yyocamlc_lib.Utf8_utils.StringUtils.utf8_length line))
    quatrain_lines
...
let test_seven_character_artistic_conception () =
  ...
  in
  List.iter
    (fun (poetic, _technical) ->
      Alcotest.(check int)
        "UTF-8 length should be 7" 7
        (Yyocamlc_lib.Utf8_utils.StringUtils.utf8_length poetic))
    artistic_expressions

(* 测试平仄协调 *)
let test_tonal_harmony () =
  let tonal_pairs =
    [ ("令春为阳数", "令秋为阴值"); (* 平仄相配 *) ("令东风轻柔", "令西雨急骤"); (* 音韵和谐 *) ("函名求加和", "参数甲乙丙") (* 声调配合 *) ]
  in
  List.iter
    (fun (first, second) ->
      Alcotest.(check int)
...
let test_structural_parallelism () =
  let structural_pairs =
    [ ("若晨光初现", "则鸟鸣啁啾"); (* 条件对结果 *) ("若暮色将临", "则虫声细碎"); (* 对偶结构 *) ("创建数组春", "内含花名录") (* 动宾结构 *) ]
  in
  List.iter
    (fun (_first, _second) ->
      (* 验证结构对称性 *)
...
let test_rhyme_scheme () =
  let rhyming_groups =
    [ [ "春"; "存"; "功" ]; (* 韵脚组1 *) [ "录"; "路"; "径" ]; (* 韵脚组2 *) [ "佳"; "加"; "华" ] (* 韵脚组3 *) ]
  in
  List.iter
    (fun rhyme_group ->
      (* 验证韵脚的音韵相似性 *)
...
let test_classical_chinese_grammar () =
  let classical_patterns =
    [ "令甲为零也"; (* 判断句式 *) "若甲大乙则"; (* 假设句式 *) "函数之定义"; (* 定语结构 *) "返回之结果" (* 定语结构 *) ]
  in
  List.iter
    (fun pattern ->
      Alcotest.(check bool)
        "Should follow classical Chinese grammar" true
...
let test_classical_vocabulary () =
  let classical_terms =
    [ ("令", "变量声明"); ("若", "条件判断"); ("则", "分支执行"); ("返回", "函数返回"); ("循环", "迭代控制") ]
  in
  List.iter
    (fun (_classical, _modern) ->
...
let aesthetic_tests =
  [ ("文学美感", `Quick, test_literary_beauty); ("古典现代融合", `Quick, test_classical_modern_fusion) ]

let grammar_tests =
  [ ("文言文语法", `Quick, test_classical_chinese_grammar); ("古典词汇", `Quick, test_classical_vocabulary) ]
