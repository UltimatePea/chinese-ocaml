...
let create_test_context () = create_evaluation_context test_single_verse test_verses
...
let test_evaluation_dimension_completeness () =
  ...
  List.iter
    (fun dim ->
      match dim with
      | RhymeHarmony | TonalBalance | MetricalForm | Parallelism | Imagery | Rhythm | Elegance
      | ContentDepth | FormBeauty | SoundHarmony | ContextMood | EmotionExpression | Innovation
      | Overall ->
          ())
    dimensions
...
let test_artistic_evaluation_structure () =
  ...
  Alcotest.(check bool)
    "艺术水平等级有效" true
    (match evaluation.artistic_level with `Beginner | `Intermediate | `Advanced | `Master -> true);
  Alcotest.(check bool)
    "质量等级有效" true
    (match evaluation.quality_grade with `Excellent | `Good | `Fair | `Poor -> true)
...
let test_comprehensive_evaluation_consistency () =
  ...
  Alcotest.(check int)
    "维度评分数量一致"
    (List.length evaluation1.dimension_scores)
    (List.length evaluation2.dimension_scores)
...
let test_mood_analysis () =
  ...
  Alcotest.(check bool)
    "意境强度范围正确" true
    (mood_analysis.mood_intensity >= 0.0 && mood_analysis.mood_intensity <= 1.0);
  Alcotest.(check bool)
    "意境连贯性范围正确" true
    (mood_analysis.mood_coherence >= 0.0 && mood_analysis.mood_coherence <= 1.0);
...
let test_rhetoric_analysis () =
  ...
  Alcotest.(check bool)
    "修辞丰富度范围正确" true
    (rhetoric_analysis.rhetoric_richness >= 0.0 && rhetoric_analysis.rhetoric_richness <= 1.0);
  Alcotest.(check bool) "技法示例列表初始化" true (List.length rhetoric_analysis.technique_examples >= 0);
  Alcotest.(check bool)
    "技法有效性评分初始化" true
    (List.length rhetoric_analysis.technique_effectiveness >= 0)
...
let test_evaluation_formatting () =
  ...
  Alcotest.(check bool)
    "JSON格式基本验证" true
    (String.contains json_export '{' && String.contains json_export '}')
...
let test_backward_compatibility_advanced () =
  ...
  let grade_valid = match overall_grade with `Excellent | `Good | `Fair | `Poor -> true in
...
let () =
  run "统一艺术评价引擎全面测试"
    [
      ...
      (* 单维度评价测试 *)
      ("单维度评价", [ test_case "单维度评价功能" `Quick test_single_dimension_evaluation ]);
      (* 综合评价测试 *)
      ( "综合艺术性评价",
        [
      ...
      (* 工具功能测试 *)
      ("工具和格式化", [ test_case "评价结果格式化" `Quick test_evaluation_formatting ]);
      (* 向后兼容性测试 *)
      ( "向后兼容性",
        [
