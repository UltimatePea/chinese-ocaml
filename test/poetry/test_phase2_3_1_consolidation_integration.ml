...
(* 标准测试诗句 - 春晓 *)
let standard_test_poem = [ "春眠不觉晓"; "处处闻啼鸟"; "夜来风雨声"; "花落知多少" ]

(* 复杂测试诗句 - 包含各种诗歌特征 *)
let complex_test_poem = [ "红楼梦断有情人"; "黄土埋香无定骨"; "青灯照壁人初睡"; "白首依然读旧书" ]

(* 边界条件测试诗句 *)
let _edge_case_poems =
  [
    []; (* 空诗句 *) [ "单句" ]; (* 单句诗 *) [ "很长的诗句测试内容，用于验证系统对于超长输入的处理能力，包含各种中文字符和标点符号！？。，" ]; (* 超长句 *)
  ]
...
let test_cross_module_data_flow () =
  ...
  (* 验证所有维度都能正常工作 *)
  Alcotest.(check bool) "韵律评价数据流正常" true (match rhyme_result with Some _ -> true | None -> false);
  Alcotest.(check bool)
    "对仗评价数据流正常" true
  ...
  Alcotest.(check bool) "综合评价整合所有维度" true (List.length comprehensive_result.dimension_scores >= 3)
...
let test_poetry_engine_pipeline_integrity () =
  ...
  let total_suggestions =
    List.length form_suggestions + List.length content_suggestions + List.length sound_suggestions
  in
...
let test_backward_compatibility_comprehensive () =
  ...
  (* 验证所有兼容函数返回有效结果 *)
  List.iter
    (fun (name, score) -> Alcotest.(check bool) (name ^ "兼容性") true (score >= 0.0 && score <= 1.0))
    [
      ("韵律和谐", rhyme_compat);
      ("声调平衡", tonal_compat);
...
let test_form_specific_evaluation_integration () =
  ...
  let scores =
    [
      siyan_result.overall_score;
      wuyan_result.overall_score;
      qiyan_result.overall_score;
      form_generic_result.overall_score;
    ]
  in
...
let test_comprehensive_workflow_integration () =
  ...
  (* 步骤2: 执行各维度评价 *)
  let dimensions_to_test =
    [ RhymeHarmony; TonalBalance; Parallelism; Imagery; Rhythm; Elegance; FormBeauty ]
  in
  let dimension_results =
    List.filter_map
      (fun dim -> evaluate_single_dimension dim context engine_state)
      dimensions_to_test
  in
  ...
  Alcotest.(check bool)
    "专项评分有效" true
    (form_score >= 0.0 && content_score >= 0.0 && sound_score >= 0.0);
  ...
  Alcotest.(check bool)
    "JSON导出正常" true
    (String.length json_export > 0 && String.contains json_export '{');

  (* 验证结果的逻辑一致性 *)
  let dimension_avg =
    if List.length dimension_results > 0 then
      List.fold_left (fun acc result -> acc +. result.score) 0.0 dimension_results
...
let test_edge_cases_comprehensive_handling () =
  ...
  (* 测试混合语言输入的处理 *)
  let mixed_poems =
    [ "春眠不觉晓 spring sleep"; "处处闻啼鸟 birds everywhere"; "夜来风雨声 night storm"; "花落知多少 flowers fall" ]
  in
