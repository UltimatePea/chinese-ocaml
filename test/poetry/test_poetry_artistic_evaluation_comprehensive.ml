...
let test_artistic_report_structure () =
  ...
  check bool "对仗评分范围正确" true (test_report.parallelism_score >= 0.0 && test_report.parallelism_score <= 1.0);
...
let test_poetry_standards () =
  ...
  check bool "四言节律权重正确" true (siyan_standards.rhythm_weight >= 0.0 && siyan_standards.rhythm_weight <= 1.0);
  ...
  check bool "五言节律权重正确" true (wuyan_standards.rhythm_weight >= 0.0 && wuyan_standards.rhythm_weight <= 1.0)
