...
let test_artistic_evaluation_structure () =
  (* 创建测试评价结构 *)
  let test_evaluation = Poetry_artistic_engine.{
    overall_score = 0.85;
    dimension_scores = [
      (Content, 0.9);
      (Form, 0.8);
...
let test_mood_analysis_structure () =
  (* 创建测试意境分析结构 *)
  let test_mood = Poetry_artistic_engine.{
    primary_mood = "清幽雅致";
...
let test_rhetoric_analysis_structure () =
  (* 创建测试修辞分析结构 *)
  let test_rhetoric = Poetry_artistic_engine.{
...
let test_basic_artistic_evaluation () =
  ...
  (* 模拟基础艺术性评价 *)
  let mock_evaluation = Poetry_artistic_engine.{
    overall_score = 0.8;
    dimension_scores = [
      (Content, 0.85);
      (Form, 0.75);
...
let test_artistic_level_classification () =
  ...
  List.iter (fun (level, description) ->
    (* 验证每个水平都有对应描述 *)
    assert (String.length description > 0);
    
    (* 模拟不同水平的评价 *)
    let score = match level with
      | `Beginner -> 0.3
      | `Intermediate -> 0.6
      | `Advanced -> 0.8
      | `Master -> 0.95
...
let test_boundary_conditions () =
  (* 测试空字符串处理 *)
  let empty_evaluation = Poetry_artistic_engine.{
    overall_score = 0.0;
    dimension_scores = [];
    strengths = [];
    weaknesses = ["无内容"];
  ...
  (* 测试极限分数 *)
  let perfect_evaluation = Poetry_artistic_engine.{
    overall_score = 1.0;
    dimension_scores = [
      (Content, 1.0);
      (Form, 1.0);
