...
let test_enhanced_imagery_evaluation () =
  let nature_verse = "山花水鸟月风雨云雪" in
  (* 丰富自然意象 *)
  let plain_verse = "今天天气很好" in
  (* 普通描述 *)
...
let test_enhanced_elegance_evaluation () =
  let elegant_verse = "雅致清淡幽静深远高妙" in
  (* 雅致词汇 *)
  let vulgar_verse = "俗低粗恶脏丑" in
  (* 粗俗词汇 *)
...
let test_enhanced_rhythm_evaluation () =
  let standard_qiyan = "春眠不觉晓处处闻啼鸟夜来风雨声花落知多少" in
  (* 标准七言长度 *)
  let irregular_length = "短" in
  (* 不规范长度 *)
...
let test_comprehensive_evaluation_enhanced () =
  ...
  let evaluation = evaluate_artistic_work (CoreEvaluation ComprehensiveEvaluation) context in

  (* 验证评价结果结构完整性 *)
  A.(check bool)
    "总体分数应在有效范围内" true
    (evaluation.overall_score >= 0.0 && evaluation.overall_score <= 1.0);
  A.(check bool) "维度分数列表不应为空" true (List.length evaluation.dimension_scores > 0);
  A.(check bool)
    "元数据应包含增强版本信息" true
    (List.exists
       (fun (k, v) -> k = "version" && String.contains v 'v')
       evaluation.evaluation_metadata);
  A.(check bool)
    "应包含算法版本标记" true
    (List.exists
       (fun (k, v) -> k = "algorithm_version" && v = "phase2-enhanced")
       evaluation.evaluation_metadata)
...
let test_weighted_evaluation_system () =
  ...
  (* 测试不同评价类型 *)
  let rhyme_eval = evaluate_artistic_work (CoreEvaluation RhymeHarmonyEvaluation) context in
  let imagery_eval = evaluate_artistic_work (CoreEvaluation ImageryEvaluation) context in
...
let test_evaluation_consistency () =
  ...
  (* 多次评价应该产生一致结果（由于缓存） *)
  let eval1 = evaluate_artistic_work (CoreEvaluation ComprehensiveEvaluation) context in
  let eval2 = evaluate_artistic_work (CoreEvaluation ComprehensiveEvaluation) context in

  A.(check (float 0.001)) "多次评价应产生一致结果" eval1.overall_score eval2.overall_score
...
let test_batch_evaluation_quality () =
  ...
  let results = batch_evaluate_artistic_works (CoreEvaluation RhymeHarmonyEvaluation) contexts in

  A.(check int) "批量评价应返回正确数量结果" 2 (List.length results);
  List.iter
    (fun eval ->
      A.(check bool) "批量评价结果分数应在有效范围内" true (eval.overall_score >= 0.0 && eval.overall_score <= 1.0))
    results
