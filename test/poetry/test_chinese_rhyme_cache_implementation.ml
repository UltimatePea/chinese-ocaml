(** 中文韵律缓存实现测试 - Issue #1473 Phase 5.2 韵律检测缓存系统
    ...
    此模块测试韵律检测缓存机制的具体实现，验证缓存系统的正确性、 性能和安全性，确保满足Phase 5.2的性能优化目标。
...
module ChineseRhymeCache = struct
  ...
  type rhyme_result = { chars_rhyme : bool; rhyme_class : string }
  ...
  (** 缓存统计实例 *)
  let cache_stats = { total_requests = 0; cache_hits = 0; cache_misses = 0; cache_size = 0 }
  ...
  let get_rhyme_class char =
    ...
    | None ->
        ...
        let class_info =
          try
            let _, rhyme_class, rhyme_name =
              List.find (fun (c, _, _) -> String.equal c char) char_rhyme_mapping
            in
  ...
  let check_cache_size_limit max_size =
    if Hashtbl.length rhyme_cache > max_size then (
      (* 简单的LRU实现：清除一半缓存 *)
...
module TestFramework = struct
  ...
  let assert_true condition message = if condition then Pass else Fail message

  let assert_equal expected actual message =
    if expected = actual then Pass
    else
      Fail
        (Printf.sprintf "%s: expected %s, got %s" message (string_of_bool expected)
           (string_of_bool actual))

  let assert_float_range value min_val max_val message =
    if value >= min_val && value <= max_val then Pass
    else
      Fail (Printf.sprintf "%s: value %.3f not in range [%.3f, %.3f]" message value min_val max_val)
...
module BasicFunctionalityTests = struct
  ...
  let test_basic_rhyme_detection () =
    ...
    assert_true (result1.chars_rhyme && result2.chars_rhyme && not result3.chars_rhyme) "基本韵律检测结果正确"
  ...
  let test_cache_functionality () =
    ...
    assert_true (stats2.cache_hits > stats1.cache_hits && hit_rate > 0.0) "缓存功能正常工作"

  (** 测试韵律类别一致性 *)
  let test_rhyme_class_consistency () =
    clear_cache ();
    let result1 = get_rhyme "春" "心" in
    let result2 = get_rhyme "心" "春" in
    (* 顺序相反 *)

    assert_equal result1.chars_rhyme result2.chars_rhyme "韵律类别检测一致性"
...
module PerformanceTests = struct
  ...
  let test_cache_hit_rate () =
    ...
    assert_float_range hit_rate 0.6 1.0 (* 期望60%以上命中率 *) "缓存命中率达到预期"
  ...
  let test_bulk_processing_performance () =
    ...
    assert_float_range processing_time 0.0 1.0 (* 期望在1秒内完成 *) "大批量处理性能满足要求"
  ...
  let test_cache_size_control () =
    ...
    assert_true (stats.cache_size <= 200) (* 缓存大小应该受到控制 *) "缓存大小控制有效"
...
module BoundaryConditionTests = struct
  ...
  let test_empty_string_handling () =
    ...
    try
      ...
      assert_true (not result.chars_rhyme) "空字符串处理正确"
    with _ -> Fail "空字符串处理异常"
  ...
  let test_unknown_character_handling () =
    ...
    assert_true ((not result.chars_rhyme) && String.equal result.rhyme_class "不同韵") "未知字符处理正确"
  ...
  let test_duplicate_key_handling () =
    ...
    assert_true
      (result1.chars_rhyme = result2.chars_rhyme && result2.chars_rhyme = result3.chars_rhyme)
      "重复键值对处理一致"
...
module MemorySafetyTests = struct
  ...
  let test_memory_leak_protection () =
    ...
    (* 大量操作 *)
    for i = 1 to 1000 do
      let char1 = Printf.sprintf "测试%d" (i mod 50) in
      (* 限制不同键的数量 *)
    ...
    assert_true (final_size - initial_size < 100) (* 缓存增长受控 *) "内存泄漏防护有效"
  ...
  let test_cache_cleanup () =
    ...
    assert_true (stats_after.cache_size = 0 && stats_after.total_requests = 0) "缓存清理功能正常"

  let all_tests = [ ("内存泄漏防护", test_memory_leak_protection); ("缓存清理", test_cache_cleanup) ]
end
      ...
      (fun (suite_name, tests) ->
        ...
        List.iter
          (fun (test_name, test_func) ->
            incr total_tests;
            if TestFramework.run_test test_name test_func then incr passed_tests)
          tests)
      test_suites;
...
(** 测试执行 *)
let () =
  if Array.length Sys.argv > 1 && Sys.argv.(1) = "--test" then run_rhyme_cache_tests ()
  else Printf.printf "中文韵律缓存实现测试模块已加载。\n使用 --test 参数运行完整测试。\n"
