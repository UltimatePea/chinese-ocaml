(** 诗词整合引擎测试 * * 测试新整合的韵律引擎和艺术性分析引擎 * Issue #1155 诗词模块整合优化验证 *)
  ...
  List.iter
    (fun (char1, char2) ->
      ...
      Printf.printf "  %s-%s: %s (分数:%.2f) - %s\n" char1 char2
        ...
        result.similarity_score result.explanation)
    test_pairs;
