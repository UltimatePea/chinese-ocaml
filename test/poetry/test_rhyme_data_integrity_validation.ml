  ...
  let config_with_duplicates =
    {
      ...
      description = "重复字符测试";
      ping_sheng_chars = [ "王"; "光"; "黄"; "王" ];
      (* "王"重复 *)
      ze_sheng_chars = [ "望"; "亮"; "况"; "望" ];
      (* "望"重复 *)
    }
  in
...
let test_data_migration_consistency () =
  ...
  let old_count =
    match get_rhyme_data_by_group TianRhyme with Some data -> List.length data.entries | None -> 0
  in
  ...
  | Some data -> Alcotest.(check string) "应保留最新的描述" "新天韵数据" data.group_description
...
let test_registry_validation_comprehensive () =
  ...
  List.iter (fun config -> register_rhyme_group (build_from_config config)) test_cases;
...
let test_boundary_conditions () =
  ...
  (* 测试极长韵组名称 *)
  let long_description = String.make 200 'A' ^ "测试长描述" in
  (* 长描述 *)
...
(** 主测试运行器 *)
let () = Alcotest.run "韵组数据完整性验证测试 - Issue #1773" [ ("数据完整性", integrity_tests) ]
