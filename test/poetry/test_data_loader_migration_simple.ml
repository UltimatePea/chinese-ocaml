...
let test_externalized_data_loader_compat () =
  ...
  try
    ...
    (* 验证数据结构字段访问 *)
    let total_data = List.length old_data.nature_nouns + List.length old_data.ping_sheng +
                     List.length old_data.shang_sheng + List.length old_data.qu_sheng +
                     List.length old_data.ru_sheng + List.length old_data.tools_objects in
...
let test_data_consistency () =
  ...
  try
    ...
    let consistent = 
      ...
      List.length data1.ru_sheng = List.length data2.ru_sheng in
...
let test_migration_validation () =
  ...
  try
    ...
    let field_count = 
      ...
      (if List.length data.ru_sheng >= 0 then 1 else 0) in
      
    if field_count = 7 then
      printf "✓ 数据结构字段完整性验证通过 (7/7)\n"
    else
      printf "! 数据结构字段完整性验证失败 (%d/7)\n" field_count;
