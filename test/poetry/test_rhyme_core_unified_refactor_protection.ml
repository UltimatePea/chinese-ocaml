...
let test_make_entry () =
  let entry =
    Poetry.Rhyme_core_unified.make_entry "春" PingSheng AnRhyme ~variants:[ "椿" ] ~frequency:0.8 ()
  in
...
let test_make_group_entries () =
  let entries =
    Poetry.Rhyme_core_unified.make_group_entries PingSheng AnRhyme test_an_rhyme_chars
  in
...
let test_an_rhyme_data_access () =
  let an_data = Poetry.Rhyme_core_unified.an_rhyme_data in
  ...
  let chars_in_entries =
    List.map (fun (e : Poetry.Rhyme_core_unified.rhyme_data_entry) -> e.character) an_data.entries
  in
  List.iter
...
let test_feng_rhyme_data_access () =
  ...
  (* 验证风韵组包含预期字符 *)
  let chars_in_entries =
    List.map (fun (e : Poetry.Rhyme_core_unified.rhyme_data_entry) -> e.character) feng_data.entries
  in
  List.iter
...
let test_find_char_rhyme_info () =
  ...
  | Some entry -> check bool "find_an_group" true (entry.group = AnRhyme)
...
let test_get_rhyme_characters () =
  let an_chars = Poetry.Rhyme_core_unified.get_chars_by_group AnRhyme in
...
let test_get_rhyme_score () =
  ...
  let score3 =
    match
      ( Poetry.Rhyme_core_unified.find_char_rhyme_info "春",
        Poetry.Rhyme_core_unified.find_char_rhyme_info "春" )
    with
...
let test_rhyme_matching_performance () =
  ...
  for i = 1 to 500 do
    ...
    let _ =
      match
        ( Poetry.Rhyme_core_unified.find_char_rhyme_info char1,
          Poetry.Rhyme_core_unified.find_char_rhyme_info char2 )
      with
...
let test_current_module_interface () =
  ...
  check bool "make_entry_callable" true
    (try
       ignore (Poetry.Rhyme_core_unified.make_entry "测" PingSheng AnRhyme ());
       true
     with _ -> false);

  check bool "find_char_rhyme_info_callable" true
    (try
       ignore (Poetry.Rhyme_core_unified.find_char_rhyme_info "测");
       true
     with _ -> false);
  ...
  check bool "check_rhyme_match_callable" true
    (try
       ...
       let _ =
         match
           ( Poetry.Rhyme_core_unified.find_char_rhyme_info "测",
             Poetry.Rhyme_core_unified.find_char_rhyme_info "节" )
         with
         | Some entry1, Some entry2 -> entry1.group = entry2.group
         | _ -> false
       in
