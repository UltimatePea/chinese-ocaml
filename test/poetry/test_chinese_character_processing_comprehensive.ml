(** 中文字符处理综合测试 - Issue #1473 Phase 5.2 功能验证
    ...
    此模块为Phase 5.2中文字符处理性能优化提供全面的功能测试和回归测试， 确保优化后的功能完整性和正确性。

    Author: Echo, 测试工程师代理 Created: 2025-07-27 Issue: #1473 Phase 5.2 中文字符处理性能优化
...
module TestUtils = struct
  type test_result = Pass of string | Fail of string | Skip of string

  let assert_equal expected actual message =
    if expected = actual then Pass message
    else
      Fail
        (sprintf "%s: expected %s, got %s" message (string_of_bool expected) (string_of_bool actual))

  let assert_string_equal expected actual message =
    if String.equal expected actual then Pass message
    else Fail (sprintf "%s: expected '%s', got '%s'" message expected actual)

  let assert_float_equal expected actual tolerance message =
    if abs_float (expected -. actual) <= tolerance then Pass message
    else
      Fail
        (sprintf "%s: expected %.6f, got %.6f (tolerance %.6f)" message expected actual tolerance)

  let assert_list_equal expected actual message =
    if List.length expected = List.length actual && List.for_all2 ( = ) expected actual then
      Pass message
    else Fail (sprintf "%s: lists not equal" message)
  ...
  let run_test_suite suite_name tests =
    ...
    List.iter
      (fun (name, test_func) ->
        incr total;
        if run_test name test_func then incr passed)
      tests;
    printf "结果: %d/%d 测试通过\n" !passed !total;
    !passed = !total
end
...
module RhymeCacheTests = struct
  ...
  type cache_stats = { mutable hits : int; mutable misses : int; mutable total : int }
  (** 缓存统计 *)
  ...
  let test_cache_basic_functionality () =
    ...
    if result1 = result2 && first_misses = 1 && second_hits = 1 then TestUtils.Pass "缓存基本功能正常"
    else TestUtils.Fail "缓存基本功能异常"
  ...
  let test_cache_hit_rate () =
    ...
    List.iter (fun (c1, c2) -> ignore (cached_rhyme_check c1 c2 stats)) test_pairs;
    ...
    else TestUtils.Fail (sprintf "缓存命中率 %.1f%% 低于预期" (hit_rate *. 100.0))
  ...
  let test_rhyme_detection_correctness () =
    ...
    if all_correct then TestUtils.Pass "韵律检测结果正确" else TestUtils.Fail "韵律检测结果错误"
  ...
  let test_cache_capacity () =
    ...
    else TestUtils.Fail (sprintf "缓存大小 %d 超出预期" cache_size)
...
module PunctuationTests = struct
  ...
  let punct_set =
    List.fold_left (fun acc p -> PunctSet.add p acc) PunctSet.empty chinese_punctuation

  (** 优化的标点符号识别 *)
  let is_chinese_punctuation char = PunctSet.mem char punct_set

  (** 朴素的标点符号识别 *)
  let is_chinese_punctuation_naive char = List.mem char chinese_punctuation
  ...
  let test_punctuation_recognition_accuracy () =
    ...
    if all_correct then TestUtils.Pass "标点符号识别准确性正确" else TestUtils.Fail "标点符号识别准确性错误"
  ...
  let test_method_consistency () =
    ...
    let all_consistent =
      List.for_all
        (fun char -> is_chinese_punctuation char = is_chinese_punctuation_naive char)
        test_chars
    in

    if all_consistent then TestUtils.Pass "优化方法与朴素方法结果一致" else TestUtils.Fail "优化方法与朴素方法结果不一致"
  ...
  let test_boundary_conditions () =
    ...
    if all_correct then TestUtils.Pass "边界条件处理正确" else TestUtils.Fail "边界条件处理错误"
...
module BatchProcessingTests = struct
  type char_info = { is_chinese : bool; is_punctuation : bool; char_type : string }

  (** 字符类型判断 *)
  let classify_char char =
    let is_chinese = List.mem char [ "春"; "夏"; "秋"; "冬"; "花"; "草"; "山"; "水" ] in
    let is_punctuation = List.mem char [ "，"; "。"; "；"; "："; "？"; "！" ] in
    let char_type = if is_chinese then "汉字" else if is_punctuation then "标点" else "其他" in
    { is_chinese; is_punctuation; char_type }

  (** 批量处理 *)
  let process_batch chars = List.map classify_char chars

  (** 逐个处理 *)
  let process_sequential chars =
    List.fold_left (fun acc char -> classify_char char :: acc) [] chars |> List.rev
  ...
  let test_batch_consistency () =
    ...
    let results_equal =
      List.length batch_result = List.length sequential_result
      && List.for_all2
           (fun b s ->
    ...
    if results_equal then TestUtils.Pass "批量处理与逐个处理结果一致" else TestUtils.Fail "批量处理与逐个处理结果不一致"
  ...
  let test_large_batch_processing () =
    ...
    try
      ...
      if List.length result = 1000 then TestUtils.Pass "大批量处理成功" else TestUtils.Fail "大批量处理结果长度错误"
    with e -> TestUtils.Fail ("大批量处理异常: " ^ Printexc.to_string e)

  (** 测试空输入处理 *)
  let test_empty_input () =
    let empty_result = process_batch [] in
    if List.length empty_result = 0 then TestUtils.Pass "空输入处理正确" else TestUtils.Fail "空输入处理错误"
...
module EncodingTests = struct
  ...
  let test_utf8_handling () =
    ...
    if all_correct then TestUtils.Pass "UTF-8编码处理正确" else TestUtils.Fail "UTF-8编码处理错误"
  ...
  let test_character_boundaries () =
    ...
    while !pos < String.length mixed_text do
      if !pos + 2 < String.length mixed_text then (
        ...
        chars := char :: !chars;
        pos := !pos + 1)
      else pos := String.length mixed_text
    done;
    ...
    else TestUtils.Fail "字符边界检测失败"

  let all_tests = [ ("UTF-8编码处理", test_utf8_handling); ("字符边界检测", test_character_boundaries) ]
end
...
module PerformanceRegressionTests = struct
  ...
  let test_rhyme_detection_performance () =
    let test_pairs = [ ("春", "心"); ("花", "家"); ("山", "间"); ("风", "中") ] in
    ...
    let _, cached_time =
      time_function (fun () ->
          for _ = 1 to 1000 do
            List.iter (fun (c1, c2) -> ignore (cached_rhyme_check c1 c2)) test_pairs
          done)
    in
    ...
    let _, uncached_time =
      time_function (fun () ->
          for _ = 1 to 1000 do
            List.iter (fun (c1, c2) -> ignore (uncached_rhyme_check c1 c2)) test_pairs
          done)
    in

    if cached_time <= uncached_time *. 1.5 then (* 允许50%的性能差异 *)
      TestUtils.Pass (sprintf "韵律检测性能正常 (缓存: %.6fs, 非缓存: %.6fs)" cached_time uncached_time)
    else TestUtils.Fail (sprintf "韵律检测性能回归 (缓存: %.6fs, 非缓存: %.6fs)" cached_time uncached_time)

  let all_tests = [ ("韵律检测性能回归", test_rhyme_detection_performance) ]
end
...
module ComprehensiveTestSuite = struct
  let run_all_tests () =
    ...
    List.iter
      (fun (suite_name, tests) ->
        let suite_passed = TestUtils.run_test_suite suite_name tests in
        if not suite_passed then all_passed := false)
      test_suites;
...
(** 测试执行 *)
let () =
  if Array.length Sys.argv > 1 && Sys.argv.(1) = "--test" then run_comprehensive_tests ()
  else printf "中文字符处理综合测试模块已加载。\n使用 --test 参数运行完整测试。\n"
