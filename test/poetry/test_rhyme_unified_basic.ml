...
let test_json_module () =
  ...
  with
  | _ -> check bool "Data loading may fail in test environment" true true

(** 主测试套件 *)
let () =
  run "Rhyme Unified Module Tests" [
    "Data Module", [
      test_case "Basic data operations" `Quick test_data_module;
    ];
    "Analysis Module", [
      test_case "Character analysis operations" `Quick test_analysis_module;
    ];
    "Utils Module", [
      test_case "Utility functions" `Quick test_utils_module;
    ];
    "Compatibility Interface", [
      test_case "Backward compatibility" `Quick test_compatibility_interface;
    ];
    "JSON Module", [
      test_case "JSON processing" `Quick test_json_module;
    ];
  ]