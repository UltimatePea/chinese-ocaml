...
let test_comprehensive_artistic_evaluation () =
  ...
  Alcotest.(check bool)
    "对仗得分应在合理范围内" true
    (report.parallelism_score >= 0.0 && report.parallelism_score <= 1.0);
  Alcotest.(check bool)
    "意象得分应在合理范围内" true
    (report.imagery_score >= 0.0 && report.imagery_score <= 1.0);
  Alcotest.(check bool) "节奏得分应在合理范围内" true (report.rhythm_score >= 0.0 && report.rhythm_score <= 1.0);
  Alcotest.(check bool)
    "雅致得分应在合理范围内" true
    (report.elegance_score >= 0.0 && report.elegance_score <= 1.0);
  ...
  run "Artistic Evaluation Tests"
    [
      ...
      ( "comprehensive_artistic_evaluation",
        [ test_case "basic" `Quick test_comprehensive_artistic_evaluation ] );
      ( "evaluate_siyan_parallel_prose",
        [ test_case "basic" `Quick test_evaluate_siyan_parallel_prose ] );
      ...
      ( "generate_improvement_suggestions",
        [ test_case "basic" `Quick test_generate_improvement_suggestions ] );
    ]
