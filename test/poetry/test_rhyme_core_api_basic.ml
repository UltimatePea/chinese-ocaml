(** 诗韵核心API基础测试 - Phase 1-A 韵律系统整合版本
    ...
    此测试文件为诗韵模块建立基础测试覆盖，确保核心韵律功能的正确性。 已适配新的统一韵律类型系统 - Phase 1-A 韵律系统整合。
...
let test_find_character_rhyme_basic () =
  ...
  check bool "无效字符应返回NotFound" true (match result_invalid with NotFound _ -> true | _ -> false)
...
let test_find_character_rhyme_common_chars () =
  ...
  (* 应该有一些字符有韵律信息 *)
  let found_results = List.filter (function Found _ -> true | _ -> false) results in
...
let test_empty_string_handling () =
  ...
  check bool "空字符串应返回NotFound" true (match result_empty with NotFound _ -> true | _ -> false)
