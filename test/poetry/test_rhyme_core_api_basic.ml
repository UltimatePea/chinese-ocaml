(** 诗韵核心API基础测试 - 解决Critical Issue #1757
    ...
    针对src/poetry/core/rhyme_core_api.ml的关键功能进行验证。
...
let test_find_character_rhyme_exn () =
  ...
  with
  | Poetry_core.Types.RhymeException _ -> 
      check bool "正确抛出RhymeException" true true
  | _ -> 
      check bool "应抛出RhymeException类型异常" false true
...
let test_unicode_characters () =
  ...
  List.iter (fun char ->
    ...
    check bool ("Unicode字符" ^ char ^ "查询不应出错") true 
...
let test_suite = [
  ...
  "异常处理", [
    test_case "异常抛出版本测试" `Quick test_find_character_rhyme_exn;
  ];
  ...
  "性能测试", [
    test_case "基础性能测试" `Quick test_performance_basic;
  ];
]
