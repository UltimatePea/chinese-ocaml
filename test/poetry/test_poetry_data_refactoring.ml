(** 诗词数据重构测试 - 技术债务修复验证 Fix #724
    ...
    测试JSON数据加载器和重构后的灰韵组数据模块功能。 确保向后兼容性和功能正确性。
...
let test_json_data_loader () =
  ...
  try
    (* 测试数据加载 *)
    let data =
      Rhyme_json_data_loader.load_rhyme_data_from_json
  ...
  with exn -> Printf.printf "❌ JSON数据加载器测试失败: %s\n\n" (Printexc.to_string exn)
...
let test_refactored_hui_rhyme_compatibility () =
  ...
  with exn -> Printf.printf "❌ 重构模块兼容性测试失败: %s\n\n" (Printexc.to_string exn)
...
let test_data_integrity () =
  ...
  try
    ...
    if expected_count = actual_count then Printf.printf "✅ 数据计数一致性验证通过\n"
    else Printf.printf "❌ 数据计数不一致\n";
    ...
    if unique_count = actual_count then Printf.printf "✅ 字符唯一性验证通过\n"
    else Printf.printf "⚠️  发现重复字符: %d个重复\n" (actual_count - unique_count);

    (* 检查韵律分类一致性 *)
    let all_ze_sheng =
      List.for_all
        (fun char_data -> char_data.category = Rhyme_json_data_loader.ZeSheng)
        data.all_characters
    in

    let all_hui_rhyme =
      List.for_all
        (fun char_data -> char_data.group = Rhyme_json_data_loader.HuiRhyme)
        data.all_characters
    in
    ...
    if all_ze_sheng && all_hui_rhyme then Printf.printf "✅ 韵律分类一致性验证通过\n"
    else Printf.printf "❌ 韵律分类不一致\n";

    Printf.printf "✅ 数据完整性测试完成\n\n"
  with exn -> Printf.printf "❌ 数据完整性测试失败: %s\n\n" (Printexc.to_string exn)
...
let test_performance () =
  ...
  with exn -> Printf.printf "❌ 性能测试失败: %s\n\n" (Printexc.to_string exn)
...
(** 如果直接运行此文件 *)
let () = if Sys.argv.(0) |> Filename.basename = "test_poetry_data_refactoring" then run_all_tests ()
