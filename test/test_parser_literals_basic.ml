(** 字面量解析模块基础测试 - Fix #1034 Phase 1 验证从大型模块分离出的parser_literals模块功能完整性 *)
...
let create_parser_state tokens =
  let positioned_tokens =
    Array.of_list
      (List.mapi (fun i token -> (token, { line = 1; column = i + 1; filename = "test" })) tokens)
  in
  {
    token_array = positioned_tokens;
    array_length = Array.length positioned_tokens;
    current_pos = 0;
  }
...
let test_string_literal () =
  ...
  let state = create_parser_state tokens in
  let expr, _new_state = parse_literal_expr state in
  check Alcotest.string "string literal"
    "(Ast.LitExpr (Ast.StringLit \"\\233\\170\\134\\232\\168\\128\"))" (show_expr expr)
