...
let test_poetry_compiler_executability () =
  ...
  if not (Sys.file_exists compiler_path) then
    failwith ("诗韵编译器文件不存在: " ^ compiler_path);
  ...
  if not (Sys.file_exists output_file) then
    failwith ("编译器未生成预期的C代码文件: " ^ output_file);
...
let test_poetry_compiler_features () =
  ...
  if Sys.file_exists output_file then (
    ...
    if not (String.contains content '#') then
      failwith "生成的C代码缺少必要的头文件";
    
    Printf.printf "✓ 测试通过: %s\n" test_name;
    Printf.printf "  - 生成的C代码包含诗词文化元素\n";
    Printf.printf "  - 代码结构完整\n\n"
  ) else (
    Printf.printf "⚠ 跳过测试: 输出文件不存在\n\n"
  )
