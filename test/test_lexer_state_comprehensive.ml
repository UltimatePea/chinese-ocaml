(** 骆言词法分析器状态管理模块综合测试
    ...
    技术债务改进：核心模块测试覆盖率提升 - Fix #1009 Phase 2 第二阶段
    覆盖范围：lexer_state.ml (144行) - 词法分析状态管理
...
module TestHelpers = struct
  ...
  let create_test_state input filename =
    create_lexer_state input filename
...
let test_create_lexer_state () =
  ...
  TestHelpers.check_state_fields state 
    ~expected_pos:0 ~expected_line:1 ~expected_col:1 ~expected_filename:"test.ml";
  ...
  TestHelpers.check_state_fields empty_state
    ~expected_pos:0 ~expected_line:1 ~expected_col:1 ~expected_filename:"empty.ml";
  check int "空字符串长度" 0 empty_state.length;

  (* 测试中文字符串 *)
  let chinese_state = TestHelpers.create_test_state "你好世界" "chinese.ml" in
  TestHelpers.check_state_fields chinese_state
    ~expected_pos:0 ~expected_line:1 ~expected_col:1 ~expected_filename:"chinese.ml";
...
let test_advance_ascii () =
  ...
  TestHelpers.check_state_fields advanced
    ~expected_pos:1 ~expected_line:1 ~expected_col:2 ~expected_filename:"test.ml";

  (* 测试连续前进多个字符 *)
  let twice_advanced = advance (advance state) in
  TestHelpers.check_state_fields twice_advanced
    ~expected_pos:2 ~expected_line:1 ~expected_col:3 ~expected_filename:"test.ml";

  (* 测试到达字符串末尾 *)
  let end_state = { state with position = 5 } in
  let still_at_end = advance end_state in
  TestHelpers.check_state_fields still_at_end
    ~expected_pos:5 ~expected_line:1 ~expected_col:1 ~expected_filename:"test.ml";
  ()
...
let test_advance_newlines () =
  ...
  TestHelpers.check_state_fields after_a
    ~expected_pos:1 ~expected_line:1 ~expected_col:2 ~expected_filename:"test.ml";

  let after_newline = advance after_a in
  TestHelpers.check_state_fields after_newline
    ~expected_pos:2 ~expected_line:2 ~expected_col:1 ~expected_filename:"test.ml";
  ...
  TestHelpers.check_state_fields step3
    ~expected_pos:3 ~expected_line:3 ~expected_col:1 ~expected_filename:"test.ml";
  ...
  TestHelpers.check_state_fields after_n
    ~expected_pos:3 ~expected_line:2 ~expected_col:1 ~expected_filename:"test.ml";
  ()
...
let test_get_position () =
  ...
  TestHelpers.check_position 
    (TestHelpers.make_position 1 1 "example.ml") pos;

  (* 测试前进后的位置 *)
  let advanced_state = { state with current_line = 3; current_column = 5 } in
  let advanced_pos = get_position advanced_state in
  TestHelpers.check_position
    (TestHelpers.make_position 3 5 "example.ml") advanced_pos;
  ...
  TestHelpers.check_position
    (TestHelpers.make_position 1 1 "other.ml") different_pos;
  ()
...
let test_skip_comment_basic () =
  ...
  TestHelpers.check_state_fields after_comment
    ~expected_pos:10 ~expected_line:1 ~expected_col:11 ~expected_filename:"test.ml";
  ...
  TestHelpers.check_state_fields after_nested
    ~expected_pos:23 ~expected_line:1 ~expected_col:24 ~expected_filename:"test.ml";
  ...
  TestHelpers.check_state_fields after_multiline
    ~expected_pos:15 ~expected_line:2 ~expected_col:8 ~expected_filename:"test.ml";
  ()
...
let test_skip_chinese_comment () =
  ...
  (* 验证跳过了整个中文注释 *)
  check bool "跳过中文注释后位置正确" true 
    (after_chinese_comment.position > chinese_state.position);
  ...
  check bool "多行中文注释处理" true
    (after_multiline_chinese.position > multiline_chinese_state.position);
  ()
...
let test_skip_whitespace_basic () =
  ...
  check bool "仅空白字符处理" true
    (after_only_whitespace.position >= only_whitespace_state.position);
  ...
  | _ -> check bool "跳过混合空白字符" true 
      (after_mixed.position > mixed_whitespace_state.position));
  ()
...
let test_skip_whitespace_comments () =
  ...
  | _ -> check bool "跳过空白和注释" true
      (after_combined.position > combined_state.position));
  ...
  check bool "跳过空白和中文注释" true
    (after_chinese_combined.position > chinese_combined_state.position);
  ...
  check bool "跳过嵌套空白和注释" true
    (after_nested.position > nested_state.position);
  ()
...
let test_performance_large_input () =
  ...
  let rec advance_multiple state count =
    ...
    else advance_multiple (advance state) (count - 1)
  in
  
  let advanced_large = advance_multiple large_state 10 in
  check bool "大文件位置前进" true
    (advanced_large.position > large_state.position);
  ...
  check bool "大量空白字符跳过" true
    (after_large_whitespace.position > large_whitespace_state.position);
  ()
...
let test_extreme_edge_cases () =
  (* 测试位置在字符串末尾的各种操作 *)
  let end_state = { 
    (TestHelpers.create_test_state "abc" "test.ml") with position = 3 
  } in
