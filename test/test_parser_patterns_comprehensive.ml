...
module BasicPatternTests = struct
  ...
  let test_var_pattern () =
    let tokens = [
      make_token (QuotedIdentifierToken "x") 1 1;
      make_token EOF 1 2;
    ] in
    let state = create_test_state tokens in
    let (pattern, _final_state) = parse_pattern state in
  ...
  let test_string_literal_pattern () =
    let tokens = [
      make_token (StringToken "hello") 1 1;
      make_token EOF 1 2;
    ] in
    let state = create_test_state tokens in
    let (pattern, _final_state) = parse_pattern state in
  ...
  let test_chinese_number_pattern () =
    let tokens = [
      make_token (ChineseNumberToken "三") 1 1;
      make_token EOF 1 2;
    ] in
    let state = create_test_state tokens in
    let (pattern, _final_state) = parse_pattern state in
...
module PolymorphicVariantTests = struct
  ...
  let test_simple_polymorphic_variant () =
    let tokens = [
      ...
      make_token (QuotedIdentifierToken "Red") 1 2;
      make_token EOF 1 3;
    ] in
    let state = create_test_state tokens in
    let (pattern, _final_state) = parse_pattern state in
...
module ErrorHandlingTests = struct
  ...
  let test_unsupported_token_error () =
    let tokens = [
      make_token LeftBrace 1 1;  (* 不支持的token *)
      make_token EOF 1 2;
    ] in
    let state = create_test_state tokens in
  ...
  let tests = [
    test_case "Unsupported token error" `Quick test_unsupported_token_error;
  ]
end
