(** 骆言内置字符串模块测试 - Phase 26 内置模块测试覆盖率提升
    ...
    本测试模块专门针对 builtin_string.ml 模块进行全面功能测试，
    重点测试字符串处理的正确性、中文字符支持和边界条件处理。
...
module TestUtils = struct
  ...
  let apply_curried_function func arg1 arg2 =
    match func [arg1] with
    | BuiltinFunctionValue next_func -> next_func [arg2]
    | other -> other
  ...
  let check_string_list result expected_strings =
    ...
    | ListValue items ->
        ...
        List.for_all2 (fun actual expected ->
          match actual with
          | StringValue s -> s = expected
          | _ -> false
...
module StringConcatTests = struct
  ...
  let test_basic_string_concat () =
    (* 测试英文字符串连接 *)
    let result1 = TestUtils.apply_curried_function string_concat_function 
                    (StringValue "Hello") (StringValue "World") in
    check bool "英文字符串连接应正确" true 
      (TestUtils.values_equal result1 (StringValue "HelloWorld"));
    
    (* 测试带空格的连接 *)
    let result2 = TestUtils.apply_curried_function string_concat_function 
                    (StringValue "Hello ") (StringValue "World") in
    check bool "带空格连接应正确" true 
      (TestUtils.values_equal result2 (StringValue "Hello World"));
    
    (* 测试空字符串连接 *)
    let result3 = TestUtils.apply_curried_function string_concat_function 
                    (StringValue "") (StringValue "Test") in
    check bool "空字符串连接应正确" true 
      (TestUtils.values_equal result3 (StringValue "Test"))

  (** 测试中文字符串连接 *)
  let test_chinese_string_concat () =
    (* 测试中文字符串连接 *)
    let result1 = TestUtils.apply_curried_function string_concat_function 
                    (StringValue "你好") (StringValue "世界") in
    check bool "中文字符串连接应正确" true 
      (TestUtils.values_equal result1 (StringValue "你好世界"));
    
    (* 测试中英文混合连接 *)
    let result2 = TestUtils.apply_curried_function string_concat_function 
                    (StringValue "Hello") (StringValue "世界") in
    check bool "中英文混合连接应正确" true 
      (TestUtils.values_equal result2 (StringValue "Hello世界"));
    
    (* 测试诗词连接 *)
    let result3 = TestUtils.apply_curried_function string_concat_function 
                    (StringValue "春眠不觉晓，") (StringValue "处处闻啼鸟。") in
    check bool "诗词连接应正确" true 
      (TestUtils.values_equal result3 (StringValue "春眠不觉晓，处处闻啼鸟。"))

  (** 测试字符串连接的边界条件 *)
  let test_concat_boundary_cases () =
    (* 测试两个空字符串连接 *)
    let result1 = TestUtils.apply_curried_function string_concat_function 
                    (StringValue "") (StringValue "") in
    check bool "两个空字符串连接应为空" true 
      (TestUtils.values_equal result1 (StringValue ""));
    
    (* 测试很长字符串连接 *)
    let long_string1 = String.make 1000 'A' in
    let long_string2 = String.make 1000 'B' in
    let result2 = TestUtils.apply_curried_function string_concat_function 
                    (StringValue long_string1) (StringValue long_string2) in
    let expected_long = long_string1 ^ long_string2 in
    check bool "长字符串连接应正确" true 
      (TestUtils.values_equal result2 (StringValue expected_long))
...
module StringContainsTests = struct
  ...
  let test_basic_string_contains () =
    (* 测试字符包含检测 *)
    let result1 = TestUtils.apply_curried_function string_contains_function 
                    (StringValue "Hello World") (StringValue "H") in
    check bool "应检测到包含字符H" true 
      (TestUtils.values_equal result1 (BoolValue true));
    
    let result2 = TestUtils.apply_curried_function string_contains_function 
                    (StringValue "Hello World") (StringValue "X") in
    check bool "应检测到不包含字符X" true 
      (TestUtils.values_equal result2 (BoolValue false))
  ...
  let test_chinese_string_contains () =
    ...
    (* 测试包含ASCII字符的中文字符串 *)
    let result1 = TestUtils.apply_curried_function string_contains_function 
                    (StringValue "Hello你好") (StringValue "H") in
    check bool "应检测到包含ASCII字符'H'" true 
      (TestUtils.values_equal result1 (BoolValue true));
    
    let result2 = TestUtils.apply_curried_function string_contains_function 
                    (StringValue "春眠不觉晓ABC") (StringValue "A") in
    check bool "应检测到包含ASCII字符'A'" true 
      (TestUtils.values_equal result2 (BoolValue true))

  (** 测试包含检测的边界条件 *)
  let test_contains_boundary_cases () =
    (* 测试空字符串中查找 *)
    let result1 = TestUtils.apply_curried_function string_contains_function 
                    (StringValue "") (StringValue "a") in
    check bool "空字符串不应包含任何字符" true 
      (TestUtils.values_equal result1 (BoolValue false));
    
    (* 测试单字符字符串 *)
    let result2 = TestUtils.apply_curried_function string_contains_function 
                    (StringValue "A") (StringValue "A") in
    check bool "单字符字符串应包含自身" true 
      (TestUtils.values_equal result2 (BoolValue true))
...
module StringSplitTests = struct
  ...
  let test_basic_string_split () =
    ...
    let result1 = TestUtils.apply_curried_function string_split_function 
                    (StringValue "a,b,c") (StringValue ",") in
    check bool "逗号分割应正确" true 
      (TestUtils.check_string_list result1 ["a"; "b"; "c"]);
    
    (* 测试空格分割 *)
    let result2 = TestUtils.apply_curried_function string_split_function 
                    (StringValue "Hello World Test") (StringValue " ") in
    check bool "空格分割应正确" true 
      (TestUtils.check_string_list result2 ["Hello"; "World"; "Test"])
  ...
  let test_chinese_string_split () =
    ...
    (* 测试分号分割 *)
    let result2 = TestUtils.apply_curried_function string_split_function 
                    (StringValue "春眠不觉晓;处处闻啼鸟") (StringValue ";") in
    check bool "分号分割应正确" true 
      (TestUtils.check_string_list result2 ["春眠不觉晓"; "处处闻啼鸟"])
  ...
  let test_split_boundary_cases () =
    ...
    let result1 = TestUtils.apply_curried_function string_split_function 
                    (StringValue "") (StringValue ",") in
    check bool "空字符串分割应返回包含空字符串的列表" true 
      (TestUtils.check_string_list result1 [""]);
    
    (* 测试不包含分隔符的字符串 *)
    let result2 = TestUtils.apply_curried_function string_split_function 
                    (StringValue "NoSeparator") (StringValue ",") in
    check bool "不包含分隔符应返回原字符串" true 
      (TestUtils.check_string_list result2 ["NoSeparator"])
...
module StringMatchTests = struct
  ...
  let test_basic_string_match () =
    (* 测试简单字符匹配 *)
    let result1 = TestUtils.apply_curried_function string_match_function 
                    (StringValue "Hello") (StringValue "H.*") in
    check bool "应匹配以H开头的模式" true 
      (TestUtils.values_equal result1 (BoolValue true));
    
    let result2 = TestUtils.apply_curried_function string_match_function 
                    (StringValue "Hello") (StringValue "A.*") in
    check bool "不应匹配以A开头的模式" true 
      (TestUtils.values_equal result2 (BoolValue false))
  ...
  let test_complex_string_match () =
    (* 测试数字模式 *)
    let result1 = TestUtils.apply_curried_function string_match_function 
                    (StringValue "123") (StringValue "[0-9]+") in
    check bool "应匹配数字模式" true 
      (TestUtils.values_equal result1 (BoolValue true));
    
    (* 测试精确匹配 *)
    let result2 = TestUtils.apply_curried_function string_match_function 
                    (StringValue "test") (StringValue "test") in
    check bool "应精确匹配" true 
      (TestUtils.values_equal result2 (BoolValue true))
  ...
  let test_match_boundary_cases () =
    ...
    let result1 = TestUtils.apply_curried_function string_match_function 
                    (StringValue "") (StringValue ".*") in
    check bool "空字符串应匹配.*模式" true 
      (TestUtils.values_equal result1 (BoolValue true));
    
    (* 测试单字符匹配 *)
    let result2 = TestUtils.apply_curried_function string_match_function 
                    (StringValue "a") (StringValue ".") in
    check bool "单字符应匹配.模式" true 
      (TestUtils.values_equal result2 (BoolValue true))
...
module StringLengthReverseTests = struct
  ...
  let test_string_length () =
    ...
    check bool "英文字符串长度应为5" true 
      (TestUtils.values_equal result1 (IntValue 5));
    
    (* 测试中文字符串长度（UTF-8字节数） *)
    let result2 = string_length_function [StringValue "你好"] in
    check bool "中文字符串字节长度应为6" true 
      (TestUtils.values_equal result2 (IntValue 6));
    
    (* 测试空字符串长度 *)
    let result3 = string_length_function [StringValue ""] in
    check bool "空字符串长度应为0" true 
      (TestUtils.values_equal result3 (IntValue 0))

  (** 测试字符串反转功能 *)
  let test_string_reverse () =
    (* 测试英文字符串反转 *)
    let result1 = string_reverse_function [StringValue "Hello"] in
    check bool "英文字符串反转应正确" true 
      (TestUtils.values_equal result1 (StringValue "olleH"));
    
    (* 测试简单字符反转 *)
    let result2 = string_reverse_function [StringValue "abc"] in
    check bool "简单字符反转应正确" true 
      (TestUtils.values_equal result2 (StringValue "cba"));
    
    (* 测试空字符串反转 *)
    let result3 = string_reverse_function [StringValue ""] in
    check bool "空字符串反转应为空" true 
      (TestUtils.values_equal result3 (StringValue ""))
  ...
  let test_length_reverse_boundary_cases () =
    ...
    check bool "单字符长度应为1" true 
      (TestUtils.values_equal length_result (IntValue 1));
    check bool "单字符反转应为自身" true 
      (TestUtils.values_equal reverse_result (StringValue "A"));
    
    (* 测试长字符串 *)
    let long_string = StringValue (String.make 1000 'X') in
    let long_length = string_length_function [long_string] in
    check bool "长字符串长度应为1000" true 
      (TestUtils.values_equal long_length (IntValue 1000))
...
module StringIntegrationTests = struct
  ...
  let test_string_function_composition () =
    (* 连接后分割 *)
    let concat_result = TestUtils.apply_curried_function string_concat_function 
                          (StringValue "a,b") (StringValue ",c,d") in
    let split_result = TestUtils.apply_curried_function string_split_function 
                         concat_result (StringValue ",") in
    check bool "连接后分割应正确" true 
      (TestUtils.check_string_list split_result ["a"; "b"; "c"; "d"]);
    ...
    check bool "反转后长度应保持一致" true 
      (TestUtils.values_equal original_length reversed_length)
  ...
  let test_string_functions_table () =
    ...
    List.iter (fun expected ->
      check bool (Printf.sprintf "函数表应包含'%s'" expected) true 
        (List.mem expected actual_functions)
    ) expected_functions;
  ...
  let test_string_performance () =
    ...
    let large_concat = TestUtils.apply_curried_function string_concat_function 
                         large_string1 large_string2 in
    let large_length = string_length_function [large_concat] in
    check bool "大字符串连接长度应正确" true 
      (TestUtils.values_equal large_length (IntValue 10000))
...
module ChineseStringTests = struct
  ...
  let test_unicode_emoji_strings () =
    ...
    check bool "表情符号字符串应有长度" true 
      (match emoji_length with IntValue n -> n > 0 | _ -> false);
    
    (* 测试表情符号连接 *)
    let spring_emoji = StringValue "🌸" in
    let chinese_spring = StringValue "春天" in
    let combined = TestUtils.apply_curried_function string_concat_function 
                     spring_emoji chinese_spring in
  ...
  let test_traditional_chinese_processing () =
    ...
    check bool "繁体中文字符串应有长度" true 
      (match trad_length with IntValue n -> n > 0 | _ -> false);
    
    (* 测试古文字符 *)
    let classical = StringValue "夫君子之行，靜以修身，儉以養德" in
    let classical_split = TestUtils.apply_curried_function string_split_function 
                            classical (StringValue "，") in
  ...
  let test_chinese_programming_concepts () =
    ...
    (* 连接所有编程术语 *)
    let combined_terms = List.fold_left (fun acc term ->
      TestUtils.apply_curried_function string_concat_function acc (StringValue (term ^ "、"))
    ) (StringValue "編程概念：") programming_terms in
    
    let terms_length = string_length_function [combined_terms] in
    check bool "编程术语字符串应有合理长度" true 
      (match terms_length with IntValue n -> n > 20 | _ -> false)
...
module StringErrorHandlingTests = struct
  ...
  let test_string_function_error_handling () =
    ...
    check bool "非字符串参数应抛出错误" true 
      (TestUtils.expect_runtime_error error_case1);
    
    let error_case2 () = string_reverse_function [BoolValue true] in
    check bool "非字符串反转应抛出错误" true 
      (TestUtils.expect_runtime_error error_case2);
    
    (* 测试参数数量错误 *)
    let error_case3 () = string_length_function [] in
    check bool "参数缺失应抛出错误" true 
      (TestUtils.expect_runtime_error error_case3)
