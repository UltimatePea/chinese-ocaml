...
let () =
  ...
  with
  | e -> Printf.printf "❌ Unicode常量模块导入失败: %s\n" (Printexc.to_string e));
  ...
  with
  | e -> Printf.printf "❌ Unicode字符分类测试失败: %s\n" (Printexc.to_string e));
  ...
  with
  | e -> Printf.printf "❌ Unicode编码处理测试失败: %s\n" (Printexc.to_string e));
  ...
  with
  | e -> Printf.printf "❌ Unicode标准化测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    (* 测试一些可能有问题的Unicode序列 *)
    let error_cases = [
      ("无效UTF-8序列", "\xC0\x80");        (* 过长编码 *)
      ("截断的UTF-8", "\xE4\xB8");         (* 不完整的3字节序列 *)
      ("孤立续字节", "\x80");              (* 单独的续字节 *)
      ("超出范围", "\xF4\x90\x80\x80");    (* 超出Unicode范围 *)
    ] in
    
    Printf.printf "🚨 Unicode错误处理测试:\n";
    List.iter (fun (desc, invalid_seq) ->
      ...
      with
      | e -> Printf.printf "检测到错误: %s\n" (Printexc.to_string e))
    ) error_cases;
    
    Printf.printf "✅ Unicode错误处理测试完成\n";
  with
  | e -> Printf.printf "❌ Unicode错误处理测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    if duration < 1.0 then
      Printf.printf "✅ Unicode处理性能优秀\n"
    else
      Printf.printf "⚠️  Unicode处理性能可能需要优化\n";
  with
  | e -> Printf.printf "❌ Unicode性能测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    for _i = 1 to consistency_test_count do
      ...
      try
        ...
        ()
      with
      | _ -> inconsistency_found := true
    done;
    
    if not !inconsistency_found then
      Printf.printf "✅ Unicode常量一致性测试通过 (%d次检查)\n" consistency_test_count
    else
      Printf.printf "❌ 发现Unicode常量不一致\n";
  with
  | e -> Printf.printf "❌ Unicode常量一致性测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    List.rev !analysis_results |> List.iter (fun (key, value) ->
      Printf.printf "  - %s: %s\n" key value
    );
    
    Printf.printf "✅ 综合Unicode处理测试完成\n";
  with
  | e -> Printf.printf "❌ 综合Unicode处理测试失败: %s\n" (Printexc.to_string e));
