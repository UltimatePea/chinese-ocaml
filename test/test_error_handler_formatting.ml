(** 错误处理格式化模块测试套件
    ...
    验证error_handler_formatting.ml模块的错误格式化功能
    包括错误消息格式化、彩色输出、日志记录等功能
...
let test_format_enhanced_error () =
  ...
  (* 测试语法错误格式化 *)
  let syntax_error = create_test_enhanced_error 
    (SyntaxError ("语法错误测试", create_test_position ())) 
    SkipAndContinue in
  let formatted_syntax = format_enhanced_error syntax_error in
  
  check (bool) "语法错误格式化应包含错误信息" true 
    (contains_substring formatted_syntax "语");
  check (bool) "语法错误格式化应包含上下文信息" true 
    (contains_substring formatted_syntax "上下文");
  check (bool) "语法错误格式化应包含恢复策略" true 
    (contains_substring formatted_syntax "恢复");
  
  (* 测试类型错误格式化 *)
  let type_error = create_test_enhanced_error 
    (TypeError ("类型错误测试", Some (create_test_position ()))) 
    SyncToNextStatement in
  let formatted_type = format_enhanced_error type_error in
  
  check (bool) "类型错误格式化应包含错误信息" true 
    (String.length formatted_type > 0);
  check (bool) "类型错误格式化应包含同步策略" true 
    (contains_substring formatted_type "同步");
  
  (* 测试内部错误格式化 *)
  let internal_error = create_test_enhanced_error 
    (InternalError "内部错误测试") 
    Abort in
  let formatted_internal = format_enhanced_error internal_error in
  
  check (bool) "内部错误格式化应包含错误信息" true 
    (String.length formatted_internal > 0);
  check (bool) "内部错误格式化应包含终止策略" true 
    (contains_substring formatted_internal "终止");
...
let test_recovery_strategy_formatting () =
  ...
  check (bool) "跳过策略应正确格式化" true 
    (contains_substring formatted_skip "跳过");
  ...
  check (bool) "同步策略应正确格式化" true 
    (contains_substring formatted_sync "同步");
  ...
  check (bool) "替代方案策略应正确格式化" true 
    (contains_substring formatted_alt "替代");
  ...
  check (bool) "用户输入策略应正确格式化" true 
    (contains_substring formatted_input "用户输入");
  ...
  check (bool) "终止处理策略应正确格式化" true 
    (contains_substring formatted_abort "终止");
...
let test_colorize_error_message () =
  ...
  check (bool) "警告消息应包含颜色代码" true 
    (String.length colored_warning >= String.length test_message);
  
  (* 测试错误级别彩色输出 *)
  let colored_error = colorize_error_message Error test_message in
  check (bool) "错误消息应包含颜色代码" true 
    (String.length colored_error >= String.length test_message);
  
  (* 测试严重错误级别彩色输出 *)
  let colored_fatal = colorize_error_message Fatal test_message in
  check (bool) "严重错误消息应包含颜色代码" true 
    (String.length colored_fatal >= String.length test_message);
...
let test_attempt_count_formatting () =
  ...
  (* 测试无重试的情况 *)
  let no_retry_error = create_test_enhanced_error 
    (SyntaxError ("无重试测试", create_test_position ())) 
    SkipAndContinue in
  let formatted_no_retry = format_enhanced_error no_retry_error in
  check (bool) "无重试情况应正确格式化" true 
    (String.length formatted_no_retry > 0);
  
  (* 测试有重试的情况 *)
  let retry_error = { 
    (create_test_enhanced_error 
      (SyntaxError ("重试测试", create_test_position ())) 
      SkipAndContinue) 
    with attempt_count = 3 
  } in
  let formatted_retry = format_enhanced_error retry_error in
  check (bool) "重试情况应包含重试信息" true 
    (contains_substring formatted_retry "重试");
...
let test_context_formatting () =
  ...
  let detailed_context = {
    ...
    module_name = "详细测试模块";
    timestamp = 1640995200.0; (* 固定时间戳用于测试 *)
  ...
  check (bool) "上下文格式化应包含文件名" true 
    (contains_substring formatted_context "详细");
  check (bool) "上下文格式化应包含模块信息" true 
    (contains_substring formatted_context "模块");
  check (bool) "上下文格式化应包含函数信息" true 
    (contains_substring formatted_context "函数");
...
let test_log_error_to_file () =
  ...
  (* 创建测试错误用于日志记录 *)
  let log_error = create_test_enhanced_error 
  ...
  with
  | _ -> check (bool) "日志记录功能出现异常" false true;
...
let test_edge_cases () =
  ...
  check (bool) "空调用栈应正确格式化" true 
    (String.length formatted_empty_stack > 0);
  
  (* 测试长错误消息 *)
  let long_message = String.make 1000 'A' in
  let long_error = create_test_enhanced_error 
    (RuntimeError (long_message, None)) 
    Abort in
  let formatted_long = format_enhanced_error long_error in
  check (bool) "长错误消息应正确格式化" true 
    (String.length formatted_long > 0);
