...
let test_chinese_number_constant_error_handling () =
  ...
  let test_error_case value expected_error_fragment =
    ...
    with
    | RuntimeError msg ->
        check bool ("错误消息包含" ^ expected_error_fragment) true (String.contains msg expected_error_fragment.[0])
    | _ -> fail "应该抛出运行时错误"
  in
...
let test_chinese_number_constants_table () =
  ...
  List.iter (fun expected ->
    check bool ("常量表包含" ^ expected) true (List.mem expected actual_numbers)
  ) expected_numbers;
...
let test_chinese_number_constants_values () =
  ...
  List.iter (fun (chinese_name, expected_value) ->
    ...
    | BuiltinFunctionValue f ->
        ...
        check_runtime_value 
          (chinese_name ^ "常量值测试") 
          (create_test_int expected_value) 
          result
...
let test_out_of_range_numbers () =
  ...
  let test_out_of_range value =
    ...
    with
    | RuntimeError msg ->
        check bool "错误消息包含数字" true (String.length msg > 0)
    | _ -> fail "应该抛出运行时错误"
  in
...
let test_constants_type_checking () =
  ...
  List.iter (fun (name, value) ->
    ...
    | BuiltinFunctionValue _ -> 
        check bool (name ^ "是内置函数值") true true
    | _ -> 
        fail (name ^ "应该是内置函数值类型")
...
let test_constants_table_indexing () =
  ...
  try
    ...
    fail "不应该找到'十'"
  with
  | Not_found -> check bool "正确处理不存在的常量" true true
