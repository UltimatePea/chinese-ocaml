(** 解析器基础表达式模块增强测试 - Phase 25 测试覆盖率提升
    ...
    本测试模块专门针对 parser_expressions_primary.ml 进行全面测试， 覆盖各种表达式解析场景，包括边界条件和错误处理。
...
module BasicExpressionTests = struct
  ...
  let test_identifier_parsing () =
    let test_cases =
      [ ("x", "简单标识符"); ("变量名", "中文标识符"); ("var_123", "带数字标识符"); ("诗词_解析器", "中英混合标识符") ]
    in
    List.iter
      (fun (id, desc) ->
        let tokens =
...
module PostfixExpressionTests = struct
  ...
  let test_chinese_bracket_array_access () =
    ...
    check (testable pp_expr equal_expr) "中文括号数组访问解析"
      (ArrayAccessExpr (VarExpr "数组", LitExpr (IntLit 1)))
      expr
end
...
module UnicodeTests = struct
  ...
  let test_chinese_identifiers () =
    let test_cases =
      [ ("变量", "简单中文标识符"); ("诗词解析器", "复杂中文标识符"); ("用户_输入", "中英混合标识符"); ("数据_2024", "带数字的中文标识符") ]
    in
    List.iter
      (fun (id, desc) ->
        let tokens =
...
module ErrorHandlingTests = struct
  ...
  let test_invalid_token_handling () =
    ...
    let tokens = [ (EOF, { line = 1; column = 1; filename = "test" }) ] in
    let state = create_test_state tokens in
    try
...
module PerformanceTests = struct
  ...
  let test_multiple_array_access () =
    ...
    let expected =
      ArrayAccessExpr
        ( ArrayAccessExpr (ArrayAccessExpr (VarExpr "arr", LitExpr (IntLit 0)), LitExpr (IntLit 1)),
          LitExpr (IntLit 2) )
    in
