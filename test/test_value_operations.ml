...
module TestDataGenerator = struct
  ...
  let create_function_values () =
    ...
    [
      ...
      ("标签函数", LabeledFunctionValue ([{label_name="test"; param_name="test"; param_type=None; is_optional=false; default_value=None}], VarExpr "test", simple_env));
    ]
  ...
  let create_test_env () =
    [
      ...
      ("nested_module", ModuleValue [
        ("SubModule", ModuleValue [("value", IntValue 42)])
      ]);
    ]
end
...
module TestBasicValueOperations = struct
  let test_empty_env () =
    ...
    Printf.printf "    空环境长度: %d (期望: 0) %s\n" 
      (List.length env) (if List.length env = 0 then "✓" else "✗");
  ...
  let test_bind_var () =
    ...
    Printf.printf "    绑定后环境长度: %d (期望: 2) %s\n" 
      (List.length env2) (if List.length env2 = 2 then "✓" else "✗");
    assert (List.length env2 = 2);
    
    (* 检查变量顺序（新变量应该在前面） *)
    (match env2 with
    | ("y", StringValue "hello") :: ("x", IntValue 42) :: [] ->
        Printf.printf "    变量绑定顺序正确 ✓\n"
    | _ -> 
  ...
  let test_lookup_var_basic () =
    ...
    List.iter (fun (var_name, expected_value) ->
  ...
  let test_lookup_var_error () =
    ...
    List.iter (fun var_name ->
      ...
      with
      | RuntimeError msg ->
          Printf.printf "    查找'%s': 正确抛出RuntimeError: %s ✓\n" var_name msg
      | _ ->
    ...
    with
    | RuntimeError msg ->
        Printf.printf "    空变量名: 正确抛出RuntimeError: %s ✓\n" msg
    | _ ->
  ...
  let test_lookup_var_module_access () =
    ...
    with
    | RuntimeError msg ->
        Printf.printf "    Math.undefined: 正确抛出RuntimeError: %s ✓\n" msg
    | _ -> assert false);
...
module TestValueToString = struct
  ...
  let test_function_value_to_string () =
    ...
    let labeled_value = LabeledFunctionValue ([{label_name="test"; param_name="test"; param_type=None; is_optional=false; default_value=None}], VarExpr "test", env) in
  ...
  let test_constructor_value_to_string () =
    ...
    let variant2 = PolymorphicVariantValue ("Point", Some (TupleValue [IntValue 10; IntValue 20])) in
...
module TestValueConversion = struct
  let test_value_to_bool () =
    ...
    let test_cases = [
      ...
      (StringValue "你好", true);
      (UnitValue, false);
      (ListValue [], true);  (* 非基础类型默认为true *)
      (FloatValue 0.0, true);  (* 浮点数都被认为是true *)
    ] in
...
module TestConstructorRegistration = struct
  let test_register_constructors () =
    ...
    (* 创建代数类型定义 *)
    let option_type = AlgebraicType [
      ("Some", Some (TypeVar "a"));
      ("None", None);
    ] in
    
    let list_type = AlgebraicType [
      ("Cons", Some (TupleType [TypeVar "a"; ConstructType ("list", [])]));
      ("Nil", None);
    ] in
    ...
    Printf.printf "    注册后环境长度: %d (期望: 4) %s\n" 
      (List.length env2) (if List.length env2 = 4 then "✓" else "✗");
...
module TestExceptionHandling = struct
  ...
  let test_exception_raised () =
    ...
    List.iter (fun exception_value ->
      try
        raise (ExceptionRaised exception_value)
      with
...
module TestEdgeCasesAndErrorHandling = struct
  let test_empty_containers () =
    ...
    (* 测试空容器的布尔值转换 *)
    assert (value_to_bool empty_list = true);  (* 非基础类型默认为true *)
  ...
  let test_circular_reference_safety () =
    ...
    with
    | Stack_overflow ->
        Printf.printf "    循环引用处理: 检测到栈溢出，符合预期 ✓\n"
    | _ ->
  ...
  let test_large_structures () =
    ...
    (* 创建大型记录 *)
    let large_record = RecordValue (List.init 20 (fun i -> 
      ("field" ^ string_of_int i, IntValue i))) in
...
module TestPerformance = struct
  ...
  let test_lookup_performance () =
    ...
    (* 创建大型环境 *)
    let large_env = List.init 1000 (fun i -> 
      ("var" ^ string_of_int i, IntValue i)) in
  ...
  let test_string_conversion_performance () =
    ...
    let test_values = Array.init 1000 (fun i ->
    ...
    let test_conversion () =
      Array.iter (fun value -> ignore (value_to_string value)) test_values
    in
  ...
  let test_type_conversion_performance () =
    ...
    let test_values = [
      IntValue 42; FloatValue 3.14; StringValue "123"; BoolValue true;
      StringValue "3.14"; IntValue 0; StringValue "false";
    ] in
