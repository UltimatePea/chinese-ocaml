...
module ModuleCategoryTests = struct
  ...
  let test_infer_value_module () =
    ...
    List.iter (fun (name, expected) ->
      ...
      check (testable (fun fmt -> function
      ...
      ) (=)) (Printf.sprintf "模块类型推断: %s" name) expected result
    ) test_cases

  (** 测试Type模块类型推断 *)
  let test_infer_type_module () =
    let test_cases = [
      ("TypesUnify", TypeModule);
      ("TypeSystem", TypeModule);
      ("TypeInference", TypeModule);
    ] in
    List.iter (fun (name, expected) ->
      let result = infer_module_category name in
      check (testable (fun fmt -> function
  ...
  let test_infer_lexer_module () =
    let test_cases = [
      ("LexerCore", LexerModule);
      ("LexerTokens", LexerModule);
      ("ChineseLexer", LexerModule);
    ] in
    List.iter (fun (name, expected) ->
      let result = infer_module_category name in
      check (testable (fun fmt -> function
  ...
  let test_infer_utility_module () =
    let test_cases = [
      ...
      ("", UtilityModule);  (* 空字符串应该默认为Utility *)
    ] in
    List.iter (fun (name, expected) ->
      let result = infer_module_category name in
      check (testable (fun fmt -> function
...
module BoundaryConditionTests = struct
  ...
  let test_unicode_module_names () =
    ...
    List.iter (fun name ->
      check unit (Printf.sprintf "Unicode模块名处理: %s" name) () (init_module_logger name)
    ) unicode_names
end
...
let () =
  run "Logger Init Helpers Tests" [
    ...
    ("兼容性", [
      test_case "replace_init_no_logger兼容性" `Quick CompatibilityTests.test_replace_init_no_logger;
    ]);
    ("边界条件", [
