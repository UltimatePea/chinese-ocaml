...
let test_parse_simple_expression () =
  ...
  check (testable pp_expr equal_expr) "简单整数表达式解析" 
    (LitExpr (IntLit 42)) expr
...
let test_parse_string_expression () =
  ...
  let state = create_parser_state tokens in
  let (expr, _) = parse_expression state in
  check (testable pp_expr equal_expr) "字符串表达式解析" 
    (LitExpr (StringLit "hello")) expr
...
let test_parse_variable_expression () =
  ...
  let state = create_parser_state tokens in
  let (expr, _) = parse_expression state in
  check (testable pp_expr equal_expr) "变量表达式解析" 
    (VarExpr "x") expr
...
let test_parse_binary_operation () =
  ...
  check (testable pp_expr equal_expr) "二元运算表达式解析" 
    (BinaryOpExpr (LitExpr (IntLit 1), Add, LitExpr (IntLit 2))) expr
...
let test_parse_function_call () =
  ...
  check (testable pp_expr equal_expr) "函数调用表达式解析" 
    (FunCallExpr (VarExpr "print", [LitExpr (StringLit "hello")])) expr
...
let test_parse_let_statement () =
  ...
  check (testable pp_stmt equal_stmt) "let语句解析" 
    (LetStmt ("x", LitExpr (IntLit 42))) stmt
...
let test_parse_simple_program () =
  ...
  check (testable pp_program equal_program) "简单程序解析" 
    [LetStmt ("x", LitExpr (IntLit 42)); ExprStmt (VarExpr "x")] program
...
let () = 
  run "Parser模块单元测试" [
    ...
    "错误处理测试", [
      test_case "语法错误处理" `Quick test_parse_error_handling;
    ];
  ]