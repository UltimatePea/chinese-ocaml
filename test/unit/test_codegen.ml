...
let test_string_and_variable_management () =
  ...
  (* 测试变量环境功能 *)
  let env_has_var1 = try let _ = lookup_var env2 "变量1" in true with _ -> false in
  let env_has_var2 = try let _ = lookup_var env2 "变量2" in true with _ -> false in
