...
let check_runtime_value msg expected actual =
  let value_to_string = function
    ...
    | Yyocamlc_lib.Value_operations.BoolValue b ->
        "Yyocamlc_lib.Value_operations.BoolValue(" ^ string_of_bool b ^ ")"
    | Yyocamlc_lib.Value_operations.UnitValue -> "Yyocamlc_lib.Value_operations.UnitValue"
    | Yyocamlc_lib.Value_operations.ListValue vals ->
        "Yyocamlc_lib.Value_operations.ListValue(["
        ^ String.concat "; " (List.map value_to_string vals)
        ^ "])"
    | _ -> "OtherValue"
  in
...
let test_binary_operations () =
  ...
  (* 测试逻辑运算 *)
  let and_result =
    execute_binary_op And (Yyocamlc_lib.Value_operations.BoolValue true)
      (Yyocamlc_lib.Value_operations.BoolValue true)
  in
  let or_result =
    execute_binary_op Or (Yyocamlc_lib.Value_operations.BoolValue false)
      (Yyocamlc_lib.Value_operations.BoolValue true)
  in
...
let test_builtin_functions () =
  ...
  let length_result =
    call_function
      ...
      [ Yyocamlc_lib.Value_operations.ListValue [ IntValue 1; IntValue 2; IntValue 3 ] ]
  in
...
let test_pattern_matching () =
  ...
  let string_match =
    match_pattern string_pattern (Yyocamlc_lib.Value_operations.StringValue "test")
  in
  ...
  let wildcard_match =
    match_pattern wildcard_pattern (Yyocamlc_lib.Value_operations.IntValue 999)
  in
