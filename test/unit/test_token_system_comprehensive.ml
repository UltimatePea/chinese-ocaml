...
let test_token_types_core () =
  ...
  check bool "FloatToken construction" true (match float_literal with FloatToken _ -> true | _ -> false);
  check bool "StringToken construction" true (match string_literal with StringToken _ -> true | _ -> false);
  check bool "BoolToken construction" true (match bool_literal with BoolToken _ -> true | _ -> false)
...
let test_token_conversion_system () =
  ...
  check bool "let keyword conversion" true 
    (match keyword_result with 
     | Token_system_core.Token_types.CoreLanguage _ -> true 
     | _ -> false);
  
  (* 测试标识符转换器 *)
  let identifier_result = Token_system_conversion.Identifier_converter.convert_simple_identifier "test_var" in
  ...
  (* 测试操作符转换器 *)
  let operator_result = Token_system_conversion.Operator_converter.convert_arithmetic_operator "+" in
...
let test_token_utils_comprehensive () =
  ...
  (* 测试Token分类工具 *)
  check bool "is_literal_token utility" true (Token_system_utils.Token_utils.is_literal literal_token);
  check bool "is_identifier_token utility" true (Token_system_utils.Token_utils.is_identifier identifier_token);
  check bool "is_literal false for identifier" false (Token_system_utils.Token_utils.is_literal identifier_token);
...
let test_token_error_handling () =
  ...
  check bool "invalid token error created" true
    (match error with 
     | Token_system_core.Token_errors.InvalidTokenType _ -> true 
     | _ -> false);
...
let test_token_statistics_and_analysis () =
  ...
  let find_count type_name =
    try List.assoc type_name stats
    with Not_found -> 0
  in
...
let test_large_scale_token_operations () =
  ...
  (* 创建大量Token *)
  let large_token_set = List.init 10000 (fun i ->
    if i mod 3 = 0 then
      Token_system_compatibility.Legacy_type_bridge.make_literal_token 
  ...
  let validation_result = Token_system_compatibility.Legacy_type_bridge.validate_token_stream large_token_set in
  ...
  check bool "large scale operation performance" true (duration < 2.0); (* 应该在2秒内完成 *)
