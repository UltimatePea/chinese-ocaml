...
(** 生成测试用的位置信息 *)
let make_test_position line column offset = { line; column; offset }

(** 断言两个Token相等 *)
let token_testable = testable (fun fmt _ -> Format.fprintf fmt "Token") ( = )

(** 生成大规模测试数据 *)
let generate_test_integers n = List.init n (fun i -> (i * 7) + 3) (* 生成非平凡的整数序列 *)

(** 生成边界值测试数据 *)
let boundary_integers = [ 0; 1; -1; max_int; min_int; 42; -42; 1000; -1000 ]

let boundary_floats = [ 0.0; 1.0; -1.0; 3.14159; -3.14159; 1e10; 1e-10; infinity; neg_infinity ]
...
let chinese_numbers =
  [ "一"; "二"; "三"; "四"; "五"; "六"; "七"; "八"; "九"; "十"; "一十二"; "二十三"; "一百"; "三点一四"; "零点五" ]
...
let test_convert_int_token () =
  ...
  check
    (testable
       (fun fmt -> function
         | IntToken i -> Format.fprintf fmt "IntToken %d" i | _ -> Format.fprintf fmt "Other")
       ( = ))
    "convert_int_token basic" (IntToken 42) result;
      ...
      check
        (testable
           (fun fmt -> function
             | IntToken i -> Format.fprintf fmt "IntToken %d" i | _ -> Format.fprintf fmt "Other")
           ( = ))
        ("convert_int_token boundary " ^ string_of_int i)
        (IntToken i) result)
    boundary_integers
...
let test_token_type_name _ =
  ...
  in
  List.iter
    (fun (token, expected_name) -> assert_equal expected_name (token_type_name token))
    test_cases
...
let test_count_token_types _ =
  ...
  let find_count name = List.assoc name counts in
...
let test_make_literal_tokens _ =
  let values =
    [
      ("int1", `Int 42); ("float1", `Float 3.14); ("string1", `String "hello"); ("bool1", `Bool true);
    ]
  in
  ...
  | Literal (IntToken 42) -> ()
  | _ -> (
      assert_failure "First token incorrect";
...
let test_validate_token_stream _ =
  ...
  assert_bool "Valid token stream validation failed" (validate_token_stream valid_tokens);

  (* 测试空Token流 *)
  assert_bool "Empty token stream validation failed" (validate_token_stream [])
...
let keyword_tests =
  "Keyword Conversion Tests" >::: [ "test_keyword_conversions" >:: test_keyword_conversions ]

let operator_tests =
  "Operator Conversion Tests" >::: [ "test_operator_conversions" >:: test_operator_conversions ]

let delimiter_tests =
  "Delimiter Conversion Tests" >::: [ "test_delimiter_conversions" >:: test_delimiter_conversions ]

let special_tests =
  "Special Token Conversion Tests"
...
let roundtrip_tests =
  "Roundtrip Consistency Tests" >::: [ "test_roundtrip_consistency" >:: test_roundtrip_consistency ]

(** 主测试套件 *)
let suite =
  "Legacy Type Bridge Comprehensive Tests"
  >::: [
         conversion_tests;
         identifier_tests;
         keyword_tests;
