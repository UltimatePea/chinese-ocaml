(** 重构后的遗留桥接模块测试 * * 验证Token系统兼容性桥接重构的正确性 * 确保重构后的代码保持向后兼容性 * * Author: Charlie, Planner Agent - Fix
    #1412 * Date: 2025-07-26 *)
...
module TestTypeConverter = struct
  ...
  let test_positioned_token_conversion () =
    ...
    | None -> Printf.printf "❌ 带位置Token转换失败\n"
...
module TestCompatibilityAPI = struct
  ...
  let test_token_conversion_compat () =
    ...
    let success_count =
      List.length (List.filter (function Some _ -> true | None -> false) batch_result)
    in
