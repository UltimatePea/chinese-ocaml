...
let test_convert_int_token () =
  ...
  check literal_token_testable "convert int 42" (Token_system_unified_core.Token_types.IntToken 42)
    result;

  let result = Token_system_unified_conversion.Legacy_type_bridge.convert_int_token 0 in
  check literal_token_testable "convert int 0" (Token_system_unified_core.Token_types.IntToken 0)
    result;

  let result = Token_system_unified_conversion.Legacy_type_bridge.convert_int_token (-1) in
  check literal_token_testable "convert int -1"
    (Token_system_unified_core.Token_types.IntToken (-1)) result

(** 测试浮点数转换 *)
let test_convert_float_token () =
  let result = Token_system_unified_conversion.Legacy_type_bridge.convert_float_token 3.14 in
  check literal_token_testable "convert float 3.14"
    (Token_system_unified_core.Token_types.FloatToken 3.14) result;

  let result = Token_system_unified_conversion.Legacy_type_bridge.convert_float_token 0.0 in
  check literal_token_testable "convert float 0.0"
    (Token_system_unified_core.Token_types.FloatToken 0.0) result

(** 测试字符串转换 *)
let test_convert_string_token () =
  let result = Token_system_unified_conversion.Legacy_type_bridge.convert_string_token "hello" in
  check literal_token_testable "convert string hello"
    (Token_system_unified_core.Token_types.StringToken "hello") result;
  ...
  check literal_token_testable "convert empty string"
    (Token_system_unified_core.Token_types.StringToken "") result

(** 测试布尔转换 *)
let test_convert_bool_token () =
  let result = Token_system_unified_conversion.Legacy_type_bridge.convert_bool_token true in
  check literal_token_testable "convert bool true"
    (Token_system_unified_core.Token_types.BoolToken true) result;

  let result = Token_system_unified_conversion.Legacy_type_bridge.convert_bool_token false in
  check literal_token_testable "convert bool false"
    (Token_system_unified_core.Token_types.BoolToken false) result

(** 测试中文数字转换 *)
let test_convert_chinese_number_token () =
  let result =
    Token_system_unified_conversion.Legacy_type_bridge.convert_chinese_number_token "一"
  in
  check literal_token_testable "convert chinese number 一"
    (Token_system_unified_core.Token_types.ChineseNumberToken "一") result
...
let test_make_literal_token () =
  ...
  check token_testable "make literal token"
    (Token_system_unified_core.Token_types.Literal
       (Token_system_unified_core.Token_types.IntToken 42))
    token

(** 测试标识符Token构造 *)
let test_make_identifier_token () =
  let simple_id =
    Token_system_unified_conversion.Legacy_type_bridge.convert_simple_identifier "test"
  in
  let token = Token_system_unified_conversion.Legacy_type_bridge.make_identifier_token simple_id in
  check token_testable "make identifier token"
    (Token_system_unified_core.Token_types.Identifier
       (Token_system_unified_core.Token_types.SimpleIdentifier "test"))
    token
...
let test_is_literal_token () =
  ...
  in
  check bool "is_literal_token should be true" true
    (Token_system_unified_conversion.Legacy_type_bridge.is_literal_token literal);

  let identifier =
    Token_system_unified_conversion.Legacy_type_bridge.make_identifier_token
      (Token_system_unified_conversion.Legacy_type_bridge.convert_simple_identifier "test")
  in
  check bool "is_literal_token should be false for identifier" false
    (Token_system_unified_conversion.Legacy_type_bridge.is_literal_token identifier)
...
let test_is_identifier_token () =
  let identifier =
    Token_system_unified_conversion.Legacy_type_bridge.make_identifier_token
      (Token_system_unified_conversion.Legacy_type_bridge.convert_simple_identifier "test")
  in
  check bool "is_identifier_token should be true" true
    (Token_system_unified_conversion.Legacy_type_bridge.is_identifier_token identifier);

  let literal =
    Token_system_unified_conversion.Legacy_type_bridge.make_literal_token
      (Token_system_unified_conversion.Legacy_type_bridge.convert_int_token 42)
  in
  check bool "is_identifier_token should be false for literal" false
    (Token_system_unified_conversion.Legacy_type_bridge.is_identifier_token literal)

(** {1 位置信息测试} *)

let test_make_position () =
  let pos =
    Token_system_unified_conversion.Legacy_type_bridge.make_position ~line:10 ~column:5 ~offset:100
  in
...
let position_tests = [ test_case "make_position" `Quick test_make_position ]
