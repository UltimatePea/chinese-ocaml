...
let test_unified_conversion_core () =
  ...
  List.iter (fun expected ->
    check bool ("转换器类型存在: " ^ (match expected with
...
let test_batch_conversion () =
  ...
  check (list (option (of_pp (fun fmt _ -> Format.fprintf fmt "token")))) 
    "空列表批量转换" [] empty_result

(** 测试转换器优先级机制 *)
let test_conversion_priority () =
  (* 验证转换器优先级顺序定义存在 *)
  let stats = Yyocamlc_lib.Token_conversion_unified.get_conversion_stats () in
  check bool "优先级信息包含在统计中" 
    (String.length stats > 30) true
...
let test_converter_customization () =
  ...
  check bool "转换器详情包含描述信息" 
    (List.exists (fun (_, desc) -> String.length desc > 0) details) true
