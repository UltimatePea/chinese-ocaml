...
module Test_PoetryFormatting = struct
  ...
  let test_data_statistics_and_reports () =
    ...
    let detailed_report =
      PoetryFormatting.format_evaluation_detailed_report "静夜思" "优秀" 9.2 "意境深远，技法纯熟"
    in
...
module Test_AncientStyleFormatting = struct
  ...
  let test_format_classical_chinese () =
    ...
    let result =
      AncientStyleFormatting.format_classical_chinese "论语·学而" "学而时习之，不亦说乎？" annotations
    in
...
module Test_PoetryAnalysisTools = struct
  ...
  let test_format_style_analysis_report () =
    ...
    let result =
      PoetryAnalysisTools.format_style_analysis_report "静夜思" style_features similarity_scores
    in
    ...
    [
      ...
      ( "诗词分析工具",
        [
          ...
          test_case "主题词汇分析" `Quick
            Test_PoetryAnalysisTools.test_format_thematic_vocabulary_analysis;
        ] );
    ]
