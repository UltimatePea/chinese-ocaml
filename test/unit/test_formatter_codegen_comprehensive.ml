...
module Test_CCodegen = struct
  ...
  let test_c_statements_and_control () =
    ...
    let template =
      CCodegen.c_template_with_includes "#include <stdio.h>" "int main() { return 0; }" "// end"
    in
  ...
  let test_function_and_struct_definitions () =
    ...
    let func_def =
      CCodegen.c_function_definition "int" "multiply" [ "int x"; "int y" ] "return x * y;"
    in
...
module Test_EnhancedCCodegen = struct
  ...
  let test_enhanced_luoyan_calls () =
    ...
    let conditional =
      EnhancedCCodegen.luoyan_conditional_binding "result" "condition" "true_expr" "false_expr"
    in
...
module Test_CodeGenUtilities = struct
  ...
  let test_code_formatting () =
    ...
    let params_with_values =
      CodeGenUtilities.c_format_parameter_list [ "int a"; "char* b"; "float c" ]
    in
  ...
  let test_code_block_management () =
    let scope_block =
      CodeGenUtilities.c_scope_block [ "int x = 0;"; "printf(\"%d\", x);"; "return x;" ]
    in
    ...
    let namespace_block =
      CodeGenUtilities.c_namespace_block "MyNamespace" "int func() { return 42; }"
    in
