...
let test_poetry_types () =
  ...
  (* 测试韵律信息结构 *)
  let rhyme_info = { rhyme_category = "平声"; rhyme_position = 4; rhyme_pattern = "AABA" } in

  check
    (testable pp_rhyme_info equal_rhyme_info)
    "韵律信息构造"
  ...
  check
    (testable pp_tone_constraint equal_tone_constraint)
    "特定平仄模式"
...
let test_meter_constraints () =
  ...
  check
    (testable pp_meter_constraint equal_meter_constraint)
    "基础韵律约束"
    {
      character_count = 5;
      syllable_pattern = Some "平仄平仄平";
...
let test_complex_poetry_structures () =
  ...
  check (testable pp_expr equal_expr) "复杂诗词结构组合"
    (PoetryAnnotatedExpr
       ( MeterValidatedExpr
           (RhymeAnnotatedExpr (LitExpr (StringLit "床前明月光疑是地上霜"), rhyme_info), meter_constraint),
         SevenCharPoetry ))
    poetry_annotated
