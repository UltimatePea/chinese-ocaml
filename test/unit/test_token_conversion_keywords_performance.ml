(** Token关键字转换性能基准测试 - Fix #1421
    ...
    验证Phase 3B重构后的性能特征，特别关注： 1. 转换器序列的线性搜索是否引起性能退化 2. 古雅体关键字转换的最坏情况性能 3. 大型文件处理的性能影响
...
let test_ancient_keyword_performance () =
  ...
  List.iter
    (fun token ->
      ...
      let expected_max_time = 0.00005 in
      (* 50µs *)
      if time_per_conversion > expected_max_time then
...
let test_large_scale_conversion_performance () =
  ...
  List.iter
    (fun size ->
      ...
      let _, time_taken =
        time_execution (fun () -> List.map TokenConversion.convert_basic_keyword_token tokens)
      in
...
let test_worst_case_performance () =
  ...
  let worst_case_tokens =
    Array.make 1000 Token_mapping.Token_definitions_unified.AncientDefineKeyword
  in
  let worst_case_list = Array.to_list worst_case_tokens in

  let _, time_taken =
    time_execution (fun () -> List.map TokenConversion.convert_basic_keyword_token worst_case_list)
  in
...
let test_memory_efficiency () =
  ...
  Printf.printf "内存使用: %d words for %d tokens (%.2f words/token)\n" memory_used
    (List.length converted_tokens) memory_per_token;
...
let test_performance_regression () =
  ...
  let baseline_basic_conversion_time = 0.00001 in
  (* 10µs *)
  let baseline_ancient_conversion_time = 0.00005 in
  (* 50µs *)
  let baseline_throughput = 100.0 in
  (* tokens/sec *)
