  ...
  let rec generate acc i =
    if i >= count then acc
      ...
      let positioned_token =
        Token_system_unified_core.Token_types.
          {
...
let test_large_scale_conversion () =
  ...
  List.iter
    (fun size ->
      ...
      let token_types, time_taken =
        time_execution (fun () -> List.map (fun pt -> Bridge.get_token_category pt.token) tokens)
      in
      ...
      (* 性能基线检查 - O(n)线性复杂度预期 *)
      let expected_max_time = 0.001 *. float_of_int size in
      (* 1ms per token baseline *)
...
let test_token_lookup_performance () =
  ...
  let is_keyword_result, time_taken =
    time_execution (fun () -> Bridge.is_keyword_token test_token.token)
  in
  ...
  let expected_max_time = 0.0001 in
  (* 0.1ms baseline *)
  if time_taken > expected_max_time then
...
let test_memory_efficiency () =
  ...
  if memory_per_token > expected_max_memory_per_token then
    Printf.printf "Warning: Memory usage %.2f words/token exceeds baseline %.2f\n" memory_per_token
      expected_max_memory_per_token

(* 边界条件性能测试 *)
let test_edge_case_performance () =
  (* 测试空Token列表 *)
  let empty_result, empty_time =
    time_execution (fun () -> List.map (fun pt -> Bridge.token_type_name pt.token) [])
  in
  ...
  in
  let single_result_name, single_time =
    time_execution (fun () -> Bridge.token_type_name single_token.token)
  in
...
let test_concurrent_safety_performance () =
  ...
  let sequential_result, sequential_time =
    time_execution (fun () -> List.map (fun pt -> Bridge.token_type_name pt.token) tokens)
  in
  ...
  (* 验证结果正确性 *)
  Alcotest.(check int)
    "sequential vs parallel count" (List.length sequential_result) (List.length parallel_result);
...
let () = run "Token System Performance Baseline Tests" [ ("Performance Tests", performance_tests) ]
