...
let test_verses_long = [ "春风又绿江南岸"; "明月何时照我还"; "千里江山如画图"; "万里长城永不倒" ]
...
let test_evaluator_registration () =
  ...
  let expected_evaluators =
    [ "韵律和谐度评价器"; "声调平衡评价器"; "对仗性评价器"; "意象评价器"; "形式美评价器"; "内容深度评价器"; "意境评价器"; "整体评价器" ]
  in
...
let test_edge_cases () =
  ...
  (try
   ...
   with e -> Printf.printf "  ⚠ 单字符异常: %s\n" (Printexc.to_string e));

  (* 测试超长文本 *)
  Printf.printf "测试超长文本处理:\n";
  let long_verses = List.init 20 (fun i -> Printf.sprintf "这是第%d句测试诗句，用于验证系统的稳定性" (i + 1)) in
  try
