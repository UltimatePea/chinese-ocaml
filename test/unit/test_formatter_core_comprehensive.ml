...
module Test_ReportFormatting = struct
  ...
  let test_extended_report_features () =
    ...
    let metrics_result =
      ReportFormatting.format_metrics_table [ ("覆盖率", "85%"); ("耗时", "120ms") ]
    in
    ...
    let comparison_result =
      ReportFormatting.format_comparison_report "版本A" "版本B" [ "性能提升10%"; "内存减少5%" ]
    in
...
module Test_StringProcessingFormatter = struct
  ...
  let test_format_message_template () =
    ...
    let multi_param =
      StringProcessingFormatter.format_message_template "名称: %s, 年龄: %s" [ "张三"; "25" ]
    in
  ...
  let test_extended_string_processing () =
    let operation_result =
      StringProcessingFormatter.format_string_operation "大写转换" "hello" "HELLO"
    in
...
module Test_ExtendedFormatting = struct
  ...
  let test_format_config_info () =
    ...
    let section_result =
      ExtendedFormatting.format_config_section "数据库" [ "host=localhost"; "port=5432" ]
    in
    ...
    [
      ...
      ( "报告格式化",
        [
          ...
          test_case "详细Token兼容性报告" `Quick
            Test_ReportFormatting.test_detailed_token_compatibility_report;
