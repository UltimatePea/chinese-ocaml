...
module Test_EnhancedPosition = struct
  ...
  (** 测试位置范围格式化 *)
  let test_span_info () =
    let same_file_same_line =
      EnhancedPosition.format_span_info ("test.ml", 10, 5) ("test.ml", 10, 15)
    in
    ...
    let same_file_diff_line =
      EnhancedPosition.format_span_info ("code.ml", 5, 10) ("code.ml", 8, 20)
    in
  ...
  let test_source_excerpt_and_multiline () =
    let excerpt =
      EnhancedPosition.format_source_excerpt "example.ml" 15 "    let result = calculate x y" 20
    in
