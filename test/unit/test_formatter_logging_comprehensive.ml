...
module Test_EnhancedLogMessages = struct
  ...
  let test_log_entry_formatting () =
    let log_entry =
      EnhancedLogMessages.format_log_entry "2025-07-29 14:30:45" "[Parser]" "\027[32m" "INFO" "解析成功"
        "\027[0m"
    in
    ...
    let simple_entry =
      EnhancedLogMessages.format_simple_log_entry "14:30:45" "[Lexer]" "" "DEBUG" "Token生成"
    in
...
module Test_LoggingFormatter = struct
  ...
  let test_json_and_debug_formatting () =
    let json_log =
      LoggingFormatter.format_json_log_entry "INFO" "2025-07-29T14:30:45Z" "Parser" "解析完成"
    in
    ...
    check bool "JSON日志格式正确" true
      (String.get json_log 0 = '{' && String.get json_log (String.length json_log - 1) = '}');

    let debug_context =
      LoggingFormatter.format_debug_context "parse_expression"
        [ ("token", "IDENTIFIER"); ("position", "line:5") ]
    in
    ...
    let error_stack =
      LoggingFormatter.format_error_stack "空指针异常" [ "main.ml:42"; "parser.ml:158"; "lexer.ml:92" ]
    in
