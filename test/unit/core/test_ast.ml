...
(* 测试主要功能 *)
let test_suite = [
  ("诗词形式测试", `Quick, test_poetry_form);
  ("韵律信息测试", `Quick, test_rhyme_info);
  ("声调类型测试", `Quick, test_tone_type);
  ("平仄模式测试", `Quick, test_tone_pattern);
  ("声调约束测试", `Quick, test_tone_constraint);
  ("基础数据类型测试", `Quick, test_basic_types);
  ("复合类型测试", `Quick, test_compound_types);
  ("基础表达式测试", `Quick, test_basic_expressions);
  ("标识符测试", `Quick, test_identifiers);
  ("运算表达式测试", `Quick, test_operations);
  ("条件表达式测试", `Quick, test_conditional);
  ("函数相关表达式测试", `Quick, test_functions);
  ("Let表达式测试", `Quick, test_let_expressions);
  ("诗词相关表达式测试", `Quick, test_poetry_expressions);
  ("语句类型测试", `Quick, test_statements);
  ("类型定义测试", `Quick, test_type_definitions);
  ("异常相关测试", `Quick, test_exceptions);
]

let () = run "AST单元测试" test_suite