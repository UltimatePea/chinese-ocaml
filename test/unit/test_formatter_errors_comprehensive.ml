(** 骆言编译器错误消息格式化模块全面测试 - Stage 2.1: Core formatter tests
    ...
    Author: Alpha, 主工作代理 Fix #1692 - 测试覆盖率提升计划第二阶段
    @since 2025-07-29 *)
...
module Test_ErrorMessages = struct
  ...
  let test_variable_errors () =
    ...
    let suggestion =
      ErrorMessages.variable_suggestion "conter" [ "counter"; "center"; "current" ]
    in
...
module Test_ErrorHandlingFormatter = struct
  ...
  let test_exception_and_debug_formatting () =
    ...
    let stack_trace =
      ErrorHandlingFormatter.format_stack_trace [ "main.ml:15"; "utils.ml:42"; "lib.ml:98" ]
    in
...
let () =
  run "骆言错误消息格式化模块全面测试"
    [
      ...
      ( "增强错误消息",
        [
          test_case "增强的变量和模块错误" `Quick
            Test_EnhancedErrorMessages.test_enhanced_variable_and_module_errors;
          test_case "Token相关增强错误" `Quick Test_EnhancedErrorMessages.test_enhanced_token_errors;
          test_case "代码生成和数据结构错误" `Quick
            Test_EnhancedErrorMessages.test_codegen_and_data_structure_errors;
          test_case "解析和类型系统错误" `Quick
            Test_EnhancedErrorMessages.test_parsing_and_type_system_errors;
      ...
      ( "错误处理格式化器",
        [
          ...
          test_case "异常和调试格式化" `Quick
            Test_ErrorHandlingFormatter.test_exception_and_debug_formatting;
        ] );
    ]
