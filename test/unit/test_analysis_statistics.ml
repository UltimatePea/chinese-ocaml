(** 分析统计模块测试
    ...
    测试覆盖analysis_statistics.ml模块的所有核心功能
    技术债务修复：提升关键编译器分析统计模块测试覆盖率 Fix #1620
...
let test_get_suggestion_statistics () =
  (* 创建各种类型的建议用于测试 *)
  let naming_suggestion1 = create_naming_suggestion "英文命名建议1" 0.85 (Some "test1.ml:10") (Some "使用中文") in
  let naming_suggestion2 = create_naming_suggestion "英文命名建议2" 0.75 (Some "test2.ml:20") (Some "使用中文") in
  let complexity_suggestion = create_complexity_suggestion 18 "函数过于复杂" 0.90 (Some "func.ml:5") (Some "简化逻辑") in
  let duplication_suggestion = create_duplication_suggestion ["func1"; "func2"] "重复代码检测" 0.70 (Some "dup.ml:15") (Some "提取公共函数") in
  let performance_suggestion = create_performance_suggestion "列表操作优化" "性能改进建议" 0.80 (Some "perf.ml:25") (Some "使用更高效算法") in

  let suggestions = [naming_suggestion1; naming_suggestion2; complexity_suggestion; duplication_suggestion; performance_suggestion] in
  let (total, (naming_count, complexity_count, duplication_count, performance_count), (high_count, medium_count, low_count)) = 
  ...
  (* 验证按优先级分类统计 (基于置信度) *)
  check int "高优先级建议统计应正确" 3 high_count; (* confidence >= 0.8: 0.85, 0.90, 0.80 = 3个 *)
  check int "中优先级建议统计应正确" 2 medium_count; (* 0.6 <= confidence < 0.8: 0.75, 0.70 *)
  check int "低优先级建议统计应正确" 0 low_count; (* confidence < 0.6: 无 *)
...
let test_confidence_boundary_conditions () =
  ...
  let boundary_suggestions = [
    exact_high_boundary; exact_medium_boundary; just_below_high; 
    just_below_medium; very_high; very_low
  ] in
  let (_, _, (high, medium, low)) = get_suggestion_statistics boundary_suggestions in

  (* 验证边界条件分类 *)
  check int "高置信度边界处理应正确" 2 high; (* 0.8, 1.0 *)
  check int "中置信度边界处理应正确" 2 medium; (* 0.79, 0.6 *)
...
let test_mixed_suggestion_types () =
  ...
  let (total, (naming, complexity, duplication, performance), (high, medium, low)) = 
    get_suggestion_statistics mixed_suggestions in
  ...
  (* 验证优先级分布 *)
  check int "混合高优先级统计" 3 high; (* 0.9, 0.85, 0.8 *)
  check int "混合中优先级统计" 3 medium; (* 0.7, 0.6, 0.75 *)
...
let test_large_suggestion_set () =
  ...
  for i = 1 to 100 do
    ...
    let suggestion = create_naming_suggestion 
      ("建议" ^ (string_of_int i))
      confidence
  ...
  let (total, (naming, complexity, duplication, performance), (high, medium, low)) = 
    get_suggestion_statistics !large_suggestions in
  ...
  check int "大量建议高优先级统计" 21 high; (* 0.80-1.00: 21个 *)
  check int "大量建议中优先级统计" 20 medium; (* 0.60-0.79: 20个 *)
...
let test_chinese_report_formatting () =
  let chinese_program = [
    LetStmt ("变量名", VarExpr "测试");
    RecLetStmt ("函数名", FunExpr (["参数"], VarExpr "参数"))
  ] in
  ...
  check bool "报告应有一致的格式结构" true 
    (contains_substring chinese_report "====" && 
     contains_substring chinese_report "个\n")
...
let test_integration_data_flow () =
  ...
  check bool "集成数据应通过analysis_engine处理" true 
    (String.length integration_report > 100); (* 确保有实际内容生成 *)
...
let () =
  run "分析统计模块测试" [
    "建议统计分析", [
      test_case "get_suggestion_statistics基础功能" `Quick test_get_suggestion_statistics;
    ];
    "快速质量检查", [
      test_case "quick_quality_check基础功能" `Quick test_quick_quality_check;
    ];
    "置信度边界条件", [
      test_case "置信度分类边界处理" `Quick test_confidence_boundary_conditions;
    ];
    "混合建议类型", [
      test_case "不同类型建议混合统计" `Quick test_mixed_suggestion_types;
    ];
    "大量数据处理", [
      test_case "大量建议性能和准确性" `Quick test_large_suggestion_set;
    ];
    "极端情况处理", [
      test_case "各种极端情况健壮性" `Quick test_extreme_cases;
    ];
    "中文报告格式", [
      test_case "中文字符和格式化支持" `Quick test_chinese_report_formatting;
    ];
    "模块集成测试", [
      test_case "与其他模块数据流集成" `Quick test_integration_data_flow;
    ];
  ]