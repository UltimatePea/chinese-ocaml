...
let create_complex_context () =
  {
    ...
    defined_vars =
      [
        ("var1", Some (BaseTypeExpr IntType));
        ("var2", None);
        ("var3", Some (BaseTypeExpr StringType));
      ];
...
let test_add_suggestions_to_ref () =
  ...
  let new_suggestions = [ create_test_suggestion "建议1" 0.8; create_test_suggestion "建议2" 0.9 ] in
  ...
  (* 测试追加功能 *)
  let additional_suggestions = [ create_test_suggestion "建议3" 0.7 ] in
...
let test_create_nested_context () =
  ...
  check int "定义变量列表长度应保持不变"
    (List.length base_context.defined_vars)
    (List.length nested_context.defined_vars);
  ...
  let rec create_deep_nested ctx depth =
    if depth <= 0 then ctx else create_deep_nested (create_nested_context ctx) (depth - 1)
  in
...
let test_analyze_variable_expression () =
  ...
  let mentions_naming =
    List.exists
      (fun msg ->
        try
          let _ = Str.search_forward (Str.regexp "命") msg 0 in
          true
        with Not_found -> false)
      suggestions_content
  in
  ...
  let has_length_concern =
    List.exists
      (fun s ->
        let msg = s.message in
        (try
           let _ = Str.search_forward (Str.regexp "长") msg 0 in
           true
         with Not_found -> false)
        ||
        try
          let _ = Str.search_forward (Str.regexp "大") msg 0 in
          true
        with Not_found -> false)
      !suggestions_ref
  in
...
let test_analyze_let_expression () =
  ...
  (* 测试基本Let表达式分析 *)
  analyze_let_expression "test_var" (VarExpr "value") (VarExpr "test_var") context analyze_mock
    suggestions_ref;
...
let test_analyze_conditional_expression () =
  ...
  analyze_conditional_expression complex_cond complex_then complex_else context analyze_mock
    suggestions_ref;
  ...
  suggestions_ref := [];
  analyze_conditional_expression cond then_expr else_expr deep_nested_context analyze_mock
    suggestions_ref;
...
let test_analyze_function_call_expression () =
  ...
  let analyze_mock _expr _ctx = incr call_count in
  ...
  check int "函数调用应分析函数和所有参数" 3 !call_count;
  ...
  check int "多参数函数调用应分析所有参数" 11 !call_count;
...
let test_analyze_match_expression () =
  ...
  check int "模式匹配应分析匹配表达式和所有分支" 3 !call_count;
    ...
    [
      {
        ...
        guard = None;
        expr = BinaryOpExpr (VarExpr "x", Add, VarExpr "y");
      };
      {
        pattern = LitPattern (StringLit "test");
        guard = None;
        expr = FunCallExpr (VarExpr "process", [ VarExpr "test" ]);
      };
      {
        pattern = VarPattern "default";
        guard = None;
        expr = CondExpr (VarExpr "check", VarExpr "yes", VarExpr "no");
      };
    ]
  in
...
let test_context_updates () =
  ...
  let analyze_mock _expr ctx = context_updates := ctx :: !context_updates in

  analyze_let_expression "new_var" (VarExpr "value") (VarExpr "new_var") base_context analyze_mock
    suggestions_ref;
...
let test_edge_cases () =
  ...
  suggestions_ref := [];
  analyze_conditional_expression (VarExpr "cond") (VarExpr "then") (VarExpr "else") deep_context
    analyze_mock suggestions_ref;
...
let test_performance_and_memory () =
  ...
  (* 测试大量建议添加的性能 *)
  let large_suggestion_list =
    List.init 10000 (fun i -> create_test_suggestion ("建议" ^ string_of_int i) (Random.float 1.0))
  in
