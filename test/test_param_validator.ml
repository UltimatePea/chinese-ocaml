  ...
  let test_extract_list () =
    ...
    List.iter
      (fun (value, expected) ->
        ...
        let matches =
          match (result, expected) with Some r, Some e -> r = e | None, None -> true | _ -> false
        in
        Printf.printf "    %s -> %s (期望: %s) %s\n" (value_to_string value)
...
module TestTypeValidators = struct
  let test_basic_validators () =
    ...
    (try
       ...
       assert false
     with
    | RuntimeError _ -> Printf.printf "    字符串验证失败: 正确抛出异常 ✓\n"
    | _ ->
    ...
    (try
       ...
       assert false
     with
    | RuntimeError _ -> Printf.printf "    整数验证失败: 正确抛出异常 ✓\n"
    | _ ->
    ...
    (try
       ...
       assert false
     with
    | RuntimeError _ -> Printf.printf "    浮点数验证失败: 正确抛出异常 ✓\n"
    | _ ->
    ...
    (try
       ...
       assert false
     with
    | RuntimeError _ -> Printf.printf "    布尔值验证失败: 正确抛出异常 ✓\n"
    | _ ->
  ...
  let test_composite_validators () =
    ...
    (* 测试数值验证器 *)
    let number_success_cases = [ (IntValue 42, "整数作为数值"); (FloatValue 3.14, "浮点数作为数值") ] in
    ...
    let number_failure_cases = [ (StringValue "42", "字符串不是数值"); (BoolValue true, "布尔值不是数值") ] in
    ...
    List.iter
      (fun (value, desc) ->
        try
          ...
          assert false
        with
        | RuntimeError _ -> Printf.printf "    %s: 正确抛出异常 ✓\n" desc
        | _ -> assert false)
      number_failure_cases;

    (* 测试字符串或列表验证器 *)
    let string_or_list_success_cases =
      [ (StringValue "hello", "字符串"); (ListValue [ IntValue 1 ], "列表") ]
    in
  ...
  let test_nonempty_list_validator () =
    ...
    let success_cases =
      [
        (ListValue [ IntValue 1 ], "单元素列表"); (ListValue [ StringValue "a"; BoolValue true ], "多元素列表");
      ]
    in
    ...
    (* 测试失败情况 *)
    let failure_cases =
      [ (ListValue [], "空列表"); (StringValue "not_list", "非列表类型"); (IntValue 42, "整数类型") ]
    in
...
module TestParameterCountValidation = struct
  ...
  let test_validate_single () =
    ...
    (try
       ...
       assert false
     with
    | RuntimeError _ -> Printf.printf "    类型错误: 正确抛出异常 ✓\n"
    | _ -> assert false);
  ...
  let test_validate_double () =
    ...
    (try
       let s, i =
         validate_double validate_string validate_int "test_function"
           [ StringValue "hello"; IntValue 42 ]
       in
  ...
  let test_validate_triple () =
    ...
    (try
       let s, i, b =
         validate_triple validate_string validate_int validate_bool "test_function"
           [ StringValue "hello"; IntValue 42; BoolValue true ]
       in
...
module TestSpecialValidators = struct
  let test_validate_non_negative () =
    ...
    (* 测试成功情况 *)
    let success_cases = [ (IntValue 0, 0); (IntValue 42, 42); (IntValue 1000, 1000) ] in
    ...
    List.iter
      (fun (value, desc) ->
        try
          ...
          assert false
        with
        | RuntimeError msg -> Printf.printf "    %s (%s): 正确抛出异常 ✓\n" (value_to_string value) desc
        | _ -> assert false)
      failure_cases;
...
module TestPerformance = struct
  ...
  let test_validation_performance () =
    ...
    let test_type_validation () =
      Array.iter
        (fun v ->
          try ignore (validate_int v)
          with _ -> (
            ();
            try ignore (validate_string v)
            with _ -> (
              ();
  ...
  let test_parameter_count_validation_performance () =
    ...
    let test_count_validation () =
      for _i = 1 to 10000 do
        try ignore (validate_single validate_string "test" [ StringValue "hello" ])
        with _ -> (
          ();
          try
            ignore
              (validate_double validate_string validate_int "test"
                 [ StringValue "hello"; IntValue 42 ])
          with _ -> (
            ();
            try ignore (validate_triple validate_string validate_int validate_bool "test" test_args)
            with _ -> ()))
      done
    in
