(** 解析器基础表达式整合模块综合测试
    ...
    本测试文件针对parser_expressions_primary_consolidated.ml模块进行全面测试。
    该模块是编译器的核心组件，负责解析所有基础表达式类型。
...
let rec simple_parse_expression state =
  ...
  | Some (IntToken i) -> 
      (LitExpr (IntLit i), advance_parser state)
  | Some (StringToken s) -> 
      (LitExpr (StringLit s), advance_parser state)
  | Some (QuotedIdentifierToken name) -> 
      (VarExpr name, advance_parser state)
...
(** 记录表达式解析器（测试用） *)
let simple_parse_record_expression state =
  (RecordExpr [], state)
...
let test_parse_primary_expr_literal _ =
  ...
  let (expr, new_state) = parse_primary_expr 
    simple_parse_expression 
    simple_parse_array_expression 
    simple_parse_record_expression 
    state in
...
let test_parse_primary_expr_boolean _ =
  ...
  let (expr_false, _) = parse_primary_expr 
    simple_parse_expression 
    simple_parse_array_expression 
    simple_parse_record_expression 
    state_false in
...
let test_parse_function_arguments_multiple _ =
  (* 测试多个参数 *)
  let tokens = [IntToken 1; CommaToken; StringToken "测试"; CommaToken; IntToken 3; RightParenToken] in
  let state = create_test_state tokens in
  let (args, new_state) = parse_function_arguments simple_parse_expression state in
  let expected = [
    LitExpr (IntLit 1);
    LitExpr (StringLit "测试");
    LitExpr (IntLit 3)
  ] in
...
let test_parse_function_arguments_boundary _ =
  ...
  assert_raises 
    (Failure "参数解析错误")
    (fun () -> parse_function_arguments simple_parse_expression state)
...
let test_parse_postfix_expr_boundary _ =
  ...
  assert_raises 
    (Failure "字段访问解析错误")
    (fun () -> parse_postfix_expr simple_parse_expression base_expr state)
...
let test_integration_complex_expression _ =
  ...
  (* 模拟复杂表达式解析流程 *)
  let (base_expr, state1) = parse_primary_expr 
    simple_parse_expression 
    simple_parse_array_expression 
    simple_parse_record_expression 
    state in
...
let suite = "Parser_expressions_primary_consolidated comprehensive tests" >::: [
  (* 基础表达式解析测试 *)
  ...
  "test_parse_function_call_or_variable_simple" >:: test_parse_function_call_or_variable_simple;
