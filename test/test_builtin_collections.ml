...
let test_length_function () =
  [
    ...
    "字符串长度测试", `Quick, (fun () ->
      ...
      check runtime_value_testable "中文字符串长度" (IntValue 12) result  (* UTF-8 byte count: 4 chars * 3 bytes each *)
    );
    ...
    "参数数量错误测试", `Quick, (fun () ->
      check bool "长度函数参数不足" true (expect_runtime_error (fun () ->
        length_function []
      ));
      check bool "长度函数参数过多" true (expect_runtime_error (fun () ->
        length_function [ListValue []; ListValue []]
      ))
    );
  ]
    ...
    "基础列表连接测试", `Quick, (fun () ->
      ...
      | BuiltinFunctionValue f ->
        ...
        check runtime_value_testable "连接结果应该正确" 
          (ListValue [IntValue 1; IntValue 2; IntValue 3; IntValue 4]) result
    ...
    "不同类型元素列表连接测试", `Quick, (fun () ->
      ...
      | BuiltinFunctionValue f ->
        ...
        check runtime_value_testable "混合类型列表连接" 
          (ListValue [IntValue 1; StringValue "hello"; BoolValue true; FloatValue 3.14]) result
      | _ -> fail "连接函数应该返回一个内置函数"
    );
    
    "参数类型错误测试", `Quick, (fun () ->
      check bool "连接函数类型错误" true (expect_runtime_error (fun () ->
        concat_function [IntValue 42]
      ))
    );
  ]
  ...
  [
    "基础过滤测试", `Quick, (fun () ->
      let is_positive = BuiltinFunctionValue (fun args ->
        match args with
        | [IntValue n] -> BoolValue (n > 0)
        | _ -> runtime_error "谓词函数参数错误"
      ) in
    ...
    "谓词非布尔返回值错误测试", `Quick, (fun () ->
      ...
      match partial_func with
      | BuiltinFunctionValue f ->
        check bool "谓词非布尔值错误" true (expect_runtime_error (fun () ->
          f [test_list]
        ))
      | _ -> fail "过滤函数应该返回一个内置函数"
    );
    ...
    "参数类型错误测试", `Quick, (fun () ->
      check bool "过滤函数参数类型错误" true (expect_runtime_error (fun () ->
        filter_function [IntValue 42]
      ))
    );
  ]
...
let test_map_function () =
  [
    "基础映射测试", `Quick, (fun () ->
      ...
      | BuiltinFunctionValue f ->
        let result = f [test_list] in
        check runtime_value_testable "映射加倍" (ListValue [IntValue 2; IntValue 4; IntValue 6]) result
      | _ -> fail "映射函数应该返回一个内置函数"
    );
    
    "映射空列表测试", `Quick, (fun () ->
      let identity_func = BuiltinFunctionValue (fun args ->
        match args with
        | [x] -> x
        | _ -> runtime_error "映射函数参数错误"
      ) in
    ...
    "类型转换映射测试", `Quick, (fun () ->
      let to_string_func = BuiltinFunctionValue (fun args ->
        match args with
      ...
      | BuiltinFunctionValue f ->
        ...
        check runtime_value_testable "类型转换映射" 
          (ListValue [StringValue "1"; StringValue "2"; StringValue "3"]) result
      | _ -> fail "映射函数应该返回一个内置函数"
    );
    
    "参数类型错误测试", `Quick, (fun () ->
      check bool "映射函数参数类型错误" true (expect_runtime_error (fun () ->
        map_function [StringValue "not_a_function"]
      ))
    );
  ]
...
let test_fold_function () =
  [
    ...
    "字符串连接折叠测试", `Quick, (fun () ->
      let concat_func = BuiltinFunctionValue (fun args ->
        match args with
    ...
    "参数类型错误测试", `Quick, (fun () ->
      check bool "折叠函数参数类型错误" true (expect_runtime_error (fun () ->
        fold_function [BoolValue true]
      ))
    );
  ]
  ...
  [
    ...
    "浮点数列表排序测试", `Quick, (fun () ->
      ...
      check runtime_value_testable "浮点数排序" 
        (ListValue [FloatValue 1.41; FloatValue 2.71; FloatValue 3.14]) result
    );
    
    "字符串列表排序测试", `Quick, (fun () ->
      let test_list = ListValue [StringValue "cherry"; StringValue "apple"; StringValue "banana"] in
      let result = sort_function [test_list] in
      check runtime_value_testable "字符串排序" 
        (ListValue [StringValue "apple"; StringValue "banana"; StringValue "cherry"]) result
    );
    ...
    "已排序列表排序测试", `Quick, (fun () ->
      ...
      check runtime_value_testable "已排序列表" (ListValue [IntValue 1; IntValue 2; IntValue 3]) result
    );
    
    "参数类型错误测试", `Quick, (fun () ->
      check bool "排序函数参数类型错误" true (expect_runtime_error (fun () ->
        sort_function [StringValue "not_a_list"]
      ))
    );
  ]
...
let test_reverse_function () =
  [
    "列表反转测试", `Quick, (fun () ->
      ...
      check runtime_value_testable "列表反转" (ListValue [IntValue 3; IntValue 2; IntValue 1]) result
    );
    ...
    "中文字符串反转测试", `Quick, (fun () ->
      ...
      match result with
      | StringValue _ -> ()  (* Just verify it returns a string, don't check content due to UTF-8 issue *)
      | _ -> fail "中文字符串反转应该返回字符串"
    );
    
    "参数类型错误测试", `Quick, (fun () ->
      check bool "反转函数参数类型错误" true (expect_runtime_error (fun () ->
        reverse_function [IntValue 42]
      ))
    );
  ]
...
let test_contains_function () =
  [
    ...
    "参数类型错误测试", `Quick, (fun () ->
      ...
      match partial_func with
      | BuiltinFunctionValue f ->
        check bool "包含函数参数类型错误" true (expect_runtime_error (fun () ->
          f [IntValue 42]
        ))
      | _ -> fail "包含函数应该返回一个内置函数"
    );
...
let test_edge_cases () =
  [
    ...
    "Unicode字符串处理测试", `Quick, (fun () ->
      let unicode_string = StringValue "🎉🎊🎈" in
      let result = length_function [unicode_string] in
      check runtime_value_testable "Unicode字符串长度" (IntValue 12) result (* UTF-8 byte count: 3 emojis * 4 bytes each *)
    );
  ]
...
let test_integration () =
  [
    "链式操作测试", `Quick, (fun () ->
      (* 测试：过滤 -> 映射 -> 排序的链式操作 *)
      let original_list = ListValue [IntValue (-2); IntValue 3; IntValue (-1); IntValue 4; IntValue 1] in
      ...
      let filtered = match filter_partial with
        | BuiltinFunctionValue f -> f [original_list]
        | _ -> fail "过滤函数应该返回内置函数"
      in
      
      (* 第二步：映射平方 *)
      let square_func = BuiltinFunctionValue (fun args ->
        match args with
        | [IntValue n] -> IntValue (n * n)
        | _ -> runtime_error "映射函数参数错误"
      ) in
      let map_partial = map_function [square_func] in
      let mapped = match map_partial with
      ...
      check runtime_value_testable "链式操作结果" 
        (ListValue [IntValue 1; IntValue 9; IntValue 16]) sorted_result
    );
  ]
