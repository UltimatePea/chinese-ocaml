...
let test_private_type_parsing () =
  ...
  try
    ...
    | TypeDefStmt (name, PrivateType _) when name = "栈" ->
      Printf.printf "✓ 私有类型定义解析成功（类型匹配）\n"
    | _ ->
      Printf.printf "✗ 私有类型定义解析失败: %s\n" (show_stmt stmt)
  with
  | e ->
    Printf.printf "✗ 私有类型定义解析异常: %s\n" (Printexc.to_string e)
...
let test_private_type_semantic_analysis () =
  ...
  try
    ...
    | Some (PrivateType_T ("栈", _)) ->
      Printf.printf "✓ 私有类型语义分析成功\n"
    | Some _ ->
      Printf.printf "✗ 私有类型语义分析失败：类型定义不正确\n"
    | None ->
      Printf.printf "✗ 私有类型语义分析失败：未找到类型定义\n"
  with
  | e ->
    Printf.printf "✗ 私有类型语义分析异常: %s\n" (Printexc.to_string e)
...
let test_private_type_access_control () =
  ...
  try
    ...
    if List.length context2.error_list > List.length context1.error_list then
      ...
      Printf.printf "✗ 私有类型访问控制可能存在问题\n"
  with
  | e ->
    Printf.printf "✗ 私有类型访问控制测试异常: %s\n" (Printexc.to_string e)
