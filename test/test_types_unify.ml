...
let test_basic_type_unification () =
  (* 测试相同基本类型的合一 *)
  check bool "相同整数类型合一应返回空替换" true 
    (SubstMap.is_empty (unify IntType_T IntType_T));
  
  check bool "相同字符串类型合一应返回空替换" true
    (SubstMap.is_empty (unify StringType_T StringType_T));
  
  check bool "相同布尔类型合一应返回空替换" true
    (SubstMap.is_empty (unify BoolType_T BoolType_T));
...
let test_type_variable_unification () =
  ...
  check bool "类型变量与不同类型变量合一应产生替换" true 
    (not (SubstMap.is_empty subst));
  
  (* 测试类型变量与相同类型变量的合一 *)
  check bool "相同类型变量合一应返回空替换" true
    (SubstMap.is_empty (unify var1 var1));
  
  (* 测试类型变量与具体类型的合一 *)
  let subst = unify (TypeVar_T "x") IntType_T in
  check bool "类型变量与具体类型合一应产生替换" true
    (SubstMap.mem "x" subst);
  check
    (testable pp_typ equal_typ)
    "类型变量应被替换为具体类型"
    IntType_T
    (SubstMap.find "x" subst)
...
let test_function_type_unification () =
  ...
  check bool "相同函数类型合一应返回空替换" true
    (SubstMap.is_empty (unify fun1 fun2));
  ...
  check bool "函数类型合一应产生参数类型替换" true
    (SubstMap.mem "a" subst);
  check
    (testable pp_typ equal_typ)
    "类型变量应被替换为正确的参数类型"
    StringType_T
    (SubstMap.find "a" subst);
...
let test_list_type_unification () =
  ...
  check bool "相同元素类型的列表合一应返回空替换" true
    (SubstMap.is_empty (unify list_int1 list_int2));
  
  (* 测试包含类型变量的列表合一 *)
  let list_var = ListType_T (TypeVar_T "t") in
  let list_string = ListType_T StringType_T in
  let subst = unify list_var list_string in
  check bool "列表类型合一应产生元素类型替换" true
    (SubstMap.mem "t" subst);
  check
    (testable pp_typ equal_typ)
    "列表元素类型变量应被正确替换"
    StringType_T
    (SubstMap.find "t" subst)
...
let test_tuple_type_unification () =
  ...
  check bool "相同元组类型合一应返回空替换" true
    (SubstMap.is_empty (unify tuple1 tuple2));
  ...
  check bool "元组类型合一应产生所有元素的替换" true
    (SubstMap.mem "x" subst && SubstMap.mem "y" subst);
...
let test_record_type_unification () =
  ...
  check bool "相同记录类型合一应返回空替换（字段顺序无关）" true
    (SubstMap.is_empty (unify record1 record2));
  ...
  check bool "记录类型合一应产生字段类型替换" true
    (SubstMap.mem "t" subst);
  check
    (testable pp_typ equal_typ)
    "记录字段类型变量应被正确替换"
