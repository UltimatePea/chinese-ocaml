...
let test_unified_loader () =
  ...
  try
    let result = Poetry_data.Unified_data_loader.load_data_unified 
      Poetry_data.Unified_data_loader.RhymeData 
      (Poetry_data.Unified_data_loader.JsonString test_json) in
    printf "✓ JSON字符串加载成功\n";
    ...
    | (word, word_class) :: _ -> 
        printf "✓ 词性数据类型正确: %s -> %s\n" word 
          (match word_class with 
           | Poetry_data.Word_class_types.Noun -> "名词"
           | _ -> "其他")
    | [] -> printf "! 人物关系名词列表为空\n";
