(** Unicode字符字节访问器模块测试 * 验证字节级别的中文字符访问逻辑正确性 * 确保各个字节位的独立访问功能 * Author: Alpha, Primary Worker Agent *
    Fix #1725 *)
...
let test_accessor_consistency () =
  ...
  verify_accessor_consistency "左引号" Unicode.Char_byte_definitions.Quote.left_quote_bytes
    (Quote.left_quote_byte1, Quote.left_quote_byte2, Quote.left_quote_byte3);

  verify_accessor_consistency "中文逗号"
    Unicode.Char_byte_definitions.ChinesePunctuation.chinese_comma_bytes
    ( ChinesePunctuation.chinese_comma_byte1,
      ChinesePunctuation.chinese_comma_byte2,
      ChinesePunctuation.chinese_comma_byte3 );
