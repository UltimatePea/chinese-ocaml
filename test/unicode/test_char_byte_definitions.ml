(** Unicode字符字节定义模块测试 * 验证中文字符字节组合定义的准确性和完整性 * 确保Unicode字符处理的正确性 * Author: Alpha, Primary Worker
    Agent * Fix #1725 *)
...
let assert_non_zero_bytes (b1, b2, b3) name =
  (* 确保非空字节组合（除特殊情况外） *)
  if (b1, b2, b3) = (0, 0, 0) then Printf.printf "⚠️  %s 字节组合为空 - 可能是字符映射缺失\n" name
  else Printf.printf "✅ %s 字节组合非空\n" name
...
let test_chinese_punctuation_bytes () =
  ...
  (* 验证配对符号不同 *)
  assert (
    ChinesePunctuation.chinese_left_paren_bytes <> ChinesePunctuation.chinese_right_paren_bytes);
...
let test_consistency () =
  ...
  let punct_bytes =
    [ ChinesePunctuation.chinese_comma_bytes; ChinesePunctuation.chinese_period_bytes ]
  in
