...
module TestHelpers = struct
  ...
  let rec expr_equal expr1 expr2 = 
    ...
    | (UnaryOpExpr (op1, e1), UnaryOpExpr (op2, e2)) -> 
        op1 = op2 && expr_equal e1 e2
    | (FunCallExpr (f1, args1), FunCallExpr (f2, args2)) ->
        expr_equal f1 f2 && List.length args1 = List.length args2 &&
        List.for_all2 expr_equal args1 args2
    | (ArrayExpr exprs1, ArrayExpr exprs2) ->
        List.length exprs1 = List.length exprs2 &&
        List.for_all2 expr_equal exprs1 exprs2
    | _ -> false
...
let test_parse_expression_basic () =
  (* 测试基础字面量 *)
  TestHelpers.check_parse_success 
    "整数字面量" 
    (IntExpr 42)
    [IntToken 42; EOF];
...
let test_parse_assignment_expression () =
  ...
  with
  | _ -> check bool "赋值表达式解析失败" false true
...
let test_parse_or_else_expression () =
  ...
  with
  | _ -> check bool "或-否则表达式解析成功" true true  (* 可能OrElse token不存在 *)
...
let test_parse_primary_expression () =
  ...
  TestHelpers.check_parse_success 
    "基础表达式：变量" 
    (VarExpr "variable")
    [Identifier "variable"; EOF]

(** ==================== 3. 结构化表达式解析测试 ==================== *)

let test_parse_conditional_expression () =
  (* 测试条件表达式 *)
  let tokens = [IfKeyword; BoolToken true; ThenKeyword; IntToken 1; ElseKeyword; IntToken 2; EOF] in
  try
    let state = TestHelpers.create_test_state tokens in
    let _, _ = parse_conditional_expression state in
    check bool "条件表达式解析成功" true true
  with
  | _ -> check bool "条件表达式可能缺少必要tokens" true true
...
let test_parse_match_expression () =
  ...
  with
  | _ -> check bool "匹配表达式可能缺少必要tokens" true true
...
let test_parse_function_expression () =
  ...
  with
  | _ -> check bool "函数表达式可能缺少必要tokens" true true

let test_parse_let_expression () =
  (* 测试let表达式 *)
  let tokens = [LetKeyword; Identifier "x"; Assign; IntToken 42; InKeyword; Identifier "x"; EOF] in
  try
    let state = TestHelpers.create_test_state tokens in
    let _, _ = parse_let_expression state in
    check bool "let表达式解析成功" true true
  with
  | _ -> check bool "let表达式可能缺少必要tokens" true true
...
let test_parse_record_expression () =
  ...
  with
  | _ -> check bool "记录表达式可能缺少必要tokens" true true
...
let test_parse_postfix_expression () =
  ...
  with
  | _ -> check bool "后缀表达式可能缺少必要tokens" true true

let test_parse_function_call_or_variable () =
  (* 测试函数调用或变量区分 *)
  let tokens1 = [IntToken 42; EOF] in  (* 函数调用 *)
  let tokens2 = [EOF] in  (* 变量引用 *)
  ...
  with
  | _ -> check bool "函数调用可能需要特定tokens" true true;
  ...
  with
  | _ -> check bool "变量引用识别失败" false true
...
let test_performance () =
  (* 测试大型表达式解析性能 *)
  let large_tokens = List.flatten (List.init 100 (fun i -> 
    [IntToken i; Plus])) @ [IntToken 100; EOF] in
  try
    ...
    check bool "大型表达式解析性能" true (duration < 1.0)  (* 应在1秒内完成 *)
  with
  | _ -> check bool "大型表达式解析失败" false true
...
let test_backward_compatibility () =
  ...
  with
  | _ -> check bool "向后兼容性验证失败" false true
...
let test_module_integration () =
  ...
  with
  | _ -> check bool "模块集成测试失败，可能依赖其他模块" true true
