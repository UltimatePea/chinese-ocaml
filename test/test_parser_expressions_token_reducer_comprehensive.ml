(** 解析器表达式Token重复消除模块全面测试套件
    测试覆盖parser_expressions_token_reducer.ml模块的所有核心功能 *)
...
module TestHelpers = struct
  ...
  let create_basic_token_list () = [
    LetKeyword; RecKeyword; IfKeyword; ThenKeyword; ElseKeyword;
    Plus; Minus; Equal; Less; Greater;
    LeftParen; RightParen; LeftBracket; RightBracket;
    IntToken 42; StringToken "测试"; BoolToken true
  ]

  (** 创建包含重复token的列表 *)
  let create_duplicate_token_list () = [
    LetKeyword; LetKeyword; RecKeyword; IfKeyword; IfKeyword; IfKeyword;
    Plus; Plus; Minus; Equal; Equal;
    LeftParen; LeftParen; RightParen; LeftBracket;
    IntToken 1; IntToken 2; StringToken "a"; StringToken "b"; BoolToken true; BoolToken false
  ]

  (** 创建文言文风格token列表 *)
  let create_wenyan_token_list () = [
    HaveKeyword; OneKeyword; NameKeyword; SetKeyword; AlsoKeyword;
    ValueKeyword; AsForKeyword; NumberKeyword
  ]

  (** 创建古雅体token列表 *)
  let create_ancient_token_list () = [
    AncientDefineKeyword; AncientEndKeyword; AncientObserveKeyword;
    AncientIfKeyword; AncientThenKeyword; AncientListStartKeyword
  ]
  ...
  let create_natural_language_token_list () = [
    ...
    IsKeyword; EqualToKeyword; EmptyKeyword; InputKeyword; OutputKeyword
  ]

  (** 创建诗词关键字列表 *)
  let create_poetry_token_list () = [
    PoetryKeyword; FiveCharKeyword; SevenCharKeyword; ParallelStructKeyword;
    RhymeKeyword; ToneKeyword
  ]

  (** 创建操作符token列表 *)
  let create_operator_token_list () = [
    Plus; Minus; Multiply; Star; Divide; Slash; Modulo;
    Equal; NotEqual; Less; LessEqual; Greater; GreaterEqual;
    AndKeyword; OrKeyword; NotKeyword;
    Assign; RefAssign; AssignArrow;
    Arrow; DoubleArrow; Dot; DoubleDot; Bang
  ]

  (** 创建分隔符token列表 *)
  let create_delimiter_token_list () = [
    LeftParen; RightParen; ChineseLeftParen; ChineseRightParen;
    LeftBracket; RightBracket; ChineseLeftBracket; ChineseRightBracket;
    LeftBrace; RightBrace; LeftQuote; RightQuote;
    Comma; Semicolon; Colon; ChineseComma; ChineseSemicolon
  ]

  (** 创建字面量token列表 *)
  let create_literal_token_list () = [
    IntToken 123; ChineseNumberToken "三"; FloatToken 3.14;
    StringToken "测试字符串"; BoolToken true; BoolToken false;
    QuotedIdentifierToken "引用标识符"; IdentifierTokenSpecial "特殊标识符"
  ]

  (** 创建混合大型token列表 *)
  let create_large_mixed_token_list () =
    create_basic_token_list () @ create_wenyan_token_list () @
    create_ancient_token_list () @ create_type_token_list () @
    create_natural_language_token_list () @ create_poetry_token_list () @
    create_operator_token_list () @ create_delimiter_token_list () @
    create_literal_token_list ()

  (** 验证关键字分组结果 *)
  let assert_keyword_group_matches actual_group =
    match actual_group with
    | Some (BasicKeywords _) | Some (WenyanKeywords _) 
    | Some (AncientKeywords _) | Some (TypeKeywords _)
    | Some (NaturalLanguageKeywords _) | Some (PoetryKeywords _) -> true
    | None -> false

  (** 验证操作符分组结果 *)
  let assert_operator_group_matches actual_group =
    match actual_group with
    | Some (ArithmeticOps _) | Some (ComparisonOps _)
    | Some (LogicalOps _) | Some (AssignmentOps _)
    | Some (SpecialOps _) -> true
    | None -> false

  (** 验证分隔符分组结果 *)
  let assert_delimiter_group_matches actual_group =
    match actual_group with
    | Some (ParenthesesGroup _) | Some (BracketGroup _)
    | Some (BraceGroup _) | Some (ChineseDelimiters _)
    | Some (PunctuationGroup _) -> true
    | None -> false

  (** 验证字面量分组结果 *)
  let assert_literal_group_matches actual_group =
    match actual_group with
    | Some (NumericLiterals _) | Some (StringLiterals _)
    | Some (BooleanLiterals _) | Some (SpecialLiterals _) -> true
    | None -> false
...
let test_unified_token_processor_default () =
  ...
  check bool "默认处理器应该能够处理基础token列表" true
    (try process_token_list processor tokens; true with _ -> false)
...
let test_token_deduplication_basic_analysis () =
  ...
  check bool "分组后token数量应该小于等于原始数量" true
    (stats.grouped_token_count <= stats.original_token_count);
...
let test_token_deduplication_duplicate_analysis () =
  ...
  check bool "重复token列表应该有显著的减少率" true
    (stats.reduction_percentage > 10.0);
  check bool "分组数量应该小于原始数量" true
    (stats.grouped_token_count < stats.original_token_count)
...
let test_token_deduplication_report_generation () =
  ...
  check bool "报告应该包含Token重复消除报告标题" true
    (String.length report > 10);
  check bool "报告应该包含原始Token数量" true
    (stats.original_token_count > 0);
  check bool "报告应该包含状态信息" true
    (String.length report > 50)
...
let test_token_deduplication_large_dataset () =
  ...
  check bool "大型数据集应该能够正确分析" true
    (stats.original_token_count = List.length large_tokens);
  ...
  check bool "大型数据集分析应该在合理时间内完成" true
...
let test_parser_expression_processor_creation () =
  ...
  check bool "表达式处理器应该能够处理基础token" true
    (try process_token processor LetKeyword; true with _ -> false)

let test_parser_expression_processor_keyword_handling () =
  let processor = create_expression_processor () in
  let test_tokens = [
    LetKeyword; HaveKeyword; AncientDefineKeyword; 
    IntTypeKeyword; DefineKeyword; PoetryKeyword
  ] in
  
  (* 测试各种关键字都能被正确处理 *)
  List.iter (fun token ->
    check bool "应该能够处理各种关键字" true
      (try process_token processor token; true with _ -> false)
  ) test_tokens
...
let test_parser_expression_processor_comprehensive_handling () =
  ...
  check bool "表达式处理器应该能够处理综合token列表" true
    (try process_token_list processor comprehensive_tokens; true with _ -> false)
...
let test_integration_performance_stress () =
  (* 创建大型token列表进行压力测试 *)
  let stress_tokens = 
    let rec repeat_list lst n acc =
      if n <= 0 then acc
      else repeat_list lst (n - 1) (lst @ acc)
    in
