...
let test_expression_error_recovery () =
  (* 测试深度嵌套表达式错误 *)
  let deep_expr = String.concat "" (Array.to_list (Array.make 50 "（")) ^ "一" ^ String.concat "" (Array.to_list (Array.make 49 "）")) in
...
let test_natural_language_function_errors () =
  (* 测试自然语言函数定义错误 *)
  check_parse_failure "Missing natural function name" 
    "定义函数 接收 「参数」 返回 「值」";
    
  check_parse_failure "Natural function missing parameters"
    "定义函数「计算」返回「值」";
    
  check_parse_failure "Natural function missing body"
    "定义函数「计算」接收「参数」";
    
  check_parse_failure "Malformed natural function"
    "定义函数「错误」接收但是缺少参数"

let test_natural_language_parsing_edge_cases () =
  (* 测试自然语言解析边界情况 *)
  check_parse_failure "Natural language with invalid characters"
    "定义函数「test@invalid」接收「参数」返回「值」";
    
  check_parse_failure "Natural function with empty name"
    "定义函数「」接收「参数」返回「值」";
    
  check_parse_failure "Natural function with special chars"
    "定义函数「函数%$#」接收「参数」返回「值」"
...
let test_pattern_matching_edge_cases () =
  (* 测试模式匹配边界情况 *)
  check_parse_failure "Nested pattern matching error" 
    "匹配 「值」 情况 「模式1」 指向 匹配 情况";
    
  check_parse_failure "Pattern with guard error"
    "匹配 「值」 情况 「模式」 当且仅当 指向 「结果」";
    
  check_parse_failure "Multiple pattern error"
    "匹配 「值」 情况 「模式1」 情况 「模式2」 指向"

(** ========== 6. 复杂语法结构错误测试 ========== *)
let test_complex_syntax_errors () =
  (* 测试复杂嵌套结构错误 *)
  check_parse_failure "Complex nesting error"
    "函数 「外层」 参数 「x」 为 函数 「内层」 参数";
    
  check_parse_failure "Mixed syntax error"
    "让 函数 「test」 为 如果 匹配";
    
  check_parse_failure "Incomplete complex structure"
    "如果 「条件」 那么 函数 「内部」"
...
let test_performance_error_cases () =
  (* 测试深度递归错误 *)
  let deep_recursion = "让 " ^ String.concat " 让 " (Array.to_list (Array.make 1000 "「var」 为 「val」")) in
...
let test_parser_regression_cases () =
  ...
  List.iter (fun (desc, input) ->
    ...
    with _ ->
      check bool ("Regression error: " ^ desc) true true
  ) regression_cases
...
let () =
  run "骆言语法分析器错误恢复全面测试"
    [
      ...
      ("错误恢复策略测试", [
        test_case "错误恢复策略" `Quick test_error_recovery_strategies;
      ]);
      ("性能错误测试", [
        test_case "性能相关错误" `Quick test_performance_error_cases;
      ]);
      ("回归测试", [
        test_case "解析器回归测试" `Quick test_parser_regression_cases;
      ]);
    ]