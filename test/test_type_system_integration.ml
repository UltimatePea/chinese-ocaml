...
module TestUtils = struct
  ...
  (** 检查统一令牌是否相等 - 基于结构比较 *)
  let check_unified_token_equal desc expected actual =
  ...
  (** 创建带位置的令牌 *)
  let _create_positioned_token token line column =
    (token, create_test_position line column)
end
...
module TypeTokenCorrespondenceTests = struct
  ...
  let test_compound_type_tokens () =
    ...
    let fun_tokens = [
      ...
      LiteralToken (IntToken 0);  (* 代表整数类型 *)
      OperatorToken Operators.Arrow;
      LiteralToken (StringToken "");  (* 代表字符串类型 *)
    ...
    (* 验证相关令牌存在 *)
    List.iter (fun token ->
      TestUtils.check_token_equal "函数类型相关令牌存在" token token
    ) fun_tokens;
    
    List.iter (fun token ->
      TestUtils.check_token_equal "列表类型相关令牌存在" token token
    ) list_tokens;
    
    List.iter (fun token ->
      TestUtils.check_token_equal "元组类型相关令牌存在" token token
    ) tuple_tokens
  ...
  let test_type_annotation_tokens () =
    ...
    let annotation_tokens = [
      KeywordToken Keywords.LetKeyword;  (* 让 *)
      IdentifierToken (Identifiers.QuotedIdentifierToken "「x」");  (* 「x」 *)
      DelimiterToken Delimiters.Colon;  (* : *)
      (* 这里应该有整数类型令牌，但我们用字面量代表 *)
      LiteralToken (Literals.IntToken 42);  (* 整数类型表示 *)
      OperatorToken Operators.Equal;  (* = *)
      LiteralToken (Literals.IntToken 42);  (* 42 *)
    ] in
    
    (* 验证所有令牌都存在且正确 *)
    List.iter (fun token ->
      TestUtils.check_token_equal "类型注解令牌存在" token token
    ) annotation_tokens;
...
module LexicalTypeIntegrationTests = struct
  ...
  let test_token_to_type_mapping () =
    ...
    List.iter (fun (token, expected_type, desc) ->
      (* 验证令牌存在 *)
  ...
  let test_type_variables_and_identifiers () =
    ...
    (* 创建对应的标识符令牌 *)
    let identifier_token = IdentifierToken (Identifiers.QuotedIdentifierToken ("「" ^ var_name ^ "」")) in
  ...
  let test_complex_expression_tokens () =
    ...
    let expression_tokens = [
      KeywordToken Keywords.LetKeyword;  (* 让 *)
      IdentifierToken (Identifiers.QuotedIdentifierToken "「f」");  (* 「f」 *)
      OperatorToken Operators.Equal;  (* = *)
      KeywordToken Keywords.FunKeyword;  (* 函数 *)
      IdentifierToken (Identifiers.QuotedIdentifierToken "「x」");  (* 「x」 *)
      OperatorToken Operators.Arrow;  (* -> *)
      IdentifierToken (Identifiers.QuotedIdentifierToken "「x」");  (* 「x」 *)
      OperatorToken Operators.Plus;  (* + *)
      LiteralToken (Literals.IntToken 1);  (* 1 *)
    ] in
    ...
    (* 验证所有令牌存在 *)
    List.iter (fun token ->
      TestUtils.check_token_equal "复杂表达式令牌存在" token token
    ) expression_tokens;
...
module ErrorHandlingIntegrationTests = struct
  ...
  let test_type_error_tokens () =
    (* 创建错误位置 *)
    let error_pos = { Token_types_core.filename = "error_test.ly"; line = 10; column = 5; offset = 150 } in
  ...
  let test_undefined_type_error () =
    ...
    (* 创建对应的错误令牌 *)
    let error_pos = { Token_types_core.filename = "undefined.ly"; line = 1; column = 1; offset = 0 } in
...
module PerformanceBoundaryTests = struct
  ...
  let test_bulk_type_creation () =
    ...
    for i = 0 to 999 do
      for j = i + 1 to 999 do
        check bool (Printf.sprintf "类型变量%d与%d不同" i j) false 
  ...
  let test_complex_type_nesting () =
    ...
    let deeply_nested = 
      let rec nest_list depth typ =
        if depth <= 0 then typ
        else nest_list (depth - 1) (ListType_T typ)
      in
  ...
  let test_bulk_token_processing () =
    ...
    (* 验证不同值的令牌不相等 *)
    for i = 0 to 998 do
      check bool (Printf.sprintf "令牌%d与%d不同" i (i+1)) false 
...
module RealWorldScenarioTests = struct
  ...
  let test_simple_variable_definition () =
    ...
    (* 验证令牌序列 *)
    List.iter (fun token ->
      TestUtils.check_token_equal "变量定义令牌存在" token token
    ) tokens;
  ...
  let test_function_definition () =
    ...
    (* 验证令牌序列 *)
    List.iter (fun token ->
      TestUtils.check_token_equal "函数定义令牌存在" token token
    ) tokens;
  ...
  let test_conditional_expression () =
    ...
    let result_type = branch_type in  (* 两个分支都是整数 *)
    
    (* 验证令牌序列 *)
    List.iter (fun token ->
      TestUtils.check_token_equal "条件表达式令牌存在" token token
    ) tokens;
  ...
  let test_type_error_scenario () =
    ...
    (* 验证正常令牌 *)
    List.iter (fun token ->
      TestUtils.check_token_equal "类型错误场景令牌存在" token token
    ) tokens;
    ...
    (* 创建类型错误令牌 *)
    let error_pos = { Token_types_core.filename = "type_error.ly"; line = 1; column = 15; offset = 15 } in
...
let () =
  run "骆言类型系统集成测试" [
    ...
    "词法分析与类型推导集成测试", [
      ...
      test_case "类型变量与标识符关系" `Quick LexicalTypeIntegrationTests.test_type_variables_and_identifiers;
