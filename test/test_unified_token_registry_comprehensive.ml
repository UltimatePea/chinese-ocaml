...
let test_empty_string_handling () =
  ...
  | Some found_entry ->
      check string "空字符串映射源字符串" "" found_entry.source
  | None ->
      (* 空字符串可能被拒绝注册，这是可以接受的 *)
  ...
  ignore empty_query; (* 不管结果如何都是可以接受的 *)
  ()
...
let test_large_scale_performance () =
  ...
  (* 注册大量映射 *)
  for i = 1 to 1000 do
    let entry = create_test_entry ("测试" ^ string_of_int i) Unified_token_core.Identifier 1 "性能测试" true in
    TokenRegistry.register_mapping entry;
  done;
...
let category_tests = [
  test_case "分类查找测试" `Quick test_category_based_lookup;
]
...
let performance_tests = [
  test_case "大规模性能测试" `Quick test_large_scale_performance;
]

(** 主测试运行器 *)
let () = run "Unified_token_registry 综合测试" [
