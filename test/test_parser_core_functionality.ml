...
module BasicExpressionTests = struct
  let test_parse_integer_literals () =
    let test_cases = [
      ([IntToken 0, create_test_pos 1 1; EOF, create_test_pos 1 2], LitExpr (IntLit 0), "零值整数");
      ([IntToken 42, create_test_pos 1 1; EOF, create_test_pos 1 3], LitExpr (IntLit 42), "正整数");
      ([IntToken (-10), create_test_pos 1 1; EOF, create_test_pos 1 4], LitExpr (IntLit (-10)), "负整数");
      ([IntToken 999999, create_test_pos 1 1; EOF, create_test_pos 1 7], LitExpr (IntLit 999999), "大整数");
    ] in
  ...
  let test_parse_string_literals () =
    let test_cases = [
      ([StringToken "", create_test_pos 1 1; EOF, create_test_pos 1 3], LitExpr (StringLit ""), "空字符串");
      ([StringToken "hello", create_test_pos 1 1; EOF, create_test_pos 1 8], LitExpr (StringLit "hello"), "简单字符串");
      ([StringToken "你好世界", create_test_pos 1 1; EOF, create_test_pos 1 6], LitExpr (StringLit "你好世界"), "中文字符串");
      ([StringToken "hello\nworld", create_test_pos 1 1; EOF, create_test_pos 2 6], LitExpr (StringLit "hello\nworld"), "多行字符串");
      ([StringToken "\"quoted\"", create_test_pos 1 1; EOF, create_test_pos 1 10], LitExpr (StringLit "\"quoted\""), "包含引号的字符串");
    ] in
    List.iter (fun (tokens, expected, desc) ->
      let state = create_parser_state tokens in
      let expr, _ = parse_expression state in
  ...
  let test_parse_boolean_literals () =
    let test_cases = [
      ([BoolToken true, create_test_pos 1 1; EOF, create_test_pos 1 5], LitExpr (BoolLit true), "true字面量");
      ([BoolToken false, create_test_pos 1 1; EOF, create_test_pos 1 6], LitExpr (BoolLit false), "false字面量");
    ] in
    List.iter (fun (tokens, expected, desc) ->
      let state = create_parser_state tokens in
      let expr, _ = parse_expression state in
  ...
  let test_parse_variable_expressions () =
    let test_cases = [
      ([QuotedIdentifierToken "x", create_test_pos 1 1; EOF, create_test_pos 1 4], VarExpr "x", "简单变量");
      ([QuotedIdentifierToken "变量名", create_test_pos 1 1; EOF, create_test_pos 1 6], VarExpr "变量名", "中文变量名");
      ([QuotedIdentifierToken "variable_name", create_test_pos 1 1; EOF, create_test_pos 1 14], VarExpr "variable_name", "下划线变量名");
      ([QuotedIdentifierToken "用户年龄", create_test_pos 1 1; EOF, create_test_pos 1 7], VarExpr "用户年龄", "描述性中文变量名");
    ] in
    List.iter (fun (tokens, expected, desc) ->
      let state = create_parser_state tokens in
      let expr, _ = parse_expression state in
...
module ArithmeticExpressionTests = struct
  let test_parse_binary_operations () =
    let test_cases = [
      (* 加法 *)
      ([IntToken 1, create_test_pos 1 1; Plus, create_test_pos 1 3; IntToken 2, create_test_pos 1 5; EOF, create_test_pos 1 6],
       BinaryOpExpr (LitExpr (IntLit 1), Add, LitExpr (IntLit 2)), "加法运算");
      
      (* 减法 *)
      ([IntToken 5, create_test_pos 1 1; Minus, create_test_pos 1 3; IntToken 3, create_test_pos 1 5; EOF, create_test_pos 1 6],
       BinaryOpExpr (LitExpr (IntLit 5), Sub, LitExpr (IntLit 3)), "减法运算");
      
      (* 乘法 *)
      ([IntToken 3, create_test_pos 1 1; Multiply, create_test_pos 1 3; IntToken 4, create_test_pos 1 5; EOF, create_test_pos 1 6],
       BinaryOpExpr (LitExpr (IntLit 3), Mul, LitExpr (IntLit 4)), "乘法运算");
      
      (* 除法 *)
      ([IntToken 8, create_test_pos 1 1; Divide, create_test_pos 1 3; IntToken 2, create_test_pos 1 5; EOF, create_test_pos 1 6],
       BinaryOpExpr (LitExpr (IntLit 8), Div, LitExpr (IntLit 2)), "除法运算");
    ] in
    List.iter (fun (tokens, expected, desc) ->
      let state = create_parser_state tokens in
      let expr, _ = parse_expression state in
  ...
  let test_parse_comparison_operations () =
    let test_cases = [
      ([IntToken 1, create_test_pos 1 1; Less, create_test_pos 1 3; IntToken 2, create_test_pos 1 5; EOF, create_test_pos 1 6],
       BinaryOpExpr (LitExpr (IntLit 1), Lt, LitExpr (IntLit 2)), "小于比较");
      
      ([IntToken 2, create_test_pos 1 1; Greater, create_test_pos 1 3; IntToken 1, create_test_pos 1 5; EOF, create_test_pos 1 6],
       BinaryOpExpr (LitExpr (IntLit 2), Gt, LitExpr (IntLit 1)), "大于比较");
      
      ([IntToken 1, create_test_pos 1 1; Equal, create_test_pos 1 3; IntToken 1, create_test_pos 1 5; EOF, create_test_pos 1 6],
       BinaryOpExpr (LitExpr (IntLit 1), Eq, LitExpr (IntLit 1)), "等于比较");
    ] in
    List.iter (fun (tokens, expected, desc) ->
      let state = create_parser_state tokens in
      let expr, _ = parse_expression state in
...
module FunctionCallTests = struct
  ...
  let test_parse_nested_function_calls () =
    ...
    let expected = FunCallExpr (
      VarExpr "outer",
      [FunCallExpr (VarExpr "inner", [LitExpr (IntLit 42)])]
    ) in
    let state = create_parser_state tokens in
    let expr, _ = parse_expression state in
...
module StatementTests = struct
  let test_parse_let_statements () =
    let test_cases = [
      (* 简单let语句 *)
      ([LetKeyword, create_test_pos 1 1; QuotedIdentifierToken "x", create_test_pos 1 5; AsForKeyword, create_test_pos 1 7; IntToken 42, create_test_pos 1 9; EOF, create_test_pos 1 11],
       LetStmt ("x", LitExpr (IntLit 42)), "简单let语句");
      
      (* let语句带表达式 *)
      ([LetKeyword, create_test_pos 1 1; QuotedIdentifierToken "sum", create_test_pos 1 5; AsForKeyword, create_test_pos 1 9; IntToken 1, create_test_pos 1 11; Plus, create_test_pos 1 13; IntToken 2, create_test_pos 1 15; EOF, create_test_pos 1 16],
       LetStmt ("sum", BinaryOpExpr (LitExpr (IntLit 1), Add, LitExpr (IntLit 2))), "let语句带运算");
    ] in
    List.iter (fun (tokens, expected, desc) ->
      let state = create_parser_state tokens in
