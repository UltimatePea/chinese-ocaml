...
let test_variable_operations () =
  (* let语句测试 *)
  let let_program = [ LetStmt ("x", LitExpr (IntLit 42)); ExprStmt (VarExpr "x") ] in
...
let test_complex_expressions () =
  ...
  let complex_program =
    [
      ...
      ExprStmt
        (BinaryOpExpr (BinaryOpExpr (VarExpr "a", Add, VarExpr "b"), Mul, LitExpr (IntLit 2)));
    ]
  in
...
let test_nested_scopes () =
  ...
  let nested_program =
    [
      LetStmt ("x", LitExpr (IntLit 10)); ExprStmt (LetExpr ("x", LitExpr (IntLit 20), VarExpr "x"));
    ]
  in
