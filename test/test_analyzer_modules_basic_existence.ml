...
let test_performance_modules_loaded () =
  ...
  let test_function_exists f = 
    try let _ = f in true with _ -> false
  in
  
  let performance_base_exists = test_function_exists Performance_analyzer_base.make_performance_suggestion in
  let performance_lists_exists = test_function_exists Performance_analyzer_lists.analyze_list_performance in
  let performance_matching_exists = test_function_exists Performance_analyzer_matching.analyze_match_performance in
  let performance_recursion_exists = test_function_exists Performance_analyzer_recursion.analyze_recursion_performance in
  let performance_complexity_exists = test_function_exists Performance_analyzer_complexity.analyze_computational_complexity in
  let performance_data_exists = test_function_exists Performance_analyzer_data_structures.analyze_data_structure_efficiency in
...
let test_refactoring_modules_loaded () =
  ...
  let refactoring_types_context_exists = test_function_exists Refactoring_analyzer_types.empty_context in
...
let () =
  run "分析器模块基础存在性测试"
    [
      ...
      ( "基础功能测试",
        [
          test_case "分析器类型创建" `Quick test_analyzer_types_creation;
        ] );
    ]