...
let test_basic_program_execution () =
  ...
  | Error msg -> check bool ("程序执行失败: " ^ msg) false true;
...
let test_arithmetic_programs () =
  ...
  | Error msg -> check bool ("加法程序执行失败: " ^ msg) false true;
  
  (* 减法程序 *)
  let sub_program = [ExprStmt (BinaryOpExpr (LitExpr (IntLit 100), Sub, LitExpr (IntLit 58)))] in
...
let test_variable_definition_programs () =
  ...
  | Error msg -> check bool ("Let定义程序执行失败: " ^ msg) false true;
  
  (* 中文变量名程序 *)
  let chinese_var_program = [
    LetStmt ("数值", LitExpr (IntLit 42));
    ExprStmt (VarExpr "数值")
  ] in
...
let test_conditional_programs () =
  ...
  let true_branch_program = [
    ExprStmt (CondExpr (LitExpr (BoolLit true), 
                       LitExpr (StringLit "真"), 
                       LitExpr (StringLit "假")))
  ] in
  ...
  | Error msg -> check bool ("真分支程序执行失败: " ^ msg) false true;
  
  (* 假分支程序 *)
  let false_branch_program = [
    ExprStmt (CondExpr (LitExpr (BoolLit false), 
                       LitExpr (StringLit "真"), 
                       LitExpr (StringLit "假")))
  ] in
...
let test_data_structure_programs () =
  ...
  | Ok result -> check bool "元组程序" true 
    (TestUtils.values_equal result (TupleValue [IntValue 1; IntValue 2]))
  | Error msg -> check bool ("元组程序执行失败: " ^ msg) false true;
  
  (* 列表程序 *)
  let list_program = [ExprStmt (ListExpr [LitExpr (IntLit 10); LitExpr (IntLit 20)])] in
  match execute_program list_program with
  | Ok result -> check bool "列表程序" true 
...
let () = run "骆言解释器核心基础测试" [
  "解释器核心基础功能", interpreter_core_tests
]