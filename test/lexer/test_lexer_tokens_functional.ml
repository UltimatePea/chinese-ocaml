...
let test_token_equality_functional () =
  ...
  in
  List.iteri
    (fun i (t1, t2) -> check bool ("相同token对 " ^ string_of_int i ^ " 相等") true (equal_token t1 t2))
    same_pairs;
...
let test_literal_tokens () =
  ...
  let tokens =
    [ int_token; float_token; string_token; chinese_num_token; bool_true_token; bool_false_token ]
  in
  List.iter
    (fun token ->
...
let () = run "Lexer_tokens模块功能测试" [ ("Lexer Tokens Functional", lexer_tokens_functional_tests) ]
