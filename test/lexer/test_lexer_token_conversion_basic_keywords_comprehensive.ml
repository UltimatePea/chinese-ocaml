...
let get_all_basic_keyword_mappings () =
  [
    ...
    ( Token_mapping.Token_definitions_unified.WithDefaultKeyword,
      Lexer_tokens.WithDefaultKeyword,
      "withdefault关键字" );
    ( Token_mapping.Token_definitions_unified.ExceptionKeyword,
      Lexer_tokens.ExceptionKeyword,
      "exception关键字" );
    ...
    ( Token_mapping.Token_definitions_unified.ExternalKeyword,
      Lexer_tokens.ExternalKeyword,
      "external关键字" );
    ( Token_mapping.Token_definitions_unified.MutableKeyword,
      Lexer_tokens.MutableKeyword,
      "mutable关键字" );
    ( Token_mapping.Token_definitions_unified.PrivateKeyword,
      Lexer_tokens.PrivateKeyword,
      "private关键字" );
    ( Token_mapping.Token_definitions_unified.VirtualKeyword,
      Lexer_tokens.VirtualKeyword,
      "virtual关键字" );
    ( Token_mapping.Token_definitions_unified.ConstraintKeyword,
      Lexer_tokens.ConstraintKeyword,
      "constraint关键字" );
    ...
    ( Token_mapping.Token_definitions_unified.InheritKeyword,
      Lexer_tokens.InheritKeyword,
      "inherit关键字" );
    ( Token_mapping.Token_definitions_unified.InitializerKeyword,
      Lexer_tokens.InitializerKeyword,
      "initializer关键字" );
...
let test_all_basic_keyword_conversions () =
  ...
  List.iter
    (fun (input_token, expected_output, description) ->
...
let test_conversion_performance () =
  ...
  for _ = 1 to 1000 do
    ...
    if result <> expected_result then fail "性能测试中发现转换错误"
  done;
...
let test_comprehensive_coverage () =
  ...
  (* 验证每个关键字转换都不会抛出异常 *)
  List.iter
    (fun (input_token, _expected, _desc) ->
      try
        ignore (convert_basic_keyword_token input_token);
        ()
      with _ -> fail "关键字转换不应抛出异常")
    all_mappings;
  ()
...
let semantic_tests = [ test_case "语义关键字测试" `Quick test_semantic_keywords ]

let error_handling_tests = [ test_case "错误处理关键字测试" `Quick test_error_handling_keywords ]
...
let object_oriented_tests = [ test_case "面向对象关键字测试" `Quick test_object_oriented_keywords ]
...
(** 主测试运行器 *)
let () =
  run "Lexer_token_conversion_basic_keywords 综合测试"
    [
