(** 骆言词法分析器边界条件全面测试 - Fix #1030 Phase 2
    ...
    测试重点： 1. UTF-8边界条件和恶意输入 2. 字符串字面量边界情况 3. 中文数字处理边界 4. 关键字冲突和优先级 5. 状态管理和位置追踪
...
let test_buffer_boundary_conditions () =
  ...
  let long_chinese_identifier =
    "「" ^ String.concat "" (Array.to_list (Array.make 1000 chinese_char)) ^ "」"
  in
  try
  ...
  with _ -> check bool "Long Chinese identifier error handled" true true
...
let test_string_literal_edge_cases () =
  ...
  with _ -> (
    ...
    try
      ...
      | StringToken s :: _ -> check bool "Escape sequences processed" true (String.contains s '\\')
      | _ -> check bool "String with escapes" true true
    with _ -> check bool "Escape sequence error handling" true true)
...
let test_string_boundary_conditions () =
  ...
  with _ -> (
    ...
    with _ -> (
      ...
      with _ -> check bool "Multiline string error handling" true true))
...
let test_chinese_decimal_edge_cases () =
  ...
  with _ -> (
    ...
    with _ -> check bool "Invalid decimal error handling" true true)

(** ========== 4. 关键字冲突和优先级测试 ========== *)
let test_keyword_prefix_conflicts () =
  (* 测试关键字前缀冲突 *)
  let prefix_conflicts = [ ("设", "设置"); ("函", "函数"); ("如", "如果") ] in
  List.iter
    (fun (prefix, full) ->
      try
        ...
        check bool
          ("Keyword prefix conflict: " ^ prefix ^ " vs " ^ full)
          true
          (List.length tokens >= 1)
      with _ -> check bool ("Keyword prefix error: " ^ prefix ^ " vs " ^ full) true true)
    prefix_conflicts
...
let test_keyword_context_sensitivity () =
  ...
  List.iter
    (fun input ->
      try
        let tokens = tokenize input "<test>" in
        check bool ("Context-sensitive keywords: " ^ input) true (List.length tokens >= 3)
      with _ -> check bool ("Context keyword error: " ^ input) true true)
    context_tests
...
let test_position_tracking_complex () =
  ...
  try
    ...
    List.iteri
      (fun i (_, pos) ->
        match i with 0 -> check int "First token line" 1 pos.line | _ -> () (* 其他token的位置测试 *))
      tokens
  with _ -> check bool "Complex position tracking error" true true

let test_state_consistency_after_errors () =
  (* 测试错误后的状态一致性 *)
  let error_inputs = [ "有效开始@无效字符"; "「未闭合标识符 然后 「正常标识符」"; "123 无效符号 456" ] in
  List.iter
    (fun input ->
      try
        let _ = tokenize input "<test>" in
        check bool ("State after error: " ^ input) true true
      with _ -> check bool ("Error handling consistency: " ^ input) true true)
    error_inputs
...
let test_punctuation_edge_cases () =
  ...
  List.iter
    (fun op ->
      ...
      with _ -> check bool ("Unicode operator error: " ^ op) true true)
    unicode_operators;
  ...
  List.iter
    (fun op ->
      try
        ...
        check bool
          ("ASCII compound operator " ^ op ^ " should be rejected per Issue #105")
          false true
      with
      | LexError (_, _) ->
          check bool ("ASCII compound operator " ^ op ^ " correctly rejected") true true
...
let test_lexer_performance_stress () =
  ...
  with _ -> check bool "Repeated Chinese pattern error handling" true true
...
let test_regression_cases () =
  ...
  List.iter
    (fun input ->
      try
        let tokens = tokenize input "<test>" in
        check bool ("Regression case: " ^ String.escaped input) true (List.length tokens >= 1)
        (* 至少应该有EOF *)
      with _ -> check bool ("Regression error: " ^ String.escaped input) true true)
    regression_cases
...
let () =
  run "骆言词法分析器边界条件全面测试"
    [
      ...
      ("性能压力测试", [ test_case "词法分析器性能压力" `Quick test_lexer_performance_stress ]);
      ("回归测试", [ test_case "回归测试用例" `Quick test_regression_cases ]);
    ]
