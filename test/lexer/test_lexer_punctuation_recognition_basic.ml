...
let test_colon_sequence_handling () =
  ...
  (* 验证函数能返回结果 *)
  (match result with
  | Some (_token, _pos, _new_state) -> check bool "冒号处理返回结果" true true
  | None -> check bool "冒号处理可能返回None" true true);
...
let test_utf8_encoding_basic () =
  (* 测试UTF-8字符的基础属性 *)
  let utf8_test_cases = [ ("：", "全角冒号"); ("，", "全角逗号"); ("。", "全角句号") ] in
...
let utf8_processing_tests = [ test_case "UTF-8编码基础处理测试" `Quick test_utf8_encoding_basic ]
...
let integration_tests = [ test_case "模块协作测试" `Quick test_module_cooperation ]

(** 主测试运行器 *)
let () =
  run "Lexer_punctuation_recognition 基础测试"
    [
