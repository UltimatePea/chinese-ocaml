...
let test_type_annotation_parsing () =
  ...
  try
    ...
    | TypeAnnotationExpr (LitExpr (IntLit 42), BaseTypeExpr IntType) ->
      Printf.printf "✓ 类型注解解析成功\n"
    | _ ->
      Printf.printf "✗ 类型注解解析失败: %s\n" (show_expr expr)
  with
  | e ->
    Printf.printf "✗ 类型注解解析异常: %s\n" (Printexc.to_string e)
...
let test_type_annotation_inference () =
  ...
  try
    ...
    | IntType_T ->
      Printf.printf "✓ 类型注解推断成功\n"
    | _ ->
      Printf.printf "✗ 类型注解推断失败: %s\n" (show_typ inferred_type)
  with
  | e ->
    Printf.printf "✗ 类型注解推断异常: %s\n" (Printexc.to_string e)
...
let test_function_with_type_annotations () =
  ...
  try
    ...
    match stmt with
    | LetStmtWithType (name, _, _) when name = "add" ->
      Printf.printf "✓ 函数类型注解解析成功\n"
    | _ ->
      Printf.printf "✗ 函数类型注解解析失败: %s\n" (show_stmt stmt)
  with
  | e ->
    Printf.printf "✗ 函数类型注解解析异常: %s\n" (Printexc.to_string e)
