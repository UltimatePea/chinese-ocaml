...
let test_pattern_extraction () =
  ...
  let conditional_expr =
    SIfThenElse (SBinaryOp (">", SVariable "分数", SLiteral "60"), SLiteral "及格", SLiteral "不及格")
  in
...
let test_complexity_calculation () =
  ...
  Printf.printf "✅ 简单表达式复杂度: 圈复杂度=%d, 嵌套深度=%d\n" simple_complexity.cyclomatic_complexity
    simple_complexity.nesting_depth;
  ...
  Printf.printf "✅ 复杂表达式复杂度: 圈复杂度=%d, 嵌套深度=%d\n" complex_complexity.cyclomatic_complexity
    complex_complexity.nesting_depth
...
let test_codebase_analysis () =
  ...
  let sample_codes =
    [ "让 加法 = 函数 x y → x + y"; "如果 条件 那么 结果1 否则 结果2"; "匹配 值 与 | 模式1 → 结果1 | 模式2 → 结果2" ]
  in
...
let test_learning_from_program () =
  ...
  let test_expressions = [ SFunctionDef ([ "x" ], SVariable "x"); SLiteral "42" ] in
