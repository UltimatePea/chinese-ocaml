(** 骆言Builtin_Array内置数组操作模块增强测试覆盖 - Fix #1313
    ...
    本测试模块为 builtin_array.ml 提供全面的增强测试覆盖，作为提升项目测试覆盖率
    从26.21%到50%目标的第四阶段实施。专注于内置数组操作函数的核心功能测试。
...
let test_create_array_function_basic () =
  ...
  (match result2 with
   | ArrayValue arr ->
     check bool "空数组长度为0" true (Array.length arr = 0)
   | _ -> check bool "应该返回ArrayValue" false true);
...
let test_copy_array_function_basic () =
  ...
  (match result2 with
   | ArrayValue copied ->
     check bool "空数组复制长度" true (Array.length copied = 0)
   | _ -> check bool "应该返回ArrayValue" false true);
  ...
  (match result3 with
   | ArrayValue copied ->
     check bool "单元素数组复制" true (copied.(0) = StringValue "单元素")
   | _ -> check bool "应该返回ArrayValue" false true)
...
let test_array_set_function_error_handling () =
  ...
  assert_runtime_error (fun () -> array_set_function [ArrayValue arr; IntValue (-1); IntValue 999]);
  
  (* 测试空数组设置 *)
  let empty_arr = [||] in
  assert_runtime_error (fun () -> array_set_function [ArrayValue empty_arr; IntValue 0; IntValue 1]);
  
  (* 测试非数组参数 *)
  assert_runtime_error (fun () -> array_set_function [IntValue 42; IntValue 0; IntValue 1]);
  
  (* 测试非整数索引 *)
  assert_runtime_error (fun () -> array_set_function [ArrayValue arr; StringValue "错误"; IntValue 1]);
...
let test_list_to_array_function_basic () =
  ...
  (match result2 with
   | ArrayValue arr ->
     check bool "空列表转数组" true (Array.length arr = 0)
   | _ -> check bool "应该返回ArrayValue" false true);
  ...
  (match result3 with
   | ArrayValue arr ->
     check bool "单元素列表转数组" true (arr.(0) = StringValue "单元素")
   | _ -> check bool "应该返回ArrayValue" false true);
   
  (* 测试混合类型列表转数组 *)
  let result4 = list_to_array_function [ListValue [IntValue 42; BoolValue false; FloatValue 1.5]] in
  (match result4 with
   | ArrayValue arr ->
...
let test_array_functions_table () =
  ...
  List.iter (fun name ->
    check bool (Printf.sprintf "函数表包含 %s" name) true (List.mem name actual_functions)
  ) expected_functions;
  ...
  List.iter (fun (name, value) ->
    ...
    | BuiltinFunctionValue _ -> 
      check bool (Printf.sprintf "%s 是内置函数值" name) true true
    | _ -> 
      check bool (Printf.sprintf "%s 应该是内置函数值" name) false true
  ) array_functions
...
let () =
  run "骆言Builtin_Array内置数组操作模块增强测试覆盖 - Fix #1313" [
    ...
    "数组列表转换一致性测试", [
      test_case "双向转换一致性验证" `Quick test_array_list_conversion_consistency;
    ];
    "数组函数表测试", [
      test_case "函数表完整性验证" `Quick test_array_functions_table;
    ];
  ]