...
let test_literal_conversions () =
  ...
  (* 测试布尔Token转换 *)
  let bool_token = Token_system_compatibility.Legacy_type_bridge.convert_bool_token true in
  check bool "布尔Token转换正确性" true
    (match bool_token with Token_system_core.Token_types.BoolToken true -> true | _ -> false)

(** 测试标识符转换函数 *)
let test_identifier_conversions () =
  (* 测试简单标识符转换 *)
  let simple_id = Token_system_compatibility.Legacy_type_bridge.convert_simple_identifier "测试变量" in
  check bool "简单标识符转换正确性" true
    (match simple_id with Token_system_core.Token_types.SimpleIdentifier "测试变量" -> true | _ -> false);
...
let test_token_construction () =
  ...
  (* 测试操作符Token构造 *)
  let plus_op = Token_system_compatibility.Legacy_type_bridge.convert_plus_op () in
  let operator_token = Token_system_compatibility.Legacy_type_bridge.make_operator_token plus_op in
