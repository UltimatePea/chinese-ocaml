...
module TestUtils = struct
  (** 检查令牌是否相等 *)
  let check_token_equal desc expected actual = check bool desc true (compare expected actual = 0)

  (** 检查令牌不相等 *)
  let check_token_not_equal desc left right = check bool desc false (compare left right = 0)
  ...
  let check_metadata_equal desc expected actual =
    ...
    | Some e, Some a ->
        check bool desc true
          (e.category = a.category && e.priority = a.priority
...
module PositionTests = struct
  ...
  let test_position_creation () =
    let pos = { filename = "test.ly"; line = 10; column = 25; offset = 150 } in
    ...
    check bool "不同位置信息不等" false
      (pos.line = diff_pos.line && pos.column = diff_pos.column && pos.offset = diff_pos.offset
...
module UnifiedTokenTests = struct
  ...
  let test_literal_tokens () =
    ...
    List.iter (fun (token, desc) -> TestUtils.check_token_equal (desc ^ "自身相等") token token) tokens;
  ...
  let test_identifier_tokens () =
    ...
    List.iter
      (fun (token, desc) -> TestUtils.check_token_equal (desc ^ "自身相等") token token)
      identifiers;
  ...
  let test_numeric_keywords () =
    ...
    List.iter
      (fun (token, desc) -> TestUtils.check_token_equal (desc ^ "关键字自身相等") token token)
      numeric_keywords;
...
module PositionedTokenTests = struct
  ...
  let test_same_token_different_positions () =
    ...
    (* 但位置不同 *)
    check bool "不同位置信息不等" false
      (pos1.line = pos2.line && pos1.column = pos2.column && pos1.offset = pos2.offset
