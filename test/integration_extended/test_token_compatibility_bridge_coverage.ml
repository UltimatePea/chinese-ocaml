(** Token兼容性桥接模块测试覆盖率提升 - Fix #1446
    ...
    目标: 为Token_compatibility_bridge模块中未测试的函数添加测试覆盖 重点: to_lexer_tokens_result 和
    from_lexer_tokens_result 函数测试
...
let test_token_conversion_error_handling () =
  ...
  let test_tokens = [ Token_unified.EOF; Token_unified.Error "测试错误"; Token_unified.UnitToken ] in

  let result = to_lexer_tokens_result test_tokens in
  (match result with
  | Ok _tokens -> Printf.printf "✓ 特殊Token转换成功\n"
  | Error msg -> Printf.printf "⚠ 特殊Token转换遇到预期错误: %s\n" msg);
  ...
  | Ok old_tokens -> (
      ...
      | Ok final_tokens ->
          Printf.printf "✓ 往返转换成功，原始: %d -> 中间: %d -> 最终: %d\n" (List.length original_tokens)
            (List.length old_tokens) (List.length final_tokens)
      | Error msg -> Printf.printf "✗ 第二步转换失败: %s\n" msg)
  | Error msg -> Printf.printf "✗ 第一步转换失败: %s\n" msg);
...
let test_batch_conversion_performance () =
  ...
  let large_token_list =
    List.init 1000 (fun i -> Token_unified.IdentifierToken ("变量" ^ string_of_int i))
  in
  ...
  (match result with
  | Ok _converted -> Printf.printf "✓ 批量转换1000个Token耗时: %.4f秒\n" (end_time -. start_time)
  | Error msg -> Printf.printf "✗ 批量转换失败: %s\n" msg);
