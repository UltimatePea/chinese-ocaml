...
let test_basic_keyword_conversion () =
  ...
  List.iter
    (fun (input, expected) ->
      Alcotest.(check bool) "Token conversion" true (convert_basic_keyword_token input = expected))
    test_cases
...
let test_classical_token_conversion () =
  ...
  List.iter
    (fun (input, expected) ->
      Alcotest.(check bool)
        "Classical token conversion" true
