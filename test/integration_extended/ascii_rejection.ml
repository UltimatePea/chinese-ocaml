...
let test_ascii_digits_rejected () =
  ...
  List.iter
    (fun digit ->
      ...
      | LexError (msg, _) ->
          let contains_expected =
            contains_utf8_substring msg "阿拉伯数字" && contains_utf8_substring msg "禁"
          in
          check bool ("ASCII数字 " ^ digit ^ " 错误消息应该包含'阿拉伯数字已禁用'") true contains_expected
