...
module TokenStringConversionTests = struct
  ...
  let test_bulk_token_conversion () =
    ...
    (* 检查所有转换结果都不为空 *)
    List.iter (fun str -> check bool "转换结果不为空" true (String.length str > 0)) converted
end
  ...
  let test_special_string_content () =
    ...
    List.iter
      (fun s ->
        ...
        check bool ("特殊字符串token转换: " ^ s) true (String.length result >= 2)
        (* 至少包含引号 *))
      special_strings
end
...
let test_suite =
  [
    ( "Token字符串转换测试",
      [
        test_case "基本token到字符串转换" `Quick
          TokenStringConversionTests.test_basic_token_to_string_conversion;
    ...
    ("性能测试", [ test_case "大量token转换性能" `Slow PerformanceTests.test_bulk_conversion_performance ]);
  ]
