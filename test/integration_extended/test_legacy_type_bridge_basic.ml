...
let test_literal_conversions () =
  ...
  check bool "浮点数Token转换正确性" true
    (match float_token with
    | Token_system_unified_core.Token_types.FloatToken 3.14 -> true
    | _ -> false);
  ...
  check bool "字符串Token转换正确性" true
    ...
    | Token_system_unified_core.Token_types.StringToken "骆言" -> true
    | _ -> false);

  (* 测试布尔Token转换 *)
  let bool_token = Token_system_unified_conversion.Legacy_type_bridge.convert_bool_token true in
  check bool "布尔Token转换正确性" true
    (match bool_token with
    | Token_system_unified_core.Token_types.BoolToken true -> true
    | _ -> false)
...
let test_identifier_conversions () =
  (* 测试简单标识符转换 *)
  let simple_id =
    Token_system_unified_conversion.Legacy_type_bridge.convert_simple_identifier "测试变量"
  in
  check bool "简单标识符转换正确性" true
    (match simple_id with
    | Token_system_unified_core.Token_types.SimpleIdentifier "测试变量" -> true
    | _ -> false);

  (* 测试引用标识符转换 *)
  let quoted_id =
    Token_system_unified_conversion.Legacy_type_bridge.convert_quoted_identifier "引用变量"
  in
  check bool "引用标识符转换正确性" true
    (match quoted_id with
    | Token_system_unified_core.Token_types.QuotedIdentifierToken "引用变量" -> true
    | _ -> false)
...
let test_token_construction () =
  ...
  let literal_token =
    Token_system_unified_conversion.Legacy_type_bridge.make_literal_token int_literal
  in
  ...
  (* 测试标识符Token构造 *)
  let simple_id =
    Token_system_unified_conversion.Legacy_type_bridge.convert_simple_identifier "test_var"
  in
  ...
  let plus_op = Token_system_unified_conversion.Legacy_type_bridge.convert_plus_op () in
  let operator_token =
    Token_system_unified_conversion.Legacy_type_bridge.make_operator_token plus_op
  in
...
(** 运行所有基础测试 *)
let () = run "Legacy Type Bridge 基础测试套件 - PR #1356" [ ("基础转换功能", basic_conversion_tests) ]
