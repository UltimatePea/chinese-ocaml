...
let test_keyword_tokens () =
  ...
  List.iter (fun (token, name) -> Printf.printf "  关键字令牌: %s\n" name) keywords;
...
let test_operator_tokens () =
  ...
  let operators = [ (Plus, "+"); (Minus, "-"); (Multiply, "*"); (Divide, "/"); (Equal, "=") ] in

  List.iter (fun (token, symbol) -> Printf.printf "  操作符令牌: %s\n" symbol) operators;
...
let test_delimiter_tokens () =
  ...
  List.iter (fun (token, symbol) -> Printf.printf "  分隔符令牌: %s\n" symbol) delimiters;
