(** 错误恢复模块测试套件
    ...
    验证error_handler_recovery.ml模块的错误恢复策略功能 包括恢复策略确定、恢复执行和继续处理判断
...
let create_test_context () =
  create_context ~source_file:"test.ly" ~function_name:"test_func" ~module_name:"Test"
...
let test_advanced_recovery_strategies () =
  ...
  check bool "运行时错误应使用RequestUserInput策略" true (runtime_strategy = RequestUserInput);
  ...
  check bool "异常错误应使用RequestUserInput策略" true (exception_strategy = RequestUserInput);
  ...
  check bool "内部错误应使用Abort策略" true (internal_strategy = Abort);
...
let test_attempt_recovery () =
  ...
  let sync_enhanced =
    create_enhanced_error ~recovery_strategy:SyncToNextStatement base_error context
  in
  ...
  (* 测试TryAlternative恢复 *)
  let alt_enhanced =
    create_enhanced_error ~recovery_strategy:(TryAlternative "替代方案") base_error context
  in
  ...
  let user_enhanced =
    create_enhanced_error ~recovery_strategy:RequestUserInput base_error context
  in
...
let test_recovery_statistics () =
  ...
  (* 检查统计是否更新 *)
  check bool "成功恢复应该增加recovered_errors计数" true (global_stats.recovered_errors > before_recovered);
...
let test_edge_cases () =
  ...
  check bool "语义错误应使用SkipAndContinue策略" true (semantic_strategy = SkipAndContinue);

  (* 测试多次尝试恢复的情况 *)
  let base_error = create_error_of_type "parse" in
  let enhanced_multiple =
    create_enhanced_error ~recovery_strategy:SyncToNextStatement ~attempt_count:3 base_error context
  in
  ...
  let main_enhanced =
    create_enhanced_error ~recovery_strategy:SkipAndContinue ~related_errors:[ related_error ]
      base_error context
  in
