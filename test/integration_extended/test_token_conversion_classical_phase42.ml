(** Token古典语言转换Phase 4.2重构测试 - Issue #1336
    ...
    测试策略模式重构后的古典语言转换系统，确保： 1. 策略模式正确实现 2. 向后兼容性保持 3. 所有古典语言token类型正确转换 4. 错误处理正确
...
let test_wenyan_strategy () =
  ...
  in
  let expected_results = [ HaveKeyword; OneKeyword; NameKeyword; SetKeyword; AlsoKeyword ] in
...
let test_natural_language_strategy () =
  ...
  let expected_results =
    [ DefineKeyword; AcceptKeyword; ReturnWhenKeyword; MultiplyKeyword; DivideKeyword ]
  in
...
let run_all_tests () =
  ...
  let run_test (name, test_func) =
    ...
    try
      ...
      if result then Printf.printf "✅ %s 通过\n" name else Printf.printf "❌ %s 失败\n" name;
      result
    with exn ->
