(** 错误统计模块测试套件
    ...
    验证error_handler_statistics.ml模块的错误统计和记录功能 包括统计更新、历史记录、报告生成和系统初始化
...
let create_test_context () =
  create_context ~source_file:"test.ly" ~function_name:"测试函数" ~module_name:"Test"
    ~call_stack:[ "main"; "测试函数" ] ()
...
let test_statistics_initialization () =
  ...
  check bool "开始时间应该在合理范围内" true (abs_float (current_time -. global_stats.start_time) < 1.0);
...
let test_error_history_recording () =
  ...
  check bool "最新错误应该在历史记录开头" true (latest_error.context.function_name = "测试函数");
...
let test_history_size_limit () =
  ...
  check bool "应该保留最新的错误" true
    (match latest_error.base_error.error with SyntaxError (msg, _) -> msg = "历史错误5" | _ -> false);
