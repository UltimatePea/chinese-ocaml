(** 全面的token转换策略测试 解决Issue #1335中的测试覆盖不足问题
...
let calculate_coverage results =
  ...
  let readable_success =
    List.fold_left (fun acc r -> if r.readable_success then acc + 1 else acc) 0 results
  in
  let fast_success =
    List.fold_left (fun acc r -> if r.fast_success then acc + 1 else acc) 0 results
  in
  let consistent =
    List.fold_left (fun acc r -> if r.results_match then acc + 1 else acc) 0 results
  in
...
let run_coverage_test () =
  ...
  Printf.printf "  可读性策略成功: %d/%d (%.1f%%)\n" readable_ok total
    (float_of_int readable_ok /. float_of_int total *. 100.0);
  Printf.printf "  性能策略成功: %d/%d (%.1f%%)\n" fast_ok total
    (float_of_int fast_ok /. float_of_int total *. 100.0);
  Printf.printf "  结果一致性: %d/%d (%.1f%%)\n" consistent total
    (float_of_int consistent /. float_of_int total *. 100.0);
  ...
  Printf.printf "  改进幅度: +%d个token (+%.0f%%)\n" (total - 16)
    ((float_of_int total -. 16.0) /. 16.0 *. 100.0);
