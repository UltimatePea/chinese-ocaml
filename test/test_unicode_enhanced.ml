...
module TestData = struct
  ...
  (** 引号测试用例 *)
  let quote_cases = [ ("『", "left_quote", "quote"); ("』", "right_quote", "quote") ]

  (** 混合中文编程示例 *)
  let chinese_programming_examples =
    [ "让「变量」= 123"; "假如「条件」那么「执行」"; "夫「函数」者，算法也。"; "设「数组」为【一，二，三】"; "输出『你好，世界！』" ]

  (** UTF-8边界测试用例 *)
  let utf8_boundary_cases =
    [
      ("hello世界", [ (0, 5); (5, 8); (8, 11) ]);
      (* ASCII + 中文 *)
      ("你好world", [ (0, 3); (3, 6); (6, 11) ]);
      (* 中文 + ASCII *)
...
module CharacterClassificationTests = struct
  let test_chinese_punctuation () =
    ...
    List.iter
      (fun (char, expected_name, expected_category) ->
      ...
      (fun (char, expected_name, expected_category) ->
        ...
        | Some def ->
            ...
            Printf.printf "字符 '%s': 类别匹配=%b, 符号分类=%s\n" char category_match
              (match symbol_class with Some c -> c | None -> "无")
        | None -> Printf.printf "字符 '%s': 未找到定义 ❌\n" char)
      ...
      (fun (char, expected_name, expected_category) ->
        ...
        | Some def ->
            ...
            Printf.printf "字符 '%s': 类别匹配=%b, 诗词分类=%s\n" char category_match
              (match poetry_class with Some c -> c | None -> "无")
        | None -> Printf.printf "字符 '%s': 未找到定义 ❌\n" char)
    ...
    List.iter
      (fun (char, expected_name, expected_category) ->
        ...
        Printf.printf "字符 '%s': 引号识别=%b, 配对=%s\n" char is_quote
          (match pair with Some p -> p | None -> "无"))
...
module UTF8ProcessingTests = struct
  let test_utf8_char_counting () =
    Printf.printf "\n=== 测试UTF-8字符计数 ===\n";
    let test_strings = [ ("hello", 5); ("你好", 2); ("hello世界", 7); ("中文，标点。", 5); ("", 0) ] in
    List.iter
      (fun (input, expected) ->
  ...
  let test_utf8_string_validation () =
    Printf.printf "\n=== 测试UTF-8字符串验证 ===\n";
    let test_strings =
      [ ("正常中文", true); ("Mixed中英文", true); ("数字123", true); ("\xff\xfe\xfd", false) (* 无效UTF-8 *) ]
    in
    List.iter
      (fun (input, should_be_valid) ->
        ...
        Printf.printf "字符串 '%s': 期望有效=%b, 实际有效=%b, 匹配=%b\n" input should_be_valid is_valid
          match_result;
        if not is_valid then
          List.iter (fun (pos, msg) -> Printf.printf "  错误位置 %d: %s\n" pos msg) errors)
      test_strings
...
module PositionTrackingTests = struct
  let test_position_conversion () =
    ...
    in
    List.iter
      (fun (input, test_positions) ->
        Printf.printf "字符串 '%s':\n" input;
        List.iter
          (fun (byte_offset, expected_char_offset) ->
            let actual_char_offset =
              PositionTracking.byte_offset_to_char_offset input byte_offset
            in
            let reverse_byte_offset =
              PositionTracking.char_offset_to_byte_offset input actual_char_offset
            in
            Printf.printf "  字节偏移 %d -> 字符偏移 %d (期望 %d), 反向转换 %d\n" byte_offset actual_char_offset
              expected_char_offset reverse_byte_offset)
          test_positions)
      test_cases
  ...
  let test_position_advancement () =
    ...
    let rec test_advancement pos chars =
      ...
      | char :: rest ->
          ...
          Printf.printf "%d, 字符位置 %d -> %d, 行号 %d -> %d\n" new_pos.byte_pos pos.char_pos
            new_pos.char_pos pos.line_num new_pos.line_num;
...
module BoundaryDetectionTests = struct
  let test_word_boundaries () =
    ...
    let test_cases =
      [
        ("hello world", [ 0; 5; 6; 11 ]); ("你好世界", [ 0; 3; 6; 9 ]); ("中文English混合", [ 0; 6; 13; 16 ]);
      ]
    in
    List.iter
      (fun (input, expected_boundaries) ->
        Printf.printf "字符串 '%s':\n" input;
        List.iter
          (fun pos ->
...
module PerformanceTests = struct
  let test_lookup_performance () =
    ...
    for i = 1 to iterations do
      List.iter (fun char -> ignore (OptimizedLookup.find_bytes_by_char_fast char)) test_chars
    done;
  ...
  let test_batch_processing () =
    ...
    Printf.printf "处理 %d 字符: %.6f秒 (%.0f 字符/秒)\n" char_count processing_time
      (float char_count /. processing_time)
end
...
module ErrorHandlingTests = struct
  let test_invalid_utf8_handling () =
    ...
    let invalid_sequences =
      [ "\xFF\xFE"; (* 无效起始字节 *) "\xE4\x80"; (* 不完整的3字节序列 *) "\xE4\xFF\x80" (* 无效继续字节 *) ]
    in
    List.iter
      (fun invalid_input ->
        Printf.printf "测试无效序列: ";
        match UTF8Processing.next_utf8_char_safe invalid_input 0 with
        | ValidChar (char, len) -> Printf.printf "意外成功: '%s' (长度 %d)\n" char len
        | InvalidSequence (pos, msg) -> Printf.printf "正确检测到错误位置 %d: %s ✅\n" pos msg
        | EndOfInput -> Printf.printf "输入结束\n")
      invalid_sequences
  ...
  let test_error_recovery () =
    ...
    List.iter (fun (pos, msg) -> Printf.printf "  位置 %d: %s\n" pos msg) errors
end
  ...
  let test_programming_examples () =
    ...
    List.iter
      (fun example ->
        ...
        let char_count = List.length char_list in
        let categories =
          List.map (fun char -> CharacterDetection.classify_unicode_char char) char_list
        in
  ...
  let test_character_suggestions () =
    ...
    List.iter
      (fun ascii_char ->
        ...
        | Some chinese_alt -> Printf.printf "ASCII '%s' -> 中文 '%s' ✅\n" ascii_char chinese_alt
        | None -> Printf.printf "ASCII '%s' -> 无建议\n" ascii_char)
      ascii_chars
end
...
let run_all_tests () =
  ...
  let name_to_char_size, char_to_bytes_size, name_to_bytes_size, category_to_defs_size =
    Statistics.get_lookup_performance_info ()
  in
  Printf.printf "\n📊 查找表性能信息:\n";
