...
(** 辅助函数：检查内置函数是否存在 *)
let check_builtin_exists name description =
  check bool description true (is_builtin_function name)

(** 测试基础IO函数类型定义 *)
let test_basic_io_functions () =
  (* 测试打印函数类型 *)
  check_builtin_function_type "打印" 
    (FunType_T (StringType_T, UnitType_T))
    "打印函数应接受字符串参数并返回单位类型";
  
  (* 测试读取函数类型 *)
  check_builtin_function_type "读取"
    (FunType_T (UnitType_T, StringType_T))
    "读取函数应接受单位类型参数并返回字符串"
...
let test_math_functions () =
  (* 测试基础数学函数 *)
  check_builtin_function_type "求和"
  ...
  check_builtin_function_type "绝对值"
    (FunType_T (IntType_T, IntType_T))
    "绝对值函数应接受整数并返回整数";
  ...
  check_builtin_function_type "正弦"
    (FunType_T (FloatType_T, FloatType_T))
    "正弦函数应接受浮点数并返回浮点数"

(** 测试扩展数学函数类型定义 *)
let test_extended_math_functions () =
  (* 测试对数函数 *)
  check_builtin_function_type "对数"
    (FunType_T (FloatType_T, FloatType_T))
    "对数函数应接受浮点数并返回浮点数";
  
  check_builtin_function_type "自然对数"
    (FunType_T (FloatType_T, FloatType_T))
    "自然对数函数应接受浮点数并返回浮点数";
  
  (* 测试三角函数 *)
  check_builtin_function_type "正切"
    (FunType_T (FloatType_T, FloatType_T))
    "正切函数应接受浮点数并返回浮点数";
  
  check_builtin_function_type "反正弦"
    (FunType_T (FloatType_T, FloatType_T))
    "反正弦函数应接受浮点数并返回浮点数";
  
  (* 测试取整函数 *)
  check_builtin_function_type "向上取整"
    (FunType_T (FloatType_T, IntType_T))
    "向上取整函数应接受浮点数并返回整数";
...
let test_string_functions () =
  (* 测试字符串长度函数 *)
  check_builtin_function_type "字符串长度"
    (FunType_T (StringType_T, IntType_T))
    "字符串长度函数应接受字符串并返回整数";
...
let test_builtin_function_queries () =
  ...
  | Some (TypeScheme ([], actual_type)) ->
    check (testable pp_typ equal_typ) "获取打印函数类型"
      (FunType_T (StringType_T, UnitType_T)) actual_type
  | Some _ -> fail "打印函数类型 scheme 不正确"
  | None -> fail "无法获取打印函数类型");
  
  (* 测试不存在的函数 *)
  check bool "不存在的函数应返回None"
    true (get_builtin_type "不存在的函数" = None);
...
let test_type_environment_completeness () =
  ...
  (* 验证数学函数在主环境中 *)
  List.iter (fun func ->
    check bool (Printf.sprintf "数学函数'%s'应在主环境中" func) 
      true (List.mem func all_functions)
  ) math_functions;
  
  (* 验证列表函数在主环境中 *)
  List.iter (fun func ->
    check bool (Printf.sprintf "列表函数'%s'应在主环境中" func) 
      true (List.mem func all_functions)
  ) list_functions;
  
  (* 验证IO函数在主环境中 *)
  List.iter (fun func ->
    check bool (Printf.sprintf "IO函数'%s'应在主环境中" func) 
      true (List.mem func all_functions)
  ) io_functions
...
let () =
  run "Types_builtin模块测试" [
    "基础IO函数类型", [
      test_case "基础IO函数类型定义" `Quick test_basic_io_functions;
    ];
    "列表函数类型", [
      test_case "列表函数类型定义" `Quick test_list_functions;
    ];
    "数学函数类型", [
    ...
    "字符串函数类型", [
      test_case "字符串函数类型定义" `Quick test_string_functions;
    ];
    "内置环境构建", [
    ...
    "中文函数名支持", [
      test_case "中文函数名支持完整性" `Quick test_chinese_function_name_support;
    ];
    "类型环境完整性", [
      test_case "类型环境完整性验证" `Quick test_type_environment_completeness;
    ];
  ]