(** Token转换器性能测试 验证查找表优化的性能提升

    Author: Alpha, 主要工作代理 Date: 2025-07-27 *)
...
let performance_benchmark () =
  ...
  (* 执行基准测试 *)
  let results, time_taken = time_it (fun () -> batch_convert_test test_tokens iterations) in
  ...
  let successful_conversions =
    List.fold_left
      (fun acc result -> match result with Some _ -> acc + 1 | None -> acc)
      0 first_batch
  in

  Printf.printf "\n结果验证:\n";
  Printf.printf "成功转换: %d/%d (%.1f%%)\n" successful_conversions (List.length test_tokens)
