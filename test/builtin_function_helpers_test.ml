(** 骆言内置函数辅助工具模块综合测试
    ...
    此测试文件为builtin_function_helpers.ml提供完整的测试覆盖， 确保所有导出的辅助函数正确处理参数验证、类型转换和错误处理。
...
let test_single_string_builtin () =
  ...
  List.iter
    (fun (args, expected, description) ->
      let result =
        single_string_builtin "test_func"
          (if description = "identity function" then identity_string else uppercase_string)
          args
      in
      match result with
      | StringValue actual -> check string description expected actual
      | _ -> failwith ("期望StringValue，实际得到其他类型: " ^ description))
    test_cases
...
let test_single_int_builtin () =
  ...
  List.iter
    (fun (args, expected, description) ->
      ...
      match result with
      | IntValue actual -> check int description expected actual
      | _ -> failwith ("期望IntValue，实际得到其他类型: " ^ description))
    test_cases

let test_single_float_builtin () =
  let test_cases =
    [
      ([ FloatValue 1.5 ], 2.5, "increment float"); ([ FloatValue 0.0 ], 1.0, "increment zero float");
    ]
  in
  ...
  List.iter
    (fun (args, expected, description) ->
      ...
      match result with
      | FloatValue actual -> check (float 0.001) description expected actual
      | _ -> failwith ("期望FloatValue，实际得到其他类型: " ^ description))
    test_cases

let test_single_bool_builtin () =
  let test_cases =
    [ ([ BoolValue true ], false, "negate true"); ([ BoolValue false ], true, "negate false") ]
  in
  ...
  List.iter
    (fun (args, expected, description) ->
      ...
      match result with
      | BoolValue actual -> check bool description expected actual
      | _ -> failwith ("期望BoolValue，实际得到其他类型: " ^ description))
    test_cases

let test_single_to_string_builtin () =
  let test_cases =
    [ ([ IntValue 42 ], "42", "int to string"); ([ FloatValue 3.14 ], "3.14", "float to string") ]
  in
  ...
  List.iter
    (fun (args, expected, description) ->
      let result =
        ...
        else single_to_string_builtin "test_func" expect_float string_of_float args
      in
      match result with
      | StringValue actual -> check string description expected actual
      | _ -> failwith ("期望StringValue，实际得到其他类型: " ^ description))
...
let test_single_conversion_builtin () =
  (* Test int conversion *)
  let result1 =
    single_conversion_builtin "test_func" expect_string int_of_string
      (fun x -> IntValue x)
      [ StringValue "42" ]
  in
  (match result1 with
  ...
  let result2 =
    single_conversion_builtin "test_func" expect_string float_of_string
      (fun x -> FloatValue x)
      [ StringValue "3.14" ]
  in
  match result2 with
...
let test_single_list_builtin () =
  ...
  | ListValue actual ->
      ...
      if check_equal actual expected_reversed then () (* 测试通过 *) else failwith "列表反转结果不正确"
  | _ -> failwith "期望ListValue，实际得到其他类型"
...
let test_curried_double_string_builtin () =
  (* 第一步：创建柯里化函数 *)
  let curried_func =
    curried_double_string_builtin "test_func" concat_strings [ StringValue "hello" ]
  in
  ...
  | BuiltinFunctionValue func -> (
      ...
      match result with
      | StringValue actual -> check string "curried concatenation" "helloworld" actual
      | _ -> failwith "期望StringValue，实际得到其他类型")
  | _ -> failwith "期望BuiltinFunctionValue，实际得到其他类型"

let test_curried_double_string_to_bool_builtin () =
  (* 测试柯里化布尔返回函数 *)
  let curried_func =
    curried_double_string_to_bool_builtin "test_func" string_equals [ StringValue "test" ]
  in
  ...
  | BuiltinFunctionValue func -> (
      ...
      | BoolValue true, BoolValue false -> () (* 测试通过 *)
      | _ -> failwith "柯里化比较函数结果不正确")
  | _ -> failwith "期望BuiltinFunctionValue，实际得到其他类型"
...
let test_curried_string_to_list_builtin () =
  ...
  | BuiltinFunctionValue func -> (
      ...
      | ListValue lst ->
          ...
          let check_equal lst1 lst2 = List.equal ( = ) lst1 lst2 in
          if check_equal lst expected then () (* 测试通过 *) else failwith "柯里化字符串分割结果不正确"
      | _ -> failwith "期望ListValue，实际得到其他类型")
  | _ -> failwith "期望BuiltinFunctionValue，实际得到其他类型"
...
let test_single_file_builtin () =
  ...
  let mock_file_operation filename =
    if filename = "test.txt" then StringValue "file content"
    else failwith ("File not found: " ^ filename)
  in

  (* 测试正常情况 *)
  let result = single_file_builtin "test_func" mock_file_operation [ StringValue "test.txt" ] in
  match result with
  | StringValue actual -> check string "file operation" "file content" actual
  | _ -> failwith "期望StringValue，实际得到其他类型"

let test_single_file_builtin_error () =
  (* 测试文件不存在的情况 *)
  let mock_file_operation filename = failwith ("File not found: " ^ filename) in
...
let test_comprehensive_builtin_workflow () =
  ...
  (* 2. 双参数函数 *)
  let result2 =
    double_string_builtin "concat" concat_strings [ StringValue "Hello"; StringValue " World" ]
  in
  (match result2 with
...
let () =
  ...
  run "骆言内置函数辅助工具模块综合测试" [ ("内置函数辅助工具", builtin_function_helpers_tests) ];
