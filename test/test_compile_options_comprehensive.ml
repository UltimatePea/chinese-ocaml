...
let () =
  ...
  (try
    ...
    Printf.printf "  - 文件名: %s\n" (match default_opts.filename with 
      | Some f -> f 
      | None -> "无");
    ...
    Printf.printf "  - C输出文件: %s\n" (match default_opts.c_output_file with 
      | Some f -> f 
      | None -> "无");
    
    (* 验证默认选项的合理性 *)
    let is_reasonable = 
      not default_opts.show_tokens &&  (* 默认不显示tokens *)
      not default_opts.show_ast &&     (* 默认不显示AST *)
      not default_opts.check_only &&   (* 默认不是仅检查模式 *)
      not default_opts.quiet_mode &&   (* 默认不是静默模式 *)
      default_opts.filename = None &&  (* 默认没有指定文件名 *)
      default_opts.recovery_mode &&    (* 默认启用错误恢复 *)
      String.length default_opts.log_level > 0 && (* 有有效的日志级别 *)
      not default_opts.compile_to_c &&           (* 默认不编译到C *)
      default_opts.c_output_file = None          (* 默认没有C输出文件 *)
    in
    
    if is_reasonable then
      Printf.printf "✅ 默认编译选项合理性检查通过\n"
    else
      Printf.printf "⚠️  默认编译选项可能需要调整\n";
  with
  | e -> Printf.printf "❌ 默认编译选项测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    Printf.printf "  - 文件名: %s\n" (match test_opts.filename with 
      | Some f -> f 
      | None -> "无");
    Printf.printf "  - 恢复模式: %b\n" test_opts.recovery_mode;
    Printf.printf "  - 日志级别: %s\n" test_opts.log_level;
    Printf.printf "  - 编译到C: %b\n" test_opts.compile_to_c;
    Printf.printf "  - C输出文件: %s\n" (match test_opts.c_output_file with 
      | Some f -> f 
      | None -> "无");
    
    (* 验证测试选项的特点 *)
    let is_test_appropriate = 
      test_opts.show_tokens &&      (* 测试时通常显示tokens *)
      test_opts.show_ast &&         (* 测试时通常显示AST *)
      test_opts.show_types &&       (* 测试时通常显示类型 *)
      not test_opts.quiet_mode &&   (* 测试时不应该静默 *)
      test_opts.recovery_mode       (* 测试时启用错误恢复 *)
    in
    
    if is_test_appropriate then
      Printf.printf "✅ 测试模式编译选项配置合理\n"
    else
      Printf.printf "⚠️  测试模式编译选项可能需要调整\n";
  with
  | e -> Printf.printf "❌ 测试模式编译选项测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    Printf.printf "  - 文件名: %s\n" (match quiet_opts.filename with 
      | Some f -> f 
      | None -> "无");
    Printf.printf "  - 恢复模式: %b\n" quiet_opts.recovery_mode;
    Printf.printf "  - 日志级别: %s\n" quiet_opts.log_level;
    Printf.printf "  - 编译到C: %b\n" quiet_opts.compile_to_c;
    Printf.printf "  - C输出文件: %s\n" (match quiet_opts.c_output_file with 
      | Some f -> f 
      | None -> "无");
    
    (* 验证静默选项的特点 *)
    let is_quiet_appropriate = 
      not quiet_opts.show_tokens &&  (* 静默时不显示tokens *)
      not quiet_opts.show_ast &&     (* 静默时不显示AST *)
      not quiet_opts.show_types &&   (* 静默时不显示类型 *)
      quiet_opts.quiet_mode &&       (* 必须是静默模式 *)
      String.equal quiet_opts.log_level "error" || 
      String.equal quiet_opts.log_level "warn"   (* 静默时日志级别较高 *)
    in
    
    if is_quiet_appropriate then
      Printf.printf "✅ 静默模式编译选项配置合理\n"
    else
      Printf.printf "⚠️  静默模式编译选项可能需要调整\n";
  with
  | e -> Printf.printf "❌ 静默模式编译选项测试失败: %s\n" (Printexc.to_string e));
  ...
  with
  | e -> Printf.printf "❌ 编译选项字段访问测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    let batch_compile_opts = {
      default_options with
      quiet_mode = true;
      recovery_mode = false;  (* 批量时不恢复，快速失败 *)
      log_level = "warn";
      compile_to_c = true;
    } in
  ...
  with
  | e -> Printf.printf "❌ 编译选项使用场景测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    let validate_options opts =
      ...
      (* 检查C编译选项的一致性 *)
      if opts.compile_to_c && opts.check_only then
        errors := "编译到C模式与仅检查模式冲突" :: !errors;
    ...
    if valid_errors = [] then
      Printf.printf "✅ 默认选项配置验证通过\n"
    else
      Printf.printf "❌ 默认选项配置存在问题: %s\n" (String.concat "; " valid_errors);
    
    Printf.printf "\n🧪 验证冲突的选项配置:\n";
    let conflicted_opts = {
      default_options with
      quiet_mode = true;
      show_tokens = true;  (* 与静默模式冲突 *)
      show_ast = true;     (* 与静默模式冲突 *)
    } in
    ...
    if conflict_errors <> [] then
      ...
      Printf.printf "⚠️  冲突配置未被检测到\n";
    
    Printf.printf "\n🧪 验证不一致的C编译配置:\n";
    let inconsistent_c_opts = {
      default_options with
      c_output_file = Some "output.c";
      compile_to_c = false;  (* 不一致：设置了输出文件但未启用C编译 *)
    } in
    ...
    if inconsistent_errors <> [] then
      ...
      Printf.printf "⚠️  不一致C配置未被检测到\n";
    
    Printf.printf "✅ 编译选项验证逻辑测试完成\n";
  with
  | e -> Printf.printf "❌ 编译选项验证测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    let compare_options o1 o2 =
      o1.show_tokens = o2.show_tokens &&
      o1.show_ast = o2.show_ast &&
      o1.show_types = o2.show_types &&
      o1.check_only = o2.check_only &&
      o1.quiet_mode = o2.quiet_mode &&
    ...
    let serialize_options opts =
      Printf.sprintf "{show_tokens=%b; show_ast=%b; show_types=%b; check_only=%b; quiet_mode=%b; filename=%s; recovery_mode=%b; log_level=%s; compile_to_c=%b; c_output_file=%s}"
        opts.show_tokens
        opts.show_ast
        opts.show_types
        opts.check_only
    ...
    if compare_options opts1 opts2 && not (compare_options opts1 opts3) then
      ...
      Printf.printf "❌ 编译选项比较测试失败\n";
  with
  | e -> Printf.printf "❌ 编译选项序列化测试失败: %s\n" (Printexc.to_string e));
  ...
  (try
    ...
    (* 大量编译选项创建和访问操作 *)
    for i = 1 to 10000 do
      let opts = if i mod 3 = 0 then test_options
                else if i mod 3 = 1 then quiet_options
                else default_options in
    ...
    if duration < 0.5 then
      Printf.printf "✅ 编译选项操作性能优秀\n"
    else
      Printf.printf "⚠️  编译选项操作性能可能需要优化\n";
  with
  | e -> Printf.printf "❌ 编译选项性能测试失败: %s\n" (Printexc.to_string e));
