...
let test_multi_param_function_call_codegen () =
  let ctx = create_test_context () in
  let call_expr =
    FunCallExpr (VarExpr "add", [ LitExpr (IntLit 10); LitExpr (IntLit 20); VarExpr "third_param" ])
  in
  let result = gen_expr ctx call_expr in
...
let test_simple_conditional_codegen () =
  let ctx = create_test_context () in
  let cond_expr = CondExpr (VarExpr "condition", LitExpr (IntLit 1), LitExpr (IntLit 0)) in
...
let test_large_integer_literal () =
  let ctx = create_test_context () in
  let large_int_expr = LitExpr (IntLit 2147483647) in
  (* 最大32位整数 *)
...
let test_large_expression_tree () =
  ...
  check bool "大型表达式生成性能可接受" true (duration < 1.0)
(* 小于1秒 *)
...
let boundary_tests =
  [
    ("大整数字面量处理", `Quick, test_large_integer_literal); ("极长字符串处理", `Quick, test_long_string_literal);
  ]

let performance_tests = [ ("大型表达式树的代码生成性能", `Quick, test_large_expression_tree) ]
