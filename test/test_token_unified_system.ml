...
let test_chinese_numbers () =
  let test_cases =
    [
      ("一", IntToken 1); ("二", IntToken 2); ("十", IntToken 10); ("三点一四", ChineseNumberToken "三点一四");
    ]
  in
    ...
    (fun (input, expected) ->
      ...
      | Some result ->
          Printf.printf "✗ 中文数字 '%s': expected %s, got %s\n" input (Utils.token_to_string expected)
            (Utils.token_to_string result)
      | None ->
          Printf.printf "✗ 中文数字 '%s': expected %s, got None\n" input
            (Utils.token_to_string expected))
    test_cases
...
let test_batch_conversion () =
  ...
  List.iter2
    (fun input result -> Printf.printf "  '%s' -> %s\n" input (Utils.token_to_string result))
    input_list results

(** Token工具函数测试 *)
let test_utility_functions () =
  let test_tokens =
    [ BasicKeyword `Let; IntToken 42; OperatorToken `Plus; DelimiterToken `LeftParen; EOF ]
  in
