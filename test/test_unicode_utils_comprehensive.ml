...
let test_fullwidth_digit_basic () =
  (* 测试全角数字范围的定义 *)
  assert (FullwidthDigit.start_byte3 = 0x90); (* ０ *)
  assert (FullwidthDigit.end_byte3 = 0x99);   (* ９ *)
  
  (* 验证范围的合理性 *)
  assert (FullwidthDigit.start_byte3 <= FullwidthDigit.end_byte3);
  assert (FullwidthDigit.end_byte3 - FullwidthDigit.start_byte3 = 9); (* 0-9共10个数字 *)
...
let test_fullwidth_digit_detection () =
  ...
  (* 测试范围内的字节 - 应该被识别为全角数字 *)
  assert (FullwidthDigit.is_fullwidth_digit 0x90); (* ０ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x91); (* １ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x92); (* ２ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x93); (* ３ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x94); (* ４ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x95); (* ５ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x96); (* ６ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x97); (* ７ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x98); (* ８ *)
  assert (FullwidthDigit.is_fullwidth_digit 0x99); (* ９ *)
...
let test_fullwidth_digit_boundaries () =
  ...
  (* 范围边界的测试 *)
  assert (FullwidthDigit.is_fullwidth_digit 0x90);  (* 下边界，应该为真 *)
  assert (FullwidthDigit.is_fullwidth_digit 0x99);  (* 上边界，应该为真 *)
  
  (* 超出范围的测试 *)
  assert (not (FullwidthDigit.is_fullwidth_digit 0x8F)); (* 下边界-1，应该为假 *)
  assert (not (FullwidthDigit.is_fullwidth_digit 0x9A)); (* 上边界+1，应该为假 *)
  
  (* 更远范围的测试 *)
  assert (not (FullwidthDigit.is_fullwidth_digit 0x00)); (* 最小值 *)
  assert (not (FullwidthDigit.is_fullwidth_digit 0xFF)); (* 最大值 *)
  assert (not (FullwidthDigit.is_fullwidth_digit 0x30)); (* ASCII '0' *)
  assert (not (FullwidthDigit.is_fullwidth_digit 0x39)); (* ASCII '9' *)
...
let test_correctness_validation () =
  ...
  let fullwidth_digit_count = 
    List.fold_left (fun acc byte3 ->
      if FullwidthDigit.is_fullwidth_digit byte3 then acc + 1 else acc
    ) 0 (List.init 256 (fun i -> i))
  in
  ...
  (* 验证前缀检查的唯一性 *)
  let prefix_values = [
    Prefix.chinese_punctuation;
    Prefix.chinese_operator;
    Prefix.arrow_symbol;
    Prefix.fullwidth;
  ] in
