(** 韵律数据工具模块全面测试覆盖
    ...
    本测试模块为 rhyme_data_utils.ml 提供全面的测试覆盖，
    确保长函数重构过程中的功能安全性。
    
    Author: Echo, 测试工程师代理
    Fix #1460 - Phase 2.1 韵律配置模块重构测试保障 *)
...
module TestHelpers = struct
  ...
  let test_character_group_loader group_name =
    if group_name = "测试组" then ["测试"; "字符"; "组"]
    else ["默认"; "字符"]
end
...
module RhymeDataTypeTests = struct
  ...
  let test_rhyme_category_types () =
    ...
    let category_count = List.fold_left (fun acc cat ->
      match cat with
      | PingSheng | ZeSheng | ShangSheng | QuSheng | RuSheng -> acc + 1
    ) 0 categories in
    check int "韵律分类模式匹配正确" 5 category_count

  (** 测试韵律组类型 *)
  let test_rhyme_group_types () =
    let groups = [AnRhyme; SiRhyme; TianRhyme; WangRhyme; QuRhyme; YuRhyme; 
                  HuaRhyme; FengRhyme; YueRhyme; XueRhyme; JiangRhyme; HuiRhyme; UnknownRhyme] in
    let group_strings = List.map string_of_rhyme_group groups in
    check int "韵律组数量" 13 (List.length group_strings);
    check bool "韵律组字符串非空" true 
      (List.for_all (fun s -> String.length s > 0) group_strings)
...
module RhymeFileConfigTests = struct
  ...
  let test_build_rhyme_file_path () =
    ...
    (* 测试不同韵律组的路径 *)
    let various_paths = List.map (build_rhyme_file_path config PingSheng) 
      [FengRhyme; YueRhyme; JiangRhyme; HuiRhyme] in
    check bool "不同韵律组产生不同路径" true 
      (List.length (List.sort_uniq String.compare various_paths) > 1)
...
module CharacterGroupTests = struct
  ...
  let test_assemble_rhyme_data () =
    ...
    check bool "所有条目有效" true 
      (List.for_all validate_rhyme_entry assembled)
end
...
module RhymeDataValidationTests = struct
  ...
  let test_deduplicate_rhyme_entries () =
    ...
    let entry3 = TestHelpers.create_test_entry "春" PingSheng TianRhyme in (* 重复 *)
    ...
    check bool "去重结果不包含重复" true 
      (List.length deduplicated < List.length entries)
