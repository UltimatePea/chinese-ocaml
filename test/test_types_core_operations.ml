...
module BasicTypeTests = struct
  let test_basic_type_creation () =
    ...
    List.iter (fun (typ, desc) ->
      check bool desc true (equal_typ typ typ)
    ) test_cases
  ...
  let test_type_pretty_printing () =
    ...
    let int_str = pp_typ Format.str_formatter IntType_T; Format.flush_str_formatter () in
    let string_str = pp_typ Format.str_formatter StringType_T; Format.flush_str_formatter () in
...
module CompoundTypeTests = struct
  let test_function_types () =
    ...
    check bool "复杂函数类型创建" true (equal_typ complex_fun (FunType_T (FunType_T (IntType_T, BoolType_T), StringType_T)))
  ...
  let test_tuple_types () =
    ...
    check bool "三元组类型" true (equal_typ triple_tuple (TupleType_T [IntType_T; StringType_T; BoolType_T]))
...
module TypeVariableTests = struct
  ...
  let test_type_variable_in_compound_types () =
    ...
    check bool "元组类型中的类型变量" true (equal_typ tuple_with_vars (TupleType_T [var_x; IntType_T; var_y]))
end
