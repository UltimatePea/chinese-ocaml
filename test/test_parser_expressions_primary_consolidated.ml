(** 骆言语法分析器基础表达式解析模块测试 - 整合版 增强版
    ...
    本测试模块验证 parser_expressions_primary_consolidated.ml 的完整功能。
    从基础接口测试扩展为全面的功能测试。
...
module TestUtils = struct
  ...
  (** 模拟的数组表达式解析器 *)
  let mock_parse_array_expression state =
    (ArrayExpr [], advance_parser state)

  (** 模拟的记录表达式解析器 *)
  let mock_parse_record_expression state =
    (RecordExpr [], advance_parser state)
...
let test_integer_literal () =
  let tokens = [
    TestUtils.make_token (IntToken 42) 1 1;
...
let test_string_literal () =
  let tokens = [
    TestUtils.make_token (StringToken "hello") 1 1;
    TestUtils.make_token EOF 1 2;
  ] in
  let state = TestUtils.create_test_state tokens in
  try
...
let test_boolean_literal () =
  let test_cases = [
    (BoolToken true, LitExpr (BoolLit true));
    (BoolToken false, LitExpr (BoolLit false));
  ] in
  List.iter (fun (token_lit, expected_expr) ->
    let tokens = [
...
let test_variable_expression () =
  let tokens = [
    TestUtils.make_token (QuotedIdentifierToken "variable_name") 1 1;
    TestUtils.make_token EOF 1 2;
  ] in
  let state = TestUtils.create_test_state tokens in
  try
...
let test_array_access_postfix () =
  ...
  let state = TestUtils.create_test_state tokens in
  try
    let (_result, _final_state) = parse_postfix_expr TestUtils.mock_parse_expression base_expr state in
...
let test_invalid_literal () =
  let tokens = [
    TestUtils.make_token (QuotedIdentifierToken "not_a_literal") 1 1;
    TestUtils.make_token EOF 1 2;
  ] in
  let state = TestUtils.create_test_state tokens in
  try
...
let test_mismatched_parentheses () =
  ...
  let state = TestUtils.create_test_state tokens in
  try
    let (_result, _final_state) = parse_primary_expr 
      TestUtils.mock_parse_expression 
      TestUtils.mock_parse_array_expression 
      TestUtils.mock_parse_record_expression 
      state in
...
let complex_tests = [
  test_case "嵌套括号表达式" `Quick test_nested_primary_expressions;
]
...
let () = run "Parser_expressions_primary_consolidated Enhanced Tests" [
  ("基础模块测试", basic_tests);
