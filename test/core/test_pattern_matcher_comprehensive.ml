...
let test_wildcard_pattern () =
  ...
  match result with Some _result_env -> () | None -> failwith "通配符模式应该匹配任何值"
...
let test_integer_literal_pattern () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> () | None -> failwith "整数字面量模式匹配失败"
...
let test_integer_literal_no_match () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> failwith "不同的整数值不应该匹配" | None -> ()
...
let test_string_literal_pattern () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> () | None -> failwith "字符串字面量模式匹配失败"
...
let test_boolean_literal_pattern () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> () | None -> failwith "布尔字面量模式匹配失败"
...
let test_unit_literal_pattern () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> () | None -> failwith "单元字面量模式匹配失败"
...
let test_empty_list_pattern () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> () | None -> failwith "空列表模式匹配失败"
...
let test_empty_list_no_match () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> failwith "空列表模式不应该匹配非空列表" | None -> ()
...
let test_constructor_pattern_no_args () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> () | None -> failwith "无参数构造器模式匹配失败"
...
let test_constructor_name_mismatch () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> failwith "不同名称的构造器不应该匹配" | None -> ()
...
let test_exception_pattern_no_payload () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> () | None -> failwith "无载荷异常模式匹配失败"
...
let test_polymorphic_variant_no_value () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> () | None -> failwith "无值多态变体模式匹配失败"
...
let test_polymorphic_variant_tag_mismatch () =
  ...
  let result = match_pattern pattern value env in
  match result with Some _ -> failwith "不同标签的多态变体不应该匹配" | None -> ()
...
let test_guard_evaluation_true () =
  ...
  match result with StringValue "guard通过" -> () | _ -> failwith "guard条件应该为真，应该执行第一个分支"
...
let test_guard_evaluation_false () =
  ...
  let result = execute_match env value branches simple_eval_expr in
  match result with StringValue "无guard" -> () | _ -> failwith "guard条件应该为假，应该执行第二个分支"

let test_guard_non_boolean_error () =
  (* 测试guard条件非布尔值错误 *)
  let env = create_test_env () in
  let guard_expr = int_lit 42 in
  (* 返回整数而不是布尔值 *)
  let branches =
...
let test_single_branch_execution () =
  ...
  let branches =
    [
      { pattern = VarPattern "匹配值"; guard = None; expr = BinaryOpExpr (var "匹配值", Add, int_lit 10) };
    ]
  in
  let value = IntValue 5 in
  let result = execute_match env value branches simple_eval_expr in
  match result with IntValue 15 -> () | _ -> failwith "单个分支执行失败"
...
let test_single_branch_with_guard () =
  ...
  let value = IntValue 5 in
  let result = execute_match env value branches simple_eval_expr in
  match result with StringValue "正数" -> () | _ -> failwith "带guard的分支执行失败"
...
let test_match_execution_first_branch () =
  ...
  let value = IntValue 5 in
  let result = execute_match env value branches simple_eval_expr in
  match result with StringValue "匹配5" -> () | _ -> failwith "应该匹配第一个分支"
...
let test_match_execution_second_branch () =
  ...
  let value = IntValue 10 in
  let result = execute_match env value branches simple_eval_expr in
  match result with StringValue "匹配10" -> () | _ -> failwith "应该匹配第二个分支"
...
let test_exception_match_execution () =
  ...
  let catch_branches =
    [
      ...
      {
        pattern = ConstructorPattern ("运行时错误", [ VarPattern "信息" ]);
        guard = None;
  ...
  match result with StringValue "运行时错误: 除零" -> () | _ -> failwith "异常匹配执行失败"
...
let test_register_polymorphic_variant_constructors () =
  ...
  (match lookup_var test_env "测试变量" with IntValue 42 -> () | _ -> failwith "bind_var基本功能失败");
  ...
  if env_size_after <= env_size_before then
    failwith
      ("register_constructors未增加环境大小: 之前=" ^ string_of_int env_size_before ^ " 之后="
     ^ string_of_int env_size_after);
