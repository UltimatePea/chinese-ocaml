(** AST模块全面测试覆盖率提升 - Fix #1817
    ...
    将AST模块测试覆盖率从0%提升到80%+ 全面测试所有类型、构造器和辅助函数
...
let test_tone_constraints () =
  ...
  check bool "特定模式构造" true
    (match SpecificPattern [ LevelTone ] with SpecificPattern _ -> true | _ -> false)
...
let test_literals () =
  let literals =
    [ IntLit 42; FloatLit 3.14; StringLit "你好"; BoolLit true; BoolLit false; UnitLit ]
  in
...
let test_advanced_expressions () =
  ...
  (* 测试匹配表达式 *)
  let match_branch = { pattern = VarPattern "x"; guard = None; expr = VarExpr "x" } in
  let match_expr = MatchExpr (VarExpr "value", [ match_branch ]) in
  check bool "匹配表达式" true (match_expr = match_expr);

  (* 测试记录表达式 *)
  let record_expr =
    RecordExpr [ ("name", LitExpr (StringLit "Alice")); ("age", LitExpr (IntLit 25)) ]
  in
...
let test_poetry_expressions () =
  ...
  (* 测试对偶结构表达式 *)
  let parallel_expr =
    ParallelStructureExpr (LitExpr (StringLit "对酒当歌"), LitExpr (StringLit "人生几何"))
  in
...
(** 测试程序类型 *)
let test_program () =
  let prog = [ LetStmt ("x", LitExpr (IntLit 42)); ExprStmt (VarExpr "x") ] in

  check int "程序语句数量" 2 (List.length prog);

  (* 测试程序相等性 *)
  let prog2 = [ LetStmt ("x", LitExpr (IntLit 42)); ExprStmt (VarExpr "x") ] in
...
let () =
  run "AST 全面测试覆盖率提升 - Fix #1817"
    [
      ...
      ("模式匹配", [ test_case "模式类型" `Quick test_patterns ]);
      ("类型系统", [ test_case "类型表达式" `Quick test_type_expressions ]);
      ( "表达式",
        [
      ...
      ("语句", [ test_case "基础语句" `Quick test_statements ]);
      ( "辅助函数和程序",
        [ test_case "辅助函数" `Quick test_helper_functions; test_case "程序类型" `Quick test_program ] );
    ]
