(** 骆言解释器核心深度测试 - Core Interpreter Deep Testing
    ...
    针对Issue #1695的核心业务逻辑深度测试策略 专注于interpreter.ml的关键算法和执行引擎验证 *)
...
module ProgramExecutionEngine = struct
  ...
  let test_program_execution_state_isolation () =
    ...
    (* 第一个程序应该成功 *)
    (match execute_program prog1 with Ok _ -> () | Error msg -> fail ("第一个程序失败: " ^ msg));
...
module ExpressionEvaluationEngine = struct
  let test_eval_expr_literal_handling () =
    ...
    List.iter
      (fun (program, expected) ->
        ...
        | Ok result ->
            fail
              (Printf.sprintf "字面量求值错误: 期望 %s，得到 %s" (value_to_string expected)
  ...
  let test_eval_expr_variable_resolution () =
    ...
    List.iter
      (fun (program, expected) ->
        ...
        | Ok result ->
            fail
              (Printf.sprintf "变量解析错误: 期望 %s，得到 %s" (value_to_string expected)
                 (value_to_string result))
  ...
  let test_eval_expr_binary_operations_arithmetic () =
    ...
    List.iter
      (fun (program, expected) ->
        ...
        | Ok result ->
            fail
              (Printf.sprintf "算术运算错误: 期望 %s，得到 %s" (value_to_string expected)
                 (value_to_string result))
  ...
  let test_eval_expr_binary_operations_comparison () =
    ...
    List.iter
      (fun (program, expected) ->
        ...
        | Ok result ->
            fail
              (Printf.sprintf "比较运算错误: 期望 %s，得到 %s" (value_to_string expected)
                 (value_to_string result))
  ...
  let test_eval_expr_error_propagation () =
    (* 测试表达式求值中的错误传播机制 *)
    let error_program =
      [ ExprStmt (BinaryOpExpr (VarExpr "undefined", Add, LitExpr (IntLit 5))) ]
    in
    match execute_program error_program with
    | Error _ -> () (* 预期的错误传播 *)
...
module InterpreterInterface = struct
  ...
  let test_interactive_eval_environment_persistence () =
    ...
    check bool "interactive_eval环境传递" true (List.length new_env >= 0)
  (* 环境是有效的 *)
end
...
module EnvironmentScopeAndBinding = struct
  ...
  let test_variable_shadowing_behavior () =
    ...
    let program =
      [
        ...
        LetStmt ("shadow_var", LitExpr (IntLit 200));
        (* 遮蔽前一个 *)
  ...
  let test_environment_persistence_across_statements () =
    ...
    let stmt2 =
      LetStmt ("depend_var", BinaryOpExpr (VarExpr "persist_var", Add, LitExpr (IntLit 8)))
    in
...
module StatementExecutionAndSideEffects = struct
  let test_let_statement_binding_correctness () =
    ...
    let env = [] in
    let stmt =
      LetStmt ("binding_test", BinaryOpExpr (LitExpr (IntLit 15), Mul, LitExpr (IntLit 3)))
    in
  ...
  let test_expression_statement_evaluation () =
    (* 测试表达式语句的求值但不绑定 *)
    let env = [ ("existing_var", IntValue 10) ] in
    let stmt = ExprStmt (BinaryOpExpr (VarExpr "existing_var", Add, LitExpr (IntLit 5))) in
    ...
    let program =
      [
        ...
        LetStmt
          ( "final",
            BinaryOpExpr
              (BinaryOpExpr (VarExpr "base", Add, VarExpr "derived"), Sub, LitExpr (IntLit 10)) );
...
module ErrorHandlingAndRecovery = struct
  let test_program_error_recovery () =
    (* 测试程序级错误恢复机制 *)
    let error_program =
      [ ExprStmt (VarExpr "undefined1"); ExprStmt (LitExpr (IntLit 42)) (* 这行不应该执行 *) ]
    in
    match execute_program error_program with
    | Error msg -> check bool "错误消息完整" true (String.length msg > 0)
    | Ok _ -> fail "错误程序不应该成功"

  let test_expression_error_handling () =
    (* 测试表达式级错误处理 *)
    let error_program =
      [ ExprStmt (BinaryOpExpr (VarExpr "undefined", Add, LitExpr (IntLit 1))) ]
    in
    match execute_program error_program with Error _ -> () (* 预期的错误 *) | Ok _ -> fail "表达式错误应该被捕获"

  let test_nested_error_propagation () =
    (* 测试嵌套错误传播机制 *)
    let nested_error_program =
      [
        ExprStmt
          (BinaryOpExpr
             (BinaryOpExpr (VarExpr "undefined", Add, LitExpr (IntLit 1)), Mul, LitExpr (IntLit 2)));
      ]
    in
  ...
  let test_error_message_quality () =
    ...
    match execute_program program with
    | Error msg -> check bool "错误消息包含相关信息" true (String.length msg > 5)
    | Ok _ -> fail "应该产生错误"
end
...
module ChineseLanguageFeatures = struct
  let test_chinese_variable_names () =
    (* 测试中文变量名支持 *)
    let program = [ LetStmt ("中文变量", LitExpr (IntLit 42)); ExprStmt (VarExpr "中文变量") ] in
    match execute_program program with
    | Ok (IntValue 42) -> ()
...
module PerformanceAndBoundaryConditions = struct
  let test_large_expression_evaluation () =
    ...
    let rec build_large_expr n acc =
      if n <= 0 then acc else build_large_expr (n - 1) (BinaryOpExpr (acc, Add, LitExpr (IntLit 1)))
    in
  ...
  let test_deep_nested_expressions () =
    ...
    in
    let deep_expr = build_nested_expr 10 in
    (* 2^10 = 1024 *)
  ...
  let test_large_program_execution () =
    (* 测试大型程序的执行能力 *)
    let large_program =
      List.init 50 (fun i -> LetStmt (Printf.sprintf "var_%d" i, LitExpr (IntLit i)))
      @ [ ExprStmt (VarExpr "var_49") ]
    in
    match execute_program large_program with
    | Ok (IntValue 49) -> ()
  ...
  let test_extreme_boundary_conditions () =
    ...
    List.iter
      (fun (program, expected) ->
        ...
        | Ok result ->
            fail
              (Printf.sprintf "边界条件错误: 期望 %s，得到 %s" (value_to_string expected)
                 (value_to_string result))
...
module IntegrationScenarios = struct
  ...
  let test_error_handling_across_layers () =
    ...
    check bool "解释器接口行为一致性" true (interpret_result = quiet_result && quiet_result = test_result)
...
module InterpreterAlgorithmValidation = struct
  let test_operator_precedence_evaluation () =
    ...
    List.iter
      (fun (program, expected) ->
        ...
        | Ok result ->
            fail
              (Printf.sprintf "运算符优先级错误: 期望 %s，得到 %s" (value_to_string expected)
                 (value_to_string result))
  ...
  let test_division_by_zero_handling () =
    (* 测试除零错误的处理 *)
    let zero_div_program =
      [ ExprStmt (BinaryOpExpr (LitExpr (IntLit 42), Div, LitExpr (IntLit 0))) ]
    in
  ...
  let test_type_consistency_across_operations () =
    ...
    let type_programs =
      [
        (* 字符串操作 *)
        ( [ LetStmt ("str_var", LitExpr (StringLit "hello")); ExprStmt (VarExpr "str_var") ],
          StringValue "hello" );
        (* 布尔操作 *)
        ( [ LetStmt ("bool_var", LitExpr (BoolLit true)); ExprStmt (VarExpr "bool_var") ],
          BoolValue true );
        (* 整数计算 *)
        ( [
            LetStmt ("int_var", BinaryOpExpr (LitExpr (IntLit 10), Add, LitExpr (IntLit 5)));
            ExprStmt (VarExpr "int_var");
          ],
          IntValue 15 );
      ]
    in
    ...
    List.iter
      (fun (program, expected) ->
        ...
        | Ok result ->
            fail
              (Printf.sprintf "类型一致性错误: 期望 %s，得到 %s" (value_to_string expected)
                 (value_to_string result))
    ...
    [
      ...
      ( "表达式求值引擎深度",
        [
          ...
          test_case "算术运算实现" `Quick
            ExpressionEvaluationEngine.test_eval_expr_binary_operations_arithmetic;
          test_case "比较运算逻辑" `Quick
            ExpressionEvaluationEngine.test_eval_expr_binary_operations_comparison;
          test_case "复杂嵌套表达式" `Quick
            ExpressionEvaluationEngine.test_eval_expr_complex_nested_expressions;
          test_case "错误传播机制" `Quick ExpressionEvaluationEngine.test_eval_expr_error_propagation;
        ] );
      ( "解释器接口函数",
        [
          test_case "interpret输出行为" `Quick
            InterpreterInterface.test_interpret_function_output_behavior;
          test_case "interpret错误行为" `Quick
            InterpreterInterface.test_interpret_function_error_behavior;
          ...
          test_case "interactive_eval环境持久化" `Quick
            InterpreterInterface.test_interactive_eval_environment_persistence;
        ] );
      ( "环境变量作用域和绑定",
        [
          ...
          test_case "环境跨语句持久化" `Quick
            EnvironmentScopeAndBinding.test_environment_persistence_across_statements;
        ] );
      ( "语句执行和副作用",
        [
          test_case "Let语句绑定正确性" `Quick
            StatementExecutionAndSideEffects.test_let_statement_binding_correctness;
          test_case "表达式语句求值" `Quick
            StatementExecutionAndSideEffects.test_expression_statement_evaluation;
          test_case "语句错误传播" `Quick
            StatementExecutionAndSideEffects.test_statement_error_propagation;
      ...
      ( "性能和边界条件",
        [
          test_case "大型表达式求值" `Quick
            PerformanceAndBoundaryConditions.test_large_expression_evaluation;
          ...
          test_case "极端边界条件" `Quick
            PerformanceAndBoundaryConditions.test_extreme_boundary_conditions;
        ] );
      ( "集成测试场景",
        [
      ...
      ( "解释器算法正确性验证",
        [
          test_case "运算符优先级求值" `Quick
            InterpreterAlgorithmValidation.test_operator_precedence_evaluation;
          test_case "短路求值行为" `Quick InterpreterAlgorithmValidation.test_short_circuit_evaluation;
          test_case "递归表达式求值" `Quick
            InterpreterAlgorithmValidation.test_recursive_expression_evaluation;
          test_case "除零错误处理" `Quick InterpreterAlgorithmValidation.test_division_by_zero_handling;
          test_case "类型一致性验证" `Quick
            InterpreterAlgorithmValidation.test_type_consistency_across_operations;
        ] );
    ]
