...
module TestHelpers = struct
  ...
  let check_unify_success msg typ1 typ2 =
    ...
    with _ -> check bool msg false true
  ...
  let check_unify_fails msg typ1 typ2 =
    ...
    with _ -> check bool msg true true (* 应该失败 *)
  ...
  let check_unify_result msg typ1 typ2 expected_subst_effects =
    try
      let subst = unify typ1 typ2 in
      List.iter
        (fun (var_name, expected_type) ->
          let var_type = TypeVar_T var_name in
          let result = apply_subst subst var_type in
          check bool (msg ^ " - 变量" ^ var_name ^ "统一正确") (equal_typ result expected_type) true)
        expected_subst_effects
    with _ -> check bool (msg ^ " - 统一应该成功") false true
end
...
module FunctionTypeUnificationTests = struct
  ...
  let test_function_types_with_variables () =
    ...
    check_unify_result "函数类型with变量统一" func1 func2 [ ("x", IntType_T); ("y", StringType_T) ]
  ...
  let test_nested_function_types () =
    ...
    check_unify_result "嵌套函数类型统一" func1 func2 [ ("a", StringType_T); ("b", BoolType_T) ]
...
module TupleTypeUnificationTests = struct
  ...
  let test_tuples_with_variables () =
    ...
    check_unify_result "元组with变量统一" tuple1 tuple2 [ ("x", IntType_T); ("y", BoolType_T) ]
...
module RecordTypeUnificationTests = struct
  ...
  let test_record_with_variables () =
    ...
    check_unify_result "记录类型with变量统一" record1 record2 [ ("a", IntType_T); ("b", BoolType_T) ]
...
module ComplexTypeUnificationTests = struct
  ...
  let test_deeply_nested_types () =
    ...
    let complex1 =
      FunType_T
        (TupleType_T [ ListType_T var_x; ArrayType_T var_y ], ConstructType_T ("Option", [ var_x ]))
    in
    ...
    check_unify_result "深层嵌套类型统一" complex1 complex2 [ ("x", IntType_T); ("y", StringType_T) ]
  ...
  let test_partial_type_information () =
    ...
    check_unify_result "部分类型信息统一" partial1 partial2 [ ("x", IntType_T); ("y", StringType_T) ]
end
...
module EdgeCaseTests = struct
  ...
  let test_occurs_check_complex () =
    ...
    (* 复杂的occurs check场景 *)
    let complex_cycle = FunType_T (var_x, TupleType_T [ ListType_T var_x; ArrayType_T var_x ]) in
  ...
  let test_empty_substitution_identity () =
    (* 测试空替换的恒等性 *)
    let complex_type = FunType_T (TupleType_T [ IntType_T; StringType_T ], ListType_T BoolType_T) in
...
let () =
  run "类型统一算法专项测试"
    [
      ...
      ( "类型变量统一",
        [
          test_case "类型变量与具体类型统一" `Quick
            TypeVariableUnificationTests.test_type_variable_with_concrete_type;
          test_case "类型变量间统一" `Quick
            TypeVariableUnificationTests.test_type_variable_with_type_variable;
      ...
      ( "函数类型统一",
        [
          ...
          test_case "函数类型with变量统一" `Quick
            FunctionTypeUnificationTests.test_function_types_with_variables;
          test_case "嵌套函数类型统一" `Quick FunctionTypeUnificationTests.test_nested_function_types;
          test_case "不兼容函数类型统一失败" `Quick
            FunctionTypeUnificationTests.test_incompatible_function_types;
        ] );
      ( "元组类型统一",
        [
      ...
      ( "列表类型统一",
        [
          ...
          test_case "不兼容列表元素类型统一失败" `Quick
            ListTypeUnificationTests.test_incompatible_list_element_types;
        ] );
      ( "数组类型统一",
        [
          ...
          test_case "不兼容数组元素类型统一失败" `Quick
            ArrayTypeUnificationTests.test_incompatible_array_element_types;
        ] );
      ( "构造类型统一",
        [
          test_case "相同构造类型统一" `Quick ConstructTypeUnificationTests.test_identical_construct_types;
          test_case "构造类型with变量统一" `Quick
            ConstructTypeUnificationTests.test_construct_type_with_variables;
          test_case "不同构造器名称统一失败" `Quick
            ConstructTypeUnificationTests.test_different_constructor_names;
          test_case "不同参数数量构造类型统一失败" `Quick
            ConstructTypeUnificationTests.test_different_parameter_count;
        ] );
      ( "记录类型统一",
        [
      ...
      ( "复杂类型统一",
        [
          ...
          test_case "递归结构兼容性" `Quick
            ComplexTypeUnificationTests.test_recursive_structure_compatibility;
