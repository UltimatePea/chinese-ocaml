...
let test_make_binary_op_comprehensive () =
  ...
  TestUtils.check_expr_equality "变量运算"
    (BinaryOpExpr (VarExpr "x", Add, VarExpr "y")) var_expr

(** 专项测试make_call辅助函数 *)
let test_make_call_comprehensive () =
  (* 测试无参数函数调用 *)
  let no_args_call = make_call (make_var "get_random") [] in
  TestUtils.check_expr_equality "无参数函数调用"
    (FunCallExpr (VarExpr "get_random", [])) no_args_call;
  
  (* 测试单参数函数调用 *)
  let single_arg_call = make_call (make_var "print") [make_string "Hello"] in
  TestUtils.check_expr_equality "单参数函数调用"
    (FunCallExpr (VarExpr "print", [LitExpr (StringLit "Hello")])) single_arg_call;
  
  (* 测试多参数函数调用 *)
  let multi_args_call = make_call (make_var "add3") [make_int 1; make_int 2; make_int 3] in
  TestUtils.check_expr_equality "多参数函数调用"
    (FunCallExpr (VarExpr "add3", [LitExpr (IntLit 1); LitExpr (IntLit 2); LitExpr (IntLit 3)])) multi_args_call;
  
  (* 测试不同类型参数的函数调用 *)
  let mixed_args_call = make_call (make_var "process") [
    make_string "input";
    make_int 42;
    make_bool true;
    make_var "config"
  ] in
  let expected_mixed = FunCallExpr (VarExpr "process", [
    LitExpr (StringLit "input");
    LitExpr (IntLit 42);
  ...
  (* 测试嵌套函数调用 *)
  let nested_call = make_call 
    (make_var "outer_func")
    [make_call (make_var "inner_func") [make_int 10]] in
  let expected_nested_call = FunCallExpr (VarExpr "outer_func", [
    FunCallExpr (VarExpr "inner_func", [LitExpr (IntLit 10)])
  ]) in
  TestUtils.check_expr_equality "嵌套函数调用" expected_nested_call nested_call;
  
  (* 测试表达式作为函数的调用 *)
  let expr_func_call = make_call 
    (make_binary_op (make_var "func_map") Add (make_int 0))
    [make_string "arg"] in
  let expected_expr_func = FunCallExpr (
    BinaryOpExpr (VarExpr "func_map", Add, LitExpr (IntLit 0)),
    [LitExpr (StringLit "arg")]
  ) in
  ...
  TestUtils.check_expr_equality "中文函数名调用"
    (FunCallExpr (VarExpr "计算", [LitExpr (IntLit 5); LitExpr (IntLit 10)])) chinese_func_call

(** 测试辅助函数组合使用 *)
let test_helper_functions_combination () =
  (* 测试辅助函数相互组合 *)
  let combined_expr1 = make_call 
    (make_var "calculate")
    [make_binary_op (make_int 10) Mul (make_int 5); make_bool true] in
  let expected_combined1 = FunCallExpr (VarExpr "calculate", [
    BinaryOpExpr (LitExpr (IntLit 10), Mul, LitExpr (IntLit 5));
    LitExpr (BoolLit true)
  ]) in
  TestUtils.check_expr_equality "辅助函数组合1" expected_combined1 combined_expr1;
  
  (* 测试复杂组合表达式 *)
  let complex_expr = make_binary_op
    (make_call (make_var "func1") [make_int 1; make_string "test"])
    Add
    (make_call (make_var "func2") [make_bool false]) in
  let expected_complex = BinaryOpExpr (
  ...
  (* 测试深层嵌套组合 *)
  let deep_nested = make_call (make_var "outer") [
    make_call (make_var "middle") [
      make_call (make_var "inner") [
        make_binary_op (make_int 1) Add (make_int 2)
      ]
    ]
  ] in
  let expected_deep = FunCallExpr (VarExpr "outer", [
    FunCallExpr (VarExpr "middle", [
      FunCallExpr (VarExpr "inner", [
...
let test_helper_functions_edge_cases () =
  ...
  (* 测试非常复杂的嵌套 *)
  let ultra_complex = make_binary_op
    (make_call 
      (make_var "函数A") 
      [make_binary_op (make_int 10) Div (make_int 2); make_string "参数"])
    Concat
    (make_string (show_expr (make_bool true))) in
  
  (* 验证这个复杂表达式可以正确构造 *)
  check bool "超复杂表达式构造成功" true (match ultra_complex with
...
let () =
  run "AST模块辅助函数专项测试 - Fix #2147"
    [
      ( "make_int函数测试",
        [
          test_case "make_int全面测试" `Quick test_make_int_comprehensive;
        ] );
      ( "make_string函数测试",
        [
          test_case "make_string全面测试" `Quick test_make_string_comprehensive;
        ] );
      ( "make_bool函数测试",
        [
          test_case "make_bool全面测试" `Quick test_make_bool_comprehensive;
        ] );
      ( "make_var函数测试",
        [
          test_case "make_var全面测试" `Quick test_make_var_comprehensive;
        ] );
      ( "make_binary_op函数测试",
        [
      ...
      ( "make_call函数测试",
        [
          test_case "make_call全面测试" `Quick test_make_call_comprehensive;
        ] );
      ( "辅助函数组合测试",
        [
