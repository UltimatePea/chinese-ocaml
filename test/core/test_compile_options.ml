...
module Test_helpers = struct
  (** 验证编译选项结构的辅助函数 *)
  let check_option_field field_name expected actual = check bool field_name expected actual

  let check_option_string field_name expected actual = check string field_name expected actual
...
let test_compile_options_updates () =
  ...
  let updated_multiple =
    { base_options with show_tokens = true; show_ast = true; log_level = "verbose" }
  in
...
let test_predefined_options_differences () =
  ...
  (* default vs test 的差异 *)
  check bool "default和test的quiet_mode相同" (default_opts.quiet_mode = test_opts.quiet_mode) true;
  check bool "default和test的log_level不同" (default_opts.log_level <> test_opts.log_level) true;

  (* default vs quiet 的差异 *)
  check bool "default和quiet的quiet_mode不同" (default_opts.quiet_mode <> quiet_opts.quiet_mode) true;
  check bool "default和quiet的log_level不同" (default_opts.log_level <> quiet_opts.log_level) true;

  (* test vs quiet 的差异 *)
  check bool "test和quiet的quiet_mode不同" (test_opts.quiet_mode <> quiet_opts.quiet_mode) true;
  check bool "test和quiet的log_level相同" (test_opts.log_level = quiet_opts.log_level) true
...
let test_option_combinations () =
  ...
  check bool "调试模式组合有效" true
    (debug_options.show_tokens && debug_options.show_ast && debug_options.show_types
  ...
  (* 测试静默检查模式组合 *)
  let quiet_check_options = { quiet_options with check_only = true } in

  check bool "静默检查模式组合有效" true
    (quiet_check_options.quiet_mode && quiet_check_options.check_only
    && quiet_check_options.log_level = "quiet")

(** 测试边界情况和异常选项 *)
let test_edge_cases () =
  (* 测试极端的日志级别组合 *)
  let verbose_quiet = { default_options with quiet_mode = true; log_level = "verbose" } in

  check bool "矛盾选项组合可以创建" true (verbose_quiet.quiet_mode && verbose_quiet.log_level = "verbose");
...
let test_default_values_rationality () =
  ...
  check bool "默认不显示调试信息（合理）" true
    ((not opts.show_tokens) && (not opts.show_ast) && not opts.show_types);
