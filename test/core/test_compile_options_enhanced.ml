...
module TestHelpers = struct
  ...
  let make_custom_options ?(show_tokens = false) ?(show_ast = false) ?(show_types = false)
      ?(check_only = false) ?(quiet_mode = false) ?(filename = None) ?(recovery_mode = true)
      ?(log_level = "normal") ?(compile_to_c = false) ?(c_output_file = None) () =
    {
      show_tokens;
      show_ast;
      show_types;
...
module OptionsConstructionTests = struct
  ...
  let test_custom_options_construction () =
    let custom_options =
      make_custom_options ~show_tokens:true ~show_ast:true ~log_level:"debug" ~compile_to_c:true ()
    in
  ...
  let test_partial_options_modification () =
    ...
    let modified_options =
      { base_options with show_tokens = true; quiet_mode = true; log_level = "verbose" }
    in
  ...
  let test_c_output_file_options () =
    let with_c_output =
      { default_options with compile_to_c = true; c_output_file = Some "output.c" }
    in
...
module OptionsCombinationTests = struct
  ...
  let test_quiet_mode_combination () =
    let quiet_options = { default_options with quiet_mode = true; log_level = "error" } in
  ...
  let test_check_only_combination () =
    let check_only_options = { default_options with check_only = true; compile_to_c = false } in
