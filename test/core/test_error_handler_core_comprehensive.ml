(** 错误处理核心模块综合测试
    ...
    本测试文件针对error_handler_core.ml模块进行全面测试。 该模块是编译器错误处理的核心组件，负责错误的捕获、处理和恢复。
...
let create_test_error severity msg line col =
  {
    severity;
    message = msg;
    location = { line; column = col; filename = "test.ly"; offset = (line * 100) + col };
...
let test_handle_error_batch _ =
  ...
  List.iter (fun result -> assert_bool "批量处理结果应该有效" (validate_error_result result)) results

(** ==================== 错误恢复策略测试 ==================== *)

let test_error_recovery_skip_token _ =
  (* 测试跳过Token恢复策略 *)
  let config = { (create_test_handler_config ()) with recovery_strategies = [ SkipToken ] } in
  let handler = create_error_handler config in
  let error = create_test_error Error "跳过Token测试" 5 10 in

  let result = handle_error_with_recovery handler error in
  match result with
  | HandlerRecovered (strategy, _) -> assert_equal SkipToken strategy
  | _ -> assert_failure "应该执行跳过Token恢复"

let test_error_recovery_insert_token _ =
  (* 测试插入Token恢复策略 *)
  let config = { (create_test_handler_config ()) with recovery_strategies = [ InsertToken ] } in
  let handler = create_error_handler config in
  let error = create_test_error Error "插入Token测试" 8 15 in

  let result = handle_error_with_recovery handler error in
  match result with
  | HandlerRecovered (strategy, _) -> assert_equal InsertToken strategy
  | _ -> assert_failure "应该执行插入Token恢复"

let test_error_recovery_replace_token _ =
  (* 测试替换Token恢复策略 *)
  let config = { (create_test_handler_config ()) with recovery_strategies = [ ReplaceToken ] } in
  let handler = create_error_handler config in
  let error = create_test_error Error "替换Token测试" 12 8 in

  let result = handle_error_with_recovery handler error in
  match result with
  | HandlerRecovered (strategy, _) -> assert_equal ReplaceToken strategy
  ...
  List.iter
    (fun result ->
      ...
      | HandlerRecovered (strategy, _) ->
          assert_bool "恢复策略应该是有效的" (List.mem strategy [ SkipToken; InsertToken; ReplaceToken ])
...
let test_error_priority_handling _ =
  ...
  let high_priority_error = { (create_test_error Error "高优先级错误" 1 1) with priority = High } in
  let low_priority_error = { (create_test_error Error "低优先级错误" 2 2) with priority = Low } in
...
let test_error_severity_filtering _ =
  (* 测试错误严重性过滤 *)
  let config = { (create_test_handler_config ()) with min_severity = Warning } in
  let handler = create_error_handler config in
...
let test_error_context_capture _ =
  (* 测试错误上下文捕获 *)
  let config = { (create_test_handler_config ()) with include_context = true; context_lines = 2 } in
  let handler = create_error_handler config in

  let source_lines = [ "第一行代码"; "第二行代码"; "第三行有错误的代码"; "第四行代码"; "第五行代码" ] in
...
let test_error_context_line_numbers _ =
  (* 测试错误上下文行号 *)
  let config = { (create_test_handler_config ()) with include_context = true; context_lines = 1 } in
  let handler = create_error_handler config in
...
let test_error_chain_handling _ =
  ...
  let child_error1 = { (create_test_error Error "子错误1" 2 2) with parent_error = Some root_error } in
  let child_error2 = { (create_test_error Error "子错误2" 3 3) with parent_error = Some root_error } in
...
let test_max_errors_limit _ =
  (* 测试最大错误数限制 *)
  let config = { (create_test_handler_config ()) with max_errors = 3 } in
  let handler = create_error_handler config in
...
let test_stop_on_first_error _ =
  (* 测试首次错误停止 *)
  let config = { (create_test_handler_config ()) with stop_on_first_error = true } in
  let handler = create_error_handler config in
  ...
  match result2 with HandlerError _ -> () (* 预期行为 *) | _ -> assert_failure "第二个错误应该被拒绝处理"
...
let test_integration_complete_workflow _ =
  ...
  (* 1. 设置源代码上下文 *)
  let source_code = [ "定义 变量 为 整数"; "设置 变量 为 \"错误类型\""; (* 类型错误 *) "输出 变量" ] in
  set_source_context handler source_code;

  (* 2. 处理类型错误 *)
  let type_error =
    { (create_test_error Error "类型不匹配：期望整数，得到字符串" 2 8) with suggestions = [ "检查变量类型"; "使用正确的赋值" ] }
  in
  ...
  | HandlerRecovered (strategy, processed_error) ->
      ...
      assert_bool "应选择合适的恢复策略" (List.mem strategy [ SkipToken; InsertToken; ReplaceToken ])
  ...
  let file1_error =
    {
      ...
      location = { line = 10; column = 5; filename = "main.ly"; offset = 1005 };
    }
  in
  ...
  let file2_error =
    {
      ...
      location = { line = 20; column = 15; filename = "utils.ly"; offset = 2015 };
    }
  in
...
let suite =
  "Error_handler_core comprehensive tests"
  >::: [
         (* 错误处理器初始化测试 *)
