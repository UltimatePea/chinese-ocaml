(** AST模块高级测试覆盖率提升 - Fix #2124
    ...
    专注于进一步提升ast.ml核心模块测试覆盖率从18.27%到80%+
    新增测试场景：
...
let test_advanced_type_expressions () =
  ...
  check bool "高阶函数类型构造正确" true (match fun_type3 with FunType (BaseTypeExpr IntType, _) -> true | _ -> false)
...
let test_advanced_type_definitions () =
  ...
  (* 多态变体类型定义测试 *)
  let poly_variant_def1 = PolymorphicVariantTypeDef [("Variant1", Some int_type); ("Variant2", None)] in
  let poly_variant_def2 = PolymorphicVariantTypeDef [("Variant1", Some int_type); ("Variant2", None)] in
...
let test_advanced_expression_types () =
  ...
  check bool "字段访问表达式构造正确" true (match field_access with FieldAccessExpr (_, _) -> true | _ -> false);
  
  (* 记录更新表达式测试 *)
  let record_update = RecordUpdateExpr (var_expr, [("field1", int_expr)]) in
  check bool "记录更新表达式构造正确" true (match record_update with RecordUpdateExpr (_, _) -> true | _ -> false);
  ...
  check bool "数组访问表达式构造正确" true (match array_access with ArrayAccessExpr (_, _) -> true | _ -> false);
  
  (* 数组更新表达式测试 *)
  let array_update = ArrayUpdateExpr (array_expr, int_expr, make_int 100) in
  check bool "数组更新表达式构造正确" true (match array_update with ArrayUpdateExpr (_, _, _) -> true | _ -> false)
...
let test_reference_and_exception_expressions () =
  ...
  let branch = { pattern = pattern; guard = None; expr = int_expr } in
  ...
  check bool "构造器表达式构造正确" true (match constructor_expr with ConstructorExpr (_, _) -> true | _ -> false)
...
let test_module_and_functor_expressions () =
  ...
  check bool "模块访问表达式构造正确" true (match module_access with ModuleAccessExpr (_, _) -> true | _ -> false);
  
  (* 函子调用表达式测试 *)
  let functor_call = FunctorCallExpr (var_expr, var_expr) in
  check bool "函子调用表达式构造正确" true (match functor_call with FunctorCallExpr (_, _) -> true | _ -> false);
  ...
  check bool "类型注解表达式构造正确" true (match type_annotation with TypeAnnotationExpr (_, _) -> true | _ -> false)
...
let test_polymorphic_variant_and_poetry_expressions () =
  ...
  check bool "带值多态变体表达式构造正确" true (match poly_variant1 with PolymorphicVariantExpr (_, Some _) -> true | _ -> false);
  check bool "无值多态变体表达式构造正确" true (match poly_variant2 with PolymorphicVariantExpr (_, None) -> true | _ -> false);
  
  (* 诗词注解表达式测试 *)
  let poetry_annotated = PoetryAnnotatedExpr (int_expr, FiveCharPoetry) in
  check bool "诗词注解表达式构造正确" true (match poetry_annotated with PoetryAnnotatedExpr (_, _) -> true | _ -> false);
  
  (* 对偶结构表达式测试 *)
  let parallel_structure = ParallelStructureExpr (int_expr, make_string "对偶") in
  check bool "对偶结构表达式构造正确" true (match parallel_structure with ParallelStructureExpr (_, _) -> true | _ -> false);
  ...
  check bool "押韵注解表达式构造正确" true (match rhyme_annotated with RhymeAnnotatedExpr (_, _) -> true | _ -> false);
  ...
  check bool "平仄注解表达式构造正确" true (match tone_annotated with ToneAnnotatedExpr (_, _) -> true | _ -> false);
  
  (* 韵律验证表达式测试 *)
  let meter_constraint = { character_count = 7; syllable_pattern = None; caesura_position = None; rhyme_scheme = None } in
  let meter_validated = MeterValidatedExpr (int_expr, meter_constraint) in
  check bool "韵律验证表达式构造正确" true (match meter_validated with MeterValidatedExpr (_, _) -> true | _ -> false)
...
let test_advanced_statement_types () =
  ...
  check bool "模块类型定义语句构造正确" true (match module_type_def with ModuleTypeDefStmt (_, _) -> true | _ -> false);
  ...
  check bool "带类型异常定义语句构造正确" true (match exception_def1 with ExceptionDefStmt (_, Some _) -> true | _ -> false);
  check bool "无类型异常定义语句构造正确" true (match exception_def2 with ExceptionDefStmt (_, None) -> true | _ -> false);
...
let test_complex_label_params_and_macros () =
  ...
  check bool "标签函数表达式构造正确" true (match labeled_fun with LabeledFunExpr (_, _) -> true | _ -> false);
  ...
  check bool "标签函数调用表达式构造正确" true (match labeled_call with LabeledFunCallExpr (_, _) -> true | _ -> false);
...
let test_helper_functions_comprehensive () =
  ...
  check bool "make_int创建正确的整数表达式" true (match int_expr with LitExpr (IntLit 100) -> true | _ -> false);
  
  (* 测试make_string函数 *)
  let string_expr = make_string "测试字符串" in
  check bool "make_string创建正确的字符串表达式" true (match string_expr with LitExpr (StringLit "测试字符串") -> true | _ -> false);
  
  (* 测试make_bool函数 *)
  let bool_expr_true = make_bool true in
  let bool_expr_false = make_bool false in
  check bool "make_bool创建正确的布尔表达式true" true (match bool_expr_true with LitExpr (BoolLit true) -> true | _ -> false);
  check bool "make_bool创建正确的布尔表达式false" true (match bool_expr_false with LitExpr (BoolLit false) -> true | _ -> false);
...
(** 执行测试 *)
let () =
  Alcotest.run "AST高级覆盖率测试 - Fix #2124" [
    ("ast_advanced_coverage", test_suite ())
  ]