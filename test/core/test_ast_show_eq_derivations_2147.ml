...
let test_binary_op_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun op -> check bool "运算符自相等" true (equal_binary_op op op)) ops;
...
let test_literal_show_eq () =
  let literals =
    [ IntLit 42; FloatLit 3.14; StringLit "测试字符串"; BoolLit true; BoolLit false; UnitLit ]
  in
...
let test_poetry_form_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun form -> check bool "诗词形式自相等" true (equal_poetry_form form form)) forms;
...
let test_tone_type_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun tone -> check bool "声调自相等" true (equal_tone_type tone tone)) tones;
...
let test_tone_constraint_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter
    (fun constraint_ -> check bool "声调约束自相等" true (equal_tone_constraint constraint_ constraint_))
    constraints;
...
let test_tone_pattern_show_eq () =
  ...
  in
  let pattern3 =
    { tone_sequence = [ FallingTone; LevelTone ]; tone_constraints = [ ParallelTones ] }
  in
...
let test_pattern_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun pattern -> check bool "模式自相等" true (equal_pattern pattern pattern)) patterns;
...
let test_type_expr_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun typ -> check bool "类型表达式自相等" true (equal_type_expr typ typ)) types;
...
let test_type_def_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun typedef -> check bool "类型定义自相等" true (equal_type_def typedef typedef)) type_defs
...
let test_macro_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun param -> check bool "宏参数自相等" true (equal_macro_param param param)) macro_params;
...
let test_module_show_eq () =
  let sig_items =
    [
      SigValue
        ("add", FunType (BaseTypeExpr IntType, FunType (BaseTypeExpr IntType, BaseTypeExpr IntType)));
  ...
  let module_def =
    { module_def_name = "TestModule"; module_type_annotation = None; exports = []; statements = [] }
  in

  let module_import = { module_import_name = "List"; imports = [ ("map", Some "list_map") ] } in
...
let test_expr_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun expr -> check bool "表达式自相等" true (equal_expr expr expr)) simple_exprs
...
let test_stmt_show_eq () =
  ...
  (* 测试equal函数 *)
  List.iter (fun stmt -> check bool "语句自相等" true (equal_stmt stmt stmt)) stmts
