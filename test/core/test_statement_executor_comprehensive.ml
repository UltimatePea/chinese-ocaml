...
let test_expr_stmt_execution () =
  ...
  match result with Ok (_, IntValue 42) -> () | _ -> failwith "表达式语句执行失败"

let test_expr_stmt_string () =
  (* 测试字符串表达式语句 *)
  let stmt = ExprStmt (str_lit "测试字符串") in
  let result = execute_single_stmt stmt in
  match result with Ok (_, StringValue "测试字符串") -> () | _ -> failwith "字符串表达式语句执行失败"

let test_expr_stmt_boolean () =
  (* 测试布尔表达式语句 *)
  let stmt = ExprStmt (bool_lit true) in
  let result = execute_single_stmt stmt in
  match result with Ok (_, BoolValue true) -> () | _ -> failwith "布尔表达式语句执行失败"

let test_expr_stmt_unit () =
  (* 测试单元表达式语句 *)
  let stmt = ExprStmt unit_lit in
  let result = execute_single_stmt stmt in
  match result with Ok (_, UnitValue) -> () | _ -> failwith "单元表达式语句执行失败"
...
let test_let_stmt_complex_expr () =
  ...
  | Ok (new_env, IntValue 30) -> (
      match lookup_var new_env "计算结果" with IntValue 30 -> () | _ -> failwith "复杂表达式let语句变量绑定失败")
...
let test_let_with_type_stmt () =
  ...
  | Ok (new_env, IntValue 50) -> (
      match lookup_var new_env "类型变量" with IntValue 50 -> () | _ -> failwith "带类型let语句变量绑定失败")
...
let test_rec_let_stmt_basic () =
  ...
  let func_expr =
    FunExpr
      ( [ "n" ],
        CondExpr
          ( BinaryOpExpr (var "n", Eq, int_lit 0),
            int_lit 1,
            BinaryOpExpr
...
let test_rec_let_with_type () =
  ...
  match result with
  | Ok (new_env, _) -> (
      match lookup_var new_env "递归函数" with FunctionValue _ -> () | _ -> failwith "带类型递归函数绑定失败")
...
let test_type_def_record () =
  ...
  let result = execute_single_stmt stmt in
  match result with Ok (_, UnitValue) -> () | _ -> failwith "记录类型定义语句执行失败"
...
let test_semantic_let_stmt () =
  ...
  | Ok (new_env, IntValue 123) -> (
      match lookup_var new_env "数学值" with IntValue 123 -> () | _ -> failwith "语义let语句变量绑定失败")
...
let test_semantic_let_string () =
  ...
  | Ok (new_env, StringValue "语义标签") -> (
      match lookup_var new_env "文本" with StringValue "语义标签" -> () | _ -> failwith "语义let字符串变量绑定失败")
...
let test_undefined_variable_error () =
  ...
  let result = execute_single_stmt stmt in
  match result with Error _ -> () (* 预期错误 *) | Ok _ -> failwith "未定义变量应该产生错误"
...
let test_execute_program_simple () =
  ...
  match result with Ok (IntValue 30) -> () | _ -> failwith "简单程序执行失败"
...
let test_execute_program_with_function () =
  ...
  in
  let result = execute_program program in
  match result with Ok (IntValue 40) -> () | _ -> failwith "包含函数的程序执行失败"

let test_execute_program_error () =
  (* 测试程序执行错误处理 *)
  let program = [ ExprStmt (var "不存在的变量") ] in
  let result = execute_program program in
  match result with Error _ -> () (* 预期错误 *) | Ok _ -> failwith "错误程序应该返回错误"
