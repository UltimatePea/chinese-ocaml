...
let test_builtin_function_string () =
  ...
  match result with StringValue "前缀后缀" -> () | _ -> failwith "字符串内置函数调用失败"

let test_builtin_function_no_args () =
  (* 测试无参数内置函数调用 *)
  let _env = create_test_env () in
  let no_arg_func =
    BuiltinFunctionValue (function [] -> IntValue 42 | _ -> raise (RuntimeError "此函数不需要参数"))
  in
  let args = [] in
  let result = call_function no_arg_func args simple_eval_expr in
  match result with IntValue 42 -> () | _ -> failwith "无参数内置函数调用失败"
...
let test_user_function_call () =
  ...
  match result with IntValue 40 -> () | _ -> failwith "用户定义函数调用失败"
...
let test_user_function_single_param () =
  ...
  let result = call_function user_func args simple_eval_expr in
  match result with IntValue 14 -> () | _ -> failwith "单参数用户函数调用失败"

let test_user_function_complex_body () =
  (* 测试复杂函数体的用户函数调用 *)
  let _env = create_test_env () in
  let func_body =
    CondExpr (BinaryOpExpr (var "x", Gt, int_lit 0), var "x", BinaryOpExpr (int_lit 0, Sub, var "x"))
  in
  let user_func = FunctionValue ([ "x" ], func_body, _env) in

  (* 测试正数 *)
  let result1 = call_function user_func [ IntValue 5 ] simple_eval_expr in
  (match result1 with IntValue 5 -> () | _ -> failwith "复杂函数体正数测试失败");

  (* 测试负数 *)
  let result2 = call_function user_func [ IntValue (-3) ] simple_eval_expr in
  match result2 with IntValue 3 -> () | _ -> failwith "复杂函数体负数测试失败"
...
let test_parameter_count_match () =
  ...
  let result = call_function user_func args simple_eval_expr in
  match result with IntValue 42 -> () | _ -> failwith "参数数量匹配测试失败"
...
let test_parameter_count_mismatch_error () =
  ...
  let user_func = FunctionValue ([ "x" ], func_body, _env) in
  let args = [ IntValue 1; IntValue 2 ] in
  (* 参数过多 *)
...
let test_parameter_shortage_recovery () =
  ...
  let result = call_function user_func args simple_eval_expr in
  match result with IntValue 10 -> () (* 10 + 0(默认值) = 10 *) | _ -> failwith "参数不足错误恢复失败"
...
let test_parameter_excess_recovery () =
  ...
  let func_body = var "x" in
  let user_func = FunctionValue ([ "x" ], func_body, _env) in
  let args = [ IntValue 42; IntValue 100; IntValue 200 ] in
  (* 参数过多 *)
  ...
  let result = call_function user_func args simple_eval_expr in
  match result with IntValue 42 -> () (* 只使用第一个参数 *) | _ -> failwith "参数过多错误恢复失败"
...
let test_closure_environment () =
  ...
  match result with IntValue 150 -> () | _ -> failwith "闭包环境测试失败"
...
let test_nested_closure () =
  ...
  let func_body = BinaryOpExpr (BinaryOpExpr (var "外层值", Add, var "内层值"), Add, var "参数") in
  ...
  match result with IntValue 35 -> () (* 10 + 20 + 5 = 35 *) | _ -> failwith "嵌套闭包测试失败"
...
let test_labeled_function_call () =
  ...
  let label_args =
    [ { arg_label = "长度"; arg_value = int_lit 8 }; { arg_label = "宽度"; arg_value = int_lit 6 } ]
  in
  let result = call_labeled_function labeled_func label_args _env simple_eval_expr in
  match result with IntValue 48 -> () | _ -> failwith "标签函数调用失败"
  ...
  let label_params =
    [
      ...
      };
      {
        ...
        param_type = None;
        is_optional = true;
        default_value = Some (int_lit 1);
      };
    ]
  in
  let func_body = BinaryOpExpr (var "基础值", Add, var "增量") in
  let labeled_func = LabeledFunctionValue (label_params, func_body, _env) in
  let label_args = [ { arg_label = "基础值"; arg_value = int_lit 10 } (* 省略可选参数 *) ] in
  let result = call_labeled_function labeled_func label_args _env simple_eval_expr in
  match result with IntValue 11 -> () (* 10 + 1(默认值) = 11 *) | _ -> failwith "带可选参数的标签函数调用失败"
      ...
      ( [ "n" ],
        CondExpr
          ( BinaryOpExpr (var "n", Le, int_lit 1),
            int_lit 1,
            BinaryOpExpr
              (var "n", Mul, FunCallExpr (var "阶乘", [ BinaryOpExpr (var "n", Sub, int_lit 1) ])) )
      )
  in
...
let test_recursive_non_function_error () =
  (* 测试递归let非函数表达式错误 *)
  let _env = create_test_env () in
  let non_func_expr = int_lit 42 in
  (* 不是函数表达式 *)
...
let test_function_argument_evaluation () =
  ...
  let result = call_function user_func args simple_eval_expr in
  match result with IntValue 16 -> () (* (5 + 3) * 2 = 16 *) | _ -> failwith "函数参数求值测试失败"
