...
let test_pattern_matching_comprehensive () =
  ...
  TestUtils.check_pattern_equality "构造器模式" (ConsPattern (VarPattern "head", VarPattern "tail")) cons_pat;
  TestUtils.check_pattern_equality "列表模式" (ListPattern [VarPattern "a"; VarPattern "b"]) list_pat;
  TestUtils.check_pattern_equality "元组模式" (TuplePattern [VarPattern "x"; VarPattern "y"]) tuple_pat;
  TestUtils.check_pattern_equality "构造器模式" (ConstructorPattern ("Some", [VarPattern "value"])) constructor_pat
...
let test_type_expressions_comprehensive () =
  ...
  TestUtils.check_type_equality "函数类型" (FunType (BaseTypeExpr IntType, BaseTypeExpr StringType)) arrow_type;
  TestUtils.check_type_equality "元组类型" (TupleType [BaseTypeExpr IntType; BaseTypeExpr StringType; BaseTypeExpr BoolType]) tuple_type;
...
let test_async_expressions () =
  let async_expr = AsyncExpr (AsyncFunc (FunCallExpr (VarExpr "fetch_data", [LitExpr (StringLit "url")]))) in
  let await_expr = AsyncExpr (AwaitExpr (VarExpr "promise")) in
  
  let expected_async = AsyncExpr (AsyncFunc (FunCallExpr (VarExpr "fetch_data", [LitExpr (StringLit "url")]))) in
...
let test_macro_system_comprehensive () =
  ...
  let macro_call = {
    macro_call_name = "add_macro";
    args = [LitExpr (IntLit 5); LitExpr (IntLit 3)]
  } in
...
let test_module_system_comprehensive () =
  ...
  let module_import = {
    ...
    imports = [("map", Some "list_map"); ("filter", None)]
  } in
...
let test_match_branches_comprehensive () =
  ...
  let branch2 = {
    pattern = LitPattern (IntLit 0);
    guard = None;
    expr = LitExpr (StringLit "zero")
  } in
  
  let branch3 = {
    pattern = WildcardPattern;
    guard = None;
    expr = LitExpr (StringLit "other")
  } in
...
let test_statement_types_comprehensive () =
  ...
  let type_def_stmt = TypeDefStmt ("MyType", AlgebraicType [("A", None); ("B", Some (BaseTypeExpr IntType))]) in
  ...
  TestUtils.check_stmt_equality "表达式语句" (ExprStmt (FunCallExpr (VarExpr "print", [LitExpr (StringLit "hello")]))) expr_stmt;
  TestUtils.check_stmt_equality "类型定义语句" (TypeDefStmt ("MyType", AlgebraicType [("A", None); ("B", Some (BaseTypeExpr IntType))])) type_def_stmt
...
let test_helper_functions_comprehensive () =
  ...
  TestUtils.check_expr_equality "make_binary_op辅助函数" 
    (BinaryOpExpr (LitExpr (IntLit 5), Mul, LitExpr (IntLit 7))) binary_expr;
  TestUtils.check_expr_equality "make_call辅助函数"
    (FunCallExpr (VarExpr "func", [LitExpr (IntLit 1); LitExpr (StringLit "arg")])) call_expr
