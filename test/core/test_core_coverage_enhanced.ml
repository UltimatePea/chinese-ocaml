(** 核心模块测试覆盖率增强 - Phase 25 测试覆盖率提升
    ...
    本测试模块专门针对核心编译器模块进行基础功能测试， 重点测试词法分析、语法分析和AST构建的关键功能。
...
module AstConstructionTests = struct
  ...
  let test_variable_expression_construction () =
    let var_exprs =
      [ ("x", "简单变量"); ("变量名", "中文变量名"); ("var_123", "带数字变量名"); ("函数名_中英混合", "中英混合变量名") ]
    in
  ...
  let test_binary_operation_construction () =
    let operations =
      [ (Add, "加法"); (Sub, "减法"); (Mul, "乘法"); (Div, "除法"); (Eq, "相等比较"); (Lt, "小于比较") ]
    in
  ...
  let test_compound_expression_construction () =
    ...
    let expected_tuple =
      TupleExpr [ LitExpr (IntLit 1); LitExpr (StringLit "test"); LitExpr (BoolLit true) ]
    in
  ...
  let test_number_token_recognition () =
    ...
    List.iter
      (fun num ->
        ...
        check (testable pp_token equal_token)
          (Printf.sprintf "数字%d token识别" num)
          token (fst token_with_pos))
      numbers
  ...
  let test_string_token_recognition () =
    ...
    List.iter
      (fun str ->
        ...
        let token_with_pos = (token, pos) in
        check (testable pp_token equal_token)
          (Printf.sprintf "字符串'%s' token识别" str)
          token (fst token_with_pos))
      strings
  ...
  let test_identifier_token_recognition () =
    ...
    List.iter
      (fun id ->
        ...
        let token_with_pos = (token, pos) in
        check (testable pp_token equal_token)
          (Printf.sprintf "标识符'%s' token识别" id)
          token (fst token_with_pos))
      identifiers
  ...
  let test_operator_token_recognition () =
    ...
    List.iter
      (fun (token, desc) ->
        ...
        let token_with_pos = (token, pos) in
        check (testable pp_token equal_token)
          (Printf.sprintf "运算符'%s' token识别" desc)
          token (fst token_with_pos))
      operators
end
...
module UnicodeHandlingTests = struct
  ...
  let test_chinese_string_handling () =
    let chinese_strings = [ "春眠不觉晓"; "处处闻啼鸟"; "夜来风雨声"; "花落知多少" ] in

    List.iter
      (fun text ->
        let str_expr = make_string text in
        check (testable pp_expr equal_expr)
          (Printf.sprintf "中文字符串'%s'处理" text)
          (LitExpr (StringLit text)) str_expr)
      chinese_strings

  (** 测试混合字符处理 *)
  let test_mixed_character_handling () =
    let mixed_strings = [ "Hello世界"; "测试123"; "中English文"; "数字456汉字" ] in

    List.iter
      (fun text ->
        let str_expr = make_string text in
        check (testable pp_expr equal_expr)
          (Printf.sprintf "混合字符'%s'处理" text)
          (LitExpr (StringLit text)) str_expr)
      mixed_strings

  (** 测试Unicode表情符号处理 *)
  let test_unicode_emoji_handling () =
    let emoji_strings = [ "🌸春天"; "🌙月亮"; "🔥火焰"; "💻代码" ] in

    List.iter
      (fun text ->
        let str_expr = make_string text in
        check (testable pp_expr equal_expr)
          (Printf.sprintf "表情符号'%s'处理" text)
          (LitExpr (StringLit text)) str_expr)
      emoji_strings
end
...
module PerformanceTests = struct
  ...
  let test_bulk_expression_construction () =
    ...
    List.iteri
      (fun i expr ->
        check (testable pp_expr equal_expr) (Printf.sprintf "批量表达式%d构建" i) (LitExpr (IntLit i)) expr)
      expressions;
  ...
  let test_deep_nested_expressions () =
    ...
    let rec create_nested_expr depth =
      if depth <= 0 then make_int 1
      else BinaryOpExpr (create_nested_expr (depth - 1), Add, make_int depth)
    in
    ...
    let rec count_depth expr =
      match expr with BinaryOpExpr (left, _, _) -> 1 + count_depth left | _ -> 0
    in
