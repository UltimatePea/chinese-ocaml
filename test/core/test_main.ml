...
module Main_functions = struct
  ...
  (** 检查是否是帮助命令 *)
  let is_help_command args = List.mem "-h" args || List.mem "-help" args

  (** 检查是否是交互模式命令 *)
  let is_interactive_mode args = args = [] || List.mem "-i" args
end
...
let test_option_precedence () =
  ...
  (* 测试输出文件覆盖 *)
  let output_override =
    Main_functions.parse_args [ "-o"; "first.c"; "-o"; "second.c" ] base_options
  in
