(** AST模块穷尽模式匹配测试 - Fix #2147
    ...
    通过穷尽模式匹配测试来提高AST模块的覆盖率。
    测试所有AST构造器的匹配分支，确保每个构造器都被实际使用。
    专门针对bisect覆盖率工具，通过运行时模式匹配来触发代码覆盖。
...
let test_binary_op_pattern_matching () =
  ...
  let ops_and_classes = [
    ...
    (Div, "arithmetic"); (Mod, "arithmetic"); (Concat, "string");
    (Eq, "comparison"); (Neq, "comparison"); (Lt, "comparison");
    (Le, "comparison"); (Gt, "comparison"); (Ge, "comparison");
    (And, "logical"); (Or, "logical")
  ] in
  
  List.iter (fun (op, expected_class) ->
...
let test_poetry_form_pattern_matching () =
  ...
  List.iter (fun (form, expected_desc) ->
...
let test_tone_type_pattern_matching () =
  ...
  List.iter (fun (tone, expected_symbol) ->
...
let test_tone_constraint_pattern_matching () =
  let describe_constraint constraint_ =
    ...
    | SpecificPattern tones -> 
        "特定模式：" ^ (String.concat "" (List.map (function 
          | LevelTone -> "平" 
          | _ -> "仄") tones))
  in
  ...
  List.iter (fun (constraint_, expected_desc) ->
...
let test_literal_pattern_matching () =
  ...
  List.iter (fun (literal, expected_desc) ->
...
let test_pattern_exhaustive_matching () =
  ...
  List.iter (fun (pattern, expected_count) ->
...
let test_type_expr_deep_matching () =
  let calculate_type_depth typ =
    let rec depth t =
      ...
      | PolymorphicVariantType variants -> 
          1 + (List.fold_left (fun acc (_, opt_t) -> 
            match opt_t with 
            | Some t -> max acc (depth t) 
            | None -> acc) 0 variants)
    in
    depth typ
  in
  ...
  List.iter (fun (typ, expected_depth) ->
...
let test_expr_complex_pattern_matching () =
  ...
  List.iter (fun (expr, expected_count) ->
...
let test_stmt_exhaustive_matching () =
  ...
  List.iter (fun (stmt, expected_class) ->
...
let test_async_expr_pattern_matching () =
  ...
  List.iter (fun (async_expr, expected_desc) ->
...
let test_macro_param_pattern_matching () =
  ...
  List.iter (fun (param, expected_type) ->
...
let test_helper_functions_execution () =
  ...
  check bool "make_string结果正确" true (match string_expr with LitExpr (StringLit "测试") -> true | _ -> false);
  check bool "make_bool结果正确" true (match bool_expr with LitExpr (BoolLit true) -> true | _ -> false);
  check bool "make_var结果正确" true (match var_expr with VarExpr "变量" -> true | _ -> false);
  check bool "make_binary_op结果正确" true (match binary_expr with 
    | BinaryOpExpr (LitExpr (IntLit 100), Add, LitExpr (IntLit 50)) -> true 
    | _ -> false);
  check bool "make_call结果正确" true (match call_expr with 
...
let () =
  run "AST模块穷尽模式匹配测试 - Fix #2147"
    [
      ...
      ( "辅助函数执行测试",
        [
          test_case "辅助函数实际执行" `Quick test_helper_functions_execution;
        ] );
    ]