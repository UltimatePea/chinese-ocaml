(** AST模块增强覆盖率测试 - Fix #2124
    ...
    专门针对AST模块中未覆盖的代码路径进行测试
    目标: 将AST覆盖率从39.34%提升到80%+
...
module ASTTestUtils = struct
  ...
  (** 创建表达式测试用例 *)
  let test_expr desc expected actual =
    check (testable pp_expr expr_equal) desc expected actual
  ...
  (** 创建语句测试用例 *)
  let test_stmt desc expected actual =
    check (testable pp_stmt stmt_equal) desc expected actual
end
...
let test_ast_helper_functions () =
  ...
  ASTTestUtils.test_expr "make_int应创建正确的整数表达式" 
    (LitExpr (IntLit 42)) int_expr;

  (* 测试make_string *)
  let string_expr = make_string "你好世界" in
  ASTTestUtils.test_expr "make_string应创建正确的字符串表达式"
    (LitExpr (StringLit "你好世界")) string_expr;

  (* 测试make_bool *)
  let bool_true_expr = make_bool true in
  let bool_false_expr = make_bool false in
  ASTTestUtils.test_expr "make_bool应创建正确的布尔表达式(真)"
    (LitExpr (BoolLit true)) bool_true_expr;
  ASTTestUtils.test_expr "make_bool应创建正确的布尔表达式(假)"
    (LitExpr (BoolLit false)) bool_false_expr;

  (* 测试make_var *)
  let var_expr = make_var "变量名" in
  ASTTestUtils.test_expr "make_var应创建正确的变量表达式"
    (VarExpr "变量名") var_expr;
  ...
  ASTTestUtils.test_expr "make_binary_op应创建正确的二元运算表达式"
    (BinaryOpExpr (LitExpr (IntLit 10), Add, LitExpr (IntLit 5))) add_expr;
  ...
  ASTTestUtils.test_expr "make_call应创建正确的函数调用表达式"
    (FunCallExpr (VarExpr "函数名", [LitExpr (IntLit 1); LitExpr (StringLit "参数")])) call_expr
...
let test_poetry_ast_nodes () =
  ...
  List.iteri (fun i form ->
    ...
    check bool (Printf.sprintf "诗词形式%d应有有效字符串表示" i) 
      true (String.length form_str > 0)
  ) poetry_forms;
  ...
  List.iteri (fun i tone ->
    let tone_str = show_tone_type tone in
    check bool (Printf.sprintf "声调类型%d应有有效字符串表示" i)
      true (String.length tone_str > 0)
  ) tone_types;
  ...
  List.iteri (fun i constraint_type ->
    let constraint_str = show_tone_constraint constraint_type in
    check bool (Printf.sprintf "声调约束%d应有有效字符串表示" i)
      true (String.length constraint_str > 0)
  ) tone_constraints;
...
let test_poetry_annotated_expressions () =
  ...
  ASTTestUtils.test_expr "诗词注解表达式应正确构造"
    (PoetryAnnotatedExpr (LitExpr (StringLit "春眠不觉晓"), FiveCharPoetry)) poetry_expr;
  ...
  ASTTestUtils.test_expr "对偶结构表达式应正确构造"
    (ParallelStructureExpr (LitExpr (StringLit "晴川历历汉阳树"), 
                           LitExpr (StringLit "芳草萋萋鹦鹉洲"))) parallel_expr;
  ...
  ASTTestUtils.test_expr "押韵注解表达式应正确构造"
    (RhymeAnnotatedExpr (LitExpr (StringLit "春眠不觉晓"), rhyme_info)) rhyme_expr;
  ...
  ASTTestUtils.test_expr "平仄注解表达式应正确构造"
    (ToneAnnotatedExpr (LitExpr (StringLit "春眠不觉晓"), tone_pattern)) tone_expr;
  ...
  ASTTestUtils.test_expr "韵律验证表达式应正确构造"
    (MeterValidatedExpr (LitExpr (StringLit "春眠不觉晓"), meter_constraint)) meter_expr
...
let test_advanced_type_expressions () =
  ...
  ASTTestUtils.test_type_expr "函数类型应正确构造"
    (FunType (BaseTypeExpr IntType, BaseTypeExpr StringType)) fun_type;

  (* 测试元组类型 *)
  let tuple_type = TupleType [BaseTypeExpr IntType; BaseTypeExpr StringType; BaseTypeExpr BoolType] in
  ASTTestUtils.test_type_expr "元组类型应正确构造"
    (TupleType [BaseTypeExpr IntType; BaseTypeExpr StringType; BaseTypeExpr BoolType]) tuple_type;
  ...
  ASTTestUtils.test_type_expr "构造类型应正确构造"
    (ConstructType ("Option", [BaseTypeExpr IntType])) construct_type;
  ...
  (* 测试多态变体类型 *)
  let poly_variant_type = PolymorphicVariantType [("Red", Some (BaseTypeExpr IntType)); ("Blue", None)] in
  ASTTestUtils.test_type_expr "多态变体类型应正确构造"
    (PolymorphicVariantType [("Red", Some (BaseTypeExpr IntType)); ("Blue", None)]) poly_variant_type
...
let test_async_expressions () =
  ...
  ASTTestUtils.test_expr "异步函数表达式应正确构造"
    (AsyncExpr (AsyncFunc (LitExpr (StringLit "异步操作")))) async_func;

  (* 测试等待表达式 *)
  let await_expr = AsyncExpr (AwaitExpr base_expr) in
  ASTTestUtils.test_expr "等待表达式应正确构造"
    (AsyncExpr (AwaitExpr (LitExpr (StringLit "异步操作")))) await_expr;

  (* 测试创建任务表达式 *)
  let spawn_expr = AsyncExpr (SpawnExpr base_expr) in
  ASTTestUtils.test_expr "创建任务表达式应正确构造"
    (AsyncExpr (SpawnExpr (LitExpr (StringLit "异步操作")))) spawn_expr;

  (* 测试通道操作表达式 *)
  let channel_expr = AsyncExpr (ChannelExpr base_expr) in
  ASTTestUtils.test_expr "通道操作表达式应正确构造"
    (AsyncExpr (ChannelExpr (LitExpr (StringLit "异步操作")))) channel_expr
...
let test_macro_system () =
  ...
  List.iteri (fun i param ->
    ...
    check bool (Printf.sprintf "宏参数%d应有有效字符串表示" i)
      true (String.length param_str > 0)
  ) macro_params;

  (* 测试宏调用 *)
  let macro_call = { 
    macro_call_name = "my_macro"; 
    args = [make_int 1; make_string "test"] 
  } in
  let macro_call_expr = MacroCallExpr macro_call in
  ASTTestUtils.test_expr "宏调用表达式应正确构造"
    (MacroCallExpr { macro_call_name = "my_macro"; 
                    args = [LitExpr (IntLit 1); LitExpr (StringLit "test")] }) macro_call_expr

(** 测试标签函数系统 *)
let test_labeled_function_system () =
  (* 测试标签参数 *)
  let label_params = [
    { label_name = "名称"; param_name = "name"; param_type = Some (BaseTypeExpr StringType); 
      is_optional = false; default_value = None };
    { label_name = "年龄"; param_name = "age"; param_type = Some (BaseTypeExpr IntType); 
      is_optional = true; default_value = Some (make_int 18) };
  ] in

  (* 测试标签函数表达式 *)
  let labeled_fun = LabeledFunExpr (label_params, make_string "函数体") in
  ASTTestUtils.test_expr "标签函数表达式应正确构造"
    (LabeledFunExpr (label_params, LitExpr (StringLit "函数体"))) labeled_fun;
  ...
  ASTTestUtils.test_expr "标签函数调用表达式应正确构造"
    (LabeledFunCallExpr (VarExpr "labeled_function", label_args)) labeled_call
...
let test_complex_statements () =
  ...
  ASTTestUtils.test_stmt "Let语句应正确构造" 
    (LetStmt ("x", LitExpr (StringLit "hello"))) let_stmt;

  (* 测试带类型的Let语句 *)
  let let_type_stmt = LetStmtWithType ("y", BaseTypeExpr IntType, make_int 10) in
  ASTTestUtils.test_stmt "带类型的Let语句应正确构造"
    (LetStmtWithType ("y", BaseTypeExpr IntType, LitExpr (IntLit 10))) let_type_stmt;

  (* 测试递归Let语句 *)
  let rec_let_stmt = RecLetStmt ("factorial", make_var "factorial_impl") in
  ASTTestUtils.test_stmt "递归Let语句应正确构造"
    (RecLetStmt ("factorial", VarExpr "factorial_impl")) rec_let_stmt;

  (* 测试带类型的递归Let语句 *)
  let rec_let_type_stmt = RecLetStmtWithType ("fib", 
    FunType (BaseTypeExpr IntType, BaseTypeExpr IntType), make_var "fib_impl") in
  ASTTestUtils.test_stmt "带类型的递归Let语句应正确构造"
    (RecLetStmtWithType ("fib", FunType (BaseTypeExpr IntType, BaseTypeExpr IntType), 
                        VarExpr "fib_impl")) rec_let_type_stmt;

  (* 测试语义Let语句 *)
  let semantic_let_stmt = SemanticLetStmt ("result", "计算结果", make_int 100) in
  ASTTestUtils.test_stmt "语义Let语句应正确构造"
    (SemanticLetStmt ("result", "计算结果", LitExpr (IntLit 100))) semantic_let_stmt;

  (* 测试类型定义语句 *)
  let type_def_stmt = TypeDefStmt ("MyType", AliasType (BaseTypeExpr StringType)) in
  ASTTestUtils.test_stmt "类型定义语句应正确构造"
    (TypeDefStmt ("MyType", AliasType (BaseTypeExpr StringType))) type_def_stmt;

  (* 测试异常定义语句 *)
  let exception_def_stmt = ExceptionDefStmt ("MyException", Some (BaseTypeExpr StringType)) in
  ASTTestUtils.test_stmt "异常定义语句应正确构造"
    (ExceptionDefStmt ("MyException", Some (BaseTypeExpr StringType))) exception_def_stmt
...
(** 执行测试 *)
let () =
  Alcotest.run "AST模块增强覆盖率测试 - Fix #2124" [
    ("ast_enhanced_coverage", test_suite ())
  ]