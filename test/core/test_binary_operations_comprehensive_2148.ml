(** 二元运算模块全面测试覆盖率提升 - Fix #2148
    ...
    目标：将binary_operations.ml模块测试覆盖率从50.90%提升至80%+
    策略：系统化测试所有运算符类型、错误处理路径和类型转换机制
...
let test_float_arithmetic_comprehensive () =
  ...
  check bool "浮点数加法接近6.0" true (
    ...
    | FloatValue f -> abs_float (f -. 6.0) < 0.001
    | _ -> false
  );
  ...
  check bool "浮点数减法接近7.3" true (
    ...
    | FloatValue f -> abs_float (f -. 7.3) < 0.001
    | _ -> false
  );
  ...
  check bool "浮点数零除法得到无穷大" true (
    match zero_div with 
    | FloatValue f -> Float.is_infinite f
    | _ -> false
  )
...
let test_division_errors_comprehensive () =
  (* 整数除零 *)
  expect_runtime_error "整数除零应抛出错误" (fun () ->
    execute_binary_op Div (IntValue 42) (IntValue 0));
  
  (* 负数除零 *)
  expect_runtime_error "负数除零应抛出错误" (fun () ->
    execute_binary_op Div (IntValue (-15)) (IntValue 0));
  
  (* 取模零 *)
  expect_runtime_error "取模零应抛出错误" (fun () ->
    execute_binary_op Mod (IntValue 25) (IntValue 0));
  
  (* 负数取模零 *)
  expect_runtime_error "负数取模零应抛出错误" (fun () ->
    execute_binary_op Mod (IntValue (-7)) (IntValue 0))
...
let test_comparison_errors () =
  ...
  expect_runtime_error "单元值与整数比较应失败" (fun () ->
    execute_binary_op Le UnitValue (IntValue 42))
...
let test_unsupported_unary_operations () =
  (* 字符串取负 *)
  expect_runtime_error "字符串取负应失败" (fun () ->
    execute_unary_op Neg (StringValue "cannot negate"));
  
  (* 布尔值取负 *)
  expect_runtime_error "布尔值取负应失败" (fun () ->
    execute_unary_op Neg (BoolValue true));
  
  (* 单元值取负 *)
  expect_runtime_error "单元值取负应失败" (fun () ->
    execute_unary_op Neg UnitValue);
...
let test_boundary_and_special_values () =
  ...
  check bool "极小浮点数运算" true (
    match tiny_result with 
    | FloatValue f -> f >= 0.0
    | _ -> false
  )
...
let test_incompatible_type_operations () =
  ...
  check bool "整数与字符串逻辑与（通过value_to_bool转换）" true (
    match logical_result with 
    | BoolValue b -> b = true  (* 非零整数和非空字符串都为真 *)
    | _ -> false
  )
