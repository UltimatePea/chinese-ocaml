...
let test_basic_type_conversion () =
  (* 测试from_base_type函数 *)
  check (testable pp_typ equal_typ) "整数类型转换" IntType_T (from_base_type IntType);

  check (testable pp_typ equal_typ) "浮点数类型转换" FloatType_T (from_base_type FloatType);

  check (testable pp_typ equal_typ) "字符串类型转换" StringType_T (from_base_type StringType);

  check (testable pp_typ equal_typ) "布尔类型转换" BoolType_T (from_base_type BoolType);

  check (testable pp_typ equal_typ) "单元类型转换" UnitType_T (from_base_type UnitType)

(** 测试字面量类型推断 *)
let test_literal_type_inference () =
  (* 测试literal_type函数 *)
  check (testable pp_typ equal_typ) "整数字面量类型推断" IntType_T (literal_type (IntLit 42));

  check (testable pp_typ equal_typ) "浮点数字面量类型推断" FloatType_T (literal_type (FloatLit 3.14));

  check (testable pp_typ equal_typ) "字符串字面量类型推断" StringType_T (literal_type (StringLit "hello"));

  check (testable pp_typ equal_typ) "布尔字面量类型推断" BoolType_T (literal_type (BoolLit true));

  check (testable pp_typ equal_typ) "单元字面量类型推断" UnitType_T (literal_type UnitLit)
...
let test_type_expression_conversion () =
  (* 测试基础类型表达式转换 *)
  check (testable pp_typ equal_typ) "基础类型表达式转换 - 整数" IntType_T
    (type_expr_to_typ (BaseTypeExpr IntType));

  (* 测试类型变量转换 *)
  check (testable pp_typ equal_typ) "类型变量转换" (TypeVar_T "a") (type_expr_to_typ (TypeVar "a"));
  ...
  check (testable pp_typ equal_typ) "函数类型转换" expected_fun_type fun_type;

  (* 测试列表类型转换 *)
  let list_type = type_expr_to_typ (ListType (BaseTypeExpr IntType)) in
  check (testable pp_typ equal_typ) "列表类型转换" (ListType_T IntType_T) list_type;
  ...
  check (testable pp_typ equal_typ) "元组类型转换" expected_tuple tuple_type
...
let test_complex_nested_types () =
  (* 测试嵌套函数类型 *)
  let nested_fun_expr =
    FunType (FunType (BaseTypeExpr IntType, BaseTypeExpr StringType), BaseTypeExpr BoolType)
  in
  ...
  (* 测试包含类型变量的复杂类型 *)
  let complex_expr =
    FunType (TupleType [ TypeVar "a"; BaseTypeExpr IntType ], ListType (TypeVar "a"))
  in
  let converted = type_expr_to_typ complex_expr in
  let expected = FunType_T (TupleType_T [ TypeVar_T "a"; IntType_T ], ListType_T (TypeVar_T "a")) in
...
let test_construct_type_conversion () =
  ...
  (* 测试多参数构造类型 *)
  let multi_param_construct =
    ConstructType ("Either", [ BaseTypeExpr IntType; BaseTypeExpr StringType ])
  in
...
let () =
  run "Types_Convert模块测试"
    [
      ("基础类型转换", [ test_case "from_base_type函数测试" `Quick test_basic_type_conversion ]);
      ("字面量类型推断", [ test_case "literal_type函数测试" `Quick test_literal_type_inference ]);
      ( "运算符类型推断",
        [
      ...
      ("模式匹配支持", [ test_case "模式变量绑定提取测试" `Quick test_pattern_binding_extraction ]);
      ("中文显示功能", [ test_case "中文类型显示测试" `Quick test_chinese_type_display ]);
      ("错误处理和边界情况", [ test_case "错误处理和边界情况测试" `Quick test_error_handling ]);
    ]
