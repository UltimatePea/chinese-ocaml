...
let test_non_arithmetic_operation_errors () =
  ...
  check value_testable "浮点数取模（转换为整数）" (IntValue 1) float_mod; (* 5 mod 2 = 1 *)
...
let test_comparison_operation_errors () =
  ...
  expect_runtime_error "单元值与整数比较应失败" (fun () ->
    execute_binary_op Lt UnitValue (IntValue 5));
...
let test_string_conversion_fallback () =
  ...
  check bool "列表与单元值加法（转换为字符串）" true (
    match list_unit_add with 
    | StringValue s -> String.length s > 0  (* 应该得到某种字符串表示 *)
    | _ -> false
  );
  ...
  check bool "单元值与列表加法（转换为字符串）" true (
    match unit_list_add with 
    | StringValue s -> String.length s > 0
    | _ -> false
  );
  
  (* 测试列表与列表的字符串转换 *)
  let list_list_add = execute_binary_op Add (ListValue [IntValue 1; IntValue 2]) (ListValue [StringValue "test"]) in
  check bool "列表与列表加法（转换为字符串）" true (
    match list_list_add with 
    | StringValue s -> String.length s > 0
    | _ -> false
  )
...
let test_non_standard_operator_combinations () =
  ...
  expect_runtime_error "单元值与布尔值小于比较应失败" (fun () ->
    execute_binary_op Lt UnitValue (BoolValue false));
...
let test_float_special_values () =
  ...
  check bool "负无穷大 - 1.0 应该得到负无穷大" true (
    match neg_inf_sub with 
    | FloatValue f -> Float.is_infinite f && f < 0.0
    | _ -> false
  )
