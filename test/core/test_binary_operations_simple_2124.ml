(** 二元运算模块简化测试覆盖率提升 - Fix #2124
    ...
    专注于提升binary_operations.ml模块测试覆盖率从61.68%到80%+
    通过公共API测试关键路径，避免复杂的内部函数测试
...
let test_division_by_zero () =
  ...
  expect_runtime_error "整数除零应抛出错误" (fun () ->
    execute_binary_op Div (IntValue 5) (IntValue 0));
  
  (* 取模零 *)
  expect_runtime_error "取模零应抛出错误" (fun () ->
    execute_binary_op Mod (IntValue 5) (IntValue 0))
...
let test_special_numeric_values () =
  ...
  check bool "浮点数乘法结果应接近0.3" true (
    match float_result with 
    | FloatValue f -> abs_float (f -. 0.3) < 0.0001
    | _ -> false
  )
...
let test_string_operations_edge_cases () =
  ...
  check (testable pp_value runtime_value_equal) "长字符串连接" (StringValue (long_str ^ "end")) long_result;
...
let test_type_mismatch_errors () =
  ...
  (* 单元值运算 *)
  expect_runtime_error "单元值乘法应失败" (fun () ->
    execute_binary_op Mul UnitValue (IntValue 5));
...
let test_unsupported_unary_operations () =
  ...
  expect_runtime_error "字符串取负应失败" (fun () ->
    execute_unary_op Neg (StringValue "test"));
  
  (* 列表取负 *)
  expect_runtime_error "列表取负应失败" (fun () ->
    execute_unary_op Neg (ListValue [IntValue 1]))
...
(** 执行测试 *)
let () =
  Alcotest.run "二元运算简化覆盖率测试 - Fix #2124" [
    ("binary_operations_simple_coverage", test_suite ())
  ]