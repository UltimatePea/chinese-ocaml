...
let test_parallel_structure_expressions () =
  ...
  (* 测试嵌套对偶结构 *)
  let nested_left = ParallelStructureExpr (
    LitExpr (StringLit "两个黄鹂鸣翠柳"),
    LitExpr (StringLit "一行白鹭上青天")
  ) in
  let nested_right = ParallelStructureExpr (
    LitExpr (StringLit "窗含西岭千秋雪"),
    LitExpr (StringLit "门泊东吴万里船")
  ) in
...
let test_module_system_expressions () =
  ...
  let signature = Signature [SigValue ("compare", FunType (BaseTypeExpr IntType, BaseTypeExpr IntType))] in
...
let test_record_expressions () =
  ...
  (* 测试记录更新表达式 *)
  let update_fields = [("age", LitExpr (IntLit 26)); ("email", LitExpr (StringLit "newemail@example.com"))] in
  ...
  (* 测试嵌套记录 *)
  let nested_record = RecordExpr [
    ("person", RecordExpr [("name", LitExpr (StringLit "李四")); ("age", LitExpr (IntLit 30))]);
    ("address", RecordExpr [("city", LitExpr (StringLit "北京")); ("street", LitExpr (StringLit "朝阳区"))])
  ] in
  let expected_nested = RecordExpr [
    ("person", RecordExpr [("name", LitExpr (StringLit "李四")); ("age", LitExpr (IntLit 30))]);
    ("address", RecordExpr [("city", LitExpr (StringLit "北京")); ("street", LitExpr (StringLit "朝阳区"))])
  ] in
...
let test_array_expressions () =
  (* 测试数组创建表达式 *)
  let array_elements = [LitExpr (IntLit 1); LitExpr (IntLit 2); LitExpr (IntLit 3); LitExpr (IntLit 4)] in
...
let test_reference_expressions () =
  ...
  let expected_complex_ref = RefExpr (BinaryOpExpr (LitExpr (IntLit 10), Add, LitExpr (IntLit 20))) in
...
let test_advanced_type_expressions () =
  ...
  (* 测试复杂嵌套类型 *)
  let complex_type = FunType (
    TupleType [BaseTypeExpr IntType; RefType (BaseTypeExpr StringType)],
    ListType (PolymorphicVariantType [("Success", Some (BaseTypeExpr IntType)); ("Error", Some (BaseTypeExpr StringType))])
  ) in
  let expected_complex = FunType (
    TupleType [BaseTypeExpr IntType; RefType (BaseTypeExpr StringType)],
    ListType (PolymorphicVariantType [("Success", Some (BaseTypeExpr IntType)); ("Error", Some (BaseTypeExpr StringType))])
  ) in
...
let test_polymorphic_variant_expressions () =
  ...
  (* 测试带值的多态变体 *)
  let valued_variant = PolymorphicVariantExpr ("RGB", Some (TupleExpr [LitExpr (IntLit 255); LitExpr (IntLit 128); LitExpr (IntLit 0)])) in
  let expected_valued = PolymorphicVariantExpr ("RGB", Some (TupleExpr [LitExpr (IntLit 255); LitExpr (IntLit 128); LitExpr (IntLit 0)])) in
  TestUtils.check_expr_equality "带值多态变体" expected_valued valued_variant;
  
  (* 测试多态变体模式匹配 *)
  let variant_value = VarExpr "color" in
  let red_branch = { pattern = PolymorphicVariantPattern ("Red", None); guard = None; expr = LitExpr (StringLit "红色") } in
  let rgb_branch = { pattern = PolymorphicVariantPattern ("RGB", Some (TuplePattern [VarPattern "r"; VarPattern "g"; VarPattern "b"])); guard = None; expr = LitExpr (StringLit "自定义颜色") } in
...
let test_type_annotation_expressions () =
  ...
  let func_type = FunType (BaseTypeExpr IntType, FunType (BaseTypeExpr IntType, BaseTypeExpr IntType)) in
...
let test_constructor_expressions () =
  ...
  (* 测试多参数构造器 *)
  let person_constructor = ConstructorExpr ("Person", [LitExpr (StringLit "张三"); LitExpr (IntLit 25)]) in
  let expected_person = ConstructorExpr ("Person", [LitExpr (StringLit "张三"); LitExpr (IntLit 25)]) in
  TestUtils.check_expr_equality "Person构造器" expected_person person_constructor;
  
  (* 测试嵌套构造器 *)
  let nested_constructor = ConstructorExpr ("Result", [ConstructorExpr ("Ok", [LitExpr (IntLit 100)])]) in
  let expected_nested = ConstructorExpr ("Result", [ConstructorExpr ("Ok", [LitExpr (IntLit 100)])]) in
...
let test_advanced_pattern_matching () =
  ...
  (* 测试复杂嵌套模式 *)
  let complex_pattern = ConstructorPattern ("Tree", [
    VarPattern "value";
    OrPattern (
  ...
  let expected_complex_pattern = ConstructorPattern ("Tree", [
    VarPattern "value";
    OrPattern (
      ConstructorPattern ("Leaf", []),
      ConstructorPattern ("Node", [VarPattern "left"; VarPattern "right"])
...
let test_comprehensive_statement_types () =
  (* 测试递归let语句 *)
  let rec_let_stmt = RecLetStmt ("factorial", FunExpr (["n"], 
    CondExpr (
      BinaryOpExpr (VarExpr "n", Le, LitExpr (IntLit 1)),
      LitExpr (IntLit 1),
      BinaryOpExpr (VarExpr "n", Mul, FunCallExpr (VarExpr "factorial", [BinaryOpExpr (VarExpr "n", Sub, LitExpr (IntLit 1))]))
    ))) in
  let expected_rec_let = RecLetStmt ("factorial", FunExpr (["n"], 
    CondExpr (
      BinaryOpExpr (VarExpr "n", Le, LitExpr (IntLit 1)),
      LitExpr (IntLit 1),
      BinaryOpExpr (VarExpr "n", Mul, FunCallExpr (VarExpr "factorial", [BinaryOpExpr (VarExpr "n", Sub, LitExpr (IntLit 1))]))
  ...
  (* 测试语义let语句 *)
  let semantic_let = SemanticLetStmt ("result", "计算结果", BinaryOpExpr (LitExpr (IntLit 10), Mul, LitExpr (IntLit 5))) in
  let expected_semantic_let = SemanticLetStmt ("result", "计算结果", BinaryOpExpr (LitExpr (IntLit 10), Mul, LitExpr (IntLit 5))) in
...
let test_semantic_and_combine_expressions () =
  ...
  let expected_semantic_let_expr = SemanticLetExpr ("pi", "圆周率", LitExpr (FloatLit 3.14159), VarExpr "pi") in
...
let () =
  run "AST模块测试覆盖率提升至80%+ - Fix #2147"
    [
      ...
      ( "模块系统表达式测试",
        [
          test_case "模块系统表达式测试" `Quick test_module_system_expressions;
        ] );
      ( "记录和数组表达式测试",
        [
      ...
      ( "语义和组合表达式测试",
        [
          test_case "语义和组合表达式测试" `Quick test_semantic_and_combine_expressions;
        ] );
    ]