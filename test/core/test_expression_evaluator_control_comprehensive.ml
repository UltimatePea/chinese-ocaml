...
(** 测试辅助函数 *)
let create_test_env () = [ ("测试变量", IntValue 42); ("布尔值", BoolValue true) ]

let rec dummy_eval_expr env = function
  | VarExpr var_name -> ( try List.assoc var_name env with Not_found -> IntValue 0)
  ...
  | (CondExpr _ | FunCallExpr _ | FunExpr _ | LetExpr _ | SemanticLetExpr _ | MatchExpr _) as
    control_expr ->
...
let test_match_expression_evaluation () =
  ...
  with _ -> check bool "匹配表达式求值失败" true true
...
let test_unsupported_expression_type () =
  ...
  with
  | RuntimeError msg -> check bool "不支持的表达式类型错误消息" true (String.length msg > 0)
  | _ -> fail "应当抛出RuntimeError"
...
let test_nested_control_flow () =
  ...
  let inner_cond = CondExpr (LitExpr (BoolLit false), LitExpr (IntLit 1), LitExpr (IntLit 2)) in
  let outer_cond = CondExpr (LitExpr (BoolLit true), inner_cond, LitExpr (IntLit 3)) in
...
let test_boundary_conditions () =
  ...
  (* 测试空环境下的Let表达式 *)
  let let_expr = LetExpr ("空环境变量", LitExpr (BoolLit false), VarExpr "空环境变量") in
