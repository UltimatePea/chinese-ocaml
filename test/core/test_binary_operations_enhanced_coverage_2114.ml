(** 二元运算模块增强测试覆盖率提升 - Fix #2114
    ...
    专注于提升binary_operations.ml核心模块测试覆盖率到80%+ 新增测试场景：
    - 所有算术运算的完整测试
    - 类型转换和强制转换路径
    - 错误处理和边界条件
    - 浮点数特殊值处理
...
module TestUtils = struct
  ...
  (* 浮点数近似相等比较 *)
  let float_approx_equal a b = abs_float (a -. b) < float_epsilon

  (* 值比较函数，特殊处理浮点数 *)
  let value_equal v1 v2 =
    match (v1, v2) with FloatValue f1, FloatValue f2 -> float_approx_equal f1 f2 | _ -> v1 = v2

  let value_testable =
    testable (fun fmt v -> Format.fprintf fmt "%s" (value_to_string v)) value_equal
...
let test_string_operations_comprehensive () =
  (* 基本字符串连接 *)
  TestUtils.check_binary_result "字符串连接" Add (StringValue "Hello") (StringValue " World")
    (StringValue "Hello World");
  TestUtils.check_binary_result "Concat运算符" Concat (StringValue "骆言") (StringValue "编程")
    (StringValue "骆言编程");

  (* 空字符串处理 *)
  TestUtils.check_binary_result "空字符串连接" Add (StringValue "") (StringValue "test")
    (StringValue "test");
  TestUtils.check_binary_result "连接空字符串" Add (StringValue "test") (StringValue "")
    (StringValue "test");
  TestUtils.check_binary_result "两个空字符串" Add (StringValue "") (StringValue "") (StringValue "");

  (* 特殊字符测试 *)
  TestUtils.check_binary_result "特殊字符连接" Add (StringValue "中文") (StringValue "测试")
    (StringValue "中文测试");
  TestUtils.check_binary_result "数字字符串" Add (StringValue "123") (StringValue "456")
    (StringValue "123456");
  TestUtils.check_binary_result "符号字符串" Add (StringValue "!@#") (StringValue "$%^")
    (StringValue "!@#$%^");
  ...
  TestUtils.check_binary_result "长字符串连接" Add (StringValue long_str1) (StringValue long_str2)
    (StringValue expected_long)
...
let test_comparison_operations_comprehensive () =
  ...
  (* 字符串比较 *)
  TestUtils.check_binary_result "字符串相等" Eq (StringValue "test") (StringValue "test")
    (BoolValue true);
  TestUtils.check_binary_result "字符串不等" Neq (StringValue "abc") (StringValue "def") (BoolValue true);
  TestUtils.check_binary_result "字符串小于" Lt (StringValue "apple") (StringValue "banana")
    (BoolValue true);
  TestUtils.check_binary_result "字符串大于" Gt (StringValue "zebra") (StringValue "apple")
    (BoolValue true);
...
let test_logical_operations_comprehensive () =
  (* And运算真值表 *)
  TestUtils.check_binary_result "true AND true" And (BoolValue true) (BoolValue true)
    (BoolValue true);
  TestUtils.check_binary_result "true AND false" And (BoolValue true) (BoolValue false)
    (BoolValue false);
  TestUtils.check_binary_result "false AND true" And (BoolValue false) (BoolValue true)
    (BoolValue false);
  TestUtils.check_binary_result "false AND false" And (BoolValue false) (BoolValue false)
    (BoolValue false);

  (* Or运算真值表 *)
  TestUtils.check_binary_result "true OR true" Or (BoolValue true) (BoolValue true) (BoolValue true);
  TestUtils.check_binary_result "true OR false" Or (BoolValue true) (BoolValue false)
    (BoolValue true);
  TestUtils.check_binary_result "false OR true" Or (BoolValue false) (BoolValue true)
    (BoolValue true);
  TestUtils.check_binary_result "false OR false" Or (BoolValue false) (BoolValue false)
    (BoolValue false)
...
let test_error_handling_comprehensive () =
  ...
  TestUtils.expect_runtime_error "整数取模零" (fun () ->
      execute_binary_op Mod (IntValue 10) (IntValue 0));

  (* 类型不匹配错误 - 使用无法进行类型转换的操作 *)
  TestUtils.expect_runtime_error "单元值与整数相乘" (fun () -> execute_binary_op Mul UnitValue (IntValue 5));
  TestUtils.expect_runtime_error "字符串取模运算" (fun () ->
      execute_binary_op Mod (StringValue "abc") (StringValue "def"));
  TestUtils.expect_runtime_error "单元值除法运算" (fun () -> execute_binary_op Div UnitValue UnitValue);

  (* 不支持的运算 *)
  TestUtils.expect_runtime_error "字符串除法" (fun () ->
      execute_binary_op Div (StringValue "abc") (StringValue "def"));
  TestUtils.expect_runtime_error "布尔值加法" (fun () ->
      execute_binary_op Add (BoolValue true) (BoolValue false));
...
let test_type_conversion_paths () =
  ...
  TestUtils.expect_runtime_error "尝试转换不兼容类型进行算术运算" (fun () ->
      execute_binary_op Add (StringValue "hello") (IntValue 42));

  TestUtils.expect_runtime_error "尝试转换不兼容类型进行比较运算" (fun () ->
      execute_binary_op Lt (BoolValue true) (FloatValue 3.14));

  (* 测试with_conversion路径 *)
  TestUtils.expect_runtime_error "转换后仍不兼容的运算" (fun () ->
      execute_binary_op Mul (StringValue "test") (BoolValue false));
...
let test_internal_function_paths () =
  ...
  TestUtils.check_binary_result "覆盖find_arithmetic_operation-Add" Add (IntValue 1) (IntValue 2)
    (IntValue 3);
  TestUtils.check_binary_result "覆盖find_arithmetic_operation-Sub" Sub (IntValue 5) (IntValue 2)
    (IntValue 3);
  TestUtils.check_binary_result "覆盖find_arithmetic_operation-Mul" Mul (IntValue 3) (IntValue 4)
    (IntValue 12);
  TestUtils.check_binary_result "覆盖find_arithmetic_operation-Div" Div (IntValue 12) (IntValue 3)
    (IntValue 4);
  TestUtils.check_binary_result "覆盖find_arithmetic_operation-Mod" Mod (IntValue 13) (IntValue 5)
    (IntValue 3);

  (* 测试字符串运算路径 *)
  TestUtils.check_binary_result "字符串Add路径" Add (StringValue "a") (StringValue "b")
    (StringValue "ab");
  TestUtils.check_binary_result "字符串Concat路径" Concat (StringValue "x") (StringValue "y")
    (StringValue "xy");
...
let test_boundary_and_special_cases () =
  ...
  (* 浮点特殊值 *)
  TestUtils.check_binary_result "无穷大相等" Eq (FloatValue infinity) (FloatValue infinity)
    (BoolValue true);
  TestUtils.check_binary_result "负无穷大相等" Eq (FloatValue neg_infinity) (FloatValue neg_infinity)
    (BoolValue true);

  (* Unicode字符串 *)
  TestUtils.check_binary_result "Unicode字符串连接" Add (StringValue "你好") (StringValue "世界")
    (StringValue "你好世界");
  TestUtils.check_binary_result "Emoji字符串连接" Add (StringValue "😀") (StringValue "😂")
    (StringValue "😀😂");
