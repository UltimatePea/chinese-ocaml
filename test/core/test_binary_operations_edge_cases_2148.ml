(** 二元运算模块边界情况测试 - 专门针对未覆盖代码路径 目标：达到80%+覆盖率，解决PR #2152中Delta代理识别的覆盖率缺陷 Author: Whisky, PR Worker Fix
    #2148: 针对性解决未覆盖的错误处理路径 *)
...
let test_string_comparison_operators () =
  ...
  let test_string_le () =
    ...
    match result with BoolValue true -> () (* "a" <= "b" 应该为真 *) | _ -> failwith "字符串 <= 比较失败"
  in

  (* 测试字符串的 > 运算符 *)
  let test_string_gt () =
    let result = execute_binary_op Gt (StringValue "b") (StringValue "a") in
    match result with BoolValue true -> () (* "b" > "a" 应该为真 *) | _ -> failwith "字符串 > 比较失败"
  in
...
let test_float_operation_edge_cases () =
  ...
  let test_float_negation () =
    ...
    match result with FloatValue f when f < 0.0 -> () (* 预期的负数结果 *) | _ -> failwith "浮点数取反失败"
  in
...
(** 测试运行器 *)
let () = run "二元运算边界情况测试 - 80%覆盖率目标" [ ("edge_cases_coverage", edge_cases_tests) ]
