...
let test_type_substitution () =
  ...
  with Not_found -> check bool "替换查找失败" false true
...
let test_advanced_type_construction () =
  ...
  check bool "记录类型创建" true (match record_type with RecordType_T _ -> true | _ -> false);

  (* 引用类型 *)
  let ref_type = RefType_T IntType_T in
  check bool "引用类型创建" true (match ref_type with RefType_T IntType_T -> true | _ -> false)
...
let test_nested_compound_types () =
  ...
  check bool "元组数组类型构造" true
    (match tuple_array with
    | ArrayType_T (TupleType_T [ StringType_T; IntType_T ]) -> true
    | _ -> false);

  (* 嵌套函数类型 *)
  let nested_func = FunType_T (IntType_T, FunType_T (StringType_T, BoolType_T)) in
  check bool "嵌套函数类型构造" true
    (match nested_func with
    | FunType_T (IntType_T, FunType_T (StringType_T, BoolType_T)) -> true
    | _ -> false)
