(** 统一Token转换引擎深度测试 - Phase 3 核心业务逻辑测试
    ...
    Author: Alpha, 主工作代理
    针对 conversion_engine.ml 的核心业务逻辑深度测试
    符合 Issue #1695 提出的测试质量标准要求
...
module ErrorHandlingTests = struct
  ...
  let test_error_messages_consistency () =
    ...
    let messages = [
      error_to_string compat_error;
      error_to_string valid_error; 
      error_to_string system_error;
    ] in
  ...
  let test_error_handling_side_effects () =
    ...
    check bool "错误处理不抛异常" true (try test_fn (); true with _ -> false)
end
...
module ConversionStrategyTests = struct
  ...
  let test_auto_strategy_behavior () =
    ...
    let expected_total = List.length classical_converters + 
                        List.length modern_converters + 
                        List.length lexer_converters in
...
module ConverterRegistryTests = struct
  let test_registry_initialization () =
    ...
    (* 应该有一些预注册的转换器 *)
    check bool "有预注册的转换器" true 
      ((initial_classical + initial_modern + initial_lexer) > 0)
  ...
  let test_converter_execution () =
    (* 测试注册的转换器能正确执行 *)
    let test_converter input = 
      if input = "中文关键字" then Some "ChineseKeyword" else None in
...
module CoreConversionTests = struct
  ...
  let test_single_token_conversion_error_handling () =
    (* 测试单个token转换的错误处理 *)
    let result = convert_token ~strategy:Classical ~source:"unknown_token" ~target_format:"unknown_format" in
    match result with
    | Success _ -> 
  ...
  let test_batch_conversion_all_success () =
    ...
    match result with
    | Success results -> 
        check int "结果数量正确" (List.length tokens) (List.length results)
...
module FastPathTests = struct
  let test_common_token_recognition () =
    ...
    List.iter2 (fun expected actual ->
      check string "快速路径转换正确" expected actual
  ...
  let test_fast_path_performance_characteristics () =
    ...
    check bool "快速路径性能合理" true (duration < 0.1) (* 应该在100ms内完成 *)
  ...
  let test_core_fallback_integration () =
    ...
    | Error _ -> check bool "已知token应该转换成功" false true;
...
module BackwardCompatibilityTests = struct
  let test_optional_conversion_api () =
    ...
    | None -> check bool "已知token应该有返回值" false true;
  ...
  let test_exception_message_chinese () =
    ...
    let test_chinese_error () =
      ...
      with Invalid_argument msg ->
        (* 检查异常消息包含中文 *)
        String.length msg > 0 && (let contains_chinese s = 
          let rec check i = 
            if i >= String.length s then false
            else if Char.code (String.get s i) > 127 then true
            else check (i + 1) in
          check 0 in contains_chinese msg)
      | _ -> false in
...
module StatisticsTests = struct
  ...
  let test_statistics_numerical_accuracy () =
    ...
    (* 提取数字信息（简单的字符包含检查）*)
    let contains_digits = String.contains stats '0' || String.contains stats '1' ||
                         String.contains stats '2' || String.contains stats '3' ||
                         String.contains stats '4' || String.contains stats '5' in
  ...
  let test_statistics_consistency_with_registry () =
    ...
    (* 验证统计信息反映了实际的注册表状态 *)
    check bool "统计信息反映注册表状态" true 
      (classical_count >= 0 && modern_count >= 0 && lexer_count >= 0)
...
module ChineseLanguageTests = struct
  ...
  let test_chinese_token_handling () =
    ...
    List.iter (fun token ->
      ...
      match result with
      | Success _ | Error _ -> check bool ("能处理中文token: " ^ token) true true
    ) chinese_tokens
  ...
  let test_mixed_language_handling () =
    ...
    List.iter (fun token ->
      ...
      match result with
      | Some _ | None -> check bool ("能处理混合token: " ^ token) true true
    ) mixed_tokens
end
...
module PerformanceTests = struct
  let test_large_batch_conversion () =
    ...
    check bool "大批量转换性能合理" true (duration < 2.0) (* 应该在2秒内完成 *)
  ...
  let test_boundary_conditions () =
    ...
    List.iter (fun input ->
      ...
      match result with
      | Some _ | None -> check bool ("能处理边界输入: " ^ (String.sub input 0 (min 10 (String.length input)))) true true
    ) test_inputs
end
...
module IntegrationTests = struct
  let test_full_conversion_pipeline () =
    ...
    | (Some _, Success _, Some _) -> 
        check bool "转换管线一致性" true true
    | _ -> 
  ...
  let test_error_propagation () =
    ...
    | Success _ -> check bool "核心转换应该失败" false true;
  ...
  let test_statistics_integration () =
    ...
    check bool "统计信息能反映注册变化" true 
      (String.length updated_stats >= String.length initial_stats)
end
...
let core_conversion_tests = [
  ...
  test_case "单token转换错误处理测试" `Quick CoreConversionTests.test_single_token_conversion_error_handling;
...
let () = run "Conversion Engine Deep Testing" [
