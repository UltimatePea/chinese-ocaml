...
let test_add_io_functions () =
  ...
  check type_testable "读取函数类型" 
    (create_fun_type UnitType_T StringType_T) 
    read_entry.symbol_type
...
let test_add_list_functions () =
  ...
  let expected_concat_type = create_fun_type 
    (create_list_type (create_type_var "'a"))
    (create_fun_type (create_list_type (create_type_var "'a")) (create_list_type (create_type_var "'a"))) in
...
let test_list_higher_order_functions () =
  ...
  let expected_filter_type = create_fun_type
  ...
  let expected_map_type = create_fun_type
    (create_fun_type (create_type_var "'a") (create_type_var "'b"))
    (create_fun_type (create_list_type (create_type_var "'a")) (create_list_type (create_type_var "'b"))) in
...
let test_add_array_functions () =
  ...
  let expected_create_type = create_fun_type 
    IntType_T 
    (create_fun_type (create_type_var "'a") (create_array_type (create_type_var "'a"))) in
...
let test_add_math_functions () =
  ...
  check type_testable "绝对值函数类型" 
    (create_fun_type IntType_T IntType_T) 
    abs_entry.symbol_type;
  
  (* 检查平方根函数 *)
  let sqrt_entry = SymbolTable.find "平方根" math_symbols in
  check type_testable "平方根函数类型" 
    (create_fun_type FloatType_T FloatType_T) 
    sqrt_entry.symbol_type;
...
let test_add_string_functions () =
  ...
  check type_testable "字符串长度函数类型" 
    (create_fun_type StringType_T IntType_T) 
    length_entry.symbol_type;
  
  (* 检查字符串连接函数 *)
  let concat_entry = SymbolTable.find "字符串连接" string_symbols in
  let expected_concat_type = create_fun_type StringType_T (create_fun_type StringType_T StringType_T) in
...
let test_add_file_functions () =
  ...
  check type_testable "读取文件函数类型" 
    (create_fun_type StringType_T StringType_T) 
    read_entry.symbol_type;
  
  (* 检查写入文件函数 *)
  let write_entry = SymbolTable.find "写入文件" file_symbols in
  let expected_write_type = create_fun_type StringType_T (create_fun_type StringType_T UnitType_T) in
  ...
  check type_testable "文件存在函数类型" 
    (create_fun_type StringType_T BoolType_T) 
    exists_entry.symbol_type
...
let test_add_builtin_functions () =
  ...
  | builtin_scope :: original_scope :: [] ->
    ...
    check int "内置函数总数" 18 builtin_count; (* 2+5+3+3+2+3 = 18 *)
...
let test_all_builtin_functions_integration () =
  ...
  (* 测试每个类别的代表性函数 *)
  check bool "I/O-打印函数可访问" true (check_builtin_function enhanced_context "打印" (create_fun_type (create_type_var "'a") UnitType_T));
  check bool "列表-长度函数可访问" true (check_builtin_function enhanced_context "长度" (create_fun_type (create_list_type (create_type_var "'a")) IntType_T));
  check bool "数组-数组长度函数可访问" true (check_builtin_function enhanced_context "数组长度" (create_fun_type (create_array_type (create_type_var "'a")) IntType_T));
  check bool "数学-绝对值函数可访问" true (check_builtin_function enhanced_context "绝对值" (create_fun_type IntType_T IntType_T));
  check bool "字符串-字符串长度函数可访问" true (check_builtin_function enhanced_context "字符串长度" (create_fun_type StringType_T IntType_T));
  check bool "文件-读取文件函数可访问" true (check_builtin_function enhanced_context "读取文件" (create_fun_type StringType_T StringType_T))
...
let test_context_invariants () =
  ...
  (* 检查原始上下文的其他字段保持不变 *)
  check (option type_testable) "函数返回类型不变" 
    initial_context.current_function_return_type 
    enhanced_context.current_function_return_type;
  check (list string) "错误列表不变" 
    initial_context.error_list 
    enhanced_context.error_list;
  check (list (pair string string)) "宏列表不变" 
...
let io_function_tests = [
  "I/O函数添加", `Quick, test_add_io_functions;
  "I/O函数完整性", `Quick, test_io_functions_completeness;
]
