...
module TestUtils = struct
  ...
  let safe_call_builtin name args =
    try
      Some (call_builtin_function name args)
    with
    | RuntimeError _ -> None
    | _ -> None
end
...
module BuiltinTableStructureTests = struct
  ...
  let test_function_table_structure () =
    ...
    List.iter (fun (name, value) ->
      check TestUtils.bool_testable (Printf.sprintf "函数名'%s'不应为空" name) true (String.length name > 0);
      check TestUtils.bool_testable (Printf.sprintf "函数'%s'应为内置函数值" name) true 
  ...
  let test_function_table_completeness () =
    ...
    List.iter (fun (category, expected_functions) ->
      let found_count = List.fold_left (fun acc expected ->
        if List.exists (fun name -> String.contains name (String.get expected 0)) function_names 
        then acc + 1 else acc
      ) 0 expected_functions in
...
module HashTableCacheTests = struct
  ...
  let test_hash_table_consistency () =
    ...
    List.iter (fun name ->
      check TestUtils.bool_testable (Printf.sprintf "哈希表应包含函数'%s'" name) true 
    ...
    List.iter (fun name ->
      check TestUtils.bool_testable (Printf.sprintf "哈希表不应包含'%s'" name) false
        (is_builtin_function name)
    ) nonexistent_names
  ...
  let test_function_call_error_handling () =
    ...
    check TestUtils.bool_testable "调用不存在的函数应抛出错误" true
      (TestUtils.expect_runtime_error (fun () ->
        call_builtin_function "完全不存在的函数" []));
...
module FunctionQueryTests = struct
  let test_function_existence_check () =
    (* 测试已知存在的函数 *)
    let likely_existing_functions = [
      "整数转字符串"; "字符串转整数"; "布尔值转字符串";
      "字符串长度"; "打印"; "读取"
    ] in
    
    List.iter (fun name ->
      if is_builtin_function name then
        Printf.printf "    确认函数'%s'存在\n" name
      else
    ...
    let nonexistent_functions = [
      "完全不存在的函数"; ""; "fake_function"; "test_123"
    ] in
    
    List.iter (fun name ->
      check TestUtils.bool_testable (Printf.sprintf "'%s'不应为内置函数" name) false
        (is_builtin_function name)
  ...
  let test_get_function_names () =
    ...
    (* 检查列表长度与函数表一致 *)
    check TestUtils.int_testable "函数名称列表长度应与函数表一致" 
      (List.length builtin_functions) (List.length names);
    
    (* 检查是否包含预期的函数名 *)
    let contains_conversion_func = List.exists (fun name -> 
      String.contains name (String.get "转" 0)) names in
    if contains_conversion_func then
      Printf.printf "    确认包含类型转换函数\n"
    else
  ...
  let test_query_consistency () =
    ...
    List.iter (fun name ->
      check TestUtils.bool_testable (Printf.sprintf "函数'%s'应通过存在性检查" name) true
        (is_builtin_function name)
    ) all_names;
    
    (* 验证函数表中的每个函数都能在名称列表中找到 *)
    List.iter (fun (name, _) ->
      check TestUtils.bool_testable (Printf.sprintf "函数表中的'%s'应在名称列表中" name) true
...
module ModuleIntegrationTests = struct
  let test_submodule_integration () =
    ...
    List.iter (fun (module_name, keywords) ->
      let found_functions = List.filter (fun func_name ->
  ...
  let test_function_categorization () =
    ...
    let count_functions_with_pattern pattern = 
      List.length (List.filter (fun name -> 
        String.contains name (String.get pattern 0)
      ) function_names) in
    ...
    List.iter (fun (category, pattern) ->
...
module PerformanceOptimizationTests = struct
  ...
  let test_memory_efficiency () =
    ...
    (* 验证函数表大小合理 *)
    check TestUtils.bool_testable "函数表大小应合理" true 
      (function_count > 10 && function_count < 1000);
    
    (* 检查是否有重复的函数实例 *)
    let names = List.map fst builtin_functions in
    let unique_names = List.sort_uniq String.compare names in
    check TestUtils.int_testable "不应有重复的函数名" 
...
module ErrorHandlingTests = struct
  let test_invalid_function_calls () =
    ...
    List.iter (fun name ->
      check TestUtils.bool_testable (Printf.sprintf "特殊字符函数名'%s'应失败" name) true
        (TestUtils.expect_runtime_error (fun () ->
  ...
  let test_parameter_edge_cases () =
    ...
    let test_function_if_exists name test_cases =
      if is_builtin_function name then (
        List.iter (fun (desc, args, should_succeed) ->
  ...
  let test_error_message_quality () =
    ...
    let test_error_message func_name args expected_keywords =
      ...
      with
      | RuntimeError msg ->
          let contains_keyword = List.exists (fun keyword ->
            String.contains msg (String.get keyword 0)
          ) expected_keywords in
          if contains_keyword then
            Printf.printf "    错误消息包含预期关键词\n"
          else
            Printf.printf "    错误消息: %s\n" msg
      | _ ->
          Printf.printf "    错误: 抛出了错误类型的异常\n"
    in
...
module ChineseProgrammingTests = struct
  let test_chinese_function_names () =
    ...
    (* 检查是否有中文函数名 *)
    let chinese_functions = List.filter (fun name ->
      (* 简单检查是否包含中文字符 *)
      let has_chinese = ref false in
      String.iter (fun c -> 
        if Char.code c > 127 then has_chinese := true
      ) name;
      !has_chinese
    ) function_names in
...
module FunctionalityCompletenessTests = struct
  let test_essential_functions_availability () =
    ...
    List.iter (fun (category, functions) ->
      let available_count = List.fold_left (fun acc func_name ->
        if is_builtin_function func_name then acc + 1 else acc
      ) 0 functions in
    ...
    let test_consistency func_name args =
      if is_builtin_function func_name then (
        ...
        | (Some v1, Some v2) when v1 = v2 ->
            Printf.printf "    函数'%s'调用一致性: 通过\n" func_name
        | (Some v1, Some v2) ->
            Printf.printf "    函数'%s'调用不一致: %s vs %s\n" func_name 
              (value_to_string v1) (value_to_string v2)
        | (None, None) ->
            Printf.printf "    函数'%s'调用一致性: 都失败\n" func_name
        | _ ->
            Printf.printf "    函数'%s'调用不一致: 一次成功一次失败\n" func_name
      )
    in
  ...
  let test_comprehensive_coverage () =
    ...
    let callable_functions = List.fold_left (fun acc (name, _) ->
...
let test_suite = [
  ...
  ("功能完整性", [
    test_case "基本函数可用性" `Quick FunctionalityCompletenessTests.test_essential_functions_availability;
