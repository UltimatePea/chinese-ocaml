...
module TestHelpers = struct
  ...
  let rec expr_equal expr1 expr2 = 
    ...
    | (MatchExpr (e1, cases1), MatchExpr (e2, cases2)) ->
        expr_equal e1 e2 && List.length cases1 = List.length cases2 &&
...
let test_parse_natural_arithmetic_continuation () =
  ...
  with
  | _ -> check bool "算术延续表达式解析失败" false true;
  ...
  with
  | _ -> check bool "无延续解析失败" false true
...
let test_parse_ancient_match_expression () =
  ...
  let tokens = [
    ...
    StringToken "one";         (* 表达式 *)
    AncientObserveEndKeyword;  (* 观毕 *)
    EOF
  ] in
  
  let expected_case = { 
    pattern = LiteralPattern (IntLit 1); 
    guard = None; 
    expr = StringExpr "one" 
  } in
...
let test_parse_wenyan_let_minimal () =
  ...
  let tokens = [
    ...
    OneKeyword;                (* 一 *)
    NameKeyword;               (* 名曰 *)
    QuotedIdentifierToken "x"; (* 「x」 *)
    ValueKeyword;              (* 其值 *)
    IntToken 5;                (* 五 *)
    InKeyword;                 (* 在 *)
    Identifier "x";            (* body表达式 *)
    EOF
  ] in
...
let test_parse_wenyan_simple_let_no_dot () =
  ...
  let tokens = [
    SetKeyword;                (* 设 *)
    QuotedIdentifierToken "y"; (* 「y」 *)
    AsForKeyword;              (* 为 *)
    IntToken 20;               (* 二十 *)
    Identifier "y";            (* body表达式 *)
    EOF
  ] in
...
let test_error_handling_ancient_function () =
  (* 测试古雅体函数定义错误处理 *)
  TestHelpers.check_parse_failure 
    "缺少关键字的古雅体函数定义" 
...
let test_performance_ancient () =
  ...
  let large_list_tokens = 
    ...
    (List.flatten (List.init 50 (fun i -> 
      [IntToken i] @ 
      (match i mod 3 with
  ...
  with
  | _ -> check bool "大型古雅体列表解析失败" false true
...
let test_integration_with_other_modules () =
  ...
  with
  | _ -> check bool "模块集成失败" false true
...
let test_suite = [
  ...
  (* 5. 复杂和集成测试 *)
  ("复杂和集成测试", [
    test_case "复杂古雅体表达式" `Quick test_complex_ancient_expressions;
  ]);
  ...
  (* 7. 集成和兼容性测试 *)
  ("集成和兼容性", [
    test_case "模块集成测试" `Quick test_integration_with_other_modules;
  ])
]
