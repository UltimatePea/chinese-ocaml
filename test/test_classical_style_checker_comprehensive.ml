(** 古雅体适用性检查器综合测试 - 修复 Issue #1342
    ...
    作为测试工程师，为古雅体风格实施状态检查创建全面的测试覆盖。
    这些测试确保古雅体适用性检查器正确识别和报告各种古雅体使用问题。
    
    Author: Echo, 测试工程师代理
    Fix #1342 - 检查古雅体风格的实施状态
*)
...
let test_basic_classical_style_check () =
  ...
  let has_excessive = List.exists (fun r -> 
...
let test_mixed_style_detection () =
  ...
  let has_mixed = List.exists (fun r ->
    match r.violation with
  ...
  let has_mixed2 = List.exists (fun r ->
    match r.violation with
    | ModernizationSuggestion(_, issue, _) when issue = "古今混用" -> true
    | _ -> false
  ) results2 in
...
let test_ai_unfriendly_detection () =
  ...
  let has_ai_issue = List.exists (fun r ->
    match r.violation with
  ...
  (* 验证严重度为Error *)
  let error_severity = List.exists (fun r ->
    r.severity = Error
  ) results1 in
  ...
  let has_ai_issue2 = List.exists (fun r ->
    match r.violation with
    | ModernizationSuggestion(_, issue, _) when issue = "AI理解困难" -> true
    | _ -> false
  ) results2 in
...
let test_severity_filter () =
  ...
  let has_warning_or_error = List.for_all (fun r -> 
...
let test_result_structure () =
  ...
  (* 检查结果结构的完整性 *)
  Alcotest.(check bool) "包含violation字段" true (
    match result.violation with
    | ModernizationSuggestion(_, _, _) -> true
    | _ -> false
  );
  
  (* 检查confidence字段 *)
  Alcotest.(check bool) "confidence在合理范围内" true (
    result.confidence >= 0.0 && result.confidence <= 1.0
  );
  
  (* 检查ai_friendly字段 *)
  Alcotest.(check bool) "ai_friendly字段存在" true (
