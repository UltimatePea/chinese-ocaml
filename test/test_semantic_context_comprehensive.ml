...
let test_create_initial_context () =
  ...
  check (list (list (pair string string))) "scope_stack" [[]] 
    (List.map (fun st -> SymbolTable.bindings st |> List.map (fun (k, v) -> (k, v.symbol_name))) ctx.scope_stack);
...
let test_symbol_table_to_env () =
  ...
  let ctx_with_symbols = 
    ...
    |> fun c -> add_symbol c "变量C" BoolType_T true
  in
...
let test_complex_types () =
  ...
  let ctx_with_complex = 
    ctx
    |> fun c -> add_symbol c "整数列表" list_type false
    |> fun c -> add_symbol c "测试函数" function_type false
    |> fun c -> add_symbol c "记录变量" record_type false
  in
...
let test_edge_cases () =
  ...
  let ctx_with_redefined = 
    ctx
    |> fun c -> add_symbol c "重定义测试" IntType_T false
    |> fun c -> add_symbol c "重定义测试" StringType_T true
  in
...
let () =
  run "Semantic_context 综合测试" [
    "基础符号表条目", [
      test_case "创建符号表条目" `Quick test_create_symbol_entry;
    ];
    "语义上下文初始化", [
    ...
    "类型定义管理", [
      test_case "类型定义添加和查找" `Quick test_type_definition_operations;
    ];
    "环境转换", [
      test_case "符号表到环境转换" `Quick test_symbol_table_to_env;
    ];
    "错误处理和边界条件", [
    ...
    "性能测试", [
      test_case "大量符号处理" `Slow test_large_symbol_table;
    ];
  ]