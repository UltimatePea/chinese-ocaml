...
module TestCCodegen = struct
  ...
  let test_format_function_def () =
    ...
    check string "函数定义格式化正确" "luoyan_function_create(func_name_impl_param1, env, \"func_name\")"
      result
  ...
  let test_format_var_expr () =
    ...
    check string "变量表达式格式化正确" "({ luoyan_value_t* expr_var = expr_code; luoyan_match(expr_var); })"
      result
