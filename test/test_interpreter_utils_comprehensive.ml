...
module ClosestVariableTests = struct
  ...
  let test_basic_closest_variable () =
    ...
    (* 测试拼写错误修正 *)
    let closest1 = find_closest_var "variabe" available_vars in  (* 缺少l *)
    let closest2 = find_closest_var "valu" available_vars in     (* 缺少e *)
    let closest3 = find_closest_var "resul" available_vars in    (* 缺少t *)
  ...
  let test_no_similar_variables () =
    ...
    let check_none desc actual =
      match actual with
      | None -> check bool desc true true
      | Some _ -> check bool desc false true
    in
  ...
  let test_chinese_variable_similarity () =
    ...
    let closest1 = find_closest_var "变数" available_vars in  (* 变量的近似 *)
    let closest2 = find_closest_var "数字" available_vars in  (* 数值的近似 *)
...
module PerformanceTests = struct
  ...
  let test_bulk_similarity_calculation_performance () =
    ...
    let check_result = match closest with
...
let test_suite = [
  ("基本功能测试", [
    test_case "函数可用性测试" `Quick BasicFunctionTests.test_function_availability;
  ]);
