...
let test_apply_subst_to_scheme_no_conflict () =
  let subst = create_subst [("'a", IntType_T); ("'b", StringType_T)] in
  let scheme = create_scheme ["'c"] (create_fun_type (create_type_var "'a") (create_type_var "'c")) in
  ...
  check (testable (fun fmt (TypeScheme (vars, typ)) -> 
...
let test_apply_subst_to_scheme_with_conflict () =
  let subst = create_subst [("'a", IntType_T); ("'b", StringType_T)] in
  let scheme = create_scheme ["'a"; "'c"] (create_fun_type (create_type_var "'a") (create_type_var "'b")) in
  (* 'a被绑定，不应该被替换；'b未绑定，应该被替换 *)
  let expected = create_scheme ["'a"; "'c"] (create_fun_type (create_type_var "'a") StringType_T) in
  let actual = apply_subst_to_scheme subst scheme in
  check (testable (fun fmt (TypeScheme (vars, typ)) -> 
    Fmt.string fmt (String.concat "," vars ^ " -> " ^ string_of_typ typ)) (=)) 
...
let test_instantiate_basic () =
  let scheme = create_scheme ["'a"; "'b"] (create_fun_type (create_type_var "'a") (create_type_var "'b")) in
...
let composition_tests = [
  "基础替换合成", `Quick, test_compose_subst_basic;
  "替换合成优先级", `Quick, test_compose_subst_priority;
]

let generalization_tests = [
  "基础类型泛化", `Quick, test_generalize_basic;
  "有环境约束的泛化", `Quick, test_generalize_with_env;
]
...
let boundary_tests = [
  "空替换测试", `Quick, test_empty_subst;
  "深度嵌套类型替换", `Quick, test_deep_nested_types;
]

let integration_tests = [
  "替换和泛化集成", `Quick, test_subst_generalize_integration;
]
