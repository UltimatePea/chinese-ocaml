...
let test_tone_data_loader () =
  ...
  try
    (* 测试声调数据解析 *)
    let sample_tone_json = "{\"rhyme_groups\": {
    ...
    let (fallback_ping, fallback_shang, fallback_qu, fallback_ru) = 
      Poetry_tone_data.get_tone_data_with_fallback () in
    Printf.printf "✓ 降级数据获取成功: 平声%d字, 上声%d字, 去声%d字, 入声%d字\n" 
      (List.length fallback_ping) (List.length fallback_shang) 
      (List.length fallback_qu) (List.length fallback_ru);
...
let test_type_unification () =
  ...
  try
    ...
    (* 测试类型转换函数 *)
    let category_str = Poetry_types.Rhyme_types.rhyme_category_to_string 
      Poetry_types.Rhyme_types.PingSheng in
...
let run_all_tests () =
  ...
  if List.length passed = List.length tests then
    print_endline "🎉 所有测试通过！JSON统一化重构成功！"
  else
    print_endline "⚠️  部分测试失败，需要进一步调试";
