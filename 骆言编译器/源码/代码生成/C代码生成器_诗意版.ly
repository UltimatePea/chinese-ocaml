...
// === 主要导出函数：如诗词的题跋，总结全文 ===
函数 生成丙代码（程序： AST。程序） －〉 结果（字符串， Errors。错误列表） 『
  // 整个代码生成过程如写诗作词，需要灵感与技巧并重
  
  上下文 ＝ 创建生成上下文（）；
  
  // 添加必要的头文件，如诗词的题记
  上下文1 ＝ 添加代码行（上下文， 「号include 〈stdio。h〉「）；
  上下文1 ＝ 添加代码行（上下文1， 「号include 〈stdlib。h〉「）；
  上下文1 ＝ 添加代码行（上下文1， 「号include 〈string。h〉「）；
  上下文1 ＝ 添加代码行（上下文1， 「「）；
