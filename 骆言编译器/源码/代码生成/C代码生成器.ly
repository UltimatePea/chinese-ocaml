...
夫「生成变量名」者受 上下文 前缀 焉算法乃
  设「id」为上下文.下一个变量ID 在
  上下文.下一个变量ID :=id + 1;
  『luoyan_var_』 ^ 前缀 ^ 『_』 ^ （字符串_从_整数 id） 也

「：生成唯一标签名：」
夫「生成标签名」者受 上下文 前缀 焉算法乃
  设「id」为上下文.下一个标签ID 在
  上下文.下一个标签ID :=id + 1;
  『luoyan_label_』 ^ 前缀 ^ 『_』 ^ （字符串_从_整数 id） 也
...
夫「转义标识符」者受 名称 焉算法乃
  ...
  在
  字符串_从_字符列表 （转义字符 （字符串_到_字符列表 名称） 空空如也） 也
...
夫「为C转义字符串」者受 字符串 焉算法乃
  ...
  在
  字符串_从_字符列表 （转义字符 （字符串_到_字符列表 字符串） 空空如也） 也
...
夫「生成二元运算」者受 上下文 运算符 左表达式 右表达式 焉算法乃
  ...
  | 字符串连接 -> 『luoyan_string_concat（』 ^ 左代码 ^ 『, 』 ^ 右代码 ^ 『）』 观毕也
...
夫「生成一元运算」者受 上下文 运算符 表达式 焉算法乃
  ...
  | 数值取反 -> 『luoyan_subtract（luoyan_int（0）, 』 ^ 操作数代码 ^ 『）』 观毕也
...
夫「生成让绑定表达式」者受 上下文 变量 值表达式 主体表达式 焉算法乃
  ...
  『（{ luoyan_env_bind（env, \』『 ^ 转义变量 ^ 』\『, 』 ^ 值代码 ^ 『）; 』 ^ 主体代码 ^ 『; }）』 也
...
夫「生成函数表达式」者受 上下文 参数列表 主体 焉算法乃
  ...
  | 空空如也 -> 『luoyan_unit（）』
  | 第一个参数 :: _ ->
    『luoyan_function_create（』 ^ 函数名 ^ 『_impl_』 ^ 第一个参数 ^ 『, env, \』『 ^ 函数名 ^ 』\『）』 观毕也
...
夫「生成匹配表达式」者受 上下文 表达式 模式列表 焉算法乃
  ...
  『（{ luoyan_value_t* 』 ^ 表达式变量 ^ 『=』 ^ 表达式代码 ^ 『; 』 ^ （生成模式列表 模式列表） ^ 『; }）』 也
...
夫「生成元组表达式」者受 上下文 表达式列表 焉算法乃
  ...
  | _ ->
    ...
    『（{ luoyan_value_t* 』 ^ 元组变量 ^ 『=』 ^ 初始化代码 ^ 『; 』 ^ 赋值代码 ^ 『; 』 ^ 元组变量 ^ 『; }）』 观毕也
...
夫「生成记录表达式」者受 上下文 字段列表 焉算法乃
  ...
  | _ ->
    ...
    ） 字段列表 在
    设「赋值代码」为字符串_连接 『; 』 字段赋值 在
    『（{ luoyan_value_t* 』 ^ 记录变量 ^ 『=』 ^ 初始化代码 ^ 『; 』 ^ 赋值代码 ^ 『; 』 ^ 记录变量 ^ 『; }）』 观毕也

「：生成字段访问表达式代码：」
夫「生成字段访问表达式」者受 上下文 记录表达式 字段名 焉算法乃
  设「记录代码」为生成表达式 上下文 记录表达式 在
  设「转义字段」为转义标识符 字段名 在
  『luoyan_record_get_field（』 ^ 记录代码 ^ 『, \』『 ^ 转义字段 ^ 』\『）』 也
...
夫「生成构造器表达式」者受 上下文 构造器名 参数列表 焉算法乃
  ...
  | _ ->
    ...
    『luoyan_constructor_create（\』『 ^ 构造器名称 ^ 』\『, luoyan_array_from_values（』 ^ 参数数组 ^ 『））』 观毕也
