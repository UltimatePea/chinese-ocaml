...
(* 生成唯一变量名 *)
夫 生成变量名 者 受 上下文 前缀 焉 算法 乃
  设 id 为 上下文.下一个变量ID 在
  上下文.下一个变量ID := id + 1;
  "luoyan_var_" ^ 前缀 ^ "_" ^ (字符串_从_整数 id) 是谓

(* 生成唯一标签名 *)
夫 生成标签名 者 受 上下文 前缀 焉 算法 乃
  设 id 为 上下文.下一个标签ID 在
  上下文.下一个标签ID := id + 1;
  "luoyan_label_" ^ 前缀 ^ "_" ^ (字符串_从_整数 id) 是谓

(* 转义标识符名称 *)
夫 转义标识符 者 受 名称 焉 算法 乃
  设 递归 转义字符 为 函数 字符列表 结果 ->
    观 字符列表 之 性
    | [] -> 反向 结果
    | 字符 :: 剩余 ->
      若 (字符 >= '0' && 字符 <= '9') || 
  ...
  在
  字符串_从_字符列表 (转义字符 (字符串_到_字符列表 名称) []) 是谓

(* 生成C类型名 *)
夫 骆言类型_到_C类型 者 受 类型 焉 算法 乃
  观 类型 之 性
  | "整数" -> "luoyan_int_t"
  | "浮点数" -> "luoyan_float_t"
  | "字符串" -> "luoyan_string_t*"
  ...
  | "数组" -> "luoyan_array_t*"
  | "引用" -> "luoyan_ref_t*"
  | _ -> "luoyan_value_t*" 观毕 是谓

(* 生成表达式代码 *)
设 递归 生成表达式 为 函数 上下文 表达式 ->
  观 表达式 之 性
...
(* 生成字面量代码 *)
夫 生成字面量 者 受 字面量 焉 算法 乃
  观 字面量 之 性
  | 整数字面量 i -> "luoyan_int(" ^ (字符串_从_整数 i) ^ "L)"
  | 浮点字面量 f -> "luoyan_float(" ^ (字符串_从_浮点数 f) ^ ")"
  | 字符串字面量 s -> 
    设 转义字符串 为 为C转义字符串 s 在
    "luoyan_string(\"" ^ 转义字符串 ^ "\")"
  | 布尔字面量 b -> 
    "luoyan_bool(" ^ (若 b 则 答 "true" 余者 答 "false") ^ ")"
  | 单元字面量 -> "luoyan_unit()" 观毕 是谓

(* 为C转义字符串 *)
夫 为C转义字符串 者 受 字符串 焉 算法 乃
  设 递归 转义字符 为 函数 字符列表 结果 ->
    观 字符列表 之 性
    | [] -> 反向 结果
    | '"' :: 剩余 -> 转义字符 剩余 ('"' :: '\\' :: 结果)
  ...
  在
  字符串_从_字符列表 (转义字符 (字符串_到_字符列表 字符串) []) 是谓

(* 生成二元运算代码 *)
夫 生成二元运算 者 受 上下文 运算符 左表达式 右表达式 焉 算法 乃
  设 左代码 为 生成表达式 上下文 左表达式 在
  设 右代码 为 生成表达式 上下文 右表达式 在
  观 运算符 之 性
  ...
  | 字符串连接 -> "luoyan_string_concat(" ^ 左代码 ^ ", " ^ 右代码 ^ ")" 观毕 是谓

(* 生成一元运算代码 *)
夫 生成一元运算 者 受 上下文 运算符 表达式 焉 算法 乃
  设 操作数代码 为 生成表达式 上下文 表达式 在
  观 运算符 之 性
  | 逻辑非 -> "luoyan_logical_not(" ^ 操作数代码 ^ ")"
  | 数值取反 -> "luoyan_subtract(luoyan_int(0), " ^ 操作数代码 ^ ")" 观毕 是谓

(* 生成条件表达式代码 *)
夫 生成条件表达式 者 受 上下文 条件 那么分支 否则分支 焉 算法 乃
  设 条件变量 为 生成变量名 上下文 "cond" 在
  设 条件代码 为 生成表达式 上下文 条件 在
  设 那么代码 为 生成表达式 上下文 那么分支 在
  设 否则代码 为 生成表达式 上下文 否则分支 在
  "({ luoyan_value_t* " ^ 条件变量 ^ " = " ^ 条件代码 ^ "; " ^
  "((" ^ 条件变量 ^ "->type == LUOYAN_BOOL && " ^ 条件变量 ^ "->data.bool_val)) ? (" ^ 
  那么代码 ^ ") : (" ^ 否则代码 ^ "); })" 是谓

(* 生成让绑定表达式代码 *)
夫 生成让绑定表达式 者 受 上下文 变量 值表达式 主体表达式 焉 算法 乃
  设 值代码 为 生成表达式 上下文 值表达式 在
  设 转义变量 为 转义标识符 变量 在
  设 主体代码 为 生成表达式 上下文 主体表达式 在
  "({ luoyan_env_bind(env, \"" ^ 转义变量 ^ "\", " ^ 值代码 ^ "); " ^ 主体代码 ^ "; })" 是谓

(* 生成函数表达式代码 *)
夫 生成函数表达式 者 受 上下文 参数列表 主体 焉 算法 乃
  设 函数名 为 生成变量名 上下文 "func" 在
  设 主体代码 为 生成表达式 上下文 主体 在
  
  (* 为多参数函数创建curry化的函数 *)
  设 递归 创建curry化函数 为 (函数 剩余参数 主体代码 ->
    匹配 剩余参数 与
    | [] -> 主体代码
    | 参数 :: 其他参数 ->
      设 转义参数 为 转义标识符 参数 在
      设 内部主体 为 创建curry化函数 其他参数 主体代码 在
      如果 其他参数 = []
      那么
        (* 最后一个参数，直接返回主体 *)
        设 函数实现 为
  ...
  设 _ 为 创建curry化函数 参数列表 主体代码 在
  
  匹配 参数列表 与
  | [] -> "luoyan_unit()"
  | 第一个参数 :: _ ->
    "luoyan_function_create(" ^ 函数名 ^ "_impl_" ^ 第一个参数 ^ ", env, \"" ^ 函数名 ^ "\")" 观毕 是谓

(* 生成函数调用代码 *)
夫 生成调用表达式 者 受 上下文 函数表达式 参数表达式列表 焉 算法 乃
  设 函数代码 为 生成表达式 上下文 函数表达式 在
  匹配 参数表达式列表 与
  | [] -> 函数代码  (* 无参数调用 *)
  | [参数表达式] ->
    设 参数代码 为 生成表达式 上下文 参数表达式 在
...
(* 生成模式匹配表达式代码 *)
夫 生成匹配表达式 者 受 上下文 表达式 模式列表 焉 算法 乃
  设 表达式变量 为 生成变量名 上下文 "match_expr" 在
  设 表达式代码 为 生成表达式 上下文 表达式 在
  
  设 递归 生成模式列表 为 (函数 模式列表 ->
    匹配 模式列表 与
    | [] -> "luoyan_unit()" (* 应该不会到达这里 *)
    | (模式, 表达式) :: 剩余 ->
      设 模式检查 为 生成模式检查 上下文 表达式变量 模式 在
      设 表达式代码 为 生成表达式 上下文 表达式 在
      如果 剩余 = []
...
(* 生成模式检查代码 *)
夫 生成模式检查 者 受 上下文 表达式变量 模式 焉 算法 乃
  匹配 模式 与
  | 字面量模式 (整数字面量 i, _) -> 
  ...
  | 列表头尾模式 (头模式, 尾模式, _) -> 
    设 头检查 为 生成模式检查 上下文 ("luoyan_list_head(" ^ 表达式变量 ^ ")") 头模式 在
    设 尾检查 为 生成模式检查 上下文 ("luoyan_list_tail(" ^ 表达式变量 ^ ")") 尾模式 在
    "(!luoyan_list_is_empty(" ^ 表达式变量 ^ ")->data.bool_val && " ^ 头检查 ^ " && " ^ 尾检查 ^ ")"
  | 通配符模式 _ -> "true"
  | _ -> 错误 "不支持的模式类型" 观毕 是谓

(* 生成列表表达式代码 *)
夫 生成列表表达式 者 受 上下文 表达式列表 焉 算法 乃
  设 递归 构建列表 为 (函数 表达式列表 ->
    观 表达式列表 之 性
    | [] -> "luoyan_list_empty()"
    | 表达式 :: 剩余 -> 
      设 元素代码 为 生成表达式 上下文 表达式 在
      设 剩余代码 为 构建列表 剩余 在
      "luoyan_list_cons(" ^ 元素代码 ^ ", " ^ 剩余代码 ^ ")" 观毕) 在
  构建列表 表达式列表 是谓

(* 生成元组表达式代码 *)
夫 生成元组表达式 者 受 上下文 表达式列表 焉 算法 乃
  观 表达式列表 之 性
  | [] -> "luoyan_unit()"
  | [单个] -> 生成表达式 上下文 单个
  | _ ->
    (* 元组存储为记录，字段名为_0, _1, _2等 *)
    设 元组变量 为 生成变量名 上下文 "tuple" 在
    设 初始化代码 为 "luoyan_record_create()" 在
    设 字段赋值 为 列表_映射_索引 (函数 i 表达式 ->
      设 字段名 为 "_" ^ (字符串_从_整数 i) 在
      设 字段代码 为 生成表达式 上下文 表达式 在
      "luoyan_record_set_field(" ^ 元组变量 ^ ", \"" ^ 字段名 ^ "\", " ^ 字段代码 ^ ")"
    ) 表达式列表 在
    设 赋值代码 为 字符串_连接 "; " 字段赋值 在
    "({ luoyan_value_t* " ^ 元组变量 ^ " = " ^ 初始化代码 ^ "; " ^ 赋值代码 ^ "; " ^ 元组变量 ^ "; })" 观毕 是谓

(* 生成记录表达式代码 *)
夫 生成记录表达式 者 受 上下文 字段列表 焉 算法 乃
  设 记录变量 为 生成变量名 上下文 "record" 在
  设 初始化代码 为 "luoyan_record_create()" 在
  匹配 字段列表 与
  | [] -> 初始化代码
  | _ -> 
    设 字段赋值 为 列表_映射 (函数 (字段名, 字段表达式) ->
      设 字段代码 为 生成表达式 上下文 字段表达式 在
      设 转义字段 为 转义标识符 字段名 在
      "luoyan_record_set_field(" ^ 记录变量 ^ ", \"" ^ 转义字段 ^ "\", " ^ 字段代码 ^ ")"
    ) 字段列表 在
    设 赋值代码 为 字符串_连接 "; " 字段赋值 在
    "({ luoyan_value_t* " ^ 记录变量 ^ " = " ^ 初始化代码 ^ "; " ^ 赋值代码 ^ "; " ^ 记录变量 ^ "; })" 观毕 是谓

(* 生成字段访问表达式代码 *)
夫 生成字段访问表达式 者 受 上下文 记录表达式 字段名 焉 算法 乃
  设 记录代码 为 生成表达式 上下文 记录表达式 在
  设 转义字段 为 转义标识符 字段名 在
  "luoyan_record_get_field(" ^ 记录代码 ^ ", \"" ^ 转义字段 ^ "\")" 是谓

(* 生成记录更新表达式代码 *)
夫 生成记录更新表达式 者 受 上下文 记录表达式 更新列表 焉 算法 乃
  设 记录代码 为 生成表达式 上下文 记录表达式 在
  匹配 更新列表 与
  | [] -> 记录代码
  | (字段名, 字段表达式) :: 剩余 ->
    ...
    列表_折叠_左 (函数 累积代码 (字段名, 字段表达式) ->
      设 字段代码 为 生成表达式 上下文 字段表达式 在
      设 转义字段名 为 转义标识符 字段名 在
      "luoyan_record_update(" ^ 累积代码 ^ ", \"" ^ 转义字段名 ^ "\", " ^ 字段代码 ^ ")"
    ) 第一次更新 剩余 观毕 是谓

(* 生成数组表达式代码 *)
夫 生成数组表达式 者 受 上下文 表达式列表 焉 算法 乃
  匹配 表达式列表 与
  | [] -> "luoyan_array_empty()"
  | _ ->
    设 数组变量 为 生成变量名 上下文 "array" 在
    设 初始化代码 为 "luoyan_array_create(" ^ (字符串_从_整数 (列表_长度 表达式列表)) ^ ")" 在
    设 元素赋值 为 列表_映射_索引 (函数 i 表达式 ->
      设 元素代码 为 生成表达式 上下文 表达式 在
      "luoyan_array_set(" ^ 数组变量 ^ ", " ^ (字符串_从_整数 i) ^ ", " ^ 元素代码 ^ ")"
    ) 表达式列表 在
    设 赋值代码 为 字符串_连接 "; " 元素赋值 在
    "({ luoyan_value_t* " ^ 数组变量 ^ " = " ^ 初始化代码 ^ "; " ^ 赋值代码 ^ "; " ^ 数组变量 ^ "; })" 观毕 是谓

(* 生成数组访问表达式代码 *)
夫 生成数组访问表达式 者 受 上下文 数组表达式 索引表达式 焉 算法 乃
  设 数组代码 为 生成表达式 上下文 数组表达式 在
  设 索引代码 为 生成表达式 上下文 索引表达式 在
  "luoyan_array_get(" ^ 数组代码 ^ ", " ^ 索引代码 ^ ")" 是谓

(* 生成数组更新表达式代码 *)
夫 生成数组更新表达式 者 受 上下文 数组表达式 索引表达式 值表达式 焉 算法 乃
  设 数组代码 为 生成表达式 上下文 数组表达式 在
  设 索引代码 为 生成表达式 上下文 索引表达式 在
  设 值代码 为 生成表达式 上下文 值表达式 在
...
(* 生成引用表达式代码 *)
夫 生成引用表达式 者 受 上下文 表达式 焉 算法 乃
  设 值代码 为 生成表达式 上下文 表达式 在
  "luoyan_ref_create(" ^ 值代码 ^ ")" 是谓

(* 生成解引用表达式代码 *)
夫 生成解引用表达式 者 受 上下文 表达式 焉 算法 乃
  设 引用代码 为 生成表达式 上下文 表达式 在
  "luoyan_ref_get(" ^ 引用代码 ^ ")" 是谓

(* 生成赋值表达式代码 *)
夫 生成赋值表达式 者 受 上下文 引用表达式 值表达式 焉 算法 乃
  设 引用代码 为 生成表达式 上下文 引用表达式 在
  设 值代码 为 生成表达式 上下文 值表达式 在
  "luoyan_ref_set(" ^ 引用代码 ^ ", " ^ 值代码 ^ ")" 是谓

(* 生成构造器表达式代码 *)
夫 生成构造器表达式 者 受 上下文 构造器名 参数列表 焉 算法 乃
  设 构造器名称 为 转义标识符 构造器名 在
  匹配 参数列表 与
  | [] -> 
    (* 无参数构造器，创建一个带有构造器名的记录 *)
    "luoyan_constructor_create(\"" ^ 构造器名称 ^ "\", NULL)"
  | _ ->
    设 参数代码 为 列表_映射 (生成表达式 上下文) 参数列表 在
    设 参数数组 为 字符串_连接 ", " 参数代码 在
    "luoyan_constructor_create(\"" ^ 构造器名称 ^ "\", luoyan_array_from_values(" ^ 参数数组 ^ "))" 观毕 是谓

(* 生成序列表达式代码 *)
夫 生成序列表达式 者 受 上下文 表达式列表 焉 算法 乃
  匹配 表达式列表 与
  | [] -> "luoyan_unit()"
  | [表达式] -> 生成表达式 上下文 表达式
  | _ ->
    设 表达式代码列表 为 列表_映射 (生成表达式 上下文) 表达式列表 在
    设 最后一个 为 列表_最后一个 表达式代码列表 在
    设 前面的 为 列表_去掉最后一个 表达式代码列表 在
    "({ " ^ (字符串_连接 "; " 前面的) ^ "; " ^ 最后一个 ^ "; })" 观毕 是谓

(* 生成语句代码 *)
夫 生成语句 者 受 上下文 语句 焉 算法 乃
  观 语句 之 性
  | 表达式语句 表达式 -> 
    设 表达式代码 为 生成表达式 上下文 表达式 在
    表达式代码 ^ ";"
  ...
  | 异常定义语句 (_, _, _) -> "/* 异常定义在C生成中被忽略 */" 观毕 是谓

(* 生成程序代码 *)
夫 生成程序 者 受 上下文 程序 焉 算法 乃
  设 递归 生成语句列表 为 (函数 语句列表 ->
    观 语句列表 之 性
    | [] -> ""
    | 语句 :: 剩余 ->
      设 语句代码 为 生成语句 上下文 语句 在
      语句代码 ^ "\n" ^ (生成语句列表 剩余) 观毕) 在
  生成语句列表 程序 是谓

(* 生成完整的C代码 *)
夫 生成C代码 者 受 配置 程序 焉 算法 乃
  设 上下文 为 创建上下文 配置 在
  
  (* 生成主要代码 *)
  设 主代码 为 生成程序 上下文 程序 在
  ...
  设 转义打印 为 转义标识符 "打印" 在
  设 转义读取 为 转义标识符 "读取" 在
  设 转义读取文件 为 转义标识符 "内置_读取文件" 在
  设 转义写入文件 为 转义标识符 "内置_写入文件" 在
  设 转义文件存在 为 转义标识符 "内置_文件存在" 在
  设 转义系统参数 为 转义标识符 "系统_参数列表" 在
  设 转义系统退出 为 转义标识符 "系统_退出" 在
  设 转义字符串长度 为 转义标识符 "内置_字符串长度" 在
  设 转义字符串连接 为 转义标识符 "内置_字符串连接" 在
  设 转义整数转字符串 为 转义标识符 "字符串_从_整数" 在
  设 转义字符串转整数 为 转义标识符 "整数_从_字符串" 在
  包含文件 ^ "\n\n" ^
  函数定义 ^ "\n\n" ^
...
(* 主要编译函数 *)
夫 编译到C 者 受 配置 程序 焉 算法 乃
  设 C代码 为 生成C代码 配置 程序 在
  
  (* 写入C文件 *)
  设 输出通道 为 打开_输出文件 配置.输出文件 在
  输出_字符串 输出通道 C代码;
  关闭_输出通道 输出通道;
