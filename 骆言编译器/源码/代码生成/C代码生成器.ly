...
夫「生成变量名」者受 上下文 前缀 焉算法乃
  设「标识」为上下文点下一个变量标识 在
  上下文点下一个变量标识 ：等于标识 加 1；
  『骆言变量下划线』 乘方 前缀 乘方 『下划线』 乘方 （字符串下划线从整数 标识） 也

「：生成唯一标签名：」
夫「生成标签名」者受 上下文 前缀 焉算法乃
  设「标识」为上下文点下一个标签标识 在
  上下文点下一个标签标识 ：等于标识 加 1；
  『骆言label下划线』 乘方 前缀 乘方 『下划线』 乘方 （字符串下划线从整数 标识） 也
  ...
  设 递归 转义字符为函数 字符列表 结果 减大于
    ...
    或 字符 ：： 剩余 减大于
      若 （字符 大于等于『0『 和和 字符 小于等于『9『） 或或
         ...
         （字符 大于等于『大写字母甲『 和和 字符 小于等于『大写字母终『） 或或
         字符等于『下划线『
      则 答 转义字符 剩余 （字符 ：： 结果）
      余者答
        设「代码」为字符下划线到整数 字符 在
...
夫「生成二元运算」者受 上下文 运算符 左表达式 右表达式 焉算法乃
  ...
  或 除法 减大于 『骆言div标识e（』 乘方 左代码 乘方 『， 』 乘方 右代码 乘方 『）』
  或 模运算 减大于 『骆言modulo（』 乘方 左代码 乘方 『， 』 乘方 右代码 乘方 『）』
  或 等于 减大于 『骆言equal（』 乘方 左代码 乘方 『， 』 乘方 右代码 乘方 『）』
  或 不等于 减大于 『骆言非equal（』 乘方 左代码 乘方 『， 』 乘方 右代码 乘方 『）』
...
夫「生成一元运算」者受 上下文 运算符 表达式 焉算法乃
  ...
  观「运算符」之性
  或 逻辑非 减大于 『骆言logical非（』 乘方 操作数代码 乘方 『）』
...
夫「生成条件表达式」者受 上下文 条件 那么分支 否则分支 焉算法乃
  ...
  『（（』 乘方 条件变量 乘方 『减大于type等于等于LUOY大写字母ANBOOL 和和 』 乘方 条件变量 乘方 『减大于data点布尔val）） 问号 （』 乘方
...
夫「生成函数表达式」者受 上下文 参数列表 主体 焉算法乃
  ...
  「：为多参数函数创建柯里化的函数：」
  设 递归 创建柯里化函数为（函数 剩余参数 主体代码 减大于
    匹配 剩余参数 与
    或 空空如也 减大于 主体代码
    或 参数 ：： 其他参数 减大于
      设「转义参数」为转义标识符 参数 在
      设「内部主体」为创建柯里化函数 其他参数 主体代码 在
      如果 其他参数等于空空如也
      那么
        「：最后一个参数，直接返回主体：」
...
夫「生成模式检查」者受 上下文 表达式变量 模式 焉算法乃
  ...
  或 字面量模式 （字符串字面量 串， 下划线） 减大于
    『骆言equals（』 乘方 表达式变量 乘方 『， 骆言字符串（\』『 乘方 （为丙转义字符串 串） 乘方 』\『））』
...
夫「生成构造器表达式」者受 上下文 构造器名 参数列表 焉算法乃
  ...
  或 空空如也 减大于
    ...
    『骆言常量ructorcreate（\』『 乘方 构造器名称 乘方 』\『， 空）』
  或 下划线 减大于
...
夫「生成语句」者受 上下文 语句 焉算法乃
  ...
  或 递归设绑定语句 （变量， 表达式， 下划线） 减大于
    ...
    『骆言envbind（env， \』『 乘方 转义变量 乘方 』\『， 』 乘方 表达式代码 乘方 『）；』
  或 类型定义语句 下划线 减大于 『除乘 类型定义在丙生成中被忽略 乘除』
  或 异常定义语句 （下划线， 下划线， 下划线） 减大于 『除乘 异常定义在丙生成中被忽略 乘除』 观毕也
...
夫「生成丙代码」者受 配置 程序 焉算法乃
  ...
  「：生成完整的丙文件：」
  设「包含文件」为字符串连接 『\数』 （列表映射 （函数 文件 减大于 『号包含 \』『 异或 文件 异或 』\『』） 上下文。包含文件） 在
  ...
  包含文件 乘方 『\数\数』 乘方
  函数定义 乘方 『\数\数』 乘方
  『整数 主（整数 argc， 字符乘乘 argv） 「\数』 乘方
  『  骆言运行时init（）；\数』 乘方
  『  骆言下划线set下划线systemargs（argc， argv）；\数』 乘方
  『  骆言envt乘 env等于骆言envcreate（空）；\数』 乘方
  『  \数』 乘方
  『  ／／ 添加内置函数\数』 异或
  『  骆言envbind（env， \』『 乘方 转义打印 乘方 』\『， 骆言函数create（骆言下划线builtin打印整数， env， \』打印\『））；\数』 乘方
...
「：主要编译函数：」
夫「编译到丙」者受 配置 程序 焉算法乃
  设「丙代码」为生成丙代码 配置 程序 在
