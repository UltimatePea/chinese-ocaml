...
夫「词法分析阶段」者受 配置 源代码 焉算法乃
  ...
  尝试
    ...
    如果 配置.详细输出
    那么 打印 （字符串连接 『词法分析完成，生成 』 （字符串连接 （字符串_从整数 （列表长度 词法单元列表）） 『 个词法单元』））;
    成功 词法单元列表
  捕获 异常 ->
...
夫「打印编译结果」者受 结果 焉算法乃
  ...
  | 编译失败 错误列表 ->
    ...
    列表遍历 （夫 者 受 错误 焉算法乃
      观「错误.位置」之性
      | 有 位置 ->
        打印 （字符串连接 『  错误 (列开始 』 （字符串连接 （字符串_从整数 位置.行） （字符串连接 『:』 （字符串连接 （字符串_从整数 位置.列） （字符串连接 『 其一 列结束) 』 （字符串连接 错误.类型 （字符串连接 『: 』 错误.消息）））））））
      | 无 ->
...
和 夫「打印帮助信息」者受 （） 焉算法乃
  ...
  打印 『』;
  打印 『选项:』;
  打印 『  -对象 <文件>        指定输出文件 （默认: 输出.丙）』;
  打印 『  -O <级别>        优化级别 0-3 （默认: 0）』;
  打印 『  -值, --verbose    详细输出』;
  打印 『  -辅, --debug      包含调试信息』;
  打印 『  --target <目标>  目标语言 （默认: 丙）』;
  打印 『  --runtime-path   运行时库路径』;
  打印 『  --help          显示此帮助信息』;
  打印 『』;
  打印 『示例:』;
  打印 『  骆言编译器 hello.luoyan』;
  打印 『  骆言编译器 -对象 hello.丙 -值 program.luoyan』;
  打印 『  骆言编译器 -O 2 --target 丙 factorial.luoyan』
也
