...
「：获取当前词法单元：」
夫「当前词法单元」者受 状态 焉算法乃
  if 状态.当前索引 >=List.length 状态.词法单元列表 则
    无
  余者
...
夫「预览词法单元」者受 状态 偏移量 焉算法乃
  设「索引」为状态.当前索引 加 偏移量 在
  if 索引 >=List.length 状态.词法单元列表 则
    无
  余者
...
「：前进到下一个词法单元：」
夫「前进词法单元」者受 状态 焉算法乃
  if 状态.当前索引 小于 List.length 状态.词法单元列表 则
    状态.当前索引 :=状态.当前索引 加 1
也
  ...
  | 无 ->
      ...
      | 无 ->
          ...
          if 检查词法单元类型 状态 左括号 则 （
            ...
            | 无 -> 无
            观毕
          ） 余者 if 检查词法单元类型 状态 （关键字词法单元 『如果』） 则 （
            「：解析条件表达式：」
          ...
          ） 余者 if 检查词法单元类型 状态 （关键字词法单元 『让』） 则 （
            ...
            | 无 -> 无
            观毕
          ） 余者 if 检查词法单元类型 状态 （关键字词法单元 『函数』） 则 （
            「：解析函数表达式：」
