...
「：检查是否是关键字：」
夫 是关键字 者 受 标识符 焉 算法 乃
  List.exists (函数 (关键字, _) -> 关键字 = 标识符) 中文关键字表
是谓

「：创建词法分析器状态：」
夫 创建词法分析器 者 受 输入 文件名 焉 算法 乃
  设 错误收集器 为 创建错误收集器 () 在 {
    输入 = 输入;
    当前位置 = 创建初始位置 文件名;
    字符索引 = 0;
...
「：读取数字字面量：」
夫 读取数字 者 受 状态 焉 算法 乃
  设 开始位置 为 状态.当前位置 在
  设 缓冲区 为 ref "" 在
  设 是浮点数 为 ref false 在
...
「：读取双字符运算符：」
夫 尝试读取双字符运算符 者 受 状态 第一字符 焉 算法 乃
  若 状态.字符索引 + 1 >= 字符串长度 状态.输入 则 答
    无
  余者 答
    设 第二字符 为 状态.输入.[状态.字符索引 + 1] 在
    观 (第一字符, 第二字符) 之 性
    | ('=', '=') -> 有 等号
    | ('!', '=') -> 有 不等号
    | ('<', '=') -> 有 小于等于号
...
「：词法分析入口函数：」
夫 词法分析 者 受 输入 文件名 焉 算法 乃
  设 状态 为 创建词法分析器 输入 文件名 在
  设 词法单元列表 为 ref [] 在
  
  设 递归 收集词法单元 为 函数 () ->
    设 词法单元 为 获取下一个词法单元 状态 在
    词法单元列表 := 词法单元 :: !词法单元列表;
    观 词法单元.类型 之 性
...
夫 是中文 者 受 字符 焉 算法 乃
  ...
  设 代码 为 Char.code 字符 在
  代码 >= 0x4e00 && 代码 <= 0x9fff
是谓

夫 是字母或中文 者 受 字符 焉 算法 乃
  (是字母 字符) || (是中文 字符)
是谓

夫 是字母或数字或下划线 者 受 字符 焉 算法 乃
