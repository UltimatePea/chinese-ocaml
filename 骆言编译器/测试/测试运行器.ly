...
夫「更新测试统计」者受 统计 通过 焉算法乃
  统计.总测试数 :=统计.总测试数 加 1;
  if 通过 则
    统计.通过测试数 :=统计.通过测试数 加 1
  余者
    统计.失败测试数 :=统计.失败测试数 加 1
也
...
夫「打印测试统计」者受 统计 焉算法乃
  ...
  设「通过率」为    if 统计.总测试数 大于 0 则
      （浮点数转字符串 （（float_of整数 统计.通过测试数） /. （float_of整数 统计.总测试数） *. 100.0）） ^ 『%』
    余者
      『N/A』
  在
  打印调试信息 （『通过率: 』 ^ 通过率）;
...
夫「运行测试模块」者受 模块名 测试函数 统计 焉算法乃
  ...
  打印调试信息 （『-』 乘 30）;
  if 结果 则
    打印调试信息 （模块名 ^ 『 测试通过 ✓ （耗时: 』 ^ （浮点数转字符串 耗时） ^ 『串）』）
  余者
    打印调试信息 （模块名 ^ 『 测试失败 ✗ （耗时: 』 ^ （浮点数转字符串 耗时） ^ 『串）』）
也
...
夫「运行集成测试」者受 （） 焉算法乃
  ...
  设「测试程序」为『夫「阶乘」者受 数 -> 若 数 <=1 则 答 1 余者答 数 乘 阶乘 （数 减 1） 在 阶乘 5』 在
...
夫「运行性能测试」者受 （） 焉算法乃
  ...
  设「大型程序」为String.concat 『\数』 (列开始 『夫 x1为1 在』 其一 『夫 x2为2 在』 其二 『夫 x3为3 在』 其三 『夫 x4为4 在』 其一 『夫 x5为5 在』 其二 『x1 加 x2 加 x3 加 x4 加 x5』 其三 列结束) 在
  ...
  | 成功 （词法单元列表, _） 焉算法乃
      ...
      | 成功 （程序, _） ->
          ...
          打印调试信息 （『性能测试通过：编译耗时 』 ^ （浮点数转字符串 总耗时） ^ 『串』）;
...
夫「运行错误处理测试」者受 （） 焉算法乃
  ...
  设「错误程序列表」为(列开始 （『语法错误1』 其一 『夫 甲=』） 其二 「：缺少值表达式：」
