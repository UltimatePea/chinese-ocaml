...
「：运行测试模块：」
夫 运行测试模块 者 受 模块名 测试函数 统计 焉 算法 乃
  打印调试信息 （『』）;
  打印调试信息 （『运行 』 ^ 模块名 ^ 『 测试...』）;
  打印调试信息 （『-』 * 30）;
  
  设 开始时间 为 Unix.gettimeofday （） 在
  设 结果 为 测试函数 （） 在
  设 结束时间 为 Unix.gettimeofday （） 在
  设 耗时 为 结束时间 -. 开始时间 在
...
夫 运行集成测试 者 受 （） 焉 算法 乃
  ...
  设 测试程序 为 『夫 阶乘 者 受 n -> 若 n <= 1 则 答 1 余者 答 n * 阶乘 （n - 1） 在 阶乘 5』 在
...
夫 运行性能测试 者 受 （） 焉 算法 乃
  ...
  设 词法分析结果 为 词法分析.词法分析 大型程序 『性能测试』 在
  ...
  | 失败 _ -> 
      ...
      false
  | 成功 （词法单元列表, _） 焉 算法 乃
      设 语法分析结果 为 语法分析.语法分析 词法单元列表 『性能测试』 在
...
「：错误处理测试：」
夫 运行错误处理测试 者 受 （） 焉 算法 乃
  打印调试信息 『运行错误处理测试...』;
  
  设 错误程序列表 为 [
    （『语法错误1』, 『夫 x =』）;  「：缺少值表达式：」
    （『语法错误2』, 『若 真 则 答 1』）;  「：缺少否则分支：」
    （『语法错误3』, 『（1 + 2』）;  「：不匹配的括号：」
    （『词法错误』, 『\』未终止字符串"）;  「：未终止的字符串：」
  ] 在
  
  设 通过数量 为 ref 0 在
  设 总数量 为 List.length 错误程序列表 在
  
  List.iter （夫 参数函数 者 受 （错误名, 错误程序） 焉 算法 乃
