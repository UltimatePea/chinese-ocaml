...
夫「打印测试统计」者受 统计 焉算法乃
  ...
  设「通过率」为    如果 统计点总测试数 大于 0 则
      （浮点数转字符串 （（浮点下划线of整数 统计点通过测试数） 除点 （浮点下划线of整数 统计点总测试数） 乘点 一百点零）） 乘方 『百分』
    余者
      『N除A』
  在
  打印调试信息 （『通过率： 』 乘方 通过率）；
...
夫「运行性能测试」者受 （） 焉算法乃
  ...
  设「大型程序」为字符串点连接 『\数』 （列开始 『夫 x1为1 在』 其一 『夫 x2为2 在』 其二 『夫 x3为3 在』 其三 『夫 x4为4 在』 其一 『夫 x5为5 在』 其二 『x1 加 x2 加 x3 加 x4 加 x5』 其三 列结束） 在
...
夫「运行错误处理测试」者受 （） 焉算法乃
  ...
  设「通过数量」为引用 0 在
  设「总数量」为列表点长度 错误程序列表 在

  列表点迭代 （夫「参数函数」者受 （错误名， 错误程序） 焉算法乃
...
夫「主函数」者受 （） 焉算法乃
  ...
  打印调试信息 『骆言编译器测试套件』；
  打印调试信息 （『开始时间： 』 乘方 （系统点获取时间 （） 或大于 浮点数转字符串））；
