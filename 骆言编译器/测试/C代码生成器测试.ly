...
设「测试配置」为「
  ...
  包含调试信息为真；
  优化级别为0；
  运行时路径为『点点除点点除C后端除运行时』；
」

「：测试字面量生成：」
夫「测试字面量生成」者受 （） 焉算法乃
  设「测试用例」为（列开始 （整数字面量 42 其一 『骆言整数（42L）』） 其二 （浮点字面量 3点14 其三 『骆言浮点（3点14）』） 其一 （字符串字面量 『你好』 其二 『骆言字符串（\』你好\『）』） 其三 （布尔字面量 真 其一 『骆言布尔（真）』） 其二 （布尔字面量 假 其三 『骆言布尔（假）』） 其一 （单元字面量 其二 『骆言unit（）』） 其三 列结束） 在
...
夫「运行测试」者受 （） 焉算法乃
  ...
  打印 （『C代码生成器测试完成： 』 乘方 （字符串下划线从整数 通过） 乘方 『除』 乘方 （字符串下划线从整数 总数） 乘方 『 通过』）；
