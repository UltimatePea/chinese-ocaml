...
设「测试配置」为{
  ...
  包含调试信息为真;
  优化级别为0;
  运行时路径为『../../cbackend/runtime』;
}

「：测试字面量生成：」
夫「测试字面量生成」者受 （） 焉算法乃
  设「测试用例」为(列开始 （整数字面量 42 其一 『luoyan整数（42L）』） 其二 （浮点字面量 3.14 其三 『luoyanfloat（3.14）』） 其一 （字符串字面量 『你好』 其二 『luoyan字符串（\』你好\『）』） 其三 （布尔字面量 真 其一 『luoyan布尔（真）』） 其二 （布尔字面量 假 其三 『luoyan布尔（假）』） 其一 （单元字面量 其二 『luoyanunit（）』） 其三 列结束) 在
...
「：测试标识符转义：」
夫「测试标识符转义」者受 （） 焉算法乃
  设「测试用例」为(列开始 （『hello』 其一 『hello』） 其二 （『变量』 其三 『_53d8_91cf_』） 其一 （『test123』 其二 『test123』） 其三 （『函数名』 其一 『_51fd__6570_540d_』） 其二 列结束) 在
...
夫「测试简单表达式生成」者受 （） 焉算法乃
  ...
  设「期望变量」为『luoyanenvlookup（env, \』甲\『）』 在
  ...
  设「期望加法」为『luoyanadd（luoyan整数（1L）, luoyan整数（2L））』 在
...
夫「测试记录表达式生成」者受 （） 焉算法乃
  ...
  （字符串包含 结果 『luoyanrecordcreate』） &&
  （字符串包含 结果 『luoyan_record_setfield』）
也
...
夫「测试完整程序生成」者受 （） 焉算法乃
  ...
  尝试
    ...
    （字符串包含 结果 『整数 main（）』） &&
    （字符串包含 结果 『luoyanruntimeinit』） &&
    （字符串包含 结果 『luoyanenvbind』）
  捕获 _ -> 假
也
...
夫「测试数组表达式生成」者受 （） 焉算法乃
  ...
  （字符串包含 结果 『luoyanarraycreate』） &&
  （字符串包含 结果 『luoyanarrayset』）
也
...
夫「测试引用操作生成」者受 （） 焉算法乃
  ...
  （字符串包含 引用结果 『luoyanrefcreate』） &&
  （字符串包含 解引用结果 『luoyanrefget』）
也
...
夫「运行测试」者受 （） 焉算法乃
  ...
  设 （通过, 总数）为列表_折叠左 （夫「参数函数」者受 （通过, 总数） （名称, 测试函数） 焉算法乃
    打印 （『  运行测试: 』 ^ 名称）;
    若 测试函数 （）
  ...
  打印 （『C代码生成器测试完成: 』 ^ （字符串_从整数 通过） ^ 『/』 ^ （字符串_从整数 总数） ^ 『 通过』）;
