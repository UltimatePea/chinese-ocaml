...
夫「测试基础类型检查」者受 （） 焉算法乃
  ...
  设「测试用例」为(列开始 「：测试基础字面量：」
    ...
    （『夫 加法为1 加 2』 其三 『整数加法应该通过类型检查』） 其一 （『夫 乘法为3.0 乘 4.0』 其二 『浮点数乘法应该通过类型检查』） 其三 「：测试逻辑运算：」
    （『夫 逻辑为真 && 假』 其一 『布尔逻辑运算应该通过类型检查』） 其二 （『夫 比较为5 大于 3』 其三 『数值比较应该通过类型检查』） 其一 列结束) 在

  List.iteri （夫「参数函数」者受 索引 （代码, 描述） 焉算法乃
    打印字符串 （『  测试 』 ^ （字符串化整数 （索引 加 1）） ^ 『: 』 ^ 描述）;
    打印换行 （）;
    观 解析并分析 代码 之 性
    | 编译成功 （） ->
        打印字符串 『    ✓ 通过』;
...
夫「测试类型错误检测」者受 （） 焉算法乃
  ...
  设「错误测试用例」为(列开始 「：类型不匹配错误：」
    （『夫 错误1为1 加 \』字符串\『』 其一 『整数和字符串相加应该报错』） 其二 （『夫 错误2为真 乘 假』 其三 『布尔值相乘应该报错』） 其一 （『夫 错误3为\』文本\『 && 真』 其二 『字符串和布尔值逻辑运算应该报错』） 其三 「：条件表达式类型错误：」
    （『夫 错误4为如果 5 那么 1 否则 2』 其一 『非布尔条件应该报错』） 其二 （『夫 错误5为如果 真 那么 1 否则 \』不匹配\『』 其三 『分支类型不匹配应该报错』） 其一 「：函数调用类型错误：」
    （『夫「函」者受 甲 -> 甲 加 1 在 函 \』错误\『』 其二 『参数类型错误应该报错』） 其三 列结束) 在
...
夫「测试变量作用域」者受 （） 焉算法乃
  ...
  设「作用域测试用例」为(列开始 「：正确的作用域使用：」
    （『夫 x为1 在 夫 y为x 加 2 在 乙』 其一 『内层作用域访问外层变量应该成功』） 其二 （『夫「函」者受 甲 -> 夫 y为x 乘 2 在 乙 在 函 5』 其三 『函数参数和局部变量应该正确』） 其一 「：作用域遮蔽：」
    （『夫 x为1 在 夫 x为2 在 甲』 其二 『变量遮蔽应该使用内层定义』） 其三 「：递归函数：」
    （『夫 递归 阶乘 者 受 数 -> 如果 数 <=1 那么 1 否则 数 乘 阶乘 （数 减 1） 在 阶乘 5』 其一 『递归函数应该能够引用自身』） 其二 列结束) 在
...
夫「测试未定义变量检测」者受 （） 焉算法乃
  ...
  设「未定义变量测试」为(列开始 （『未定义变量』 其一 『直接使用未定义变量应该报错』） 其二 （『夫 x为y』 其三 『使用未定义变量y应该报错』） 其一 （『夫「函」者受 甲 -> 乙 在 函 1』 其二 『函数体中使用未定义变量应该报错』） 其三 列结束) 在
...
夫「测试函数类型推导」者受 （） 焉算法乃
  ...
  设「函数测试用例」为(列开始 「：简单函数：」
    （『夫「身份函数」者受 甲 -> 甲』 其一 『身份函数应该通过类型检查』） 其二 （『夫「常量函数」者受 甲 -> 42』 其三 『常量函数应该通过类型检查』） 其一 （『夫「加一」者受 甲 -> 甲 加 1』 其一 『整数函数应该通过类型检查』） 其一 「：高阶函数：」
    （『夫「应用」者受 函 -> 函数 甲 -> 函 甲』 其一 『高阶函数应该通过类型检查』） 其一 （『夫「组合」者受 函 -> 函数 辅 -> 函数 甲 -> 函 （辅 甲）』 其一 『函数组合应该通过类型检查』） 其一 「：递归函数：」
    （『夫 递归 长度 者 受 列表 -> 观「列表」之性 | 空空如也 -> 0 | _ :: 尾部 -> 1 加 长度 尾部』 其一 『递归列表函数应该通过类型检查』） 其一 列结束) 在
