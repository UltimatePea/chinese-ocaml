...
使用 」丙代码生成器下划线诗意版」 作为 CodeGen；
使用 」抽象语法树」 作为 AST；
使用 」测试框架」 作为 Test；

// === 第一章：基础表达式测试 - 如春花初绽 ===

测试用例 「整数字面量生成下划线如山石恒固「 『
  // 整数如山石，坚固不变，生成应当准确
  表达式 ＝ AST。整数字面量（42）；
...
// === 测试运行器：如诗词的总序 ===
函数 运行所有测试（） 『
  Test。开始测试套件（「丙代码生成器诗意测试「）；
  
  // 第一章：基础表达式测试
  Test。运行测试（「整数字面量生成下划线如山石恒固「）；
  Test。运行测试（「浮点字面量生成下划线如流水潺潺「）；
  Test。运行测试（「字符串字面量生成下划线如珠玉之言「）；
  Test。运行测试（「布尔字面量生成下划线如阴阳分明「）；
  
  // 第二章：二元运算测试
  Test。运行测试（「加法运算生成下划线如春暖花开「）；
  Test。运行测试（「减法运算生成下划线如秋叶凋零「）；
  Test。运行测试（「复合运算生成下划线如诗词层递「）；
  
  // 第三章：变量和函数测试
  Test。运行测试（「变量访问生成下划线如人之姓名「）；
  Test。运行测试（「函数调用生成下划线如雁字传书「）；
  
  // 第四章：语句生成测试
  Test。运行测试（「条件语句生成下划线如岔路择径「）；
  Test。运行测试（「循环语句生成下划线如四季轮回「）；
  
  // 第五章：完整程序测试
  Test。运行测试（「简单程序生成下划线如五言绝句「）；
  Test。运行测试（「斐波那契程序生成下划线如七言律诗「）；
  
  // 第六章：错误处理测试
  Test。运行测试（「变量映射失败处理下划线如名不正言不顺「）；
  
  // 第七章：性能和质量测试
  Test。运行测试（「缩进格式正确性下划线如诗词的平仄「）；
  Test。运行测试（「大型程序生成性能下划线如长篇巨制「）；
