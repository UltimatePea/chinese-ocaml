...
let test_other_tokens =
  [
    ...
    (HaveKeyword, "have");
    (* 文言文关键字 *)
    (AncientDefineKeyword, "ancient_define");
    (* 古雅体关键字 *)
  ]
...
let test_strategy_consistency () =
  ...
  List.iter
    (fun (token, name) ->
      try
        ...
        if readable_result = fast_result then Printf.printf "✅ %s: 两种策略结果一致\n" name
        else (
          Printf.printf "❌ %s: 策略结果不一致!\n" name;
          failures := name :: !failures)
      with Unknown_keyword_token msg ->
  ...
  if !failures = [] then (
    ...
    Printf.printf "\n💥 发现%d个失败案例: %s\n" (List.length !failures) (String.concat ", " !failures);
    false)
