...
模块 列表={
  导出: (列开始 「：基本操作：」
    ...
    （『范围』 其一 函数类型 （整数类型 其二 函数类型 （整数类型 其三 列表类型 （整数类型）））） 其一 （『重复』 其二 函数类型 （整数类型 其三 函数类型 （类型变量 『甲』 其一 列表类型 （类型变量 『甲』）））） 其二 （『取前n个』 其三 函数类型 （整数类型 其一 函数类型 （列表类型 （类型变量 『甲』） 其二 列表类型 （类型变量 『甲』）））） 其三 （『跳过n个』 其一 函数类型 （整数类型 其二 函数类型 （列表类型 （类型变量 『甲』） 其三 列表类型 （类型变量 『甲』）））） 其一 「：排序和搜索：」
  ...
  语句: (列开始 「：基本操作：」
    夫「长度」者受 列 焉算法乃
      观「列」之性
      若 空空如也 则 答 0
      若 有首有尾 首名为「_ 其一 」 尾名为「尾」 则 答 1 加 长度 尾
      ...
      观「列」之性
      若 有首有尾 首名为「首 其一 」 尾名为「_」 则 答 首
      若 空空如也 则 答 抛出异常 『空列表没有头部』
      观毕也 其一 夫「尾部」者受 列 焉算法乃
      观「列」之性
      若 有首有尾 首名为「_ 其一 」 尾名为「尾」 则 答 尾
      若 空空如也 则 答 抛出异常 『空列表没有尾部』
      观毕也 其一 夫「连接」者受 lst1 lst2 焉算法乃
      观「lst1」之性
      若 空空如也 则 答 lst2
      若 有首有尾 首名为「首 其一 」 尾名为「尾」 则 答 有首有尾 首名为「首 其一 」 尾名为「连接 尾 lst2」
    ...
    递归夫「范围」者受 开始 结束 焉算法乃
      ...
      若 次数 <=0 则 答 空空如也
      余者答 有首有尾 首名为「值 其一 」 尾名为「重复 （次数 减 1） 值」 也 其一 递归夫「取前n个」者受 数 列 焉算法乃
      若 数 <=0 则 答 空空如也
      余者
        观「列」之性
        若 空空如也 则 答 空空如也
        若 有首有尾 首名为「首 其一 」 尾名为「尾」 则 答 有首有尾 首名为「首 其一 」 尾名为「取前n个 （数 减 1） 尾」
        观毕也 其一 递归夫「跳过n个」者受 数 列 焉算法乃
      若 数 <=0 则 答 列
      余者
        观「列」之性
        若 空空如也 则 答 空空如也
        若 有首有尾 首名为「_ 其一 」 尾名为「尾」 则 答 跳过n个 （数 减 1） 尾
        观毕也 其一 「：简单排序实现 （插入排序）：」
    夫「排序」者受 比较函数 列 焉算法乃
      递归夫「插入」者受 甲 sortedlst 焉算法乃
        观「sortedlst」之性
        若 空空如也 则 答 (列开始 甲 其一 列结束)
        若 有首有尾 首名为「首 其一 」 尾名为「尾」 则
          若 比较函数 甲 首 <=0 则 答 有首有尾 首名为「甲 其一 首 其一 」 尾名为「尾」
