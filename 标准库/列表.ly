...
模块 列表等于「
  ...
  语句: (列开始 「:基本操作:」
    夫「长度」者受 列 焉算法乃
      递归夫「长度辅助」者受 累计 列 焉算法乃
        观「列」之性
        若 空空如也 则 答 累计
        若 有首有尾 首名为「下划线 其一 」 尾名为「尾」 则 答 长度辅助 (累计 加 1) 尾
        观毕
      在 长度辅助 0 列 也 其二 夫「是否为空」者受 列 焉算法乃
      观「列」之性
      若 空空如也 则 答 真
      余者则 答 假
      ...
      递归夫「连接辅助」者受 反转列表一 列表二 焉算法乃
        观「反转列表一」之性
        若 空空如也 则 答 列表二
        若 有首有尾 首名为「首 其一 」 尾名为「尾」 则 答 连接辅助 尾 有首有尾 首名为「首 其一 」 尾名为「列表二」
        观毕
      在 连接辅助 (反转 列表一) 列表二 也 其一 夫「反转」者受 列 焉算法乃
    ...
    夫「映射」者受 函 列 焉算法乃
      递归夫「映射辅助」者受 累计 函 列 焉算法乃
        观「列」之性
        若 空空如也 则 答 反转 累计
        若 有首有尾 首名为「首 其一 」 尾名为「尾」 则 答 映射辅助 有首有尾 首名为「函 首 其一 」 尾名为「累计」 函 尾
        观毕
      在 映射辅助 空空如也 函 列 也 其一 夫「过滤」者受 条件 列 焉算法乃
      递归夫「过滤辅助」者受 累计 条件 列 焉算法乃
        观「列」之性
        若 空空如也 则 答 反转 累计
        若 有首有尾 首名为「首 其一 」 尾名为「尾」 则
          若 条件 首 则
            答 过滤辅助 有首有尾 首名为「首 其一 」 尾名为「累计」 条件 尾
          余者
            答 过滤辅助 累计 条件 尾
        观毕
      在 过滤辅助 空空如也 条件 列 也 其一 递归夫「折叠左」者受 函 累 列 焉算法乃
      观「列」之性
      若 空空如也 则 答 累
      ...
      观毕也 其一 「:列表构造:」
    夫「范围」者受 开始 结束 焉算法乃
      递归夫「范围辅助」者受 累计 当前 结束 焉算法乃
        若 当前 大于 结束 则 答 反转 累计
        余者答 范围辅助 有首有尾 首名为「当前 其一 」 尾名为「累计」 (当前 加 1) 结束
      在 范围辅助 空空如也 开始 结束 也 其一 夫「重复」者受 次数 值 焉算法乃
      递归夫「重复辅助」者受 累计 次数 值 焉算法乃
        若 次数 小于等于0 则 答 累计
        余者答 重复辅助 有首有尾 首名为「值 其一 」 尾名为「累计」 (次数 减 1) 值
      在 重复辅助 空空如也 次数 值 也 其一 夫「取前个」者受 数 列 焉算法乃
      递归夫「取前个辅助」者受 累计 数 列 焉算法乃
        若 数 小于等于0 则 答 反转 累计
        余者
          观「列」之性
          若 空空如也 则 答 反转 累计
          若 有首有尾 首名为「首 其一 」 尾名为「尾」 则 答 取前个辅助 有首有尾 首名为「首 其一 」 尾名为「累计」 (数 减 1) 尾
          观毕
      在 取前个辅助 空空如也 数 列 也 其一 递归夫「跳过个」者受 数 列 焉算法乃
      若 数 小于等于0 则 答 列
      余者
        观「列」之性
        若 空空如也 则 答 空空如也
