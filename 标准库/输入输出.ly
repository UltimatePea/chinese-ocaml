...
模块 输入输出={
  ...
  语句: (列开始 「：基本输入输出实现：」
    夫「打印」者受 值 焉算法乃
      「：调用内置打印函数：」
      内置打印 值 是谓 其一 夫「打印行」者受 值 焉算法乃
      「：打印值并添加换行符：」
      打印 值 其二 打印 『\n』 是谓 其三 夫「打印错误」者受 值 焉算法乃
      「：打印到错误流：」
      内置打印错误 值 是谓 其一 夫「读取」者受 （） 焉算法乃
      「：读取一行输入 其一 不包含换行符：」
      内置读取行 （） 是谓 其一 夫「读取行」者受 （） 焉算法乃
      「：读取一行输入：」
      内置读取行 （） 是谓 其一 夫「读取字符」者受 （） 焉算法乃
      「：读取单个字符：」
      内置读取字符 （） 是谓 其一 「：格式化输出：」
    夫「格式化打印」者受 格式字符串 参数列表 焉算法乃
      ...
      递归夫「格式化辅助」者受 模板 参数 焉算法乃
        ...
        若 有首有尾 首名为「头部 其一 」 尾名为「尾部」 则
          ...
          格式化辅助 替换后 尾部
        观毕
      在 格式化辅助 格式字符串 参数列表 是谓 其一 夫「打印到字符串」者受 值 焉算法乃
      「：将值转换为字符串表示：」
      值转字符串 值 是谓 其一 「：文件操作 （需要底层支持）：」
    夫「读取文件」者受 文件路径 焉算法乃
      「：读取整个文件内容：」
      尝试 {
        内置读取文件 文件路径
      ...
      } 是谓 其一 夫「写入文件」者受 文件路径 内容 焉算法乃
      「：写入内容到文件：」
      尝试 {
        内置写入文件 文件路径 内容
      } 捕获 异常 -> {
        抛出异常 （『无法写入文件: 』 + 文件路径 + 『 - 』 + 异常消息 异常）
      } 是谓 其一 夫「追加文件」者受 文件路径 内容 焉算法乃
      「：追加内容到文件末尾：」
      尝试 {
        内置追加文件 文件路径 内容
      } 捕获 异常 -> {
        抛出异常 （『无法追加文件: 』 + 文件路径 + 『 - 』 + 异常消息 异常）
      } 是谓 其一 夫「文件存在」者受 文件路径 焉算法乃
      「：检查文件是否存在：」
      尝试 {
        内置文件存在 文件路径
      } 捕获 _ -> {
    ...
    夫「值转字符串」者受 值 焉算法乃
      ...
      余者答 『<复杂类型>』
      观毕是谓 其一 夫「列表转字符串」者受 lst 焉算法乃
      「：将列表转换为字符串表示：」
      『(列开始 』 + （连接列表 『 其一 』 （映射 值转字符串 lst）） + 『 其二 列结束)』 是谓 其一 夫「整数转字符串」者受 i 焉算法乃
      「：需要底层实现：」
      内置整数转字符串 i 是谓 其一 夫「浮点转字符串」者受 f 焉算法乃
      「：需要底层实现：」
      内置浮点转字符串 f 是谓 其一 夫「异常消息」者受 异常 焉算法乃
      「：提取异常消息：」
      内置异常消息 异常 是谓 其一 列结束);
}
...
模块 IO便捷={
  ...
  语句: (列开始 「：调试输出：显示标签和值 其一 然后返回值：」
    夫「打印调试」者受 标签 值 焉算法乃
      打印 （标签 + 『: 』 + 打印到字符串 值） 其二 值 是谓 其三 「：打印列表 其一 每个元素一行：」
    夫「打印列表」者受 lst 焉算法乃
      映射 （夫「元素函数」者受 元素 焉算法乃 打印行 元素 是谓） lst 其二 （） 是谓 其三 「：打印空行：」
    夫「打印换行」者受 （） 焉算法乃
      打印 『\n』 是谓 其一 「：询问用户输入：」
    夫「询问用户」者受 问题 焉算法乃
      打印 问题 其二 打印 『 』 其三 读取行 （） 是谓 其一 「：确认操作 （y/n）：」
    夫「确认操作」者受 问题 焉算法乃
      设「回答」为询问用户 （问题 + 『 （y/n）』） 在
      观「回答」之性
    ...
    夫「字符串转整数」者受 s 焉算法乃
      内置字符串转整数 s 是谓 其二 夫「字符串转浮点」者受 s 焉算法乃
      内置字符串转浮点 s 是谓 其三 列结束);
}