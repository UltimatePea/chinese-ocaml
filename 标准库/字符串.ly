...
模块 字符串={
  ...
  语句: (列开始 「：这里使用内置函数作为基础实现：」
    ...
    夫「是否为空」者受 串 焉算法乃
      长度 串 得 0 也 其三 夫「连接」者受 s1 s2 焉算法乃
      s1 ^ s2 也 其一 「：使用内置字符串连接操作符：」

    夫「重复」者受 次数 串 焉算法乃
      若 次数 小于等于 0 则 答 『』
      余者答 串 ^ （重复 （次数 减 1） 串） 也 其一 「：简化的字符串操作实现：」
    ...
    夫「包含」者受 子串 字符串 焉算法乃
      ...
      递归夫「检查位置」者受 位置 焉算法乃
        ...
        余者
          递归夫「匹配检查」者受 索引 焉算法乃
            若 索引 >=长度 子串 则 答 真
            余者 若 取字符 （位置 加 索引） 字符串=取字符 索引 子串
            则 答 匹配检查 （索引 加 1）
            余者答 假
          在
          若 匹配检查 0 则 答 真
      ...
      余者
        递归夫「检查」者受 索引 焉算法乃
          ...
          余者
            ...
            若 取字符 字符串索引 字符串=取字符 索引 后缀
            则 答 检查 （索引 加 1）
            余者答 假
        在 检查 0 也 其一 「：查找子串第一次出现的位置：」
    夫「查找位置」者受 子串 字符串 焉算法乃
      递归夫「查找」者受 位置 焉算法乃
        若 位置 加 长度 子串 大于 长度 字符串 则 答 -1
        余者
          递归夫「匹配检查」者受 索引 焉算法乃
            若 索引 >=长度 子串 则 答 真
            余者 若 取字符 （位置 加 索引） 字符串=取字符 索引 子串
    ...
    夫「连接列表」者受 分隔符 字符串列表 焉算法乃
      ...
      若 有首有尾 首名为「头部 其一 」 尾名为「尾部」 则
        递归夫「连接辅助」者受 累 列 焉算法乃
          观「列」之性
          若 空空如也 则 答 累
          若 有首有尾 首名为「元素 其一 」 尾名为「剩余」 则 答 连接辅助 （累 ^ 分隔符 ^ 元素） 剩余
        观毕
        在 连接辅助 头部 尾部
      观毕也 其一 「：格式化函数：」
