...
模块 字符串={
  ...
  语句: (列开始 「：这里使用内置函数作为基础实现：」
    ...
    夫「长度」者受 s 焉算法乃 长度 s 也 其二 「：使用内置长度函数：」

    夫「是否为空」者受 s 焉算法乃
      长度 s 得 0 也 其三 夫「连接」者受 s1 s2 焉算法乃
      s1 ^ s2 也 其一 「：使用内置字符串连接操作符：」

    夫「重复」者受 次数 s 焉算法乃
      若 次数 小于等于 0 则 答 『』
      余者答 s ^ （重复 （次数 - 1） s） 也 其一 「：简化的字符串操作实现：」
    ...
    夫「填充左对齐」者受 总宽度 填充字符 字符串 焉算法乃
      ...
      余者
        设「需要填充」为总宽度 - 当前长度 在
        重复 需要填充 （字符串从字符 填充字符） ^ 字符串 也 其一 夫「填充右对齐」者受 总宽度 填充字符 字符串 焉算法乃
      设「当前长度」为长度 字符串 在
      若 当前长度 >=总宽度 则 答 字符串
      余者
        设「需要填充」为总宽度 - 当前长度 在
        字符串 ^ 重复 需要填充 （字符串从字符 填充字符） 也 其一 「：辅助函数：从字符创建字符串：」
    夫「字符串从字符」者受 字符 焉算法乃
