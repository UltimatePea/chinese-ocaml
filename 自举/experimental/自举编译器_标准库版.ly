...
「：语法分析主函数：」
夫「语法分析」者受 词元列表 焉算法乃
  让「AST节点列表」=映射 （夫「匿名」者受 词元 焉算法乃 答 觧析标识符 词元 也） 词元列表 在
  答 程序 AST节点列表 也
...
递归夫「生成节点C代码」者受 节点 缩进级别 焉算法乃
  ...
  | 字符串字面量 值 -> 答 连接 缩进 （『\』『 加 值 加 』\『』）
  | 让绑定 （名称, 表达式） ->
    让「声明」=连接 缩进 （『整数 』 加 名称 加 『=』） 在
    让「表达式代码」=生成节点C代码 表达式 0 在
    答 连接 声明 （连接 表达式代码 『;』）
  | 程序 节点列表 ->
    让「节点代码列表」=映射 （夫「匿名」者受 节点 焉算法乃 答 生成节点C代码 节点 缩进级别 也） 节点列表 在
    答 连接列表 『\数』 节点代码列表
  | _ -> 答 连接 缩进 『/* 未实现的节点类型 */』
  观毕也
...
/* 骆言运行时函数 */
void luoyan_pr整数整数（整数 value） {
    pr整数f（\『%丁\\数\』, value）;
}

void luoyan_pr整数字符串（const char* value） {
    pr整数f（\『%串\\数\』, value）;
}
...
夫「编译管道」者受 源代码 文件名 焉算法乃
  让
    ...
    _=打印 『===骆言自举编译器 （标准库版）===』 在
    _=打印 （『编译文件: 』 加 文件名） 在
    _=打印 『\数(列开始 阶段1 其一 列结束) 词法分析』 在
    （词元列表, 词法状态）=词法分析 源代码 在
    _=记录步骤 （『生成了 』 加 整数转字符串 （长度 词元列表） 加 『 个词元』） 在

    「：阶段2：语法分析：」
    _=打印 『\数(列开始 阶段2 其一 列结束) 语法分析』 在
    ast=语法分析 词元列表 在
    _=记录步骤 『构建抽象语法树』 在

    「：阶段3：语义分析：」
    _=打印 『\数(列开始 阶段3 其一 列结束) 语义分析』 在
    （检查后ast, 类型信息）=语义分析 ast 在
    _=记录步骤 『类型检查通过』 在

    「：阶段4：代码生成：」
    _=打印 『\数(列开始 阶段4 其一 列结束) C代码生成』 在
...
「：显示编译器信息：」
夫「显示编译器信息」者受 （） 焉算法乃
  让「信息行」=(列开始 『骆言自举编译器 减 标准库版』 其一 『版本: 』 加 编译器版本 其二 『作者: 』 加 编译器作者 其三 『使用标准库模块: 基础 其一 数学 其二 列表 其三 字符串』 其一 『编译目标: C语言代码』 其二 列结束) 在
  让
    _=打印 『=====================================』 在
    _=映射 打印 信息行 在
    _=打印 『=====================================』 在
...
夫「运行编译器」者受 （） 焉算法乃
  让
    ...
    _=打印 （『字符串长度最大值: 』 加 整数转字符串 最大数） 在

    「：使用列表模块：」
    编译阶段列表=(列开始 『词法分析』 其一 『语法分析』 其二 『语义分析』 其三 『代码生成』 其一 列结束) 在
    阶段数量=长度 编译阶段列表 在
    _=打印 （『编译阶段数量: 』 加 整数转字符串 阶段数量） 在

    「：使用字符串模块：」
    编译器标语=连接 『骆言』 （连接 『 减 』 『AI友好的编程语言』） 在
    _=打印 （『编译器标语: 』 加 编译器标语） 在
