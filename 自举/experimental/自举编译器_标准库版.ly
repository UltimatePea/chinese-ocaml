...
夫「语义分析」者受 ast 焉算法乃
  ...
  或 程序 节点列表 减大于
    ...
    答 （ast， 类型信息）
  或 下划线 减大于 答 （ast， 空空如也）
  观毕也

「：等于等于等于 C代码生成器 等于等于等于：」
...
递归夫「生成节点C代码」者受 节点 缩进级别 焉算法乃
  ...
  或 让绑定 （名称， 表达式） 减大于
    ...
    让「表达式代码」等于生成节点C代码 表达式 0 在
    答 连接 声明 （连接 表达式代码 『；』）
  或 程序 节点列表 减大于
    让「节点代码列表」等于映射 （夫「匿名」者受 节点 焉算法乃 答 生成节点C代码 节点 缩进级别 也） 节点列表 在
    答 连接列表 『\数』 节点代码列表
...
/* 骆言运行时函数 */
void 骆言下划线pr整数整数（整数 值） 「
    pr整数f（\『百分丁\\数\』， 值）；
」

void 骆言下划线pr整数字符串（常量 char乘 值） 「
    pr整数f（\『百分串\\数\』， 值）；
」
...
夫「编译管道」者受 源代码 文件名 焉算法乃
  让
    ...
    （检查后ast， 类型信息）等于语义分析 ast 在
    下划线等于记录步骤 『类型检查通过』 在

    「：阶段4：代码生成：」
    下划线等于打印 『\数（列开始 阶段4 其一 列结束） C代码生成』 在
    c代码等于生成C程序 检查后ast 文件名 在
    下划线等于记录步骤 『C代码生成完成』 在

    「：输出结果：」
    下划线等于打印 『\数等于等于等于生成的C代码等于等于等于』 在
    下划线等于打印 c代码 在
...
「：显示编译器信息：」
夫「显示编译器信息」者受 （） 焉算法乃
  让「信息行」等于（列开始 『骆言自举编译器 减 标准库版』 其一 『版本： 』 加 编译器版本 其二 『作者： 』 加 编译器作者 其三 『使用标准库模块： 基础 其一 数学 其二 列表 其三 字符串』 其一 『编译目标： C语言代码』 其二 列结束） 在
  让
