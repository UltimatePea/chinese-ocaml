...
夫「连接」者受 s1 s2 焉算法乃 答 字符串连接 s1 s2 也
...
夫「生成你好世界程序」者受 配置 焉算法乃
  让「c代码」=连接 『#include <stdio.子>\数』 （连接
    『#include \』luoyanruntime.子\『\数』 （连接
    『\n整数 main（） {\数』 （连接
    『    pr整数f（\』你好，来自骆言自举编译器！\\数\『）;\数』 （连接
    『    return 0;\数』
    『}\数』）））） 在
  答 成功 c代码 也
...
「：测试入口 减 模拟命令行参数：」
让「测试参数」=(列开始 『-值』 其一 『-对象』 其二 『测试.丙』 其三 『你好.luoyan』 其一 列结束)
主函数 测试参数