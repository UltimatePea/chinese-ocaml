(** 骆言编译器日志格式化模块
    ...
    本模块专注于日志和调试信息的格式化，从unified_formatter.ml中拆分出来。
    提供统一的日志格式化接口，消除Printf.sprintf依赖。
...
module CompilerMessages = struct
  ...
  let compilation_progress current total filename =
    concat_strings [ 
      "编译进度 ["; int_to_string current; "/"; int_to_string total; "] "; filename 
    ]
end
...
module EnhancedLogMessages = struct
  ...
  (** 性能日志 *)
  let performance_start operation = 
    context_message_pattern "[PERF-START]" operation
  ...
  let memory_usage operation heap_mb stack_mb =
    concat_strings [ 
      "[MEMORY] "; operation; " - 堆内存: "; int_to_string heap_mb; "MB, 栈内存: "; 
      int_to_string stack_mb; "MB" 
    ]
  ...
  let test_suite_summary total_tests passed_tests failed_tests =
    concat_strings [ 
      "[TEST-SUMMARY] 总计: "; int_to_string total_tests; 
      ", 通过: "; int_to_string passed_tests; 
      ", 失败: "; int_to_string failed_tests
    ]
end
...
module LoggingFormatter = struct
  ...
  let format_timestamp year month day hour minute second =
    concat_strings [ 
      int_to_string year; "-"; 
      (if month < 10 then "0" else ""); int_to_string month; "-";
      (if day < 10 then "0" else ""); int_to_string day; " ";
      (if hour < 10 then "0" else ""); int_to_string hour; ":";
      (if minute < 10 then "0" else ""); int_to_string minute; ":";
      (if second < 10 then "0" else ""); int_to_string second
    ]
  ...
  let format_simple_log_entry level message =
    concat_strings [ "["; level; "] "; message ]
  ...
  (** 日志分隔符 *)
  let log_separator length char =
    String.make length (String.get char 0)
  ...
  let format_debug_context function_name variables =
    let var_list = List.map (fun (name, value) -> 
      concat_strings [ name; "="; value ]) variables in
    concat_strings [ "[DEBUG-CONTEXT] "; function_name; " {"; join_with_separator ", " var_list; "}" ]
...
module DebugFormatter = struct
  ...
  let format_variable_list variables =
    let formatted_vars = List.map (fun (name, typ, value) -> 
      format_variable_state name typ value) variables in
    join_with_separator "\n" formatted_vars
  ...
  let format_call_stack calls =
    let numbered_calls = List.mapi (fun i call -> 
      concat_strings [ int_to_string (i + 1); ". "; call ]) calls in
    join_with_separator "\n" numbered_calls

  (** 表达式求值追踪 *)
  let format_expression_eval expr result =
    concat_strings [ expr; " => "; result ]

  let format_step_by_step_eval steps =
    let numbered_steps = List.mapi (fun i step -> 
      concat_strings [ "步骤 "; int_to_string (i + 1); ": "; step ]) steps in
    join_with_separator "\n" numbered_steps

  (** AST节点格式化 *)
  let format_ast_node node_type node_content =
    concat_strings [ node_type; "("; node_content; ")" ]
