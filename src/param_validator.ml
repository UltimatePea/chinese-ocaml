...
(** 创建类型验证器的通用函数 *)
let create_type_validator (type_name : string) (extractor : runtime_value -> 'a option) :
    'a validator =
 fun value ->
...
(** 复合类型提取器 *)
let extract_number = function (IntValue _ | FloatValue _) as v -> Some v | _ -> None

let extract_string_or_list = function (StringValue _ | ListValue _) as v -> Some v | _ -> None
let extract_nonempty_list = function ListValue [] -> None | ListValue lst -> Some lst | _ -> None
...
let validate_triple (v1 : 'a validator) (v2 : 'b validator) (v3 : 'c validator) function_name args =
  let arg1, arg2, arg3 =
    match args with [ arg1; arg2; arg3 ] -> (arg1, arg2, arg3) | _ -> runtime_error "期望三个参数"
  in
  let wrapped_v1 = with_function_name v1 function_name in
  let wrapped_v2 = with_function_name v2 function_name in
