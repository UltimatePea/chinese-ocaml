(** 计算复杂度性能分析器模块
    ...
    专门分析计算复杂度和嵌套循环，从refactoring_analyzer_performance.ml中提取 创建目的：减少主文件复杂度，专注于复杂度相关的性能优化 创建时间：技术债务清理
    Fix #654 *)
...
let analyze_computational_complexity expr =
  let rec detect_nested_loops_with_suggestions expr nesting_level =
    ...
    | MatchExpr (matched_expr, branches) ->
        let _, matched_suggestions =
          detect_nested_loops_with_suggestions matched_expr nesting_level
        in
