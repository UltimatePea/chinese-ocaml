...
module KeywordTable = struct
  ...
  let find_chinese_keyword keyword =
    try Some (Hashtbl.find chinese_table keyword) 
    with Not_found -> None

  (** 查找ASCII关键字 *)
  let find_ascii_keyword keyword =
    try Some (Hashtbl.find ascii_table keyword)
    with Not_found -> None
...
module OptimizedMatcher = struct
      ...
      | (keyword, token) :: rest ->
          ...
          if state.position + keyword_len <= state.length then
            ...
            if substring = keyword then
              (* 检查关键字边界 *)
              if BoundaryDetection.is_chinese_keyword_boundary state.input state.position keyword then
                (* 找到完整关键字，但继续检查是否有更长的匹配 *)
                let new_match = Some (token, keyword_len) in
                (match best_match with
                | None -> try_keywords rest new_match
                | Some (_, best_len) when keyword_len > best_len -> 
                    try_keywords rest new_match
  ...
  let match_by_prefix state =
    let rec try_keywords keywords best_match =
      ...
      | (keyword, token) :: rest ->
          let keyword_len = String.length keyword in
          if state.position + keyword_len <= state.length &&
             String.sub state.input state.position keyword_len = keyword &&
             BoundaryDetection.is_chinese_keyword_boundary state.input state.position keyword then
            let new_match = Some (token, keyword_len) in
            (match best_match with
            | None -> try_keywords rest new_match
            | Some (_, best_len) when keyword_len > best_len -> 
...
module KeywordAnalytics = struct
  ...
  let analyze_keywords () =
    ...
    
    {
      ...
      min_length = List.fold_left min (List.hd all_lengths) (List.tl all_lengths);
    }
...
(** 主要匹配函数 - 对外接口 *)
let match_keyword state =
  OptimizedMatcher.match_by_prefix state

(** 快速关键字查找 - 用于其他模块 *)
let lookup_keyword keyword_string =
  KeywordTable.find_keyword keyword_string

(** 检查字符串是否为关键字 *)
let is_keyword keyword_string =
  match lookup_keyword keyword_string with
  | Some _ -> true
  | None -> false