(** 骆言编译器C代码生成格式化模块
    ...
    本模块专注于C代码生成的格式化功能，从unified_formatter.ml中拆分出来。
    提供类型安全的C代码格式化接口，消除Printf.sprintf依赖。
...
module CCodegen = struct
  ...
  let c_while_loop condition body =
    concat_strings [ "while ("; condition; ") {\n"; body; "\n}" ]
  ...
  let c_struct_definition struct_name fields =
    let formatted_fields = List.map (fun (field_type, field_name) ->
      concat_strings [ "    "; field_type; " "; field_name; ";" ]) fields in
    concat_strings [ 
      "typedef struct {\n"; 
      join_with_separator "\n" formatted_fields; 
      "\n} "; struct_name; ";" 
...
module EnhancedCCodegen = struct
  ...
  let luoyan_array_get array index = function_call_format "luoyan_array_get" [ array; int_to_string index ]
  let luoyan_array_set array index value = function_call_format "luoyan_array_set" [ array; int_to_string index; value ]

  let luoyan_record_create field_count = function_call_format "luoyan_record_create" [ int_to_string field_count ]
  let luoyan_record_get record field_name = function_call_format "luoyan_record_get" [ record; "\"" ^ field_name ^ "\"" ]
  let luoyan_record_set record field_name value = function_call_format "luoyan_record_set" [ record; "\"" ^ field_name ^ "\""; value ]

  (** 类型检查 *)
  let luoyan_type_check value expected_type = function_call_format "luoyan_type_check" [ value; "\"" ^ expected_type ^ "\"" ]
  let luoyan_is_type value type_name = function_call_format "luoyan_is_type" [ value; "\"" ^ type_name ^ "\"" ]

  (** 错误处理 *)
  let luoyan_error_throw error_code message = function_call_format "luoyan_error_throw" [ int_to_string error_code; "\"" ^ message ^ "\"" ]
  ...
  (** 调试和性能 *)
  let luoyan_debug_trace function_name args = function_call_format "luoyan_debug_trace" [ "\"" ^ function_name ^ "\""; args ]
  let luoyan_profile_start label = function_call_format "luoyan_profile_start" [ "\"" ^ label ^ "\"" ]
...
module CodeGenUtilities = struct
  ...
  let c_indent_block block indent_level =
    ...
    let indented_lines = List.map (fun line -> 
      if String.trim line = "" then line 
      else concat_strings [ indent_str; line ]) lines in
    join_with_separator "\n" indented_lines

  let c_format_parameter_list params =
    if List.length params = 0 then "void"
    else join_with_separator ", " params
