(** 骆言集合值操作模块 - Collection Value Operations Module
    ...
    技术债务改进：大型模块重构优化 Phase 2.3 - 集合值类型操作模块化 处理集合值类型：ListValue, ArrayValue, TupleValue
...
let list_nth list index =
  ...
  | ListValue items, IntValue i ->
      if i < 0 || i >= List.length items then raise (RuntimeError "列表索引超出范围") else List.nth items i
...
let array_get array index =
  ...
  | ArrayValue arr, IntValue i ->
      if i < 0 || i >= Array.length arr then raise (RuntimeError "数组索引超出范围") else arr.(i)
...
let array_set array index value =
  match (array, index) with
  | ArrayValue arr, IntValue i ->
      if i < 0 || i >= Array.length arr then raise (RuntimeError "数组索引超出范围")
      else (
        arr.(i) <- value;
        UnitValue)
...
let array_make length init_value =
  match length with
  | IntValue len ->
      if len < 0 then raise (RuntimeError "数组长度不能为负数") else ArrayValue (Array.make len init_value)
...
let tuple_nth tuple index =
  ...
  | TupleValue items, IntValue i ->
      if i < 0 || i >= List.length items then raise (RuntimeError "元组索引超出范围") else List.nth items i
