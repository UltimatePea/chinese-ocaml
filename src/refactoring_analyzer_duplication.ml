...
let extract_expression_pattern expr =
  let rec analyze_structure expr =
    ...
    | BinaryOpExpr (left, op, right) ->
        ...
        Unified_logger.Legacy.sprintf "BinaryOp(%s,%s,%s)" left_pattern (show_binary_op op) right_pattern
    | UnaryOpExpr (op, expr) ->
    ...
    | MatchExpr (_, branches) ->
        Unified_logger.Legacy.sprintf "Match(%d)" (List.length branches)
    | LetExpr (_, _, _) -> "LetBinding"
    | FunExpr (params, _) ->
        Unified_logger.Legacy.sprintf "Function(%d)" (List.length params)
    | ListExpr exprs ->
        Unified_logger.Legacy.sprintf "List(%d)" (List.length exprs)
    | RecordExpr fields ->
        Unified_logger.Legacy.sprintf "Record(%d)" (List.length fields)
    | _ -> "Other"
  in
...
let detect_structural_duplication exprs =
  ...
  List.iter (fun expr ->
    ...
    let existing = try Hashtbl.find pattern_groups pattern.pattern_signature with Not_found -> [] in
    Hashtbl.replace pattern_groups pattern.pattern_signature (expr :: existing)
  ) exprs;

  (* 检查每个组的重复数量 *)
  Hashtbl.iter (fun pattern_sig exprs_list ->
...
let detect_function_duplication function_exprs =
  ...
  List.iter (fun (name, expr) ->
    let pattern = extract_expression_pattern expr in
    let existing = try Hashtbl.find function_patterns pattern.pattern_signature with Not_found -> [] in
    Hashtbl.replace function_patterns pattern.pattern_signature ((name, expr) :: existing)
  ) function_exprs;

  (* 检查相似的函数 *)
  Hashtbl.iter (fun _pattern_sig functions ->
    ...
    if count >= 2 then
      ...
      suggestions := {
        ...
        message = Unified_logger.Legacy.sprintf "函数 %s 具有相似的结构，可能存在重复逻辑" 
...
let detect_code_clones exprs =
  ...
  List.iter (fun expr ->
    ...
    let exact_count = try Hashtbl.find exact_patterns exact_pattern.pattern_signature with Not_found -> 0 in
    ...
    in
    let struct_count = try Hashtbl.find structural_patterns structural_pattern with Not_found -> 0 in
    Hashtbl.replace structural_patterns structural_pattern (struct_count + 1)
  ) exprs;
  
  (* 检查Type-1克隆 *)
  Hashtbl.iter (fun _pattern count ->
    if count >= Config.min_duplication_threshold then
      suggestions := {
        suggestion_type = DuplicatedCode [];
...
let analyze_duplication_impact suggestions =
  let duplication_suggestions = List.filter (function
    | {suggestion_type = DuplicatedCode _; _} -> true
    | _ -> false
  ) suggestions in
  
  let high_impact = List.filter (fun s -> s.confidence >= 0.8) duplication_suggestions in
  let medium_impact = List.filter (fun s -> s.confidence >= 0.6 && s.confidence < 0.8) duplication_suggestions in
...
let generate_duplication_report suggestions =
  ...
  Buffer.add_string report (Unified_logger.Legacy.sprintf "   📈 总计: %d 个重复问题\n\n" total_duplications);
  
  if total_duplications = 0 then
    Buffer.add_string report "✅ 恭喜！没有发现明显的代码重复问题。\n"
  else (
    Buffer.add_string report "🛠️ 重复代码优化建议:\n";
    if high_impact > 0 then
      Buffer.add_string report "   1. 优先处理高影响的重复代码，这些会显著影响维护性\n";
    if medium_impact > 0 then
      Buffer.add_string report "   2. 考虑重构中等影响的重复代码\n";
    if low_impact > 0 then
      Buffer.add_string report "   3. 评估低影响的重复代码，根据实际情况决定是否重构\n";
