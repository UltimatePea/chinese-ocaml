...
let find_similar_variables target_var available_vars =
  let similarities = List.map (fun var ->
...
let analyze_undefined_variable var_name available_vars =
  ...
  let suggestions = match similar_vars with
  | [] when List.length available_vars = 0 ->
    ["当前作用域中没有可用变量，检查是否需要先定义变量"]
  | [] ->
    ["检查变量名拼写是否正确";
     "确认变量是否在当前作用域中定义";
     "可用变量: " ^ String.concat "、" (let rec take n lst = if n <= 0 then [] else match lst with [] -> [] | h::t -> h :: take (n-1) t in take 5 available_vars)]
...
let analyze_type_mismatch expected_type actual_type =
  let type_suggestions = match (expected_type, actual_type) with
  | ("整数类型", "字符串类型") ->
    ["尝试将字符串转换为整数: 转换为整数 「字符串值」";
     "检查是否误用了字符串字面量而非数字"]
  | ("字符串类型", "整数类型") ->
    ["尝试将整数转换为字符串: 转换为字符串 数字值";
     "检查是否需要字符串插值"]
  | ("列表类型", _) ->
    ["检查是否遗漏了方括号: [元素1; 元素2]";
     "确认是否需要将单个值包装为列表"]
  ...
  | _ ->
    ["检查表达式的类型是否正确";
     "考虑是否需要类型转换"]
  in
  let fix_hints = match (expected_type, actual_type) with
  | ("整数类型", "字符串类型") ->
    ["添加类型转换: (转换为整数 值)"]
  | ("字符串类型", "整数类型") ->
    ["添加类型转换: (转换为字符串 值)"]
...
let analyze_function_arity expected_count actual_count function_name =
  ...
  let fix_hints =
    if actual_count < expected_count then
      ...
      [Printf.sprintf "移除多余的 %d 个参数" (actual_count - expected_count)]
  in
  {
...
let intelligent_error_analysis error_type error_details context =
  ...
  | "undefined_variable" ->
    let (var_name, available_vars) = match error_details with
