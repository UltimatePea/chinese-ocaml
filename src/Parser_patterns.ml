...
let rec parse_pattern state =
  ...
  | _ ->
    ...
    else if is_left_bracket token then
      parse_list_pattern state
    else
      raise (SyntaxError ("期望模式", pos))

(** 解析构造器参数 *)
and parse_constructor_args state parse_expression_fn =
  let rec loop acc state =
    if is_right_paren (current_token state |> fst) then
      (List.rev acc, state)
    else
...
and build_cons_pattern = function
  ...
  | [single] -> single
  | head :: tail ->
    ConsPattern (head, build_cons_pattern tail)
...
and parse_match_cases state parse_expression_fn =
  let rec loop acc state =
    if not (is_pipe (current_token state |> fst)) then
      (List.rev acc, state)
    else
      let state = advance_parser state in
      let pattern, state = parse_pattern state in
  ...
  in
  if is_pipe (current_token state |> fst) then
    loop [] state
  else
    raise (SyntaxError ("期望 | 开始匹配分支", current_token state |> snd))
