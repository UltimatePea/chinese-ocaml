...
module CCodegen = struct
  ...
  (** 格式化一元运算 *)
  let format_unary_op op_name operand = Unified_logger.Legacy.sprintf "%s(%s)" op_name operand
  ...
  (** 格式化整数字面量 *)
  let format_int_literal i = Unified_logger.Legacy.sprintf "luoyan_int(%d)" i

  (** 格式化浮点数字面量 *)
  let format_float_literal f = Unified_logger.Legacy.sprintf "luoyan_float(%g)" f
...
module ErrorMessages = struct
  ...
  (** 格式化意外词元错误 *)
  let format_unexpected_token token = Unified_logger.Legacy.sprintf "意外的词元: %s" token
