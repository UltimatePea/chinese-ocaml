...
module CCodegen = struct
  ...
  let format_function_call func_name args = Base_formatter.function_call_format func_name args
  ...
  let format_unary_op op_name operand = Base_formatter.function_call_format op_name [ operand ]

  (** 格式化变量绑定 - 使用Base_formatter消除Printf.sprintf *)
  let format_var_binding var_name value = Base_formatter.luoyan_env_bind_pattern var_name value

  (** 格式化字符串字面量 - 使用Base_formatter消除Printf.sprintf *)
  let format_string_literal s =
    Base_formatter.luoyan_function_pattern "string"
      (Base_formatter.concat_strings [ "\""; String.escaped s; "\"" ])
  ...
  let format_unit_literal () = Base_formatter.luoyan_function_pattern "unit" ""
...
module ErrorMessages = struct
  ...
  let format_param_count_mismatch expected actual =
    Base_formatter.concat_strings
      [
        "函数期望";
        Base_formatter.int_to_string expected;
        "个参数，提供了";
        Base_formatter.int_to_string actual;
        "个";
      ]
  ...
  let format_missing_params_filled expected actual missing =
    Base_formatter.concat_strings
      [
        ...
        Base_formatter.int_to_string actual;
        "个，用默认值填充缺失的";
        Base_formatter.int_to_string missing;
        "个参数";
      ]
  ...
  let format_extra_params_ignored expected actual extra =
    Base_formatter.concat_strings
      [
        ...
        Base_formatter.int_to_string actual;
        "个，忽略多余的";
        Base_formatter.int_to_string extra;
        "个参数";
      ]

  (** 格式化意外词元错误 - 使用Base_formatter消除Printf.sprintf *)
  let format_unexpected_token token = Base_formatter.concat_strings [ "意外的词元: "; token ]
...
module General = struct
  ...
  let format_identifier name = Base_formatter.concat_strings [ "「"; name; "」" ]

  (** 格式化函数签名 - 使用Base_formatter消除Printf.sprintf *)
  let format_function_signature name params = Base_formatter.function_call_format name params
  ...
  let format_module_path path = Base_formatter.join_with_separator "." path

  (** 格式化列表 - 使用Base_formatter消除Printf.sprintf *)
  let format_list items separator = Base_formatter.join_with_separator separator items

  (** 格式化键值对 - 使用Base_formatter消除Printf.sprintf *)
  let format_key_value key value = Base_formatter.concat_strings [ key; ": "; value ]
end
