...
(** 类型别名 - 从 Package_utils 导入 *)
type package_info = Package_utils.package_info

(** 函数别名 - 从其他模块导入 *)
let parse_package_config = Package_config_parser.parse_package_config
let validate_package_config = Package_config_parser.validate_package_config
...
(** TOML解析器 - 已迁移到 Package_config_parser 模块 *)

(** 包配置解析和验证 - 已迁移到 Package_config_parser 模块 *)
...
let find_package_config dir_path =
  let config_path = Filename.concat dir_path Package_utils.package_config_filename in
  if Sys.file_exists config_path then
    try
      let content = 
        ...
        content
      in
      Package_config_parser.parse_package_config content
    with
...
let get_package_cache_dir () =
  ...
  Filename.concat home_dir Package_utils.package_cache_dir_name

let get_package_install_dir package_name =
  let cache_dir = get_package_cache_dir () in
  Filename.concat (Filename.concat cache_dir Package_utils.package_install_dir_name) package_name
    ...
    try
      ...
      Array.fold_left (fun acc entry ->
        ...
        if Sys.is_directory package_dir then
          ...
          | Ok config ->
            let info = Package_utils.create_package_info config package_dir in
            info :: acc
          | Error _ -> acc
        else acc
      ) [] entries
       ...
       | Some registry ->
         (match find_package_in_registry registry package_name None with
          ...
          | Some (version, _config) ->
            let cache_dir = get_package_cache_dir () in
            let packages_dir = Filename.concat cache_dir Package_utils.package_install_dir_name in
            mkdir_p packages_dir;
            (match download_package registry package_name version packages_dir with
             | Ok package_dir ->
               audit_log "PACKAGE_INSTALL" (Printf.sprintf "Installed package %s v%s" package_name version);
               StringValue ("成功安装包: " ^ package_name ^ " v" ^ version ^ " 到目录: " ^ package_dir)
             | Error (PackageNotFound msg) -> StringValue ("包不存在: " ^ msg)
             | Error (NetworkError msg) -> StringValue ("网络错误: " ^ msg)
             | Error (RegistryNotAvailable msg) -> StringValue ("仓库不可用: " ^ msg)
             | Error _ -> StringValue ("安装失败: 未知错误"))))
       ...
       | Some registry ->
         (match find_package_in_registry registry package_name (Some ("=" ^ version)) with
          ...
          | Some (found_version, _config) ->
            ...
            let packages_dir = Filename.concat cache_dir Package_utils.package_install_dir_name in
            mkdir_p packages_dir;
            (match download_package registry package_name found_version packages_dir with
             | Ok package_dir ->
               audit_log "PACKAGE_INSTALL" (Printf.sprintf "Installed package %s v%s" package_name found_version);
               StringValue ("成功安装包: " ^ package_name ^ " v" ^ found_version ^ " 到目录: " ^ package_dir)
             | Error (PackageNotFound msg) -> StringValue ("包不存在: " ^ msg)
             | Error (NetworkError msg) -> StringValue ("网络错误: " ^ msg)
             | Error (RegistryNotAvailable msg) -> StringValue ("仓库不可用: " ^ msg)
             | Error _ -> StringValue ("安装失败: 未知错误"))))
...
let list_packages_function args =
  handle_package_error "列出包" "包列表查询" (fun () ->
    ...
    | [] ->
      ...
      StringValue (Package_utils.format_package_list packages)
    ...
    | [StringValue search_term] ->
      (match get_default_registry () with
       ...
       | Some registry ->
         ...
         StringValue (Package_utils.format_search_results search_term results))
  ...
  handle_package_error "包信息" "包信息查询" (fun () ->
    ...
    | [StringValue package_name] ->
      ...
      (match List.find_opt (fun info -> info.config.name = package_name) packages with
       | Some info -> StringValue (Package_utils.format_package_info info)
