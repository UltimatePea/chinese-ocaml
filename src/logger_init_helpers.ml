(** 日志器初始化助手模块 - Logger Initialization Helpers
    ...
    本模块提供标准化的日志器初始化函数，消除项目中12个文件的重复代码。
    通过提供模块级别的初始化宏，简化日志器初始化流程。
    
    改进目标：
    1. 消除 `let () = Logger_utils.init_no_logger "ModuleName"` 重复模式
    2. 提供更高效的批量初始化机制
    3. 支持按模块类型的智能初始化策略
    4. 减少代码重复，提高可维护性
...
let infer_module_category module_name =
  ...
  | name when String.contains name 'V' && (String.contains name 'a' || String.contains name 'O') -> ValueModule
...
(** 批量初始化多个模块的日志器 *)
let init_multiple_modules module_names =
  List.iter init_module_logger module_names
