...
(** 解析带标签的函数调用
    ...
    处理形如 "函数名 ~标签1:值1 ~标签2:值2" 的调用语法。
    标签参数允许以任意顺序传递，提高函数调用的可读性。
...
(** 收集无括号函数调用的参数
    ...
    持续收集基础表达式作为函数参数，直到遇到不能作为参数的token。
    支持的参数类型包括：
    - 带引号的标识符
    - 整数和浮点数字面量
    - 中文数字
...
(** {7 设计说明}
    ...
    1. **模块解耦**: 避免循环依赖，保持模块间的清晰边界
    2. **解析器组合**: 支持不同类型表达式解析器的灵活组合
    3. **错误传播**: 统一的错误处理和状态传递机制
    4. **语法扩展**: 便于添加新的函数调用语法支持
