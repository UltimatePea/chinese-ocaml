...
(** 检查字符是否为已知韵字 *)
let _is_known_rhyme_char = Rhyme_api_core.is_known_rhyme_char

(** 获取字符的韵律描述 *)
let _get_rhyme_description = Rhyme_api_core.get_rhyme_description
...
(** 分析诗句的韵律结构 *)
let _analyze_poem_line_structure = Rhyme_advanced_analysis.analyze_poem_line_structure

(** 检测诗句间的押韵关系 *)
let _detect_poem_rhyme_scheme = Rhyme_advanced_analysis.detect_poem_rhyme_scheme

(** 评估文本的韵律质量 *)
let _evaluate_rhyme_quality = Rhyme_advanced_analysis.evaluate_rhyme_quality

(** 建议押韵字符 *)
let _suggest_rhyming_chars = Rhyme_advanced_analysis.suggest_rhyming_chars
...
(** 获取指定韵组的字符集 *)
let _get_rhyme_group_chars = Unified_rhyme_data.get_rhyme_group_chars

(** 获取所有韵组列表 *)
let _get_all_rhyme_groups () = 
  List.map (fun (group, _) -> group) (Unified_rhyme_data.get_all_rhyme_groups ())

(** 获取韵律数据统计信息 *)
let _get_data_stats = Unified_rhyme_data.get_data_stats
