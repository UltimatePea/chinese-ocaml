(** 统一韵律API模块 - 消除音韵数据重复
    ...
    此模块提供统一的韵律数据访问接口，消除项目中273处音韵相关的重复代码。 基于现有的韵律数据，提供高效、一致的韵律检测和处理功能。
...
let load_rhyme_data () =
  if not !initialized then (
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (PingSheng, AnRhyme)) an_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (PingSheng, SiRhyme)) si_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (PingSheng, TianRhyme)) tian_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (PingSheng, WangRhyme)) wang_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (QuSheng, QuRhyme)) qu_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (PingSheng, YuRhyme)) yu_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (PingSheng, HuaRhyme)) hua_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (PingSheng, FengRhyme)) feng_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (RuSheng, YueRhyme)) yue_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (RuSheng, XueRhyme)) xue_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (ZeSheng, JiangRhyme)) jiang_chars;
    ...
    in
    List.iter (fun char -> Hashtbl.replace rhyme_cache char (ShangSheng, HuiRhyme)) hui_chars;
...
let get_rhyme_characters group =
  load_rhyme_data ();
  try Hashtbl.find rhyme_group_chars group with Not_found -> []
