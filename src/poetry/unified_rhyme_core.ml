(** 统一韵律核心模块 - Poetry模块整合优化 (简化版本)
    ...
    此模块整合所有分散的韵律数据文件，提供统一的数据访问接口。 这是Poetry模块技术债务清理计划的核心成果。
  ...
  [
    {
      ...
      description = "平声安韵组，古典诗词常用韵组";
    };
    {
      group_id = SiRhyme;
      group_name = "思韵";
      entries = si_rhyme_entries;
      description = "平声思韵组，表达思考情感";
    };
    {
      group_id = TianRhyme;
      group_name = "天韵";
      entries = tian_rhyme_entries;
      description = "平声天韵组，常用于描写天地自然";
    };
    {
      group_id = YuRhyme;
      group_name = "鱼韵";
      entries = yu_rhyme_entries;
      description = "平声鱼韵组，描写水中生物和居住";
    };
    {
      group_id = HuaRhyme;
      group_name = "花韵";
      entries = hua_rhyme_entries;
      description = "仄声花韵组，描写美好事物";
    };
  ]
...
let build_character_index groups =
  ...
  List.iter
    (fun group -> List.iter (fun entry -> Hashtbl.add index entry.character entry) group.entries)
    groups;
  index
...
let calculate_stats groups =
  let total_characters =
    List.fold_left (fun acc group -> acc + List.length group.entries) 0 groups
  in
...
let find_rhyme_info character =
  try Some (Hashtbl.find unified_database.index character) with Not_found -> None
...
let get_available_rhyme_groups () =
  List.map
    (fun group -> (group.group_id, group.group_name, group.description))
    unified_database.groups
