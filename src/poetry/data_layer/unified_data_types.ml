(** 骆言诗词统一数据类型模块 - Poetry模块整合优化 Fix #1707
    
    此模块是数据层第一个模块，统一定义所有诗词分析系统的核心类型。
    整合来源：poetry_core_types.ml, poetry_types_consolidated.ml, core/poetry_types.ml
...
(** 艺术维度转字符串 *)
let string_of_artistic_dimension = function
  | RhymeHarmony -> "韵律和谐"
  | TonalBalance -> "声调平衡"
  | Parallelism -> "对仗工整"
  | Imagery -> "意象深度"
...
(** 诗词形式转字符串 *)
let string_of_poem_form = function
  | LuShi -> "律诗"
  | JueShi -> "绝句"
  | GuFengShi -> "古风诗"
  | CiPai name -> "词牌·" ^ name
...
let string_of_poetry_form = function
  | SiYanPianTi -> "四言骈体"
  | WuYanLuShi -> "五言律诗"
  | QiYanJueJu -> "七言绝句"
  | CiPaiForm name -> "词牌·" ^ name
  | ModernPoetry -> "现代诗"
  | SiYanParallelProse -> "四言骈体散文"
...
let string_to_rhyme_group s =
  ...
  | "天韵" | "TianRhyme" | "tian" -> Some TianRhyme
  | "望韵" | "WangRhyme" | "wang" -> Some WangRhyme
  | "去韵" | "QuRhyme" | "qu_rhyme" -> Some QuRhyme
  | "鱼韵" | "YuRhyme" | "yu" -> Some YuRhyme
  | "花韵" | "HuaRhyme" | "hua" -> Some HuaRhyme
  | "风韵" | "FengRhyme" | "feng" -> Some FengRhyme
  | "月韵" | "YueRhyme" | "yue" -> Some YueRhyme
  | "雪韵" | "XueRhyme" | "xue" -> Some XueRhyme
  | "江韵" | "JiangRhyme" | "jiang" -> Some JiangRhyme
  | "灰韵" | "HuiRhyme" | "hui" -> Some HuiRhyme
...
(** 创建空报告 *)
let create_empty_artistic_report verse = {
  verse;
  rhyme_score = 0.0;
  tone_score = 0.0;
...
let dimension_to_string = string_of_artistic_dimension
let grade_to_string = string_of_evaluation_grade
let form_to_string = string_of_poetry_form