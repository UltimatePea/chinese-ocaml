...
(** 声调类型 *)
type tone_type = Ping  (** 平声 *) | Ze  (** 仄声 *) | Unknown  (** 未知 *)
...
let wuyan_jueju_standard =
  {
    ...
    weight_distribution =
      [ ("韵律和谐", 0.25); ("平仄协调", 0.25); ("意境营造", 0.20); ("用词精练", 0.15); ("结构完整", 0.15) ];
    minimum_score = 0.6;
...
let qiyan_lushi_standard =
  {
    ...
    weight_distribution =
      [ ("平仄协调", 0.30); ("对仗工整", 0.25); ("韵律和谐", 0.20); ("意境深远", 0.15); ("章法严谨", 0.10) ];
    minimum_score = 0.7;
...
let gushi_standard =
  {
    ...
    weight_distribution = [ ("意境营造", 0.35); ("情感表达", 0.25); ("语言优美", 0.20); ("创意新颖", 0.20) ];
    minimum_score = 0.5;
...
let ci_standard =
  {
    ...
    applicable_forms = [ Ci ];
    weight_distribution =
      [ ("音律协调", 0.30); ("情感真挚", 0.25); ("词牌规范", 0.20); ("意象丰富", 0.15); ("语言精美", 0.10) ];
    minimum_score = 0.65;
    excellence_threshold = 0.85;
  }
...
let evaluate_against_standard poem_lines standard =
  ...
  let applies = match form with Some f -> List.mem f standard.applicable_forms | None -> false in

  if not applies then (false, 0.0, "不适用该标准")
  else
    ...
    let rhyme_score =
      let unique_rhymes =
        String.to_seq rhyme_pattern
        |> Seq.fold_left (fun acc c -> if List.mem c acc then acc else c :: acc) []
      in
    ...
    let tone_score =
      ...
      in
      let balance_ratio =
        if total_chars = 0 then 0.5 else float_of_int ping_count /. float_of_int total_chars
      in
    ...
    let evaluation_text =
      Printf.sprintf "结构评分: %.2f, 韵律评分: %.2f, 平仄评分: %.2f, 综合: %.2f" structure_score rhyme_score
        tone_score overall_score
    in
...
(** 所有预定义标准 *)
let all_standards = [ wuyan_jueju_standard; qiyan_lushi_standard; gushi_standard; ci_standard ]

(** 查找标准 *)
let find_standard name = List.find_opt (fun std -> std.name = name) all_standards

(** 列出所有标准名称 *)
let list_standard_names () = List.map (fun std -> std.name) all_standards
...
let normalize_weights weights =
  let total = List.fold_left (fun acc (_, weight) -> acc +. weight) 0.0 weights in
  if total = 0.0 then weights else List.map (fun (name, weight) -> (name, weight /. total)) weights
