...
let validate_verse_input verse =
  ...
  if String.length trimmed = 0 then Error "诗句不能为空"
  else if String.length trimmed > 100 then Error "诗句过长（超过100字符）"
  else if String.length trimmed < 2 then Error "诗句过短（少于2字符）"
  else Ok trimmed

(** 验证诗句列表输入 *)
let validate_verses_input verses =
  if List.length verses = 0 then Error "诗句列表不能为空"
  else if List.length verses > 20 then Error "诗句数量过多（超过20句）"
  else
    ...
    if List.length errors > 0 then Error ("输入验证失败: " ^ String.concat "; " errors)
    else
      let valid_verses =
        List.filter_map (function Ok v -> Some v | Error _ -> None) validate_all
      in
      Ok valid_verses
...
let is_dimension_applicable dimension context =
  ...
  | RhymeHarmony -> verse_count >= min_verses && verse_count >= RhymeConfig.min_verses_for_analysis
  | Parallelism -> verse_count >= min_verses && verse_count mod 2 = 0
  | FormBeauty -> verse_count >= 1
  | TonalBalance | Imagery | Rhythm | Elegance -> String.length context.verse > 0
  | ContentDepth -> String.length context.verse > 10 || List.length context.metadata > 0
  | _ -> true
...
let filter_low_confidence_scores dimension_scores =
  List.filter
    (fun score -> score.confidence >= TextConfig.min_confidence_threshold)
    dimension_scores
...
let filter_outlier_scores dimension_scores =
  List.filter
    (fun score ->
      score.score >= 0.0 && score.score <= 1.0 && score.confidence >= 0.0 && score.confidence <= 1.0)
    dimension_scores

(** 限制建议数量 *)
let limit_suggestions suggestions max_count =
  if List.length suggestions <= max_count then suggestions else list_take max_count suggestions
...
let detect_poetry_form verses =
  ...
  let avg_length =
    if verse_count = 0 then 0 else List.fold_left ( + ) 0 verse_lengths / verse_count
  in
...
let filter_by_poetry_form poetry_form dimensions =
  ...
  | Some "五言绝句" | Some "七言绝句" ->
      (* 绝句重点关注韵律、节奏、意象 *)
      List.filter
        (function RhymeHarmony | Rhythm | Imagery | Elegance -> true | _ -> false)
        dimensions
  | Some "五言律诗" | Some "七言律诗" ->
      (* 律诗重点关注对仗、韵律、形式美 *)
      List.filter
        (function RhymeHarmony | Parallelism | FormBeauty | TonalBalance -> true | _ -> false)
        dimensions
  | Some "自由体诗" ->
      (* 自由体重点关注意象、内容深度 *)
      List.filter (function Imagery | ContentDepth | Elegance -> true | _ -> false) dimensions
...
let normalize_verses verses =
  verses |> List.map clean_verse_text
