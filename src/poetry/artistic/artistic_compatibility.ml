...
module V1_API = struct
  ...
  let simple_evaluate poem_text =
    ...
    let avg_length = if line_count = 0 then 0 
                    else List.fold_left (+) 0 (List.map String.length lines) / line_count in
    ...
    let grade = if final_score >= 0.8 then "优秀" 
    ...
    
    {
      ...
      grade;
      comments = [
        Printf.sprintf "共%d行，平均每行%d字" line_count avg_length;
        "使用V1.0兼容接口评价";
      ];
    }
...
module V2_API = struct
  ...
  let modular_evaluate poem_text =
    ...
    let overall_score = List.fold_left (fun acc (_, score) -> acc +. score) 0.0 dimension_scores 
...
let find_new_function_name old_name =
  try Some (List.assoc old_name function_name_mapping)
  with Not_found -> None
...
let convert_error_to_legacy error_msg =
  if String.contains error_msg (String.get "parse" 0) then
    raise (Legacy_Parse_Error error_msg)
  else
    raise (Legacy_Evaluation_Error error_msg)

(** {1 配置管理} *)

(** 设置兼容性模式 *)
let set_compatibility_mode mode =
  compatibility_mode_ref := mode
...
(** API版本转为字符串 *)
let api_version_to_string = function
  | V1_0 -> "1.0"
  | V2_0 -> "2.0"
  | V3_0 -> "3.0"
...
let check_deprecated_usage code_text =
  ...
  List.filter (fun func_name -> 
    String.contains code_text (String.get func_name 0)
  ) deprecated_functions

(** 建议迁移步骤 *)
let suggest_migration_steps deprecated_functions =
  List.map (fun old_func ->
...
let test_v1_compatibility () =
  ...
  with e -> 
    Some ("V1.0兼容性测试失败", (0.0, [], [Printexc.to_string e]))
