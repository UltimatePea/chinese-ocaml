...
let get_cached key =
  try
    ...
    (* 更新访问统计 *)
    let updated_entry =
      { entry with access_count = entry.access_count + 1; last_access = Unix.time () }
    in
...
let rec set_cached key value =
  ...
  let entry = { value; created_at = current_time; access_count = 1; last_access = current_time } in
...
and apply_cache_policy () =
  ...
  | TTL ttl_seconds -> evict_expired ttl_seconds
  | NoEviction -> ()

and evict_lru count =
  let entries = Hashtbl.fold (fun key entry acc -> (key, entry) :: acc) cache_metadata [] in
  let sorted_entries =
    List.sort (fun (_, e1) (_, e2) -> compare e1.last_access e2.last_access) entries
  in
...
let get_cache_stats () =
  ...
  let ages =
    Hashtbl.fold (fun _ entry acc -> (current_time -. entry.created_at) :: acc) cache_metadata []
  in

  let avg_age =
    if ages = [] then 0.0 else List.fold_left ( +. ) 0.0 ages /. float_of_int (List.length ages)
  in
...
let get_hot_entries limit =
  let entries =
    Hashtbl.fold (fun key entry acc -> (key, entry.access_count) :: acc) cache_metadata []
  in
...
let optimize_cache_size () =
  ...
  let avg_access =
    Hashtbl.fold (fun _ entry acc -> acc +. float_of_int entry.access_count) cache_metadata 0.0
    /. float_of_int current_size
  in

  (* 根据访问模式调整策略 *)
  if avg_access < 2.0 then set_cache_policy (LRU (current_size / 2))
  else if avg_access > 10.0 then set_cache_policy (LRU (current_size + 100))
...
let export_cache () =
  ...
  List.map (fun (key, value) -> key ^ ":" ^ value) entries |> String.concat "\n"
