...
let generate_rhyme_feedback rhyme_score rhyme_chars rhyme_diversity =
  let rhyme_details =
    Printf.sprintf "韵脚字符: [%s], 韵律多样性: %.1f%%" 
      (String.concat "; " rhyme_chars) (rhyme_diversity *. 100.0)
  in
...
let generate_form_beauty_feedback verse_count avg_length structural_score =
  ...
  let suggestions = 
    if structural_score >= 0.9 then
      [base_feedback ^ "，形式结构优美"]
    else if structural_score >= 0.7 then
      [base_feedback ^ "，形式较为协调"]
    else
      [base_feedback ^ "，建议调整诗句结构以增强形式美感"]
  in
...
let generate_parallelism_feedback left_verse right_verse score =
  let suggestions = 
    if score >= 0.8 then
      [Printf.sprintf "对仗工整，「%s」与「%s」相对和谐" left_verse right_verse]
    else if score >= 0.6 then
      [Printf.sprintf "对仗基本合理，可进一步优化「%s」与「%s」的对称性" left_verse right_verse]
    else
      [Printf.sprintf "建议改善「%s」与「%s」的对仗关系" left_verse right_verse]
  in
...
let generate_imagery_feedback _verse found_keywords keyword_count =
  ...
  let suggestions = 
    if keyword_count >= 3 then
      [Printf.sprintf "意象丰富，包含「%s」等意象词汇，营造了良好的诗意氛围" keywords_str]
    else if keyword_count >= 1 then
      [Printf.sprintf "包含「%s」等意象，可适当增加更多意象词汇增强表现力" keywords_str]
    else
      ["建议增加具体的意象词汇，如山水、花鸟、季节等，以增强诗意表达"]
  in
...
let analyze_strengths dimension_scores =
  ...
  let strength_descriptions = List.map (fun score ->
  ...
  let limited_strengths = 
    if List.length strength_descriptions > ReportConfig.max_strengths_count 
    then list_take ReportConfig.max_strengths_count strength_descriptions
...
let analyze_weaknesses dimension_scores =
  ...
  let weakness_descriptions = List.map (fun score ->
    match score.dimension with
  ...
  let limited_weaknesses = 
    if List.length weakness_descriptions > ReportConfig.max_weaknesses_count 
    then list_take ReportConfig.max_weaknesses_count weakness_descriptions
...
let generate_improvement_suggestions dimension_scores context =
  ...
  (* 根据诗词形式添加特定建议 *)
  let form_specific_suggestions = match context.poem_type with
    | Some "五言绝句" | Some "七言绝句" ->
        ["绝句应注重意境营造和韵律音响效果"]
    | Some "五言律诗" | Some "七言律诗" ->
        ["律诗应重视对仗工整和平仄协调"]
  ...
  if List.length unique_suggestions > ReportConfig.max_suggestions_count 
  then list_take ReportConfig.max_suggestions_count unique_suggestions
...
let create_evaluation_metadata context =
  ...
  let form_metadata = match context.poem_type with
    | Some form -> [("detected_form", form)]
    | None -> []
  in
  
  let context_metadata = match context.author with
    | Some author -> [("author", author)]
    | None -> []
  in
...
let generate_comprehensive_report dimension_scores context overall_score =
  ...
  let evaluation_metadata = 
    if ReportConfig.include_metadata 
    then create_evaluation_metadata context
    else []
  in
