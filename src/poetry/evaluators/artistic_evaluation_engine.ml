...
(** 综合艺术性评价 *)
let comprehensive_artistic_evaluation (verses : string list) (_state : engine_state) : artistic_evaluation =
  match verses with
  | [] -> raise (ArtisticEngineError "Empty verse list provided")
  | _ -> evaluate_multiple_verses verses

(** 单维度评价 *)
let evaluate_single_dimension (dim : evaluation_dimension) (ctx : evaluation_context) (_state : engine_state) : dimension_score option =
  try
    let evaluator = List.find (fun eval_module ->
      let module E = (val eval_module : EVALUATOR) in
      E.dimension = dim
    ) available_evaluators in
...
(** 生成改进指导 *)
let generate_improvement_guidance (evaluation : artistic_evaluation) (_state : engine_state) : string list =
  evaluation.improvement_suggestions @ [
    "基于当前评价结果，建议重点关注评分较低的维度";
    "可参考古典诗词名作进行对比学习";
  ]
...
let format_evaluation_result (evaluation : artistic_evaluation) : string =
  ...
  Buffer.add_string buf (Printf.sprintf "艺术水平: %s\n" (match evaluation.artistic_level with
    | `Master -> "大师级"
    | `Advanced -> "高级"
    | `Intermediate -> "中级"
    | `Beginner -> "初级"));
  Buffer.add_string buf (Printf.sprintf "质量等级: %s\n" (match evaluation.quality_grade with
    | `Excellent -> "极佳"
    | `Good -> "良好"
    | `Fair -> "尚可"
    | `Poor -> "较差"));
...
let export_evaluation_json (evaluation : artistic_evaluation) : string =
  ...
  Buffer.add_string json_buf (Printf.sprintf "  \"overall_score\": %.2f,\n" evaluation.overall_score);
  Buffer.add_string json_buf "  \"dimension_scores\": [\n";
  let scores_json = List.map (fun score ->
    Printf.sprintf "    {\"dimension\": \"%s\", \"score\": %.2f}" 
      (match score.dimension with
       ...
       | _ -> "other")
      score.score
  ) evaluation.dimension_scores in
  Buffer.add_string json_buf (String.concat ",\n" scores_json);
  Buffer.add_string json_buf "\n  ],\n";
  Buffer.add_string json_buf (Printf.sprintf "  \"artistic_level\": \"%s\",\n" (match evaluation.artistic_level with
    | `Master -> "master"
    | `Advanced -> "advanced"
    | `Intermediate -> "intermediate"
    | `Beginner -> "beginner"));
  Buffer.add_string json_buf (Printf.sprintf "  \"quality_grade\": \"%s\"\n" (match evaluation.quality_grade with
    | `Excellent -> "excellent"
    | `Good -> "good"
    | `Fair -> "fair"
    | `Poor -> "poor"));
