          ...
          let rec find_last_char pos =
            if pos <= 0 then None
              ...
              if byte < 0x80 then (* ASCII *)
                if pos = len - 1 then Some (String.sub trimmed pos 1)
                ...
                in
                if pos + char_len = len then
                  Some (String.sub trimmed pos char_len)
                else find_last_char (pos - 1)
          in
      ...
      if rhyme_count >= 2 then (
        ...
        let unique_chars = 
          let rec unique acc = function
            | [] -> List.rev acc
            | h :: t -> 
                if List.mem h acc then unique acc t
                else unique (h :: acc) t
          in
          unique [] rhyme_chars
        in
        ...
        let rhyme_details = Printf.sprintf "韵脚字符: [%s], 韵律多样性: %.1f%%" 
          (String.concat "; " rhyme_chars) (rhyme_diversity *. 100.0) in
