...
let analyze_content_depth (lines: string list) : float * string list =
  ...
  if total_chars > 20 then (
    ...
    suggestions := "建议增加内容的深度和复杂性" :: !suggestions
  );
  ...
  if line_count >= 4 then (
    ...
    suggestions := "可以考虑扩展诗歌的结构" :: !suggestions
  );
...
let analyze_sound_harmony (lines: string list) : float * string list =
  ...
  if List.length lines >= 2 then (
    ...
    let matching_lines = List.filter (fun (_, result) -> result.Poetry_rhyme_engine.is_match) rhyme_engine in
    let rhyme_ratio = float_of_int (List.length matching_lines) /. float_of_int (List.length lines - 1) in
    
    score := !score +. (rhyme_ratio *. 0.4);
    if rhyme_ratio > 0.5 then
      suggestions := "韵律和谐，音韵效果良好" :: !suggestions
    else
      suggestions := "建议改善韵脚以增强音韵美感" :: !suggestions
  );
...
let analyze_mood_creation (lines: string list) : mood_analysis =
  ...
  let nature_count = List.fold_left (fun acc word -> 
    if String.contains text (String.get word 0) then acc + 1 else acc
  ) 0 nature_words in
...
let comprehensive_artistic_evaluation (lines: string list) : artistic_evaluation =
  ...
  List.iter (fun (dim, score) ->
    let dim_name = match dim with
      | Content -> "内容深度" | Form -> "形式美感" | Sound -> "音韵和谐"
      | Context -> "意境营造" | Emotion -> "情感表达" | Innovation -> "创新修辞" in
    
    if score >= 0.7 then
      strengths := (dim_name ^ "表现出色") :: !strengths
    else if score < 0.4 then
      weaknesses := (dim_name ^ "有待改进") :: !strengths
  ) dimension_scores;
  ...
  List.iter (fun (dim, score) ->
    if score < 0.5 then
      let suggestion = match dim with
        ...
        | Innovation -> "尝试使用一些修辞手法，增加诗歌的表现力" in
