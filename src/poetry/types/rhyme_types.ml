...
let create_empty_database () =
  { groups = []; version = "2.0"; last_updated = "2025-07-27"; sources = [ "unified_system" ] }

(** 创建韵组数据容器 *)
let create_rhyme_group_data group items metadata = { group; items; metadata }

(** 获取韵组中的所有字符 *)
let get_characters_from_group group_data = List.map (fun item -> item.character) group_data.items

(** 过滤韵律数据项 *)
let filter_by_category category items = List.filter (fun item -> item.category = category) items

let filter_by_group group items = List.filter (fun item -> item.group = group) items
...
(** 验证函数 *)
let validate_rhyme_data_item item = String.length item.character > 0 && item.source <> ""
