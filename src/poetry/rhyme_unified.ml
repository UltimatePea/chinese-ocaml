...
module Data = struct
  ...
  let get_rhyme_group_category group_name =
    ...
    with
    | Not_found -> Poetry_core.Poetry_types.PingSheng

  let clear_cache () =
    Poetry_core.Json_core.Cache.clear_cache ()

  let get_cache_stats () =
    Poetry_core.Json_core.Cache.get_cache_stats ()
end
...
module Json = struct
  let parse_rhyme_json json_string =
    try
      Poetry_core.Json_core.Parser.parse_rhyme_json json_string
    with
    | Poetry_core.Json_core.Json_parse_error msg -> raise (Json_parse_error msg)
  ...
  let load_from_file ?filename () =
    try
      ...
      | None ->
          let content = Poetry_core.Json_core.Io.safe_read_file 
            Poetry_core.Json_core.Io.default_rhyme_data_path in
          parse_rhyme_json content
    with
    | Poetry_core.Json_core.Json_parse_error msg -> raise (Json_parse_error msg)
    | Sys_error msg -> raise (Json_parse_error ("File error: " ^ msg))

  let clean_json_string json_string =
    Poetry_core.Json_core.Parser.clean_json_string json_string
end
      ...
      let rec find_in_groups = function
        ...
        | (group_name, group_data) :: rest ->
            if List.mem char group_data.characters then
              (* 构造 rhyme_data_item *)
              let category = match Poetry_core.Json_core.string_to_rhyme_category group_data.category with
                | Some cat -> cat  
                | None -> Poetry_core.Poetry_types.PingSheng in
              let group = match Poetry_core.Json_core.string_to_rhyme_group group_name with
                | Some grp -> grp
                | None -> Poetry_core.Poetry_types.UnknownRhyme in
              Some {
...
module Utils = struct
  let string_to_rhyme_category s =
    Poetry_core.Json_core.string_to_rhyme_category s

  let string_to_rhyme_group s =  
    Poetry_core.Json_core.string_to_rhyme_group s

  let get_data_statistics () =
    let groups = Data.get_all_rhyme_groups () in
    let num_groups = List.length groups in
    let total_chars = groups 
      |> List.fold_left (fun acc (_, group_data) -> 
          acc + List.length group_data.characters) 0 in
