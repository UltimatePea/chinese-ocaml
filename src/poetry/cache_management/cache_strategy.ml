...
let get_strategy_for_key (key : string) : cache_strategy =
  let rec find_matching_strategy patterns =
    ...
    | (pattern, strategy) :: rest ->
        if Cache_utils.matches_pattern pattern key then strategy
        else find_matching_strategy rest
  in
  let patterns = Hashtbl.fold (fun pattern strategy acc ->
    (pattern, strategy) :: acc
  ) cache_state.strategies [] in
...
let find_victim_for_eviction (strategy : cache_strategy) : string option =
  ...
  | [] -> None
  | _ ->
      let victim = match strategy with
      | LRU ->
          (* 找到最长时间未访问的条目 *)
          List.fold_left (fun (min_key, min_entry) (key, entry) ->
            if entry.metadata.last_accessed < min_entry.metadata.last_accessed then
              (key, entry)
      ...
      | LFU ->
          (* 找到访问次数最少的条目 *)
          List.fold_left (fun (min_key, min_entry) (key, entry) ->
            if entry.metadata.access_count < min_entry.metadata.access_count then
              (key, entry)
            else (min_key, min_entry)
          ) (List.hd entries) entries
      | FIFO ->
          (* 找到最早创建的条目 *)
          List.fold_left (fun (min_key, min_entry) (key, entry) ->
            if entry.metadata.created_time < min_entry.metadata.created_time then
              (key, entry)
            else (min_key, min_entry)
          ) (List.hd entries) entries
      | TTL _ ->
          (* 找到第一个过期的条目 *)
          let expired = List.find_opt (fun (_, entry) ->
            Cache_utils.is_entry_expired entry
          ) entries in
          (match expired with
          | Some (key, entry) -> (key, entry)
          | None -> List.hd entries)  (* 如果没有过期的，选择第一个 *)
      | Custom _ ->
          (* 自定义策略，选择第一个满足条件的 *)
          let matching = List.find_opt (fun (_, entry) ->
            should_evict_entry entry strategy
          ) entries in
          (match matching with
          | Some (key, entry) -> (key, entry)
