...
let list_keys_by_pattern (pattern : string) : string list =
  Hashtbl.fold
    (fun key _ acc -> if Cache_utils.matches_pattern pattern key then key :: acc else acc)
...
let list_keys_by_tags (tags : string list) : string list =
  Hashtbl.fold
    (fun key entry acc ->
      if Cache_utils.has_matching_tags entry.metadata.tags tags then key :: acc else acc)
    cache_state.data_map []
