...
let analyze_text_artistic_elements (text : string) : (word_category * string list) list query_result =
  try
    ...
    let imagery_words = List.filter (fun word -> 
      List.mem word Artistic_legacy_compat.default_imagery_words) text_words in
    let elegant_words = List.filter (fun word -> 
      List.mem word Artistic_legacy_compat.default_elegant_words) text_words in
...
let get_nature_imagery () : string list query_result =
  ...
  | NotFound -> 
      ...
      ) Artistic_legacy_compat.default_imagery_words in
      Found nature_words
