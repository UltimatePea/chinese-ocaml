  ...
  if Hashtbl.mem rhyme_data_table group_type then
    Printf.printf "警告: 韵组 %s 重复注册，将覆盖原数据\n" 
      (match group_type with 
       ...
       | WangRhyme -> "王韵" | QuRhyme -> "曲韵" | YuRhyme -> "鱼韵"
       | HuaRhyme -> "花韵" | FengRhyme -> "风韵" | YueRhyme -> "月韵"
       | JiangRhyme -> "江韵" | HuiRhyme -> "会韵" | XueRhyme -> "雪韵"
       | UnknownRhyme -> "未知韵")
  else
    registration_order := group_type :: !registration_order;
...
let get_rhyme_data_by_group group_type =
  try Some (Hashtbl.find rhyme_data_table group_type)
  with Not_found -> None
...
(** 检查韵组是否已注册
    @param group_type 韵组类型
    @return 是否已注册 *)
let is_registered group_type =
  Hashtbl.mem rhyme_data_table group_type

(** 获取已注册韵组的数量 *)
let get_registered_count () =
  Hashtbl.length rhyme_data_table

(** 获取所有已注册的韵组类型列表 *)
let get_registered_groups () =
  List.rev !registration_order
...
let get_rhyme_data_by_group_safe group_type =
  ...
  | Some data -> data
  | None -> 
    { group_name = UnknownRhyme; 
      group_description = "未知韵组"; 
      entries = []; 
      example_poems = [] }
...
let validate_registry () =
  ...
  (* 检查是否有空的韵组数据 *)
  Hashtbl.iter (fun group_type data ->
    if List.length data.entries = 0 then
      add_issue (Printf.sprintf "韵组 %s 没有字符数据" 
        (match group_type with 
         | AnRhyme -> "安韵" | SiRhyme -> "思韵" | _ -> "其他"))
  ) rhyme_data_table;
  
  (* 检查注册顺序一致性 *)
  let registered_in_table = Hashtbl.length rhyme_data_table in
  let registered_in_order = List.length !registration_order in
  if registered_in_table <> registered_in_order then
    add_issue "注册顺序与实际注册数据不一致";
