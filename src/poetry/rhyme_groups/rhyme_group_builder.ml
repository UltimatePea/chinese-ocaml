...
let validate_config config = 
  let errors = [] in
  let errors = 
    if config.ping_sheng_chars = [] && config.ze_sheng_chars = [] then
      "韵组不能同时缺少平声和仄声字符" :: errors
    else errors
  in
  let errors =
    if String.trim config.description = "" then
      "韵组描述不能为空" :: errors  
    else errors
  in
  match errors with
...
(** 从多个配置批量构建韵组
    @param configs 配置列表
    @return 韵组数据列表 *)
let build_multiple_groups configs =
  List.map build_from_config configs
