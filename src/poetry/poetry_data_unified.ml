  ...
  let parse_simple_json content =
    try
      ...
      List.iter (fun line ->
        ...
        (* 处理字符数组 *)
        if String.contains trimmed '[' && Str.string_match (Str.regexp ".*characters.*") line 0 then
        ...
        (* 处理字符数据 *)
          ...
          if char_data <> "" then
            state.current_chars <- char_data :: state.current_chars
      ) lines;
      
      finalize_group state;
      List.rev state.result
    with
    | exn -> raise (DataError (ParseError (Printexc.to_string exn)))
end
...
module SourceRegistry = struct
  ...
  let get_by_name name =
    try Some (List.find (fun e -> e.name = name) !registered_sources)
    with Not_found -> None
  ...
  let load_from_json_file filename =
    let full_path = 
      if Filename.is_relative filename then 
        FileUtils.resolve_relative_path filename
      else filename in
...
module Database = struct
  ...
  let lookup_char database char =
    try Some (Hashtbl.find database.char_to_rhyme char)
    with Not_found -> None
...
let load_data_safe source_name =
  try load_data source_name
  with _ -> Fallback.use_fallback_data ()
...
let register_standard_sources () =
  register_source "rhyme_data" 
    (JsonFile "data/poetry/rhyme_groups/rhyme_data.json") 
    ~priority:100 "主要韵律数据";
  register_source "tone_data"
    (JsonFile "data/poetry/tone_data.json")
    ~priority:90 "声调数据";
  register_source "expanded_rhyme" 
    (JsonFile "data/poetry/expanded_rhyme_data.json")
    ~priority:80 "扩展韵律数据"
