(** 骆言诗词统一数据加载模块 - Poetry Module Integration Optimization
 *
...
let find_json_section content category_name =
  ...
  try
    ...
    let rec find_matching_bracket pos depth =
      if pos >= String.length content then raise (Failure "Unmatched bracket")
      else
...
let default_imagery_keywords =
  natural_imagery_keywords @ emotional_imagery_keywords @ cultural_imagery_keywords
...
let imagery_keywords =
  lazy
    (let data_path = "data/poetry/imagery_keywords.json" in
     ...
     if loaded_words = [] then default_imagery_keywords else loaded_words)
...
let elegant_words =
  lazy
    (let data_path = "data/poetry/elegant_words.json" in
     let loaded_words = load_words_from_json_file data_path in
     if loaded_words = [] then default_elegant_words else loaded_words)
...
module Poetry_data_loader_compat = struct
  ...
  let elegant_words = elegant_words
end

(** Artistic_data_loader兼容接口 *)
module Artistic_data_loader_compat = struct
  let read_file_safely = read_file_safely
  let find_json_section = find_json_section
  let extract_words_from_category = extract_words_from_category
  let load_words_from_json_file = load_words_from_json_file
  let default_imagery_keywords = default_imagery_keywords
  let default_elegant_words = default_elegant_words
  let imagery_keywords = imagery_keywords
  let elegant_words = elegant_words
  let get_imagery_keywords = get_imagery_keywords
  let get_elegant_words = get_elegant_words
end
