...
let calculate_artistic_score (text : string) : (evaluation_dimension * float) list query_result =
  ...
  let adjusted_scores = List.map (fun (dim, score) -> (dim, score *. length_factor)) base_scores in

  Found adjusted_scores

let compare_artistic_quality (text1 : string) (text2 : string) :
    (evaluation_dimension * float * float) list query_result =
  match (calculate_artistic_score text1, calculate_artistic_score text2) with
  | Found scores1, Found scores2 ->
      let comparison =
        List.map2
          (fun (dim1, score1) (dim2, score2) ->
            assert (dim1 = dim2);
            (dim1, score1, score2))
          scores1 scores2
      in
...
let suggest_improvements (_ : string) (focus_dimension : evaluation_dimension) :
    string list query_result =
  let suggestions =
    match focus_dimension with
...
let assess_word_elegance (word : string) : float query_result =
  ...
  if List.mem word [ "之"; "者"; "也"; "矣"; "乎"; "哉"; "焉"; "夫"; "其"; "若" ] then Found 0.8
  else Found 0.1
