...
let evaluate_rhyme_harmony verse =
  ...
  let rhyme_score = 
    if List.length chars > 0 then 0.8 else 0.0
  in
  rhyme_score
...
let evaluate_imagery verse =
  ...
  let imagery_count = List.fold_left (fun acc keyword ->
    if String.contains verse (String.get keyword 0) then acc + 1 else acc
  ) 0 imagery_keywords in
...
let evaluate_elegance verse =
  ...
  let elegance_count = List.fold_left (fun acc char ->
...
let comprehensive_artistic_evaluation poem =
  ...
  {
    overall_score = (avg_score rhyme_scores +. avg_score tonal_scores +. 
                    avg_score imagery_scores +. avg_score rhythm_scores +. 
                    avg_score elegance_scores) /. 5.0;
    dimension_scores = [
...
let evaluate_qiyan_jueju text =
  ...
  let char_score = float_of_int (List.length seven_char_lines) /. float_of_int (List.length lines) in
...
let analyze_mood poem =
  ...
  let detected_moods = List.filter_map (fun (name, keywords) ->
    if detect_mood (name, keywords) then Some name else None
  ) mood_keywords in
...
let analyze_rhetoric poem =
  ...
  let detected = List.filter_map (fun (name, patterns) ->
    if detect_rhetoric (name, patterns) then Some (name, "示例片段") else None
  ) rhetoric_patterns in
...
let provide_artistic_guidance poem evaluation =
  ...
  let specific_advice = match evaluation.overall_score with
...
let evaluate_poetic_soul poem =
  let cultural_depth = 
    ...
    List.fold_left (fun acc keyword ->
      if String.contains poem (String.get keyword 0) then acc + 1 else acc
    ) 0 cultural_keywords
  in
