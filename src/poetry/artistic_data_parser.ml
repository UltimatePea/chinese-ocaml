  ...
  try
    ...
    List.map
      (fun word_json ->
        ...
        let synonyms =
          try word_json |> member "synonyms" |> to_list |> List.map to_string with _ -> []
        in
        let contexts =
          try word_json |> member "contexts" |> to_list |> List.map to_string with _ -> []
        in
        let examples =
          try word_json |> member "examples" |> to_list |> List.map to_string with _ -> []
        in
...
let parse_evaluation_standards_from_json (json : Yojson.Basic.t) :
  ...
  try
    ...
    List.map
      (fun (dim_str, standards_json) ->
        let dimension = evaluation_dimension_from_string dim_str in
        let standards =
          standards_json |> to_list
          |> List.map (fun std_json ->
                 ...
                 let criteria =
                   try
                     std_json |> member "criteria" |> to_list
                     |> List.map (fun c ->
