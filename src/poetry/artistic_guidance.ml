...
let enhanced_comprehensive_artistic_evaluation verse =
  ...
  let detailed_suggestions = 
    ...
    (* 根据具体得分给出详细建议 *)
    if rhyme_score < 0.4 then 
      basic_suggestions := "韵律和谐度较低，建议选择协调的韵母组合" :: !basic_suggestions
    else if rhyme_score < 0.7 then
      basic_suggestions := "韵律基本和谐，可进一步优化音韵搭配" :: !basic_suggestions;

    if tone_score < 0.4 then
      basic_suggestions := "声调平衡需要改善，注意平仄相间的节奏" :: !basic_suggestions
    else if tone_score < 0.7 then  
      basic_suggestions := "声调搭配尚可，可追求更丰富的音调变化" :: !basic_suggestions;

    if imagery_score < 0.4 then
      basic_suggestions := "意象表达不够深刻，建议融入更多诗意元素" :: !basic_suggestions
    else if imagery_score < 0.7 then
      basic_suggestions := "意象有一定深度，可进一步提升意境层次" :: !basic_suggestions;

    if rhythm_score < 0.4 then
      basic_suggestions := "节奏感需要改善，注意字数和声调的协调" :: !basic_suggestions
    else if rhythm_score < 0.7 then  
      basic_suggestions := "节奏基本流畅，可追求更优美的韵律节拍" :: !basic_suggestions;

    if elegance_score < 0.4 then
      basic_suggestions := "用词雅致度有待提升，建议采用更典雅的表达" :: !basic_suggestions
    else if elegance_score < 0.7 then
      basic_suggestions := "用词较为雅致，可进一步提升文学品味" :: !basic_suggestions;

    if !basic_suggestions = [] then
      ["诗句各方面表现优秀，已达到较高的艺术水准"]
    else  
      List.rev !basic_suggestions
  in
...
let poetic_critique verse poetry_type =
  ...
  let type_specific_suggestions = match poetry_type with
    | WuYanLuShi -> [
        "五言律诗要求平仄严格，颔联颈联必须对仗";
        "意境要深远含蓄，体现古典诗词的意蕴";
        "用词要典雅精炼，避免俗语白话";
  ...
  { base_evaluation with 
    suggestions = base_evaluation.suggestions @ type_specific_suggestions }
...
let poetic_aesthetics_guidance verse poetry_type =
  ...
  let aesthetic_principles = match poetry_type with
    | WuYanLuShi -> [
        "五言律诗追求'含蓄蕴藉'的美学效果";
        "通过精炼的五言表达深刻的思想情感";
        "格律严谨是基础，意境深远是目标";
  ...
  { evaluation with 
    suggestions = evaluation.suggestions @ aesthetic_principles }