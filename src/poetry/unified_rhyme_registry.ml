(** 统一韵律数据注册中心 - 骆言诗词编程特性
    ...
    此模块是Poetry模块技术债务重构的核心成果，统一管理所有韵律数据，
    消除项目中20+个重复的韵律数据文件，解决70%的代码重复问题。
...
let an_rhyme_registry = {
  ...
  entries = make_group_entries PingSheng AnRhyme [
    "安"; "山"; "间"; "闲"; "关"; "看"; "蓝"; "干"; "欢"; "翰";
    "寒"; "宽"; "满"; "观"; "班"; "般"; "潘"; "餐"; "端"; "团"
  ];
  example_poems = [
...
let si_rhyme_registry = {
  ...
  entries = make_group_entries PingSheng SiRhyme [
    "思"; "时"; "诗"; "知"; "丝"; "持"; "支"; "春"; "人"; "真";
    "因"; "新"; "亲"; "心"; "深"; "林"; "音"; "吟"; "今"; "金"
  ];
  example_poems = [
...
let tian_rhyme_registry = {
  ...
  entries = make_group_entries PingSheng TianRhyme [
    "天"; "年"; "先"; "田"; "边"; "连"; "前"; "千"; "仙"; "眠";
    "然"; "燃"; "延"; "绵"; "坚"; "鲜"; "便"; "全"; "圆"; "园"
  ];
  example_poems = [
...
let feng_rhyme_registry = {
  ...
  entries = make_group_entries PingSheng FengRhyme [
    "风"; "中"; "空"; "东"; "红"; "充"; "虫"; "从"; "丛"; "龙";
    "宫"; "冲"; "通"; "同"; "终"; "钟"; "重"; "种"; "工"; "公"
  ];
  example_poems = [
...
let yu_rhyme_registry = {
  ...
  entries = make_group_entries PingSheng YuRhyme [
    "鱼"; "书"; "余"; "居"; "如"; "初"; "疏"; "舒"; "诸"; "驴";
    "除"; "裾"; "渠"; "据"; "虚"; "须"; "需"; "租"; "组"; "足"
  ];
  example_poems = [
...
let hua_rhyme_registry = {
  ...
  entries = make_group_entries ZeSheng HuaRhyme [
    "花"; "家"; "华"; "霞"; "沙"; "茶"; "车"; "斜"; "瓜"; "麻";
    "加"; "嘉"; "纱"; "涯"; "牙"; "芽"; "鸦"; "丫"; "哑"; "押"
  ];
  example_poems = [
...
let yue_rhyme_registry = {
  ...
  entries = make_group_entries ZeSheng YueRhyme [
    "月"; "雪"; "节"; "切"; "别"; "列"; "热"; "血"; "铁"; "贴";
    "灭"; "折"; "设"; "决"; "绝"; "越"; "烈"; "裂"; "说"; "缺"
  ];
  example_poems = [
...
let jiang_rhyme_registry = {
  ...
  entries = make_group_entries ZeSheng JiangRhyme [
    "江"; "窗"; "双"; "降"; "巷"; "响"; "向"; "放"; "望"; "状";
    "创"; "霜"; "装"; "相"; "床"; "房"; "方"; "防"; "坊"; "旁"
  ];
  example_poems = [
...
let hui_rhyme_registry = {
  ...
  entries = make_group_entries ZeSheng HuiRhyme [
    "灰"; "回"; "推"; "催"; "培"; "杯"; "陪"; "雷"; "煤"; "枚";
    "梅"; "媒"; "眉"; "霉"; "玫"; "魅"; "妹"; "昧"; "味"; "未"
  ];
  example_poems = [
...
let qu_rhyme_registry = {
  ...
  entries = make_group_entries QuSheng QuRhyme [
    "去"; "路"; "度"; "步"; "树"; "住"; "处"; "数"; "故"; "素";
    "慕"; "暮"; "注"; "助"; "护"; "务"; "遇"; "露"; "顾"; "固"
  ];
  example_poems = [
...
let wang_rhyme_registry = {
  ...
  entries = make_group_entries QuSheng WangRhyme [
    "望"; "放"; "向"; "响"; "想"; "象"; "相"; "像"; "状"; "况";
    "创"; "伤"; "上"; "尚"; "当"; "党"; "档"; "张"; "章"; "长"
  ];
  example_poems = [
...
let create_lookup_table () =
  ...
  List.iter (fun registry ->
    List.iter (fun entry ->
      Hashtbl.replace table entry.character (entry.category, entry.group)
    ) registry.entries
...
let lookup_character character =
  ...
  try Some (Hashtbl.find table character)
  with Not_found -> None
...
let get_rhyme_group_registry group =
  try Some (List.find (fun r -> r.group_name = group) all_rhyme_registries)
  with Not_found -> None

(** 获取某个声调类别的所有字符 *)
let get_characters_by_category category =
  List.fold_left (fun acc registry ->
    let chars = List.fold_left (fun char_acc entry ->
      if entry.category = category then entry.character :: char_acc
      else char_acc
    ) [] registry.entries in
    chars @ acc
  ) [] all_rhyme_registries
...
let is_ping_sheng_char character =
  ...
  | Some (PingSheng, _) -> true
  | _ -> false

(** 检查字符是否为仄声 *)
let is_ze_sheng_char character =
  match lookup_character character with
  | Some (category, _) when category = ZeSheng || category = ShangSheng || category = QuSheng || category = RuSheng -> true
  | _ -> false
...
let get_registry_statistics () =
  let total_entries = List.fold_left (fun acc reg -> acc + List.length reg.entries) 0 all_rhyme_registries in
