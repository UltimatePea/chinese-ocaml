...
(** 检测两字符是否押韵 * 整合了原 rhyme_matching.ml 和 rhyme_validation.ml 的功能 *)
...
(** 验证诗句的韵律一致性 * 整合了原 rhyme_pattern.ml 的功能 *)
let validate_poem_rhyme (lines : string list) : (int * rhyme_match_result) list =
  let line_endings =
    List.mapi
      (fun i line ->
        if String.length line > 0 then (i, String.sub line (String.length line - 1) 1) else (i, ""))
      lines
  in
  ...
  List.iter
    (fun (i, char1) ->
      if i < List.length line_endings - 1 then
        ...
        if char1 <> "" && char2 <> "" then results := (i, check_rhyme_match char1 char2) :: !results)
    line_endings;
  List.rev !results
...
(** 检测诗词符合哪种韵律模式 * 整合了原 rhyme_scoring.ml 的评分功能 *)
    ...
    (fun pattern ->
      ...
      for i = 0 to total - 1 do
        ...
        | Some actual_group ->
            ...
            if actual_group = expected_group then score := !score +. 1.0
            else if pattern.allow_variations then score := !score +. 0.5
        | None -> ()
      done;
...
let suggest_rhyme_improvements (lines : string list) : string list =
  ...
  List.iter
    (fun (line_idx, match_result) ->
      if not match_result.is_match then
        let suggestion =
          Printf.sprintf "第%d行和第%d行不押韵: %s" (line_idx + 1) (line_idx + 2) match_result.explanation
        in
  ...
  let best_pattern_score =
    List.fold_left (fun acc (_, score) -> max acc score) 0.0 pattern_results
  in
  if best_pattern_score < 0.8 then suggestions := "建议调整韵脚以符合常见诗词格律" :: !suggestions;
