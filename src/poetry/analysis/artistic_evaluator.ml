...
module RhymeEvaluator : EVALUATOR = struct
  ...
  let evaluate context =
    ...
    let score =
      if has_rhyme_ending then consistency_score else 0.1
      (* 无韵脚的情况 *)
    in
    ...
    let details =
      ...
      | Some ending ->
          Some
            (Printf.sprintf "韵脚: %s, 一致性: %s" ending
    ...
    { dimension; score; max_score = 1.0; details; confidence = 0.9; suggestions }
...
module ToneEvaluator : EVALUATOR = struct
  ...
  let evaluate context =
    ...
    let details =
      Some (Printf.sprintf "声调模式长度: %d, 平仄交替度: %.2f" (List.length pattern) tone_score)
    in

    let suggestions = if tone_score < 0.5 then [ "建议增加平仄变化" ] else [] in

    { dimension; score = tone_score; max_score = 1.0; details; confidence = 0.7; suggestions }

  let is_applicable context = List.length context.rhythm_analysis.rhyme_pattern > 0
end
...
module RhythmEvaluator : EVALUATOR = struct
  ...
  let evaluate context =
    ...
    { dimension; score = length_score; max_score = 1.0; details; confidence = 0.8; suggestions }
...
module ConsistencyEvaluator : EVALUATOR = struct
  ...
  let evaluate context =
    ...
    | Some multi ->
        ...
        let details =
          Some
            (Printf.sprintf "一致性: %.2f, 整体质量: %.2f, 诗句数: %d" consistency_score quality_score
               (List.length multi.verses))
        in
  ...
  let registry =
    {
      ...
      weights = [ (Rhyme, 0.25); (Tone, 0.15); (Rhythm, 0.15); (Overall, 0.20) ];
    }
  in
...
let register_evaluator dimension evaluator_module evaluator_state =
  let updated_evaluators = (dimension, evaluator_module) :: evaluator_state.registry.evaluators in
  let module E = (val evaluator_module : EVALUATOR) in
  let updated_weights = (dimension, E.weight) :: evaluator_state.registry.weights in
  let updated_registry = { evaluators = updated_evaluators; weights = updated_weights } in
...
let create_evaluation_context verse verses rhythm_analyzer =
  ...
  let multi_analysis =
    if List.length verses > 1 then Some (analyze_multi_verse_rhythm verses rhythm_analyzer)
    else None
  in

  { verse; verses; rhythm_analysis; multi_analysis; metadata = [] }
...
let evaluate_dimension dimension context evaluator_state =
  try
    ...
    | Some (_, evaluator_module) ->
        let module E = (val evaluator_module : EVALUATOR) in
        if E.is_applicable context then Some (E.evaluate context) else None
    | None -> None
...
let evaluate_comprehensive verse verses evaluator_state =
  ...
  | None -> (
      try
        ...
        result
      with exn -> raise (ArtisticEvaluatorError ("综合评价失败: " ^ Printexc.to_string exn)))
...
let format_evaluation_result result =
  ...
  Printf.sprintf "%s: %s (置信度: %.2f)\n详情: %s\n建议: %s" dimension_str score_str result.confidence
    details_str suggestions_str
...
let format_comprehensive_evaluation evaluation =
  ...
  let eval_results_str =
    List.map format_evaluation_result evaluation.dimension_results |> String.concat "\n---\n"
  in
  ...
  Printf.sprintf
    "=== 诗词艺术性综合评价 ===\n整体评分: %.2f (质量等级: %s)\n置信度: %.2f\n\n%s\n\n优点: %s\n不足: %s\n建议: %s"
    evaluation.overall_score quality_str evaluation.overall_confidence eval_results_str
    strengths_str weaknesses_str suggestions_str
