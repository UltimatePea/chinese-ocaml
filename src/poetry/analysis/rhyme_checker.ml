...
let calculate_rhyme_compliance_score rhyme_compliance =
  if List.length rhyme_compliance = 0 then 1.0
    ...
    let compliant_count = List.fold_left (fun acc x -> if x then acc + 1 else acc) 0 rhyme_compliance in
...
let generate_rhyme_suggestions rhyme_violations expected_scheme =
  ...
  if List.length rhyme_violations > 0 then (
    ...
    (* 根据预期韵式给出具体建议 *)
    let rhyme_positions = List.mapi (fun i opt -> 
      match opt with Some _ -> Some (i + 1) | None -> None
    ) expected_scheme |> List.filter_map (fun x -> x) in
...
let validate_rhyme_scheme scheme =
  ...
  let rhyme_count = List.fold_left (fun acc opt -> 
    match opt with Some _ -> acc + 1 | None -> acc
  ) 0 scheme in
  
  if not has_rhyme then
    (false, ["韵律模式中没有押韵要求"])
  else if rhyme_count < 2 then
    (false, ["韵律模式中押韵行数不足"])
  else
    (true, [])
...
let compare_rhyme_schemes scheme1 scheme2 =
  if List.length scheme1 <> List.length scheme2 then 0.0
    ...
    let matches = List.map2 (fun opt1 opt2 ->
...
let analyze_rhyme_usage verses rhythm_analyzer_state =
  let rhyme_groups = List.map (fun verse -> get_line_rhyme_group verse rhythm_analyzer_state) verses in
  let rhyme_count = List.fold_left (fun acc opt ->
    match opt with Some _ -> acc + 1 | None -> acc
  ) 0 rhyme_groups in
  ...
  
  {
    ...
    rhyme_ratio = if total_lines = 0 then 0.0 else float_of_int rhyme_count /. float_of_int total_lines;
    unique_rhyme_groups = 
      rhyme_groups 
      |> List.filter_map (fun x -> x)
