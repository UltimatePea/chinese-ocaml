...
let check_line_count verses pattern =
  ...
  if expected_count = 0 then (true, []) (* 不限制行数的诗体 *)
  else if actual_count = expected_count then (true, [])
  else
  ...
  if List.length expected_lengths = 0 then
    ...
    let rec check_lines vs els violations compliances =
      ...
      | _, [] ->
          ...
          let vs_violations =
            List.mapi (fun i _ -> Printf.sprintf "第%d行：超出预期行数" (i + List.length compliances + 1)) vs
          in
      ...
      | v :: vs_rest, el :: els_rest ->
          ...
          if actual_length = el then check_lines vs_rest els_rest violations (true :: compliances)
          else
            let violation =
              Printf.sprintf "第%d行字数不符：期望%d字，实际%d字" (List.length compliances + 1) el actual_length
            in
...
let generate_line_suggestions line_count_violations line_length_violations =
  ...
  if List.length line_count_violations > 0 then suggestions := "调整诗句行数以符合格律要求" :: !suggestions;

  if List.length line_length_violations > 0 then suggestions := "调整各行字数以符合格律模式" :: !suggestions;
...
let calculate_line_compliance_score line_count_ok line_length_compliance =
  ...
  let line_length_score =
    if List.length line_length_compliance = 0 then 1.0
      ...
      let compliant_count =
        List.fold_left (fun acc x -> if x then acc + 1 else acc) 0 line_length_compliance
      in
