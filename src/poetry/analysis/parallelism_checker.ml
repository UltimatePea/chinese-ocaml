...
let validate_parallelism_requirements pattern =
  List.for_all (fun (line1, line2) ->
    line1 >= 1 && line1 <= pattern.required_lines &&
    line2 >= 1 && line2 <= pattern.required_lines &&
    line1 <> line2
  ) pattern.parallelism_requirements
  ...
  if List.length violations = 0 then
    ...
    let specific_suggestions = 
      if List.length violations <= 2 then
        ["重点调整违规行对的对仗关系"; "确保对仗句字数相等、平仄相对"]
      else
        ["建议参考标准对仗格式进行全面调整"; "注意词性相对、意义相关的要求"]
    in
  ...
  if List.length pattern.parallelism_requirements = 0 then
      ...
      if line1_idx <= Array.length verse_array && line2_idx <= Array.length verse_array then (
        ...
        let tonal_score = 
          if List.length analysis1.rhyme_pattern = List.length analysis2.rhyme_pattern then
            let opposites = List.map2 (fun c1 c2 ->
              match c1, c2 with
              | PingSheng, ZeSheng | ZeSheng, PingSheng -> 1.0
              | _ -> 0.0
            ) analysis1.rhyme_pattern analysis2.rhyme_pattern in
    ...
    let avg_structural = 
      if List.length !structural_scores > 0 then
        List.fold_left (+.) 0.0 !structural_scores /. float_of_int (List.length !structural_scores)
      else 1.0
    in
    ...
    let semantic_score = 0.8 in (* 语义对仗需要更复杂的NLP分析，暂定固定值 *)
