...
let load_database_to_analyzer database analyzer_state =
  ...
  | RhymeDataEngineError msg -> 
      raise (RhythmAnalyzerError ("数据加载失败: " ^ msg))
...
let analyze_character character analyzer_state =
  ...
  | None ->
      try
        ...
        let category = match rhyme_data_opt with
          | Some info -> Some info.category
          | None -> None
        in
        let group = match rhyme_data_opt with
        ...
        let result = {
          ...
          category;
          group;
          is_rhyme_ending = false; (* 需要在句子上下文中确定 *)
        } in
      ...
      with
      | RhymeDataEngineError msg ->
          raise (RhythmAnalyzerError ("字符分析失败: " ^ msg))
...
let check_character_rhyme_match char1 char2 analyzer_state =
  ...
  with
  | RhymeDataEngineError msg ->
      raise (RhythmAnalyzerError ("韵律匹配检查失败: " ^ msg))
...
let analyze_verse_rhythm verse analyzer_state =
  ...
  | None ->
      ...
      (* 提取韵律模式 *)
      let rhyme_pattern = 
        List.filter_map (fun result -> result.category) updated_results
      in
      
      (* 检查韵组一致性 *)
      let rhyme_groups = 
        List.filter_map (fun result -> result.group) updated_results
      in
...
let analyze_multi_verse_rhythm verses analyzer_state =
  let verse_analyses = List.map (fun verse -> 
    analyze_verse_rhythm verse analyzer_state
  ) verses in
  ...
  let consistency_score = 
    ...
    | first :: rest ->
        let matches = List.fold_left (fun acc group -> 
          if group = first then acc + 1 else acc
        ) 1 rest in
...
let suggest_rhyme_characters_for_group group analyzer_state =
  ...
  with
  | RhymeDataEngineError msg ->
      raise (RhythmAnalyzerError ("韵字推荐失败: " ^ msg))
...
let clear_analyzer_cache analyzer_state =
  ...
  { analyzer_state with 
    data_engine = cleared_engine;
    last_analysis_time = Unix.time () }

(** 验证分析器状态 *)
let validate_analyzer_state analyzer_state =
  validate_engine_state analyzer_state.data_engine

(** {1 工具函数} *)

(** 格式化韵律分析结果 *)
let format_rhythm_analysis_result result =
  let category_str = Option.map rhyme_category_to_string result.category |> Option.value ~default:"未知" in
...
let format_verse_analysis analysis =
  let results_str = 
    List.map format_rhythm_analysis_result analysis.rhythm_results
    |> String.concat ", "
  in
  ...
  Printf.sprintf "诗句: %s\n韵律: %s\n韵脚: %s\n一致性: %s" 
    analysis.verse results_str ending_str consistency_str

(** 格式化多句分析结果 *)
let format_multi_verse_analysis analysis =
  let verse_results = 
    List.map format_verse_analysis analysis.verse_analyses
    |> String.concat "\n---\n"
  in
  Printf.sprintf "%s\n===\n一致性评分: %.2f\n整体质量: %.2f" 
