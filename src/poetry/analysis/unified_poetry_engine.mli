...
val check_meter_only : string list -> unified_engine_state -> (form_recognition_result * meter_check_result)
...
(** 使用自定义配置执行分析
    ...
    @param unified_state 统一引擎状态
    @return (完整分析结果, 更新后的引擎状态) *)
val analyze_with_config : string list -> analysis_config -> unified_engine_state -> (complete_poetry_analysis * unified_engine_state)