...
let wuyan_lushi_pattern = {
  ...
  rhyme_scheme = [None; Some YuRhyme; None; Some YuRhyme; None; Some YuRhyme; None; Some YuRhyme];
  tonal_pattern = [
...
let recognize_poetry_form verses meter_state =
  ...
  | None ->
      ...
      if verse_count > 0 then
        candidates := (GuTi, 0.6) :: !candidates;
...
let check_line_count verses pattern =
  ...
  if pattern.required_lines = 0 then
    (true, []) (* 古体诗不限行数 *)
  else if actual_count = pattern.required_lines then
    (true, [])
  else
    (false, [Printf.sprintf "行数不符：要求%d行，实际%d行" pattern.required_lines actual_count])
...
let check_line_lengths verses pattern =
  ...
  if List.length pattern.line_lengths = 0 then
    ...
    let compliance = List.map2 (fun actual expected -> actual = expected) actual_lengths pattern.line_lengths in
    let violations = 
      List.mapi (fun i ((actual, expected), compliant) ->
...
let check_rhyme_compliance verses pattern meter_state =
  if List.length pattern.rhyme_scheme = 0 then
    ...
    let violations = 
      List.mapi (fun i ((actual_opt, expected_opt), compliant) ->
        if not compliant then
...
let check_tonal_compliance verses pattern meter_state =
  if List.length pattern.tonal_pattern = 0 then
    ...
    let verse_analyses = List.map (fun verse -> 
      analyze_verse_rhythm verse meter_state.rhythm_analyzer
    ) verses in
    ...
    let compliance = List.map2 (fun actual expected ->
      if List.length actual = List.length expected then
        ...
        false
    ) actual_patterns pattern.tonal_pattern in
...
let check_parallelism_compliance verses pattern meter_state =
  if List.length pattern.parallelism_requirements = 0 then
    ...
    List.iter (fun (line1_idx, line2_idx) ->
      if line1_idx <= Array.length verse_array && line2_idx <= Array.length verse_array then
        ...
        let pattern_match = List.length analysis1.rhyme_pattern = List.length analysis2.rhyme_pattern in
...
let check_meter verses pattern meter_state =
  let cache_key = (String.concat "|" verses) ^ "#" ^ (match pattern.form with
  ...
  | None ->
      try
        ...
        let total_checks = 
          ...
          (List.fold_left (fun acc x -> acc +. if x then 1.0 else 0.0) 0.0 line_length_compliance) +.
          (List.fold_left (fun acc x -> acc +. if x then 1.0 else 0.0) 0.0 rhyme_compliance) +.
          (List.fold_left (fun acc x -> acc +. if x then 1.0 else 0.0) 0.0 tonal_compliance) +.
          (List.fold_left (fun acc x -> acc +. if x then 1.0 else 0.0) 0.0 parallelism_compliance)
        in
        ...
        (* 汇总违规项和建议 *)
        let all_violations = line_count_violations @ line_length_violations @ rhyme_violations @ tonal_violations @ parallelism_violations in
      ...
      with
      | exn -> raise (MeterEngineError ("格律检查失败: " ^ Printexc.to_string exn))
...
let format_recognition_result result =
  let alternatives_str = 
    List.map (fun (form, conf) -> 
      Printf.sprintf "%s (%.2f)" (format_poetry_form form) conf
    ) result.alternatives
    |> String.concat ", "
  in
  ...
  Printf.sprintf "识别诗体: %s (置信度: %.2f)\n识别依据: %s\n备选: %s"
    (format_poetry_form result.detected_form) result.confidence reasons_str alternatives_str
...
let format_meter_check_result result =
  ...
  let violations_str = 
    ...
    else String.concat "; " result.violations
  in
  let suggestions_str = String.concat "; " result.suggestions in
  
  Printf.sprintf "=== 格律检查结果 ===\n诗体: %s\n%s\n违规项: %s\n建议: %s"
    (format_poetry_form result.pattern.form) compliance_str violations_str suggestions_str