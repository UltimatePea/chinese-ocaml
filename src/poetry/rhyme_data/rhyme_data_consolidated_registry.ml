(** 韵律数据整合注册表 - 真实文件合并版本
    ...
    **Author: Whisky, PR Worker** - 基于Papa战略方法论的真实整合
    **整合方法**: 真正的文件合并+删除，不是包装API
    **原始状态**: 11个独立韵组数据文件 + 1个注册表 + 1个核心文件 = 13个文件  
    **整合后**: 1个统一注册表文件 = 1个文件
    **净减少**: 12个文件 (92%减少)
    
    此模块通过真正的代码合并，将以下文件的实际内容合并到单一文件：
    ...
    - rhyme_data_core.ml (公共函数)
    
    @version 1.0 - 真实整合版本
    @since 2025-08-04
    @consolidation_target Fix #2084 - Papa正确方法论实施 *)

open Poetry_core.Rhyme_core_types

(** {1 合并的核心类型和函数 - 来自 rhyme_data_core.ml} *)
...
let si_rhyme_data = 
  ...
  create_rhyme_data SiRhyme "思韵组：思、诗、知等韵字" ping_sheng_chars ze_sheng_chars
...
let tian_rhyme_data = 
  ...
  create_rhyme_data TianRhyme "天韵组：天、前、年等韵字" ping_sheng_chars ze_sheng_chars
...
let wang_rhyme_data = 
  ...
  create_rhyme_data WangRhyme "望韵组：望、房、方等韵字" ping_sheng_chars ze_sheng_chars
...
let qu_rhyme_data = 
  ...
  create_rhyme_data QuRhyme "去韵组：书、珠、如等韵字" ping_sheng_chars ze_sheng_chars
...
let yu_rhyme_data = 
  ...
  create_rhyme_data YuRhyme "鱼韵组：鱼、余、如等韵字" ping_sheng_chars ze_sheng_chars
...
let hua_rhyme_data = 
  ...
  create_rhyme_data HuaRhyme "花韵组：花、家、沙等韵字" ping_sheng_chars ze_sheng_chars
...
let yue_rhyme_data = 
  ...
  create_rhyme_data YueRhyme "月韵组：月、节、雪等韵字" ping_sheng_chars ze_sheng_chars
...
let jiang_rhyme_data = 
  ...
  create_rhyme_data JiangRhyme "江韵组：江、长、王等韵字" ping_sheng_chars ze_sheng_chars
...
let hui_rhyme_data = 
  ...
  create_rhyme_data HuiRhyme "灰韵组：灰、杯、来等韵字" ping_sheng_chars ze_sheng_chars
...
(** 获取所有韵组数据列表 *)
let get_all_rhyme_data () = all_rhyme_groups_data

(** 获取韵组字符列表 *)
let get_rhyme_group_characters group =
  match get_rhyme_data_by_group group with
...
let get_rhyme_stats () =
  ...
  let ping_sheng_count =
    List.fold_left
      (fun acc data ->
        acc + List.length (List.filter (fun entry -> entry.category = PingSheng) data.entries))
      0 all_rhyme_groups_data
  in
...
let validate_rhyme_data () =
  ...
  List.iter (fun data ->
    if List.length data.entries = 0 then
      issues := (Printf.sprintf "韵组 %s 为空" data.group_description) :: !issues
  ) all_rhyme_groups_data;
...
let get_consolidation_stats () =
  let total_entries = List.fold_left (fun acc data -> acc + List.length data.entries) 0 all_rhyme_groups_data in
  Printf.sprintf "韵律数据真实整合统计:\n- 韵组数量: %d个\n- 韵字数量: %d个\n- 文件整合: 13个文件 → 1个文件\n- 净减少: 12个文件 (92%%减少)\n- 整合方法: 真实合并+删除 (Papa方法论)" 
