...
(** {1 高级韵律分析API - 整合自unified_rhyme_api} *)
...
let analyze_poem_line_structure poem_line =
  ...
  List.map
    (fun char ->
      let rhyme_info = find_rhyme_info char in
      let description = get_rhyme_description char in
...
let detect_poem_rhyme_scheme poem_lines =
  ...
  let rhyme_groups = List.map detect_rhyme_group last_chars in
...
let suggest_rhyming_chars reference_char exclude_chars =
  let rhyming_chars = find_rhyming_characters reference_char in
...
let preload_rhyme_data () =
  load_rhyme_data ();
  Yyocamlc_lib.Unified_logger.info "RhymeApiCore" "韵律数据预加载完成"
...
let get_rhyme_analysis_report text =
  ...
  Buffer.add_string buffer (Printf.sprintf "文本长度: %d字符\n" (String.length text));
  ...
  List.iter
    (fun (category, count) ->
      let category_name =
        ...
        | ZeSheng -> "仄声"
      in
      Buffer.add_string buffer (Printf.sprintf "  %s: %d个\n" category_name count))
    categories;
...
let validate_rhyme_data_integrity () =
  ...
  match !issues with
  | [] ->
      Yyocamlc_lib.Unified_logger.info "RhymeApiCore" "韵律数据完整性验证通过";
      true
  | issues ->
      List.iter (fun issue -> Yyocamlc_lib.Unified_logger.warning "RhymeApiCore" issue) issues;
      false
...
let safe_find_rhyme_info char =
  ...
  with e ->
    Yyocamlc_lib.Unified_logger.error "RhymeApiCore"
...
let initialize () =
  ...
  if integrity_ok then Yyocamlc_lib.Unified_logger.info "RhymeApiCore" "韵律API核心模块初始化完成"
  else Yyocamlc_lib.Unified_logger.warning "RhymeApiCore" "韵律API核心模块初始化完成，但存在数据完整性警告"
