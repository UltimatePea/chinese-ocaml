(** 骆言诗词艺术性评价核心模块 - 重构版本
    ...
    此模块作为诗词艺术性评价系统的统一接口和协调器。 重构后的模块将原有的627行代码分解为6个专门的子模块， 大幅提升了代码的可维护性、可测试性和可复用性。
...
(** 全面诗词艺术性分析协调器
    ...
    此函数作为整个艺术性评价系统的协调器， 综合调用各个子模块的功能，提供一站式的诗词分析服务。
...
let comprehensive_poetry_analysis verses form_hint =
  let _detected_form =
    match form_hint with Some form -> form | None -> IntelligentEvaluator.auto_detect_form verses
  in
...
let comparative_quality_analysis poems =
  let analyses =
    List.map (fun verses -> (verses, comprehensive_poetry_analysis verses None)) poems
  in
  ...
  List.sort
    (fun (_, a1) (_, a2) ->
      let score1 =
        match a1.final_grade with Excellent -> 4.0 | Good -> 3.0 | Fair -> 2.0 | Poor -> 1.0
      in
      let score2 =
        match a2.final_grade with Excellent -> 4.0 | Good -> 3.0 | Fair -> 2.0 | Poor -> 1.0
      in
