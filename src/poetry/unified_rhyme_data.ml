...
let load_rhyme_data_to_cache () =
  if not (Rhyme_cache.is_initialized ()) then (
    List.iter (fun (group, category, chars) ->
      (* 添加字符到缓存 *)
      List.iter (fun char -> 
        Rhyme_cache.add_to_cache char category group
      ) chars;
      (* 添加韵组字符集 *)
...
(** 获取韵律数据统计信息 *)
let get_data_stats () =
  let total_chars = List.fold_left (fun acc (_, _, chars) -> 
    acc + List.length chars) 0 rhyme_groups_data in
