...
let update_load_time_history (source_name : string) (load_time : float) : unit =
  let history = 
    try 
      Hashtbl.find engine_state.load_time_history source_name 
    with Not_found -> []
  in
  let new_history = 
    let updated = load_time :: history in
    if List.length updated > 10 then 
...
let update_stats (cache_hit : bool) (load_time : float option) (error : bool) : unit =
  ...
  engine_state.stats <- {
    ...
    average_load_time = (match load_time with
      | Some time -> 
...
let parse_json_file (filepath : string) : Yojson.Basic.t load_result =
  ...
  | Success content ->
      (try
        Success (Yojson.Basic.from_string content)
      with
...
let load_from_source (source : data_source) : 'a load_result =
  ...
  | External url ->
      Failure (LoadingFailed (url, "外部数据源暂不支持"))
...
let initialize ?(cache_size = 1000) ?(preload_categories = []) () =
  if engine_state.initialized then
    failwith "引擎已经初始化，请先调用shutdown"
  else (
    ...
    List.iter (fun category ->
      let sources_to_preload = Hashtbl.fold (fun name record acc ->
        if record.category = category && record.mode = Preloaded then
          name :: acc
        else acc
      ) engine_state.sources [] in
      
      List.iter (fun source_name ->
        try
...
let register_data_source (name : string) (category : data_category) 
                         (source : data_source) (mode : access_mode) =
  if not engine_state.initialized then
    failwith "引擎未初始化，请先调用initialize";
...
let list_registered_sources () =
  Hashtbl.fold (fun _ record acc ->
    (record.name, record.category, record.source, record.mode) :: acc
...
let load_string_list (source_name : string) : string list load_result =
  if not engine_state.initialized then
    Failure (InvalidConfiguration "引擎未初始化")
  else
    try
      let record = Hashtbl.find engine_state.sources source_name in
...
let load_custom_data (source_name : string) (transformer : 'a -> 'b) (default_data : 'a) : 'b load_result =
  match load_string_list source_name with
  | Success data -> 
      (try
        Success (transformer (Obj.magic data))
      with exn ->
        Failure (LoadingFailed (source_name, "数据转换失败: " ^ Printexc.to_string exn)))
  | Failure _ ->
      Success (transformer default_data)

let query_by_category (category : data_category) : string list =
  Hashtbl.fold (fun name record acc ->
    if record.category = category then name :: acc else acc
  ) engine_state.sources []
...
let search_data_sources (pattern : string) : string list =
  ...
  Hashtbl.fold (fun name _ acc ->
    if Str.string_match pattern_regex name 0 then name :: acc else acc
  ) engine_state.sources []

let bulk_load (source_names : string list) : (string * 'a load_result) list =
  List.map (fun name ->
    (name, load_string_list name)
  ) source_names
...
let clear_cache ?(category = None) () =
  ...
  | Some cat ->
      let keys_to_remove = Hashtbl.fold (fun name record acc ->
        if record.category = cat then name :: acc else acc
      ) engine_state.sources [] in
...
let rec preload_category (category : data_category) : unit load_result =
  ...
  let errors = List.filter_map (fun (name, result) ->
    match result with
    | Failure err -> Some (name, err)
    | Success _ -> None
  ) results in
  if errors = [] then
    Success ()
  else
    let error_msg = String.concat "; " (List.map (fun (name, err) ->
      name ^ ": " ^ format_error err
    ) errors) in
...
let get_cache_info () : (string * int * float) list =
  Hashtbl.fold (fun name entry acc ->
    (name, entry.size_bytes, entry.timestamp) :: acc
  ) engine_state.cache []
...
let enable_profiling (enable : bool) =
  engine_state.profiling_enabled <- enable
...
let diagnose_source (source_name : string) : string =
  try
    let record = Hashtbl.find engine_state.sources source_name in
    let cache_status = 
      if Hashtbl.mem engine_state.cache source_name then "已缓存" else "未缓存"
    in
    Printf.sprintf "数据源: %s\n类别: %s\n模式: %s\n状态: %s\n缓存状态: %s\n加载次数: %d\n错误次数: %d\n最后访问: %.2f"
      record.name
      (match record.category with Poetry -> "诗歌" | Artistic -> "艺术" | Linguistic -> "语言学" | Configuration -> "配置")
      (match record.mode with Immediate -> "立即" | Cached -> "缓存" | Lazy -> "懒加载" | Preloaded -> "预加载")
      (match record.status with Loading -> "加载中" | Ready -> "就绪" | Error msg -> "错误: " ^ msg | Stale -> "过期")
      cache_status
      record.load_count
      record.error_count
