(** 韵律数据统一整合模块
    ...
    本模块整合了原有的12个分散韵律数据文件，将所有韵字数据统一管理。 这是Issue #1999韵律模块整合的核心数据层。
...
let all_rhyme_groups_data =
  [
    ...
    create_rhyme_group_data JiangRhyme "江韵" "江韵组：江、窗、床等韵字" jiang_rhyme_ping_chars
      jiang_rhyme_ze_chars;
...
let group_lookup_table =
  ...
  List.iter
    (fun group_data -> Hashtbl.replace table group_data.group_id group_data)
    all_rhyme_groups_data;
  table
...
(** 查询韵组的完整数据 *)
let lookup_group group = Hashtbl.find_opt group_lookup_table group

(** 获取韵组的所有字符 *)
let get_group_characters group =
  match lookup_group group with Some group_data -> group_data.all_characters | None -> []

(** 获取韵组的平声字符 *)
let get_ping_sheng_characters group =
  match lookup_group group with Some group_data -> group_data.ping_sheng_chars | None -> []

(** 获取韵组的仄声字符 *)
let get_ze_sheng_characters group =
  match lookup_group group with Some group_data -> group_data.ze_sheng_chars | None -> []
...
(** 批量查询字符 *)
let batch_lookup_characters chars = List.map lookup_character chars

(** {1 统计和分析接口} *)

(** 获取所有韵组列表 *)
let get_all_groups () = all_rhyme_groups_data

(** 计算韵律统计信息 *)
let get_statistics () =
  let all_chars : char_rhyme_info list =
    Hashtbl.fold (fun _ char acc -> char :: acc) character_lookup_table []
  in
  let total_chars = List.length all_chars in
  let ping_chars =
    List.filter (fun (c : char_rhyme_info) -> c.rhyme_category = PingSheng) all_chars
  in
  let ze_chars =
    List.filter (fun (c : char_rhyme_info) -> is_ze_sheng c.rhyme_category) all_chars
  in

  let group_counts =
    List.map
      (fun group_data -> (group_data.group_id, List.length group_data.all_characters))
      all_rhyme_groups_data
  in
...
let validate_data_integrity () =
  ...
  List.iter
    (fun (char, count) ->
      if count > 1 then issues := Printf.sprintf "字符 '%s' 出现 %d 次重复" char count :: !issues)
    char_counts;
...
let get_module_info () =
  ...
  Printf.sprintf "韵律数据整合模块 v1.0\n总字符: %d，总韵组: %d\n平声: %d，仄声: %d" stats.total_characters
    stats.total_groups stats.ping_sheng_count stats.ze_sheng_count
