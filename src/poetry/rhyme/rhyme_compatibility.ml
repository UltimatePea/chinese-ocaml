(** 韵律模块向后兼容性接口
    ...
    提供与原有12个韵律数据文件完全兼容的接口，确保现有代码无需修改。 通过适配器模式将新的统一接口映射到原有的API格式。
...
module Legacy_Query = struct
  ...
  (** 传统字符查询 *)
  let rhyme_lookup char = convert_query_result (lookup_character char)
  ...
  let get_rhyme_group char =
    match lookup_character char with Found rhyme_char -> Some rhyme_char.rhyme_group | _ -> None
end
  ...
  List.for_all2
    (fun legacy new_res ->
      ...
      | Legacy_Query.Found legacy_entry, Found new_char ->
          legacy_entry.character = new_char.character && legacy_entry.group = new_char.rhyme_group
...
let get_compatibility_report () =
  ...
  let compatible_groups =
    List.fold_left
      (fun acc group -> match lookup_group group with Some _ -> acc + 1 | None -> acc)
      0 all_rhyme_groups
  in

  let is_compatible = verify_compatibility () in

  Printf.sprintf "韵律模块兼容性报告\n兼容韵组: %d/%d\n总体兼容性: %s\n支持的传统接口: 11个韵组模块 + 核心查询接口" compatible_groups
    total_groups
    (if is_compatible then "✓ 完全兼容" else "✗ 存在问题")
