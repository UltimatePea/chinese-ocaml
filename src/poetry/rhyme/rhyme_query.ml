(** 韵律查询引擎
    ...
    高性能的韵律查询接口，提供O(1)查询能力和智能缓存系统。 整合了原有的多个查询引擎模块，提供统一的查询API。
...
let create_lru_cache capacity =
  { capacity; size = 0; table = Hashtbl.create capacity; head = None; tail = None }
...
let cache_put cache key value =
  ...
  | None ->
      ...
      if cache.size >= cache.capacity then remove_tail cache;
...
let smart_character_query char =
  ...
  | NotFound _ -> (
      (* 尝试查找可能的异体字匹配 *)
      let all_chars =
        get_all_groups () |> List.fold_left (fun acc group -> group.all_characters @ acc) []
      in
      let matching_variants = List.filter (fun c -> List.mem char c.variants) all_chars in
...
let fuzzy_character_query char threshold =
  ...
  let all_chars =
    get_all_groups ()
    |> List.fold_left
         (fun acc group -> List.map rhyme_character_to_char_info group.all_characters @ acc)
         []
  in
  let similar_chars =
    List.filter (fun c -> calculate_similarity char c.character >= threshold) all_chars
  in
...
(** 批量字符查询（带缓存优化） *)
let batch_query_optimized chars = List.map query_character_cached chars
  ...
  let rec process_chunks acc = function
    ...
    | chunk ->
        let current_chunk, remaining =
          ...
          else (take chunk_size chunk, drop chunk_size chunk)
        in
...
let find_same_tone_characters char tone =
  ...
  in
  List.filter
    (fun (c : rhyme_character) -> c.rhyme_category = tone && c.character <> char)
    all_chars
...
let query_stats =
  ref { total_queries = 0; cache_hits = 0; cache_misses = 0; average_response_time = 0.0 }
...
let get_performance_report () =
  ...
  Printf.sprintf "韵律查询引擎性能报告\n总查询: %d\n缓存命中率: %.1f%%\n平均响应时间: %.4fms\n查询缓存大小: %d\n韵组缓存大小: %d"
    stats.total_queries hit_rate
...
let detect_rhyme_group char =
  ...
  | MultipleMatches matches -> (
      ...
      | first :: rest ->
          let best_match =
            List.fold_left
              (fun acc c -> if c.usage_frequency > acc.usage_frequency then c else acc)
              first rest
          in
