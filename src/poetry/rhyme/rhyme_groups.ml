(** 统一韵组管理模块
    ...
    本模块整合了原本分散在多个文件中的韵组数据，建立统一的韵组管理架构。 这是Issue #1999韵律模块整合的关键组成部分。
...
let all_rhyme_characters =
  [
    ...
    ("仙", PingSheng, TianRhyme);
    ("船", PingSheng, TianRhyme);
    ("眠", PingSheng, TianRhyme);
    ("绵", PingSheng, TianRhyme);
    (* 王韵组 - 平声 *)
    ("王", PingSheng, WangRhyme);
    ("香", PingSheng, WangRhyme);
    ("方", PingSheng, WangRhyme);
    ("长", PingSheng, WangRhyme);
    ("光", PingSheng, WangRhyme);
    ("黄", PingSheng, WangRhyme);
    ("当", PingSheng, WangRhyme);
    ("堂", PingSheng, WangRhyme);
    ("房", PingSheng, WangRhyme);
    ("常", PingSheng, WangRhyme);
    ("强", PingSheng, WangRhyme);
    ("张", PingSheng, WangRhyme);
    (* 花韵组 - 平声 *)
    ...
    ("穷", PingSheng, FengRhyme);
    ("雄", PingSheng, FengRhyme);
    ("熊", PingSheng, FengRhyme);
    ...
    ("决", RuSheng, YueRhyme);
    (* 江韵组 - 仄声 *)
    ("江", QuSheng, JiangRhyme);
    ("窗", QuSheng, JiangRhyme);
    ("床", QuSheng, JiangRhyme);
    ("双", QuSheng, JiangRhyme);
    ("霜", QuSheng, JiangRhyme);
    ("亡", QuSheng, JiangRhyme);
    ("忘", QuSheng, JiangRhyme);
    ("望", QuSheng, JiangRhyme);
    ("创", QuSheng, JiangRhyme);
    ("狂", QuSheng, JiangRhyme);
    ("装", QuSheng, JiangRhyme);
    ("庄", QuSheng, JiangRhyme);
    (* 灰韵组 - 仄声 *)
    ("灰", QuSheng, HuiRhyme);
    ("开", QuSheng, HuiRhyme);
    ("来", QuSheng, HuiRhyme);
...
(** 获取平声韵组 *)
let get_ping_sheng_groups () =
  [ AnRhyme; SiRhyme; TianRhyme; WangRhyme; HuaRhyme; FengRhyme; YuRhyme ]
...
let create_rhyme_group_data group =
  ...
  let ping_chars =
    List.filter (fun (c : rhyme_character) -> c.rhyme_category = PingSheng) characters
    |> List.map (fun (c : rhyme_character) -> c.character)
  in
  let ze_chars =
    List.filter (fun (c : rhyme_character) -> is_ze_sheng c.rhyme_category) characters
    |> List.map (fun (c : rhyme_character) -> c.character)
  in
  {
    group_id = group;
    ...
    description =
      Printf.sprintf "%s韵组，包含%d个字符" (string_of_rhyme_group group) (List.length characters);
...
(** 获取所有韵组数据 *)
let get_all_rhyme_group_data () = List.map create_rhyme_group_data all_rhyme_groups
...
let get_rhyme_statistics () =
  ...
  let ping_count =
    List.length (List.filter (fun (_, tone, _) -> tone = PingSheng) all_rhyme_characters)
  in
  ...
  let most_frequent =
    List.fold_left
      (fun (max_group, max_count) (group, count) ->
        if count > max_count then (group, count) else (max_group, max_count))
      (UnknownRhyme, 0) group_distribution
    |> fst
  in
  let least_frequent =
    List.fold_left
      (fun (min_group, min_count) (group, count) ->
        if count < min_count || min_count = 0 then (group, count) else (min_group, min_count))
      (UnknownRhyme, max_int) group_distribution
    |> fst
  in
  {
...
module Compat = struct
  (** 获取花韵组字符 *)
  let get_hua_rhyme_chars () =
    get_group_characters HuaRhyme |> List.map (fun (c : rhyme_character) -> c.character)

  (** 获取风韵组字符 *)
  let get_feng_rhyme_chars () =
    get_group_characters FengRhyme |> List.map (fun (c : rhyme_character) -> c.character)

  (** 获取鱼韵组字符 *)
  let get_yu_rhyme_chars () =
    get_group_characters YuRhyme |> List.map (fun (c : rhyme_character) -> c.character)

  (** 获取月韵组字符 *)
  let get_yue_rhyme_chars () =
    get_group_characters YueRhyme |> List.map (fun (c : rhyme_character) -> c.character)

  (** 获取江韵组字符 *)
  let get_jiang_rhyme_chars () =
    get_group_characters JiangRhyme |> List.map (fun (c : rhyme_character) -> c.character)

  (** 获取灰韵组字符 *)
  let get_hui_rhyme_chars () =
    get_group_characters HuiRhyme |> List.map (fun (c : rhyme_character) -> c.character)
end
