...
(** 添加字符到韵律缓存 *)
let add_to_cache cache char category group = Hashtbl.replace cache.char_cache char (category, group)

(** 添加韵组字符集 *)
let add_rhyme_group_chars cache group chars = Hashtbl.replace cache.group_chars_cache group chars

(** 查询字符的韵律信息 *)
let lookup_rhyme cache char = try Some (Hashtbl.find cache.char_cache char) with Not_found -> None
...
(** 获取所有缓存的字符 *)
let get_all_cached_chars cache = Hashtbl.fold (fun char _ acc -> char :: acc) cache.char_cache []
...
let add_to_cache_global char category group =
  add_to_cache (Lazy.force global_cache) char category group

let add_rhyme_group_chars_global group chars =
  add_rhyme_group_chars (Lazy.force global_cache) group chars
