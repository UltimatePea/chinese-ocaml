...
let ensure_initialized () =
  if Hashtbl.length rhyme_lookup_table = 0 then
...
let lookup_rhyme_expanded_fast char =
  ensure_initialized ();
  try
    Some (Hashtbl.find expanded_rhyme_lookup_table char)
  with Not_found -> None
...
let lookup_rhyme_group_fast char =
  ensure_initialized ();
  try
    Some (Hashtbl.find rhyme_group_lookup_table char)
  with Not_found -> None
...
let lookup_rhyme_category_fast char =
  ensure_initialized ();
  try
    Some (Hashtbl.find rhyme_category_lookup_table char)
  with Not_found -> None
...
let lookup_string_rhyme_groups str =
  ...
  List.map (fun char -> 
    (char, lookup_rhyme_group_fast (String.make 1 char))
  ) chars
...
let lookup_string_rhyme_categories str =
  ...
  let chars = String.to_seq str |> List.of_seq in
  List.map (fun char -> 
    (char, lookup_rhyme_category_fast (String.make 1 char))
  ) chars
...
let is_all_ping_sheng str =
  ...
  List.for_all (fun char ->
    is_ping_sheng_fast (String.make 1 char)
  ) chars
...
let is_all_ze_sheng str =
  let chars = String.to_seq str |> List.of_seq in
  List.for_all (fun char ->
    is_ze_sheng_fast (String.make 1 char)
  ) chars
...
let get_all_rhyme_categories () =
  ensure_initialized ();
  let categories = Hashtbl.fold (fun _ category acc -> category :: acc) rhyme_category_lookup_table [] in
...
let group_chars_by_rhyme_category () =
  ...
  Hashtbl.iter (fun char category ->
...
let benchmark_list_performance test_chars iterations =
  ...
  for _i = 1 to iterations do
    List.iter (fun char ->
      ignore (is_in_expanded_rhyme_database char)
    ) test_chars
  done;
  let end_time = Unix.gettimeofday () in
  end_time -. start_time
...
let get_cache_stats () =
  ...
  {|
缓存统计信息：
- 音韵查询表大小: |} ^ string_of_int (Hashtbl.length rhyme_lookup_table) ^ {|
- 扩展音韵查询表大小: |} ^ string_of_int (Hashtbl.length expanded_rhyme_lookup_table) ^ {|
- 韵组查询表大小: |} ^ string_of_int (Hashtbl.length rhyme_group_lookup_table) ^ {|
- 韵类查询表大小: |} ^ string_of_int (Hashtbl.length rhyme_category_lookup_table) ^ {|
|}
