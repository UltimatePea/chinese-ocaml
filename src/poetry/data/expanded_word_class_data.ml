...
let get_loaded_data () =
  load_all_data ();
  match !_loaded_data with
  | Some data -> data
  | None -> failwith "数据加载失败"

(** {1 数据生成辅助函数} *)

(** 词性数据生成辅助函数 - 消除代码重复的核心函数 *)
let _make_word_class_list words word_class =
...
let person_relation_nouns =
  let (person_relation, _, _, _, _, _, _, _, _, _) = 
    let (nouns, _, _, _, _, _) = get_loaded_data () in nouns in
  person_relation
...
let movement_position_verbs =
  let (movement_position, _, _, _, _, _, _, _, _, _, _) = 
    let (_, verbs, _, _, _, _) = get_loaded_data () in verbs in
  movement_position
...
let size_adjectives =
  let (size, _, _, _, _, _, _, _, _, _, _, _) = 
    let (_, _, adjectives, _, _, _) = get_loaded_data () in adjectives in
  size
...
let degree_adverbs =
  let (degree, _, _) = 
    let (_, _, _, adverbs, _, _) = get_loaded_data () in adverbs in
  degree
...
let cardinal_numbers =
  let (cardinal_numbers, _, _) = 
    let (_, _, _, _, nums_cls, _) = get_loaded_data () in nums_cls in
  cardinal_numbers
...
let measuring_classifiers =
  let (_, _, classifiers) = 
    let (_, _, _, _, nums_cls, _) = get_loaded_data () in nums_cls in
  classifiers @ (List.map (fun word -> (word, Classifier)) WordClassStorage.measuring_classifiers_list)
...
let pronoun_words =
  let (pronouns, _, _, _, _) = 
    let (_, _, _, _, _, func_words) = get_loaded_data () in func_words in
  pronouns
...
(** 全部扩展词性数据的合并列表 *)
let all_expanded_word_class_data = lazy (
  nature_nouns @
...
let _get_data_statistics () =
  ...
  let nouns_count = List.length nature_nouns + List.length person_relation_nouns + 
                   List.length social_status_nouns + List.length building_place_nouns +
                   List.length geography_politics_nouns + List.length tools_objects_nouns +
                   List.length emotional_psychological_nouns + List.length moral_virtue_nouns +
                   List.length knowledge_learning_nouns + List.length time_space_nouns +
                   List.length affairs_activity_nouns in
  let verbs_count = List.length movement_position_verbs + List.length sensory_action_verbs +
                   List.length cognitive_activity_verbs + List.length social_communication_verbs +
                   List.length emotional_expression_verbs + List.length social_behavior_verbs +
                   List.length agricultural_verbs + List.length manufacturing_verbs +
                   List.length transportation_verbs + List.length commercial_verbs +
                   List.length cleaning_verbs in
  let adjectives_count = List.length size_adjectives + List.length shape_adjectives +
                        List.length color_adjectives + List.length texture_adjectives +
                        List.length value_judgment_adjectives + List.length emotional_state_adjectives +
                        List.length motion_state_adjectives + List.length temperature_texture_adjectives +
                        List.length purity_cleanliness_adjectives + List.length moral_character_adjectives +
                        List.length wisdom_brightness_adjectives + List.length precision_degree_adjectives in
  let adverbs_count = List.length degree_adverbs + List.length temporal_adverbs + List.length manner_adverbs in
  let numerals_count = List.length cardinal_numbers + List.length ordinal_numbers + List.length measuring_classifiers in
  let function_words_count = List.length pronoun_words + List.length preposition_words +
                            List.length conjunction_words + List.length particle_words +
                            List.length interjection_words in
...
let get_expanded_char_list () =
  List.map fst (get_expanded_word_class_database ())

(** 查找字符的词性 *)
let find_word_class char =
  try
    Some (List.assoc char (get_expanded_word_class_database ()))
  with Not_found -> None

(** 按词性获取字符列表 *)
let get_chars_by_class word_class =
  List.fold_left (fun acc (word, wc) -> 
    if wc = word_class then word :: acc else acc
