...
let convert_comprehensive_database comprehensive_data =
  List.map (fun (char, category, group) ->
...
let register_data_source name source ?(priority=0) description =
  let entry = {
    Data_source_manager.name = name;
    Data_source_manager.source = source;
    Data_source_manager.priority = priority;
    Data_source_manager.description = description;
  } in
...
let is_char_in_database char =
  is_char_in_database_comprehensive char

let get_char_rhyme_info char =
  match get_char_rhyme_info_comprehensive char with
  | Some (c, category, group) -> 
      Some (c, convert_rhyme_category category, convert_rhyme_group group)
  | None -> None
...
let get_database_stats () =
  ...
  (* 计算唯一韵组数量 *)  
  let unique_groups = 
    List.fold_left (fun acc (_, _, group) -> 
  ...
  let unique_categories = 
    List.fold_left (fun acc (_, category, _) -> 
...
let validate_database () =
  try
    ...
    (is_valid, errors)
  with
  | e -> (false, ["数据完整性验证异常: " ^ (Printexc.to_string e)])
...
let print_registered_sources () =
  Printf.printf "\n=== 已注册数据源 ===\n";
  if Hashtbl.length registered_sources = 0 then
    Printf.printf "暂无注册的数据源\n"
  else
...
let load_from_source source =
  ...
  try
    Data_source_manager.load_from_source source
  with
  | e -> 
...
let find_data_source name =
  try
    Some (Hashtbl.find registered_sources name)
  with Not_found -> None
