...
let parse_rhyme_database json_content =
  try
    ...
    let parsed_groups =
      List.map
        (fun (group_name, group_json) ->
          ...
          let items = List.map (fun char -> create_rhyme_item char category group) characters in
    ...
    let version = try json |> member "version" |> to_string with _ -> "unknown" in
    ...
    let sources =
      try json |> member "sources" |> to_list |> List.map to_string with _ -> [ "json_loader" ]
    in

    { groups = parsed_groups; version; last_updated; sources }
  with
  | Yojson.Json_error msg -> raise (JsonLoaderError ("JSON parsing failed: " ^ msg))
  | Yojson.Safe.Util.Type_error (msg, _) -> raise (JsonLoaderError ("Type error: " ^ msg))
