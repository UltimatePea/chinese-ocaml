...
let raise_externalized_error error =
  raise (ExternalizedDataError error)
...
let extract_word_list json_data =
  try
    ...
    | `Assoc assoc_list ->
        ...
        List.fold_left (fun acc (_key, value) ->
          match value with
          | `Assoc inner_assoc ->
              (match Yojson.Safe.Util.to_option (Yojson.Safe.Util.member "words") (`Assoc inner_assoc) with
               | Some (`List word_list) ->
                   List.fold_left (fun inner_acc word_obj ->
                     match word_obj with
                     | `Assoc word_fields ->
                         (match Yojson.Safe.Util.to_option (Yojson.Safe.Util.member "word") (`Assoc word_fields) with
    ...
    | `List word_list ->
        ...
        List.fold_left (fun acc item ->
          ...
          | `Assoc fields ->
              (match Yojson.Safe.Util.to_option (Yojson.Safe.Util.member "word") (`Assoc fields) with
  ...
  | exn -> 
      raise_externalized_error (ValidationError ("数据提取错误: " ^ Printexc.to_string exn))
...
let load_all_word_class_data () = 
  let (ping_sheng, shang_sheng, qu_sheng, ru_sheng) = safe_load_tone_data () in
  {
    ...
    ping_sheng = ping_sheng;
    shang_sheng = shang_sheng;
    qu_sheng = qu_sheng;
    ru_sheng = ru_sheng;
  }
...
let validate_data_integrity () =
  try
    ...
    let total_words = 
      ...
      List.length all_data.ping_sheng +
      List.length all_data.shang_sheng +
      List.length all_data.qu_sheng +
      List.length all_data.ru_sheng
    in
