(** 鱼韵组数据模块 - 第二阶段技术债务重构版本
    ...
    基于数据外化重构，将原有硬编码的长函数数据移动到外部JSON文件， 实现数据与代码分离，大幅减少代码行数，提升可维护性。
...
(** 获取鱼韵组字符总数 *)
let yu_yun_char_count = lazy (List.length (Lazy.force yu_yun_ping_sheng))

(** 获取鱼韵组核心字符数量 *)
let yu_yun_core_count = lazy (List.length (Lazy.force yu_yun_core_chars))

(** 获取鱼韵组贾价系列字符数量 *)
let yu_yun_jia_count = lazy (List.length (Lazy.force yu_yun_jia_chars))

(** 获取鱼韵组棋期系列字符数量 *)
let yu_yun_qi_count = lazy (List.length (Lazy.force yu_yun_qi_chars))

(** 获取鱼韵组鱼类扩展字符数量 *)
let yu_yun_fish_count = lazy (List.length (Lazy.force yu_yun_fish_chars))
