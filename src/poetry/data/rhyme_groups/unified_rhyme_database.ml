(** 统一韵律数据库接口模块 - 骆言诗词编程特性
    ...
    此模块整合所有韵组数据，提供统一的数据库访问接口。 保持与原 expanded_rhyme_data.ml 的完全向后兼容性， 同时提供新的模块化查询接口。
...
let expanded_rhyme_database =
  ...
  let deduplicate data =
    ...
    List.fold_left
      (fun acc (char, category, group) ->
        if Hashtbl.mem seen char then acc (* 跳过重复字符 *)
        else (
...
(** 按韵组获取数据
    ...
    @param group 韵组
    @return 该韵组的所有韵律数据 *)
let get_rhyme_data_by_group group = List.filter (fun (_, _, g) -> g = group) expanded_rhyme_database
...
let get_all_rhyme_groups () =
  expanded_rhyme_database |> List.map (fun (_, _, group) -> group) |> List.sort_uniq compare
...
(** 获取特定韵组的字符数量
    ...
    @param group 韵组
    @return 该韵组的字符数量 *)
let get_rhyme_group_char_count group = List.length (get_rhyme_data_by_group group)
...
(** 获取特定韵类的字符数量
    ...
    @param category 韵类
    @return 该韵类的字符数量 *)
let get_rhyme_category_char_count category = List.length (get_rhyme_data_by_category category)
