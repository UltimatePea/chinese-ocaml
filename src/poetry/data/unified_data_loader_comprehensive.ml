...
let format_comprehensive_error = function
  ...
  | UnifiedLoadError msg -> 
      Printf.sprintf "统一加载器错误: %s" msg
...
let load_ping_sheng_rhymes_comprehensive () =
  try
    ...
    let json_data = load_comprehensive_data 
      (RhymeDataType PingShengRhymes) 
      (JsonFile file_path) in
...
let load_tone_chars data_type =
  try
    ...
    Yojson.Safe.Util.to_list json_data
...
let get_ping_sheng_chars_comprehensive () = 
  load_tone_chars (ToneDataType PingSheng)

let get_shang_sheng_chars_comprehensive () = 
  load_tone_chars (ToneDataType ShangSheng)

let get_qu_sheng_chars_comprehensive () = 
  load_tone_chars (ToneDataType QuSheng)

let get_ru_sheng_chars_comprehensive () = 
  load_tone_chars (ToneDataType RuSheng)
...
let get_unified_database_comprehensive () =
  load_complete_rhyme_database_comprehensive ()
...
let load_all_data_types () =
  let all_types = [
    RhymeDataType PingShengRhymes; RhymeDataType ZeShengRhymes; 
    RhymeDataType CompleteRhymeDatabase;
    ToneDataType PingSheng; ToneDataType ShangSheng; 
    ToneDataType QuSheng; ToneDataType RuSheng; ToneDataType AllToneData;
    PoetryDataType UnifiedDatabase; PoetryDataType DataSourceRegistry;
    PoetryDataType CacheManagement;
    WordClassDataType; ArtisticDataType
  ] in
...
let clear_comprehensive_cache () =
  ...
  clear_cache (); (* 调用unified_data_loader的缓存清理 *)
...
let get_comprehensive_cache_info () =
  let all_types = [
    RhymeDataType PingShengRhymes; RhymeDataType ZeShengRhymes;
    ToneDataType PingSheng; ToneDataType ShangSheng;
    ToneDataType QuSheng; ToneDataType RuSheng;
    PoetryDataType UnifiedDatabase; WordClassDataType; ArtisticDataType
  ] in
  List.map (fun data_type ->
    let is_cached = Hashtbl.mem comprehensive_cache data_type in
    let cache_size = if is_cached then 1 else 0 in (* 简化的缓存大小 *)
...
let print_comprehensive_status () =
  ...
  Printf.printf "\n--- 缓存状态 ---\n";
  Hashtbl.iter (fun data_type _ ->
    Printf.printf "已缓存: %s\n" (data_type_to_string data_type)
  ) comprehensive_cache;
  
  Printf.printf "\n--- 性能统计 ---\n";
  Hashtbl.iter (fun data_type (total_time, count) ->
    let avg_time = if count > 0 then total_time /. (float_of_int count) else 0.0 in
    Printf.printf "%s: 调用%d次, 平均%.3fms\n" 
      (data_type_to_string data_type) count (avg_time *. 1000.0)
  ) performance_stats;
  Printf.printf "========================\n\n"
