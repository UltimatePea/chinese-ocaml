  ...
  | RhymeSuccess items -> (
      ...
      | `CSV ->
          let csv_lines =
            ...
            :: List.map
                 (fun item ->
                   Printf.sprintf "%s,%s,%s,%d" item.character
                     (format_tone_string item.tone)
      ...
      | `XML ->
          let xml_items =
            List.map
              (fun item ->
                Printf.sprintf "<item><character>%s</character><tone>%s</tone><priority>%d</priority></item>"
                  item.character
                  (format_tone_string item.tone)
                  item.source_priority)
              items
...
let export_all_data ~format =
  ...
  let mock_query = QueryByCharacter "" in  (* Mock query for export_rhyme_data *)
  match format with
  | `JSON ->
      let json_items =
        List.map
...
let import_rhyme_data source ~format data = 
  (* 简化实现 - 实际需要解析不同格式的数据 *)
  debug_log (Printf.sprintf "Import request received for format: %s" 
...
let create_data_snapshot () =
  ...
  | RhymeSuccess backup_data -> 
      RhymeSuccess (snapshot_name, backup_data)
  | error -> error
...
let get_rhyme_statistics () =
  ...
  debug_log (Printf.sprintf "Statistics: chars=%d, groups=%d, categories=%d, sources=%d" 
    total_chars rhyme_groups rhyme_categories data_sources);
...
let validate_rhyme_data () =
  ...
  if source_count = 0 then
    errors := "No rhyme sources registered" :: !errors;

  let is_valid = !errors = [] in
  debug_log (Printf.sprintf "Validation completed: %s (%d errors)" 
    (if is_valid then "VALID" else "INVALID") (List.length !errors));
...
let find_data_conflicts () =
  ...
  Hashtbl.iter (fun char item ->
    ...
    let source_conflicts = [] in  (* 简化实现 *)
    if source_conflicts <> [] then
      conflicts := (char, source_conflicts) :: !conflicts
  ) character_rhyme_index;
...
let resolve_conflicts_automatically () =
  ...
  debug_log (Printf.sprintf "Auto-resolved %d conflicts based on source priority" (List.length resolved));
...
let check_source_health () =
  ...
  Hashtbl.iter (fun source (loader, priority, description, timestamp) ->
    try
      ...
      | RhymeWarning (items, warn) -> 
          health_report := (source, "WARNING: " ^ warn, List.length items, description) :: !health_report
    with exn ->
      health_report := (source, "EXCEPTION: " ^ Printexc.to_string exn, 0, description) :: !health_report
  ) rhyme_sources;
...
let cleanup_unused_sources () =
  ...
  Hashtbl.iter (fun source (_, _, description, timestamp) ->
