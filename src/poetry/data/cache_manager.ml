...
let create_empty_cache () = {
  character_index = Hashtbl.create 20000;  (* 增大初始容量 *)
...
let rec merge_data_sources sources =
  ...
  Hashtbl.fold (fun _ item acc ->
    (item.character, item.category, item.group) :: acc
...
and merge_data_sources_optimized sources =
  ...
  (* 按优先级排序数据源 *)
  let sorted_sources = List.sort (fun a b -> 
    compare b.Data_source_manager.priority a.Data_source_manager.priority
  ) sources in
    ...
    try
      ...
      List.iter (fun (char, category, group) ->
        if not (Hashtbl.mem seen_characters char) then (
          ...
          (* O(1) 韵组索引更新 *)
          let group_chars = match Hashtbl.find_opt cache.group_index group with
            | Some lst -> char :: lst
            | None -> [char]
          in
          ...
          (* O(1) 韵类索引更新 *)
          let category_chars = match Hashtbl.find_opt cache.category_index category with
            | Some lst -> char :: lst
            | None -> [char]
          in
...
let is_char_in_database char =
  ...
  cache_statistics := { !cache_statistics with 
    ...
    cache_hits = (if result then !cache_statistics.cache_hits + 1 else !cache_statistics.cache_hits);
    cache_misses = (if result then !cache_statistics.cache_misses else !cache_statistics.cache_misses + 1);
  };
...
let get_char_rhyme_info char =
  ...
  cache_statistics := { !cache_statistics with 
    total_queries = !cache_statistics.total_queries + 1;
    cache_hits = (if result <> None then !cache_statistics.cache_hits + 1 else !cache_statistics.cache_hits);
    cache_misses = (if result <> None then !cache_statistics.cache_misses else !cache_statistics.cache_misses + 1);
  };
...
let _get_detailed_cache_stats () =
  if is_cache_loaded () then
    ...
    let source_stats = Hashtbl.fold (fun source_name items acc ->
      (source_name, List.length items) :: acc
    ) cache.source_index [] in
...
let validate_database () =
  ...
  (* 检查字符索引一致性 *)
  Hashtbl.iter (fun char item ->
    if char <> item.character then
      errors := (Printf.sprintf "Index inconsistency: key=%s, item.character=%s" char item.character) :: !errors;
    if char = "" then
      errors := "Empty character found in index" :: !errors
  ) cache.character_index;
    ...
    List.iter (fun char ->
      ...
      | Some item ->
          if item.group <> group then
            errors := (Printf.sprintf "Group index inconsistency: char=%s expected_group=%s actual_group=%s" 
                         char (Obj.repr group |> Obj.tag |> string_of_int) (Obj.repr item.group |> Obj.tag |> string_of_int)) :: !errors
      | None ->
  ...
  (* 检查韵类索引一致性 *)
  Hashtbl.iter (fun category char_list ->
    List.iter (fun char ->
      match Hashtbl.find_opt cache.character_index char with
      | Some item ->
      ...
      | None ->
          errors := (Printf.sprintf "Character %s in category index but not in character index" char) :: !errors
    ) char_list
  ) cache.category_index;

  (* 检查数据源索引一致性 *)
  Hashtbl.iter (fun source_name items ->
    List.iter (fun item ->
      if item.source_id <> source_name then
        errors := (Printf.sprintf "Source index inconsistency: item.source_id=%s expected=%s" item.source_id source_name) :: !errors
    ) items
  ) cache.source_index;
    ...
    List.iter (fun item ->
      ...
      | Some (existing_source, existing_priority) ->
          if existing_source <> source_name then
            conflicts := (item.character, existing_source, source_name, existing_priority, item.priority) :: !conflicts
      | None ->
          Hashtbl.add char_sources item.character (source_name, item.priority)
    ) items
  ) cache.source_index;
