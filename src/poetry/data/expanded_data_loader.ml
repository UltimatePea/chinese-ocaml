...
module WordClassLoader = struct
  ...
  let load_nouns () =
    ...
    (person_relation, social_status, building_place, geography_politics, tools_objects,
  ...
  let load_verbs () =
    ...
    (movement_position, sensory_action, cognitive_activity, social_communication, 
  ...
  let load_adjectives () =
    ...
    (size, shape, color, texture, value_judgment, emotional_state, motion_state,
...
let safe_load_nouns () =
  try
    WordClassLoader.load_nouns ()
  with DataLoadError err ->
    Printf.eprintf "警告: %s，使用默认名词数据\n" (format_error err);
    (FallbackData.basic_person_relation_nouns, FallbackData.basic_social_status_nouns, 
     [], [], [], [], [], [], [], [])

let safe_load_verbs () =
  try
    WordClassLoader.load_verbs ()
  with DataLoadError err ->
...
let safe_load_adjectives () =
  try
    WordClassLoader.load_adjectives ()
  with DataLoadError err ->
...
let safe_load_numerals_classifiers () =
  try
    WordClassLoader.load_numerals_classifiers ()
  with DataLoadError err ->
...
let safe_load_function_words () =
  try
    WordClassLoader.load_function_words ()
  with DataLoadError err ->
