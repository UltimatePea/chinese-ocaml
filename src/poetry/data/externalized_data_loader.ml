(** 外化数据加载器 - 重构版本，使用现有模块化组件
    ...
    这个文件作为协调器，使用已经模块化的组件来提供统一的数据访问接口。 重构目标：减少代码重复，使用现有的模块化结构。
...
let basic_classifiers = [ "个"; "只"; "条"; "根"; "支"; "片"; "张"; "块"; "团"; "群" ]
let basic_tools_objects = [ "桌"; "椅"; "床"; "柜"; "笔"; "墨"; "纸"; "砚"; "琴"; "瑟" ]
...
let safe_load_nature_nouns () =
  try
    ...
    let _, _, _, geography_politics, _, _, _, _, time_space, _ =
      Poetry_word_class_loader.load_nouns ()
    in
...
let safe_load_tools_objects () =
  try
    ...
    let _, _, _, _, tools_objects, _, _, _, _, _ = Poetry_word_class_loader.load_nouns () in
...
let get_geography_politics_nouns () =
  try
    let _, _, _, geography_politics, _, _, _, _, _, _ = Poetry_word_class_loader.load_nouns () in
...
let get_person_relation_nouns () =
  try
    let person_relation, _, _, _, _, _, _, _, _, _ = Poetry_word_class_loader.load_nouns () in
...
let get_social_status_nouns () =
  try
    let _, social_status, _, _, _, _, _, _, _, _ = Poetry_word_class_loader.load_nouns () in
...
let get_building_place_nouns () =
  try
    let _, _, building_place, _, _, _, _, _, _, _ = Poetry_word_class_loader.load_nouns () in
...
let load_all_data () =
  ...
  { nature_nouns; classifiers; tools_objects; ping_sheng; shang_sheng; qu_sheng; ru_sheng }
