...
val query_data_streaming : query_criteria -> (unified_data_item -> unit) -> unit data_result
(** 流式数据查询 - 适用于大数据集 *)
...
val validate_data_integrity : data_source_id list -> (bool * string list) data_result
(** 验证数据完整性

    @param sources 要验证的数据源列表
    @return (是否有效, 错误信息列表) *)

val detect_data_conflicts :
  data_source_id list -> (string * data_source_id * data_source_id) list data_result
(** 检测数据冲突
...
val get_data_statistics : unit -> (int * int * int * float) data_result
(** 获取数据统计

    @return (总数据项数, 数据源数, 缓存大小, 平均查询时间ms) *)

val get_source_statistics : data_source_id -> (int * float * bool) data_result
(** 获取特定数据源统计
