...
let load_all_data () =
  let sources_with_priority =
    Hashtbl.fold
      (fun source_id (loader, priority, _, _) acc -> (source_id, loader, priority) :: acc)
      registered_sources []
...
let rec query_data_impl criteria =
  ...
  | None ->
      ...
      (match result with Success data -> QueryCache.put criteria data | _ -> ());
...
module Cache = struct
  ...
  let preload_cache source_list =
    try
      List.iter
        (fun source_id ->
          match query_data (BySource source_id) with Success _ -> () (* 数据已经被缓存 *) | Error _ -> ())
        source_list;
      Success ()
    with exn ->
      Error
...
let validate_data_integrity source_list =
            ...
            | Success items ->
                List.iter
                  (fun item ->
                    ...
                    else Hashtbl.add seen_chars item.character true)
                  items
            | Error _ -> all_errors := "Failed to load source" :: !all_errors)
        | None -> all_errors := "Source not found" :: !all_errors)
      source_list;
  ...
  with exn ->
    Error
      (Poetry_core.Poetry_errors.DataSourceError ("Validation failed: " ^ Printexc.to_string exn))
            ...
            | Success items ->
                List.iter
                  (fun item ->
                    ...
                    | None -> Hashtbl.add char_sources item.character source_id)
                  items
            | Error _ -> ())
        | None -> ())
      source_list;
...
let merge_conflicting_data ~resolve_conflict source_list =
            ...
            | Success items ->
                List.iter
                  (fun item ->
                    ...
                    | None -> Hashtbl.add char_items item.character item)
                  items
            | Error _ -> ())
        | None -> ())
      source_list;
  ...
  with exn ->
    Error
      (Poetry_core.Poetry_errors.DataSourceError ("Data merge failed: " ^ Printexc.to_string exn))
...
let get_data_statistics () =
  try
    ...
    let avg_query_time = 0.0 in
    (* 暂未实现查询时间统计 *)
    Success (total_items, source_count, cache_size, avg_query_time)
  with exn ->
    Error
      (Poetry_core.Poetry_errors.DataSourceError ("Statistics failed: " ^ Printexc.to_string exn))
...
module Compatibility = struct
  ...
  let is_char_in_database char = Hashtbl.mem character_index char
