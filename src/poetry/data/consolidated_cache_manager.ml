...
let find_cache_by_name name =
  try Some (Hashtbl.find cache_registry name)
  with Not_found -> None
...
let create_cache ?(config = default_cache_config) name =
  let cache = {
    ...
    stats = {
      hits = 0; misses = 0; hit_rate = 0.0; total_requests = 0;
      current_size = 0; evictions = 0; expired_count = 0;
...
let rec set ~cache_name key value =
  ...
  | Some cache ->
      (* 检查是否需要驱逐 *)
      if Hashtbl.length cache.entries >= cache.config.max_size then
...
let set_multiple ~cache_name pairs =
  List.iter (fun (key, value) -> set ~cache_name key value) pairs

let get_multiple ~cache_name keys =
  List.map (fun key -> (key, get ~cache_name key)) keys

let remove_multiple ~cache_name keys =
  List.fold_left (fun acc key ->
    if remove ~cache_name key then acc + 1 else acc
  ) 0 keys
...
let cleanup_expired ~cache_name =
  match find_cache_by_name cache_name with
  | None -> 0
  | Some cache -> cleanup_expired_entries cache

let cleanup_all_expired () =
  Hashtbl.fold (fun _ cache acc ->
    acc + cleanup_expired_entries cache
  ) cache_registry 0
...
let get_stats ~cache_name =
  ...
  | None -> 
      Printf.printf "警告: 缓存 '%s' 不存在\n" cache_name;
      { hits = 0; misses = 0; hit_rate = 0.0; total_requests = 0;
        current_size = 0; evictions = 0; expired_count = 0; memory_usage_bytes = 0 }
  | Some cache ->
      (* 更新当前内存使用量 *)
      let memory_usage = calculate_memory_usage cache.entries in
      { cache.stats with 
...
let list_caches () =
  Hashtbl.fold (fun name _ acc -> name :: acc) cache_registry []
...
module PoetryCache = struct
  ...
  let () = if find_cache_by_name cache_name = None then 
  ...
  let set_rhyme_data key data =
    let data_str = String.concat ";" (List.map (fun (c, cat, grp) -> 
      String.concat "," [c; cat; grp]) data) in
    let bytes = Bytes.of_string data_str in
    set ~cache_name key (CustomValue ("rhyme_data", bytes))

  let get_tone_data key =
    match get ~cache_name key with
    | Some (StringListValue data) -> Some data
    | _ -> None

  let set_tone_data key data =
    set ~cache_name key (StringListValue data)
  ...
  let set_word_class_data key data =
    set ~cache_name key (StringListValue data)
end
...
module JsonCache = struct
  ...
  let get_json key =
    match get ~cache_name key with
    | Some (JsonValue json) -> Some json
    | _ -> None

  let set_json key json =
...
module FileCache = struct
  ...
  let get_file_content file_path =
    match get ~cache_name file_path with
    | Some (StringValue content) -> Some content
    | _ -> None

  let set_file_content file_path content =
    set ~cache_name file_path (StringValue content)
...
let migrate_cache ~old_name ~new_name =
  ...
  | Some old_cache ->
      if find_cache_by_name new_name = None then
        create_cache ~config:old_cache.config new_name;
      
      let pairs = Hashtbl.fold (fun key entry acc ->
        (key, entry.value) :: acc
      ) old_cache.entries [] in
...
let optimize_cache ~cache_name =
  ...
  | Some cache ->
      ...
      Printf.printf "缓存 '%s' 优化完成: 清理 %d 个过期项 (%d -> %d)\n" 
        cache_name expired before_size after_size
...
let get_total_memory_usage () =
  List.fold_left (fun acc (name, _) ->
    acc + get_memory_usage ~cache_name:name
  ) 0 (get_all_stats ())
...
module CacheManagerCompat = struct
  ...
  let get key = 
    ...
    | Some (CustomValue (_, bytes)) -> 
        (try Some (Marshal.from_bytes bytes 0) with _ -> None)
    | _ -> None
...
module ManagersCacheCompat = struct
  ...
  let cache_set key value =
    set ~cache_name key (StringValue value)
...
let validate_cache_integrity ~cache_name =
  ...
  | Some cache ->
      ...
      let hash_keys = Hashtbl.fold (fun k _ acc -> k :: acc) cache.entries [] |> List.sort_uniq String.compare in
      ...
      Hashtbl.iter (fun _ entry ->
        if is_expired entry then incr expired_count
      ) cache.entries;
      
      if !expired_count > 0 then
        errors := Printf.sprintf "发现 %d 个过期项" !expired_count :: !errors;
...
let benchmark_cache_performance ~cache_name operations =
  ...
  | None -> (0.0, 0.0)
  | Some _ ->
      let test_data = Array.init operations (fun i -> 
        ("key_" ^ string_of_int i, StringValue ("value_" ^ string_of_int i))) in
...
let debug_cache_content ~cache_name =
  ...
  | Some cache ->
      ...
      Hashtbl.iter (fun key entry ->
        let value_desc = match entry.value with
          | JsonValue _ -> "JSON数据"
...
let get_cache_health_report ~cache_name =
  ...
  | Some cache ->
      ...
      let expired_count = Hashtbl.fold (fun _ entry acc ->
        if is_expired entry then acc + 1 else acc
      ) cache.entries 0 in
      ...
      Printf.sprintf 
        ...
        stats.current_size cache.config.max_size
        (stats.hit_rate *. 100.0)
        expired_count
        stats.memory_usage_bytes
