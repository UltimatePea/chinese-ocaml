...
let convert_comprehensive_to_tone_error = function
  ...
  | ComprehensiveLoadError (UnifiedLoadError msg) -> 
      ToneDataError (InvalidData msg)
  | ComprehensiveLoadError (CompatibilityError msg) -> 
      ToneDataError (InvalidData msg)
  | _ -> 
      ToneDataError (InvalidData "未知错误")

(** {1 错误处理实现} *)

let format_error = function
  | FileNotFound filename -> 
      Printf.sprintf "声调数据文件未找到: %s" filename
  | ParseError msg -> 
      Printf.sprintf "声调数据JSON解析错误: %s" msg
  | InvalidData msg -> 
      Printf.sprintf "声调数据格式无效: %s" msg

(** {1 数据加载接口实现} *)

let get_ping_sheng_chars () =
  try
    get_ping_sheng_chars_comprehensive ()
  with
  | ComprehensiveLoadError _ as e -> 
      raise (convert_comprehensive_to_tone_error e)
  | e -> 
      raise (ToneDataError (InvalidData (Printexc.to_string e)))

let get_shang_sheng_chars () =
  try
    get_shang_sheng_chars_comprehensive ()
  with
  | ComprehensiveLoadError _ as e -> 
      raise (convert_comprehensive_to_tone_error e)
  | e -> 
...
let get_qu_sheng_chars () =
  try
    get_qu_sheng_chars_comprehensive ()
  with
  | ComprehensiveLoadError _ as e -> 
      raise (convert_comprehensive_to_tone_error e)
  | e -> 
...
let get_ru_sheng_chars () =
  try
    get_ru_sheng_chars_comprehensive ()
  with
  | ComprehensiveLoadError _ as e -> 
      raise (convert_comprehensive_to_tone_error e)
  | e -> 
