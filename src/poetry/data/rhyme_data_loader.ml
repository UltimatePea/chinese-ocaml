(** 韵律数据加载器 - 第六阶段优化重构版本
    ...
    从318行优化为紧凑结构，消除重复逻辑，提升可维护性
    第六阶段技术债务清理：代码重复消除，通用化处理
...
type rhyme_group = AnRhyme | SiRhyme | TianRhyme | WangRhyme | QuRhyme | YuRhyme | HuaRhyme | FengRhyme | YueRhyme | JiangRhyme | HuiRhyme | UnknownRhyme
...
let ping_sheng_config = {
  ...
  subgroups = ["core_chars"; "jia_jia_series"; "qi_qi_series"; "extended_fish_chars"; "basic_chars"];
}
    ...
    try
      ...
      let rec find_value_end pos depth in_string =
        if pos >= json_len then pos
          ...
          | '"' when pos = 0 || json.[pos-1] <> '\\' -> find_value_end (pos + 1) depth (not in_string)
...
let parse_rhyme_category = function
  ...
  | "去声" -> QuSheng | "入声" -> RuSheng | _ -> PingSheng

let parse_rhyme_group = function
  | "AnRhyme" -> AnRhyme | "SiRhyme" -> SiRhyme | "TianRhyme" -> TianRhyme
  | "WangRhyme" -> WangRhyme | "QuRhyme" -> QuRhyme | "YuRhyme" -> YuRhyme
  | "HuaRhyme" -> HuaRhyme | "FengRhyme" -> FengRhyme | "YueRhyme" -> YueRhyme
  | "JiangRhyme" -> JiangRhyme | "HuiRhyme" -> HuiRhyme | _ -> UnknownRhyme
...
let load_rhyme_group_generic config =
  ...
  let load_single_group group_name =
    ...
    List.fold_left (fun acc subgroup_name ->
      acc @ (load_subgroup subgroup_name)
    ) [] config.subgroups
  in
  
  List.fold_left (fun acc group_name -> 
    acc @ (load_single_group group_name)
  ) [] config.groups
...
let load_complete_rhyme_database () =
  (load_ping_sheng_rhymes ()) @ (load_ze_sheng_rhymes ())
