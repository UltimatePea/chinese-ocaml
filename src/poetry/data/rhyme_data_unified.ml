...
let debug_log msg =
  if !debug_mode then
    Printf.eprintf "[RhymeDataUnified] %s\n" msg
...
let rebuild_character_index data_list =
  ...
  List.iter (fun item ->
    Hashtbl.replace character_rhyme_index item.character item
  ) data_list;
...
let rebuild_group_index data_list =
  ...
  List.iter (fun item ->
    let existing = match Hashtbl.find_opt rhyme_group_index item.rhyme_group with
      | Some lst -> lst
      | None -> []
    in
...
let rebuild_category_index data_list =
  ...
  List.iter (fun item ->
    let existing = match Hashtbl.find_opt rhyme_category_index item.rhyme_category with
      | Some lst -> lst
      | None -> []
    in
...
let rebuild_tone_index data_list =
  ...
  List.iter (fun item ->
    let existing = match Hashtbl.find_opt tone_index item.tone with
      | Some lst -> lst
      | None -> []
    in
...
let rebuild_all_indexes () =
  let (all_data, build_time) = measure_time (fun () ->
    let sources_with_priority = 
      Hashtbl.fold (fun source (loader, priority, _, _) acc ->
        (source, loader, priority) :: acc
      ) rhyme_sources []
    ...
    List.iter (fun (source, loader, _) ->
      ...
      | RhymeError err ->
          debug_log (Printf.sprintf "Failed to load from source: %s" err)
...
let register_rhyme_source source loader ?(priority=0) description =
  try
    ...
    RhymeSuccess ()
  with
  | exn -> RhymeError ("Failed to register rhyme source: " ^ (Printexc.to_string exn))

let get_available_sources () =
  Hashtbl.fold (fun source (_, priority, description, _) acc ->
    (source, description, priority) :: acc
  ) rhyme_sources []
...
let remove_rhyme_source source =
  try
    if Hashtbl.mem rhyme_sources source then (
      ...
      RhymeError "Rhyme source not found"
  with
  | exn -> RhymeError ("Failed to remove rhyme source: " ^ (Printexc.to_string exn))

(** {1 韵律查询实现} *)

let query_rhyme_data query =
  let (result, query_time) = measure_time (fun () ->
    performance_stats := { !performance_stats with total_queries = !performance_stats.total_queries + 1 };
        ...
        (match Hashtbl.find_opt rhyme_group_index group with
         | Some char_list ->
             let items = List.filter_map (fun char ->
               Hashtbl.find_opt character_rhyme_index char
             ) char_list in
             RhymeSuccess items
         | None -> RhymeSuccess [])
        ...
        (match Hashtbl.find_opt character_rhyme_index char with
         | Some item ->
             (match Hashtbl.find_opt rhyme_group_index item.rhyme_group with
              | Some char_list ->
                  let similar_items = List.filter_map (fun c ->
                    if c <> char then Hashtbl.find_opt character_rhyme_index c
                    else None
                  ) char_list in
    ...
    | RhymeCompatibilityQuery (char1, char2) ->
        let is_compatible = match 
          (Hashtbl.find_opt character_rhyme_index char1,
           Hashtbl.find_opt character_rhyme_index char2) with
        | (Some item1, Some item2) ->
            item1.rhyme_group = item2.rhyme_group
        | _ -> false
        in
...
let check_rhyme_compatibility char1 char2 =
  ...
  | Some result -> 
      performance_stats := { !performance_stats with cache_hits = !performance_stats.cache_hits + 1 };
      RhymeSuccess result
  | None ->
      let result = match 
        (Hashtbl.find_opt character_rhyme_index char1,
...
let analyze_rhyme_pattern char_list =
  ...
  List.iter (fun char ->
    match Hashtbl.find_opt character_rhyme_index char with
    | Some item ->
        let existing = match Hashtbl.find_opt group_map item.rhyme_group with
          | Some lst -> lst
          | None -> []
        in
  ...
  let patterns = Hashtbl.fold (fun group chars acc ->
    (group, List.rev chars) :: acc
  ) group_map [] in
...
let get_rhyme_statistics () =
  ...
  let conflicts = 0 in (* 简化实现，实际应该计算冲突数 *)
...
let validate_rhyme_data () =
  ...
  (* 检查空字符 *)
  Hashtbl.iter (fun char _ ->
    if char = "" then
      errors := "Empty character found" :: !errors
  ) character_rhyme_index;
  
  (* 检查索引一致性 *)
  Hashtbl.iter (fun group char_list ->
    List.iter (fun char ->
      if not (Hashtbl.mem character_rhyme_index char) then
        errors := (Printf.sprintf "Character %s in group index but not in character index" char) :: !errors
    ) char_list
  ) rhyme_group_index;
...
let resolve_conflicts_automatically () =
  (* 简化实现 - 实际应该基于优先级解决冲突 *)
  let resolved = Hashtbl.fold (fun char item acc ->
    (char, item) :: acc
  ) character_rhyme_index [] in
...
module FastRhyme = struct
  let build_rhyme_index source_list =
    try
      ...
      RhymeSuccess ()
    with
    | exn -> RhymeError ("Index build failed: " ^ (Printexc.to_string exn))
  
  let lookup_character_fast char =
    Hashtbl.find_opt character_rhyme_index char
  
  let lookup_rhyme_group_fast group =
    match Hashtbl.find_opt rhyme_group_index group with
    | Some char_list -> char_list
    | None -> []
  ...
  let rebuild_index () =
    try
      ...
      RhymeSuccess ()
    with
    | exn -> RhymeError ("Index rebuild failed: " ^ (Printexc.to_string exn))
  
  let get_index_status () =
    Hashtbl.fold (fun source (_, _, _, last_update) acc ->
...
module TraditionalRhyme = struct
  ...
  let get_ze_sheng_characters () =
    ...
    let all_ze_chars = List.fold_left (fun acc tone ->
      match Hashtbl.find_opt tone_index tone with
  ...
  let check_ping_ze_pattern char_list =
    let patterns = List.map (fun char ->
...
module ModernRhyme = struct
  ...
  let convert_traditional_to_modern group =
    find_rhyme_group_characters group
end
...
let export_rhyme_data query ~format =
      ...
      (match format with
       | `JSON ->
           let json_items = List.map (fun item ->
             Printf.sprintf "{\"character\":\"%s\",\"tone\":\"%s\"}"
               item.character
       ...
       | `CSV ->
           let csv_lines = "character,tone,rhyme_group" :: 
             (List.map (fun item ->
       ...
       | `YAML ->
           let yaml_items = List.map (fun item ->
  ...
  | RhymeError err -> RhymeError err
  | RhymeWarning (items, warn) -> 
      RhymeWarning ("Export completed with warnings", warn)
...
module Compatibility = struct
  let get_expanded_rhyme_database () =
    Hashtbl.fold (fun _ item acc ->
      (item.character, item.rhyme_category, item.rhyme_group) :: acc
    ) character_rhyme_index []
  
  let is_in_expanded_rhyme_database char =
    Hashtbl.mem character_rhyme_index char
  
  let get_an_yun_data () =
    (* 简化实现 - 兼容an_yun_data.ml *)
    Hashtbl.fold (fun _ item acc ->
      (item.character, "default_category", "default_group") :: acc
    ) character_rhyme_index []
  
  let get_feng_rhyme_data () =
    (* 简化实现 - 兼容feng_rhyme_data.ml *)
    Hashtbl.fold (fun _ item acc ->
      (item.character, "default_info") :: acc
    ) character_rhyme_index []
...
let get_memory_usage () =
  let char_index_size = Hashtbl.length character_rhyme_index * 100 in (* 估算 *)
...
let get_performance_metrics () =
  ...
  (stats.avg_query_time, stats.index_build_time *. 1000.0, 
