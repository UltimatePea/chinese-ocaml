...
val register_rhyme_source : rhyme_source -> 
...
val find_rhyme_group_characters : Poetry_core.Json_core.rhyme_group -> 
  string list rhyme_result
(** 查找韵组内所有字符 - O(1)性能 *)

val find_characters_by_tone : [`PingSheng | `ShangSheng | `QuSheng | `RuSheng] -> 
  rhyme_data_item list rhyme_result
(** 按声调查找字符 *)
...
val find_rhyming_characters : string -> 
  ?max_results:int -> 
  unit -> 
  string list rhyme_result
(** 查找与指定字符押韵的其他字符
...
val analyze_rhyme_pattern : string list -> 
  (Poetry_core.Json_core.rhyme_group * string list) list rhyme_result
(** 分析字符列表的韵律模式 *)

val suggest_rhyme_alternatives : string -> 
  (string * float) list rhyme_result
(** 建议韵律替代字符
...
val get_rhyme_statistics : unit -> 
  (int * int * int * int * int) rhyme_result
(** 获取韵律数据统计
...
val find_data_conflicts : unit -> 
  (string * rhyme_source * rhyme_source) list rhyme_result
(** 查找数据源间的冲突 
    
    @return (冲突字符, 数据源1, 数据源2) 列表 *)

val resolve_conflicts_automatically : unit -> 
  (string * rhyme_data_item) list rhyme_result
(** 自动解决数据冲突（基于优先级）
...
module FastRhyme : sig
  ...
  val lookup_rhyme_group_fast : Poetry_core.Json_core.rhyme_group -> 
    string list
  (** O(1) 韵组查找 - 高性能版本 *)
...
module TraditionalRhyme : sig
  ...
  val classify_tone : string -> [`PingSheng | `ShangSheng | `QuSheng | `RuSheng] option rhyme_result
  (** 声调分类 *)
  
  val check_ping_ze_pattern : string list -> [`Ping | `Ze] list rhyme_result
  (** 检查平仄模式 *)
  
  val suggest_ping_ze_correction : string list -> 
    (int * string * string) list rhyme_result
  (** 建议平仄修正
...
module ModernRhyme : sig
  ...
  val convert_traditional_to_modern : Poetry_core.Json_core.rhyme_group -> 
    string list rhyme_result
  (** 传统韵组到现代韵律的转换 *)
end
...
val export_rhyme_data : rhyme_query -> 
  format:[`JSON | `CSV | `XML | `YAML] -> 
  string rhyme_result
(** 导出韵律数据到不同格式 *)

val import_rhyme_data : rhyme_source -> 
  format:[`JSON | `CSV | `XML] -> 
  string -> 
  unit rhyme_result
(** 从外部格式导入韵律数据 *)
...
module Compatibility : sig
  val get_expanded_rhyme_database : unit -> 
...
val get_performance_metrics : unit -> 
  (float * float * int * int) 
