...
let build_lookup_table database =
  ...
  |> List.iter (fun group_data ->
    group_data.items
    |> List.iter (fun item ->
      Hashtbl.replace lookup_table item.character item));
  lookup_table
...
let build_group_index database =
  ...
  database.groups
  |> List.iter (fun group_data ->
    Hashtbl.replace group_index group_data.group group_data.items);
  group_index
...
let build_category_index database =
  ...
  |> List.iter (fun category ->
    Hashtbl.replace category_index category []);
...
let lookup_character character engine_state =
  if not engine_state.is_initialized then
    raise (RhymeDataEngineError "Engine not initialized");
  
  let _updated_stats, result =
    match Hashtbl.find_opt engine_state.lookup_table character with
    | Some item -> 
        (update_cache_hit engine_state.cache_stats, Some item)
    | None -> 
        (update_cache_miss engine_state.cache_stats, None)
  in
...
let get_group_characters group engine_state =
  ...
  match Hashtbl.find_opt engine_state.group_index group with
  | Some items -> items
  | None -> []
...
(** 清理缓存统计 *)
let clear_cache_stats engine_state =
  {
    engine_state with
    cache_stats = create_empty_cache_stats ();
  }
...
let find_similar_characters character engine_state =
  ...
  | Some item -> 
      ...
      |> List.filter (fun similar_item -> 
           similar_item.character <> character)

(** 批量查询字符 *)
let batch_lookup_characters characters engine_state =
  characters
  |> List.map (fun char -> (char, lookup_character char engine_state))
...
let get_performance_metrics engine_state =
  ...
  let hit_rate = 
    if stats.total_queries > 0 then 
      (float_of_int stats.hits) /. (float_of_int stats.total_queries)
    else 0.0
  in
