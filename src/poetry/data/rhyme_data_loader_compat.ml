...
let convert_comprehensive_error = function
  ...
  | ComprehensiveLoadError (UnifiedLoadError msg) -> 
      RhymeDataLoadError (RhymeValidationError msg)
  | ComprehensiveLoadError (CompatibilityError msg) -> 
      RhymeDataLoadError (RhymeValidationError msg)
  | _ -> 
      RhymeDataLoadError (RhymeValidationError "未知错误")

(** {1 错误处理实现} *)

let format_rhyme_error = function
  | RhymeFileNotFound filename -> 
      Printf.sprintf "韵律数据文件未找到: %s" filename
  | RhymeParseError (msg, detail) -> 
      Printf.sprintf "韵律数据解析错误: %s (详细: %s)" msg detail
  | RhymeValidationError msg -> 
      Printf.sprintf "韵律数据验证错误: %s" msg
...
let load_ping_sheng_rhymes () =
  ...
  with
  | ComprehensiveLoadError _ as e -> 
      raise (convert_comprehensive_error e)
  | e -> 
      raise (RhymeDataLoadError (RhymeValidationError (Printexc.to_string e)))
...
let get_rhyme_char_count database =
