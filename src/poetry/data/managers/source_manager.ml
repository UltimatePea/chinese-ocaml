...
let list_registered_sources () =
  Hashtbl.fold
    (fun source_id info acc -> (source_id, info.description, info.priority) :: acc)
    registered_sources []

(** 检查数据源是否已注册
    @param source_id 数据源标识符
    @return 是否已注册 *)
let is_source_registered source_id = Hashtbl.mem registered_sources source_id
...
let load_all_data () =
  let sources_with_priority =
    Hashtbl.fold
      (fun source_id info acc -> (source_id, info.loader, info.priority) :: acc)
      registered_sources []
      ...
      | Success data ->
          List.iter
            (fun item ->
              ...
              | _ -> Hashtbl.replace character_sources item.character source_id)
            data
      | Error _ -> () (* 忽略加载失败的数据源 *))
    source_list;

  if !conflicts = [] then Success [] else Success !conflicts
      ...
      | Success data ->
          List.iter
            (fun item ->
              ...
              | None -> Hashtbl.replace character_data item.character item)
            data
      | Error _ -> () (* 忽略加载失败的数据源 *))
    source_list;
...
let get_data_source_statistics () =
  ...
  let source_details =
    Hashtbl.fold
      (fun source_id info acc ->
        let data_count =
          match info.loader () with Success data -> List.length data | Error _ -> 0
        in
        (source_id, info.priority, data_count, info.register_time) :: acc)
      registered_sources []
  in

  { total_sources; source_details }
...
let print_performance_report () =
  ...
  Printf.printf "\nSource Details:\n";
  List.iter
    (fun (source_id, priority, data_count, register_time) ->
...
(** 清理所有数据源注册 *)
let clear_all_sources () = Hashtbl.clear registered_sources

(** 重新加载所有数据源 *)
let reload_all_sources () = load_all_data ()
