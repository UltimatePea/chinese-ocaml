...
let load_data_source source =
  match source with
  | FileSource filename ->
    begin match load_from_file filename with
    | Some content -> parse_data_content content
    | None -> []
    end
...
(** 获取数据条目 *)
let get_data_entry id =
  try Some (Hashtbl.find global_registry.entries id)
  with Not_found -> None
...
(** 删除数据条目 *)
let remove_data_entry id =
  Hashtbl.remove global_registry.entries id

(** 列出所有条目ID *)
let list_entry_ids () =
  Hashtbl.fold (fun key _value acc -> key :: acc) global_registry.entries []

(** 查询数据条目 *)
let query_entries predicate =
  Hashtbl.fold (fun _key value acc ->
    if predicate value then value :: acc else acc
...
let get_template template_id =
  try Some (Hashtbl.find global_registry.templates template_id)
  with Not_found -> None
...
(** 列出所有模板 *)
let list_templates () =
  Hashtbl.fold (fun key _value acc -> key :: acc) global_registry.templates []

(** {1 数据注册表管理} *)

(** 添加数据源 *)
let add_data_source source =
  global_registry.sources <- source :: global_registry.sources
...
let initialize_registry () =
  ...
  register_template "artistic_analysis" "艺术分析报告\n评价对象: {subject}\n整体评分: {overall_score}\n详细分析: {analysis}";
  
  (* 设置默认数据源 *)
  add_data_source (MemorySource [
...
let reload_all_sources () =
  List.iter (fun source ->
    ...
    List.iter (fun (key, value) ->
      set_data_entry key value [("source", "auto_loaded")]
    ) data
...
let export_data_to_string () =
  let entries = Hashtbl.fold (fun key value acc ->
    Printf.sprintf "%s:%s" key value.content :: acc
  ) global_registry.entries [] in
  String.concat "\n" entries

(** 导出模板到字符串 *)
let export_templates_to_string () =
  let templates = Hashtbl.fold (fun key value acc ->
    Printf.sprintf "模板[%s]:%s" key value :: acc
  ) global_registry.templates [] in
  String.concat "\n" templates
...
let validate_data_integrity () =
  ...
  Hashtbl.iter (fun key entry ->
    if entry.content = "" then
      errors := ("空内容条目: " ^ key) :: !errors;
    if entry.id <> key then
      errors := ("ID不匹配: " ^ key) :: !errors
  ) global_registry.entries;
  
  (* 检查模板有效性 *)
  Hashtbl.iter (fun key template ->
    if template = "" then
      errors := ("空模板: " ^ key) :: !errors
...
let batch_set_data data_list =
  List.iter (fun (id, content, metadata) ->
    set_data_entry id content metadata
  ) data_list
...
let restore_data backup_id =
  ...
  | Some backup_entry ->
    ...
    Hashtbl.clear global_registry.entries;
    List.iter (fun (key, value) ->
      set_data_entry key value [("source", "restored")]
    ) data;
    true
  | None -> false
