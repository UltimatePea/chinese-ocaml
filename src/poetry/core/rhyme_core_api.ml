(** 韵律核心API模块 - 骆言诗词编程特性
    ...
    整合来自 rhyme_api_core.ml、unified_rhyme_api.ml、poetry_recommended_api.ml 等模块的功能。
...
let get_rhyme_group_description (group : rhyme_group) : string =
  match List.assoc_opt group rhyme_group_descriptions with
  | Some desc -> desc
  | None -> "未知韵组描述"

(** 获取韵组的典型诗句示例 *)
let get_rhyme_group_examples (group : rhyme_group) : string list =
  match List.assoc_opt group example_poems_by_group with
...
let can_rhyme_together (char1 : string) (char2 : string) : rhyme_match_result =
  ...
  | Some entry1, Some entry2 ->
    ...
    let category_compatible = match entry1.category, entry2.category with
      ...
      | _ -> false
    in
    let quality = if group_match && category_compatible then 1.0
                 else if group_match then 0.7
                 else if category_compatible then 0.3
                 else 0.0 in
    let reason = if group_match && category_compatible then "同韵组同声调，完美押韵"
  ...
  | Some _target_entry ->
    List.filter_map (fun entry ->
      if entry.character <> char then
        ...
        if match_result.match_quality >= min_quality then
...
let analyze_character (char : string) ?(confidence : float = 1.0) () : char_rhyme_info option =
  ...
  | Some entry -> Some {
      character = char;
      rhyme_category = entry.category;
      rhyme_group = entry.group;
...
let analyze_verse (verse : string) ?config () : verse_rhyme_analysis =
  let _ = config in (* 消除未使用警告 *)
  let chars = List.init (String.length verse) (String.get verse) in
  let char_strings = List.map (String.make 1) chars in
  
  (* 分析每个字符 *)
  let char_analyses = List.filter_map (fun char_str ->
    analyze_character char_str ()
  ) char_strings in
  
  (* 确定韵脚 - 通常是最后一个字符 *)
  let rhyme_ending = if String.length verse > 0 then
    Some (String.sub verse (String.length verse - 1) 1)
  else None in
  
  (* 确定主要韵组和声韵类别 *)
  let group_counts = List.fold_left (fun acc analysis ->
    let count = try List.assoc analysis.rhyme_group acc + 1
               with Not_found -> 1 in
    (analysis.rhyme_group, count) :: List.remove_assoc analysis.rhyme_group acc
  ) [] char_analyses in
  
  let category_counts = List.fold_left (fun acc analysis ->
    let count = try List.assoc analysis.rhyme_category acc + 1
               with Not_found -> 1 in
    (analysis.rhyme_category, count) :: List.remove_assoc analysis.rhyme_category acc
  ) [] char_analyses in
  
  let dominant_group = match List.sort (fun (_, c1) (_, c2) -> compare c2 c1) group_counts with
    | (group, _) :: _ -> group
    | [] -> UnknownRhyme in
    
  let dominant_category = match List.sort (fun (_, c1) (_, c2) -> compare c2 c1) category_counts with
    | (category, _) :: _ -> category
    | [] -> PingSheng in
  
  (* 计算韵律质量评分 *)
  let quality_score = if List.length char_analyses > 0 then
    let total_confidence = List.fold_left (fun acc analysis -> 
      acc +. analysis.confidence) 0.0 char_analyses in
...
let analyze_poem (verses : string list) ?config () : poem_rhyme_analysis =
  ...
  let consistency_score = 
    let group_consistency = if List.length all_groups <= 2 then 1.0
                          else if List.length all_groups <= 4 then 0.7
                          else 0.4 in
    let category_consistency = if List.length all_categories <= 2 then 1.0
                             else 0.6 in
    (group_consistency +. category_consistency) /. 2.0 in
  
  (* 计算艺术质量评分 *)
  let artistic_score = 
    let avg_verse_quality = List.fold_left (fun acc analysis ->
      acc +. analysis.rhyme_quality_score) 0.0 verse_analyses 
      /. (float_of_int (List.length verse_analyses)) in
    (avg_verse_quality +. consistency_score) /. 2.0 in
  
  (* 生成改进建议 *)
  let suggestions = 
    let low_quality_verses = List.filter (fun analysis ->
      analysis.rhyme_quality_score < 0.6) verse_analyses in
    if List.length low_quality_verses > 0 then
      ["考虑优化韵律质量较低的诗句以提升整体效果"]
    else [] in
  
  let inconsistent_suggestions = 
    if consistency_score < 0.7 then
      ["建议统一韵律风格，减少韵组和声韵类别的混用"] @ suggestions
    else suggestions in
...
let cache_get (key : string) : 'a option =
  try Some (List.assoc key !query_cache)
  with Not_found -> None
