...
module Parser = struct
  ...
  let parse_rhyme_json json_content =
    try
      ...
      let parsed_groups =
        List.map (fun (group_name, group_json) ->
          let category = 
            Yojson.Safe.Util.member "category" group_json 
    ...
    | Yojson.Safe.Util.Type_error (msg, _) -> 
        raise (Json_parse_error ("类型错误: " ^ msg))
    | exn -> 
        raise (Json_parse_error ("未知解析错误: " ^ Printexc.to_string exn))
  
  (** 解析简化JSON格式（向后兼容） *)
  let parse_simple_json json_content =
    try
      parse_rhyme_json json_content
      ...
      try
        ...
        List.iter (fun line ->
          ...
          if String.contains trimmed ':' && not (String.contains trimmed '[') then (
            ...
            | Some (name, category) ->
                rhyme_groups := (name, { category; characters = List.rev !current_chars }) :: !rhyme_groups
            | None -> ());
...
module Io = struct
  ...
  let safe_read_file file_path =
    ...
    with
    | Sys_error msg -> 
        raise (Rhyme_data_not_found ("文件读取失败: " ^ file_path ^ " - " ^ msg))
    | exn -> 
        raise (Rhyme_data_not_found ("读取异常: " ^ file_path ^ " - " ^ Printexc.to_string exn))
...
let get_all_rhyme_groups ?(force_reload = false) () =
  match get_rhyme_data_safe ~force_reload () with
...
let get_rhyme_mappings ?(force_reload = false) () =
  ...
  List.iter (fun (group_name, group_data) ->
    ...
    in
    let rhyme_group = 
      match string_to_rhyme_group group_name with
      | Some grp -> grp
      | None -> UnknownRhyme
    in
    List.iter (fun char ->
