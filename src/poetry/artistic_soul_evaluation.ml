...
let analyze_imagery_deeply _code_text =
  {
    natural_imagery = [ "山"; "水" ];
    cultural_imagery = [ "道"; "理" ];
    philosophical_imagery = [ "循环"; "因果" ];
    emotional_imagery = [ "温暖"; "和谐" ];
    technical_metaphors = [ "递归如山"; "数据如水" ];
    poetic_connections = [ "代码如诗"; "算法如画" ];
  }

let analyze_cultural_content _code_text =
  {
    classical_references = [ "山重水复" ];
    philosophical_concepts = [ "中庸之道" ];
    literary_techniques = [ "比喻"; "对仗" ];
    cultural_symbols = [ "山"; "水" ];
...
let generate_soul_improvement_suggestions soul_dimensions =
  ...
  if soul_dimensions.literary_beauty < 0.7 then suggestions := "建议增加诗意表达，使用更多文学修辞手法" :: !suggestions;

  if soul_dimensions.cultural_depth < 0.6 then suggestions := "建议融入更多古典文化元素，增加文化底蕴" :: !suggestions;
...
let poetic_soul_critique code_text critic_style =
  ...
  let grade_comment =
    match soul_report.soul_grade with
