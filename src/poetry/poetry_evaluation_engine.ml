(** 骆言诗词评价引擎模块
    ...
    此模块实现诗词艺术性评价的核心算法。 从原poetry_artistic_core.ml模块中提取核心评价算法。
...
(** 评估诗句的韵律和谐度
    ...
    此函数分析诗句的韵律模式，评估音韵协调性和节奏美感。 通过检测韵脚、声调分布和音韵呼应来量化韵律质量。
...
let evaluate_rhyme_harmony verse =
  ...
  | _ ->
      ...
      let known_ratio =
        float_of_int (List.length rhyme_chars) /. float_of_int (List.length chinese_chars)
      in
  ...
  | _ -> (
      ...
      | Some expected ->
          ...
          for i = 0 to min_len - 1 do
            if List.nth tone_pattern i = List.nth expected i then incr matches
          done;
      ...
      | None ->
          ...
          for i = 0 to List.length tone_pattern - 2 do
            incr total_pairs;
            if List.nth tone_pattern i <> List.nth tone_pattern (i + 1) then incr alternations
          done;
          if !total_pairs = 0 then 0.5 else float_of_int !alternations /. float_of_int !total_pairs)
...
let evaluate_parallelism left_verse right_verse =
  ...
  let length_score =
    if left_len = right_len then 1.0
      ...
      1.0
      -. abs_float (float_of_int (left_len - right_len))
  ...
  let tone_score =
    if min_len = 0 then 0.0 else float_of_int !tone_matches /. float_of_int min_len
  in
...
(** 评估诗句的意象密度和艺术表现力
    ...
    此函数通过统计诗句中的意象关键词密度来评估艺术性。 意象词汇包括自然意象（山、水、花、月等）、情感意象和文化意象。
...
let evaluate_imagery verse =
  let imagery_count = Poetry_analysis_utils.count_imagery_words verse in
...
let evaluate_elegance verse =
  let elegant_count = Poetry_analysis_utils.count_elegant_words verse in
  let verse_len = String.length verse in
  if verse_len = 0 then 0.0
  else
    let base_score = min 1.0 (float_of_int elegant_count /. float_of_int verse_len *. 8.0) in
    (* 加上长度适中的奖励 *)
    let length_bonus = if verse_len >= 4 && verse_len <= 14 then 0.1 else 0.0 in
