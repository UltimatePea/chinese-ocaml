...
let generate_rhyme_report verse =
  ...
  let rhyme_ending =
    match rhyme_ending_char with Some char -> Some (String.make 1 char) | None -> None
  in
...
let analyze_poem_rhyme verses =
  ...
  let overall_rhyme_categories =
    List.map (fun report -> report.dominant_rhyme_category) verse_analyses
  in
  ...
  let verse_examples =
    List.mapi
      (fun i verse ->
        ...
        let ending_str = match report.rhyme_ending with Some str -> str | None -> "无" in
