...
let strings_rhyme str1 str2 =
  ...
  | Some (_, group1), Some (_, group2) -> rhyme_group_equal group1 group2 && group1 <> UnknownRhyme
  | _ -> false
  ...
  if len > 0 then
    let rec find_last_char i =
      if i < 0 then None
        ...
        let char = verse.[i] in
        if is_chinese_char char then Some char else find_last_char (i - 1)
    in
...
let verses_rhyme verse1 verse2 =
  ...
  match (ending1, ending2) with Some c1, Some c2 -> chars_rhyme c1 c2 | _ -> false
...
let detect_rhyme_pattern verses = List.filter_map extract_rhyme_ending verses
...
let validate_rhyme_consistency verses =
  ...
  | first :: rest ->
      ...
      && List.for_all (fun char -> rhyme_group_equal (detect_rhyme_group char) first_group) rest
...
let validate_specific_pattern verses pattern = validate_rhyme_scheme verses pattern
...
and evaluate_rhyme_harmony verses =
  let endings = detect_rhyme_pattern verses in
  let valid_rhymes = List.filter (fun char -> detect_rhyme_group char <> UnknownRhyme) endings in
  match endings with
  | [] -> 0.0
...
let get_rhyme_characters group =
  let data_list = get_all_rhyme_data () in
  List.fold_left
    (fun acc (str, _, g) -> if rhyme_group_equal g group then str :: acc else acc)
    [] data_list

let suggest_rhyme_characters group = get_rhyme_characters group

let find_rhyming_characters char =
  let group = detect_rhyme_group char in
  get_rhyme_characters group

let is_known_rhyme_char char = detect_rhyme_group char <> UnknownRhyme
...
let rec quick_rhyme_diagnosis verses =
  ...
  let diagnosis = if consistency then "韵律一致，押韵规范" else "韵律不一致，建议调整韵脚" in
...
let common_patterns =
  [
    ("七言绝句", [ 's'; 'h'; 'j'; 'x' ]); ("五言绝句", [ 'l'; 'c'; 'k' ]); ("律诗首联", [ 'f'; 'z'; 'z'; 'z' ]);
  ]
