...
module EvaluationFramework = struct
  ...
  (* 创建评价结果 *)
  let create_evaluation_result verse (rhyme, tone, parallelism, imagery, rhythm, elegance)
      suggestions =
    {
      verse;
      rhyme_score = rhyme;
...
let evaluate_wuyan_lushi verses =
  ...
  if Array.length verses != 8 then
    ...
    let tone_score =
      EvaluationFramework.calculate_tone_scores verses
        (Some (Array.of_list wuyan_lushi_standards.tone_pattern))
    in
    let parallelism_score =
      EvaluationFramework.calculate_parallelism_scores verses [ (2, 3); (4, 5) ]
    in
...
let evaluate_qiyan_jueju verses =
  ...
  if Array.length verses != 4 then
    ...
    let tone_score =
      EvaluationFramework.calculate_tone_scores verses
        (Some (Array.of_list qiyan_jueju_standards.tone_pattern))
    in
...
let evaluate_siyan_pianti verses =
  ...
  else EvaluationFramework.create_error_evaluation verses "输入内容为空"
...
let evaluate_siyan_parallel_prose verses =
  if Array.length verses mod 2 <> 0 then
    ...
    let tone_score =
      EvaluationFramework.calculate_tone_scores verses
        (Some (Array.of_list [ siyan_standards.tone_pattern ]))
    in
    let parallelism_score =
      EvaluationFramework.calculate_parallelism_scores verses parallelism_pairs
    in
    let imagery_score = evaluate_imagery verse_combined in
    let rhythm_score = evaluate_rhythm verse_combined in
    let elegance_score = evaluate_elegance verse_combined in
