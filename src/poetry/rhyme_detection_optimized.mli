...
module ChineseRhymeCache : sig
  ...
  val create_performance_cache :
    ...
    ?group_capacity:int ->
    unit ->
    rhyme_performance_cache
  (** 创建高性能缓存实例 *)

  val get_rhyme_with_stats :
    rhyme_performance_cache -> string -> (rhyme_category * rhyme_group) option
  (** 高性能韵律查找 - 支持缓存命中率监控 *)
...
val analyze_characters_batch_optimized :
  string list -> BatchCharacterProcessor.batch_analysis_result
(** 批量字符处理接口 *)
