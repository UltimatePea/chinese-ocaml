...
let rhyme_group_from_string s =
  if s = "" || s = "unknown" then UnknownRhyme
  else RhymeGroup s
...
let parse_poetry_patterns_from_json (json : Yojson.Basic.t) : (string * poetry_pattern list) =
  ...
  try
    ...
    let patterns = List.map (fun pattern_json ->
      ...
      let rhyme_positions = pattern_json |> member "rhyme_positions" |> to_list |> List.map to_int in
...
let lookup_char_info (char : string) : char_info option =
  ...
  | Success json ->
      ...
      (try Some (List.assoc char char_info_list) 
       with Not_found -> None)
  | Failure _ -> None
...
let get_chars_by_condition (condition : char_info -> bool) : string list =
  ...
  | Success json ->
      let char_info_list = parse_char_info_from_json json in
      List.filter_map (fun (char, info) ->
        if condition info then Some char else None
      ) char_info_list
  | Failure _ -> []
...
let initialize () =
  ...
  if not !initialized then (
    ...
    Unified_data_engine.register_data_source 
      rhyme_data_source 
      Poetry 
      (JsonFile "data/poetry/rhyme_data.json") 
      Cached;
    
    Unified_data_engine.register_data_source 
      tone_data_source 
      Poetry 
      (JsonFile "data/poetry/tone_data.json") 
      Cached;
    
    Unified_data_engine.register_data_source 
      pattern_data_source 
      Poetry 
      (JsonFile "data/poetry/poetry_patterns.json") 
      Cached;
    
    Unified_data_engine.register_data_source 
      char_info_source 
      Poetry 
      (JsonFile "data/poetry/char_info.json") 
      Preloaded;
...
let get_chars_by_rhyme_group (rhyme_group : rhyme_group) : string list query_result =
  ...
  with exn ->
    QueryError ("查询韵组字符失败: " ^ Printexc.to_string exn)
...
let get_chars_by_tone (tone : tone_type) : string list query_result =
  ...
  try
    ...
    if chars = [] then NotFound else Found chars
  with exn ->
    QueryError ("查询声调字符失败: " ^ Printexc.to_string exn)
...
let get_rhyme_endings (rhyme_group : rhyme_group) : rhyme_ending_info list query_result =
  ...
  | Found chars ->
      let endings = List.map (fun char ->
        {
          char;
          rhyme_group;
          usage_frequency = 1; (* 简化实现，实际应从统计数据获取 *)
          example_poems = []; (* 简化实现 *)
        }
      ) chars in
      Found endings
...
let analyze_tone_pattern (text : string) : bool list query_result =
  ...
  try
    ...
    let tones = List.map (fun char_str ->
    ...
    if List.for_all (function Some _ -> true | None -> false) tones then
      ...
      QueryError "文本包含无法识别声调的字符"
  with exn ->
    QueryError ("声调分析失败: " ^ Printexc.to_string exn)

let validate_tone_pattern (text : string) (expected_pattern : bool list) : bool query_result =
  match analyze_tone_pattern text with
  | Found actual_pattern -> 
      Found (actual_pattern = expected_pattern)
  | NotFound -> NotFound
  | QueryError err -> QueryError err
...
let get_poetry_patterns (poetry_type : string) : poetry_pattern list query_result =
  ...
  | Success json ->
      ...
      let filtered_patterns = List.filter (fun pattern ->
        String.contains pattern.name (String.get poetry_type 0) (* 简化匹配 *)
      ) all_patterns in
      if filtered_patterns = [] then NotFound else Found filtered_patterns
  | Failure err ->
      QueryError ("加载格律模式失败: " ^ Unified_data_engine.format_error err)

let search_chars_by_criteria ?tone ?rhyme_group ?rhyme_category () : string list query_result =
  if not !initialized then initialize ();
  try
    let condition info =
      let tone_match = match tone with
        | None -> true
        | Some t -> info.tone = t
      in
      let rhyme_group_match = match rhyme_group with
        | None -> true
        | Some rg -> info.rhyme_group = rg
      in
      let rhyme_category_match = match rhyme_category with
        | None -> true
        | Some rc -> info.rhyme_category = rc
      in
...
let analyze_poem_rhyme_scheme (lines : string list) : (int * rhyme_group) list query_result =
  ...
  try
    ...
    let rhyme_info = List.mapi (fun i line ->
  ...
  with exn ->
    QueryError ("分析押韵方案失败: " ^ Printexc.to_string exn)
...
let get_rhyme_group_statistics () : (rhyme_group * int) list query_result =
  ...
  | Failure err ->
      QueryError ("获取韵组统计失败: " ^ Unified_data_engine.format_error err)
...
let get_tone_distribution () : (tone_type * int) list query_result =
  ...
  | Success json ->
      ...
      Found stats
  | Failure err ->
      QueryError ("获取声调分布失败: " ^ Unified_data_engine.format_error err)

let get_data_source_info () : (string * string * int) list =
  if not !initialized then initialize ();
  let sources = Unified_data_engine.list_registered_sources () in
  List.filter_map (fun (name, category, source, _) ->
    if category = Poetry then
      let source_type = match source with
        | JsonFile _ -> "JSON文件"
        | TextFile _ -> "文本文件"
        | CsvFile _ -> "CSV文件"
        | Embedded _ -> "内嵌数据"
