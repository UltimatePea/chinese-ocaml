...
let lookup_word_info (word : string) : word_info option =
  ...
  | Success json -> (
      ...
      try Some (List.assoc word word_info_list) with Not_found -> None)
  | Failure _ -> None
...
let get_words_by_condition (condition : word_info -> bool) : string list =
  ...
  | Success json ->
      let word_info_list = parse_word_info_from_json json in
      List.filter_map
        (fun (word, info) -> if condition info then Some word else None)
        word_info_list
  | Failure _ -> []
...
let get_word_info (word : string) : word_info query_result =
  match lookup_word_info word with Some info -> Found info | None -> NotFound
...
let get_words_by_category (category : word_category) : string list query_result =
  ...
  with exn -> QueryError ("查询类别词汇失败: " ^ Printexc.to_string exn)

let search_words_by_pattern (pattern : string) : string list query_result =
  try
    let pattern_regex = Str.regexp pattern in
    let words = get_words_by_condition (fun info -> Str.string_match pattern_regex info.word 0) in
    if words = [] then NotFound else Found words
  with exn -> QueryError ("模式搜索失败: " ^ Printexc.to_string exn)
...
let get_high_value_words (category : word_category) (limit : int) :
    (string * float) list query_result =
  match Unified_data_engine.load_json_data word_info_source with
  | Success json ->
      let parsed_word_info : (string * word_info) list = parse_word_info_from_json json in
      let category_words =
        List.filter (fun (_, (info : word_info)) -> info.category = category) parsed_word_info
      in
      let sorted_words =
        List.sort
          (fun (_, info1) (_, info2) -> compare info2.artistic_value info1.artistic_value)
          category_words
      in
  ...
  | Failure err -> QueryError ("获取高价值词汇失败: " ^ Unified_data_engine.format_error err)

(** {1 评价标准查询} *)

let get_evaluation_standards (dimension : evaluation_dimension) :
    evaluation_standard list query_result =
  ...
  | Success json -> (
      ...
      try Found (List.assoc dimension standards_list) with Not_found -> NotFound)
  | Failure err -> QueryError ("获取评价标准失败: " ^ Unified_data_engine.format_error err)
...
let get_artistic_templates (category : word_category) : artistic_template list query_result =
  ...
  | Success json -> (
      ...
      try Found (List.assoc category templates_list) with Not_found -> NotFound)
  | Failure err -> QueryError ("获取艺术模板失败: " ^ Unified_data_engine.format_error err)
...
let get_word_category_statistics () : (word_category * int) list query_result =
  ...
  | Success json ->
      ...
      let stats =
        Hashtbl.fold (fun category count acc -> (category, count) :: acc) category_counts []
      in
      Found stats
