(** 韵律数据JSON加载器 - 骆言诗词编程特性技术债务重构
    ...
    此模块负责从JSON文件加载诗词韵律数据，替代原有的硬编码数据结构。
    实现数据与代码分离，提高可维护性和扩展性。
...
type rhyme_series = {
  name: string;
  description: string;
  characters: rhyme_char_data list;
}
...
let rhyme_group_of_string = function
  ...
  | "XueRhyme" -> XueRhyme | "JiangRhyme" -> JiangRhyme | "HuiRhyme" -> HuiRhyme
  | _ -> UnknownRhyme
...
let get_hui_rhyme_data () =
  ...
  | None ->
    let data = load_rhyme_data_from_json "data/poetry/rhyme_groups/ze_sheng/hui_rhyme_data.json" in
