...
module ParallelismEvaluator : EVALUATOR = struct
  ...
  let evaluate context =
    ...
    let contains_parallelism =
      List.exists
        (fun keyword -> String.contains context.verse (String.get keyword 0))
...
module RhythmEvaluator : EVALUATOR = struct
  ...
  let evaluate context =
    ...
    let structure_score =
      if
        String.contains context.verse (String.get "，" 0)
        || String.contains context.verse (String.get "。" 0)
      then 0.8
      else 0.6
    in
...
module EleganceEvaluator : EVALUATOR = struct
  ...
  let evaluate context =
    ...
    let elegant_count =
      List.fold_left
        (fun acc char -> if String.contains context.verse (String.get char 0) then acc + 1 else acc)
        0 elegant_chars
    in
