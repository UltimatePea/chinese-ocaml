(** 骆言诗词形式评价器模块
    ...
    此模块提供针对不同诗词形式的专用评价器。 从原poetry_artistic_core.ml模块中提取诗体专用评价功能。
...
let evaluate_siyan_parallel_prose verses =
  ...
  if verse_count < 2 then
    Poetry_evaluation_engine.comprehensive_artistic_evaluation
      (if verse_count > 0 then verses.(0) else "")
      None
  else
    (* 四言骈体要求两两对仗 *)
...

    {
      ...
      overall_grade =
        Poetry_evaluation_engine.determine_overall_grade
          {
...
let poetic_critique verse form =
  ...
  let critique_suggestions =
    match form with
    | SiYanPianTi | SiYanParallelProse -> [ "骈体之美在于对仗工整，声韵和谐" ]
    | WuYanLuShi -> [ "律诗之妙在于格律严谨，意境深远" ]
    | QiYanJueJu -> [ "绝句之巧在于起承转合，言简意赅" ]
    | _ -> [ "诗词之道，在于意境与声韵并重" ]
  in
...
let poetic_aesthetics_guidance verse form =
  ...
  let guidance =
    match form with
    | SiYanPianTi | SiYanParallelProse -> [ "注重对偶工整"; "追求声律和谐"; "用词典雅得体" ]
    | WuYanLuShi -> [ "遵循格律规范"; "中间两联对仗"; "首尾呼应" ]
    | QiYanJueJu -> [ "起句立意"; "承句发展"; "转句生变"; "合句收束" ]
    | _ -> [ "意境为先"; "声韵和谐"; "用词精练" ]
  in
