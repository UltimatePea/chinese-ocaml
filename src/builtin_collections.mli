(** 骆言内置集合操作函数模块接口 - Chinese Programming Language Builtin Collection Functions Module Interface
    ...
    这个模块提供了骆言编程语言的集合（列表、字符串）操作相关内置函数，
    包括长度查询、连接、过滤、映射、折叠、排序等高阶函数功能。集合操作
...
(** 列表过滤函数（柯里化）
    ...
    这是一个柯里化函数：
    1. 第一次调用传入谓词函数（接受单个元素，返回布尔值）
    2. 返回的函数接受列表，返回满足谓词条件的元素组成的新列表
...
(** 列表映射函数（柯里化）
    ...
    这是一个柯里化函数：
    1. 第一次调用传入变换函数（接受单个元素，返回变换后的值）
    2. 返回的函数接受列表，返回每个元素经过变换后组成的新列表
...
(** 列表折叠函数（柯里化）
    ...
    这是一个柯里化函数，实现左折叠（fold_left）语义：
    1. 第一次调用传入折叠函数（接受累积值和当前元素，返回新的累积值）
    2. 返回的函数需要继续柯里化，依次接受初始值和列表
...
(** 列表排序函数
    ...
    使用默认的比较函数对列表元素进行排序，支持同质数据类型的列表。
    返回一个新的已排序列表，不修改原列表。
...
(** 集合包含检查函数（柯里化）
    ...
    这是一个柯里化函数：
    1. 第一次调用传入要搜索的集合（列表或字符串）
    2. 返回的函数接受要查找的元素，返回是否包含的布尔值
