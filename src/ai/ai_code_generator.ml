...
let generate_function_code (description: string) (_context: string option) : generation_result =
  ...
  | (best_template, confidence) :: alternatives ->
...

    {
      ...
      quality_metrics = {
        syntax_correctness = 0.95; (* 模板都是语法正确的 *)
        chinese_compliance = 0.90; (* 使用中文编程规范 *)
        readability = 0.85;        (* 代码可读性良好 *)
...
(* 生成数据处理代码 *)
let generate_data_processing_code (operations: data_operation list) (_description: string) : generation_result =
...
(* 智能代码生成接口 *)
let intelligent_code_generation (description: string) ?(context: string option = None) ?(constraints: generation_constraint list = []) () : generation_result =
  ...
  List.iteri (fun i description ->
    ...
    if List.length result.alternatives > 0 then (
      ...
      List.iteri (fun j alt ->
        Printf.printf "  %d. %s (置信度: %.0f%%)\n" (j + 1) alt.alt_description (alt.alt_confidence *. 100.0)
      ) result.alternatives;
      Printf.printf "\n"
    );
