  ...
  let rec split_by_separators str seps =
    ...
    | sep :: rest_seps ->
        ...
        List.fold_left (fun acc part ->
          acc @ (split_by_separators part rest_seps)
        ) [] parts
  in
...
let extract_semantic_units (text: string) : semantic_unit list =
  ...
  List.map (fun word ->
    ...
    let confidence = match word_type with
...
let identify_intent (semantic_units: semantic_unit list) : programming_intent =
  ...
  if has_word_type (function Verb "define" | Verb "create" -> true | _ -> false) &&
    ...
    let function_name = match identifiers with
      | name :: _ -> name
      | [] -> "未知函数"
    in
...
let code_to_natural_language (code: string) : string =
  (* 简化的代码分析 *)
  if (try let _ = Str.search_forward (Str.regexp_string "递归") code 0 in true with Not_found -> false) then
    "这是一个递归函数定义"
  else if (try let _ = Str.search_forward (Str.regexp_string "匹配") code 0 in true with Not_found -> false) then
    "这是一个模式匹配表达式"
  else if (try let _ = Str.search_forward (Str.regexp_string "如果") code 0 in true with Not_found -> false) then
    "这是一个条件判断语句"
  else if (try let _ = Str.search_forward (Str.regexp_string "让") code 0 in true with Not_found -> false) then
    "这是一个变量定义"
  else
    "这是一段骆言代码"
