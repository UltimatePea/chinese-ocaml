 ...
 * - 基于类型信息生成参数和返回值说明
 * - 智能生成使用示例
 * - 支持多种输出格式（Markdown、HTML、OCamlDoc）
 * - 中文表达优化，符合中文阅读习惯
 *)
...
let analyze_parameter_usage (param: string) (body: simple_expr) : string =
  let rec analyze_expr expr =
    ...
    | SFunction (_, args) ->
      List.flatten (List.map analyze_expr args)
...
let detect_function_features (func_info: function_info) : string list =
  ...
  let rec check_expr expr =
    ...
    | SBinary (left, _, right) -> check_expr left; check_expr right
...
(** 生成模块级文档 *)
let generate_module_documentation (module_name: string) (functions: function_info list) (config: doc_generation_config) : module_doc =
  let function_docs = List.map (fun func_info ->
...

  {
    ...
    types = []; (* 类型提取待后续实现 *)
    dependencies = []; (* 依赖分析待后续实现 *)
    usage_guide;
  }

(** 主要API：为单个函数生成文档 *)
let generate_function_doc (func_info: function_info) (config: doc_generation_config) : generated_doc =
