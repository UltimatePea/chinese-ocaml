...
let analyze_code_intent (code: string) : string =
  ...
  | best_match :: _ ->
      Printf.sprintf "检测到 %s 模式 (置信度: %.0f%%)\n描述: %s"
        ...
        best_match.pattern.description
...
let format_pattern_match (pattern_match: pattern_match) : string =
  let params_str = String.concat ", " (List.map (fun (k, v) -> k ^ "=" ^ v) pattern_match.extracted_params) in
  Printf.sprintf "模式: %s [%.0f%%]\n分类: %s (复杂度: %d)\n描述: %s\n参数: %s\n示例: %s"
    pattern_match.pattern.name
    (pattern_match.confidence *. 100.0)
    pattern_match.pattern.category
    pattern_match.pattern.complexity
