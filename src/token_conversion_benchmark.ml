...
let create_test_tokens () =
  [
    ...
    MatchKeyword; WithKeyword; InKeyword; RecKeyword; AndKeyword;
    
    (* 语义关键字 *)
    AsKeyword; CombineKeyword; WithOpKeyword; WhenKeyword;
    
    (* 错误恢复关键字 *)
    ExceptionKeyword; RaiseKeyword; TryKeyword; CatchKeyword;
    
    (* 模块系统关键字 *)
    ModuleKeyword; TypeKeyword; RefKeyword; IncludeKeyword;
    
    (* 自然语言关键字 *)
    DefineKeyword; AcceptKeyword; MultiplyKeyword; DivideKeyword;
    
    (* 文言文关键字 *)
    HaveKeyword; SetKeyword; AsForKeyword; NumberKeyword;
    
    (* 古雅体关键字 - 低频使用 *)
    IfWenyanKeyword; ThenWenyanKeyword; AncientDefineKeyword; AncientEndKeyword;
  ]
...
let benchmark_original_version tokens iterations =
  ...
  for _i = 1 to iterations do
    List.iter (fun token ->
      try ignore (Token_conversion_keywords.convert_basic_keyword_token token)
      with _ -> ()
    ) tokens
  done;
...
let benchmark_optimized_version tokens iterations =
  ...
  for _i = 1 to iterations do
    List.iter (fun token ->
      try ignore (Token_conversion_keywords_refactored.convert_basic_keyword_token_optimized token)
      with _ -> ()
    ) tokens
  done;
...
let generate_performance_report () =
  ...
  let memory_saving_refactored = ((float_of_int original_memory) -. (float_of_int refactored_memory)) /. (float_of_int original_memory) *. 100.0 in
  let memory_saving_optimized = ((float_of_int original_memory) -. (float_of_int optimized_memory)) /. (float_of_int original_memory) *. 100.0 in
  ...
  Printf.printf "\n🎯 重构效果评估:\n";
  if refactored_speedup > 1.05 && optimized_speedup > 1.1 then
    Printf.printf "  ✅ 重构成功! 显著提升了性能\n"
  else if refactored_speedup > 0.95 && optimized_speedup > 1.0 then
    Printf.printf "  ✅ 重构良好! 保持或改善了性能\n"
  else
    Printf.printf "  ⚠️  需要进一步优化\n";
  
  Printf.printf "\n📈 建议:\n";
  if optimized_speedup > refactored_speedup *. 1.2 then
    Printf.printf "  • 建议逐步迁移到优化版本以获得最佳性能\n";
