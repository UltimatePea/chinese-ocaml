  ...
  if next_pos >= state.length then true (* 文件结尾 *)
    ...
    | ValidChar (next_char, _) ->
        if String.length next_char = 1 then
          ...
          (* 使用增强的字符分类 *)
          CharacterDetection.is_separator_char c
          || c = ' ' || c = '\t' || c = '\n' || CharacterDetection.is_digit c
          ||
          (* 允许关键字后面跟数字 *)
...
let count_utf8_chars sequence = UTF8Processing.count_utf8_chars sequence
...
let create_keyword_state state keyword_len =
  ...
  let char_count = count_utf8_chars keyword_text in
  {
    ...
    current_column = state.current_column + char_count;
    (* 使用字符数而不是字节数 *)
  }
...
let try_keyword_or_error state pos =
  ...
  with EnhancedLexError (msg, _error_pos, suggestion) ->
    (* 转换为原有的错误类型以保持兼容性 *)
    let enhanced_msg =
      match suggestion with Some alt -> msg ^ Printf.sprintf " [建议: %s]" alt | None -> msg
    in
...
let handle_letter_or_chinese_char state pos =
  ...
  | EndOfInput -> raise (LexError ("意外的输入结束", pos))
