...
let check_single_arg args function_name =
  check_args_with_pattern `Single args function_name

(** 双参数检查 *)
let check_double_args args function_name =
  check_args_with_pattern `Double args function_name

(** 无参数检查 *)
let check_no_args args function_name =
  check_args_with_pattern `None args function_name
...
let validate_type_with_context validator function_name value =
  let context = make_builtin_error_context ~function_name () in
  validate_with_context validator context value
  |> function
  | Ok result -> result
...
let check_array_bounds index array_length function_name =
  if index < 0 || index >= array_length then
    runtime_error (generic_function_error function_name 
