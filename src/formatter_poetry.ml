(** 骆言编译器诗词格式化模块
    ...
    本模块专注于诗词分析和格式化功能，从unified_formatter.ml中拆分出来。 提供统一的诗词格式化接口，消除Printf.sprintf依赖。
...
module PoetryFormatting = struct
  ...
  let verse_analysis verse_num verse ending_str rhyme_group =
    concat_strings
      [ "第"; int_to_string verse_num; "句："; verse; "，韵脚："; ending_str; "，韵组："; rhyme_group ]

  (** 诗词结构分析 *)
  let poetry_structure_analysis poem_type expected_lines actual_lines =
    concat_strings
      [
        poem_type; "结构分析：期望"; int_to_string expected_lines; "句，实际"; int_to_string actual_lines; "句";
      ]

  (** Phase 3C 新增格式化函数 *)

  (** 文本长度信息格式化 *)
  let format_text_length_info length = concat_strings [ "文本长度: "; int_to_string length; " 字符\n" ]
  ...
  let format_rhyme_data_stats series_count char_count =
    concat_strings
      [ "韵律数据统计: "; int_to_string series_count; "个系列, "; int_to_string char_count; "个字符" ]
  ...
  (** 数据加载错误格式化 *)
  let format_data_loading_error context error_msg = concat_strings [ context; ": "; error_msg ]

  (** 字符组查找错误格式化 *)
  let format_group_not_found_error group_name = concat_strings [ "字符组 '"; group_name; "' 不存在" ]

  (** JSON解析错误格式化 *)
  let format_json_parse_error operation error_msg = concat_strings [ operation; ": "; error_msg ]
  ...
  let format_hui_rhyme_stats version total_chars series_count description =
    concat_strings
      [
        ...
        version;
        "\n- 总字符数: ";
        int_to_string total_chars;
        "\n- 系列数: ";
        int_to_string series_count;
        "\n- 描述: ";
        description;
      ]
  ...
  let format_data_integrity_failure expected actual =
    concat_strings
      [ "❌ 数据完整性验证失败: 期望"; int_to_string expected; "个字符，实际"; int_to_string actual; "个字符" ]

  let format_data_integrity_exception error_msg = concat_strings [ "❌ 数据完整性验证异常: "; error_msg ]
end
...
module ClassicalFormatting = struct
  ...
  let format_prosody_analysis verses rhyme_scheme tone_pattern =
    ...
    let verse_analysis =
      List.mapi (fun i verse -> concat_strings [ "第"; int_to_string (i + 1); "句: "; verse ]) verses
    in
...
module AncientStyleFormatting = struct
  ...
  let format_classical_chinese title content annotations =
    ...
    if List.length annotations > 0 then
      ...
      let formatted_annotations =
        List.mapi (fun i note -> concat_strings [ int_to_string (i + 1); ". "; note ]) annotations
      in
      join_with_separator "\n" (header :: main_text :: annotation_section :: formatted_annotations)
    else join_with_separator "\n" [ header; main_text ]
  ...
  let format_ancient_verse poem_title verses style_notes =
    ...
    in
    let style_section =
      if List.length style_notes > 0 then
  ...
  let format_parallel_prose title paragraphs =
    ...
    let formatted_paragraphs =
      List.mapi
        (fun i para -> concat_strings [ "第"; int_to_string (i + 1); "段:\n"; para ])
        paragraphs
    in
  ...
  let format_fu_poem title author sections =
    ...
    let formatted_sections =
      List.map
        (fun (section_name, content) -> concat_strings [ section_name; ":\n"; content ])
        sections
    in
...
module PoetryAnalysisTools = struct
  ...
  let format_character_frequency chars_with_counts =
    ...
    let formatted_items =
      List.map
        (fun (char, count) -> concat_strings [ "  "; char; ": "; int_to_string count; "次" ])
        sorted_chars
    in
  ...
  let format_rhyme_pattern_analysis patterns =
    ...
    let formatted_patterns =
      List.mapi (fun i pattern -> concat_strings [ int_to_string (i + 1); ". "; pattern ]) patterns
    in
    join_with_separator "\n" (header :: formatted_patterns)

  (** 声律检查报告 *)
  let format_prosody_check_report violations corrections =
    let violation_section =
      if List.length violations > 0 then
        let header = "声律违例:" in
        let formatted_violations =
          List.map (fun violation -> concat_strings [ "- "; violation ]) violations
        in
        header :: formatted_violations
      else [ "声律检查: 无违例" ]
    in

    let correction_section =
      if List.length corrections > 0 then
        let header = "修正建议:" in
        let formatted_corrections =
          List.map (fun correction -> concat_strings [ "- "; correction ]) corrections
        in
  ...
  let format_style_analysis_report poem_title style_features similarity_scores =
    ...
    let formatted_features =
      List.map (fun feature -> concat_strings [ "- "; feature ]) style_features
    in

    let similarity_section = "相似度评分:" in
    let formatted_similarities =
      List.map
        (fun (poet_name, score) -> concat_strings [ "- "; poet_name; ": "; float_to_string score ])
        similarity_scores
    in
  ...
  let format_thematic_vocabulary_analysis themes =
    ...
    let formatted_themes =
      List.map
        (fun (theme_name, words) ->
          let word_list = join_with_separator "、" words in
          concat_strings [ "- "; theme_name; ": "; word_list ])
        themes
    in
