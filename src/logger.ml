...
(** 设置日志级别 *)
let set_level level =
  global_config.current_level <- level

(** 设置是否显示时间戳 *)
let set_show_timestamps enabled =
  global_config.show_timestamps <- enabled

(** 设置是否显示模块名 *)
let set_show_module_name enabled =
  global_config.show_module_name <- enabled

(** 设置输出通道 *)
let set_output_channel channel =
  global_config.output_channel <- channel
...
let format_message level module_name message =
  let timestamp = if global_config.show_timestamps then 
    "[" ^ get_timestamp () ^ "] " else "" in
  let module_part = if global_config.show_module_name then 
    "[" ^ module_name ^ "] " else "" in
  let level_str = level_to_string level in
  let color = level_to_color level in
  Printf.sprintf "%s%s%s%s[%s] %s%s"
    timestamp module_part color level_str level_str message reset_color
...
let time_operation module_name operation_name f =
  ...
  | e ->
    let end_time = Unix.gettimeofday () in
    let duration = end_time -. start_time in
    error module_name (Printf.sprintf "失败 %s (耗时: %.3f秒): %s" 
