...
(** 更新运行时配置字段 *)
let update_debug_mode debug = current := { !current with debug_mode = debug }

let update_verbose_logging verbose = current := { !current with verbose_logging = verbose }
let update_log_level level = current := { !current with log_level = level }
let update_max_error_count count = current := { !current with max_error_count = count }
