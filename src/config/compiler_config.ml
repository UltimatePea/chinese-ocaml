...
(** 更新编译器配置字段 *)
let update_buffer_size size = current := { !current with buffer_size = size }

let update_compilation_timeout timeout = current := { !current with compilation_timeout = timeout }
let update_output_directory dir = current := { !current with output_directory = dir }
let update_temp_directory dir = current := { !current with temp_directory = dir }
let update_c_compiler compiler = current := { !current with c_compiler = compiler }
let update_optimization_level level = current := { !current with optimization_level = level }
...
let env_var_mappings =
  [
    ( "CHINESE_OCAML_BUFFER_SIZE",
      fun v ->
        match parse_positive_int_env_var v with Some size -> update_buffer_size size | None -> () );
