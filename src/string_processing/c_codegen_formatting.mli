...
(** [function_call func_name args] 
    生成函数调用代码，格式为 "func_name(arg1, arg2, ...)" *)
val function_call : string -> string list -> string

(** [binary_function_call func_name e1_code e2_code] 
    生成双参数函数调用代码 *)
val binary_function_call : string -> string -> string -> string

(** {1 特定类型的C代码生成} *)

(** [string_equality_check expr_var escaped_string] 
    生成字符串相等性检查代码 *)
val string_equality_check : string -> string -> string

(** [type_conversion target_type expr] 
    生成类型转换代码，格式为 "(target_type)expr" *)
val type_conversion : string -> string -> string

(** {1 环境操作代码生成} *)

(** [env_bind var_name expr_code] 
    生成环境绑定代码 *)
...
(** [recursive_binding var_name expr_code] 
    生成递归函数绑定代码 *)
val recursive_binding : string -> string -> string

(** {1 C语言控制结构} *)

(** [if_statement condition then_code else_code_opt] 
    生成if语句代码，支持可选的else分支 *)
...
(** [function_declaration return_type func_name params] 
    生成函数声明代码 *)
