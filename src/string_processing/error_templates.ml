...
let unexpected_state state context = Printf.sprintf "意外的状态: %s (上下文: %s)" state context

[@@@warning "-32"]

let invalid_character char = Printf.sprintf "无效字符: %c" char

[@@@warning "-32"]

let syntax_error message position = Printf.sprintf "语法错误 %s: %s" position message
...
let poetry_verse_count_warning count = Printf.sprintf "绝句包含%d句，通常为4句" count
