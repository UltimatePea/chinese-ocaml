(** 骆言编译器Token字符串表示格式化器 - Printf.sprintf重复消除重构
    ...
    此模块提供统一的Token字符串格式化接口，完全消除Printf.sprintf依赖。 使用Base_formatter底层基础设施，实现零重复的Token格式化。
...
module Token_formatter = struct
  ...
  let token_with_position token_str line column = token_position_pattern token_str line column

  (** Token序列格式化 - 使用Base_formatter *)
  let token_sequence tokens = list_format tokens
  ...
  let function_call_token func_name args =
    token_pattern "FunctionCallToken" (concat_strings [ func_name; ", "; list_format args ])

  let module_access_token module_name member_name =
    token_pattern "ModuleAccessToken" (module_access_format module_name member_name)
