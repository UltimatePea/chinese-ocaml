...
let stats_line icon category count =
  Utils.Base_formatter.stat_report_line_pattern icon category count

(** 分析结果格式 - 使用Base_formatter统一模式 *)
let analysis_result_line icon message =
  Utils.Base_formatter.analysis_message_line_pattern icon message
...
let suggestion_line current suggestion =
  Utils.Base_formatter.suggestion_replacement_pattern current suggestion

(** 相似度建议格式 - 使用Base_formatter统一模式 *)
let similarity_suggestion match_name score =
  Utils.Base_formatter.similarity_match_pattern match_name score
