(** 字符串处理工具统一入口模块 - 重构版本
    ...
    本模块已重构为模块化架构，原先的358行超长模块 已拆分为多个专门的子模块，提升了代码的可维护性和可读性。
...
module PositionFormatting = struct
  let format_position_with_fields =
    String_processing.Position_formatting.format_position_with_fields

  let format_optional_position_with_extractor =
    String_processing.Position_formatting.format_optional_position_with_extractor
end
