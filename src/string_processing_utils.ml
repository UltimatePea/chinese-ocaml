...
module PositionFormatting = struct
  ...
  let error_with_position_extractor pos_opt error_type msg ~get_filename ~get_line ~get_column =
    let pos_str = format_optional_position_with_extractor pos_opt ~get_filename ~get_line ~get_column in
...
module ReportFormatting = struct
  ...
  (** 分析结果格式 *)
  let analysis_result_line icon message =
    Printf.sprintf "%s %s\n\n" icon message
  
  (** 上下文信息格式 *)
  let context_line context =
    Printf.sprintf "📍 上下文: %s\n\n" context
  
  (** 建议信息格式 *)
  let suggestion_line current suggestion =
    Printf.sprintf "建议将「%s」改为「%s」" current suggestion
...
module BufferHelpers = struct
  ...
  (** 安全地向Buffer添加格式化字符串 *)
  let add_formatted_string buffer format_fn =
    Buffer.add_string buffer (format_fn ())
  
  (** 批量添加统计信息 *)
  let add_stats_batch buffer stats_list =
    List.iter (fun (icon, category, count) ->
