...
module KeywordTable = struct
  ...
  let ascii_table =
    Hashtbl.create (List.length (Keyword_matcher_tables_ascii.get_ascii_keywords ()))

  (* 初始化哈希表 *)
  let () =
    List.iter (fun (k, v) -> Hashtbl.add chinese_table k v) chinese_keywords;
    List.iter
      (fun (k, v) -> Hashtbl.add ascii_table k v)
  ...
  let get_all_keywords () =
    List.rev_append chinese_keywords (Keyword_matcher_tables_ascii.get_ascii_keywords ())
end
