(** 骆言C代码生成器控制流模块 - C Code Generator Control Flow Module
    重构说明：统一错误处理模式，消除重复代码，提升代码质量，Printf.sprintf统一化重构 *)
...
let safe_format_string parts =
  with_error_handling ~func_name:"safe_format_string" (fun () -> concat_strings parts)
...
let gen_func_call_expr gen_expr_fn ctx func_expr args =
  with_error_handling ~func_name:"gen_func_call_expr" (fun () ->
      ...
      safe_format_string
        [ "luoyan_call("; func_code; ", "; int_to_string arg_count; ", "; args_code; ")" ])
