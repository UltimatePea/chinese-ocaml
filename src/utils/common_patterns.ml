...
let match_token_with_handlers token handlers ~default =
  try
    List.find_map
      (fun (pattern, handler) -> if pattern token then Some (handler token) else None)
      handlers
    |> function
    | Some result -> result
    | None -> default token
...
let token_to_string_with_mapping mappings token ~default_formatter =
  try List.assoc token mappings with Not_found -> default_formatter token

(** 安全的token查找 *)
let safe_token_lookup table key ~default =
  try Some (Hashtbl.find table key) with Not_found -> default

(** Result类型的token处理 *)
let safe_token_operation operation token =
  try Ok (operation token) with exn -> Error (sprintf "Token处理错误: %s" (Printexc.to_string exn))
...
(** 带回退的数据文件查找 *)
let find_data_file_with_candidates candidates = List.find_opt Sys.file_exists candidates
...
let collect_with_accumulator collector initial items =
  List.fold_left
    (fun acc item -> match collector item with Some result -> result :: acc | None -> acc)
    initial items
  |> List.rev

(** 列表连接组装工具 - 用于韵律数据组装 *)
let assemble_data_groups data_groups transformer = List.concat (List.map transformer data_groups)

(** 安全的列表索引访问 *)
let safe_list_nth list index ~default =
  try List.nth list index with Failure _ -> default | Invalid_argument _ -> default
...
let format_position ?(include_file = false) pos =
  ...
  else sprintf "第%d行第%d列" pos.line pos.column

(** 调试消息格式化 *)
let format_debug_message operation_name status = sprintf "[调试] %s: %s" operation_name status
...
let collect_with_terminator terminator parse_expr get_current_token state =
  let rec collect acc current_state =
    ...
    if terminator token then Ok (List.rev acc, current_state)
    else
...
let validate_and_advance get_current_token advance_state expected_token_checker state =
  ...
  if expected_token_checker token then Ok (advance_state state)
  else Error (sprintf "期望的token类型不匹配 在%s" (format_position pos))

(** ======================================================================== 统一的printf模式 -
    解决84个重复的printf调用 ======================================================================== *)

(** 错误消息打印 *)
let print_error context message = eprintf "%s\n" (format_contextual_error context message)

(** 调试信息打印 *)
let print_debug_info ?(prefix = "[调试]") message = eprintf "%s %s\n" prefix message

(** 警告信息打印 *)
let print_warning ?(prefix = "警告") message = eprintf "%s: %s\n" prefix message
