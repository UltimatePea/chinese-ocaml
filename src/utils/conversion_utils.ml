...
let convert_with_mapping mapping _pos variant =
  try Ok (List.assoc variant mapping)
  with Not_found -> Error "未知关键字"

(** 批量关键字转换器生成器 *)
let create_keyword_converter mapping error_message =
  fun _pos variant -> 
    try Ok (List.assoc variant mapping)
    with Not_found -> Error error_message

(** 合并多个映射表 *)
let merge_mappings mappings =
  List.fold_left (@) [] mappings

(** 条件转换器 - 根据条件选择不同的转换策略 *)
let conditional_converter condition converter1 converter2 =
  fun pos variant ->
    if condition variant 
    then converter1 pos variant
    else converter2 pos variant
...
let compose_converters c1 c2 =
  fun pos variant ->
    match c1 pos variant with
    | Ok intermediate -> c2 pos intermediate
    | Error msg -> Error msg

(** 简化的映射转换器 - 最常见的使用模式 *)
let simple_mapping_converter mapping error_message =
  create_keyword_converter mapping error_message
...
let update_stats stats result =
  ...
  match result with
  | Ok _ -> { total_conversions = total; successful_conversions = stats.successful_conversions + 1; failed_conversions = stats.failed_conversions }
  | Error _ -> { total_conversions = total; successful_conversions = stats.successful_conversions; failed_conversions = stats.failed_conversions + 1 }
