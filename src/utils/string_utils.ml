...
module StringBuilder = struct
  ...
  let add_string builder str = 
    Buffer.add_string builder.buffer str
  
  let add_char builder ch = 
    Buffer.add_char builder.buffer ch
  
  let add_strings builder strings = 
    List.iter (Buffer.add_string builder.buffer) strings
  
  let contents builder = 
    Buffer.contents builder.buffer
  
  let clear builder = 
    Buffer.clear builder.buffer
end
...
module PoetryFormatting = struct
  let format_couplet left_content right_content =
...
module CCodeGenFormatting = struct
  ...
  let format_if_condition condition =
    Printf.sprintf "if (%s)" condition
  
  let format_string_literal content =
...
module UnicodeUtils = struct
  ...
  let count_chinese_chars str =
    let count = ref 0 in
    String.iter (fun ch ->
      if is_chinese_char ch then incr count
    ) str;
    !count
end