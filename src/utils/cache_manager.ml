...
let default_config ?(name="unnamed") () = {
  initial_size = 128;  (* 增大默认大小，减少rehashing *)
  max_size = Some 1024;
  strategy = LRU 1024;
  name = name;
}
...
let update_hit_rate stats =
  ...
  { stats with hit_rate = hit_rate }

(* LRU 实现辅助函数 *)
let create_lru_node key value = {
  key = key;
  value = value;
  prev = None;
  next = None;
...
(* 创建缓存 *)
let create config =
  let impl = match config.strategy with
    | Simple -> SimpleCache (Hashtbl.create config.initial_size)
    | LRU max_size -> LRUCache {
        data = Hashtbl.create config.initial_size;
        head = None;
        tail = None;
        max_size = max_size;
      }
    | LFU max_size -> LFUCache {
        data = Hashtbl.create config.initial_size;
        max_size = max_size;
      }
    | TTL ttl -> TTLCache {
...
let cleanup_ttl_cache data =
  ...
  Hashtbl.iter (fun key (_, expiry) ->
    if expiry < now then to_remove := key :: !to_remove
  ) data;
      ...
      (match Hashtbl.find_opt cache_data.data key with
       | Some (value, expiry) ->
           if expiry >= current_time () then (
             ...
             cache.stats <- { cache.stats with misses = cache.stats.misses + 1; evictions = cache.stats.evictions + 1 } |> update_hit_rate;
             None
           )
       | None ->
           cache.stats <- { cache.stats with misses = cache.stats.misses + 1 } |> update_hit_rate;
...
let clear cache =
  ...
  | TTLCache cache_data -> Hashtbl.clear cache_data.data;
  cache.stats <- init_stats ()

(* 获取缓存统计信息 *)
let stats cache = 
  (* 更新当前大小 *)
  let current_size = match cache.impl with
...
let get_batch cache keys =
  List.map (fun key -> 
    ...
    | None -> None
  ) keys |> List.filter_map (fun x -> x)

let put_batch cache pairs =
  List.iter (fun (key, value) -> put cache key value) pairs
...
let warm_up cache loader keys =
  List.iter (fun key ->
    if not (mem cache key) then
      ...
      | Some value -> put cache key value
      | None -> ()
  ) keys

(* 缓存迁移 - 用于逐步替换现有缓存 *)
let migrate_from_hashtbl cache hashtbl =
  Hashtbl.iter (put cache) hashtbl
