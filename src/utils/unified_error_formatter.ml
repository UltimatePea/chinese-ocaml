(** 骆言编译器统一错误格式化器 - Printf.sprintf重复消除架构重构
    ...
    此模块合并了之前冲突的三个错误格式化模块： 1. constants/error_constants.ml - 常量错误消息 2.
    string_processing/error_message_formatter.ml - 通用错误格式化 3. utils/formatting/error_formatter.ml -
    格式化工具
...
module Unified_error_formatter = struct
  ...
  module Test_errors = struct
    ...
    let wrong_error_type desc exn =
      context_message_pattern (concat_strings [ desc; " 错误类型不正确" ]) (Printexc.to_string exn)
    ...
    let uniqueness_test_failure exn = context_message_pattern "字符唯一性测试失败" (Printexc.to_string exn)
