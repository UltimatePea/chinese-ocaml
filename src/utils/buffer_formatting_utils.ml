...
module Formatting = struct
  ...
  (** 格式化键值对 *)
  let format_pairs ~buffer ~open_delim ~close_delim ~pair_sep ~kv_sep ~key_formatter
      ~value_formatter pairs =
    Buffer.add_string buffer open_delim;
    (match pairs with
    | [] -> ()
  ...
  let format_ocaml_record ~key_formatter ~value_formatter pairs =
    ...
    format_pairs ~buffer ~open_delim:"{" ~close_delim:"}" ~pair_sep:"; " ~kv_sep:" = "
      ~key_formatter ~value_formatter pairs;
    Buffer.contents buffer
  ...
  let format_method_list ~key_formatter ~value_formatter methods =
    let buffer = Buffer.create BufferSizes.medium in
    format_pairs ~buffer ~open_delim:"< " ~close_delim:" >" ~pair_sep:"; " ~kv_sep:": "
      ~key_formatter ~value_formatter methods;
    Buffer.contents buffer
...
module Advanced = struct
  ...
  let format_if_non_empty ~formatter ~empty_message items =
    if items = [] then empty_message else formatter items

  (** 限制长度的格式化 *)
  let format_with_limit ~formatter ~limit ~truncate_message items =
    if List.length items <= limit then formatter items
    else
      let rec take n lst =
...
(** 格式化简单列表 *)
let format_simple_list _separator formatter items = Formatting.format_ocaml_list ~formatter items
