...
module Formatting : sig
  ...
  val format_ocaml_record :
    key_formatter:('a -> string) -> value_formatter:('b -> string) -> ('a * 'b) list -> string
...
module Advanced : sig
  ...
  val format_if_non_empty :
    formatter:('a list -> string) -> empty_message:string -> 'a list -> string
  (** 条件格式化 - 仅当列表非空时才格式化 *)

  val format_with_limit :
    formatter:('a list -> string) -> limit:int -> truncate_message:string -> 'a list -> string
  (** 限制长度的格式化（超出时截断） *)
end
