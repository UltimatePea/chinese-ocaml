...
let find_data_file file_path = if Sys.file_exists file_path then Some file_path else None
...
let load_character_groups loader group_names = List.map loader group_names
...
type rhyme_file_config = { base_dir : string; file_extension : string; default_encoding : string }
(** 韵律文件配置信息 *)
...
let default_rhyme_config =
  { base_dir = "data/rhyme"; file_extension = ".json"; default_encoding = "utf-8" }
...
let parse_json_rhyme_data _json_content =
  (* 简化的占位符实现 - 实际项目中可以扩展 *)
  { characters = []; category = PingSheng; group = UnknownRhyme; metadata = [] }

(** 安全加载JSON文件 *)
let safe_load_json_file file_path =
  load_file_with_recovery file_path (fun content -> Some (parse_json_rhyme_data content))
...
(** 统一的字符组加载模式 *)
let load_rhyme_character_groups loader group_names = load_character_groups loader group_names
...
let create_rhyme_entries characters category group =
  List.map
    (fun char -> { character = char; category; group; tone_info = None; usage_notes = None })
    characters
...
let validate_rhyme_entry entry =
  ...
  let category_valid =
    match entry.category with PingSheng | ZeSheng | ShangSheng | QuSheng | RuSheng -> true
  in
...
let create_rhyme_validator (entries : rhyme_entry list) : string -> bool =
 fun character -> List.exists (fun e -> e.character = character) entries
...
let analyze_rhyme_data (entries : rhyme_entry list) : string =
  ...
  List.iter
    (fun (entry : rhyme_entry) ->
      ...
      category_counts :=
        (category_str, current + 1) :: List.remove_assoc category_str !category_counts)
...
let batch_load_rhyme_files config category_group_pairs =
  List.fold_left
    (fun acc (category, group) ->
      match find_rhyme_data_file config category group with
      | Some file_path -> (
          match safe_load_json_file file_path with Some data -> data :: acc | None -> acc)
      | None -> acc)
...
let load_rhyme_data config category group =
  print_debug_info
    (sprintf "加载韵律数据: %s/%s" (string_of_rhyme_category category) (string_of_rhyme_group group));
  let data = batch_load_rhyme_files config [ (category, group) ] in
  List.concat
    (List.map (fun json_data -> create_rhyme_entries json_data.characters category group) data)

(** 性能报告 *)
let performance_report config =
  let config_info = sprintf "韵律配置: 基础目录=%s" config.base_dir in
  sprintf "韵律系统性能报告:\n%s" config_info
...
let create_simple_cache capacity =
  ...
  let put key value =
    (if Hashtbl.length cache >= capacity then
       ...
       List.iteri (fun i key -> if i < remove_count then Hashtbl.remove cache key) keys_to_remove);
