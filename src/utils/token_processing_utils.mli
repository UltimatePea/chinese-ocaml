...
(** Token验证结果 *)
type token_validation_result = ValidToken of string | InvalidToken of string | UnknownToken
...
val safe_token_type_check :
  'token token_validator -> 'token -> Common_patterns.error_context -> (string, string) result
(** 安全的Token类型检查 *)
...
val convert_token_with_rules :
  'token token_rule list -> 'token -> default_handler:('token -> string) -> string
(** 统一的Token转换器 *)
...
val register_token_mappings : ('key, 'value) token_registry -> ('key * 'value) list -> unit
(** 批量注册Token映射 *)
...
val dispatch_token : 'token token_classifier -> 'token token_dispatcher -> 'token -> string
(** 使用分发器处理Token *)
...
val resilient_token_processing :
  ('token -> string) -> token_recovery_strategy -> 'token -> (string, string) result
(** 错误恢复的Token处理 *)
