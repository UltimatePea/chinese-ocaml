...
(** 创建Token类型错误 *)
let create_token_error category token_desc = sprintf "不是%s类型的Token: %s" category token_desc
...
let convert_token_by_category token =
  ...
  convert_token_with_rules all_rules token ~default_handler:(fun _t -> "未知Token")
...
(** 清理Token注册表 *)
let cleanup_token_registry registry = Hashtbl.clear registry
...
module TokenCache = struct
  ...
  let cache_stats cache = sprintf "缓存大小: %d" (Hashtbl.length cache)
end

(** 延迟Token映射加载器 *)
let create_lazy_token_mapping loader_func = create_lazy_data_loader loader_func

(** ======================================================================== Token批处理工具 -
    优化批量Token操作 ======================================================================== *)

(** 批量Token转换 *)
let batch_convert_tokens converter tokens = List.map converter tokens
...
let process_tokens_with_state initial_state processor tokens =
  let rec process_aux state acc = function
    ...
    | token :: rest -> (
        ...
        | Some (new_state, result) -> process_aux new_state (result :: acc) rest
        | None -> Error "Token处理失败")
  in
