...
let safe_lex_operation f =
  ...
  | Lexer_core.LexError (msg, pos) ->
      ...
      LexError error_info
  | exn ->
      let error_info = extract_error_info (internal_error ("词法分析内部错误: " ^ Printexc.to_string exn)) in
      LexError error_info
...
let integrate_with_error_handler state error_info =
  let context = Error_handler.create_context 
    ~source_file:state.Lexer_core.filename
    ~function_name:"lexer_core"
