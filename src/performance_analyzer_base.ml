(** 性能分析器公共基础模块
    ...
    提供所有性能分析器共享的通用表达式分析逻辑和建议生成功能
    创建目的：消除代码重复，提供统一的分析接口
    创建时间：技术债务清理 Fix #794 *)
  ...
  let child_suggestions = 
    ...
    | MatchExpr (matched_expr, branches) ->
        ...
        let branches_suggestions = 
          List.concat_map (fun branch -> analyze_expr_recursively specific_analyzer branch.expr) branches 
        in
...
module SuggestionBuilder = struct
  ...
  let list_optimization_suggestion operation_name specific_message =
    make_performance_suggestion
      ~hint_type:(operation_name ^ "优化")
      ~message:specific_message
      ~confidence:0.65
      ~location:(operation_name ^ "操作")
      ~fix:"考虑使用更高效的数据结构或算法"
  ...
  let pattern_matching_suggestion branch_count _severity =
    let (hint_type, message, confidence, fix) = 
      if branch_count > 20 then
        ("过多分支警告", 
         Printf.sprintf "匹配表达式包含%d个分支，严重影响性能和可读性" branch_count,
         0.85,
         "强烈建议重构为多个函数或使用映射表")
      else
        ("大量分支优化",
         Printf.sprintf "匹配表达式包含%d个分支，可能影响性能" branch_count,
         0.70,
         "考虑重构为更小的函数或使用查找表")
    in
    make_performance_suggestion
