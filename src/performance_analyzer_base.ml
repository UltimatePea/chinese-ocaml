  ...
  let pattern_matching_suggestion branch_count _severity =
    let hint_type, message, confidence, fix =
      if branch_count > 20 then
        ( "过多分支警告",
          concat_strings [ "匹配表达式包含"; int_to_string branch_count; "个分支，严重影响性能和可读性" ],
          0.85,
          "强烈建议重构为多个函数或使用映射表" )
      else
        ( "大量分支优化",
          concat_strings [ "匹配表达式包含"; int_to_string branch_count; "个分支，可能影响性能" ],
          0.70,
          "考虑重构为更小的函数或使用查找表" )
    in
