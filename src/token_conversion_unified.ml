...
let default_converters : converter_registry =
  [
    (`Identifier, convert_identifier_token);
    (`Literal, convert_literal_token);
    (`BasicKeyword, convert_basic_keyword_token);
    (`TypeKeyword, convert_type_keyword_token);
    (`Classical, convert_classical_token);
  ]
...
(** 获取指定类型的转换器 *)
let get_converter (conv_type : converter_type) = List.assoc conv_type !active_converters

(** 尝试单个转换器转换 - 返回option类型 *)
let try_convert_with_type (conv_type : converter_type)
    (token : Token_mapping.Token_definitions_unified.token) =
  try
...
let convert_token_exn (token : Token_mapping.Token_definitions_unified.token) =
  ...
  | None ->
      ...
      raise
        (Unified_conversion_failed (`Identifier, "No converter found for token type: " ^ token_info))
  ...
  let priority_info =
    String.concat " -> "
      (List.map
         ...
         conversion_priority)
  in
  Printf.sprintf "统一Token转换系统: %d个转换器 | 优先级顺序: %s" converter_count priority_info
...
(** 重置转换器到默认状态 *)
let reset_converters () = active_converters := default_converters
...
module CompatibilityInterface = struct
  ...
  let convert_basic_keyword_token token =
    ...
    | Some result -> result
    | None ->
        raise (Unified_conversion_failed (`BasicKeyword, "Failed to convert basic keyword token"))
  ...
  let convert_type_keyword_token token =
    ...
    | Some result -> result
    | None ->
        raise (Unified_conversion_failed (`TypeKeyword, "Failed to convert type keyword token"))
