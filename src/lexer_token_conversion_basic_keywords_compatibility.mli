(** 基础关键字Token转换模块接口 - 向后兼容性包装器

    此模块为原有的lexer_token_conversion_basic_keywords.ml模块提供向后兼容性。 现在所有功能都通过Token_conversion_core模块实现。
...
(** 转换基础关键字Token
    ...
    @raise Unknown_basic_keyword_token 如果基础关键字Token无法识别 *)
