...
module ErrorMessages = struct
  ...
  let undefined_variable var_name = 
    Unified_logger.Legacy.sprintf "未定义的变量: %s" var_name
    
  let variable_already_defined var_name =
    Unified_logger.Legacy.sprintf "变量已定义: %s" var_name
  ...
  let function_not_found func_name = 
    Unified_logger.Legacy.sprintf "函数未找到: %s" func_name
  ...
  let unknown_type type_name = 
    Unified_logger.Legacy.sprintf "未知类型: %s" type_name
    
  let invalid_type_operation op_name = 
    Unified_logger.Legacy.sprintf "无效的类型操作: %s" op_name
  ...
  let unexpected_token token = 
    Unified_logger.Legacy.sprintf "意外的Token: %s" token
    
  let expected_token expected actual = 
    Unified_logger.Legacy.sprintf "期望Token %s，得到 %s" expected actual
    
  let syntax_error message = 
    Unified_logger.Legacy.sprintf "语法错误: %s" message
    
  (** 文件操作错误 *)
  let file_not_found filename = 
    Unified_logger.Legacy.sprintf "文件未找到: %s" filename
    
  let file_read_error filename = 
    Unified_logger.Legacy.sprintf "文件读取错误: %s" filename
    
  let file_write_error filename = 
    Unified_logger.Legacy.sprintf "文件写入错误: %s" filename
  ...
  let module_not_found mod_name = 
    Unified_logger.Legacy.sprintf "未定义的模块: %s" mod_name
  ...
  let config_parse_error message = 
    Unified_logger.Legacy.sprintf "配置解析错误: %s" message
    
  let invalid_config_value key value = 
    Unified_logger.Legacy.sprintf "配置值无效: %s = %s" key value
    
  (** 操作错误 *)
  let invalid_operation operation = 
    Unified_logger.Legacy.sprintf "无效操作: %s" operation
  ...
  let generic_error context message =
    Unified_logger.Legacy.sprintf "%s: %s" context message
end

(** 编译器状态消息格式化 *)
module CompilerMessages = struct
  let compiling_file filename = 
    Unified_logger.Legacy.sprintf "正在编译文件: %s" filename
    
  let compilation_complete filename = 
    Unified_logger.Legacy.sprintf "编译完成: %s" filename
...
module LogMessages = struct
  ...
  let trace func_name message =
    Unified_logger.Legacy.sprintf "[TRACE] %s: %s" func_name message
end
...
module General = struct
  ...
  let format_variable_definition var_name =
    Unified_logger.Legacy.sprintf "让 「%s」 = 值" var_name
...
module Collections = struct
  ...
  let list_operation_error operation =
    Unified_logger.Legacy.sprintf "列表操作错误: %s" operation
end

(** 转换和类型转换格式化 *)
module Conversions = struct
  let type_conversion target_type expr =
    Unified_logger.Legacy.sprintf "(%s)%s" target_type expr
