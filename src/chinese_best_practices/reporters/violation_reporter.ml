...
(** 获取严重程度图标 *)
let get_severity_icon = function
  | Error -> "🚨"
  | Warning -> "⚠️"
  | Style -> "🎨"
  | Info -> "💡"
...
(** 获取严重程度文本 *)
let get_severity_text = function
  | Error -> "错误"
  | Warning -> "警告"
  | Style -> "风格"
  | Info -> "提示"
...
let generate_violation_details violations =
  ...
  violations |> List.iteri (fun i result ->
    ...
    append_line buffer 
      (Printf.sprintf "%d. %s [%s] %s%s" 
        (i + 1) icon severity_text result.message ai_friendly_mark);
    append_line buffer 
      (Printf.sprintf "   💡 建议: %s" result.suggestion);
    append_line buffer 
      (Printf.sprintf "   🎯 置信度: %.0f%%" (result.confidence *. 100.0));
    append_line buffer "";
  );
...
let generate_stats_report violations =
  ...
  let (error_count, warning_count, style_count, info_count) = 
    count_violations_by_severity violations in
...
let generate_improvement_suggestions violations =
  ...
  if error_count > 0 then
    append_line buffer "   1. 优先修复所有错误级别的问题，这些会影响AI代理的理解";
  
  if warning_count > 0 then
    append_line buffer "   2. 处理警告级别的问题，提升代码的地道性";
  
  if style_count > 0 then
    append_line buffer "   3. 统一编程风格，保持代码一致性";
  
  if info_count > 0 then
    append_line buffer "   4. 考虑信息级别的建议，进一步优化表达";
