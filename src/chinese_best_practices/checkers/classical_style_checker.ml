...
let check_category code category =
  let category_rules = match category with
    | "excessive_classical" -> 
        List.filter (fun rule -> rule.issue = "过度古雅") classical_style_rules
    | "mixed_expression" ->
        List.filter (fun rule -> rule.issue = "古今混用") classical_style_rules
    | "ai_unfriendly" ->
        List.filter (fun rule -> rule.issue = "AI理解困难") classical_style_rules
