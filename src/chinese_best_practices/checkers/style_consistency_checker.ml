...
let style_consistency_rules = [
  (* 变量命名风格 *)
  {
    pattern = "「.*」.*「.*」";
    issue = "引用符号使用";
    suggestion = "保持一致的引用符号风格";
    severity = Style;
  };
  {
    pattern = "让.*=.*让.*=";
    issue = "变量定义风格";
    suggestion = "保持一致的变量定义间距";
    severity = Style;
  };
  (* 函数定义风格 *)
  {
    pattern = "函数.*→.*函数.*→";
    issue = "函数定义风格";
    suggestion = "保持一致的函数定义格式";
    severity = Style;
  };
  {
    pattern = "递归.*让.*递归.*让";
    issue = "递归标记风格";
    suggestion = "保持一致的递归标记使用";
    severity = Style;
  };
  (* 注释风格 *)
  {
...
let check_category code category =
  ...
  let category_rules = match category with
    | "variable_naming" -> 
        List.filter (fun rule -> contains_substring rule.issue "引" || contains_substring rule.issue "变") style_consistency_rules
    | "function_definition" ->
        List.filter (fun rule -> contains_substring rule.issue "函" || contains_substring rule.issue "递") style_consistency_rules
    | "comment_style" ->
...
let check_with_severity_filter code min_severity =
  let severity_rules = List.filter (fun rule -> 
