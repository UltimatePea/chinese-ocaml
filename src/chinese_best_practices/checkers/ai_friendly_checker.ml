...
let ai_friendly_rules = [
  (* 清晰的意图表达 *)
  {
    pattern = "计算";
    issue = "动作明确";
    suggestion = "建议加上具体的计算对象";
    severity = Info;
  };
  {
    pattern = "处理";
    issue = "动作模糊";
    suggestion = "建议明确处理的内容和方式";
    severity = Warning;
  };
  {
    pattern = "操作";
    issue = "动作模糊";
    suggestion = "建议明确操作的对象和方法";
    severity = Warning;
  };
  (* 避免歧义表达 *)
  {
  ...
  (* 鼓励声明式表达 *)
  {
    pattern = "循环.*直到";
    issue = "命令式表达";
    suggestion = "考虑使用递归或高阶函数";
    severity = Info;
  };
  {
    pattern = "逐个.*处理";
    issue = "命令式表达";
    suggestion = "考虑使用映射或过滤函数";
    severity = Info;
  };
]
  ...
  let category_rules = match category with
    | "intent_clarity" -> 
        List.filter (fun rule -> 
          rule.issue = "动作明确" || rule.issue = "动作模糊") ai_friendly_rules
    | "ambiguity_avoidance" ->
        List.filter (fun rule -> rule.issue = "指代不明") ai_friendly_rules
    | "declarative_style" ->
        List.filter (fun rule -> rule.issue = "命令式表达") ai_friendly_rules
    | _ -> ai_friendly_rules
  in
