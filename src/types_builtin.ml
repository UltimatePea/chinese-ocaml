...
(** 获取内置函数列表 *)
let get_builtin_functions () = 
  TypeEnv.bindings builtin_env |> List.map fst

(** 检查是否是内置函数 *)
let is_builtin_function name = 
  TypeEnv.mem name builtin_env

(** 获取内置函数类型 *)
let get_builtin_type name = 
  try
    Some (TypeEnv.find name builtin_env)
  with
  | Not_found -> None

(** 添加自定义内置函数 *)
let add_builtin_function name type_scheme env =
  TypeEnv.add name type_scheme env

(** 获取类别的内置函数 *)
let get_math_functions () = [
  "求和"; "最大值"; "最小值"; "平均值"; "乘积"; "绝对值"; "平方"; "幂运算";
  "余弦"; "正弦"; "平方根"; "取整"; "随机数"; "对数"; "自然对数"; "十进制对数";
  "指数"; "正切"; "反正弦"; "反余弦"; "反正切"; "向上取整"; "向下取整"; 
  "四舍五入"; "最大公约数"; "最小公倍数"
]

let get_list_functions () = [
  "长度"; "连接"; "过滤"; "映射"; "折叠"; "范围"; "排序"; "反转"; "包含"
]

let get_string_functions () = [
  "字符串长度"; "字符串连接"; "字符串分割"; "大写转换"; "小写转换"; 
  "去除空白"; "字符串替换"; "子字符串"; "字符串比较"
]

let get_io_functions () = [
  "打印"; "读取"; "读取文件"; "写入文件"; "文件存在"
]

let get_conversion_functions () = [
  "整数到字符串"; "浮点数到字符串"; "字符串到整数"; "字符串到浮点数"
]

let get_array_functions () = [
  "创建数组"; "数组长度"; "复制数组"
]

let get_reference_functions () = [
  "引用"
]