...
module UnificationOptimization = struct
  ...
  (** 快速类型变量检查 *)
  let is_type_var = function TypeVar_T _ -> true | _ -> false
  ...
  let rec occurs_check var_name typ =
    ...
    | TupleType_T type_list -> List.exists (occurs_check var_name) type_list
    | ListType_T elem_type -> occurs_check var_name elem_type
    | _ -> false
...
and infer_type_uncached env expr =
  ...
  | LabeledFunExpr (label_params, body) ->
      ...
      let extended_env =
        List.fold_left
          (fun acc_env (param_name, param_type) ->
      ...
      let applied_param_types =
        List.map (fun (name, typ) -> (name, apply_subst subst typ)) param_types
      in

      (* 简化：暂时使用普通函数类型表示标签函数 *)
      let fun_type =
        List.fold_right
          (fun (_, param_type) acc -> FunType_T (param_type, acc))
