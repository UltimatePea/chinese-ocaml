...
let handle_filesystem_error operation path f =
  ...
  with
  | Sys_error msg -> 
      runtime_error (Printf.sprintf "文件系统操作失败 [%s] %s: %s" operation path msg)
  ...
  | e -> 
      runtime_error (Printf.sprintf "未知错误 [%s] %s: %s" operation path (Printexc.to_string e))
...
let join_paths path1 path2 =
  if path2 = "" then path1
    ...
    let sep = if String.length path1 > 0 && path1.[String.length path1 - 1] = '/' then "" else "/" in
    path1 ^ sep ^ path2
...
let copy_file_function args =
  ...
  BuiltinFunctionValue
    (fun dest_args ->
      ...
      handle_filesystem_error "复制文件" (source ^ " -> " ^ dest) (fun () ->
...
let move_file_function args =
  ...
  BuiltinFunctionValue
    (fun dest_args ->
      ...
      handle_filesystem_error "移动文件" (source ^ " -> " ^ dest) (fun () ->
...
let rename_file_function args =
  ...
  BuiltinFunctionValue
    (fun newname_args ->
      ...
      handle_filesystem_error "重命名文件" (oldname ^ " -> " ^ newname) (fun () ->
...
let read_binary_file_function args =
  ...
  handle_filesystem_error "读取二进制文件" filename (fun () ->
    ...
    close_in ic;
    let byte_list = Bytes.to_seq bytes |> Seq.map (fun c -> IntValue (Char.code c)) |> List.of_seq in
    ListValue byte_list)
  ...
  let rec delete_recursive path =
    if Sys.is_directory path then (
      ...
      Array.iter (fun entry ->
        delete_recursive (Filename.concat path entry)
      ) entries;
      Unix.rmdir path
    ) else (
      Sys.remove path
    ...
    (fun handler_args ->
      ...
      let rec traverse path =
        if Sys.is_directory path then (
          let entries = Sys.readdir path in
          Array.iter (fun entry ->
            traverse (Filename.concat path entry)
          ) entries
        ) else (
...
let absolute_path_function args =
  ...
  let abs_path = if Filename.is_relative path then
...
let remove_extension_function args =
  ...
  let name_without_ext = try
    let dot_pos = String.rindex basename '.' in
    String.sub basename 0 dot_pos
  with Not_found -> basename in
  let result = if dirname = "." then name_without_ext else Filename.concat dirname name_without_ext in
  StringValue result
...
let get_home_directory_function args =
  ...
  | [] | [UnitValue] ->
      (try
        StringValue (Sys.getenv "HOME")
      with Not_found ->
        try
          StringValue (Sys.getenv "USERPROFILE")  (* Windows *)
        with Not_found ->
          StringValue (Sys.getcwd ()))
...
let get_temp_directory_function args =
  ...
  | [] | [UnitValue] ->
      (try
        StringValue (Sys.getenv "TMPDIR")  (* Unix *)
      with Not_found ->
        try
          StringValue (Sys.getenv "TEMP")   (* Windows *)
        with Not_found ->
          StringValue "/tmp")
