(** 骆言日志兼容性模块 - 为旧代码提供兼容性支持 Printf.sprintf 依赖消除 Phase 5.1 - 完成日志兼容性模块迁移
...
(** 替代Printf.sprintf - 使用Base_formatter消除Printf.sprintf依赖 这个函数被弃用，建议使用Utils.Base_formatter中的具体格式化函数
*)
