...
let estimate_recursion_depth expr =
  let rec count_nested_calls expr depth =
    ...
    | FunCallExpr (VarExpr _, args) ->
        let args_depth = List.fold_left (fun acc arg -> max acc (count_nested_calls arg (depth + 1))) depth args in
        args_depth + 1
    ...
    | MatchExpr (matched_expr, branches) ->
        ...
        let branches_depth = List.fold_left (fun acc branch -> max acc (count_nested_calls branch.expr depth)) 0 branches in
