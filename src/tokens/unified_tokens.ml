...
(** 特定类型判断函数 *)
let is_numeric_token = function Literal lt -> is_numeric_literal lt | _ -> false

let is_string_token = function Literal lt -> is_string_literal lt | _ -> false
let is_control_flow_token = function Keyword kt -> is_control_flow_keyword kt | _ -> false
let is_binary_op_token = function Operator ot -> is_binary_operator ot | _ -> false
let is_unary_op_token = function Operator ot -> is_unary_operator ot | _ -> false
let is_left_delimiter_token = function Delimiter dt -> is_left_delimiter dt | _ -> false
let is_right_delimiter_token = function Delimiter dt -> is_right_delimiter dt | _ -> false

(** 获取操作符优先级 *)
let get_token_precedence = function Operator ot -> get_operator_precedence ot | _ -> 0
