...
module OperatorMapping = struct
  ...
  (** 查找符号操作符 *)
  let lookup_symbol_operator text =
    Hashtbl.find_opt symbol_operator_table text
  
  (** 查找中文操作符 *)
  let lookup_chinese_operator text =
    Hashtbl.find_opt chinese_operator_table text
  ...
  (** 操作符转换为符号 *)
  let operator_to_symbol op =
    Hashtbl.find_opt operator_to_symbol_table op
  
  (** 操作符转换为中文 *)
  let operator_to_chinese op =
    Hashtbl.find_opt operator_to_chinese_table op
  
  (** 检查是否为操作符 *)
  let is_operator text =
    match lookup_operator text with
    | Some _ -> true
    | None -> false
  ...
  let is_binary_operator = function
    | Arithmetic _ | Comparison _ | Logical (And | Or) | Assignment _ 
    | Bitwise (BitwiseAnd | BitwiseOr | BitwiseXor | LeftShift | RightShift) -> true
    | _ -> false
  ...
  (** 获取所有符号操作符 *)
  let get_all_symbol_operators () =
    List.map fst symbol_to_operator
  
  (** 获取所有中文操作符 *)
  let get_all_chinese_operators () =
    List.map fst chinese_to_operator
  
  (** 操作符统计信息 *)
  let get_operator_stats () = {|
符号操作符数量: |} ^ (string_of_int (List.length symbol_to_operator)) ^ {|
中文操作符数量: |} ^ (string_of_int (List.length chinese_to_operator)) ^ {|
算术操作符: |} ^ (string_of_int (List.length (get_operators_by_category "arithmetic"))) ^ {|
比较操作符: |} ^ (string_of_int (List.length (get_operators_by_category "comparison"))) ^ {|
逻辑操作符: |} ^ (string_of_int (List.length (get_operators_by_category "logical"))) ^ {|
赋值操作符: |} ^ (string_of_int (List.length (get_operators_by_category "assignment"))) ^ {|
位运算操作符: |} ^ (string_of_int (List.length (get_operators_by_category "bitwise")))
end
...
module OperatorPrecedenceTable = struct
  ...
  let precedence_table = [
    ...
    { operator = Comparison GreaterThan; precedence = 4; associativity = NonAssoc; arity = `Binary };
    { operator = Comparison GreaterEqual; precedence = 4; associativity = NonAssoc; arity = `Binary };
  ...
  (** 查找操作符优先级信息 *)
  let find_precedence_info op =
    List.find_opt (fun entry -> entry.operator = op) precedence_table
