...
module LiteralMapping = struct
  ...
  let chinese_bool_mapping = [
    ("真", true); ("是", true); ("对", true); ("有", true);
    ("假", false); ("否", false); ("错", false); ("无", false);
  ]
  ...
  let is_string_literal text =
    ...
    len >= 2 && (
      (text.[0] = '"' && text.[len - 1] = '"') ||
      (text.[0] = '\'' && text.[len - 1] = '\'') ||
      (String.length text >= 6 && String.sub text 0 3 = "\"\"\"" && String.sub text (len - 3) 3 = "\"\"\"")
    )
  
  (** 提取字符串内容 *)
  let extract_string_content text =
    let len = String.length text in
    if len >= 6 && String.sub text 0 3 = "\"\"\"" then
      String.sub text 3 (len - 6) (* 三重引号字符串 *)
    else if len >= 2 then
      String.sub text 1 (len - 2) (* 普通字符串 *)
    else
      text
  
  (** 查找中文数字 *)
  let lookup_chinese_digit text =
    Hashtbl.find_opt chinese_digit_table text
  
  (** 查找中文单位 *)
  let lookup_chinese_unit text =
    Hashtbl.find_opt chinese_unit_table text
  ...
  let is_chinese_number text =
    match parse_simple_chinese_number text with
    | Some _ -> true
    | None -> false
  
  (** 查找中文布尔值 *)
  let lookup_chinese_bool text =
    Hashtbl.find_opt chinese_bool_table text
  
  (** 查找英文布尔值 *)
  let lookup_english_bool text =
    Hashtbl.find_opt english_bool_table text
  
  (** 通用布尔值查找 *)
  let lookup_bool text =
    match lookup_chinese_bool text with
    | Some b -> Some b
    | None -> lookup_english_bool text
  
  (** 字面量识别和解析 *)
  let parse_literal text =
    (* 尝试按优先级解析不同类型的字面量 *)
    if is_string_literal text then
      Some (StringToken (extract_string_content text))
    else
      ...
      | None ->
        ...
        | None ->
          ...
          | None ->
            if is_chinese_number text then
              Some (ChineseNumberToken text)
            else
              None
  ...
  let is_string_literal_token = function
    | StringToken _ -> true
    | _ -> false
  ...
  let get_literal_value = function
    ...
    | ChineseNumberToken s -> 
      match parse_simple_chinese_number s with
      | Some i -> `Int i
      | None -> `String s
  ...
  (** 获取所有支持的中文数字 *)
  let get_all_chinese_digits () =
    List.map fst chinese_digit_mapping
  
  (** 获取所有支持的中文单位 *)
  let get_all_chinese_units () =
    List.map fst chinese_unit_mapping
  
  (** 字面量统计信息 *)
  let get_literal_stats () = {|
支持的中文数字: |} ^ (string_of_int (List.length chinese_digit_mapping)) ^ {|
支持的中文单位: |} ^ (string_of_int (List.length chinese_unit_mapping)) ^ {|
支持的中文布尔值: |} ^ (string_of_int (List.length chinese_bool_mapping)) ^ {|
支持的英文布尔值: |} ^ (string_of_int (List.length english_bool_mapping))
end

(** 字面量验证器 *)
module LiteralValidator = struct
  (** 验证整数范围 *)
  let validate_int_range i ~min_val ~max_val =
    i >= min_val && i <= max_val
  
  (** 验证浮点数范围 *)
  let validate_float_range f ~min_val ~max_val =
    f >= min_val && f <= max_val
  
  (** 验证字符串长度 *)
  let validate_string_length s ~max_length =
