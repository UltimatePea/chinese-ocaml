(** 简化的Token兼容性桥接模块 - 技术债务清理 Issue #1375
    ...
    提供基本的兼容性支持，允许渐进式迁移。 这是一个简化版本，避免复杂的构造器映射问题。
...
let safe_to_legacy_string token = Utils.token_to_string token
...
(** 检查Token是否可以安全转换 *)
let is_compatible_with_legacy = function Error _ -> false | _ -> true
...
let tokens_to_strings tokens = List.map Utils.token_to_string tokens

(** 批量转换：字符串列表 -> 统一Token列表 *)
let strings_to_tokens strings = List.map safe_from_legacy_string strings
