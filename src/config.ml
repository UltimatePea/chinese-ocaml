...
let env_var_mappings = [
  ...
  ("CHINESE_OCAML_TEMP_DIR", fun v ->
    if String.length v > 0 then
      compiler_config := { !compiler_config with temp_directory = v });
  
  ("CHINESE_OCAML_C_COMPILER", fun v ->
    if String.length v > 0 then
      compiler_config := { !compiler_config with c_compiler = v });
    ...
    if String.length trimmed > 0 && not (String.get trimmed 0 = '#') then
      try
        ...
        let value = 
          ...
          let v = if String.length v > 0 && String.get v 0 = '"' then String.sub v 1 (String.length v - 1) else v in
          let v = if String.length v > 0 && String.get v (String.length v - 1) = '"' then String.sub v 0 (String.length v - 1) else v in
          let v = if String.length v > 0 && String.get v (String.length v - 1) = ',' then String.sub v 0 (String.length v - 1) else v in
          String.trim v
        in
        (* 根据键名设置配置 *)
        ...
        | "buffer_size" ->
          (try compiler_config := { !compiler_config with buffer_size = int_of_string value } with _ -> ())
        | "timeout" ->
          (try compiler_config := { !compiler_config with compilation_timeout = float_of_string value } with _ -> ())
        | "output_directory" ->
          compiler_config := { !compiler_config with output_directory = value }
        | "c_compiler" ->
          compiler_config := { !compiler_config with c_compiler = value }
        | "optimization_level" ->
          (try compiler_config := { !compiler_config with optimization_level = int_of_string value } with _ -> ())
        | _ -> ()
      with _ -> ()
  ) lines
...
let validate_config () =
  ...
  (* 验证缓冲区大小 *)
  if compiler_cfg.buffer_size <= 0 then
    errors := "缓冲区大小必须大于0" :: !errors;
  
  (* 验证超时时间 *)
  if compiler_cfg.compilation_timeout <= 0.0 then
    errors := "编译超时时间必须大于0" :: !errors;
  ...
  (* 验证最大错误数量 *)
  if runtime_cfg.max_error_count <= 0 then
    errors := "最大错误数量必须大于0" :: !errors;
