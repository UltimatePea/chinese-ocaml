...
module LexerBenchmark = struct
  ...
  (** 词法分析模拟函数 *)
  let dummy_lexer text = String.length text
  ...
  let get_lexer_test_configs () =
    [
      ...
      { Benchmark_core.name = "大型文本词法分析"; iterations = 10; data_size = 1000; description = "大规模测试" };
    ]

  (** 运行词法分析器基准测试 *)
  let run_lexer_benchmark () =
    let test_configs = get_lexer_test_configs () in
    TestExecutor.execute_benchmark_tests test_configs dummy_lexer
      Benchmark_core.Utils.simple_content_generator

  (** 运行带内存监控的词法分析器基准测试 *)
  let run_lexer_benchmark_with_memory () =
    let test_configs = get_lexer_test_configs () in
    TestExecutor.execute_benchmark_tests_with_memory test_configs dummy_lexer
      Benchmark_core.Utils.simple_content_generator
end
