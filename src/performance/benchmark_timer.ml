(** 性能基准测试计时器模块
    ...
    创建目的：从performance_benchmark.ml中分离计时功能，提供专门的时间测量工具 创建时间：技术债务改进 Phase 5.1 - Fix #1083 *)
...
module Timer = struct
  ...
  let regression_timing_test f input baseline_time iterations tolerance =
    ...
    let metric = BenchmarkCore.create_metric "回归检测测试" avg_time ~variance iterations in
...
module TimerConfig = struct
  (** 默认测试配置 *)
  let default_config =
    { name = "默认计时测试"; iterations = 10; data_size = 100; description = "标准性能测试配置" }

  (** 快速测试配置 *)
  let quick_config = { name = "快速计时测试"; iterations = 5; data_size = 50; description = "快速性能验证配置" }

  (** 详细测试配置 *)
  let detailed_config =
    { name = "详细计时测试"; iterations = 50; data_size = 500; description = "详细性能分析配置" }
