(** 性能基准测试配置管理模块
    ...
    创建目的：从performance_benchmark.ml中分离配置逻辑，实现配置的集中管理 创建时间：技术债务改进 Phase 5.1 - Fix #1083 *)
...
module BenchmarkConfig = struct
  ...
  let lexer_test_configs =
    [
      { name = "小型文本词法分析"; iterations = 50; data_size = 10; description = "小规模文本的词法分析性能测试" };
      { name = "中型文本词法分析"; iterations = 20; data_size = 100; description = "中等规模文本的词法分析性能测试" };
      { name = "大型文本词法分析"; iterations = 5; data_size = 1000; description = "大规模文本的词法分析性能测试" };
    ]

  (** 语法分析器测试配置 *)
  let parser_test_configs =
    [
      { name = "简单表达式解析"; iterations = 100; data_size = 1; description = "简单表达式的语法分析性能测试" };
      { name = "中等复杂表达式解析"; iterations = 50; data_size = 5; description = "中等复杂度表达式的语法分析性能测试" };
      { name = "复杂嵌套表达式解析"; iterations = 20; data_size = 8; description = "复杂嵌套表达式的语法分析性能测试" };
    ]

  (** 诗词功能测试配置 *)
  let poetry_test_configs =
    [
      { name = "短诗词分析"; iterations = 30; data_size = 2; description = "短篇诗词的艺术性分析性能测试" };
      { name = "中篇诗词分析"; iterations = 15; data_size = 10; description = "中篇诗词的艺术性分析性能测试" };
      { name = "长篇诗词分析"; iterations = 5; data_size = 50; description = "长篇诗词的艺术性分析性能测试" };
    ]
...
module PerformanceThresholds = struct
  ...
  let get_performance_threshold test_name =
    try List.assoc test_name performance_thresholds
    with Not_found -> List.assoc "默认" performance_thresholds

  (** 获取内存阈值 *)
  let get_memory_threshold test_name =
    try List.assoc test_name memory_thresholds with Not_found -> List.assoc "默认" memory_thresholds

  (** 添加自定义阈值 *)
  let add_custom_threshold test_name performance_threshold memory_threshold =
    (* 这里可以实现动态添加阈值的逻辑 *)
    Printf.sprintf "已添加自定义阈值：%s (性能: %.2f, 内存: %s)" test_name performance_threshold
      (Utils.format_memory_usage memory_threshold)
end
...
module GlobalConfig = struct
  ...
  (** 更新全局配置 *)
  let update_config new_config = current_config := new_config
    ...
    let config =
      {
        ...
        baseline_file =
          (try Sys.getenv "BENCHMARK_BASELINE_FILE" with Not_found -> "performance_baseline.json");
      }
    in
