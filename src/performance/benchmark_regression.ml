...
module RegressionDetector = struct
  ...
  let generate_regression_report ?(threshold = default_threshold) baseline_results current_results =
    let baseline_map =
      List.fold_left (fun acc metric -> (metric.name, metric) :: acc) [] baseline_results
    in
    ...
    if List.length all_regressions = 0 then [ "✅ 未检测到性能回归" ]
    else
      List.map
        (fun regression ->
          concat_strings
            [
              ...
              " (变化: ";
              string_of_float regression.change_percentage;
              "%)";
            ])
        all_regressions

  (** 生成详细的回归分析报告 *)
  let generate_detailed_regression_report ?(threshold = default_threshold) baseline_results
      current_results =
    let baseline_map =
      List.fold_left (fun acc metric -> (metric.name, metric) :: acc) [] baseline_results
    in
    ...
    let report_header =
      [
        "# 性能回归分析报告\n";
        concat_strings
          [ "检测阈值: 性能下降"; string_of_float (threshold.performance_degradation *. 100.0); "%, " ];
          ...
          (fun regression ->
            concat_strings
              [
                ...
                "\n";
                "- **当前值**: ";
                string_of_float regression.current_value;
                "\n";
                "- **基线值**: ";
                string_of_float regression.baseline_value;
                "\n";
                "- **变化幅度**: ";
                string_of_float regression.change_percentage;
                "%\n";
  ...
  let analyze_performance_regression ?(threshold = default_threshold) baseline_suite current_suite =
    let baseline_metrics =
      List.fold_left (fun acc result -> result.metrics @ acc) [] baseline_suite.results
    in

    let current_metrics =
      List.fold_left (fun acc result -> result.metrics @ acc) [] current_suite.results
    in
