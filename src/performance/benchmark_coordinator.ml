  ...
  let execute_benchmark_tests test_configs test_function data_generator =
    List.map
      (fun config ->
        ...
        let test_data = Benchmark_core.Utils.generate_test_data data_size data_generator in
  ...
  let execute_benchmark_tests_with_memory test_configs test_function data_generator =
    List.map
      (fun config ->
        ...
        let execution_time = end_time -. start_time in
        { name; execution_time; memory_usage; cpu_usage = None; iterations; variance = None })
      test_configs
end
