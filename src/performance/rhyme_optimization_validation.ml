...
module PerformanceTest = struct
  ...
  let compare_results legacy optimized =
    let time_improvement = (legacy.execution_time -. optimized.execution_time) /. legacy.execution_time *. 100.0 in
    let ops_improvement = (optimized.operations_per_second -. legacy.operations_per_second) /. legacy.operations_per_second *. 100.0 in
    let memory_change = if legacy.memory_usage <> 0 then
      float_of_int (legacy.memory_usage - optimized.memory_usage) /. float_of_int legacy.memory_usage *. 100.0
    else 0.0 in
...
let run_validation_tests () =
  ...
  (* 测试韵律查找性能 *)
  printf "--- 韵律查找性能对比 ---\n";
  let legacy_lookup = PerformanceTest.benchmark "传统线性查找" iterations (fun () ->
    List.iter (fun char -> ignore (LegacyRhymeLookup.lookup_rhyme_linear char)) test_chars
  ) in

  let optimized_lookup = PerformanceTest.benchmark "Hashtbl优化查找" iterations (fun () ->
  ...
  (* 测试韵组查找性能 *)
  printf "--- 韵组查找性能对比 ---\n";
  let legacy_group = PerformanceTest.benchmark "传统韵组线性查找" iterations (fun () ->
    List.iter (fun char -> ignore (LegacyRhymeLookup.lookup_rhyme_group_linear char)) test_chars
  ) in

  let optimized_group = PerformanceTest.benchmark "Hashtbl韵组优化查找" iterations (fun () ->
    List.iter (fun char -> ignore (OptimizedRhymeLookup.lookup_rhyme_group_optimized char)) test_chars
  ) in
  ...
  (* 测试字符存在性检查性能 *)
  printf "--- 字符存在性检查性能对比 ---\n";
  let legacy_exists = PerformanceTest.benchmark "传统存在性线性检查" iterations (fun () ->
    List.iter (fun char -> ignore (LegacyRhymeLookup.is_rhyme_char_linear char)) test_chars
  ) in

  let optimized_exists = PerformanceTest.benchmark "Hashtbl存在性优化检查" iterations (fun () ->
    List.iter (fun char -> ignore (OptimizedRhymeLookup.is_rhyme_char_optimized char)) test_chars
  ) in
  ...
  let large_legacy = PerformanceTest.benchmark "大规模传统查找" large_iterations (fun () ->
    List.iter (fun char -> ignore (LegacyRhymeLookup.lookup_rhyme_linear char)) large_test_chars
  ) in

  let large_optimized = PerformanceTest.benchmark "大规模优化查找" large_iterations (fun () ->
    List.iter (fun char -> ignore (OptimizedRhymeLookup.lookup_rhyme_optimized char)) large_test_chars
  ) in
