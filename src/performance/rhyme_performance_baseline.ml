...
module BenchmarkUtils = struct
  ...
  let create_result name execution_time iterations memory_usage =
    ...
    { name; execution_time; operations_per_second = ops_per_sec; 
      memory_usage; iterations }
end
...
module TestData = struct
  ...
  let generate_test_data scale =
    let rec replicate_list n lst acc =
      if n <= 0 then acc
      else replicate_list (n-1) lst (lst @ acc)
    in
  ...
  let generate_query_sequence size =
    ...
    Array.init size (fun i -> 
      List.nth sample_characters (i mod total_chars))
end
...
module RhymeLookupBenchmark = struct
  ...
  let simulate_optimized_lookup char_map target =
    try Some (StringMap.find target char_map)
    with Not_found -> None

  (** 创建Map用于优化测试 *)
  let create_char_map chars =
    List.fold_left (fun acc char -> 
      StringMap.add char char acc) StringMap.empty chars
  ...
  let test_linear_lookup iterations data_scale =
    ...
    let test_fn () =
      Array.iter (fun target ->
        ignore (simulate_current_lookup test_data target)
      ) queries
    in
  ...
  let test_map_lookup iterations data_scale =
    ...
    let test_fn () =
      Array.iter (fun target ->
        ignore (simulate_optimized_lookup char_map target)
      ) queries
    in
...
module CodeDuplicationAnalysis = struct
  ...
  (** 模拟统一的JSON解析器 *)
  let simulate_unified_json_parser data =
    String.length data * 8  (* 相同功能但只有一个实现 *)
...
module ReportGenerator = struct
  ...
  let compare_results baseline optimized =
    let time_improvement = (baseline.execution_time -. optimized.execution_time) /. baseline.execution_time *. 100.0 in
    let ops_improvement = (optimized.operations_per_second -. baseline.operations_per_second) /. baseline.operations_per_second *. 100.0 in
