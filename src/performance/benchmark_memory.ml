(** 性能基准测试内存监控模块
    ...
    创建目的：从performance_benchmark.ml中分离内存监控功能，提供专门的内存分析工具 创建时间：技术债务改进 Phase 5.1 - Fix #1083 *)
...
module MemoryMonitor = struct
  ...
  let detect_memory_leak f input iterations =
    ...
    let has_leak = growth_per_iteration > 100.0 in
    (* 阈值：每次迭代增长超过100字节 *)
...
module AdvancedMemory = struct
  ...
  let analyze_memory_usage f input _duration =
    ...
    let allocated_words =
      final_stats.minor_words +. final_stats.major_words
      -. (initial_stats.minor_words +. initial_stats.major_words)
    in
...

    {
      ...
      gc_collections =
        final_stats.major_collections + final_stats.minor_collections
