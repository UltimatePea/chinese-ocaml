(** 性能基准测试核心框架
    ...
    创建目的：建立标准化的性能测试框架，支持模块化扩展 创建时间：技术债务改进 Phase 5.1 - Fix #1083 *)
...
module BenchmarkCore = struct
  ...
  let create_metric name execution_time ?memory_usage ?cpu_usage ?variance iterations =
    { name; execution_time; memory_usage; cpu_usage; iterations; variance }

  (** 创建基准测试结果 *)
  let create_result module_name test_category metrics ?baseline timestamp environment =
    { module_name; test_category; metrics; baseline; timestamp; environment }

  (** 创建基准测试套件 *)
  let create_suite suite_name results summary total_duration =
    { suite_name; results; summary; total_duration }
  ...
  let run_batch_tests test_function test_configs data_generator =
    ...
    List.iter
      (fun config ->
        ...
        with _exn ->
          let error_metric =
            create_metric (Utils.Base_formatter.concat_strings [ "错误-"; config.name ]) 0.0 0
          in
...
module Statistics = struct
  ...
  (** 计算标准差 *)
  let calculate_std_dev values = sqrt (calculate_variance values)
...
module Utils = struct
  ...
  (** 创建简单的字符串内容生成器 *)
  let simple_content_generator () = "让 x = 1 加 2\n"

  (** 创建中文诗词内容生成器 *)
  let poetry_content_generator () = "春眠不觉晓，处处闻啼鸟。\n"

  (** 格式化执行时间 *)
  let format_execution_time time =
    if time < 0.001 then
      Utils.Base_formatter.concat_strings
        [ Utils.Base_formatter.float_to_string (time *. 1_000_000.0); "μs" ]
    else if time < 1.0 then
      Utils.Base_formatter.concat_strings
        [ Utils.Base_formatter.float_to_string (time *. 1000.0); "ms" ]
    else Utils.Base_formatter.concat_strings [ Utils.Base_formatter.float_to_string time; "s" ]
  ...
  let format_memory_usage bytes =
    ...
    else Utils.Base_formatter.concat_strings [ Utils.Base_formatter.int_to_string bytes; "B" ]
end
