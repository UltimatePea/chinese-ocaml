...
module ParserBenchmark = struct
  ...
  (** 语法分析模拟函数 *)
  let dummy_parser text = String.length text * 2
  ...
  let get_parser_test_configs () =
    [
      ...
      { Benchmark_core.name = "深层嵌套表达式解析"; iterations = 20; data_size = 12; description = "深层嵌套测试" };
    ]

  (** 运行语法分析器基准测试 *)
  let run_parser_benchmark () =
    let test_configs = get_parser_test_configs () in
    TestExecutor.execute_benchmark_tests test_configs dummy_parser
      Benchmark_core.Utils.simple_content_generator

  (** 运行带内存监控的语法分析器基准测试 *)
  let run_parser_benchmark_with_memory () =
    let test_configs = get_parser_test_configs () in
    TestExecutor.execute_benchmark_tests_with_memory test_configs dummy_parser
      Benchmark_core.Utils.simple_content_generator
  ...
  let run_complex_expression_benchmark () =
    let expression_configs =
      [
        { Benchmark_core.name = "表达式深度3"; iterations = 200; data_size = 3; description = "中等深度表达式" };
        { Benchmark_core.name = "表达式深度5"; iterations = 100; data_size = 5; description = "较深表达式" };
        { Benchmark_core.name = "表达式深度8"; iterations = 50; data_size = 8; description = "深层表达式" };
      ]
    in
