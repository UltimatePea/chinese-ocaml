...
val convert_token_list_safe : Token_mapping.Token_definitions_unified.token list -> Lexer_tokens.token option list

(** 转换器管理接口 *)

(** 注册新的转换器 *)
val register_converter : converter_type -> (Token_mapping.Token_definitions_unified.token -> Lexer_tokens.token) -> unit

(** 获取指定类型的转换器 *)
val get_converter : converter_type -> (Token_mapping.Token_definitions_unified.token -> Lexer_tokens.token)

(** 尝试使用指定类型的转换器转换 - 返回option类型 *)
val try_convert_with_type : converter_type -> Token_mapping.Token_definitions_unified.token -> Lexer_tokens.token option
