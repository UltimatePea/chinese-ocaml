(** 骆言内置函数模块接口 - Chinese Programming Language Builtin Functions Module Interface
    ...
    这个模块定义了骆言编程语言的所有内置函数，提供语言的基础功能和系统接口。
    内置函数是语言运行时环境的重要组成部分，覆盖了数学运算、字符串处理、
    输入输出、数据结构操作等核心功能。
...
(** 获取完整的内置函数表
    ...
    该值提供对所有内置函数的统一访问接口。函数表在模块加载时初始化，
    包含了语言支持的所有内置功能。每个函数都以 (函数名, 函数值) 的形式存储。
...
(** 检查指定名称是否为内置函数
    ...
    该函数用于运行时的函数名称解析，帮助区分用户定义函数和内置函数。
    在函数调用过程中，解释器会先检查函数是否为内置函数，然后决定调用路径。
