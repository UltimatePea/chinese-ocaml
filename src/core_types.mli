(** 骆言类型系统核心类型定义接口 - Core Type Definitions Interface
    ...
    这个模块定义了骆言编程语言类型系统的核心数据结构和操作函数。
    类型系统是静态类型语言的基础，负责类型检查、类型推断和类型安全保证。
...
type typ =
  ...
  (** 浮点数类型，表示双精度浮点数值 *)
  | StringType_T
  (** 字符串类型，表示UTF-8编码的字符序列 *)
  | BoolType_T
  (** 布尔类型，表示真值或假值 *)
  | UnitType_T
  (** 单位类型，表示无意义的值，类似于void *)
...
(** TypeScheme (type_params, typ) 其中：
    ...
    类型方案用于表示多态函数和泛型类型，允许类型参数的绑定和实例化。
    例如：'a -> 'a -> 'a 可以表示为 TypeScheme (["a"], FunType_T(...)) *)

module TypeEnv : Map.S with type key = string
(** 类型环境模块，提供类型名称到类型方案的映射
    
    类型环境是类型检查器的核心数据结构，维护当前作用域中所有标识符的类型信息。
    使用高效的映射数据结构实现，支持快速的类型查找和环境扩展。 *)
...
(** 函数重载表模块，管理函数重载的类型信息
    ...
    支持同一函数名称的多个类型签名，用于实现函数重载和特化。
    每个函数名称可以对应多个类型方案，类型检查器在解析时选择最匹配的版本。 *)
...
(** 类型替换模块，管理类型变量的替换操作
    ...
    类型替换是类型推断的核心操作，用于将类型变量替换为具体类型。
    支持复杂的类型替换操作，包括递归替换和循环检测。 *)
...
(** 生成新的类型变量，用于类型推断
    ...
    该函数生成唯一的类型变量标识符，用于类型推断过程中的类型占位。
    生成的类型变量保证在整个推断过程中的唯一性，避免变量名冲突。
...
(** 获取类型中的自由类型变量列表
    ...
    自由变量是指在类型表达式中出现但未被绑定的类型变量。
    该函数递归遍历类型结构，收集所有自由变量：
    - 基础类型：无自由变量
    - 类型变量：变量本身
    - 复合类型：递归收集子类型的自由变量
...
(** 检查类型是否包含指定的类型变量
    ...
    该函数用于检查类型中是否包含特定的类型变量，
    常用于类型推断中的occurs检查，防止无限递归类型的产生。
