...
let convert_token token =
  Token_conversion_unified.convert_token token

(** 批量转换Token列表 - 基于统一转换系统 *)
let convert_token_list tokens =
  try
    Token_conversion_unified.convert_token_list tokens
  with
  | Token_conversion_unified.Unified_conversion_failed (conv_type, msg) ->
      let conv_type_str = match conv_type with
        | `Identifier -> "标识符"
        | `Literal -> "字面量"
        | `BasicKeyword -> "基础关键字"
        | `TypeKeyword -> "类型关键字"
...
let get_conversion_stats () =
  Token_conversion_unified.get_conversion_stats ()