...
let diagnose_error_with_ai error_msg error_context =
  ...
  let error_type =
    if List.exists (fun (_, action) -> action = "undefined" || action = "未定义") error_keywords then
      "未定义错误"
    else if List.exists (fun (_, obj) -> obj = "type" || obj = "类型") error_keywords then "类型错误"
    else if List.exists (fun (_, obj) -> obj = "function" || obj = "函数") error_keywords then "函数错误"
    else "语法错误"
  in
  ...
  (* 上下文分析 *)
  (match error_context.source_location with
  | Some loc -> Buffer.add_string diagnosis (Printf.sprintf "📍 源码位置: %s\n" loc)
  | None -> ());

  (match error_context.function_name with
...
let generate_fix_strategies error_analysis _error_context =
  ...
  | "undefined_variable" ->
      (* 自动修复：变量名纠错 *)
      (match error_analysis.suggestions with
      | suggestion :: _ when String.contains suggestion (String.get "「" 0) -> (
          ...
          | _ :: rest -> (
              ...
              match var_parts with
              | var_name :: _ ->
                  strategies :=
                    AutoFix ("错误的变量名", Printf.sprintf "使用正确的变量名「%s」" var_name) :: !strategies
              | _ -> ())
          | _ -> ())
      | _ -> ());

      (* 建议模式：变量定义 *)
      strategies := SuggestPattern ("变量定义模式", "让 「变量名」 = 值") :: !strategies;

      (* 示例代码 *)
      strategies :=
        ExampleCode ("常见变量定义示例", "让 「用户名」 = \"张三\"\n让 「年龄」 = 25\n让 「分数列表」 = [85; 90; 78]")
        :: !strategies
  | "type_mismatch" ->
      (* 自动修复：类型转换 *)
      (match error_analysis.fix_hints with
      | hint :: _ when String.contains hint '(' ->
          strategies := AutoFix ("类型不匹配", "添加类型转换: " ^ hint) :: !strategies
      | _ -> ());

      (* 重构提示 *)
      strategies := RefactorHint [ "检查变量定义时的类型"; "考虑使用显式类型标注"; "验证函数参数和返回值类型" ] :: !strategies;

      (* 示例代码 *)
      strategies :=
        ExampleCode ("类型转换示例", "让 「数字字符串」 = \"123\"\n让 「数字」 = 转换为整数 「数字字符串」\n让 「文本」 = 转换为字符串 42")
        :: !strategies
  | "function_arity" ->
      (* 建议模式：函数调用 *)
      strategies := SuggestPattern ("正确的函数调用模式", "函数名 参数1 参数2 参数3") :: !strategies;

      (* 示例代码 *)
      strategies :=
        ExampleCode ("函数定义和调用示例", "递归 让 「加法」 = 函数 a b → a + b\n让 「结果」 = 「加法」 10 20") :: !strategies
  | "pattern_match" ->
      (* 建议模式：完整模式匹配 *)
      strategies :=
        SuggestPattern ("完整模式匹配模式", "匹配 表达式 与\n｜ 模式1 → 结果1\n｜ 模式2 → 结果2\n｜ _ → 默认结果") :: !strategies;

      (* 示例代码 *)
      strategies :=
        ExampleCode ("模式匹配示例", "匹配 「数字」 与\n｜ 0 → \"零\"\n｜ 1 → \"一\"\n｜ _ → \"其他数字\"") :: !strategies
  | _ ->
      (* 通用重构提示 *)
      strategies := RefactorHint [ "检查代码语法是否正确"; "验证变量和函数定义"; "查看相关文档和示例" ] :: !strategies);
    ...
    | "undefined_variable" ->
        {
          ...
          cause_analysis = [ "变量名可能存在拼写错误"; "变量可能未在当前作用域中定义"; "变量可能在定义之前被使用" ];
    ...
    | "type_mismatch" ->
        {
          ...
          cause_analysis = [ "函数参数类型与定义不匹配"; "运算符操作数类型不正确"; "变量赋值时类型不兼容" ];
    ...
    | "function_arity" ->
        {
          ...
          cause_analysis = [ "调用函数时遗漏了必需的参数"; "调用函数时提供了多余的参数"; "函数定义与调用处理解不一致" ];
    ...
    | "pattern_match" ->
        {
          ...
          cause_analysis = [ "遗漏了某些可能的匹配模式"; "没有提供默认的通配符模式"; "对数据类型的理解不完整" ];
...
let generate_ai_error_report error_type error_details context =
  ...
  (* 生成修复策略 *)
  let strategies =
    generate_fix_strategies analysis
      {
  ...
  (* AI诊断 *)
  let ai_diagnosis =
    diagnose_error_with_ai analysis.error_message
      {
        source_location = context;
        function_name = None;
        variable_scope = [];
...
let test_intelligent_error_handler () =
  ...
  List.iter
    (fun (error_type, error_details, context) ->
