...
(** 解析古雅体函数定义
    语法：夫 函数名 者 受 参数 焉 算法为 表达式 是谓
    例如：夫 阶乘 者 受 数 焉 算法为 数 乘 数减一 之 阶乘 是谓
*)
val parse_ancient_function_definition : expr_parser -> parser_state -> expr * parser_state

(** 解析古雅体匹配表达式  
    语法：观 标识符 之 性 若 模式 则 表达式 余者 则 表达式 观毕
    例如：观 数 之 性 若 零 则 一 余者 则 数 乘 数减一 之 阶乘 观毕
*)
val parse_ancient_match_expression : expr_parser -> pattern_parser -> parser_state -> expr * parser_state

(** 解析古雅体列表表达式
    语法：列开始 元素1 其一 元素2 其二 元素3 其三 列结束
    例如：列开始 一 其一 二 其二 三 其三 列结束
*)
val parse_ancient_list_expression : expr_parser -> parser_state -> expr * parser_state

(** 解析古雅体条件表达式
    语法：若 条件 则 表达式 否则 表达式
    例如：若 数 等于 零 则 一 否则 数 乘 数减一 之 阶乘
*)
