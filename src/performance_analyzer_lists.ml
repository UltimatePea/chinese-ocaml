...
let list_specific_analysis expr =
  ...
  | FunCallExpr (VarExpr "连接", [ ListExpr _; ListExpr _ ]) ->
      [SuggestionBuilder.list_optimization_suggestion "列表连接" 
         "检测到列表连接操作，对于大量数据建议使用更高效的方法"]
  | FunCallExpr (VarExpr "追加", args) when List.length args >= 2 ->
      [make_performance_suggestion
         ...
         ~confidence:0.60
         ~location:"列表追加操作"
         ~fix:"考虑使用可变数据结构或反向构建再反转"]
  | _ -> []

(** 分析列表操作性能 *)
let analyze_list_performance expr =
  create_performance_analyzer list_specific_analysis expr
