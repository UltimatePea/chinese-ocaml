...
module BasicKeywords : sig
  (** 转换基础语言关键字 *)
  val convert_basic_language_keywords : Token_mapping.Token_definitions_unified.token -> 
    token option
...
module SemanticKeywords : sig
  (** 转换语义关键字 *)
  val convert_semantic_keywords : Token_mapping.Token_definitions_unified.token -> 
    token option
...
module ErrorRecoveryKeywords : sig
  (** 转换错误恢复关键字 *)
  val convert_error_recovery_keywords : Token_mapping.Token_definitions_unified.token -> 
    token option
...
module ModuleKeywords : sig
  (** 转换模块关键字 *)
  val convert_module_keywords : Token_mapping.Token_definitions_unified.token -> 
    token option
...
(** 快速路径转换 - 消除重复代码的公共函数 *)
val convert_common_tokens : Token_mapping.Token_definitions_unified.token -> 
  token option

(** 统一的现代语言转换接口 *)
val convert_modern_token : ?strategy:modern_conversion_strategy -> 
...
(** 获取现代语言token类别 *)
val get_modern_token_category : Token_mapping.Token_definitions_unified.token -> 
  modern_token_category option
...
module BackwardCompatibility : sig
  ...
  (** 主转换函数（抛出异常版本） *)
  val convert_modern_token_exn : Token_mapping.Token_definitions_unified.token -> 
    token
end
