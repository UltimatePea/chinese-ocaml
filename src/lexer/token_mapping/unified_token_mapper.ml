...
let map_token source_token_name value_data =
  ...
  | Some entry -> 
      (try
        ...
        let result_token = match entry.target_token, value_data with
          (* 字面量tokens *)
      ...
      with 
      | exn -> ConversionError (source_token_name, Printexc.to_string exn))
  | None -> NotFound source_token_name

(** 便利的token映射函数，用于不同类型的值 *)

(** 映射整数token *)
let map_int_token value = 
  map_token "IntToken" (Some (Int value))

(** 映射浮点数token *)
let map_float_token value = 
  map_token "FloatToken" (Some (Float value))

(** 映射字符串token *)
let map_string_token value = 
  map_token "StringToken" (Some (String value))

(** 映射布尔token *)
let map_bool_token value = 
  map_token "BoolToken" (Some (Bool value))

(** 映射中文数字token *)
let map_chinese_number_token value = 
  map_token "ChineseNumberToken" (Some (String value))

(** 映射引用标识符token *)
let map_quoted_identifier_token value = 
  map_token "QuotedIdentifierToken" (Some (String value))

(** 映射特殊标识符token *)
let map_special_identifier_token value = 
  map_token "IdentifierTokenSpecial" (Some (String value))

(** 映射关键字token *)
let map_keyword_token keyword_name = 
  map_token keyword_name None

(** 映射运算符token *)
let map_operator_token operator_name = 
  map_token operator_name None

(** 批量映射tokens *)
let map_tokens token_specs =
  List.map (fun (name, value_data) -> 
    (name, map_token name value_data)
  ) token_specs
...
let validate_mapping_result = function
  ...
  | ConversionError (name, error) -> 
      Printf.printf "❌ 转换错误: %s - %s\n" name error

(** 批量验证映射结果 *)
let validate_mapping_results results =
  let (success_count, error_count) = List.fold_left (fun (s, e) (_, result) ->
    match result with
    | Success _ -> (s + 1, e)
    | _ -> (s, e + 1)
  ) (0, 0) results in
...
成功率: %.1f%%
  |} success_count error_count 
     (if success_count + error_count > 0 
      then (float_of_int success_count) /. (float_of_int (success_count + error_count)) *. 100.0 
      else 0.0)
...
平均耗时: %.9f 秒/次
吞吐量: %.0f 次/秒
  |} (iterations * 4) duration (duration /. (float_of_int (iterations * 4)))
