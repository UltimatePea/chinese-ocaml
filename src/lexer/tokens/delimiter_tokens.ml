...
let is_bracket_pair = function
  | LeftParen | RightParen | LeftBracket | RightBracket | LeftBrace | RightBrace | LeftArray
  | RightArray | LeftQuote | RightQuote ->
      true
  | _ -> false
...
let get_matching_bracket = function
  ...
  | LeftArray -> Some RightArray
  | RightArray -> Some LeftArray
  | LeftQuote -> Some RightQuote
  | RightQuote -> Some LeftQuote
  | _ -> None

let is_punctuation = function Comma | Semicolon | Colon | Pipe -> true | _ -> false
