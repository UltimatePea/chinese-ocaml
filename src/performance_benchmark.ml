...
module PerformanceBenchmark = struct
  ...
  (** 委托核心框架函数 *)
  let get_current_timestamp = Performance.Benchmark_coordinator.SuiteCoordinator.get_current_timestamp
  let get_system_environment = Performance.Benchmark_coordinator.SuiteCoordinator.get_system_environment
  let create_benchmark_result = Performance.Benchmark_coordinator.SuiteCoordinator.create_benchmark_result
  ...
  let run_all_module_benchmarks timestamp environment =
    ...
    List.map (fun (module_name, test_category, test_runner) ->
