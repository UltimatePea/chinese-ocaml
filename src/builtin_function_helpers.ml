(** 骆言内置函数辅助工具模块 - 消除参数验证代码重复 * Chinese Programming Language Builtin Function Helpers - Eliminate
    Parameter Validation Code Duplication * Phase 10.1 - Reduce code duplication and standardize
    error handling *)
...
let single_conversion_builtin func_name expect_func converter_func result_wrapper args =
  let param = expect_func (check_single_arg args func_name) func_name in
  try result_wrapper (converter_func param)
  with Failure _ -> runtime_error ("无法将参数转换: " ^ func_name)
...
let wrong_arg_count_error func_name expected actual =
  format_builtin_error func_name "参数数量错误" (Printf.sprintf "期望%d个参数，实际收到%d个参数" expected actual)
...
let type_mismatch_error func_name expected_type actual_value =
  let actual_type =
    ...
    | TupleValue _ -> "元组"
  in
  format_builtin_error func_name "类型错误" (Printf.sprintf "期望%s类型，收到%s" expected_type actual_type)
