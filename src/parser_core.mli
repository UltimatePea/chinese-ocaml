(** 骆言语法分析器核心模块接口 - Chinese Programming Language Parser Core Interface Phase 2A: 整合 parser.mli +
    parser_utils.mli Author: Alpha, 主要工作Agent *)

open Ast
open Lexer

(** ======================================================================== 核心类型定义和异常
    ======================================================================== *)

exception SyntaxError of string * position
(** 语法错误异常 *)

type parser_state = { token_array : positioned_token array; array_length : int; current_pos : int }
(** 解析器状态类型 *)

(** ======================================================================== 统一的错误消息生成函数 - 避免重复的错误模式
    ======================================================================== *)

val make_unexpected_token_error : string -> position -> exn
(** 创建意外词元错误 *)

val make_expected_but_found_error : string -> string -> position -> exn
(** 创建期望错误 *)

(** ======================================================================== 基础解析器状态操作
    ======================================================================== *)

val create_parser_state : positioned_token list -> parser_state
(** 创建解析状态 - 用于单元测试和增量解析 *)

val current_token : parser_state -> positioned_token
(** 获取当前词元 *)

val peek_token : parser_state -> positioned_token
(** 查看下一个词元（不消费） *)

val advance_parser : parser_state -> parser_state
(** 向前移动 *)

val expect_token : parser_state -> token -> parser_state
(** 期望特定词元 *)

val is_token : parser_state -> token -> bool
(** 检查是否为特定词元 *)

(** ======================================================================== 标识符解析
    ======================================================================== *)

val parse_identifier : parser_state -> string * parser_state
(** 解析标识符（严格引用模式）*)

val parse_identifier_allow_keywords : parser_state -> string * parser_state
(** 解析标识符（允许关键字）*)

val parse_wenyan_compound_identifier : parser_state -> string * parser_state
(** 解析wenyan风格的复合标识符 *)

(** ======================================================================== 中文标点符号辅助函数
    ======================================================================== *)

val is_right_paren : token -> bool
(** 检查右括号 *)

val is_left_bracket : token -> bool
(** 检查左方括号 *)

val is_right_bracket : token -> bool
(** 检查右方括号 *)

val is_left_paren : token -> bool
(** 检查左圆括号 *)

val is_left_brace : token -> bool
(** 检查左大括号 *)

val is_right_brace : token -> bool
(** 检查右大括号 *)

val is_comma : token -> bool
(** 检查逗号 *)

val is_semicolon : token -> bool
(** 检查分号 *)

val is_colon : token -> bool
(** 检查冒号 *)

val is_double_colon : token -> bool
(** 检查双冒号 *)

val is_pipe : token -> bool
(** 检查管道符号 *)

val is_arrow : token -> bool
(** 检查箭头 *)

val is_left_array : token -> bool
(** 检查左数组括号 *)

(** ======================================================================== Token分类辅助函数
    ======================================================================== *)

val is_identifier_like : token -> bool
(** 检查是否是标识符类型的token *)

val is_literal_token : token -> bool
(** 检查是否是字面量token *)

val is_type_colon : token -> bool
(** 检查是否是类型注解的双冒号 *)

val is_punctuation : parser_state -> (token -> bool) -> bool
(** 检查当前token是否为指定的标点符号 *)

val expect_token_punctuation : parser_state -> (token -> bool) -> string -> parser_state
(** 期望指定的标点符号 *)

(** ======================================================================== 数字和字面量解析
    ======================================================================== *)

val chinese_digit_to_int : string -> int
(** 中文数字转换为整数 *)

val chinese_number_to_int : string -> int
(** 将中文数字字符串转换为整数 *)

val parse_literal : parser_state -> literal * parser_state
(** 解析字面量 *)

(** ======================================================================== 运算符解析
    ======================================================================== *)

val token_to_binary_op : token -> binary_op option
(** 将token转换为二元运算符 *)

(** ======================================================================== 跳过换行符函数 - 通用版本
    ======================================================================== *)

val skip_newlines : parser_state -> parser_state
(** 跳过换行符 - 通用版本 *)

val skip_newlines_simple : parser_state -> parser_state
(** 跳过换行符 - 简单版本（仅Newline） *)

(** ======================================================================== 诗词解析公共工具函数 - 消除重复代码
    ======================================================================== *)

val parse_identifier_content : parser_state -> string * parser_state
(** 解析标识符内容（支持引用标识符、特殊标识符和字符串） *)

val parse_specific_keyword : parser_state -> string -> parser_state
(** 解析特定关键字（支持引用标识符和特殊标识符） *)

(** ======================================================================== 类型解析公共工具函数 - 消除重复代码
    ======================================================================== *)

val try_parse_basic_type : token -> parser_state -> (type_expr * parser_state) option
(** 尝试解析基本类型 *)

(** ======================================================================== 位置转换函数
    ======================================================================== *)

val lexer_pos_to_compiler_pos : Lexer.position -> Compiler_errors.position
(** 位置转换函数 *)

(** ======================================================================== 主要入口点函数 - 解析器API
    ======================================================================== *)

val parse_program : positioned_token list -> program
(** 主解析函数 - 将词元列表解析为程序AST
    @param tokens 带位置信息的词元列表
    @return 解析后的程序AST *)

val parse_expression : parser_state -> expr * parser_state
(** 解析表达式 - 用于单元测试和REPL
    @param state 解析器状态
    @return 解析后的表达式和更新的解析器状态 *)

val parse_statement : parser_state -> stmt * parser_state
(** 解析语句 - 用于单元测试和增量解析
    @param state 解析器状态
    @return 解析后的语句和更新的解析器状态 *)
