...
let analyze_recursion_performance expr =
  ...
  let rec count_recursive_calls func_name expr depth =
    ...
    | FunCallExpr (VarExpr name, args) when name = func_name ->
        let args_depth = List.fold_left (fun acc arg -> 
          max acc (count_recursive_calls func_name arg (depth + 1))
        ) depth args in
        args_depth + 1
    ...
    | MatchExpr (matched_expr, branches) ->
        ...
        let branches_depth = List.fold_left (fun acc branch ->
          max acc (count_recursive_calls func_name branch.expr depth)
        ) 0 branches in
    ...
    | FunCallExpr (func, args) ->
        ...
        let args_depth = List.fold_left (fun acc arg ->
          max acc (count_recursive_calls func_name arg depth)
        ) 0 args in
...
let analyze_computational_complexity expr =
  ...
  let rec detect_nested_loops expr nesting_level =
    match expr with
    | FunCallExpr (VarExpr func_name, _) when 
        List.mem func_name ["映射"; "过滤"; "折叠"; "遍历"] ->
        if nesting_level >= 2 then
          suggestions := {
            suggestion_type = PerformanceHint "嵌套循环优化";
            message = Unified_logger.Legacy.sprintf "检测到%d层嵌套的循环操作，复杂度可能为O(n^%d)" nesting_level nesting_level;
            confidence = 0.75;
            location = Some "嵌套循环";
...
let analyze_performance_hints expr _context =
  ...
  (* 合并所有建议 *)
  let all_suggestions = 
    list_suggestions @ match_suggestions @ recursion_suggestions @ 
    data_structure_suggestions @ complexity_suggestions in
...
let generate_performance_report suggestions =
  let performance_suggestions = List.filter (function
  ...
  Buffer.add_string report (Unified_logger.Legacy.sprintf "📊 性能问题统计: %d 个\n\n" (List.length performance_suggestions));
  
  if List.length performance_suggestions = 0 then
