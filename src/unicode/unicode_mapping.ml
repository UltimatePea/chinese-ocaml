...
module CharMap = struct
  ...
  (* 新的高性能查找函数 *)
  let find_char_by_name name = Hashtbl.find_opt (Lazy.force name_to_char_tbl) name
  let find_triple_by_name name = Hashtbl.find_opt (Lazy.force name_to_triple_tbl) name
  let find_triple_by_char char_str = Hashtbl.find_opt (Lazy.force char_to_triple_tbl) char_str
end
...
module Legacy = struct
  ...
  (** 查找字符对应的UTF-8三元组 - 优化版 *)
  let find_triple_by_char char_str = CharMap.find_triple_by_char char_str

  (** 查找名称对应的字符 - 优化版 *)
  let find_char_by_name name = CharMap.find_char_by_name name
...
module Optimized = struct
  ...
  let batch_find_chars_by_names names =
    ...
    List.filter_map
      (fun name ->
        match Hashtbl.find_opt tbl name with Some char -> Some (name, char) | None -> None)
      names

  type mapping_stats = { total_definitions : int; hash_table_size : int; cache_size : int }
