...
module OptimizedLookup = struct
  ...
  let find_char_by_name_fast name =
    try Some (Hashtbl.find name_to_char_table name) with Not_found -> None

  (** O(1)查找：根据字符查找字节三元组 *)
  let find_bytes_by_char_fast char_str =
    try Some (Hashtbl.find char_to_bytes_table char_str) with Not_found -> None

  (** O(1)查找：根据字符名称查找字节三元组 *)
  let find_bytes_by_name_fast name =
    try Some (Hashtbl.find name_to_bytes_table name) with Not_found -> None

  (** O(1)查找：根据类别查找字符定义列表 *)
  let find_definitions_by_category_fast category =
    try Some (Hashtbl.find category_to_definitions_table category) with Not_found -> None

  (** 获取所有已知字符名称 *)
  let get_all_char_names () = Hashtbl.fold (fun name _ acc -> name :: acc) name_to_char_table []

  (** 获取所有已知字符 *)
  let get_all_chars () = Hashtbl.fold (fun _ char acc -> char :: acc) name_to_char_table []
...
module FastChecks = struct
  ...
  let is_likely_chinese_punctuation first_byte = List.mem first_byte chinese_punctuation_first_bytes

  (** 快速检查字节三元组是否为已知的中文字符 *)
  let is_known_chinese_char_bytes (b1, b2, b3) =
    let char_found = ref false in
    Hashtbl.iter
      (fun _ bytes -> if bytes = (b1, b2, b3) then char_found := true)
      OptimizedLookup.char_to_bytes_table;
    !char_found

  (** 快速检查字符串是否为已知的中文字符 *)
  let is_known_chinese_char char_str = Hashtbl.mem OptimizedLookup.char_to_bytes_table char_str

  (** 快速批量检查字符列表 *)
  let check_char_list chars =
    List.map (fun char_str -> (char_str, is_known_chinese_char char_str)) chars
end
...
module AdvancedAPI = struct
  ...
  type smart_find_result =
    [ `FoundByName of string * byte_triple option | `FoundByChar of string * byte_triple option ]
  ...
  (** 批量字符查找 *)
  let batch_find inputs = List.map (fun input -> (input, smart_find input)) inputs
...
module OptimizedLegacyAPI = struct
  ...
  let get_char_bytes_by_name name =
    match OptimizedLookup.find_bytes_by_name_fast name with Some bytes -> bytes | None -> (0, 0, 0)
