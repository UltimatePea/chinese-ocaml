(** 骆言编译器Unicode字符处理常量模块 - 统一版本
    ...
    本模块提供中文诗词编程语言所需的所有Unicode字符常量定义。 字符按照功能分类组织，支持：
    - 中文标点符号（引号、括号、逗号等）
    - 全角符号（数字、字母、操作符）
    - 箭头符号（赋值、逻辑推导）
    - 中文数字（零到万亿）
...
type char_definition = { name : string; char : string; bytes : byte_triple; category : string }
(** 字符定义记录类型 *)
    ...
    [
      {
        ...
        char = "（";
        bytes = left_parentheses_bytes;
        category = "punctuation";
      };
      {
        name = "right_parentheses";
        char = "）";
        bytes = right_parentheses_bytes;
        category = "punctuation";
      };
    ...
    [
      {
        ...
        char = "【";
        bytes = left_square_bracket_bytes;
        category = "symbol";
      };
      {
        name = "right_square_bracket";
        char = "】";
        bytes = right_square_bracket_bytes;
        category = "symbol";
      };
...
module UnifiedCharDefinitions = struct
  ...
  (** 按字符查找定义 *)
  let find_by_char char_str = List.find_opt (fun def -> def.char = char_str) all_char_definitions

  (** 按名称查找定义 *)
  let find_by_name name = List.find_opt (fun def -> def.name = name) all_char_definitions
end
