...
let analyze_conditional_expression cond then_expr else_expr new_ctx analyze suggestions =
  ...
  if has_nested_conditions && updated_ctx.nesting_level >= 1 then
    let complexity_suggestion = {
      ...
      message = concat_strings ["检测到嵌套条件表达式（"; int_to_string updated_ctx.nesting_level; "层），可能影响代码可读性"];
      confidence = 0.75;
      location = Some "条件表达式";
  ...
  if updated_ctx.nesting_level >= 2 then
    let performance_hint = {
      ...
      message = concat_strings ["条件表达式嵌套深度达到"; int_to_string updated_ctx.nesting_level; "层，建议优化"];
      confidence = 0.70;
      location = Some "条件表达式";
