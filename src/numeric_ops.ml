...
let create_numeric_folder (op : numeric_binary_op) (error_msg : string) =
 fun acc elem -> try apply_numeric_binary_op op acc elem with _ -> runtime_error error_msg

(** 预定义的常用数值操作 *)

(** 加法操作 *)
let add_op = { int_op = ( + ); float_op = ( +. ); mixed_op = ( +. ) }

(** 最大值操作 *)
let max_op = { int_op = max; float_op = max; mixed_op = max }

(** 最小值操作 *)
let min_op = { int_op = min; float_op = min; mixed_op = min }

(** 乘法操作 *)
let multiply_op = { int_op = ( * ); float_op = ( *. ); mixed_op = ( *. ) }
...
(** 数值类型检查 *)
let is_numeric = function IntValue _ | FloatValue _ -> true | _ -> false

(** 验证列表是否为数值列表 *)
let validate_numeric_list lst function_name =
  if List.for_all is_numeric lst then lst else runtime_error (function_name ^ "只能用于数字列表")
