(** 骆言内置工具函数模块接口 - Chinese Programming Language Builtin Utility Functions Module Interface
    ...
    这个模块提供了骆言编程语言的实用工具相关内置函数，包括文件过滤、
    注释处理、字符串清理等辅助功能。工具函数为语言的文本处理和
    代码分析功能提供基础支持，特别是针对骆言语言特有的语法特性。
...
(** 过滤.ly文件
    ...
    从给定的文件名列表中筛选出扩展名为.ly的文件。
    这个函数专门用于处理骆言语言源文件的识别。
...
(** 移除井号注释
    ...
    从给定的字符串中移除井号（#）开始的行尾注释。
    处理单行的井号注释，保留注释符号之前的内容。
...
(** 移除双斜杠注释
    ...
    从给定的字符串中移除双斜杠（//）开始的行尾注释。
    处理C风格的单行注释，保留注释符号之前的内容。
...
(** 移除骆言字符串
    ...
    从给定的字符串中移除骆言语言特有的字符串常量。
    这个函数用于代码分析时排除字符串内容的干扰。
