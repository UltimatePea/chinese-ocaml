(** 骆言词法分析器 - 工具函数模块接口 (模块化重构版本)
    ...
    本模块提供词法分析器所需的各种实用工具函数， 包括字符处理、字符串解析、数字解析等功能。 这是模块化重构版本，提高了代码的可维护性。
...
val create_unsupported_char_error :
  string -> Lexer_tokens.position -> Compiler_errors.compiler_error
(** 创建不支持字符错误 *)
