...
let create_position ~filename ~line ~column = 
  { filename; line; column }

let position_from_line_col ~filename ~line ~column = 
  create_position ~filename ~line ~column

let unknown_position ~filename = 
  create_position ~filename ~line:0 ~column:0
...
let suggest_type_fix ~expected ~actual =
  [
    Printf.sprintf "期望类型：%s，实际类型：%s" expected actual;
    "检查变量类型是否正确";
    "考虑使用类型转换函数";
  ]

let suggest_syntax_fix ~expected =
  [
    Printf.sprintf "期望：%s" expected;
    "检查语法是否正确";
    "参考语言规范文档";
  ]
...
let create_codegen_error ~context ?(suggestions=[]) msg =
  let error_info = {
    error = CodegenError (msg, context);
    severity = Error;
...
let legacy_parse_error msg line column =
  ...
  create_parse_error ~pos msg 
    ~suggestions:["检查语法规范"; "使用现代解析器"]

let legacy_codegen_error msg context =
  create_codegen_error ~context msg 
    ~suggestions:["检查代码生成逻辑"; "使用现代代码生成器"]

let legacy_semantic_error msg context =
  create_semantic_error msg ~context 
    ~suggestions:["检查语义分析规则"; "使用现代语义分析器"]