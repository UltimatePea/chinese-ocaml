...
module Unified_logger_compat = struct
  ...
  module Performance = struct
    let compilation_stats ~files_compiled ~total_time ~memory_used =
      Unified_logging.infof "Performance" "编译完成: %d文件, 耗时%.2f秒, 内存%dMB" 
        files_compiled total_time memory_used
        
    let cache_stats ~hits ~misses ~hit_rate =
      Unified_logging.infof "Cache" "缓存统计: 命中%d次, 未命中%d次, 命中率%.1f%%" 
...
module Logger_utils_compat = struct
  ...
  let init_all_loggers module_name =
    Unified_logging.create_module_logger module_name
...
let create_migration_report module_files =
  ...
  let migrated_count = List.fold_left (fun acc file ->
    if is_module_migrated file then acc + 1 else acc
  ) 0 module_files in
  
  Printf.sprintf "迁移进度报告:\n总文件数: %d\n已迁移: %d\n待迁移: %d\n进度: %.1f%%"
    total_files migrated_count (total_files - migrated_count)
    (if total_files > 0 then (float_of_int migrated_count) /. (float_of_int total_files) *. 100.0 else 0.0)
...
let suggest_migration_order module_files =
  ...
  let other_modules = List.filter (fun m -> 
    not (List.mem m priority_modules) && not (List.mem m core_modules)
  ) module_files in
