...
(** 转换结果类型 *)
type conversion_result = Success of positioned_token | Failure of conversion_error
...
module ConverterRegistry = struct
  ...
  let register_converter entry =
    let existing_converters =
      try Hashtbl.find registry entry.converter_type with Not_found -> []
    in
  ...
  let get_all_converters () =
    Hashtbl.fold
      (fun conv_type entries acc -> (conv_type, List.filter (fun e -> e.enabled) entries) :: acc)
      registry []
  ...
  let get_stats () =
    ...
    let enabled_count =
      Hashtbl.fold
        (fun _ entries acc -> acc + List.length (List.filter (fun e -> e.enabled) entries))
        registry 0
    in
  ...
  let convert_int_literal text pos =
    ...
    with _ ->
      Failure
        {
          source_text = text;
          error_message = "Invalid integer literal";
          position = Some pos;
          conversion_type = "IntLiteral";
          timestamp = Unix.time ();
        }
  ...
  let convert_float_literal text pos =
    ...
    with _ ->
      Failure
        {
          source_text = text;
          error_message = "Invalid float literal";
          position = Some pos;
          conversion_type = "FloatLiteral";
          timestamp = Unix.time ();
        }
...
module KeywordConverters = struct
  ...
  let keyword_mappings =
    [
      ...
      ("且", Keywords.AndKeyword);
      ("或", Keywords.OrKeyword);
      ...
      ("int", Keywords.IntTypeKeyword);
      ("整数", Keywords.IntTypeKeyword);
      ("float", Keywords.FloatTypeKeyword);
      ("浮点", Keywords.FloatTypeKeyword);
      ("string", Keywords.StringTypeKeyword);
      ("字符串", Keywords.StringTypeKeyword);
      ("bool", Keywords.BoolTypeKeyword);
      ("布尔", Keywords.BoolTypeKeyword);
      (* 文言文关键字 *)
      ("若", Keywords.WenyanIf);
  ...
  let convert_keyword text pos =
    ...
    with Not_found ->
      Failure
        {
          source_text = text;
          error_message = "Unknown keyword";
          position = Some pos;
          conversion_type = "Keyword";
          timestamp = Unix.time ();
        }
end
...
module OperatorConverters = struct
  let operator_mappings =
    [
      ...
      ("||", Operators.LogicalOr);
      ("->", Operators.Arrow);
      ("=>", Operators.DoubleArrow);
      ("|>", Operators.PipeForward);
      ("<|", Operators.PipeBackward);
    ]
...
module DelimiterConverters = struct
  let delimiter_mappings =
    [
      ...
      (* 中文分隔符 *)
      ("（", Delimiters.ChineseLeftParen);
      ("）", Delimiters.ChineseRightParen);
      ("[", Delimiters.ChineseLeftBracket);
      ("]", Delimiters.ChineseRightBracket);
      ("，", Delimiters.ChineseComma);
      ("；", Delimiters.ChineseSemicolon);
      ("：", Delimiters.ChineseColon);
      ("｜", Delimiters.ChinesePipe);
    ]
  ...
  let convert_delimiter text pos =
    ...
    with Not_found ->
      Failure
        {
          source_text = text;
          error_message = "Unknown delimiter";
          position = Some pos;
          conversion_type = "Delimiter";
          timestamp = Unix.time ();
        }
end
...
module SmartConverter = struct
  ...
  let convert_smart text pos =
    (* 按优先级尝试不同的转换器 *)
    if is_string_literal text then LiteralConverters.convert_string_literal text pos
    else if is_quoted_identifier text then IdentifierConverters.convert_identifier text pos
    else if is_numeric text then
      if String.contains text '.' then LiteralConverters.convert_float_literal text pos
      else LiteralConverters.convert_int_literal text pos
    else
      (* 尝试关键字转换 *)
  ...
  lazy
    [
      (* 字面量转换器 *)
      {
        ...
        name = "IntLiteral";
        priority = 1;
        func = LiteralConverters.convert_int_literal;
        enabled = Some true;
      };
      {
        converter_type = LiteralConverter;
        name = "FloatLiteral";
        priority = 2;
        func = LiteralConverters.convert_float_literal;
        enabled = Some true;
      };
      {
        converter_type = LiteralConverter;
        name = "StringLiteral";
        priority = 3;
        func = LiteralConverters.convert_string_literal;
        enabled = Some true;
      };
      {
        converter_type = LiteralConverter;
        name = "BoolLiteral";
        priority = 4;
        func = LiteralConverters.convert_bool_literal;
        enabled = Some true;
      };
      (* 关键字转换器 *)
      {
        ...
        name = "BasicKeyword";
        priority = 1;
        func = KeywordConverters.convert_keyword;
        enabled = Some true;
      };
      (* 操作符转换器 *)
      {
        ...
        name = "BasicOperator";
        priority = 1;
        func = OperatorConverters.convert_operator;
        enabled = Some true;
      };
      (* 标识符转换器 *)
      {
        ...
        priority = 1;
        func = IdentifierConverters.convert_identifier;
        enabled = Some true;
      };
      (* 分隔符转换器 *)
      {
        ...
        name = "BasicDelimiter";
        priority = 1;
        func = DelimiterConverters.convert_delimiter;
        enabled = Some true;
      };
    ]
...
let register_default_converters () : registration_result list =
  let register_single config =
    try
      ...
      RegisterSuccess
    with e ->
      RegisterError (Printf.sprintf "Failed to register %s: %s" config.name (Printexc.to_string e))
  in
...
(** 批量转换接口 *)
let batch_convert token_texts = List.map (fun (text, pos) -> (text, convert text pos)) token_texts
  ...
  let rec try_converters = function
    | [] ->
        Failure
          {
            source_text = text;
            error_message = "No suitable converter found";
            position = Some pos;
            conversion_type = "TypeSpecific";
            timestamp = Unix.time ();
          }
    | entry :: rest -> (
...
let validate_conversion_result = function
  | Success (token, _) -> Printf.printf "✅ 转换成功: %s\n" (TokenUtils.token_to_string token)
  | Failure error -> Printf.printf "❌ 转换失败: %s - %s\n" error.source_text error.error_message
...
let initialize () =
  ...
  let success_count =
    List.fold_left
      (fun acc -> function RegisterSuccess -> acc + 1 | RegisterError _ -> acc)
      0 results
  in
  ...
  if error_count > 0 then (
    ...
    List.iter
      (function RegisterError msg -> Printf.printf "   - %s\n" msg | RegisterSuccess -> ())
      results);
