     ...
     | Some new_env -> 
       ...
       (match branch.guard with
        ...
        | Some guard_expr ->
          ...
          (match guard_result with
           | BoolValue true -> eval_expr new_env branch.expr  (* guard通过 *)
           | BoolValue false -> execute_exception_match env exc_val rest_branches  (* guard失败，尝试下一个分支 *)
...
let builtin_functions = [
  ...
  ("排序", BuiltinFunctionValue (function
    | [ListValue lst] ->
  ...
  ("反转", BuiltinFunctionValue (function
  ...
  ("包含", BuiltinFunctionValue (function
    | [search_val] ->
      BuiltinFunctionValue (function
  ...
  ("求和", BuiltinFunctionValue (function
    | [ListValue lst] ->
  ...
  ("最大值", BuiltinFunctionValue (function
    | [ListValue lst] ->
      (match lst with
  ...
  ("最小值", BuiltinFunctionValue (function
    | [ListValue lst] ->
      (match lst with
