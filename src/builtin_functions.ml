...
let get_builtin_function_stats () =
  ...
  Printf.sprintf "内置函数统计: I/O(%d), 集合(%d), 数学(%d), 字符串(%d), 数组(%d), 类型转换(%d), 工具(%d), 常量(%d), 总计(%d)"
    io_count collection_count math_count string_count array_count type_count util_count const_count
