...
let convert_to_legacy_token = function
  ...
  | Identifier (SimpleIdentifier _) -> error_result (ConversionError ("simple_identifier", "legacy_token"))
...
let check_compatibility_status () =
  let supported_conversions = [
    "literals"; "basic_identifiers"; "core_keywords"; 
    "basic_operators"; "basic_delimiters"; "special_tokens"
  ] in
  let unsupported_conversions = [
    "wenyan_keywords"; "ancient_keywords"; "natural_language_keywords";
    "semantic_keywords"; "error_handling_keywords"; "module_system_keywords";
    "macro_system_keywords"; "simple_identifiers"
  ] in
  (supported_conversions, unsupported_conversions)

(** 生成迁移建议 *)
let generate_migration_suggestions unsupported_tokens =
  let suggestions = List.map (fun token ->
    match token with
...
let generate_compatibility_report tokens =
  ...
  let (converted, failed, unsupported, errors) = List.fold_left (fun (conv, fail, unsup, errs) token ->
...
let format_compatibility_report report =
  ...
  let unsupported_info = List.mapi (fun i token ->
    Printf.sprintf "%d. %s" (i + 1) (match token with
      | Wenyan _ -> "文言文关键字"
      | Ancient _ -> "古雅体关键字"
      | NaturalLanguage _ -> "自然语言关键字"
      | _ -> "其他不支持类型")
  ) report.unsupported_tokens in
  let suggestion_lines = "" :: "迁移建议:" :: List.mapi (fun i suggestion ->
    Printf.sprintf "%d. %s" (i + 1) suggestion
  ) report.suggestions in
