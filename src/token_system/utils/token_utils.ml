...
module Debug = struct
  ...
  let rec token_to_debug_string = function
    ...
    | NaturalLanguage keyword -> Printf.sprintf "NaturalLanguage(%s)" (natural_keyword_to_string keyword)
  ...
  and core_keyword_to_string = function
    | LetKeyword -> "Let" | RecKeyword -> "Rec" | InKeyword -> "In"
    | FunKeyword -> "Fun" | ParamKeyword -> "Param"
    | IfKeyword -> "If" | ThenKeyword -> "Then" | ElseKeyword -> "Else"
    | MatchKeyword -> "Match" | WithKeyword -> "With" | OtherKeyword -> "Other"
    | TypeKeyword -> "Type" | PrivateKeyword -> "Private" | OfKeyword -> "Of"
    | TrueKeyword -> "True" | FalseKeyword -> "False"
    | AndKeyword -> "And" | OrKeyword -> "Or" | NotKeyword -> "Not"
  
  and semantic_keyword_to_string = function
    | AsKeyword -> "As" | CombineKeyword -> "Combine"
    | WithOpKeyword -> "WithOp" | WhenKeyword -> "When"
  
  and error_keyword_to_string = function
    | OrElseKeyword -> "OrElse" | WithDefaultKeyword -> "WithDefault"
    | ExceptionKeyword -> "Exception" | RaiseKeyword -> "Raise"
    | TryKeyword -> "Try" | CatchKeyword -> "Catch" | FinallyKeyword -> "Finally"
  
  and module_keyword_to_string = function
    | ModuleKeyword -> "Module" | ModuleTypeKeyword -> "ModuleType"
    | SigKeyword -> "Sig" | EndKeyword -> "End" | FunctorKeyword -> "Functor"
    | IncludeKeyword -> "Include" | RefKeyword -> "Ref"
  
  and macro_keyword_to_string = function
    | MacroKeyword -> "Macro" | ExpandKeyword -> "Expand"
  
  and wenyan_keyword_to_string = function
    | HaveKeyword -> "Have" | OneKeyword -> "One" | NameKeyword -> "Name"
    | SetKeyword -> "Set" | AlsoKeyword -> "Also" | ThenGetKeyword -> "ThenGet"
    | CallKeyword -> "Call" | ValueKeyword -> "Value" | AsForKeyword -> "AsFor"
    | NumberKeyword -> "Number" | WantExecuteKeyword -> "WantExecute"
    | MustFirstGetKeyword -> "MustFirstGet" | ForThisKeyword -> "ForThis"
    | TimesKeyword -> "Times" | EndCloudKeyword -> "EndCloud"
    | IfWenyanKeyword -> "IfWenyan" | ThenWenyanKeyword -> "ThenWenyan"
    | GreaterThanWenyan -> "GreaterThanWenyan" | LessThanWenyan -> "LessThanWenyan"
  
  and ancient_keyword_to_string = function
    | AncientDefineKeyword -> "Define" | AncientEndKeyword -> "End"
    | AncientAlgorithmKeyword -> "Algorithm" | AncientCompleteKeyword -> "Complete"
    | AncientObserveKeyword -> "Observe" | AncientNatureKeyword -> "Nature"
    | AncientIfKeyword -> "If" | AncientThenKeyword -> "Then"
    | AncientOtherwiseKeyword -> "Otherwise" | AncientAnswerKeyword -> "Answer"
    | AncientRecursiveKeyword -> "Recursive" | AncientCombineKeyword -> "Combine"
    | AncientAsOneKeyword -> "AsOne" | AncientTakeKeyword -> "Take"
    | AncientReceiveKeyword -> "Receive" | AncientParticleOf -> "ParticleOf"
    | AncientParticleFun -> "ParticleFun" | AncientParticleThe -> "ParticleThe"
    | AncientCallItKeyword -> "CallIt" | AncientEmptyKeyword -> "Empty"
    | AncientIsKeyword -> "Is" | AncientArrowKeyword -> "Arrow"
    | AncientWhenKeyword -> "When" | AncientCommaKeyword -> "Comma"
    | AncientPeriodKeyword -> "Period" | AfterThatKeyword -> "AfterThat"
  
  and natural_keyword_to_string = function
    | DefineKeyword -> "Define" | AcceptKeyword -> "Accept"
    | ReturnWhenKeyword -> "ReturnWhen" | ElseReturnKeyword -> "ElseReturn"
    | MultiplyKeyword -> "Multiply" | DivideKeyword -> "Divide"
    | AddToKeyword -> "AddTo" | SubtractKeyword -> "Subtract"
    | IsKeyword -> "Is" | EqualToKeyword -> "EqualTo"
    | LessThanEqualToKeyword -> "LessThanEqualTo" | FirstElementKeyword -> "FirstElement"
    | RemainingKeyword -> "Remaining" | EmptyKeyword -> "Empty"
    | CharacterCountKeyword -> "CharacterCount" | OfParticle -> "OfParticle"
  
  and operator_to_string = function
    | Plus -> "+" | Minus -> "-" | Multiply -> "*" | Divide -> "/"
    | Equal -> "=" | NotEqual -> "<>" | LessThan -> "<" | LessThanOrEqual -> "<="
    | GreaterThan -> ">" | GreaterThanOrEqual -> ">=" | LogicalAnd -> "&&"
    | LogicalOr -> "||" | Assignment -> ":=" | Arrow -> "->" | DoubleArrow -> "=>"
  
  and delimiter_to_string = function
    | LeftParen -> "(" | RightParen -> ")" | LeftBracket -> "[" | RightBracket -> "]"
    | LeftBrace -> "{" | RightBrace -> "}" | Semicolon -> ";" | Comma -> ","
    | Dot -> "." | Colon -> ":" | DoubleColon -> "::" | Pipe -> "|" | Underscore -> "_"
  
  and special_to_string = function
    | EOF -> "EOF" | Newline -> "Newline" 
    | Whitespace _ -> "Whitespace" | Comment _ -> "Comment"
  
  (** 打印Token列表 *)
  let print_token_list tokens =
    List.iteri (fun i token ->
      Printf.printf "%d: %s\n" i (token_to_debug_string token)
    ) tokens
  ...
  let print_positioned_token_list positioned_tokens =
    List.iteri (fun i pt ->
      Printf.printf "%d: %s at line %d, col %d\n" 
        i (token_to_debug_string pt.token) pt.position.line pt.position.column
    ) positioned_tokens
...
module Filter = struct
  ...
  (** 过滤出所有关键字 *)
  let filter_keywords tokens =
    filter_by_category CategoryKeyword tokens
  
  (** 过滤出所有字面量 *)
  let filter_literals tokens =
    filter_by_category CategoryLiteral tokens
  
  (** 过滤出所有标识符 *)
  let filter_identifiers tokens =
    filter_by_category CategoryIdentifier tokens
  
  (** 过滤出所有操作符 *)
  let filter_operators tokens =
    filter_by_category CategoryOperator tokens
  
  (** 过滤出所有分隔符 *)
  let filter_delimiters tokens =
    filter_by_category CategoryDelimiter tokens
  
  (** 查找特定Token *)
  let find_token target_token tokens =
    List.find_opt (fun token -> token = target_token) tokens
  
  (** 计算Token出现次数 *)
  let count_token target_token tokens =
    List.fold_left (fun count token ->
      if token = target_token then count + 1 else count
    ) 0 tokens
  
  (** 检查是否包含特定Token *)
  let contains_token target_token tokens =
    List.exists (fun token -> token = target_token) tokens
...
module Stream = struct
  ...
  let create_stream_from_tokens tokens =
    List.mapi (fun i token ->
      create_positioned_token token 1 (i + 1) i ""
    ) tokens
  
  (** 提取Token流中的Token *)
  let extract_tokens stream =
    List.map (fun pt -> pt.token) stream
  
  (** 提取Token流中的文本 *)
  let extract_texts stream =
    List.map (fun pt -> pt.text) stream
...
module Validation = struct
  ...
  let validate_basic_syntax stream =
    ...
    let rec check_brackets stack = function
      | [] -> if stack = [] then ok_result () else error_result (ParsingError ("括号不匹配", { line = 0; column = 0; offset = 0 }))
      | token :: rest ->
        match token with
  ...
  let validate_token token =
    match token with
    | Literal _ | Identifier _ | CoreLanguage _ | Semantic _
    | ErrorHandling _ | ModuleSystem _ | MacroSystem _ | Wenyan _
    | Ancient _ | NaturalLanguage _ | Operator _ | Delimiter _ | Special _ ->
      ok_result token
...
module Statistics = struct
  ...
  let generate_comprehensive_stats tokens =
    ...
    let by_type = List.fold_left (fun acc token ->
      let type_name = match token with
        | Literal _ -> "literals"
  ...
  let format_stats_report stats =
    ...
    let stat_lines = List.map (fun (name, count) ->
