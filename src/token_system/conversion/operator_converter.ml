...
let operator_mappings = [
  (* 算术操作符 *)
  ("+", Operator Plus); ("加", Operator Plus);
  ("-", Operator Minus); ("减", Operator Minus);
  ("*", Operator Multiply); ("乘", Operator Multiply); ("×", Operator Multiply);
  ("/", Operator Divide); ("除", Operator Divide); ("÷", Operator Divide);
  
  (* 比较操作符 *)
  ("=", Operator Equal); ("等于", Operator Equal);
  ("<>", Operator NotEqual); ("不等于", Operator NotEqual); ("≠", Operator NotEqual);
  ("<", Operator LessThan); ("小于", Operator LessThan);
  ("<=", Operator LessThanOrEqual); ("小于等于", Operator LessThanOrEqual); ("≤", Operator LessThanOrEqual);
  (">", Operator GreaterThan); ("大于", Operator GreaterThan);
  (">=", Operator GreaterThanOrEqual); ("大于等于", Operator GreaterThanOrEqual); ("≥", Operator GreaterThanOrEqual);
  
  (* 逻辑操作符 *)
  ("&&", Operator LogicalAnd); ("并且", Operator LogicalAnd);
  ("||", Operator LogicalOr); ("或者", Operator LogicalOr);
  
  (* 赋值和箭头 *)
  (":=", Operator Assignment); ("赋值", Operator Assignment);
  ("->", Operator Arrow); ("箭头", Operator Arrow); ("→", Operator Arrow);
  ("=>", Operator DoubleArrow); ("双箭头", Operator DoubleArrow); ("⇒", Operator DoubleArrow);
]

(** 分隔符映射表 *)
let delimiter_mappings = [
  (* 括号 *)
  ("(", Delimiter LeftParen); ("左括号", Delimiter LeftParen);
  (")", Delimiter RightParen); ("右括号", Delimiter RightParen);
  ("[", Delimiter LeftBracket); ("左方括号", Delimiter LeftBracket);
  ("]", Delimiter RightBracket); ("右方括号", Delimiter RightBracket);
  ("{", Delimiter LeftBrace); ("左大括号", Delimiter LeftBrace);
  ("}", Delimiter RightBrace); ("右大括号", Delimiter RightBrace);
  
  (* 标点符号 *)
  (";", Delimiter Semicolon); ("分号", Delimiter Semicolon);
  (",", Delimiter Comma); ("逗号", Delimiter Comma);
  (".", Delimiter Dot); ("点", Delimiter Dot);
  (":", Delimiter Colon); ("冒号", Delimiter Colon);
  ("::", Delimiter DoubleColon); ("双冒号", Delimiter DoubleColon);
  ("|", Delimiter Pipe); ("管道", Delimiter Pipe);
  ("_", Delimiter Underscore); ("下划线", Delimiter Underscore);
]
...
let create_lookup_tables mappings =
  ...
  List.iter (fun (text, token) ->
    Hashtbl.add text_to_token text token;
    (* 只存储第一个（通常是符号形式）作为默认输出 *)
    if not (Hashtbl.mem token_to_text token) then
      Hashtbl.add token_to_text token text
  ) mappings;
...
let operator_converter = 
  let module OperatorConverter = struct
    ...
    let token_to_string config token =
      try
        ...
        if config.enable_aliases then
          (* 查找中文版本 - 使用简化的检查 *)
          let chinese_version = List.find_opt (fun (t, tok) -> 
            tok = token && String.length t > 1
          ) operator_mappings in
          ...
          ok_result text
      with Not_found ->
        error_result (ConversionError ("operator_token", "string"))
    ...
    let can_handle_token token =
      Hashtbl.mem op_token_to_text token
...
let delimiter_converter = 
  let module DelimiterConverter = struct
    ...
    let token_to_string config token =
      try
        ...
        if config.enable_aliases then
          ...
          ok_result text
      with Not_found ->
        error_result (ConversionError ("delimiter_token", "string"))
    ...
    let can_handle_token token =
      Hashtbl.mem delim_token_to_text token
...
let is_binary_operator = function
  | Operator (Plus | Minus | Multiply | Divide | Equal | NotEqual
...
let is_unary_operator = function
  | Operator Minus -> true  (* 负号可以是一元操作符 *)
  | _ -> false

(** 检查是否为比较操作符 *)
let is_comparison_operator = function
  | Operator (Equal | NotEqual | LessThan | LessThanOrEqual 
            | GreaterThan | GreaterThanOrEqual) -> true
  | _ -> false
...
let is_logical_operator = function
  | Operator (LogicalAnd | LogicalOr) -> true
  | _ -> false
...
let get_operator_delimiter_stats token_list =
  let count_ops = List.fold_left (fun acc token ->
    match token with Operator _ -> acc + 1 | _ -> acc
  ) 0 token_list in
  let count_delims = List.fold_left (fun acc token ->
    match token with Delimiter _ -> acc + 1 | _ -> acc
  ) 0 token_list in
  [
    ("operators", count_ops);
    ("delimiters", count_delims);
    ("total", count_ops + count_delims);
  ]