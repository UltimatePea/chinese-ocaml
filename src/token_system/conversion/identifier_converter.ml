    ...
    let rec check_chars i =
      if i >= len then true
        ...
        let is_valid_char = 
          ...
          (Char.code c >= 128)
        in
        if is_valid_char then check_chars (i + 1)
        else false
    in
    (* 检查首字符不能是数字 *)
    let first_char = String.get text 0 in
    if first_char >= '0' && first_char <= '9' then false
    else check_chars 0
  ...
  let module IdentifierConverter = struct
    ...
    let string_to_token config text =
      ...
      if len >= 6 && String.sub text 0 3 = "「" && String.sub text (len - 3) 3 = "」" then
        ...
        ok_result (Identifier (QuotedIdentifierToken content))
      (* 检查是否为特殊标识符（以$开头） *)
      else if len >= 2 && String.get text 0 = '$' then
        let content = String.sub text 1 (len - 1) in
        ok_result (Identifier (IdentifierTokenSpecial content))
      (* 检查是否为有效的简单标识符 *)
      else if is_valid_identifier text then
        ok_result (Identifier (SimpleIdentifier text))
      else
        error_result (InvalidTokenFormat ("identifier", text, 
...
let validate_identifier_name name =
  if String.length name = 0 then
    error_result (InvalidTokenFormat ("identifier", name, 
      { line = 0; column = 0; offset = 0 }))
  else if is_valid_identifier name then
    ok_result name
  else
    error_result (InvalidTokenFormat ("identifier", name, 
      { line = 0; column = 0; offset = 0 }))
...
let create_identifier_token identifier_type name =
  ...
  | Ok validated_name ->
    (match identifier_type with
     ...
     | `Invalid -> error_result (InvalidTokenFormat ("identifier_type", name, 
         { line = 0; column = 0; offset = 0 })))
...
let contains_chinese_chars text =
  let rec check i =
    if i >= String.length text then false
      ...
      let c = String.get text i in
      if Char.code c >= 128 then true  (* 简单的多字节字符检测 *)
      else check (i + 1)
  in
  check 0
...
let get_identifier_stats token_list =
  let count_by_type = List.fold_left (fun (simple, quoted, special) token ->
    match token with
