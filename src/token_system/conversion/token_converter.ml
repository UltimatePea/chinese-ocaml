...
module ConverterRegistry = struct
  ...
  let get_converters_by_type conv_type =
    List.filter (fun (module C : CONVERTER) -> 
      C.converter_type = conv_type
    ) !converters
  
  (** 查找能处理字符串的转换器 *)
  let find_converter_for_string text =
    List.find_opt (fun (module C : CONVERTER) ->
      C.can_handle_string text
    ) !converters
  
  (** 查找能处理Token的转换器 *)
  let find_converter_for_token token =
    List.find_opt (fun (module C : CONVERTER) ->
      C.can_handle_token token
    ) !converters
...
module UnifiedConverter = struct
  ...
  let convert_token_to_string ?(config = default_config) token =
    ...
    | Some (module C) ->
      C.token_to_string config token
    | None ->
      error_result (ConversionError ("Token", "string"))
  ...
  let create_keyword_converter keyword_map =
    let module KeywordConv = struct
      ...
      let can_handle_string text =
        List.exists (fun (k, _) -> k = text) keyword_map
      
      let can_handle_token token =
        List.exists (fun (_, t) -> t = token) keyword_map
      
      let supported_tokens () =
        List.map snd keyword_map
    end in
  ...
  let create_literal_converter () =
    let module LiteralConv = struct
          ...
          with Failure _ ->
            ...
            | _ ->
              ...
              if String.length text >= 2 && 
                ...
                error_result (InvalidTokenFormat ("literal", text, 
      ...
      let can_handle_string text =
        ...
        with Failure _ ->
          try ignore (float_of_string text); true
          with Failure _ ->
