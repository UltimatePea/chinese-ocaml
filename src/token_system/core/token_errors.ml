...
let error_to_string = function
  ...
  | ConversionError (from_type, to_type) ->
    Printf.sprintf "Token转换错误: 无法从%s转换为%s" from_type to_type
  | RegistryError msg ->
    Printf.sprintf "Token注册表错误: %s" msg
  | TokenMismatch (expected, actual, pos) ->
    let expected_str = match Token_registry.get_token_text expected with
      | Some text -> text
      | None -> "未知Token"
    in
    let actual_str = match Token_registry.get_token_text actual with  
      | Some text -> text
      | None -> "未知Token"
    in
    Printf.sprintf "Token不匹配: 期望'%s'但得到'%s' 在第%d行第%d列" 
      expected_str actual_str pos.line pos.column
  | EmptyTokenStream ->
    "Token流为空"
  | InvalidPosition pos ->
    Printf.sprintf "无效位置: 第%d行第%d列(偏移%d)" pos.line pos.column pos.offset
  | ParsingError (msg, pos) ->
    Printf.sprintf "解析错误: %s 在第%d行第%d列" msg pos.line pos.column
  | TokenizationError (msg, line, col) ->
    Printf.sprintf "Token化错误: %s 在第%d行第%d列" msg line col
...
let collect_error collector error context =
  ...
  let should_stop = 
    collector.stop_on_fatal && severity = Fatal ||
    List.length new_errors >= collector.max_errors
  in
...
(** 检查是否有致命错误 *)
let has_fatal_errors collector =
  List.exists (fun err -> err.severity = Fatal) collector.errors

(** 格式化错误报告 *)
let format_error_report collector =
  let errors = get_all_errors collector in
  let format_error err =
    let severity_str = match err.severity with
      | Warning -> "警告"
      | Error -> "错误"
      | Fatal -> "致命错误"
    in
    let context_str = match err.context.function_name with
      | Some func -> Printf.sprintf " [%s]" func
      | None -> ""
    in
...
(** 创建带函数名的错误上下文 *)
let context_with_function func_name = {
  default_context with function_name = Some func_name
}

(** 创建带源文件的错误上下文 *)
let context_with_file file_name = {
  default_context with source_file = Some file_name  
}
...
module SafeOps = struct
  ...
  let safe_lookup_token text =
    ...
    | Token_registry.Ambiguous _tokens -> 
      error_result (RegistryError ("多个Token匹配: " ^ text))
  ...
  let safe_create_position line column offset =
    if line >= 1 && column >= 1 && offset >= 0 then
      ok_result { line; column; offset }
    else
      error_result (InvalidPosition { line; column; offset })
  ...
  let safe_process_token_stream stream f =
    ...
    | tokens -> 
      ...
      with exn -> error_result (ParsingError (Printexc.to_string exn, { line = 0; column = 0; offset = 0 }))
