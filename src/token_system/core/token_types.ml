...
let get_token_category = function
  ...
  | CoreLanguage _ | Semantic _ | ErrorHandling _ 
  | ModuleSystem _ | MacroSystem _ | Wenyan _ 
  | Ancient _ | NaturalLanguage _ -> CategoryKeyword
...
let get_operator_precedence = function
  ...
  | Operator GreaterThan | Operator GreaterThanOrEqual -> 4
...
let get_operator_associativity = function
  ...
  | Operator (Equal | NotEqual | LessThan | LessThanOrEqual 
              | GreaterThan | GreaterThanOrEqual) -> NonAssoc
  | _ -> LeftAssoc