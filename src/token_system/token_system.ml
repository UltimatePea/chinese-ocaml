...
module SystemInfo = struct
  ...
  let generate_status_report () =
    ...
    let converter_lines = List.map (fun (name, count) ->
  ...
  let run_self_check () =
    ...
    if !errors = [] then
      Errors.ok_result "系统自检通过"
    else
      Errors.error_result (Errors.RegistryError (String.concat "; " !errors))
...
module LookupAPI = struct
  ...
  (** 查找Token *)
  let find_token text =
    Registry.lookup_token_by_text text
  
  (** 获取Token文本 *)
  let get_text token =
    Registry.get_token_text token
  
  (** 检查Token是否已注册 *)
  let is_registered token =
    Registry.is_registered token
  
  (** 按类别获取Token *)
  let get_by_category category =
    Registry.get_tokens_by_category category
...
module DebugAPI = struct
  ...
  let print_tokens tokens =
    Utils.Debug.print_token_list tokens
  
  (** 打印Token流 *)
  let print_token_stream stream =
    Utils.Debug.print_positioned_token_list stream
  
  (** 生成Token统计 *)
  let generate_stats tokens =
    Utils.Statistics.generate_comprehensive_stats tokens
  
  (** 验证Token流语法 *)
  let validate_syntax stream =
    Utils.Validation.validate_basic_syntax stream
...
module BatchAPI = struct
  ...
  let process_token_file _filename =
    try
      let content = Errors.SafeOps.safe_process_token_stream [] (fun _ ->
        (* 这里应该读取文件并解析Token *)
        []
      ) in
      content
    with exn ->
      Errors.error_result (Errors.ParsingError (Printexc.to_string exn, 
        { line = 0; column = 0; offset = 0 }))
  
  (** 批量转换多个文本 *)
  let batch_convert_texts texts =
    ConversionAPI.strings_to_tokens texts
