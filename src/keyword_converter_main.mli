(** 骆言词法分析器 - 关键字转换主协调器接口
    ...
    本模块是关键字转换系统的主入口点，协调各个专门的关键字转换器， 提供统一的关键字转换接口。支持基础关键字、系统关键字、中文风格关键字和特殊类型关键字。

    @author 骆言团队
    @since 2025-07-20 *)

val convert_core_keywords :
...
val convert_system_keywords :
  ...
  | `ExpandKeyword ] ->
  Lexer_tokens.token Compiler_errors_types.error_result
(** 转换系统关键字 处理系统级关键字，包括异常处理、模块系统和宏系统相关关键字

    @param pos 位置信息，用于错误报告
    @param keyword 要转换的系统关键字多态变体
    @return 转换结果，成功时返回对应Token，失败时返回错误 *)

val convert_chinese_style_keywords :
  Compiler_errors_types.position ->
  [> `HaveKeyword
  | `OneKeyword
  | `NameKeyword
  ...
  | `TopicMarker ] ->
  Lexer_tokens.token Compiler_errors_types.error_result
(** 转换中文风格关键字 处理文言文风格、古文和自然语言风格的中文关键字

    @param pos 位置信息，用于错误报告
    @param keyword 要转换的中文风格关键字多态变体
    @return 转换结果，成功时返回对应Token，失败时返回错误 *)

val convert_special_type_keywords :
  Compiler_errors_types.position ->
  [> `IntTypeKeyword
  | `FloatTypeKeyword
  | `StringTypeKeyword
  ...
  Lexer_tokens.token Compiler_errors_types.error_result
(** 转换特殊类型关键字 处理类型关键字、古典诗词关键字和特殊标识符
...
val variant_to_token :
  ...
  | `IdentifierTokenSpecial ] ->
  Lexer_tokens.token Compiler_errors_types.error_result
(** 将多态变体转换为token类型 - 主入口函数 尝试使用各种关键字转换器来转换给定的多态变体为Token类型
