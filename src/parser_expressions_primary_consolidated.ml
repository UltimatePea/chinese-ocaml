...
(** 解析模块表达式 *)
let parse_module_expr state = Parser_expressions_utils.parse_module_expr state
...
let rec parse_primary_expr parse_expr parse_array_expr parse_record_expr state =
  ...
  | _ when is_identifier_token token -> parse_identifier_expr_safe parse_expr token state
...
let parse_function_call_or_variable name state =
  let next_token, _ = current_token state in
  if Parser_expressions_utils.is_argument_token next_token then
    let args, final_state = parse_function_arguments parse_basic_argument_expr state in
    (FunCallExpr (VarExpr name, args), final_state)
