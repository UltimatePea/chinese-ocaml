(** 骆言内置函数错误处理模块 - Chinese Programming Language Builtin Functions Error Handling
    ...
    使用新的通用模式消除代码重复，提高可维护性和一致性。 迁移前: 4095字节，多个重复的let绑定和错误处理模式 迁移后: 使用Common_patterns模块的统一工具函数
...
let check_args_count expected_count actual_count function_name =
  ...
  if actual_count <> expected_count then
    runtime_error
      (format_contextual_error context
...
let expect_builtin_function value function_name =
  let context = make_builtin_error_context ~function_name () in
  match
    validate_with_context (with_function_name validate_builtin_function function_name) context value
  with
  | Ok result -> result
  | Error msg -> runtime_error (format_contextual_error context msg)
...
let expect_string_or_list value function_name =
  let context = make_builtin_error_context ~function_name () in
  match
    validate_with_context (with_function_name validate_string_or_list function_name) context value
  with
  | Ok result -> result
  | Error msg -> runtime_error (format_contextual_error context msg)
...
let expect_nonempty_list value function_name =
  ...
  match
    safe_operation
      ...
      (fun () -> with_function_name validate_nonempty_list function_name value)
  with
  | Ok result -> result
  | Error msg -> runtime_error (format_contextual_error context msg)
...
let handle_file_error operation filename f =
  ...
  match safe_operation ~error_handler:(fun _ -> file_operation_error operation filename) f with
  | Ok result -> result
  | Error msg -> runtime_error (format_contextual_error context msg)
...
let expect_non_negative value function_name =
  ...
  match
    safe_operation
      ...
      (fun () -> with_function_name validate_non_negative function_name value)
  with
  | Ok result -> result
  | Error msg -> runtime_error (format_contextual_error context msg)
