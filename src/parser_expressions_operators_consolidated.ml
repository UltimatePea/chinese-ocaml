...
let rec parse_postfix_expression parse_expression expr state =
  let token, _ = current_token state in
  match token with
  (* 函数调用 *)
  | LeftParen | ChineseLeftParen ->
      let state1 = advance_parser state in
      let args, state2 = parse_argument_list parse_expression [] state1 in
...
let create_operator_precedence_chain parse_primary_expr =
  ...
  and parse_arithmetic_expr state = parse_arithmetic_expression parse_multiplicative_expr state
  and parse_multiplicative_expr state = parse_multiplicative_expression parse_unary_expr state
  and parse_unary_expr state = parse_unary_expression parse_unary_expr parse_postfix_expr state
