(** 骆言语法分析器表达式解析主模块 - 整合版
    ...
    本模块是表达式解析的新统一入口，整合了原有的25+个细分模块为3个逻辑清晰的模块：
    1. parser_expressions_primary_consolidated.ml - 基础表达式（字面量、标识符、关键字等）
    2. parser_expressions_operators_consolidated.ml - 运算符表达式（算术、逻辑、比较等）
    3. parser_expressions_structured_consolidated.ml - 结构化表达式（数组、记录、函数等）
...
and parse_primary_expression state = 
  (get_primary_expression_parser ()) state

(** ==================== 后缀表达式解析 ==================== *)

(** 解析后缀表达式 *)
and parse_postfix_expression expr state =
  Primary.parse_postfix_expr parse_expression expr state
...
and parse_match_expression state =
  Structured.parse_match_expression parse_expression state

(** 解析函数表达式 *)
and parse_function_expression state =
  Structured.parse_function_expression parse_expression state

(** 解析let表达式 *)
and parse_let_expression state =
  Structured.parse_let_expression parse_expression state

(** 解析数组表达式 *)
and parse_array_expression state =
  Structured.parse_array_expression parse_expression state

(** 解析记录表达式 *)
and parse_record_expression state =
  Structured.parse_record_expression parse_expression state

(** 解析try表达式 *)
and parse_try_expression state =
  Structured.parse_try_expression parse_expression state

(** 解析raise表达式 *)
and parse_raise_expression state =
  Structured.parse_raise_expression parse_expression state

(** 解析ref表达式 *)
and parse_ref_expression state =
  Structured.parse_ref_expression parse_expression state

(** 解析组合表达式 *)
and parse_combine_expression state =
  Structured.parse_combine_expression parse_expression state
...
and parse_record_updates state =
  Structured.parse_record_updates parse_expression state
