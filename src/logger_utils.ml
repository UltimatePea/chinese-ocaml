 ...
 * 使用方法：
 * 1. 对于需要所有日志级别的模块：
 *    let log_debug, log_info, log_warn, log_error = Logger_utils.init_all_loggers "ModuleName"
 * 
 * 2. 对于只需要特定日志级别的模块：
 *    let log_info, log_error = Logger_utils.init_info_error_loggers "ModuleName"
 * 
 * 3. 对于只需要调试和错误的模块：
 *    let log_debug, log_error = Logger_utils.init_debug_error_loggers "ModuleName"
 * 
 * 4. 对于只需要信息级别的模块：
 *    let log_info = Logger_utils.init_info_logger "ModuleName"
 *)
...
let smart_init module_name =
  ...
  | "ErrorRecovery" -> let _, info, error = init_debug_info_error_loggers module_name in (info, error)
