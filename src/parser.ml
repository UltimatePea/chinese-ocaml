...
and parse_pattern state =
  ...
  | OrKeyword | NotKeyword | ModuleKeyword | NumberKeyword | ValueKeyword | IdentifierToken _ ->
      ...
      let rec parse_constructor_args args state =
        let token, _ = current_token state in
        match token with
        | Arrow | ChineseArrow | Pipe | ChinesePipe | RightBracket | ChineseRightBracket
        | RightParen | ChineseRightParen | Comma | ChineseComma | AncientThenKeyword ->
            (List.rev args, state)
        | _ ->
      ...
      if args = [] then (VarPattern name, state2) else (ConstructorPattern (name, args), state2)
      ...
      | Comma | ChineseComma -> (
          ...
          | _ ->
              ...
              let rec parse_remaining_patterns patterns state =
                ...
                | RightBracket | ChineseRightBracket -> (pattern :: patterns, advance_parser state1)
                | _ -> raise (SyntaxError ("期望逗号或右方括号", snd (current_token state1)))
              in
