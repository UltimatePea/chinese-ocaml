...
let format_enhanced_error enhanced_error =
  ...
  let context_info = Printf.sprintf
    "\n[上下文] 文件: %s | 模块: %s | 函数: %s | 时间: %.0f"
    enhanced_error.context.source_file
    enhanced_error.context.module_name  
    enhanced_error.context.function_name
    enhanced_error.context.timestamp in
  
  let recovery_info = match enhanced_error.recovery_strategy with
    | SkipAndContinue -> "\n[恢复] 跳过此错误，继续处理"
    | SyncToNextStatement -> "\n[恢复] 同步到下一语句边界"
...
let get_error_report () =
  ...
  Printf.sprintf 
    ...
    global_stats.errors
    global_stats.fatal_errors
    global_stats.recovered_errors
    elapsed_time
...
let is_fatal enhanced_error =
  enhanced_error.base_error.severity = Fatal
...
let log_error_to_file enhanced_error =
  ...
  if runtime_cfg.verbose_logging then
    try
      ...
      let log_filename = Printf.sprintf "%s/error_%04d%02d%02d.log" 
        (Config.get_compiler_config ()).temp_directory
