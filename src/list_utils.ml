...
module Safe = struct
  (** 安全的列表头部获取 *)
  let head = function [] -> None | h :: _ -> Some h

  (** 安全的列表尾部获取 *)
  let tail = function [] -> None | _ :: t -> Some t

  (** 安全的列表元素访问 *)
  let rec nth lst n =
    if n < 0 then None
    else match lst with [] -> None | h :: t -> if n = 0 then Some h else nth t (n - 1)
...
module Transform = struct
  ...
  let mapi_safe f lst =
    let rec aux acc i = function
      | [] -> List.rev acc
      | h :: t -> (
          match f i h with None -> aux acc (i + 1) t | Some v -> aux (v :: acc) (i + 1) t)
    in
    aux [] 0 lst
  ...
  let filter_map f lst =
    let rec aux acc = function
      | [] -> List.rev acc
      | h :: t -> ( match f h with None -> aux acc t | Some v -> aux (v :: acc) t)
    in
    aux [] lst
...
module Aggregate = struct
  ...
  (** 求列表最大值 *)
  let max_opt = function [] -> None | h :: t -> Some (List.fold_left max h t)

  (** 求列表最小值 *)
  let min_opt = function [] -> None | h :: t -> Some (List.fold_left min h t)
  ...
  (** 计算列表中元素出现次数 *)
  let count_occurrences x lst = List.fold_left (fun acc y -> if x = y then acc + 1 else acc) 0 lst
...
module Group = struct
  ...
  let unique lst =
    let rec aux acc seen = function
      | [] -> List.rev acc
      | h :: t -> if List.mem h seen then aux acc seen t else aux (h :: acc) (h :: seen) t
    in
    aux [] [] lst
  ...
  let unique_by key_func lst =
    let rec aux acc seen = function
      ...
      | h :: t ->
          let key = key_func h in
          if List.mem key seen then aux acc seen t else aux (h :: acc) (key :: seen) t
    in
    aux [] [] lst
...
module Validate = struct
  ...
  let has_duplicates lst =
    let rec aux seen = function [] -> false | h :: t -> List.mem h seen || aux (h :: seen) t in
    aux [] lst

  (** 检查两个列表是否有相同元素 *)
  let has_intersection lst1 lst2 = List.exists (fun x -> List.mem x lst2) lst1
end
