(** 骆言内置函数错误处理模块接口 - Chinese Programming Language Builtin Functions Error Handling Module Interface
    ...
    这个模块提供了骆言编程语言内置函数的统一错误处理机制，包括参数 验证、类型检查、边界检查等安全性保障功能。错误处理是语言运行时 安全性的基础，确保所有内置函数调用的正确性和可靠性。
...
(** 处理高阶函数错误
    ...
    @raise RuntimeError 总是抛出不支持用户定义函数的错误
