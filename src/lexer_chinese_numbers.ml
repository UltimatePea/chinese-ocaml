  ...
  let digit_table =
    lazy
      (let table = Hashtbl.create 16 in
       List.iter
         (fun (char, digit) -> Hashtbl.replace table char digit)
         [
           ("一", 1);
           ("二", 2);
           ("三", 3);
  ...
  let unit_table =
    lazy
      (let table = Hashtbl.create 8 in
       List.iter
         (fun (char, unit) -> Hashtbl.replace table char unit)
         [ ("十", 10); ("百", 100); ("千", 1000); ("万", 10000); ("亿", 100000000) ];
       table)

  (* 优化版本：O(1)查找数字字符 *)
  let char_to_digit ch = try Hashtbl.find (Lazy.force digit_table) ch with Not_found -> 0

  (* 优化版本：O(1)查找单位字符 *)
  let char_to_unit ch = try Hashtbl.find (Lazy.force unit_table) ch with Not_found -> 1
