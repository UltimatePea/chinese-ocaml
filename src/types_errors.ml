...
(** 遗留异常定义 - 现在作为现代 CompilerError 系统的包装器 这些异常现在内部使用 Error_compatibility 模块，提供向后兼容性 同时将错误统一到现代
    CompilerError 系统中 *)
...
let type_mismatch_error expected actual =
  ...
  let suggestions =
    Error_compatibility.suggest_type_fix ~expected:(string_of_typ expected)
      ~actual:(string_of_typ actual)
  in
...
let unification_error typ1 typ2 =
  let msg =
    Base_formatter.concat_strings [ "无法合一类型 "; string_of_typ typ1; " 和 "; string_of_typ typ2 ]
  in
  TypeError msg
...
let occurs_check_error var_name typ =
  let msg =
    Base_formatter.concat_strings [ "循环类型检查失败: "; var_name; " 出现在 "; string_of_typ typ; " 中" ]
  in
  TypeError msg
...
let unsupported_operation_error op_name typ =
  let msg = Base_formatter.concat_strings [ "类型 "; string_of_typ typ; " 不支持操作 "; op_name ] in
  TypeError msg

(** 字段不存在错误 - 使用Base_formatter消除Printf.sprintf *)
let field_not_found_error field_name typ =
  let msg = Base_formatter.concat_strings [ "类型 "; string_of_typ typ; " 中不存在字段 "; field_name ] in
  TypeError msg
...
let overload_resolution_error func_name candidates =
  ...
  let msg =
    Base_formatter.concat_strings [ "函数 "; func_name; " 的重载解析失败, 候选类型: "; candidates_str ]
  in
  TypeError msg
...
let format_error_message = function
  ...
  | ParseError (msg, line, col) ->
      Base_formatter.concat_strings
        [
          "解析错误 (行:";
          Base_formatter.int_to_string line;
          ", 列:";
          Base_formatter.int_to_string col;
          "): ";
          msg;
        ]
  | CodegenError (msg, context) -> Base_formatter.concat_strings [ "代码生成错误 ["; context; "]: "; msg ]
  | SemanticError (msg, context) -> Base_formatter.concat_strings [ "语义错误 ["; context; "]: "; msg ]
