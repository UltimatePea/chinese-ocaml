...
let type_mismatch_error expected actual =
  let msg =
    Printf.sprintf "类型不匹配: 期望 %s, 实际 %s" (string_of_typ expected)
...
let undefined_var_error var_name =
  let msg = Printf.sprintf "未定义的变量: %s" var_name in
  TypeError msg

(** 重复定义错误 *)
let duplicate_definition_error name =
  let msg = Printf.sprintf "重复定义: %s" name in
  TypeError msg

(** 类型推断失败错误 *)
let type_inference_error expr_desc =
  let msg = Printf.sprintf "类型推断失败: %s" expr_desc in
  TypeError msg

(** 类型合一失败错误 *)
let unification_error typ1 typ2 =
  let msg =
    Printf.sprintf "无法合一类型 %s 和 %s" (string_of_typ typ1) (string_of_typ typ2)
  in
  TypeError msg
...
let arity_mismatch_error expected actual =
  let msg = Printf.sprintf "参数数量不匹配: 期望 %d, 实际 %d" expected actual in
  TypeError msg
...
let overload_resolution_error func_name candidates =
  ...
  let msg =
    Printf.sprintf "函数 %s 的重载解析失败, 候选类型: %s" func_name
...
let safe_execute f x default =
  ...
  | exn ->
      Unified_logger.error "TypesErrors"
        (Printf.sprintf "意外错误: %s" (Printexc.to_string exn));
      default
