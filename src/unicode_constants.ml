(** 骆言编译器Unicode字符处理常量模块 从constants.ml重构而来，使用数据结构化方法管理UTF-8字符 *)
...
let char_definitions =
  [
    ...
    (* 全角符号 *)
    {
      name = "fullwidth_left_paren";
      char = "（";
      triple = { byte1 = 0xEF; byte2 = 0xBC; byte3 = 0x88 };
      category = "fullwidth";
    };
    {
      name = "fullwidth_right_paren";
      char = "）";
...
module Legacy = struct
  ...
  let is_char_category char category =
    List.exists (fun def -> def.char = char && def.category = category) char_definitions
end
