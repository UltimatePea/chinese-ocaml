...
module TokenClassifier = struct
  ...
  let is_literal_token = function
    | IntToken _ | FloatToken _ | ChineseNumberToken _ 
    | StringToken _ | BoolToken _ -> true
    | _ -> false
  ...
  let is_basic_keyword_token = function
    ...
    | MacroKeyword | ExpandKeyword -> true
    | _ -> false
  ...
  let is_type_keyword_token = function
    ...
    | ArrayTypeKeyword | VariantKeyword | TagKeyword -> true
    | _ -> false
end
