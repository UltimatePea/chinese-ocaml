(** 骆言编译器核心格式化模块
    ...
    本模块包含通用格式化工具和基础功能，从unified_formatter.ml中拆分出来。
    提供统一的格式化接口，消除Printf.sprintf依赖。
...
module General = struct
  ...
  (** 扩展通用格式化 *)
  let format_range start_val end_val = concat_strings [ int_to_string start_val; ".."; int_to_string end_val ]
...
module TypeFormatter = struct
  ...
  (** 扩展类型格式化 *)
  let format_option_type inner_type = 
    concat_strings [ inner_type; " option" ]
...
module ReportFormatting = struct
  ...
  let format_metrics_table metrics =
    let header = "指标\t\t值" in
    let separator = "----\t\t----" in
    let formatted_metrics = List.map (fun (name, value) ->
      concat_strings [ name; "\t\t"; value ]) metrics in
...
module ExtendedFormatting = struct
  ...
  let format_environment_var name value sensitive =
    if sensitive then
      concat_strings [ name; " = ***（敏感信息已隐藏）" ]
    else
      concat_strings [ name; " = "; value ]
