...
let rec skip_whitespace_and_comments state =
  ...
  | Some c when is_whitespace c ->
      skip_whitespace_and_comments (advance state)
...
let read_string_literal state =
  ...
  let rec read_chars current_state =
    ...
    | None ->
        let pos = { line = current_state.current_line; column = current_state.current_column; filename = current_state.filename } in
        raise (LexError (Constants.ErrorMessages.unterminated_string, pos))
    | Some c when Char.code c = 0xE3 && ChinesePunctuation.is_string_end current_state.input current_state.position ->
        (* 找到』- 字符串结束 *)
...
let read_quoted_identifier state =
  ...
  let rec read_chars current_state =
    ...
    | Some c when Char.code c = 0xE3 && ChinesePunctuation.is_right_quote current_state.input current_state.position ->
        (* 找到」- 标识符结束 *)
        let final_state = advance (advance (advance current_state)) in
    ...
    | Some c ->
        let pos = { line = current_state.current_line; column = current_state.current_column; filename = current_state.filename } in
        raise (LexError (Constants.ErrorMessages.invalid_char_in_quoted_identifier ^ " '" ^ String.make 1 c ^ "'", pos))
  in
  read_chars state
...
let read_number state =
  ...
  let rec read_digits current_state has_dot =
    ...
    | _ ->
        ...
        let token = 
          if has_dot then
            LiteralToken (Literals.FloatToken (float_of_string number_str))
          else
            LiteralToken (Literals.IntToken (int_of_string number_str))
        in
        (token, current_state)
  in
...
let read_chinese_number state =
  ...
  let rec read_chinese_digits current_state =
    ...
    | Some (char, next_pos) when is_chinese_digit_char char ->
        ...
        read_chinese_digits { current_state with position = next_pos; current_column = current_state.current_column + 1 }
    | _ ->
...
let recognize_chinese_punctuation state pos =
  if state.position + 2 >= state.length then None
    ...
    let advance_3 = { state with position = state.position + 3; current_column = state.current_column + 1 } in
...
let next_token state =
  ...
  | _ ->
      (* 首先尝试识别中文标点符号 *)
      begin match recognize_chinese_punctuation state pos with
      | Some result -> result
      | None ->
          (* 尝试关键字匹配 *)
          let keyword_state = { 
          ...
          begin match Keyword_matcher.match_keyword keyword_state with
          | Some (keyword_token, keyword_len) ->
              let new_state = { state with position = state.position + keyword_len; current_column = state.current_column + keyword_len } in
              (KeywordToken keyword_token, pos, new_state)
          | None ->
              (* 处理其他token *)
              begin match current_char state with
              | Some c when Char.code c = 0xE3 && ChinesePunctuation.is_string_start state.input state.position ->
                  (* 『 - 字符串开始 *)
                  ...
                  (token, pos, new_state)
              | Some c when Char.code c = 0xE3 && ChinesePunctuation.is_left_quote state.input state.position ->
                  (* 「 - 引用标识符开始 *)
                  let skip_state = { state with position = state.position + 3; current_column = state.current_column + 1 } in
              ...
              | Some c when is_chinese_char c ->
                  (* 检查是否为中文数字 *)
                  begin match next_utf8_char state.input state.position with
              ...
              | Some c ->
                  (* 禁用的ASCII符号 *)
                  raise (LexError (Constants.ErrorMessages.ascii_symbols_disabled ^ " '" ^ String.make 1 c ^ "'", pos))
              | None ->
                  (* 这种情况应该不会发生，但为了完整性处理 *)
