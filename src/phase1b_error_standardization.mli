(** Phase 1-B 错误处理标准化模块接口
    ...
    Author: Whisky, PR Worker 定义统一的错误处理API，供整个编译器系统使用 *)
...
val safe_compiler_operation :
  (unit -> 'a) -> (string * int * int) option -> ('a, standard_error) result
...
val handle_error_with_default :
  default:'a -> error_handler:(standard_error -> unit) -> ('a, standard_error) result -> 'a
(** 默认错误处理 *)
