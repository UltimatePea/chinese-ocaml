...
(** 创建错误信息 *)
let make_error_info ?(severity=(Error : error_severity)) ?(context=None) ?(suggestions=[]) error =
...
let format_error_message error =
  ...
  | UnimplementedFeature (feature, context) ->
    Printf.sprintf "未实现功能 [%s]: %s" context feature
  | InternalError msg ->
    Printf.sprintf "内部错误: %s" msg
  | RuntimeError msg ->
    Printf.sprintf "运行时错误: %s" msg

(** 格式化完整错误信息 *)
let format_error_info info =
  let severity_str = match info.severity with
    | Warning -> "警告"
    | Error -> "错误"
    | Fatal -> "严重错误"
  in
  let main_msg = Printf.sprintf "[%s] %s" severity_str (format_error_message info.error) in
  let context_msg = match info.context with
...
let unimplemented_feature ?(suggestions=[]) ?(context="C代码生成") feature =
  ...
  let error_info = make_error_info ~suggestions:all_suggestions (UnimplementedFeature (feature, context)) in
  (Error error_info : 'a error_result)

let internal_error ?(suggestions=[]) msg =
  let default_suggestions = [
    "这是编译器内部错误，请报告此问题";
    "请在GitHub上创建Issue并包含重现步骤"
  ] in
  let all_suggestions = suggestions @ default_suggestions in
...
type error_collector = {
  mutable errors: error_info list;
...
let add_error collector error_info =
  ...
  if error_info.severity = Fatal then
...
let set_error_config new_config =
  error_config := new_config

(** 检查是否应该继续处理 *)
let should_continue collector =
  !error_config.continue_on_error && 
  not collector.has_fatal &&
