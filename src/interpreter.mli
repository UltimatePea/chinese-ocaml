(** 骆言解释器模块接口 - Chinese Programming Language Interpreter Module Interface
    ...
    这个模块是骆言编程语言的核心解释器模块，提供语言的执行环境和程序解释功能。
    它负责管理全局环境、执行程序语句、处理宏展开和模块系统。
...
(** 全局宏表，存储用户定义的宏定义
    ...
    该表用于在程序执行过程中查找和展开宏调用。
    宏表在程序初始化时创建，并在遇到宏定义时更新。 *)

val module_table : (string, (string * runtime_value) list) Hashtbl.t
(** 全局模块表，存储已定义的模块及其导出符号
    
    键：模块名称（字符串）
    值：模块导出的符号列表，每个符号包含名称和运行时值
    
    模块表支持骆言的模块系统，允许代码组织和命名空间管理。
    模块在首次引入时加载，并缓存在此表中以供后续引用。 *)
...
(** 全局递归函数表，存储递归函数定义
    ...
    该表专门用于处理递归函数的定义和调用，确保递归函数可以正确引用自身。
    递归函数在定义时会被预先注册到此表中，以支持递归调用。 *)

val functor_table : (string, identifier * module_type * expr) Hashtbl.t
(** 全局函子表，存储函子（参数化模块）定义
    
    键：函子名称（字符串）
    值：元组包含 (参数标识符, 参数模块类型, 函子体表达式)
    
    函子是骆言高级模块系统的一部分，允许定义参数化的模块。
    函子在定义时注册到此表中，在应用时根据实际参数进行实例化。 *)
