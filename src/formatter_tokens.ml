(** 骆言编译器Token和位置格式化模块
    ...
    本模块专注于Token格式化和位置信息处理，从unified_formatter.ml中拆分出来。 提供统一的Token格式化接口，消除Printf.sprintf依赖。
...
module TokenFormatting = struct
  ...
  let format_quoted_identifier_token name =
    concat_strings [ "QuotedIdentifierToken(\""; name; "\")" ]
  ...
  (** Token位置信息结合格式化 *)
  let format_token_with_position token line col = token_position_pattern token line col
  ...
  let format_token_stream tokens_with_positions =
    let formatted =
      List.map
        (fun (token, line, col) ->
          concat_strings [ int_to_string line; ":"; int_to_string col; " "; token ])
...
module EnhancedPosition = struct
  ...
  (** 与现有格式兼容的包装函数 *)
  let format_position_enhanced filename line column = file_position_pattern filename line column
  ...
  let format_relative_position base_line base_col target_line target_col =
    ...
    if line_diff = 0 then concat_strings [ "同行偏移 "; int_to_string col_diff; " 列" ]
  ...
  let format_multiline_error filename start_line end_line source_lines error_msg =
    let header =
      concat_strings [ filename; ":"; int_to_string start_line; "-"; int_to_string end_line ]
    in
    let numbered_lines =
      List.mapi
        (fun i line ->
          let line_num = start_line + i in
          concat_strings [ int_to_string line_num; " | "; line ])
        source_lines
    in
...
module TokenUtilities = struct
  ...
  let is_literal_token token_type =
    List.mem token_type
      [ "IntToken"; "FloatToken"; "StringToken"; "BooleanToken"; "ChineseNumberToken" ]
  ...
  let format_token_statistics total_tokens literal_count identifier_count keyword_count =
    concat_strings
      [
        "Token统计: 总计 ";
        int_to_string total_tokens;
        " (字面量: ";
        int_to_string literal_count;
        ", 标识符: ";
        int_to_string identifier_count;
        ", 关键字: ";
        int_to_string keyword_count;
        ")";
      ]

  (** Token转换助手 *)
  let token_to_string token_type token_value = concat_strings [ token_type; "("; token_value; ")" ]
  ...
  let format_tokenization_progress processed_chars total_chars current_token =
    concat_strings
      [
        "词法分析进度: ";
        int_to_string processed_chars;
        "/";
        int_to_string total_chars;
        " (";
        int_to_string (processed_chars * 100 / max total_chars 1);
        "%), 当前Token: ";
        current_token;
      ]
end
