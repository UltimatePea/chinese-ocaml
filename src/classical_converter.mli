...
(** 文言文令牌转换函数
    ...
    专门用于转换文言文语法构造的令牌。实际上是 {!convert_classical_token} 的别名，提供更明确的语义接口。
...
(** 自然语言令牌转换函数
    ...
    专门用于转换自然语言构造的令牌。实际上是 {!convert_classical_token} 的别名，用于处理更接近现代中文的语法结构。
...
(** 古代语言令牌转换函数
    ...
    专门用于转换古代语言构造的令牌。实际上是 {!convert_classical_token} 的别名，用于处理更古老的中文语法形式。
...
(** {1 使用说明}
    ...
    此模块是统一转换系统的兼容性层，为古典语言转换提供简化的接口。 所有转换功能都通过底层的统一转换系统实现，确保一致性和性能。
    ...
    {[
      try
        ...
        (* 处理转换结果 *)
        converted
      with Classical_converter.Unknown_classical_token msg ->
        (* 处理转换错误 *)
        failwith ("转换失败: " ^ msg)
    ]}
