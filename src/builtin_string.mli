(** 骆言内置字符串处理函数模块接口 - Chinese Programming Language Builtin String Functions Module Interface
    ...
    这个模块提供了骆言编程语言的字符串处理相关内置函数，包括字符串的
    连接、分割、搜索、匹配、变换等基础操作功能。字符串处理是现代编程语言
    的核心功能之一，为文本处理和数据操作提供强大支持。
...
(** 字符串连接函数（柯里化）
    ...
    这是一个柯里化函数，支持部分应用：
    1. 第一次调用传入第一个字符串，返回一个函数
    2. 返回的函数接受第二个字符串，返回连接后的结果
