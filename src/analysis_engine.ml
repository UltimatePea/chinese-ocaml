...
let analyze_expression expr context =
  ...
  let rec analyze expr ctx =
    ...
    | FunCallExpr (func, args) ->
        analyze_function_call_expression func args new_ctx analyze suggestions
    ...
    | UnaryOpExpr (op, expr) ->
        analyze_unary_operation_expression (UnaryOpExpr (op, expr)) expr new_ctx analyze suggestions
    | _ -> ()
  in
...
let analyze_statement stmt context =
  ...
  | RecLetStmt (name, expr) ->
      ...
      let complexity_suggestion =
        match
          Refactoring_analyzer_complexity.analyze_recursive_function_complexity name expr
            new_context
        with
        | Some s -> [ s ]
        | None -> []
